...
It is not installed with Tvheadend — run it straight from a source checkout
...
`--csv`           | one machine-readable row, for logging the trend over time, with the per-day cached% and first-seen% of `--coverage`
