...
int regex_compile(tvh_regex_t* regex, const char* re_str, int flags, int subsys) {
  ...
  if (flags & TVHREGEX_POSIX) {
    ...
    if (regex->re_code == NULL) {
      ...
      regex->re_extra = pcre_study(regex->re_code, PCRE_STUDY_JIT_COMPILE, &estr);
    ...
    regex->re_code = pcre2_compile((PCRE2_SPTR8)re_str, -1, options, &ecode, &eoff, NULL);
    if (regex->re_code == NULL) {
...
int regex_match_substring(tvh_regex_t* regex, unsigned number, char* buf, size_t size_buf) {
  ...
  if (regex->is_posix) {
    ...
    return pcre_copy_substring(regex->re_text,
               regex->re_match,
               (regex->re_matches == 0) ? TVHREGEX_MAX_MATCHES : regex->re_matches,
               number,
               buf,
               size_buf) < 0;
#elif ENABLE_PCRE2
