...
int tvh_pipe(int flags, th_pipe_t* p) {
  int fd[2], err;
  tvh_mutex_lock(&fork_lock);
  err = pipe(fd);
  if (err != -1) {
...
void tvh_pipe_close(th_pipe_t* p) {
  close(p->rd);
  close(p->wr);
  p->rd = -1;
  p->wr = -1;
...
static int qsort_r_wrap(const void* a, const void* b) {
...
int regex_compile(tvh_regex_t* regex,
    const char*                re_str,
    int                        flags,
    int                        subsys) {
#if ENABLE_PCRE || ENABLE_PCRE2
  ...
  if (flags & TVHREGEX_POSIX) {
    ...
    if (regex->re_code == NULL) {
      ...
      regex->re_extra =
          pcre_study(regex->re_code, PCRE_STUDY_JIT_COMPILE, &estr);
    ...
    regex->re_code =
        pcre2_compile((PCRE2_SPTR8)re_str, -1, options, &ecode, &eoff, NULL);
    if (regex->re_code == NULL) {
      ...
      if (re_str[0] &&
          pcre2_jit_compile(regex->re_code, PCRE2_JIT_COMPLETE) >= 0) {
        jsz = 0;
        if (pcre2_pattern_info(regex->re_code, PCRE2_INFO_JITSIZE, &jsz) >= 0 &&
            jsz > 0) {
          regex->re_jit_stack =
              pcre2_jit_stack_create(32 * 1024, 512 * 1024, NULL);
          if (regex->re_jit_stack)
            pcre2_jit_stack_assign(regex->re_mcontext,
                NULL,
                regex->re_jit_stack);
        }
      }
...
int regex_match(tvh_regex_t* regex, const char* str) {
  ...
  if (regex->is_posix) {
    ...
    return regexec(&regex->re_posix_code,
        str,
...
#if ENABLE_PCRE
    regex->re_text    = str;
    regex->re_matches = pcre_exec(regex->re_code,
        regex->re_extra,
        str,
        strlen(str),
        0,
        0,
        regex->re_match,
        TVHREGEX_MAX_MATCHES * 3);
    return regex->re_matches < 0;
#elif ENABLE_PCRE2
    return pcre2_match(regex->re_code,
               (PCRE2_SPTR8)str,
               -1,
               0,
               0,
...
int regex_match_substring(tvh_regex_t* regex,
    unsigned                           number,
    char*                              buf,
    size_t                             size_buf) {
  assert(buf);
  ...
  if (regex->is_posix) {
    ...
    if (regex->re_posix_match[number].rm_so == -1)
      return -1;
    ssize_t size = regex->re_posix_match[number].rm_eo -
        regex->re_posix_match[number].rm_so;
    if (size < 0 || size > (size_buf - 1))
      return -1;
    strlcpy(buf,
        regex->re_posix_text + regex->re_posix_match[number].rm_so,
        size + 1);
    return 0;
#if ENABLE_PCRE || ENABLE_PCRE2
  } else {
#if ENABLE_PCRE
    return pcre_copy_substring(regex->re_text,
               regex->re_match,
               (regex->re_matches == 0) ? TVHREGEX_MAX_MATCHES
                                        : regex->re_matches,
               number,
               buf,
               size_buf) < 0;
    ...
    return pcre2_substring_copy_bynumber(regex->re_match,
        number,
        (PCRE2_UCHAR8*)buf,
        &psiz);
#endif
...
int regex_match_substring_length(tvh_regex_t* regex, unsigned number) {
  ...
  if (regex->is_posix) {
    ...
    if (regex->re_posix_match[number].rm_so == -1)
      return -1;
    return regex->re_posix_match[number].rm_eo -
        regex->re_posix_match[number].rm_so;
#if ENABLE_PCRE || ENABLE_PCRE2
  } else {
#if ENABLE_PCRE
...
void __attribute__((no_sanitize_thread)) *
    blacklisted_memcpy(void* dest, const void* src, size_t n) {
  uint8_t*       d = dest;
  const uint8_t* s = src;
  while (n-- > 0)
    *d++ = *s++;
