 ...
 * Packet log
 */
void timeshift_packet_log0(const char* source, timeshift_t* ts, streaming_message_t* sm) {
  th_pkt_t* pkt = sm->sm_data;
  tvhtrace(LS_TIMESHIFT,
      "ts %d pkt %s - stream %d type %c pts %10" PRId64 " dts %10" PRId64
      " dur %10d len %6zu time %14" PRId64,
      ts->id,
      source,
 ...
 * Changed settings
 */
static void timeshift_conf_class_changed(idnode_t* self) {
  timeshift_fixup();
}
 ...
 * Save settings
 */
static htsmsg_t* timeshift_conf_class_save(idnode_t* self, char* filename, size_t fsize) {
 ...
 * Class
 */
static const void* timeshift_conf_class_max_size_get(void* o) {
  static uint64_t r;
  r = timeshift_conf.max_size / 1048576LL;
  return &r;
}

static int timeshift_conf_class_max_size_set(void* o, const void* v) {
...
static const void* timeshift_conf_class_ram_size_get(void* o) {
  static uint64_t r;
  r = timeshift_conf.ram_size / 1048576LL;
  return &r;
}

static int timeshift_conf_class_ram_size_set(void* o, const void* v) {
  uint64_t u64                    = *(uint64_t*)v * 1048576LL;
...
const idclass_t timeshift_conf_class = {.ic_snode = &timeshift_conf.idnode,
    .ic_class                                     = "timeshift",
...
static int timeshift_packet(timeshift_t* ts, streaming_message_t* sm) {
  th_pkt_t* pkt = sm->sm_data;
  int64_t   time;
...
static htsmsg_t* timeshift_input_info(void* opaque, htsmsg_t* list) {
  htsmsg_add_str(list, NULL, "wtimeshift input");
  return list;
}

static streaming_ops_t timeshift_input_ops = {.st_cb = timeshift_input,
