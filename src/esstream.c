 ...
 *
 */
static void elementary_stream_make_nicename(elementary_stream_t* st,
    const char*                                                  nicename) {
  char buf[256];
  if (st->es_pid != -1)
    snprintf(buf,
 ...
 *
 */
void elementary_set_init(elementary_set_t* set,
    int                                    subsys,
    const char*                            nicename,
    service_t*                             t) {
 ...
 *
 */
void elementary_set_update_nicename(elementary_set_t* set,
    const char*                                       nicename) {
  elementary_stream_t* st;
 ...
 *
 */
static void elementary_stream_init(elementary_stream_t* es) {
  es->es_cc = -1;
}
 ...
 *
 */
static void elementary_stream_clean(elementary_stream_t* es) {
...
static void elementary_set_filter_build_add(elementary_set_t* set,
    elementary_stream_t*                                      st,
    elementary_stream_t**                                     sta,
    int*                                                      p) {
  /* only once */
 ...
 *
 */
static void elementary_set_filter_print(elementary_set_t* set) {
  elementary_stream_t* st;
  caid_t*              ca;
  ...
  TAILQ_FOREACH (st, &set->set_filter, es_filter_link) {
    if (LIST_EMPTY(&st->es_caids)) {
      tvhtrace(set->set_subsys,
          "esfilter: \"%s\" %03d %05d %s %s",
          set->set_nicename,
          st->es_index,
          st->es_pid,
          streaming_component_type2txt(st->es_type),
    ...
    TAILQ_FOREACH (esf, &esfilters[i], esf_link) {
      ...
      TAILQ_FOREACH (st, &set->set_all, es_link) {
        ...
        if (esf->esf_type && (esf->esf_type & SCT_MASK(st->es_type)) == 0)
          continue;
        if (esf->esf_language[0] && strncmp(esf->esf_language, st->es_lang, 4))
          continue;
        ...
        if (i == ESF_CLASS_CA) {
          ...
          if ((esf->esf_caid != (uint16_t)-1 || esf->esf_caprovider != -1)) {
            LIST_FOREACH (ca, &st->es_caids, link) {
              if (esf->esf_caid != (uint16_t)-1 && ca->caid != esf->esf_caid)
                continue;
              if (esf->esf_caprovider != (uint32_t)-1 &&
                  ca->providerid != esf->esf_caprovider)
                continue;
              break;
            }
            if (ca == NULL)
          ...
          if (esf->esf_log)
            tvhinfo(LS_SERVICE,
                "esfilter: \"%s\" %s %03d %03d %05d %04x %06x %s",
                t->s_nicename,
                esfilter_class2txt(i),
                st->es_index,
                esf->esf_index,
                st->es_pid,
                esf->esf_caid,
          ...
          if (esf->esf_log)
            tvhinfo(LS_SERVICE,
                "esfilter: \"%s\" %s %03d %03d %05d %s %s %s",
                t->s_nicename,
                esfilter_class2txt(i),
                st->es_index,
                esf->esf_index,
 ...
 *
 */
elementary_stream_t* elementary_stream_type_modify(elementary_set_t* set,
    int                                                              pid,
    streaming_component_type_t                                       type) {
 ...
 *
 */
void elementary_stream_type_destroy(elementary_set_t* set,
    streaming_component_type_t                        type) {
  elementary_stream_t* es = elementary_stream_type_find(set, type);
  if (es)
 ...
 *
 */
static int escmp(const void* A, const void* B) {
...
streaming_start_t* elementary_stream_build_start(elementary_set_t* set) {
  ...
  ss = calloc(1,
      sizeof(streaming_start_t) + sizeof(streaming_start_component_t) * n);
