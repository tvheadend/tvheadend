 ...
 *
 */
static void elementary_stream_make_nicename(elementary_stream_t* st, const char* nicename) {
  char buf[256];
  if (st->es_pid != -1)
    snprintf(buf,
        sizeof(buf),
    ...
    snprintf(buf, sizeof(buf), "%s: %s", nicename, streaming_component_type2txt(st->es_type));
...
int elementary_set_has_streams(elementary_set_t* set, int filtered) {
  return filtered ? TAILQ_FIRST(&set->set_filter) != NULL : TAILQ_FIRST(&set->set_all) != NULL;
}
