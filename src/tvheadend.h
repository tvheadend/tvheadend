...
void GTIMER_FCN(mtimer_arm_rel)(GTIMER_TRACEID_ mtimer_t* mti,
...
#define gtimer_arm_absn(a, b, c, d) \
...
int      tvh_gzip_deflate_fd(int fd,
         const uint8_t*          data,
         size_t                  orig,
         size_t*                 size,
         int                     speed);
...
static inline int32_t deltaI32(int32_t a, int32_t b) {
  return (a > b) ? (a - b) : (b - a);
}
static inline uint32_t deltaU32(uint32_t a, uint32_t b) {
  return (a > b) ? (a - b) : (b - a);
}

#define SKEL_DECLARE(name, type) type* name;
#define SKEL_ALLOC(name)               \
  do {                                 \
    if (!name)                         \
      name = calloc(1, sizeof(*name)); \
  } while (0)
#define SKEL_USED(name) \
  do {                  \
    name = NULL;        \
  } while (0)
#define SKEL_FREE(name) \
  do {                  \
    free(name);         \
    name = NULL;        \
  } while (0)
