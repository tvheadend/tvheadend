...
static cmdline_opt_t*
cmdline_opt_find(cmdline_opt_t* opts, int num, const char* arg) {
  int i;
  int isshort = 0;
...
int main(int argc, char** argv) {
  ...
  int opt_help = 0, opt_version = 0, opt_fork = 0, opt_firstrun = 0,
      opt_stderr = 0, opt_nostderr = 0, opt_syslog = 0, opt_nosyslog = 0,
      opt_uidebug = 0, opt_abort = 0, opt_noacl = 0, opt_fileline = 0,
      opt_threadid = 0, opt_libav = 0, opt_ipv6 = 0, opt_nosatipcli = 0,
      opt_satip_rtsp = 0,
#if ENABLE_TSFILE
      opt_tsfile_tuner = 0,
#endif
      opt_dump = 0, opt_xspf = 0, opt_dbus = 0, opt_dbus_session = 0,
      opt_nobackup = 0, opt_nobat = 0, opt_subsystems = 0, opt_tprofile = 0,
      opt_thread_debug   = 0;
  const char *opt_config = NULL, *opt_user = NULL, *opt_group = NULL,
             *opt_logpath = NULL, *opt_log_debug = NULL, *opt_log_trace = NULL,
             *opt_pidpath = "/run/tvheadend.pid",
#if ENABLE_LINUXDVB
             *opt_dvb_adapters = NULL,
#endif
             *opt_bindaddr = NULL, *opt_subscribe = NULL,
             *opt_user_agent = NULL, *opt_satip_bindaddr = NULL;
  ...
  cmdline_opt_t cmdline_opts[] = {
    ...
    {'B',
        "nobackup",
        N_("Don't backup configuration tree at upgrade"),
        OPT_BOOL,
        &opt_nobackup},
    ...
    {0,
        "satip_xml",
        N_("URL with the SAT>IP server XML location"),
        OPT_STR_LIST,
        &opt_satip_xml},
#endif
    ...
    {0,
        "http_port",
        N_("Specify alternative http port"),
        OPT_INT,
        &tvheadend_webui_port},
    {0,
        "http_root",
        N_("Specify alternative http webroot"),
        OPT_STR,
        &tvheadend_webroot},
    {0,
        "htsp_port",
        N_("Specify alternative htsp port"),
        OPT_INT,
        &tvheadend_htsp_port},
    {0,
        "htsp_port2",
        N_("Specify extra htsp port"),
        OPT_INT,
    ...
    {'S',
        "nosyslog",
        N_("Disable syslog (all messages)"),
        OPT_BOOL,
        &opt_nosyslog},
    ...
    {0,
        "fileline",
        N_("Add file and line numbers to debug"),
        OPT_BOOL,
        &opt_fileline},
    ...
    {0,
        "uidebug",
        N_("Enable web UI debug (non-minified JS)"),
        OPT_BOOL,
        &opt_uidebug},
    ...
    {'j',
        "join",
        N_("Subscribe to a service permanently"),
        OPT_STR,
        &opt_subscribe},
    ...
    {0,
        "tprofile",
        N_("Gather timing statistics for the code"),
        OPT_BOOL,
        &opt_tprofile},
#if ENABLE_TRACE
  ...
  if (opt_nostderr)
    log_options &=
        ~(TVHLOG_OPT_DECORATE | TVHLOG_OPT_STDERR | TVHLOG_OPT_DBG_STDERR);
  if (opt_nosyslog)
  ...
  if (opt_fork && !opt_user && !opt_config)
    tvhwarn(LS_START,
        "Forking without --user or --config may use unexpected configuration "
        "location");
    ...
    if (pw != NULL) {
      if (getuid() != pw->pw_uid) {
        ...
        if (gnum > 0 && setgroups(gnum, glist)) {
          ...
          tvhlog(LOG_ALERT,
              LS_START,
              "setgroups(%s) failed, do you have permission?",
              buf + 1);
          return 1;
        }
      }
  ...
  if (gid != -1 && (getgid() != gid) && setgid(gid)) {
    tvhlog(LOG_ALERT,
        LS_START,
        "setgid(%d) failed, do you have permission?",
        gid);
    return 1;
  }
  if (uid != -1 && (getuid() != uid) && setuid(uid)) {
    tvhlog(LOG_ALERT,
        LS_START,
        "setuid(%d) failed, do you have permission?",
        uid);
    return 1;
  }
