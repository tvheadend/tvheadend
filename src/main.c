...
static cmdline_opt_t* cmdline_opt_find(cmdline_opt_t* opts, int num, const char* arg) {
  int i;
  int isshort = 0;
...
static void handle_sigill(int x) {
...
static int get_user_groups(const struct passwd* pw, gid_t* glist, size_t gmax) {
  int num = 0;
#if !ENABLE_ANDROID
  struct group* gr;
  char**        mem;
 ...
 */
void GTIMER_FCN(mtimer_arm_abs)(GTIMER_TRACEID_ mtimer_t* mti,
 ...
 *
 */
void GTIMER_FCN(mtimer_arm_rel)(GTIMER_TRACEID_ mtimer_t* gti,
    mti_callback_t*                                       callback,
    void*                                                 opaque,
    int64_t                                               delta) {
#if ENABLE_GTIMER_CHECK
 ...
 *
 */
static int gtimercmp(gtimer_t* a, gtimer_t* b) {
 ...
 * this routine can be called inside any locks
 */
void GTIMER_FCN(gtimer_arm_absn)(GTIMER_TRACEID_ gtimer_t* gti,
    gti_callback_t*                                        callback,
    void*                                                  opaque,
    time_t                                                 when) {
 ...
 *
 */
void GTIMER_FCN(gtimer_arm_rel)(GTIMER_TRACEID_ gtimer_t* gti,
    gti_callback_t*                                       callback,
    void*                                                 opaque,
    time_t                                                delta) {
#if ENABLE_GTIMER_CHECK
 ...
 *
 */
void tasklet_disarm(tasklet_t* tsk) {
  tvh_mutex_lock(&tasklet_lock);
 ...
 *
 */
static void* tasklet_thread(void* aux) {
  tasklet_t*      tsk;
 ...
 *
 */
static void show_usage(const char* argv0, cmdline_opt_t* opts, int num, const char* err, ...) {
  int  i;
  char buf[256];
 ...
 */
static void show_subsystems(const char* argv0) {
  int              i;
 ...
 *
 */
static inline time_t gdispatch_clock_update(void) {
 ...
 *
 */
static int64_t mdispatch_clock_update(void) {
 ...
 *
 */
static void* mtimer_tick_thread(void* aux) {
  while (tvheadend_is_running()) {
 ...
 *
 */
static void* mtimer_thread(void* aux) {
  mtimer_t*       mti;
  mti_callback_t* cb;
  int64_t         now, next;
  const char*     id;
...
int main(int argc, char** argv) {
  ...
  int opt_help = 0, opt_version = 0, opt_fork = 0, opt_firstrun = 0, opt_stderr = 0,
      opt_nostderr = 0, opt_syslog = 0, opt_nosyslog = 0, opt_uidebug = 0, opt_abort = 0,
      opt_noacl = 0, opt_fileline = 0, opt_threadid = 0, opt_libav = 0, opt_ipv6 = 0,
      opt_nosatipcli = 0, opt_satip_rtsp = 0,
#if ENABLE_TSFILE
      opt_tsfile_tuner = 0,
#endif
      opt_dump = 0, opt_xspf = 0, opt_dbus = 0, opt_dbus_session = 0, opt_nobackup = 0,
      opt_nobat = 0, opt_subsystems = 0, opt_tprofile = 0, opt_thread_debug = 0;
  const char *opt_config = NULL, *opt_user = NULL, *opt_group = NULL, *opt_logpath = NULL,
             *opt_log_debug = NULL, *opt_log_trace = NULL, *opt_pidpath = "/run/tvheadend.pid",
#if ENABLE_LINUXDVB
             *opt_dvb_adapters = NULL,
#endif
             *opt_bindaddr = NULL, *opt_subscribe = NULL, *opt_user_agent = NULL,
  ...
  cmdline_opt_t cmdline_opts[] = {
      ...
      {'C',
          "firstrun",
          N_("If no user account exists then create one with\n"
          ...
          &opt_firstrun},
#if ENABLE_DBUS_1
      {'U', "dbus", N_("Enable DBus"), OPT_BOOL, &opt_dbus},
      {'e',
          "dbus_session",
          N_("DBus - use the session message bus instead of the system one"),
          OPT_BOOL,
          &opt_dbus_session},
#endif
#if ENABLE_LINUXDVB
      {'a',
          "adapters",
          N_("Only use specified DVB adapters (comma-separated, -1 = none)"),
          OPT_STR,
          &opt_dvb_adapters},
#endif
#if ENABLE_SATIP_SERVER
      {0,
          "satip_bindaddr",
          N_("Specify bind address for SAT>IP server"),
          OPT_STR,
          &opt_satip_bindaddr},
      {0,
          "satip_rtsp",
          N_("SAT>IP RTSP port number for server\n"
      ...
      {0,
          "nosatip",
          N_("Disable SAT>IP client (deprecated flag, use nosatipcli)"),
          OPT_BOOL,
          &opt_nosatipcli},
      {0, "nosatipcli", N_("Disable SAT>IP client"), OPT_BOOL, &opt_nosatipcli},
      {0, "satip_xml", N_("URL with the SAT>IP server XML location"), OPT_STR_LIST, &opt_satip_xml},
#endif
      ...
      {0, "http_port", N_("Specify alternative http port"), OPT_INT, &tvheadend_webui_port},
      {0, "http_root", N_("Specify alternative http webroot"), OPT_STR, &tvheadend_webroot},
      {0, "htsp_port", N_("Specify alternative htsp port"), OPT_INT, &tvheadend_htsp_port},
      {0, "htsp_port2", N_("Specify extra htsp port"), OPT_INT, &tvheadend_htsp_port_extra},
      {0,
          "useragent",
          N_("Specify User-Agent header for the http client"),
          OPT_STR,
          &opt_user_agent},
      {0, "xspf", N_("Use XSPF playlist instead of M3U"), OPT_BOOL, &opt_xspf},
      ...
      {0, "noacl", N_("Disable all access control checks"), OPT_BOOL, &opt_noacl},
      {0, "nobat", N_("Disable DVB bouquets"), OPT_BOOL, &opt_nobat},
      {'j', "join", N_("Subscribe to a service permanently"), OPT_STR, &opt_subscribe},
  ...
  for (i = 1; i < argc; i++) {
    ...
    /* Find option */
    cmdline_opt_t* opt = cmdline_opt_find(cmdline_opts, ARRAY_SIZE(cmdline_opts), argv[i]);
    if (!opt) {
      show_usage(argv[0],
          cmdline_opts,
          ARRAY_SIZE(cmdline_opts),
          _("invalid option specified [%s]"),
          argv[i]);
      continue;
    }
  ...
  if (opt_fork && !opt_user && !opt_config)
    tvhwarn(LS_START,
        "Forking without --user or --config may use unexpected configuration location");
    ...
    if (pw != NULL) {
      if (getuid() != pw->pw_uid) {
        ...
        if (gnum > 0 && setgroups(gnum, glist)) {
          ...
          int  i;
          for (i = 0; i < gnum; i++)
            snprintf(buf + strlen(buf), sizeof(buf) - 1 - strlen(buf), ",%d", glist[i]);
          tvhlog(LOG_ALERT, LS_START, "setgroups(%s) failed, do you have permission?", buf + 1);
          return 1;
        }
      }
      uid     = pw->pw_uid;
  ...
  if (gid != -1 && (getgid() != gid) && setgid(gid)) {
    tvhlog(LOG_ALERT, LS_START, "setgid(%d) failed, do you have permission?", gid);
    return 1;
  }
  if (uid != -1 && (getuid() != uid) && setuid(uid)) {
    tvhlog(LOG_ALERT, LS_START, "setuid(%d) failed, do you have permission?", uid);
    return 1;
  }
