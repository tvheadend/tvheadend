...
#define SPECIAL_PROP(idval, getfcn) \
  { .type = PT_STR, .id = idval, .name = "", .get = getfcn, .opts = PO_RDONLY | PO_NOUI }
...
static wizard_page_t* page_init(const char* name, const char* class_name, const char* caption) {
...
wizard_page_t* wizard_login(const char* lang) {
  ...
  static const property_t       props[]  = {
      ...
      NEXT_BUTTON(network),
      {}};
  wizard_page_t*  page = page_init("login", "wizard_login", N_("Access Control"));
  idclass_t*      ic   = (idclass_t*)page->idnode.in_class;
  wizard_login_t* w;
  access_entry_t* ae;
  passwd_entry_t* pw;
  ...
  TAILQ_FOREACH (pw, &passwd_entries, pw_link) {
    if (!pw->pw_wizard || !pw->pw_username)
      continue;
    if (w->admin_username[0] && strcmp(w->admin_username, pw->pw_username) == 0) {
...
#define NETWORK(num)                                                                               \
  ...
  {                                                                                                \
    .type = PT_STR, .id = "network" STRINGIFY(num), .name = N_("Network type"),                    \
    .desc = N_("Select an available network type for this tuner."), .get = network_get_value##num, \
    .set = network_set_value##num, .list = network_get_list##num, .group = num,                    \
  }
...
#endif
#if ENABLE_IPTV
    if (idnode_is_instance(&mn->mn_id, &iptv_auto_network_class) && w->iptv_url[idx][0]) {
...
#define MUXES(num)                                                                         \
  ...
  {                                                                                        \
    .type = PT_STR, .id = "muxes" STRINGIFY(num), .name = N_("Pre-defined muxes"),         \
    .get = muxes_get_value##num, .set = muxes_set_value##num, .list = muxes_get_list##num, \
    .group = num,                                                                          \
  }
