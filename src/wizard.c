...
#define SPECIAL_PROP(idval, getfcn)                         \
  {                                                         \
    .type = PT_STR, .id = idval, .name = "", .get = getfcn, \
...
static wizard_page_t*
page_init(const char* name, const char* class_name, const char* caption) {
...
wizard_page_t* wizard_hello(const char* lang) {
  ...
  static const property_t       props[]  = {
      ...
      NEXT_BUTTON(login),
      {}};
  wizard_page_t*  page = page_init("hello", "wizard_hello", N_("Welcome"));
  ...
  for (idx = 0; idx < 3 && f != NULL; idx++) {
    ...
    switch (idx) {
      case 0:
        strlcpy(w->epg_lang1, s, sizeof(w->epg_lang1));
        break;
      case 1:
        strlcpy(w->epg_lang2, s, sizeof(w->epg_lang2));
        break;
      case 2:
        strlcpy(w->epg_lang3, s, sizeof(w->epg_lang3));
        break;
    }
    f = HTSMSG_NEXT(f);
  }
...
wizard_page_t* wizard_login(const char* lang) {
  ...
  static const property_t       props[]  = {
      ...
      {}};
  wizard_page_t* page =
      page_init("login", "wizard_login", N_("Access Control"));
  idclass_t*      ic = (idclass_t*)page->idnode.in_class;
  wizard_login_t* w;
  access_entry_t* ae;
  passwd_entry_t* pw;
  ...
  TAILQ_FOREACH (ae, &access_entries, ae_link) {
    ...
    if (ae->ae_admin) {
      ...
      snprintf(w->admin_username,
          sizeof(w->admin_username),
          "%s",
          ae->ae_username);
      snprintf(w->network,
          sizeof(w->network),
          "%s",
          htsmsg_get_str(c, "prefix") ?: "");
      htsmsg_destroy(c);
    } else {
  ...
  TAILQ_FOREACH (pw, &passwd_entries, pw_link) {
    ...
    if (w->admin_username[0] &&
      ...
      snprintf(w->admin_password,
          sizeof(w->admin_password),
          "%s",
          pw->pw_password);
...
#define NETWORK_GROUP(num) \
  { .name = N_("Network " STRINGIFY(num)), .number = num, }
...
#define NETWORK(num)                                                \
  ...
  {                                                                 \
    .type = PT_STR, .id = "network" STRINGIFY(num),                 \
    .name = N_("Network type"),                                     \
    .desc = N_("Select an available network type for this tuner."), \
    .get = network_get_value##num, .set = network_set_value##num,   \
    .list = network_get_list##num, .group = num,                    \
  }
...
#define NETWORK_FCN(num)                                                \
  ...
  static int network_set_tidvalue##num(void* o, const void* v) {        \
    wizard_page_t*    p = o;                                            \
    wizard_network_t* w = p->aux;                                       \
    snprintf(w->tunerid[num - 1],                                       \
        sizeof(w->tunerid[num - 1]),                                    \
        "%s",                                                           \
        (const char*)v);                                                \
    return 1;                                                           \
  }                                                                     \
  ...
  static int network_set_value##num(void* o, const void* v) {           \
    wizard_page_t*    p = o;                                            \
    wizard_network_t* w = p->aux;                                       \
    snprintf(w->network_type[num - 1],                                  \
        sizeof(w->network_type[num - 1]),                               \
        "%s",                                                           \
        (const char*)v);                                                \
    return 1;                                                           \
  }                                                                     \
...
wizard_page_t* wizard_network(const char* lang) {
  static const property_group_t groups[] = {NETWORK_GROUP(1),
      NETWORK_GROUP(2),
      NETWORK_GROUP(3),
      NETWORK_GROUP(4),
      NETWORK_GROUP(5),
  ...
  wizard_page_t* page =
      page_init("network", "wizard_network", N_("Tuner and Network"));
  idclass_t*        ic = (idclass_t*)page->idnode.in_class;
  wizard_network_t* w;
  mpegts_network_t* mn;
  tvh_input_t*      ti;
  ...
  for (ti = LIST_LAST(tvh_input_t, &tvh_inputs, ti_link); ti;
    ...
    if (name) {
      ...
      if (mn) {
        snprintf(w->network_type[nidx],
            sizeof(w->network_type[nidx]),
            "%s",
            mn->mn_id.in_class->ic_class);
      }
      w->network_types[nidx] =
          htsmsg_copy(htsmsg_get_list(m, "mpegts_network_types"));
...
static const void* muxes_progress_get(void* o) {
  ...
  snprintf(prop_sbuf,
      PROP_SBUF_LEN,
      "%s",
      tvh_gettext_lang(w->lang, N_("Progress")));
  return &prop_sbuf_ptr;
}
...
#define MUXES(num)                                                          \
  ...
  {                                                                         \
    .type = PT_STR, .id = "muxes" STRINGIFY(num),                           \
    .name = N_("Pre-defined muxes"), .get = muxes_get_value##num,           \
    .set = muxes_set_value##num, .list = muxes_get_list##num, .group = num, \
  }
...
#define MUXES_IPTV(num)                                                       \
  ...
  {                                                                           \
    .type = PT_STR, .id = "muxes" STRINGIFY(num), .name = N_("URL"),          \
    .desc = N_("URL of the M3U playlist."), .get = muxes_get_iptv_value##num, \
    .set = muxes_set_iptv_value##num, .group = num,                           \
  }
...
#define MUXES_FCN(num)                                               \
  ...
  static int muxes_set_idvalue##num(void* o, const void* v) {        \
    wizard_page_t*  p = o;                                           \
    wizard_muxes_t* w = p->aux;                                      \
    snprintf(w->networkid[num - 1],                                  \
        sizeof(w->networkid[num - 1]),                               \
        "%s",                                                        \
        (const char*)v);                                             \
    return 1;                                                        \
  }
...
#define MUXES_FCN_DVB(num)                                             \
  ...
  static int muxes_set_value##num(void* o, const void* v) {            \
    wizard_page_t*  p = o;                                             \
    wizard_muxes_t* w = p->aux;                                        \
    snprintf(w->muxes[num - 1],                                        \
        sizeof(w->muxes[num - 1]),                                     \
        "%s",                                                          \
        (const char*)v);                                               \
    return 1;                                                          \
  }                                                                    \
...
#define MUXES_IPTV_FCN(num)                                         \
  ...
  static int muxes_set_iptv_value##num(void* o, const void* v) {    \
    wizard_page_t*  p = o;                                          \
    wizard_muxes_t* w = p->aux;                                     \
    snprintf(w->iptv_url[num - 1],                                  \
        sizeof(w->iptv_url[num - 1]),                               \
        "%s",                                                       \
        (const char*)v);                                            \
    return 1;                                                       \
  }
...
wizard_page_t* wizard_muxes(const char* lang) {
  ...
  wizard_page_t* page =
      page_init("muxes", "wizard_muxes", N_("Predefined Muxes"));
  idclass_t*        ic = (idclass_t*)page->idnode.in_class;
  wizard_muxes_t*   w;
  mpegts_network_t* mn;
  int               idx, midx = 0;
...
#if ENABLE_IPTV
      if (idnode_is_instance(&mn->mn_id, &iptv_auto_network_class)) {
        snprintf(w->iptv_url[midx],
            sizeof(w->iptv_url[midx]),
            "%s",
            ((iptv_network_t*)mn)->in_url ?: "");
...
wizard_page_t* wizard_mapping(const char* lang) {
  static const property_t props[] = {
      ...
      {}};
  wizard_page_t* page =
      page_init("mapping", "wizard_mapping", N_("Service Mapping"));
  idclass_t*        ic = (idclass_t*)page->idnode.in_class;
  wizard_mapping_t* w;
  ic->ic_properties = props;
...
wizard_page_t* wizard_channels(const char* lang) {
  static const property_t props[]  = {ICON(),
      DESCRIPTION(channels),
      PREV_BUTTON(mapping),
      LAST_BUTTON(),
      {}};
  static const property_t props2[] = {ICON(),
      DESCRIPTION(channels2),
      PREV_BUTTON(mapping),
      LAST_BUTTON(),
      {}};
  wizard_page_t*          page =
      page_init("channels", "wizard_channels", N_("Finished"));
  idclass_t*      ic = (idclass_t*)page->idnode.in_class;
  access_entry_t* ae;
