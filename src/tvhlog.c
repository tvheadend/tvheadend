...
tvhlog_subsys_t tvhlog_subsystems[] = {
    ...
    [LS_TBL_BASE]        = {"tbl-base",
        N_("Base DVB SI Tables (PAT,CAT,PMT,SDT etc.)")},
...
static void tvhlog_get_subsys(bitops_ulong_t* ss, char* subsys, size_t len) {
  size_t        c     = 0;
...
/* Set subsys */
static void tvhlog_set_subsys(bitops_ulong_t* c, const char* subsys) {
  uint_fast32_t a, i;
  char *        s, *t, *r = NULL;
...
static void
tvhlog_process(tvhlog_msg_t* msg, int options, FILE** fp, const char* path) {
  int       s;
  size_t    l;
  char      buf[2048], t[128];
  struct tm tm;
...
/* Log */
static void* tvhlog_thread(void* p) {
  int           options;
...
static const void* tvhlog_class_path_get(void* o) {
  return &tvhlog_path;
}

static int tvhlog_class_path_set(void* o, const void* v) {
  const char* s = v;
...
static const void* tvhlog_class_debugsubs_get(void* o) {
  static char* p = prop_sbuf;
  tvhlog_get_debug(prop_sbuf, PROP_SBUF_LEN);
  return &p;
}

static int tvhlog_class_debugsubs_set(void* o, const void* v) {
  tvhlog_set_debug((const char*)v);
  return 1;
}

static const void* tvhlog_class_tracesubs_get(void* o) {
  static char* p = prop_sbuf;
  tvhlog_get_trace(prop_sbuf, PROP_SBUF_LEN);
  return &p;
}

static int tvhlog_class_tracesubs_set(void* o, const void* v) {
  tvhlog_set_trace((const char*)v);
  return 1;
}

static const void* tvhlog_class_enable_syslog_get(void* o) {
  static int si;
  si = (tvhlog_options & TVHLOG_OPT_SYSLOG) ? 1 : 0;
  return &si;
}

static int tvhlog_class_enable_syslog_set(void* o, const void* v) {
  tvh_mutex_lock(&tvhlog_mutex);
  if (*(int*)v)
...
static const void* tvhlog_class_debug_syslog_get(void* o) {
  static int si;
  si = (tvhlog_options & TVHLOG_OPT_DBG_SYSLOG) ? 1 : 0;
  return &si;
}

static int tvhlog_class_debug_syslog_set(void* o, const void* v) {
  tvh_mutex_lock(&tvhlog_mutex);
  if (*(int*)v)
...
static const void* tvhlog_class_trace_get(void* o) {
  static int si;
  si = atomic_get(&tvhlog_level) >= LOG_TRACE;
  return &si;
}

static int tvhlog_class_trace_set(void* o, const void* v) {
  atomic_set(&tvhlog_level, *(int*)v ? LOG_TRACE : LOG_DEBUG);
  return 1;
}

static const void* tvhlog_class_libav_get(void* o) {
  static int si;
  si = (tvhlog_options & TVHLOG_OPT_LIBAV) ? 1 : 0;
  return &si;
}

static int tvhlog_class_libav_set(void* o, const void* v) {
  tvh_mutex_lock(&tvhlog_mutex);
  if (*(int*)v)
...
idnode_t tvhlog_conf = {.in_class = &tvhlog_conf_class};

CLASS_DOC(debugging)

const idclass_t tvhlog_conf_class = {.ic_snode = &tvhlog_conf,
