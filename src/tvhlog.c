...
tvh_concatenate_subsystem_with_logsv(char* buf, int subsys, const char *fmt, va_list *args)
{
  ...
  char log[SUB_SYSTEM_TRANSCODE_LOG_LENGHT_MAX - SUB_SYSTEM_TRANSCODE_NAME_LENGHT_MAX - SUB_SYSTEM_TRANSCODE_DELIMITER_LENGHT];
  if (args)
  ...
  snprintf(buf, SUB_SYSTEM_TRANSCODE_LOG_LENGHT_MAX, "%s: %s", subsystem, log);
}
