 ...
 *
 */
static int http_resolve(http_connection_t* hc,
    http_path_t*                           _hp,
    char**                                 remainp,
    char**                                 argsp) {
  http_path_t* hp;
  ...
  while (1) {
    ...
    LIST_FOREACH (hp, hc->hc_paths, hp_link) {
      if (!strncmp(path, hp->hp_path, hp->hp_len)) {
        if (path[hp->hp_len] == 0 || path[hp->hp_len] == '/' ||
            path[hp->hp_len] == '?')
          break;
      }
    }
...
static const char* cachemonths[12] = {"Jan",
    "Feb",
    "Mar",
    "Apr",
    "May",
 ...
 *
 */
int http_header_match(http_connection_t* hc,
    const char*                          name,
    const char*                          value) {
  char *tokbuf, *tok, *saveptr = NULL, *q, *s;
...
int http_check_local_ip(http_connection_t* hc) {
  if (hc->hc_local_ip == NULL) {
    ...
    *hc->hc_local_ip = *hc->hc_self;
    hc->hc_is_local_ip =
        ip_check_is_local_address(hc->hc_peer, hc->hc_self, hc->hc_local_ip) >
        0;
  }
...
void http_error(http_connection_t* hc, int error) {
  ...
  if (error != HTTP_STATUS_FOUND && error != HTTP_STATUS_MOVED) {
    ...
    if (error == HTTP_STATUS_UNAUTHORIZED)
      level = LOG_DEBUG;
    else if (error == HTTP_STATUS_BAD_REQUEST ||
        error > HTTP_STATUS_UNAUTHORIZED)
      level = LOG_ERR;
    tvhlog(level,
        hc->hc_subsys,
        "%s: %s %s (%d) %s -- %d",
  ...
  if (hc->hc_version != RTSP_VERSION_1_0) {
    ...
    if (error == HTTP_STATUS_UNAUTHORIZED) {
      ...
      htsbuf_qprintf(&hc->hc_reply,
          "<P STYLE=\"text-align: center; margin: 2em\"><A HREF=\"%s/login\" "
          "STYLE=\"border: 1px solid; border-radius: 4px; padding: "
          ".6em\">%s</A></P>",
          tvheadend_webroot ? tvheadend_webroot : "",
 ...
 *
 */
int http_extra_send(http_connection_t* hc,
    const void*                        data,
    size_t                             data_len,
    int                                may_discard) {
...
static void http_access_verify_ticket(http_connection_t* hc) {
  ...
  tvhinfo(hc->hc_subsys,
      "%s: using ticket %s for %s",
      hc->hc_peer_ipstr,
      ticket_id,
      hc->hc_url);
}
 ...
 *
 */
static int http_verify_prepare(http_connection_t* hc,
  ...
  if (hc->hc_authhdr) {
    ...
    if (tvh_str_default(qop, NULL) == NULL) {
      ...
      if (realm == NULL || strcmp(realm, config.realm) || nonce_count == NULL ||
          cnonce == NULL) {
        goto end;
      } else {
        snprintf(all,
...
int http_access_verify(http_connection_t* hc, int mask) {
  ...
  hc->hc_access =
      access_get(hc->hc_peer, hc->hc_username, http_verify_callback, &v);
  http_verify_free(&v);
  if (hc->hc_access) {
 ...
 * Return non-zero if no access
 */
int http_access_verify_channel(http_connection_t* hc,
    int                                           mask,
    struct channel*                               ch) {
...
int http_noaccess_code(http_connection_t* hc) {
  return strempty(hc->hc_username) ? HTTP_STATUS_UNAUTHORIZED
...
static int http_exec(http_connection_t* hc, http_path_t* hp, char* remain) {
  ...
  if ((hp->hp_accessmask & ACCESS_NO_EMPTY_ARGS) != 0 &&
      TAILQ_EMPTY(&hc->hc_req_args)) {
 ...
 * Add a callback for a given "virtual path" on our HTTP server
 */
http_path_t* http_path_add(const char* path,
    void*                              opaque,
    http_callback_t*                   callback,
    uint32_t                           accessmask) {
 ...
 *
 */
int http_websocket_send(http_connection_t* hc,
    uint8_t*                               buf,
    uint64_t                               buflen,
    int                                    opcode) {
  int      op, r = 0;
...
again:
  ...
  if (op == HTTP_WSOP_TEXT) {
    p[plen] = '\0';
    msg1 =
        p[0] == '{' || p[0] == '[' ? htsmsg_json_deserialize((char*)p) : NULL;
    if (msg1) {
...
void http_serve_requests(http_connection_t* hc) {
  ...
  do {
    ...
    if (config.proxy && strncmp(cmdline, "PROXY ", 6) == 0) {
      tvhtrace(hc->hc_subsys,
          "[PROXY] PROXY protocol detected! cmdline='%s'",
          cmdline);
...
void http_server_init(const char* bindaddr) {
  static tcp_server_ops_t ops = {.start = http_serve,
      .stop                             = NULL,
      .cancel                           = http_cancel};
  RB_INIT(&http_nonces);
  if (tvheadend_webui_port > 0) {
    http_server = tcp_server_create(LS_HTTP,
        "HTTP",
        bindaddr,
