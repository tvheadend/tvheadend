...
static void service_class_get_title(idnode_t* self,
    const char*                               lang,
    char*                                     dst,
    size_t                                    dstsize) {
  snprintf(dst,
...
int service_start(service_t* t,
  ...
  if (stimeout > 0)
    mtimer_arm_rel(&t->s_receive_timer,
...
void service_remove_raw(service_t* t) {
  ...
  mtimer_arm_rel(&service_raw_remove_timer,
      service_remove_raw_timer_cb,
      NULL,
      0);
}
 ...
 *
 */
void service_instance_destroy(service_instance_list_t* sil,
    service_instance_t*                                si) {
  TAILQ_REMOVE(sil, si, si_link);
