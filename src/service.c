...
static void service_class_notify_enabled(void* obj, const char* lang) {
...
static char* service_class_channel_rend(void* obj, const char* lang) {
  service_t* svc = obj;
  return idnode_list_get_csv1(&svc->s_channels, lang);
}

static int service_class_channel_set(void* obj, const void* p) {
  service_t* svc = obj;
  return idnode_list_set1(&svc->s_id,
      &svc->s_channels,
      &channel_class,
      (htsmsg_t*)p,
...
static void service_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
  snprintf(dst, dstsize, "%s", service_get_full_channel_name((service_t*)self) ?: "");
}

static const void* service_class_encrypted_get(void* p) {
  static int t;
  service_t* s = p;
...
static const void* service_class_caid_get(void* obj) {
    ...
    switch (st->es_type) {
      case SCT_CA:
        LIST_FOREACH (c, &st->es_caids, link) {
          l = strlen(buf);
          snprintf(buf + l, sizeof(buf) - l, "%s%04X:%06X", l ? "," : "", c->caid, c->providerid);
        }
        break;
      default:
        break;
...
static htsmsg_t* service_class_auto_list(void* o, const char* lang) {
  static const struct strtab tab[] = {{N_("Auto check enabled"), 0},
      {N_("Auto check disabled"), 1},
      {N_("Missing In PAT/SDT"), 2}};
  return strtab2htsmsg(tab, 1, lang);
}

static htsmsg_t* service_type_auto_list(void* o, const char* lang) {
  static const struct strtab tab[] = {{N_("Override disabled"), ST_UNSET},
      {N_("None"), ST_NONE},
      {N_("Radio"), ST_RADIO},
      {N_("SD TV"), ST_SDTV},
      {N_("HD TV"), ST_HDTV},
...
const idclass_t service_raw_class = {.ic_class = "service_raw",
 ...
 */
void service_remove_subscriber(service_t* t, th_subscription_t* s, int reason) {
 ...
 *
 */
int service_start(service_t* t, int instance, int weight, int flags, int timeout, int postpone) {
  int r, stimeout = 10;
  ...
  if (stimeout > 0)
    mtimer_arm_rel(&t->s_receive_timer, service_data_timeout, t, sec2mono(stimeout));
  return 0;
}
 ...
 */
service_instance_t* service_find_instance(service_t* s,
    channel_t*                                       ch,
    tvh_input_t*                                     ti,
  ...
  if (ch) {
    ...
    LIST_FOREACH (ilm, &ch->ch_services, ilm_in2_link) {
      ...
      if (s->s_is_enabled(s, flags)) {
        if (pro == NULL || pro->pro_svfilter == PROFILE_SVF_NONE ||
    ...
    if (si == NULL && pro && pro->pro_svfilter == PROFILE_SVF_SD) {
      LIST_FOREACH (ilm, &ch->ch_services, ilm_in2_link) {
        s = (service_t*)ilm->ilm_in1;
    ...
    if (si == NULL && pro && pro->pro_svfilter == PROFILE_SVF_UHD) {
      LIST_FOREACH (ilm, &ch->ch_services, ilm_in2_link) {
        s = (service_t*)ilm->ilm_in1;
        if (s->s_is_enabled(s, flags) && service_is_fhdtv(s)) {
          r1 = s->s_enlist(s, ti, sil, flags, weight);
  ...
  TAILQ_FOREACH (si, sil, si_link) {
    ...
    if (!name && s)
      name = s->s_nicename;
    tvhdebug(LS_SERVICE,
        "%d: %s si %p %s weight %d prio %d error %d (%s)",
        si->si_instance,
        name,
        si,
        si->si_source,
...
static void service_remove_raw_timer_cb(void* aux) {
  service_t* t;
  while ((t = TAILQ_FIRST(&service_raw_remove)) != NULL)
    service_destroy(t, 0);
}

void service_remove_raw(service_t* t) {
  if (t == NULL)
    return;
...
static int64_t service_channel_number(service_t* s) {
  return 0;
}

static const char* service_channel_name(service_t* s) {
  return NULL;
}

static const char* service_provider_name(service_t* s) {
  return NULL;
}
 ...
 */
service_t* service_create0(service_t* t,
    int                               service_type,
...
static int service_make_nicename0(service_t* t, char* buf, size_t len, int adapter) {
  ...
  snprintf(buf,
      ...
      si.si_network ?: "",
      si.si_network && si.si_mux ? "/" : "",
      si.si_mux ?: "",
      si.si_mux && service_name ? "/" : "",
      service_name ?: "");
  prefidx = (adapter && si.si_adapter ? strlen(si.si_adapter) : 0) +
      (adapter && si.si_adapter && si.si_network ? 1 : 0) +
      (si.si_network ? strlen(si.si_network) : 0) + (si.si_network && si.si_mux ? 1 : 0) +
...
int service_is_hdtv(const service_t* t) {
  ...
  if (s_type == ST_HDTV)
    ...
    elementary_stream_t* st;
    TAILQ_FOREACH (st, &t->s_components.set_all, es_link)
      if (SCT_ISVIDEO(st->es_type) && st->es_height >= 720 && st->es_height < 1080)
        return 1;
  }
  return 0;
}
 ...
 */
const char* service_servicetype_txt(service_t* s) {
  static const char* types[] = {"HDTV", "SDTV", "Radio", "FHDTV", "UHDTV", "Other"};
  if (service_is_hdtv(s))
    return types[0];
  if (service_is_sdtv(s))
    return types[1];
  if (service_is_radio(s))
    return types[2];
  if (service_is_fhdtv(s))
    return types[3];
 ...
 *
 */
void service_set_streaming_status_flags_(service_t* t, int set) {
  lock_assert(&t->s_stream_mutex);
  ...
  tvhdebug(LS_SERVICE,
      "%s: Status changed to %s%s%s%s%s%s%s%s%s%s",
      service_nicename(t),
 ...
 *
 */
static void service_class_load(struct idnode* self, htsmsg_t* c) {
 ...
 *
 */
static void* service_saver(void* aux) {
  service_t* t;
...
static memoryinfo_t services_memoryinfo = {.my_name = "Services",
 ...
 *
 */
void service_source_info_free(struct source_info* si) {
  free(si->si_adapter);
  free(si->si_network);
 ...
 */
static int si_cmp(const service_instance_t* a, const service_instance_t* b) {
  int r;
 ...
 *
 */
void service_instance_destroy(service_instance_list_t* sil, service_instance_t* si) {
  TAILQ_REMOVE(sil, si, si_link);
...
int64_t service_get_channel_number(service_t* s) {
  if (s->s_channel_number)
    return s->s_channel_number(s);
  return 0;
}
...
const char* service_get_channel_epgid(service_t* s) {
  if (s->s_channel_epgid)
    return s->s_channel_epgid(s);
  return NULL;
}
 ...
 *
 */
static void add_caid(elementary_stream_t* st, uint16_t caid, uint32_t providerid) {
 ...
 *
 */
static void load_legacy_caid(htsmsg_t* c, elementary_stream_t* st) {
  uint32_t    a, b;
  const char* v;
 ...
 *
 */
static void load_caid(htsmsg_t* m, elementary_stream_t* st) {
  htsmsg_field_t* f;
...
void service_load(service_t* t, htsmsg_t* c) {
  ...
  if (!shared_pcr)
    elementary_stream_type_modify(&t->s_components, t->s_components.set_pcr_pid, SCT_PCR);
  else
