...
tda_save(th_dvb_adapter_t *tda)
{
  ...
  htsmsg_add_u32(m, "include_unavailable_services", tda->tda_include_unavailable_services);
 ...
 */
void
dvb_adapter_set_include_unavailable_services(th_dvb_adapter_t *tda, int on)
{
  if(tda->tda_include_unavailable_services == on)
    return;

  lock_assert(&global_lock);

  tvhlog(LOG_NOTICE, "dvb", "Adapter \"%s\" include currently unavailable services when mapping set to: %s",
   tda->tda_displayname, on ? "On" : "Off");

  tda->tda_include_unavailable_services = on;
  tda_save(tda);
}
...
tda_add(int adapter_num)
{
  ...
  tda->tda_idlescan = 1;
  tda->tda_include_unavailable_services = 0;
...
dvb_adapter_init(uint32_t adapter_mask)
{
  ...
  if(l != NULL) {
	...
	if(type != tda->tda_type)
      ...
      htsmsg_get_u32(c, "include_unavailable_services", &tda->tda_include_unavailable_services);
