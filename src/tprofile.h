...
static inline void tprofile_queue_init(qprofile_t* qprof, const char* name) {
  if (tprofile_running)
    tprofile_queue_init1(qprof, name);
}
static inline void tprofile_queue_done(qprofile_t* qprof) {
  if (tprofile_running)
    tprofile_queue_done1(qprof);
}
static inline void
tprofile_queue_set(qprofile_t* qprof, const char* id, uint64_t pos) {
  if (tprofile_running)
    tprofile_queue_set1(qprof, id, pos);
}
static inline void
tprofile_queue_add(qprofile_t* qprof, const char* id, uint64_t pos) {
  if (tprofile_running)
    tprofile_queue_add1(qprof, id, pos);
}
static inline void
tprofile_queue_drop(qprofile_t* qprof, const char* id, uint64_t pos) {
  if (tprofile_running)
    tprofile_queue_drop1(qprof, id, pos);
}
