...
#if ENABLE_EXECINFO
static int addr2lineresolve(const char* binary, intptr_t addr, char* buf0, size_t buflen) {
  char*       buf = buf0;
  int         fd[2], r, f;
  const char* argv[5];
  pid_t       p;
...
#if ENABLE_LIBUNWIND
static void traphandler_libunwind() {
...
static void traphandler(int sig, siginfo_t* si, void* UC) {
  ...
  tvhlog_spawn(LOG_ALERT, LS_CRASH, "Fault address %p (%s)", si->si_addr, reason ?: "N/A");
  ...
  for (i = 0; i < nframes; i++) {
    ...
    if (dladdr(frames[i], &dli)) {
      ...
      if (dli.dli_fname != NULL && dli.dli_fbase != NULL) {
        tvhlog_spawn(LOG_ALERT, LS_CRASH, "%s %p %p", dli.dli_fname, frames[i], dli.dli_fbase);
        continue;
      }
