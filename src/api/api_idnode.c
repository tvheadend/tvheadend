...
static struct strtab filtcmptab[] = {{"gt", IC_GT},
    {"lt", IC_LT},
    {"eq", IC_EQ}};

static void api_idnode_grid_conf(access_t* perm,
    htsmsg_t*                              args,
        ...
        if (f2) {
          ...
          if (f2->hmf_type == HMF_DBL) {
            ...
            if (!htsmsg_field_get_s64(f2, &v))
              idnode_filter_add_num(&conf->filter,
                  k,
                  v,
                  t == -1 ? IC_EQ : t,
                  intsplit);
...
int api_idnode_grid(access_t* perm,
    void*                     opaque,
    const char*               op,
    htsmsg_t*                 args,
    htsmsg_t**                resp) {
...
static int api_idnode_load(access_t* perm,
    void*                            opaque,
    const char*                      op,
    htsmsg_t*                        args,
    htsmsg_t**                       resp) {
    ...
    HTSMSG_FOREACH(f, uuids) {
      ...
      if (grid > 0) {
        ...
        if (meta > 0)
          htsmsg_add_msg(m,
              "meta",
              idclass_serialize0(in->in_class, flist, 0, perm->aa_lang_ui));
      }
...
int api_idnode_load_simple(access_t* perm,
  ...
  if ((class = htsmsg_get_str(args, "class"))) {
    tvh_mutex_lock(&global_lock);
    err =
        api_idnode_load_by_class0(perm, (void*)in->in_class, NULL, args, resp);
    tvh_mutex_unlock(&global_lock);
    return err;
  }
...
static int api_idnode_save(access_t* perm,
    void*                            opaque,
    const char*                      op,
    htsmsg_t*                        args,
    htsmsg_t**                       resp) {
...
int api_idnode_tree(access_t* perm,
    void*                     opaque,
    const char*               op,
    htsmsg_t*                 args,
    htsmsg_t**                resp) {
...
static int api_idnode_delete(access_t* perm,
    void*                              opaque,
    const char*                        op,
    htsmsg_t*                          args,
    htsmsg_t**                         resp) {
  return api_idnode_handler(NULL,
      perm,
      args,
      resp,
      api_idnode_delete_,
...
static int api_idnode_moveup(access_t* perm,
    void*                              opaque,
    const char*                        op,
    htsmsg_t*                          args,
    htsmsg_t**                         resp) {
...
static int api_idnode_movedown(access_t* perm,
    void*                                opaque,
    const char*                          op,
    htsmsg_t*                            args,
    htsmsg_t**                           resp) {
