...
static int api_channel_list(access_t* perm,
    void*                             opaque,
    const char*                       op,
    htsmsg_t*                         args,
    htsmsg_t**                        resp) {
  ...
  for (i = 0; i < count; i++) {
    ...
    htsmsg_add_msg(l,
        NULL,
        htsmsg_create_key_val(idnode_uuid_as_str(&ch->ch_id, ubuf), name));
  }
...
static void api_channel_grid(access_t* perm,
    idnode_set_t*                      ins,
...
static int api_channel_create(access_t* perm,
    void*                               opaque,
    const char*                         op,
    htsmsg_t*                           args,
    htsmsg_t**                          resp) {
...
static int api_channel_rename(access_t* perm,
    void*                               opaque,
    const char*                         op,
    htsmsg_t*                           args,
    htsmsg_t**                          resp) {
...
static int api_channel_tag_list(access_t* perm,
  ...
  TAILQ_FOREACH (ct, &channel_tags, ct_link)
    if (cfg || channel_tag_access(ct, perm, 0)) {
      ...
      htsmsg_add_msg(l,
          NULL,
          htsmsg_create_key_val(idnode_uuid_as_str(&ct->ct_id, ubuf), name));
    }
  tvh_mutex_unlock(&global_lock);
  *resp = htsmsg_create_map();
  htsmsg_add_msg(*resp, "entries", l);
...
void api_channel_init(void) {
  static api_hook_t ah[] = {
      ...
      {"channel/rename",
          ACCESS_ADMIN,
          api_channel_rename,
          NULL}, /* User convenience function */

      {"channeltag/class",
          ACCESS_ANONYMOUS,
          api_idnode_class,
          (void*)&channel_tag_class},
      {"channeltag/grid",
          ACCESS_ANONYMOUS,
          api_idnode_grid,
