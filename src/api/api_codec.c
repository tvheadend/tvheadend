...
static int
api_codec_list(access_t* perm, void* opaque, const char* op, htsmsg_t* args, htsmsg_t** resp) {
  ...
  if ((list = htsmsg_create_list())) {
    ...
    SLIST_FOREACH (tvh_codec, &tvh_codecs, link) {
      if (!(map = idclass_serialize(tvh_codec_get_class(tvh_codec), perm->aa_lang_ui))) {
        htsmsg_destroy(list);
        list = NULL;
        break;
      }
...
static int api_codec_profile_list(access_t* perm,
    void*                                   opaque,
    const char*                             op,
    htsmsg_t*                               args,
    htsmsg_t**                              resp) {
  ...
  if ((list = htsmsg_create_list())) {
    ...
    LIST_FOREACH (tvh_profile, &tvh_codec_profiles, link) {
      ...
      htsmsg_add_str(map, "uuid", idnode_uuid_as_str((idnode_t*)tvh_profile, buf));
      htsmsg_add_str(map, "title", tvh_codec_profile_get_title(tvh_profile));
      htsmsg_add_str(map, "status", tvh_codec_profile_get_status(tvh_profile));
      htsmsg_add_msg(list, NULL, map);
    }
    tvh_mutex_unlock(&global_lock);
    if (list) {
...
static int api_codec_profile_create(access_t* perm,
    void*                                     opaque,
    const char*                               op,
    htsmsg_t*                                 args,
    htsmsg_t**                                resp) {
  ...
  if ((codec_name = htsmsg_get_str(args, "class")) && (conf = htsmsg_get_map(args, "conf"))) {
...
void api_codec_init(void) {
  static api_hook_t ah[] = {
      ...
      {"codec_profile/class", ACCESS_ADMIN, api_idnode_class, (void*)&codec_profile_class},
      {NULL},
  };
