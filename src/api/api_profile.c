 ...
 *
 */
static int api_profile_is_all(access_t* perm, htsmsg_t* args) {
  return htsmsg_get_bool_or_default(args, "all", 0) && !access_verify2(perm, ACCESS_ADMIN);
}

static int api_profile_find(access_t* perm, const char* uuid) {
  htsmsg_field_t* f;
  const char*     uuid2;
 ...
 */
static int
api_profile_list(access_t* perm, void* opaque, const char* op, htsmsg_t* args, htsmsg_t** resp) {
  profile_t* pro;
  htsmsg_t*  l;
...
static int api_profile_builders(access_t* perm,
    void*                                 opaque,
    const char*                           op,
    htsmsg_t*                             args,
    htsmsg_t**                            resp) {
...
static int
api_profile_create(access_t* perm, void* opaque, const char* op, htsmsg_t* args, htsmsg_t** resp) {
  int         err = 0;
  const char* clazz;
  htsmsg_t*   conf;
  profile_t*  pro;
