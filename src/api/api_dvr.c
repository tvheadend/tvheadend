...
static void api_dvr_config_grid(access_t* perm,
    idnode_set_t*                         ins,
...
static int api_dvr_config_create(access_t* perm,
    void*                                  opaque,
    const char*                            op,
    htsmsg_t*                              args,
    htsmsg_t**                             resp) {
...
static void api_dvr_entry_grid(access_t* perm,
    idnode_set_t*                        ins,
    api_idnode_grid_conf_t*              conf,
    htsmsg_t*                            args) {
  dvr_entry_t* de;
...
static void api_dvr_entry_grid_upcoming(access_t* perm,
    idnode_set_t*                                 ins,
    api_idnode_grid_conf_t*                       conf,
    htsmsg_t*                                     args) {
  dvr_entry_t* de;
...
static void api_dvr_entry_grid_finished(access_t* perm,
    idnode_set_t*                                 ins,
    api_idnode_grid_conf_t*                       conf,
    htsmsg_t*                                     args) {
  dvr_entry_t* de;
...
static void api_dvr_entry_grid_failed(access_t* perm,
    idnode_set_t*                               ins,
    api_idnode_grid_conf_t*                     conf,
    htsmsg_t*                                   args) {
  dvr_entry_t* de;
...
static void api_dvr_entry_grid_removed(access_t* perm,
    idnode_set_t*                                ins,
    api_idnode_grid_conf_t*                      conf,
    htsmsg_t*                                    args) {
  dvr_entry_t* de;
  ...
  LIST_FOREACH (de, &dvrentries, de_global_link)
    if (dvr_entry_is_finished(de,
            DVR_FINISHED_REMOVED_SUCCESS | DVR_FINISHED_REMOVED_FAILED))
      idnode_set_add(ins, (idnode_t*)de, &conf->filter, perm->aa_lang_ui);
}

static int api_dvr_entry_create(access_t* perm,
    void*                                 opaque,
    const char*                           op,
    htsmsg_t*                             args,
    htsmsg_t**                            resp) {
...
static htsmsg_t* api_dvr_entry_create_from_single(htsmsg_t* args) {
  htsmsg_t *  entries, *m;
  const char *s1, *s2, *s3;
...
static int api_dvr_entry_create_by_event(access_t* perm,
    void*                                          opaque,
    const char*                                    op,
    htsmsg_t*                                      args,
    htsmsg_t**                                     resp) {
  ...
  HTSMSG_FOREACH(f, entries) {
    ...
    if ((e = epg_broadcast_find_by_id(strtoll(s, NULL, 10)))) {
      dvr_config_t* cfg =
          dvr_config_find_by_list(perm->aa_dvrcfgs, config_uuid);
      if (cfg) {
...
static void api_dvr_rerecord_toggle(access_t* perm, idnode_t* self) {
  dvr_entry_set_rerecord((dvr_entry_t*)self, -1);
}

static int api_dvr_entry_rerecord_toggle(access_t* perm,
    void*                                          opaque,
    const char*                                    op,
    htsmsg_t*                                      args,
    htsmsg_t**                                     resp) {
  return api_idnode_handler(&dvr_entry_class,
      perm,
      args,
      resp,
      api_dvr_rerecord_toggle,
      "rerecord",
      0);
}

static void api_dvr_rerecord_deny(access_t* perm, idnode_t* self) {
  dvr_entry_set_rerecord((dvr_entry_t*)self, 0);
}

static int api_dvr_entry_rerecord_deny(access_t* perm,
    void*                                        opaque,
    const char*                                  op,
    htsmsg_t*                                    args,
    htsmsg_t**                                   resp) {
  return api_idnode_handler(&dvr_entry_class,
      ...
      args,
      resp,
      api_dvr_rerecord_deny,
      "rerecord",
      0);
}

static void api_dvr_rerecord_allow(access_t* perm, idnode_t* self) {
  dvr_entry_set_rerecord((dvr_entry_t*)self, 1);
}

static int api_dvr_entry_rerecord_allow(access_t* perm,
    void*                                         opaque,
    const char*                                   op,
    htsmsg_t*                                     args,
    htsmsg_t**                                    resp) {
  return api_idnode_handler(&dvr_entry_class,
      ...
      args,
      resp,
      api_dvr_rerecord_allow,
      "rerecord",
      0);
}

static void api_dvr_stop(access_t* perm, idnode_t* self) {
  dvr_entry_stop((dvr_entry_t*)self);
}

static int api_dvr_entry_stop(access_t* perm,
    void*                               opaque,
    const char*                         op,
    htsmsg_t*                           args,
    htsmsg_t**                          resp) {
...
static void api_dvr_cancel(access_t* perm, idnode_t* self) {
  dvr_entry_cancel((dvr_entry_t*)self, 0);
}

static int api_dvr_entry_cancel(access_t* perm,
    void*                                 opaque,
    const char*                           op,
    htsmsg_t*                             args,
    htsmsg_t**                            resp) {
...
static void api_dvr_remove(access_t* perm, idnode_t* self) {
...
static int api_dvr_entry_remove(access_t* perm,
    void*                                 opaque,
    const char*                           op,
    htsmsg_t*                             args,
    htsmsg_t**                            resp) {
...
static void api_dvr_prevrec_toggle(access_t* perm, idnode_t* self) {
  dvr_entry_set_prevrec((dvr_entry_t*)self, -1);
}

static int api_dvr_entry_prevrec_toggle(access_t* perm,
    void*                                         opaque,
    const char*                                   op,
    htsmsg_t*                                     args,
    htsmsg_t**                                    resp) {
  return api_idnode_handler(&dvr_entry_class,
      ...
      args,
      resp,
      api_dvr_prevrec_toggle,
      "prevrec",
      0);
}

static void api_dvr_prevrec_unset(access_t* perm, idnode_t* self) {
  dvr_entry_set_prevrec((dvr_entry_t*)self, 0);
}

static int api_dvr_entry_prevrec_unset(access_t* perm,
    void*                                        opaque,
    const char*                                  op,
    htsmsg_t*                                    args,
    htsmsg_t**                                   resp) {
  return api_idnode_handler(&dvr_entry_class,
      ...
      args,
      resp,
      api_dvr_prevrec_unset,
      "prevrec",
      0);
}

static void api_dvr_prevrec_set(access_t* perm, idnode_t* self) {
  dvr_entry_set_prevrec((dvr_entry_t*)self, 1);
}

static int api_dvr_entry_prevrec_set(access_t* perm,
    void*                                      opaque,
    const char*                                op,
    htsmsg_t*                                  args,
    htsmsg_t**                                 resp) {
...
static void api_dvr_move_finished(access_t* perm, idnode_t* self) {
  dvr_entry_move((dvr_entry_t*)self, 0);
}

static int api_dvr_entry_move_finished(access_t* perm,
    void*                                        opaque,
    const char*                                  op,
    htsmsg_t*                                    args,
    htsmsg_t**                                   resp) {
  return api_idnode_handler(&dvr_entry_class,
      ...
      args,
      resp,
      api_dvr_move_finished,
      "move finished",
      0);
}

static void api_dvr_move_failed(access_t* perm, idnode_t* self) {
  dvr_entry_move((dvr_entry_t*)self, 1);
}

static int api_dvr_entry_move_failed(access_t* perm,
    void*                                      opaque,
    const char*                                op,
    htsmsg_t*                                  args,
    htsmsg_t**                                 resp) {
...
static void api_dvr_autorec_grid(access_t* perm,
    idnode_set_t*                          ins,
    api_idnode_grid_conf_t*                conf,
    htsmsg_t*                              args) {
...
static int api_dvr_autorec_create(access_t* perm,
    void*                                   opaque,
    const char*                             op,
    htsmsg_t*                               args,
    htsmsg_t**                              resp) {
...
static int api_dvr_autorec_create_by_series(access_t* perm,
    void*                                             opaque,
    const char*                                       op,
    htsmsg_t*                                         args,
    htsmsg_t**                                        resp) {
  ...
  HTSMSG_FOREACH(f, entries) {
    ...
    if ((e = epg_broadcast_find_by_id(strtoll(s, NULL, 10)))) {
      ...
      if (cfg) {
        ...
        dae =
            dvr_autorec_add_series_link(idnode_uuid_as_str(&cfg->dvr_id, ubuf),
                e,
                perm->aa_username,
...
static void api_dvr_timerec_grid(access_t* perm,
    idnode_set_t*                          ins,
    api_idnode_grid_conf_t*                conf,
    htsmsg_t*                              args) {
...
static int api_dvr_timerec_create(access_t* perm,
    void*                                   opaque,
    const char*                             op,
    htsmsg_t*                               args,
    htsmsg_t**                              resp) {
...
static int api_dvr_entry_file_moved(access_t* perm,
    void*                                     opaque,
    const char*                               op,
    htsmsg_t*                                 args,
    htsmsg_t**                                resp) {
...
void api_dvr_init(void) {
  static api_hook_t ah[] = {
      {"dvr/config/class",
          ACCESS_OR | ACCESS_ADMIN | ACCESS_RECORDER,
          api_idnode_class,
          (void*)&dvr_config_class},
      {"dvr/config/grid",
          ACCESS_OR | ACCESS_ADMIN | ACCESS_RECORDER,
          api_idnode_grid,
          api_dvr_config_grid},
      ...
      {"dvr/entry/stop",
          ACCESS_RECORDER,
          api_dvr_entry_stop,
          NULL}, /* Stop active recording gracefully */
      {"dvr/entry/cancel",
          ACCESS_RECORDER,
          api_dvr_entry_cancel,
          NULL}, /* Cancel scheduled or active recording */
      {"dvr/entry/prevrec/toggle",
          ACCESS_RECORDER,
          api_dvr_entry_prevrec_toggle,
          NULL},
      {"dvr/entry/prevrec/set",
          ACCESS_RECORDER,
          api_dvr_entry_prevrec_set,
          NULL},
      {"dvr/entry/prevrec/unset",
          ACCESS_RECORDER,
      ...
      {"dvr/entry/remove",
          ...
          NULL}, /* Remove recorded files from storage */
