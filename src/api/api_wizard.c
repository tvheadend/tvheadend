...
static int wizard_cancel(access_t* perm,
    void*                          opaque,
    const char*                    op,
    htsmsg_t*                      args,
    htsmsg_t**                     resp) {
...
static int wizard_start(access_t* perm,
    void*                         opaque,
    const char*                   op,
    htsmsg_t*                     args,
    htsmsg_t**                    resp) {
...
static int wizard_status_progress(access_t* perm,
  ...
  *resp = htsmsg_create_map();
  htsmsg_add_dbl(*resp,
      "progress",
      total ? ((double)1.0 - ((double)active / (double)total)) : 1);
...
void api_wizard_init(void) {
  static api_hook_t ah[] = {
      ...
      {"wizard/mapping/save",
          ...
          wizard_idnode_save_simple,
          wizard_mapping},
      {"wizard/channels/load",
          ACCESS_ADMIN,
          wizard_idnode_load_simple,
          wizard_channels},
      {"wizard/channels/save",
          ACCESS_ADMIN,
          wizard_idnode_save_simple,
          wizard_channels},
