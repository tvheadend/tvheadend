...
static void api_esfilter_grid(access_t* perm,
    idnode_set_t*                       ins,
...
static int api_esfilter_create(access_t* perm,
    void*                                opaque,
    const char*                          op,
    htsmsg_t*                            args,
    htsmsg_t**                           resp,
...
#define ESFILTER(func, t)                                          \
  static void api_esfilter_grid_##func(access_t* perm,             \
      idnode_set_t*                              ins,              \
      api_idnode_grid_conf_t*                    conf,             \
      htsmsg_t*                                  args) {                                            \
    return api_esfilter_grid(perm, ins, conf, args, (t));          \
  }                                                                \
  static int api_esfilter_create_##func(access_t* perm,            \
      void*                                       opaque,          \
      const char*                                 op,              \
      htsmsg_t*                                   args,            \
      htsmsg_t**                                  resp) {                                           \
