 ...
 * Inputs
 */
static int api_mpegts_input_network_list(access_t* perm,
    void*                                          opaque,
    const char*                                    op,
    htsmsg_t*                                      args,
    htsmsg_t**                                     resp) {
  ...
  if ((is = mi->mi_network_list(mi))) {
    for (i = 0; i < is->is_count; i++) {
      ...
      htsmsg_add_msg(l,
          NULL,
          htsmsg_create_key_val(idnode_uuid_as_str(&mn->mn_id, ubuf), buf));
    }
    idnode_set_free(is);
  }
  err   = 0;
 ...
 * Networks
 */
static void api_mpegts_network_grid(access_t* perm,
    idnode_set_t*                             ins,
...
static int api_mpegts_network_builders(access_t* perm,
    void*                                        opaque,
    const char*                                  op,
    htsmsg_t*                                    args,
    htsmsg_t**                                   resp) {
...
static int api_mpegts_network_create(access_t* perm,
    void*                                      opaque,
    const char*                                op,
    htsmsg_t*                                  args,
    htsmsg_t**                                 resp) {
...
static int api_mpegts_network_scan(access_t* perm,
    void*                                    opaque,
    const char*                              op,
    htsmsg_t*                                args,
    htsmsg_t**                               resp) {
...
static int api_mpegts_network_muxclass(access_t* perm,
    void*                                        opaque,
    const char*                                  op,
    htsmsg_t*                                    args,
    htsmsg_t**                                   resp) {
...
static int api_mpegts_network_muxcreate(access_t* perm,
    void*                                         opaque,
    const char*                                   op,
    htsmsg_t*                                     args,
    htsmsg_t**                                    resp) {
 ...
 * Muxes
 */
static void api_mpegts_mux_grid(access_t* perm,
    idnode_set_t*                         ins,
    api_idnode_grid_conf_t*               conf,
    htsmsg_t*                             args) {
  mpegts_network_t* mn;
  ...
  LIST_FOREACH (mn, &mpegts_network_all, mn_global_link) {
    if (hide && !mn->mn_enabled)
      continue;
    LIST_FOREACH (mm, &mn->mn_muxes, mm_network_link) {
      if (hide == 2 && !mm->mm_is_enabled(mm))
        continue;
 ...
 * Services
 */
static void api_mpegts_service_grid(access_t* perm,
    idnode_set_t*                             ins,
    api_idnode_grid_conf_t*                   conf,
    htsmsg_t*                                 args) {
  mpegts_network_t* mn;
  ...
  LIST_FOREACH (mn, &mpegts_network_all, mn_global_link) {
    if (hide && !mn->mn_enabled)
      continue;
    LIST_FOREACH (mm, &mn->mn_muxes, mm_network_link) {
      if (hide && !mm->mm_is_enabled(mm))
        continue;
      LIST_FOREACH (ms, &mm->mm_services, s_dvb_mux_link) {
        if (hide && !ms->s_verified)
          continue;
        if (hide == 2 && !ms->s_is_enabled((service_t*)ms, 0))
          continue;
 ...
 * Mux scheduler
 */
static void api_mpegts_mux_sched_grid(access_t* perm,
    idnode_set_t*                               ins,
    api_idnode_grid_conf_t*                     conf,
    htsmsg_t*                                   args) {
...
static int api_mpegts_mux_sched_create(access_t* perm,
    void*                                        opaque,
    const char*                                  op,
    htsmsg_t*                                    args,
    htsmsg_t**                                   resp) {
...
#if ENABLE_MPEGTS_DVB
static int api_dvb_orbitalpos_list(access_t* perm,
    void*                                    opaque,
    const char*                              op,
    htsmsg_t*                                args,
    htsmsg_t**                               resp) {
...
#if ENABLE_MPEGTS_DVB
static int api_dvb_scanfile_list(access_t* perm,
    void*                                  opaque,
    const char*                            op,
    htsmsg_t*                              args,
    htsmsg_t**                             resp) {
  ...
  LIST_FOREACH (r, &list->srl_regions, sfr_link) {
    LIST_FOREACH (n, &r->sfr_networks, sfn_link) {
      if (satpos != INT_MAX && abs(n->sfn_satpos - satpos) > 2 &&
          abs(satpos - n->sfn_satpos) > 2)
        continue;
      e = htsmsg_create_map();
...
void api_mpegts_init(void) {
  ...
  static api_hook_t ah[] = {
    ...
    {"mpegts/service/class",
        ACCESS_ADMIN,
        api_idnode_class,
        (void*)&mpegts_service_class},
    {"mpegts/mux_sched/class",
        ACCESS_ADMIN,
        api_idnode_class,
        (void*)&mpegts_mux_sched_class},
