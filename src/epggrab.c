...
static memoryinfo_t epggrab_data_memoryinfo = {
    .my_name = "EPG grabber data queue"};
...
static void* _epggrab_internal_thread(void* aux) {
  ...
  confver =
      epggrab_conf.int_initial ? -1 /* force first run */ : epggrab_confver;
...
static void _epggrab_load(void) {
  ...
  if (m) {
    ...
    epggrab_conf.cron =
        strdup("# Default config (00:04 and 12:04 everyday)\n4 */12 * * *");
    LIST_FOREACH (mod,
        &epggrab_modules,
        link) // enable only OTA EIT and OTA PSIP by default
...
static void epggrab_class_changed(idnode_t* self) {}
...
epggrab_conf_t epggrab_conf = {.idnode.in_class = &epggrab_class};

static void epggrab_class_cron_notify(void* self, const char* lang) {
  tvh_mutex_lock(&epggrab_mutex);
...
static void epggrab_class_ota_cron_notify(void* self, const char* lang) {
  epggrab_ota_set_cron();
}

static void epggrab_class_ota_genre_translation_notify(void* self,
    const char*                                              lang) {
...
const idclass_t epggrab_class = {.ic_snode = &epggrab_conf.idnode,
    .ic_class                              = "epggrab",
    ...
    .ic_properties = (const property_t[]){
        ...
        },
        {
            ...
            .desc  = N_("Writes the current in-memory EPG database to disk "
                       "shortly after an xmltv import has completed, so should "
                       "a crash/unexpected "
                       "shutdown occur EPG data is saved "
        ...
        },
        {
            ...
            .id   = "int_initial",
            .name =
                N_("Force initial EPG grab at start-up (internal grabbers)"),
            .desc = N_(
                "Force an initial EPG grab at start-up (internal grabbers)."),
        ...
        },
        {
            ...
            .desc = N_(
                "Translate the genre codes received from the broadcaster to "
                "another genre code."
                "<br>Use the form xxx=yyy, where xxx and yyy are "
                "'ETSI EN 300 468' content descriptor values expressed in "
                "decimal (0-255). "
...
void epggrab_init(void) {
  ...
  tvh_thread_create(&epggrab_tid,
      NULL,
      _epggrab_internal_thread,
      NULL,
      "epggrabi");
  tvh_thread_create(&epggrab_data_tid,
      NULL,
      _epggrab_data_thread,
      NULL,
      "epgdata");
}
