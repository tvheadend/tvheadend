  ...
  if (m) {
    ...
    LIST_FOREACH (mod, &epggrab_modules, link) // enable only OTA EIT and OTA PSIP by default
      if (mod->type == EPGGRAB_OTA &&
          ((mod->subsys == LS_TBL_EIT && strcmp(mod->id, "eit") == 0) || mod->subsys == LS_PSIP)) {
        mod->enabled = 1;
...
static htsmsg_t* epggrab_class_save(idnode_t* self, char* filename, size_t fsize) {
  epggrab_module_t* mod;
  htsmsg_t *        m, *a;
...
static void epggrab_class_ota_genre_translation_notify(void* self, const char* lang) {
...
const idclass_t epggrab_class = {.ic_snode = &epggrab_conf.idnode,
    ...
    .ic_properties                         = (const property_t[]){
        ...
        },
        {
                                    ...
                                    .desc =
                N_("Translate the genre codes received from the broadcaster to another genre code."
