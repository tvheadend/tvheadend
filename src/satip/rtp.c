...
static int satip_rtp_send(satip_rtp_session_t* rtp) {
  ...
  if (v->iov_len == RTP_PAYLOAD) {
    ...
    if (r != packets) {
      tvhtrace(LS_SATIPS,
          "rtp udp multisend failed (packets %d written %d)",
          packets,
          r);
      return -1;
    }
...
static inline int satip_rtp_append_data(satip_rtp_session_t* rtp,
    struct iovec**                                           _v,
    uint8_t*                                                 data) {
  struct iovec* v = *_v;
  int           r;
      ...
      TAILQ_FOREACH (tbl, &rtp->pmt_tables, link)
        if (tbl->pid == pid) {
          ...
          if (rtp->table_data.sb_ptr) {
            r = satip_rtp_append_tcp_data(rtp,
                rtp->table_data.sb_data,
                rtp->table_data.sb_ptr);
            sbuf_reset(&rtp->table_data, 10 * 188);
            if (r)
              return -1;
          }
...
static void satip_rtp_signal_status(satip_rtp_session_t* rtp,
    signal_status_t*                                     sig) {
...
void* satip_rtp_queue(th_subscription_t* subs,
  ...
  if (perm_lock) {
    int tmp =
        ((satip_server_conf.satip_iptv_sig_level * 0xffff) + 0x8000) / 245;
...
static int satip_status_build(satip_rtp_session_t* rtp, char* buf, int len) {
  ...
  switch (rtp->dmc.dmc_fe_delsys) {
    ...
    case DVB_SYS_DVBC_ANNEX_C:
      delsys =
          rtp->dmc.dmc_fe_delsys == DVB_SYS_DVBC_ANNEX_A ? "dvbc" : "dvbc2";
      bw = "";
      switch (rtp->dmc.dmc_fe_modulation) {
        case DVB_MOD_QAM_16:
          msys = "qam16";
...
void satip_rtp_init(int boot) {
  ...
  if (!boot && !atomic_get(&satip_rtcp_run)) {
    ...
    tvh_thread_create(&satip_rtcp_tid,
        NULL,
        satip_rtcp_thread,
        NULL,
        "satip-rtcp");
