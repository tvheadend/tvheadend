...
rtsp_new_session(const char* ipstr, int delsys, uint32_t nsession, int session) {
  ...
  if (count_s > 0 || count_u > 0)
    TAILQ_FOREACH (rs, &rtsp_sessions, link) {
      if (--count_s == 0) {
        tvhnotice(LS_SATIPS,
            "Max number (%i) of active RTSP sessions reached.",
            satip_server_conf.satip_max_sessions);
        return NULL;
      }
      if (strcmp(rs->peer_ipstr, ipstr) == 0 && --count_u == 0) {
        tvhnotice(LS_SATIPS,
            "Max number (%i) of active RTSP sessions per user (IP: %s).",
            satip_server_conf.satip_max_user_connections,
            ipstr);
        return NULL;
      }
    }
 ...
 *
 */
static struct session* rtsp_find_session(http_connection_t* hc, int stream) {
  struct session *rs, *first = NULL;

  if (stream <= 0)
    return NULL;
  TAILQ_FOREACH (rs, &rtsp_sessions, link) {
    if (hc->hc_session && strcmp(rs->session, hc->hc_session) == 0 &&
 ...
 *
 */
static void rtsp_session_timer_cb(void* aux) {
  session_t* rs = aux;
...
static inline void rtsp_rearm_session_timer(session_t* rs) {
 ...
 *
 */
static char* rtsp_check_urlbase(char* u) {
  char *p, *s;
  int   t;
 ...
 *
 */
static int rtsp_parse_args(http_connection_t* hc, char* u) {
  char* s;
  int   stream = 0;
 ...
 *
 */
static inline const char* rtsp_conn_ip(http_connection_t* hc, char* buf, size_t buflen, int* port) {
 ...
 *
 */
static void rtsp_slave_add(session_t* rs, mpegts_service_t* master, mpegts_service_t* slave) {
  char                  buf[128];
  ...
  sub->ths = subscription_create_from_service(&sub->prch,
      NULL,
      rs->used_weight,
      buf,
      SUBSCRIPTION_NONE,
  ...
  if (sub->ths == NULL) {
    tvherror(LS_SATIPS,
        "%i/%s/%i: unable to subscribe service %s\n",
        rs->frontend,
        rs->session,
        rs->stream,
        slave->s_nicename);
    ...
    tvhdebug(LS_SATIPS,
        "%i/%s/%i: slave service %s subscribed",
        rs->frontend,
        rs->session,
        rs->stream,
        slave->s_nicename);
 ...
 *
 */
static void rtsp_slave_remove(session_t* rs, mpegts_service_t* master, mpegts_service_t* slave) {
  ...
  if (sub == NULL)
    return;
  tvhdebug(LS_SATIPS,
      "%i/%s/%i: slave service %s unsubscribed",
      rs->frontend,
      rs->session,
      rs->stream,
      slave->s_nicename);
 ...
 *
 */
static void rtsp_clean(session_t* rs, int clean_mux) {
  slave_subscription_t* sub;
  ...
  if (rs->subs) {
    while ((sub = LIST_FIRST(&rs->slaves)) != NULL)
      rtsp_slave_remove(rs, (mpegts_service_t*)rs->subs->ths_raw_service, sub->service);
 ...
 *
 */
static void rtsp_no_data(void* opaque) {
  session_t* rs = opaque;
  rs->no_data   = 1;
}
 ...
 *
 */
static int rtsp_validate_service(mpegts_service_t* s, mpegts_apids_t* pids) {
  int                  av = 0, enc = 0;
  ...
  if (enc == 0 || av == 0)
    return 0;
  return pids == NULL || mpegts_pid_wexists(pids, s->s_components.set_pmt_pid, MPS_WEIGHT_RAW);
}

/*
 *
 */
static void rtsp_manage_descramble(session_t* rs) {
  idnode_set_t*         found;
  ...
  if (si < found->is_count)
    tvhwarn(LS_SATIPS,
        "%i/%s/%i: limit for descrambled services reached (wanted %zd allowed %d)",
        rs->frontend,
        rs->session,
        rs->stream,
        (used - si) + found->is_count,
        rtsp_descramble);
  ...
  if (rtsp_rewrite_pmt) {
    ...
    LIST_FOREACH (sub, &rs->slaves, link) {
      if ((s = sub->service) == NULL)
        continue;
      if (s->s_components.set_pmt_pid <= 0 || s->s_components.set_pmt_pid >= 8191)
        continue;
 ...
 *
 */
static int rtsp_start(http_connection_t* hc, session_t* rs, char* addrbuf, int newmux, int cmd) {
  ...
  if (newmux) {
    ...
    LIST_FOREACH (mn, &mpegts_network_all, mn_global_link) {
      if (idnode_is_instance(&mn->mn_id, &dvb_network_class)) {
        ln = (dvb_network_t*)mn;
        if (ln->ln_type == rs->dmc.dmc_fe_type && mn->mn_satip_source == rs->src) {
          if (!mn2)
            mn2 = mn;
          mux = (mpegts_mux_t*)dvb_network_find_mux((dvb_network_t*)mn,
              &rs->dmc,
              MPEGTS_ONID_NONE,
              MPEGTS_TSID_NONE,
              1,
              rtsp_muxcnf == MUXCNF_REJECT_EXACT_MATCH);
          if (mux) {
    ...
    if (mux == NULL && mn2 && (rtsp_muxcnf == MUXCNF_AUTO || rtsp_muxcnf == MUXCNF_KEEP)) {
      dvb_mux_conf_str(&rs->dmc, buf, sizeof(buf));
      tvhwarn(LS_SATIPS, "%i/%s/%i: create mux %s", rs->frontend, rs->session, rs->stream, buf);
      mux = mn2->mn_create_mux(mn2,
          (void*)(intptr_t)rs->nsession,
          MPEGTS_ONID_NONE,
          MPEGTS_TSID_NONE,
          &rs->dmc,
          1);
    ...
    if (mux == NULL) {
      dvb_mux_conf_str(&rs->dmc, buf, sizeof(buf));
      tvhwarn(LS_SATIPS,
          "%i/%s/%i: unable to create mux %s%s",
          rs->frontend,
          rs->session,
          rs->stream,
          buf,
          (rtsp_muxcnf == MUXCNF_REJECT || rtsp_muxcnf == MUXCNF_REJECT_EXACT_MATCH)
              ? " (configuration)"
              : "");
      goto endclean;
    }
    if (rs->mux == mux && rs->subs) {
      if (rs->no_data) {
        dvb_mux_conf_str(&rs->dmc, buf, sizeof(buf));
        tvhwarn(LS_SATIPS,
            "%i/%s/%i: subscription fails because mux %s can't tune",
            rs->frontend,
            rs->session,
            rs->stream,
            buf);
    ...
    rs->subs        = subscription_create_from_mux(&rs->prch,
        NULL,
        weight,
        "SAT>IP",
        rs->prch.prch_flags | SUBSCRIPTION_STREAMING,
        addrbuf,
        http_username(hc),
  ...
  if (cmd != RTSP_CMD_DESCRIBE && rs->rtp_handle == NULL) {
    if (rs->mux == NULL)
      goto endclean;
    rs->no_data    = 0;
    rs->rtp_handle = satip_rtp_queue(rs->subs,
        &rs->prch.prch_sq,
        hc,
        hc->hc_peer,
        rs->rtp_peer_port,
 ...
 *
 */
static inline int msys_to_tvh(http_connection_t* hc) {
  static struct strtab tab[] = {{"dvbs", DVB_SYS_DVBS},
      {"dvbs2", DVB_SYS_DVBS2},
      {"dvbt", DVB_SYS_DVBT},
      {"dvbt2", DVB_SYS_DVBT2},
...
static inline int pol_to_tvh(http_connection_t* hc) {
...
static int fec_to_tvh(http_connection_t* hc) {
...
static int bw_to_tvh(http_connection_t* hc) {
  int bw = atof(http_arg_get_remove(&hc->hc_req_args, "bw") ?: "0") * 1000;
  switch (bw) {
    case 0:
      return DVB_BANDWIDTH_AUTO;
...
static int rolloff_to_tvh(http_connection_t* hc) {
...
static int pilot_to_tvh(http_connection_t* hc) {
...
static int tmode_to_tvh(http_connection_t* hc) {
  static struct strtab tab[] = {
...
static int mtype_to_tvh(http_connection_t* hc) {
  static struct strtab tab[] = {
      {"auto", DVB_MOD_AUTO},
      {"qpsk", DVB_MOD_QPSK},
      {"8psk", DVB_MOD_PSK_8},
...
static int gi_to_tvh(http_connection_t* hc) {
  switch (atoi(http_arg_get_remove(&hc->hc_req_args, "gi") ?: "0")) {
    ...
    default:
      return DVB_GUARD_INTERVAL_NONE;
  }
}

static int parse_pids(char* p, mpegts_apids_t* pids) {
  char *x, *saveptr;
  int   i = 0, pid;
...
static int parse_transport(http_connection_t* hc, char* destip, size_t destip_len) {
 ...
 *
 */
static int rtsp_parse_cmd(http_connection_t* hc, int stream, int cmd, session_t** rrs, int* valid) {
  session_t*      rs      = NULL;
  ...
  *valid = freq >= 10000;
  ...
  if (mtype == DVB_MOD_NONE)
    goto end;

  src = http_arg_get(&hc->hc_req_args, "src") ? atoi(http_arg_get_remove(&hc->hc_req_args, "src"))
                                              : 1;
  if (src < 1)
    goto end;
  ...
  if (msys == DVB_SYS_DVBS || msys == DVB_SYS_DVBS2) {
    ...
    if (ro == DVB_ROLLOFF_NONE || (ro != DVB_ROLLOFF_35 && msys == DVB_SYS_DVBS))
      goto end;
    plts = pilot_to_tvh(hc);
    if (plts == DVB_PILOT_NONE)
      goto end;

    TAILQ_FOREACH (arg, &hc->hc_req_args, link)
      tvhwarn(LS_SATIPS,
          "%i/%s/%i: extra parameter '%s'='%s'",
          rs->frontend,
          rs->session,
          rs->stream,
          arg->key,
    ...
    if (freq < 0)
      goto end;
    bw = bw_to_tvh(hc);
    if (bw == DVB_BANDWIDTH_NONE)
      goto end;
    tmode = tmode_to_tvh(hc);
    if (tmode == DVB_TRANSMISSION_MODE_NONE)
      goto end;
    gi = gi_to_tvh(hc);
    if (gi == DVB_GUARD_INTERVAL_NONE)
      goto end;
    fec = fec_to_tvh(hc);
    if (fec == DVB_FEC_NONE)
      goto end;
 ...
 *
 */
static int rtsp_process_options(http_connection_t* hc) {
 ...
 *
 */
static void rtsp_describe_header(session_t* rs, htsbuf_queue_t* q) {
  ...
  htsbuf_qprintf(q,
      "s=SatIPServer:1 %d",
      satip_server_conf.satip_dvbs + satip_server_conf.satip_dvbs2);
  dvbt = satip_server_conf.satip_dvbt + satip_server_conf.satip_dvbt2;
  dvbc = satip_server_conf.satip_dvbc + satip_server_conf.satip_dvbc2;
  if (dvbc)
...
static void rtsp_describe_session(session_t* rs, htsbuf_queue_t* q) {
  char buf[4096];
 ...
 *
 */
static int rtsp_process_describe(http_connection_t* hc) {
  http_arg_list_t args;
  const char*     arg;
  char*           u = tvh_strdupa(hc->hc_url);
  session_t*      rs;
  ...
  TAILQ_FOREACH (rs, &rtsp_sessions, link) {
    ...
    if (stream > 0 && rs->stream != stream)
      continue;
    if (satip_server_conf.satip_anonymize && strcmp(hc->hc_peer_ipstr, rs->peer_ipstr))
      continue;
    if (first) {
 ...
 *
 */
static int rtsp_process_play(http_connection_t* hc, int cmd) {
  session_t*      rs;
  ...
  if (cmd == RTSP_CMD_SETUP && rs->rtp_peer_port != RTSP_TCP_DATA && !rs->rtp_udp_bound) {
    if (udp_bind_double(&rs->udp_rtp,
            &rs->udp_rtcp,
            LS_SATIPS,
            "rtsp",
            "rtcp",
            (rtp_src_ip != NULL && rtp_src_ip[0] != '\0') ? rtp_src_ip : rtsp_ip,
            0,
            NULL,
            4 * 1024,
            4 * 1024,
 ...
 *
 */
static int rtsp_process_teardown(http_connection_t* hc) {
  char*           u  = tvh_strdupa(hc->hc_url);
  struct session* rs = NULL;
  http_arg_list_t args;
  char            session[16];
  ...
  if ((u = rtsp_check_urlbase(u)) == NULL || (stream = rtsp_parse_args(hc, u)) < 0) {
    http_error(hc, HTTP_STATUS_BAD_REQUEST);
    return 0;
  }

  tvhdebug(LS_SATIPS,
      "-/%s/%i: teardown from %s:%d",
      hc->hc_session,
      stream,
      hc->hc_peer_ipstr,
      ntohs(IP_PORT(*hc->hc_peer)));
 ...
 */
static int rtsp_process_request(http_connection_t* hc, htsbuf_queue_t* spill) {
  switch (hc->hc_cmd) {
 ...
 *
 */
static void rtsp_flush_requests(http_connection_t* hc) {
 ...
 *
 */
static void rtsp_stream_status(void* opaque, htsmsg_t* m) {
  ...
  TAILQ_FOREACH (rs, &rtsp_sessions, link) {
    if (hc->hc_session && strcmp(rs->session, hc->hc_session) == 0 &&
        strcmp(rs->peer_ipstr, hc->hc_peer_ipstr) == 0 && (udpport = rs->rtp_peer_port) > 0) {
      if (udpport == RTSP_TCP_DATA) {
        if (rs->tcp_data == hc) {
          s32 = htsmsg_get_s32_or_default(m, "peer_port", -1);
          if (!tcp)
            tcp = htsmsg_create_list();
          htsmsg_add_s32(tcp, NULL, s32);
        }
      } else {
        if (!udp)
          udp = htsmsg_create_list();
  ...
  if (tcp || udp) {
    c = htsmsg_create_map();
    if (tcp)
      htsmsg_add_msg(c, "tcp", tcp);
    if (udp)
      htsmsg_add_msg(c, "udp", udp);
 ...
 */
static void
rtsp_serve(int fd, void** opaque, struct sockaddr_storage* peer, struct sockaddr_storage* self) {
 ...
 *
 */
static void rtsp_close_sessions(void) {
  session_t* rs;
  do {
    tvh_mutex_lock(&rtsp_lock);
...
void satip_server_rtsp_init(const char* bindaddr,
    ...
    const char*                         nat_ip,
    int                                 nat_port) {
  static tcp_server_ops_t ops = {.start = rtsp_serve, .stop = NULL, .cancel = http_cancel};
  int                     reg = 0;
  uint8_t                 rnd[4];
  char*                   s;
  if (!rtsp_server) {
