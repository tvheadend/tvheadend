 ...
 *
 */
static struct session* rtsp_new_session(const char* ipstr,
    int                                             delsys,
    uint32_t                                        nsession,
    int                                             session) {
...
static inline void rtsp_rearm_session_timer(session_t* rs) {
  if (!rs->shutdown_on_close || (rs->rtp_peer_port == RTSP_TCP_DATA)) {
    ...
    mtimer_arm_rel(&rs->timer,
        rtsp_session_timer_cb,
        rs,
        sec2mono(RTSP_TIMEOUT));
    tvh_mutex_unlock(&global_lock);
  }
}
 ...
 *
 */
static void rtsp_slave_add(session_t* rs,
    mpegts_service_t*                 master,
    mpegts_service_t*                 slave) {
  char                  buf[128];
 ...
 *
 */
static void rtsp_slave_remove(session_t* rs,
    mpegts_service_t*                    master,
    mpegts_service_t*                    slave) {
...
static void rtsp_clean(session_t* rs, int clean_mux) {
  ...
  if (rs->subs) {
    while ((sub = LIST_FIRST(&rs->slaves)) != NULL)
      rtsp_slave_remove(rs,
          (mpegts_service_t*)rs->subs->ths_raw_service,
          sub->service);
...
static void rtsp_manage_descramble(session_t* rs) {
  ...
  if (rs->pids.all) {
    ...
    for (i = 0; i < rs->pids.count; i++) {
      s = mpegts_service_find_by_pid((mpegts_mux_t*)rs->mux,
          rs->pids.pids[i].pid);
  ...
  if (si < found->is_count)
    tvhwarn(LS_SATIPS,
        "%i/%s/%i: limit for descrambled services reached (wanted %zd allowed "
        "%d)",
        rs->frontend,
        rs->session,
        rs->stream,
  ...
  if (rtsp_rewrite_pmt) {
    ...
    LIST_FOREACH (sub, &rs->slaves, link) {
      ...
      if (s->s_components.set_pmt_pid <= 0 ||
          s->s_components.set_pmt_pid >= 8191)
        continue;
 ...
 *
 */
static int rtsp_start(http_connection_t* hc,
    session_t*                           rs,
    char*                                addrbuf,
    int                                  newmux,
    int                                  cmd) {
    ...
    LIST_FOREACH (mn, &mpegts_network_all, mn_global_link) {
      ...
      if (idnode_is_instance(&mn->mn_id, &iptv_network_class)) {
        LIST_FOREACH (mux, &mn->mn_muxes, mm_network_link) {
          if (rs->dmc.dmc_fe_type == DVB_TYPE_T &&
              deltaU32(rs->dmc.dmc_fe_freq,
                  ((iptv_mux_t*)mux)->mm_iptv_satip_dvbt_freq) < 2000)
            break;
          if (rs->dmc.dmc_fe_type == DVB_TYPE_C &&
              deltaU32(rs->dmc.dmc_fe_freq,
                  ((iptv_mux_t*)mux)->mm_iptv_satip_dvbc_freq) < 2000)
            break;
          if (rs->dmc.dmc_fe_type == DVB_TYPE_S &&
              deltaU32(rs->dmc.dmc_fe_freq,
                  ((iptv_mux_t*)mux)->mm_iptv_satip_dvbs_freq) < 2000)
            break;
        }
        if (mux) {
          dmc           = rs->dmc;
      ...
      if (idnode_is_instance(&mn->mn_id, &dvb_network_class)) {
        LIST_FOREACH (mux, &mn->mn_muxes, mm_network_link) {
          if (rs->dmc.dmc_fe_type == DVB_TYPE_T &&
              deltaU32(rs->dmc.dmc_fe_freq,
                  ((dvb_mux_t*)mux)->mm_dvb_satip_dvbt_freq) < 2000)
            break;
          if (rs->dmc.dmc_fe_type == DVB_TYPE_C &&
              deltaU32(rs->dmc.dmc_fe_freq,
                  ((dvb_mux_t*)mux)->mm_dvb_satip_dvbc_freq) < 2000)
            break;
          if (rs->dmc.dmc_fe_type == DVB_TYPE_S &&
              deltaU32(rs->dmc.dmc_fe_freq,
                  ((dvb_mux_t*)mux)->mm_dvb_satip_dvbs_freq) < 2000)
            break;
        }
        if (mux) {
          dmc           = rs->dmc;
 ...
 *
 */
static int rtsp_parse_cmd(http_connection_t* hc,
    int                                      stream,
    int                                      cmd,
    session_t**                              rrs,
    int*                                     valid) {
  ...
  *valid = freq >= 10000;
  ...
  src = http_arg_get(&hc->hc_req_args, "src")
      ? atoi(http_arg_get_remove(&hc->hc_req_args, "src"))
      : 1;
  if (src < 1)
    goto end;
  ...
  if (msys == DVB_SYS_DVBS || msys == DVB_SYS_DVBS2) {
    ...
    if (ro == DVB_ROLLOFF_NONE ||
        (ro != DVB_ROLLOFF_35 && msys == DVB_SYS_DVBS))
      goto end;
  ...
  } else if (msys == DVB_SYS_ISDBT || msys == DVB_SYS_ATSC ||
      msys == DVB_SYS_DVBC_ANNEX_B) {
...
static int rtsp_process_play(http_connection_t* hc, int cmd) {
  ...
  if (cmd == RTSP_CMD_SETUP && rs->rtp_peer_port != RTSP_TCP_DATA &&
    ...
    if (udp_bind_double(&rs->udp_rtp,
            ...
            "rtsp",
            "rtcp",
            (rtp_src_ip != NULL && rtp_src_ip[0] != '\0') ? rtp_src_ip
                                                          : rtsp_ip,
            0,
            NULL,
            4 * 1024,
...
void satip_server_rtsp_init(const char* bindaddr,
    ...
    const char*                         nat_ip,
    int                                 nat_port) {
  static tcp_server_ops_t ops = {.start = rtsp_serve,
      .stop                             = NULL,
      .cancel                           = http_cancel};
  int                     reg = 0;
  uint8_t                 rnd[4];
  ...
  if (!rtsp_server)
    rtsp_server =
        tcp_server_create(LS_SATIPS, "SAT>IP RTSP", bindaddr, port, &ops, NULL);
  if (reg)
