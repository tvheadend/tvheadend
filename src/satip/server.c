...
static int satip_server_http_xml(http_connection_t* hc) {
#define MSG \
  "\
<?xml version=\"1.0\"?>\n\
...
%s\
  ...
  struct xml_type_xtab xtab[] = {{"DVBS", &satip_server_conf.satip_dvbs, &dvbs},
  ...
  if (devicelist == NULL || devicelist[0] == '\0') {
    tvhwarn(LS_SATIPS,
        "SAT>IP server announces an empty tuner list to a client %s (missing "
        "%s)",
        hc->hc_peer_ipstr,
        !tuners ? "tuner settings - global config" : "network assignment");
  }
  ...
  if (satip_server_rtsp_port != 554)
    snprintf(buf2,
        ...
        satip_server_conf.satip_uuid + 26);
  else
  ...
  if (satip_server_conf.satip_nom3u) {
    ...
    snprintf(purl,
        sizeof(purl),
        "<satip:X_SATIPM3U "
        "xmlns:satip=\"urn:ses-com:satip\">%s/%s</satip:X_SATIPM3U>\n",
        tvheadend_webroot ?: "",
        cs);
  }
...
static int satip_server_satip_m3u(http_connection_t* hc) {
  char path[PATH_MAX];
...
static void satips_upnp_send_byebye(void) {
#define MSG \
  "\
NOTIFY * HTTP/1.1\r\n\
...
static void satips_upnp_send_announce(void) {
#define MSG \
  "\
NOTIFY * HTTP/1.1\r\n\
HOST: 239.255.255.250:1900\r\n\
...
static void satips_upnp_send_discover_reply(struct sockaddr_storage* dst,
    const char*                                                      deviceid,
    int from_multicast) {
#define MSG \
  "\
...
CONFIGID.UPNP.ORG: 0\r\n"
  ...
  if (tvhtrace_enabled()) {
    ...
    tvhtrace(LS_SATIPS,
        "sending discover reply to %s:%d%s%s",
        buf,
        ntohs(IP_PORT(*dst)),
        deviceid ? " device: " : "",
        deviceid ?: "");
...
static void satips_upnp_discovery_received(uint8_t* data,
    size_t                                          len,
...
#define MSEARCH "M-SEARCH * HTTP/1.1"
  ...
  char* argv[10];
  char *st = NULL, *man = NULL, *host = NULL, *deviceid = NULL,
       *searchport = NULL;
  char buf2[64];
...
#undef MSEARCH
  ...
  if (tvhtrace_enabled()) {
    ...
    tvhtrace(LS_SATIPS,
        "received %s M-SEARCH from %s:%d",
        conn->multicast ? "multicast" : "unicast",
        buf2,
        ntohs(IP_PORT(*storage)));
  }
  ...
  if (!conn->multicast) {
    if (deviceid) {
      ...
      tcp_get_str_from_ip(storage, buf2, sizeof(buf2));
      tvhwarn(LS_SATIPS,
          "received duplicate SAT>IP DeviceID %s from %s:%d, using %d",
          deviceid,
          buf2,
          ntohs(IP_PORT(*storage)),
          satip_server_deviceid);
...
static void satips_upnp_discovery_destroy(upnp_service_t* upnp) {
...
static void satips_rtsp_port(int def) {
  ...
  if (!satip_server_rtsp_port_locked)
    rtsp_port =
        satip_server_conf.satip_rtsp > 0 ? satip_server_conf.satip_rtsp : def;
  if (getuid() != 0 && rtsp_port > 0 && rtsp_port < 1024 &&
      bound_port != rtsp_port) {
    tvherror(LS_SATIPS,
        "RTSP port %d specified but no root perms, using 9983",
        rtsp_port);
    rtsp_port = 9983;
  }
...
static void satip_server_info(const char* prefix, int descramble, int muxcnf) {
  ...
  tvhinfo(LS_SATIPS,
      ...
      http_server_ip,
      satip_server_rtsp_port);
  tvhinfo(LS_SATIPS, "  descramble %d, muxcnf %d", descramble, muxcnf);
  for (fe = 1; fe <= 128; fe++) {
...
static htsmsg_t* satip_server_class_muxcfg_list(void* o, const char* lang) {
  static const struct strtab tab[] = {{N_("Auto"), MUXCNF_AUTO},
      {N_("Keep"), MUXCNF_KEEP},
        ...
        .ic_properties =
            (const property_t[]){
                ...
                },
                {
                    ...
                    .name = N_("Anonymize"),
                    .desc = N_(
                        "Show only information for sessions which "
                ...
                },
                {
                    ...
                    .desc =
                        N_("Select how Tvheadend should handle muxes. See Help "
                           "for details."),
                ...
                },
                {
                    ...
                    .desc = N_(
                        "Select the payload size for RTP contents used "
                ...
                },
                {
                    ...
                    .desc  = N_("Enter external IP if behind Network address "
                               "translation (NAT). Asterisk (*) means accept "
                               "all IP addresses."),
                ...
                },
                {
                    ...
                    .desc = N_(
                        "Enter external PORT if behind Forwarding redirection."
                ...
                },
                {
                    ...
                    .name = N_("Force RTSP announcement of the external (NAT) "
                               "ip:port"),
                    .desc = N_(
                        "Advertise only NAT address and port in RTSP commands,"
                        "even for local connections."),
                    .off   = offsetof(struct satip_server_conf,
                        satip_nat_name_force),
                    .opts  = PO_EXPERT,
                    .group = 2,
                },
                {
                    ...
                    .desc = N_(
                        "Bind RTP source address of the outgoing RTP packets "
                ...
                },
                {
                    ...
                    .desc  = N_("Force signal level for all streaming (1-240, "
                               "0=do not use)."),
                    .off   = offsetof(struct satip_server_conf,
                        satip_force_sig_level),
                    .opts  = PO_EXPERT,
                    .group = 3,
                },
                {
                    ...
                    .id    = "satip_dvbs",
                    .name  = N_("DVB-S"),
                    .desc  = N_("The number of DVB-S (Satellite) tuners to "
                               "export."),
                ...
                },
                {
                    ...
                    .name = N_("DVB-S2"),
                    .desc = N_(
                        "The number of DVB-S2 (Satellite) tuners to export."),
                ...
                },
                {
                    ...
                    .name = N_("DVB-T"),
                    .desc = N_(
                        "The number of DVB-T (Terresterial) tuners to export."),
                ...
                },
                {
                    ...
                    .id    = "satip_dvbt2",
                    .name  = N_("DVB-T2"),
                    .desc  = N_("The number of DVB-T2 (Terresterial) tuners to "
                               "export."),
                ...
                },
                {
                    ...
                    .name = N_("DVB-C2"),
                    .desc =
                        N_("The number of DVB-C2 (Cable) tuners to export."),
                ...
                },
                {
                    ...
                    .id    = "satip_atsct",
                    .name  = N_("ATSC-T"),
                    .desc  = N_("The number of ATSC-T (Terresterial) tuners to "
                               "export."),
                ...
                },
                {
                    ...
                    .id    = "satip_atscc",
                    .name  = N_("ATSC-C"),
                    .desc  = N_("The number of ATSC-C (Cable/AnnexB) tuners to "
                               "export."),
                ...
                },
                {
                    ...
                    .id    = "satip_isdbt",
                    .name  = N_("ISDB-T"),
                    .desc  = N_("The number of ISDB-T (Terresterial) tuners to "
                               "export."),
                ...
                },
                {
                    ...
                    .desc = N_("The maximum number of active RTSP sessions "
                               "(if 0 no limit)."),
                    .off =
                        offsetof(struct satip_server_conf, satip_max_sessions),
                    .opts  = PO_ADVANCED,
                    .group = 4,
                },
                {
                    ...
                    .desc =
                        N_("The maximum concurrent RTSP connections from the "
                           "same IP address (if 0 no limit)."),
                    .off   = offsetof(struct satip_server_conf,
                        satip_max_user_connections),
                    .opts  = PO_ADVANCED,
                    .group = 4,
                },
                {
                    ...
                    .name = N_("Rewrite PMT"),
                    .desc =
                        N_("Rewrite Program Map Table (PMT) packets "
                           "to only include information about the currently "
                           "streamed service."),
                    .off =
                        offsetof(struct satip_server_conf, satip_rewrite_pmt),
                    .opts  = PO_EXPERT,
                    .group = 5,
                },
                {
                    ...
                    .desc  = N_("Do not send X_SATIPM3U information in the XML "
                ...
                },
                {
                    ...
                    .desc = N_(
                        "Remove server support for RTP/AVP/TCP transfer mode "
                ...
                },
                {
                    ...
                    .desc =
                        N_("Discard the frontend parameter in RTSP requests, "
...
static void satip_server_init_common(const char* prefix, int announce) {
  ...
  if (http_server_ip == NULL) {
    if (tcp_server_onall(http_server) && satip_server_bindaddr == NULL) {
      tvherror(LS_SATIPS,
          "use --satip_bindaddr parameter to select the local IP for SAT>IP");
      tvherror(LS_SATIPS,
          "using Google lookup (might block the task until timeout)");
    }
  ...
  satip_server_rtsp_init(http_server_ip,
      satip_server_rtsp_port,
      descramble,
      rewrite_pmt,
      muxcnf,
...
void satip_server_register(void) {
  ...
  if (strempty(satip_server_conf.satip_uuid)) {
    ...
    bin2hex(uu + 0, 9, u.bin, 4);
    uu[8] = '-';
    bin2hex(uu + 9, 5, u.bin + 4, 2);
    uu[13] = '-';
    bin2hex(uu + 14, 5, u.bin + 6, 2);
    uu[18] = '-';
    bin2hex(uu + 19, 5, u.bin + 8, 2);
    uu[23] = '-';
    bin2hex(uu + 24, 13, u.bin + 10, 6);
    uu[36] = 0;
