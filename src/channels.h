...
#define channel_find_by_uuid(u) (channel_t*)idnode_find(u, &channel_class, NULL)
...
char* channel_get_ename(channel_t* ch, char* dst, size_t dstlen, const char* blank, uint32_t flags);

#define CHANNEL_SPLIT ((int64_t)1000000)

static inline uint32_t channel_get_major(int64_t chnum) {
  return chnum / CHANNEL_SPLIT;
}
static inline uint32_t channel_get_minor(int64_t chnum) {
  return chnum % CHANNEL_SPLIT;
}
...
channel_t** channel_get_sorted_list_for_tag(const char* sort_type, channel_tag_t* tag, int* _count);
