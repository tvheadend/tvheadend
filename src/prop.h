...
#define PO_NOUI \
  (1 << 6) // Property should not be presented in the user interface
...
#define PO_DURATION \
  (1 << 12) // For PT_TIME - differentiate between duration and datetime
...
#define INTEXTRA_RANGE(min, max, step)                            \
  ((1 << 31U) | (((step)&0x7fU) << 24) | (((max)&0xfffU) << 12) | \
      ((min)&0xfffU))

#define INTEXTRA_IS_RANGE(e) (((e) & (1 << 31U)) != 0)
#define INTEXTRA_IS_SPLIT(e) !INTEXTRA_IS_RANGE(e)
#define INTEXTRA_GET_STEP(e) (((e) >> 24) & 0x7fU)
#define INTEXTRA_GET_MAX(e) \
  ((e) & (1 << 23) ? -(0x800 - (((e) >> 12) & 0x7ff)) : (((e) >> 12) & 0x7ff))
#define INTEXTRA_GET_MIN(e) \
  ((e) & (1 << 11) ? -(0x800 - ((e)&0x7ff)) : ((e)&0x7ff))
...
typedef struct property {
  ...
  uint32_t
      intextra; ///< intsplit: integer/remainder boundary or range: min/max/step
...
char* prop_md_doc(const char** md, const char* lang);

#define PROP_DOC(name)                                                       \
  extern const char* tvh_doc_##name##_property[];                            \
  static char* prop_doc_##name(const struct property* p, const char* lang) { \
