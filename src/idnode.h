...
struct idclass {
  ...
  void (*ic_get_title)(idnode_t* self,
      const char*                lang,
      char*                      dst,
      size_t                     dstsize);
...
typedef struct idnode_sort {
  ...
  enum { IS_ASC, IS_DSC } dir; ///< Sort direction
} idnode_sort_t;
...
typedef struct idnode_filter_ele {
  ...
  int   checked;
  char* key;                                     ///< Filter key
  enum { IF_STR, IF_NUM, IF_DBL, IF_BOOL } type; ///< Filter type
  union {
    int   b;
    char* s;
    struct {
...
#define idclass_serializedoc(idc, lang) \
  idclass_serialize0(idc, NULL, PO_DOC, lang)
#define idnode_serialize(in, lang) idnode_serialize0(in, NULL, 0, lang)
#define idnode_load(in, m)         idnode_write0(in, m, PO_NOSAVE, 0)
#define idnode_save(in, m) \
  idnode_read0(in, m, NULL, PO_NOSAVE | PO_USERAW, NULL)
...
static inline void idnode_perm_unset(idnode_t* self) {
  self->in_access = NULL;
}

#define idnode_lang(self) \
  (((idnode_t*)self)->in_access ? ((idnode_t*)self)->in_access->aa_lang : NULL)
#define idnode_lang_ui(self)                                               \
  (((idnode_t*)self)->in_access ? ((idnode_t*)self)->in_access->aa_lang_ui \
                                : NULL)
...
void idnode_filter_add_str(idnode_filter_t* f,
    const char*                             k,
    const char*                             v,
    int                                     t);
void idnode_filter_add_num(idnode_filter_t* f,
    const char*                             k,
    int64_t                                 s64,
    int                                     t,
    int64_t                                 intsplit);
void idnode_filter_add_dbl(idnode_filter_t* f,
    const char*                             k,
    double                                  dbl,
    int                                     t);
void idnode_filter_add_bool(idnode_filter_t* f, const char* k, int b, int t);
void idnode_filter_clear(idnode_filter_t* f);
int  idnode_filter(idnode_t* in, idnode_filter_t* filt, const char* lang);
