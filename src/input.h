...
struct tvh_input_stream_stats {
  int signal; ///< signal strength, value depending on signal_scale value:
              ...
              ///<  - SCALE DECIBEL  : 0.0001 dBm units. This value is generally
              ///<  negative.
  ...
  /* Note: if tc_bit > 0, BER = ec_bit / tc_bit (0...1) else BER = ber (driver
   * specific value) */
...
void* tvh_hardware_create0(void* o,
    const idclass_t*             idc,
    const char*                  uuid,
    htsmsg_t*                    conf);
...
static inline tvh_input_instance_t* tvh_input_instance_find_by_uuid(
    const char* uuid) {
  return (
      tvh_input_instance_t*)idnode_find(uuid, &tvh_input_instance_class, NULL);
}
