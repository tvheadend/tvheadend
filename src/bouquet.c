...
static int  bouquet_download_process(void* aux,
     const char*                           last_url,
     const char*                           host_url,
     char*                                 data,
     size_t                                len);
 ...
 *
 */
static int _bq_cmp(const void* a, const void* b) {
 ...
 *
 */
static void bouquet_free(bouquet_t* bq) {
 ...
 *
 */
bouquet_t* bouquet_create(const char* uuid,
    htsmsg_t*                         conf,
    const char*                       name,
    const char*                       src) {
  bouquet_t *         bq, *bq2;
  ...
  if (bq->bq_ext_url) {
    if (bq->bq_src && strncmp(bq->bq_src, "exturl://", 9) != 0) {
      ...
      snprintf(buf,
          sizeof(buf),
          "exturl://%s",
          idnode_uuid_as_str(&bq->bq_id, ubuf));
  ...
  if (bq2) {
    tvherror(LS_BOUQUET,
        "found duplicate source id: '%s', remove duplicate config",
        bq->bq_src);
    bouquet_free(bq);
    return NULL;
  }
 ...
 *
 */
static void bouquet_destroy(bouquet_t* bq) {
  if (!bq)
    return;
 ...
 *
 */
static channel_tag_t* bouquet_tag(bouquet_t* bq, int create) {
  channel_tag_t* ct;
  char           buf[128];
 ...
 *
 */
static int noname(const char* s) {
  if (!s)
    return 1;
  while (*s) {
    if (*s > ' ')
 ...
 *
 */
static void bouquet_map_channel(bouquet_t* bq, service_t* t) {
  channel_t*                   ch = NULL;
  idnode_list_mapping_t*       ilm;
  static service_mapper_conf_t sm_conf = {.check_availability = 0,
      .encrypted                                              = 1,
      .merge_same_name                                        = 0,
  ...
  if (!bq->bq_mapradio && service_is_radio(t))
    return;
  if (!bq->bq_mapnolcn && bouquet_get_channel_number(bq, t) <= 0)
    return;
...
static const char* bouquet_get_global_bouquet_src(void) {
...
/// DVB-T and DVB-S).
static bouquet_t* bouquet_get_global_bouquet(void) {
  static const char name[] = "Tvheadend Network";
  bouquet_t*        global_bq;
  enum { BOUQUET_CREATE = 1 };

  /* Still initializing, so don't do anything with a global bouquet yet
   * otherwise we run the risk of creating a new global bouquet before we load
   * the config from disk.
   */
  if (!bouquet_init_completed)
    return NULL;

  global_bq = bouquet_find_by_source(name,
      bouquet_get_global_bouquet_src(),
      BOUQUET_CREATE);
...
/// global bouquet.
static void bouquet_global_rescan_single_bouquet(const bouquet_t* bq,
    idnode_set_t*                                                 active_svcs) {
  size_t               z;
...
static void bouquet_global_rescan_cb(void* unused) {
  bouquet_t* global_bq = bouquet_get_global_bouquet();
  if (!global_bq)
    return;
  ...
  /* Now we have our list of services, remove any that should no longer exist in
   * the global bouquet.
   */
...
/// busy.
static void bouquet_global_rescan_i(int64_t mono) {
  static mtimer_t bouquet_global_bouquet_rescan_timer;
  mtimer_arm_rel(&bouquet_global_bouquet_rescan_timer,
      bouquet_global_rescan_cb,
      NULL,
      mono);
}

static void bouquet_global_rescan(void) {
  bouquet_global_rescan_i(sec2mono(5));
}

/// Even though we say "now", it could get delayed
/// if the system is busy.
static void bouquet_global_rescan_now(void) {
 ...
 *
 */
void bouquet_add_service(bouquet_t* bq,
    service_t*                      s,
    uint64_t                        lcn,
    const char*                     tag) {
  service_lcn_t*         tl;
  ...
  if (!idnode_set_exists(bq->bq_services, &s->s_id)) {
    tvhtrace(LS_BOUQUET,
        "add service %s [%p] to %s lcn %" PRIu64 "(.%" PRIu64 ")",
        s->s_nicename,
        s,
        bq->bq_name ?: "<unknown>",
        lcn / CHANNEL_SPLIT,
  ...
  if (!bq->bq_in_load && !idnode_set_exists(bq->bq_active_services, &s->s_id))
 ...
 *
 */
static void bouquet_unmap_channel(bouquet_t* bq, service_t* t) {
...
void bouquet_completed(bouquet_t* bq, uint32_t seen) {
  ...
  tvhtrace(LS_BOUQUET,
      "%s: completed: enabled=%d active=%zi old=%zi seen=%u",
      bq->bq_name ?: "",
      bq->bq_enabled,
      bq->bq_active_services->is_count,
  ...
  for (z = 0; z < bq->bq_services->is_count; z++)
    if (!idnode_set_exists(bq->bq_active_services,
            bq->bq_services->is_array[z]))
 ...
 *
 */
static const char* bouquet_get_tag_name(bouquet_t* bq, service_t* t) {
  return 0;
}
...
static void bouquet_class_delete(idnode_t* self) {
  bouquet_delete((bouquet_t*)self);
}

static void bouquet_class_get_title(idnode_t* self,
    const char*                               lang,
    char*                                     dst,
    size_t                                    dstsize) {
  bouquet_t* bq = (bouquet_t*)self;
...
static void bouquet_class_enabled_notify(void* obj, const char* lang) {
  bouquet_t* bq = obj;
   ...
   */
  bq == bouquet_get_global_bouquet() ? bouquet_global_rescan_now()
                                     : bouquet_global_rescan();
}

static void bouquet_class_maptoch_notify(void* obj, const char* lang) {
  bouquet_t* bq = obj;
  bouquet_map_to_channels(bq);
...
static void bouquet_class_lcn_offset_notify(void* obj, const char* lang) {
...
static htsmsg_t* bouquet_class_mapopt_enum(void* obj, const char* lang) {
  return idnode_slist_enum(obj, bouquest_class_mapopt_slist, lang);
}

static const void* bouquet_class_mapopt_get(void* obj) {
  return idnode_slist_get(obj, bouquest_class_mapopt_slist);
}

static char* bouquet_class_mapopt_rend(void* obj, const char* lang) {
  return idnode_slist_rend(obj, bouquest_class_mapopt_slist, lang);
}

static int bouquet_class_mapopt_set(void* obj, const void* p) {
  return idnode_slist_set(obj, bouquest_class_mapopt_slist, p);
}

static void bouquet_class_mapopt_notify(void* obj, const char* lang) {
  bouquet_t* bq = obj;
  service_t* t;
  size_t     z;
...
static htsmsg_t* bouquet_class_chtag_enum(void* obj, const char* lang) {
  return idnode_slist_enum(obj, bouquest_class_chtag_slist, lang);
}

static const void* bouquet_class_chtag_get(void* obj) {
  return idnode_slist_get(obj, bouquest_class_chtag_slist);
}

static char* bouquet_class_chtag_rend(void* obj, const char* lang) {
  return idnode_slist_rend(obj, bouquest_class_chtag_slist, lang);
}

static int bouquet_class_chtag_set(void* obj, const void* p) {
  return idnode_slist_set(obj, bouquest_class_chtag_slist, p);
}

static void bouquet_class_chtag_notify(void* obj, const char* lang) {
  bouquet_t*             bq = obj;
  service_t*             t;
  idnode_list_mapping_t* ilm;
  channel_tag_t*         ct;
...
static const void* bouquet_class_chtag_ref_get(void* obj) {
  bouquet_t* bq = obj;
...
static char* bouquet_class_chtag_ref_rend(void* obj, const char* lang) {
  bouquet_t* bq = obj;
  if (bq->bq_chtag_ptr)
...
static int bouquet_class_chtag_ref_set(void* obj, const void* p) {
  bouquet_t* bq = obj;
...
static const void* bouquet_class_services_get(void* obj) {
...
static char* bouquet_class_services_rend(void* obj, const char* lang) {
  bouquet_t*  bq = obj;
  const char* sc = N_("Service count %zi");
  char        buf[32];
  snprintf(buf,
      sizeof(buf),
      tvh_gettext_lang(lang, sc),
      bq->bq_services->is_count);
  return strdup(buf);
}

static int bouquet_class_services_set(void* obj, const void* p) {
  bouquet_t* bq = obj;
...
static const void* bouquet_class_services_count_get(void* obj) {
  static uint32_t u32;
  bouquet_t*      bq = obj;
...
static void bouquet_class_ext_url_notify(void* obj, const char* lang) {
...
const idclass_t bouquet_class = {.ic_class = "bouquet",
    ...
    .ic_properties =
        (const property_t[]){{
            ...
            },
            {
                ...
                .desc   = N_(
                    "Options to use/used when mapping - see Help for details."),
 ...
 *
 */
static void bouquet_download_trigger0(void* aux) {
  bouquet_download_t* bqd = aux;
  bouquet_t*          bq  = bqd->bq;

  download_start(&bqd->download, bq->bq_ext_url, bqd);
  mtimer_arm_rel(&bqd->timer,
      bouquet_download_trigger0,
      bqd,
...
static int parse_enigma2(bouquet_t* bq, char* data) {
  extern service_t* mpegts_service_find_e2(uint32_t stype,
      uint32_t                                      sid,
      uint32_t                                      tsid,
      uint32_t                                      onid,
      uint32_t                                      hash);
  ...
  tvhinfo(LS_BOUQUET,
      "parsed Enigma%d bouquet %s (%u services)",
      ver,
      bq->bq_name,
      seen);
  return 0;
