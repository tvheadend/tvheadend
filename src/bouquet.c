 ...
 *
 */
bouquet_t* bouquet_create(const char* uuid, htsmsg_t* conf, const char* name, const char* src) {
  bouquet_t *         bq, *bq2;
  bouquet_download_t* bqd;
...
/// global bouquet.
static void bouquet_global_rescan_single_bouquet(const bouquet_t* bq, idnode_set_t* active_svcs) {
  size_t               z;
...
static void bouquet_remove_service(bouquet_t* bq, service_t* s, int delconf) {
  bouquet_t* global_bq;
  tvhtrace(LS_BOUQUET, "remove service %s from %s", s->s_nicename, bq->bq_name ?: "<unknown>");
...
static htsmsg_t* bouquet_class_save(idnode_t* self, char* filename, size_t fsize) {
...
static void bouquet_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
  bouquet_t* bq = (bouquet_t*)self;
