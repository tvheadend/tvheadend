...
int service_mapper_link(service_t* s, channel_t* c, void* origin) {
  ...
  ilm = idnode_list_link(&s->s_id, &s->s_channels, &c->ch_id, &c->ch_services, origin, 2);
  if (ilm) {
    service_mapped(s);
    return 1;
  }
 ...
 * Process a service
 */
channel_t* service_mapper_process(const service_mapper_conf_t* conf, service_t* s, bouquet_t* bq) {
  channel_t*      chn = NULL;
 ...
 * Save settings
 */
static htsmsg_t* service_mapper_conf_class_save(idnode_t* self, char* filename, size_t fsize) {
  htsmsg_t* m;
...
static const idclass_t service_mapper_conf_class = {.ic_snode = &service_mapper_conf.idnode,
