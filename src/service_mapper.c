 ...
 * Remove service
 */
void service_mapper_remove(service_t* s) {
  service_mapper_item_t* smi;
 ...
 */
int service_mapper_link(service_t* s, channel_t* c, void* origin) {
  idnode_list_mapping_t* ilm;

  ilm = idnode_list_link(&s->s_id,
      &s->s_channels,
      &c->ch_id,
      &c->ch_services,
      origin,
 ...
 * Process a service
 */
channel_t* service_mapper_process(const service_mapper_conf_t* conf,
    service_t*                                                 s,
    bouquet_t*                                                 bq) {
  channel_t*      chn = NULL;
    ...
    if (conf->type_tags) {
      if (service_is_uhdtv(s)) {
        ...
        if (!channel_tag_map(channel_tag_find_by_name("Radio", 0), chn, chn))
          channel_tag_map(channel_tag_find_by_name("Radio channels", 1),
              chn,
              chn);
      }
    }
 ...
 *
 */
static void* service_mapper_thread(void* aux) {
  service_t*             s;
  service_mapper_item_t* smi;
  ...
  while (tvheadend_is_running()) {
    ...
    prch.prch_id = s;
    sub          = subscription_create_from_service(&prch,
        NULL,
...
static const void* service_mapper_services_get(void* obj) {
  return NULL;
}

static char* service_mapper_services_rend(void* obj, const char* lang) {
  return strdup("");
}

static int service_mapper_services_set(void* obj, const void* p) {
...
static htsmsg_t* service_mapper_services_enum(void* obj, const char* lang) {
...
static const idclass_t service_mapper_conf_class = {
    ...
    .ic_properties = (const property_t[]){
        ...
        },
        {
            ...
            .desc =
                N_("If merge same name is enabled then "
        ...
        },
        {
            ...
            .name =
                N_("Tidy the channel name such as removing trailing HD text"),
            .desc = N_(
                "Some broadcasters distinguish channels by appending "
...
void service_mapper_init(void) {
  ...
  tvh_thread_create(&service_mapper_tid,
      NULL,
      service_mapper_thread,
      NULL,
      "svcmap");
