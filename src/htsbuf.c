 ...
 */
void htsbuf_vqprintf(htsbuf_queue_t* hq, const char* fmt, va_list ap0) {
 ...
 *
 */
void htsbuf_append_and_escape_xml(htsbuf_queue_t* hq, const char* s) {
  const char* c   = s;
  ...
  while (c < e) {
    ...
    if (h & 0x80) {
      ...
      const int num_bytes = ((h & 240) == 240)
          ? 4
          : ((h & 224) == 224) ? 3 : ((h & 192) == 192) ? 2 : 0;
      ...
      if (!num_bytes) {
        ...
        if (e - c < num_bytes - 1) {
          // Invalid sequence - too few characters left in buffer for the
          // sequence. Append what we already have accumulated and ignore
 ...
 *
 */
void htsbuf_append_and_escape_jsonstr(htsbuf_queue_t* hq, const char* str) {
  const char* s = str;
