...
int rtsp_send_ext(http_client_t* hc,
    ...
    size_t                       size) {
  http_arg_list_t h;
  size_t blen = 7 + strlen(hc->hc_host) + (hc->hc_port != 554 ? 7 : 0) +
  ...
  for (i = 1; i < n; i++) {
    if (strncasecmp(argv[i], "timeout=", 8) == 0) {
      ...
      if (hc->hc_rtp_timeout < 20 || hc->hc_rtp_timeout > 3600) {
        tvhwarn(LS_RTSP,
            "timeout value out of range 20-3600 (%i)",
            hc->hc_rtp_timeout);
        return -EIO;
      }
    }
...
int rtsp_describe_decode(http_client_t* hc, const char* buf, size_t len) {
  ...
  for (n = 0; n < len; n++) {
    ...
    if (strncmp(p, "m=video", 7) == 0) {
      // Parse and select RTP/AVPF stream if available for retransmission
      // support
      if (sscanf(p, "m=video %d %s %d\n", &t, transport, &transport_type) ==
          3) {
        tvhtrace(LS_RTSP,
            "describe: found transport: %d %s %d",
            t,
            transport,
            transport_type);
