/* Compile as follows:
 * gcc -O -fbuiltin -fomit-frame-pointer -fPIC -shared -o capmt_ca.so capmt_ca.c
 * -ldl
 *
...
#define PROGINFO()                                      \
  {                                                     \
    if (!proginfo) {                                    \
      syslog(LOG_INFO, "capmt_ca.so for camd clients"); \
      proginfo = 1;                                     \
    }                                                   \
  }

int sendcw(int fd, unsigned char* buff) {
  DBG(">>>>>>>> sendcw %02x%02x %02x%02x%02x%02x%02x%02x%02x%02x "
...
static int cactl(int fd, int cai, int request, void* argp) {
  ...
  switch (request) {
    case CA_SET_DESCR:
      DBG(">>>>>>>> cactl CA_SET_DESCR fd %d cai %d req %d par %d idx %d "
          "%02x%02x%02x%02x%02x%02x%02x%02x",
          fd,
          cai,
          request,
          ...
          ca->cw[7]);
      if (ca->parity == 0)
        memcpy(&ca_info[cai][ca->index][EVEN_OFF],
            ca->cw,
            KEY_SIZE); // even key
      else if (ca->parity == 1)
        memcpy(&ca_info[cai][ca->index][ODD_OFF], ca->cw, KEY_SIZE); // odd key
      else
        ERR("ca.so: Invalid parity %d in CA_SET_DESCR for ca id %d",
            ca->parity,
            cai);
      sendcw(fd, ca_info[cai][ca->index]);
      break;
    case CA_SET_PID:
      DBG(">>>>>>>> cactl CA_SET_PID fd %d cai %d req %d (%d %04x)",
          fd,
          cai,
          request,
          cpd->index,
      ...
      if (cpd->index >= 0 && cpd->index < MAX_INDEX) {
        ...
        ERR("ca.so: Invalid index %d in CA_SET_PID (%d) for ca id %d",
            cpd->index,
            MAX_INDEX,
            cai);
      return 1;
