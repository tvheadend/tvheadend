...
void hts_settings_save(htsmsg_t* record, const char* pathfmt, ...) {
  ...
  if ((fd = tvh_open(tmppath, O_CREAT | O_TRUNC | O_RDWR, S_IRUSR | S_IWUSR)) <
      0) {
  ...
  if (!pack) {
    ...
    if (!r && msglen >= 4) {
      ...
      tvhalert(LS_SETTINGS,
          "Unable to pack the configuration data \"%s\"",
          path);
    }
    free(msgdata);
#endif
...
#if ENABLE_ZLIB
      uint32_t orig =
          (mem[8] << 24) | (mem[9] << 16) | (mem[10] << 8) | mem[11];
      if (orig > 10 * 1024 * 1024U) {
        tvhalert(LS_SETTINGS, "too big gzip for %s", filename);
        r = NULL;
      } else if (orig > 0) {
        uint8_t* unpacked =
            tvh_gzip_inflate((uint8_t*)mem + 12, size - 12, orig);
        if (unpacked) {
          if (mem[7] == '1') {
...
hts_settings_vload(const char* pathfmt, va_list ap, int depth) {
  ...
  /* Try normal path */
  _hts_settings_buildpath(fullpath,
      sizeof(fullpath),
      pathfmt,
      ap,
      settingspath);
...
int hts_settings_open_file(int flags, const char* pathfmt, ...) {
  ...
  /* Open file */
  _flags = (flags & HTS_SETTINGS_OPEN_WRITE) ? O_CREAT | O_TRUNC | O_WRONLY
                                             : O_RDONLY;
