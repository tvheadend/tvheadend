...
void http_client_unpause(http_client_t* hc) {
  if (hc->hc_pause) {
    ...
    http_client_poll_dir(hc,
        pevents_pause & TVHPOLL_IN,
...
static void http_client_cmd_destroy(http_client_t* hc,
    http_client_wcmd_t*                            cmd) {
...
header:
  *d                 = '\0';
  ...
  if (p && ver != RTSP_VERSION_1_0) {
    if (http_arg_contains(p, "close") ||
        http_arg_contains(p, "upgrade")) /* Some servers
...
#define BASIC "Basic "
    ...
    strcat(buf, pass);
    strcpy(cbuf, BASIC);
    base64_encode(cbuf + sizeof(BASIC) - 1,
        len,
        (uint8_t*)buf,
        ulen + 1 + plen);
...
void http_client_add_args(http_client_t* hc,
    http_arg_list_t*                     h,
    const char*                          args) {
  char *p, *k, *v;
  ...
  while (*p) {
    ...
    k = strstrip(k);
    v = strstrip(v);
    if (v && *v && *k && strcasecmp(k, "Connection") != 0 &&
        strcasecmp(k, "Host") != 0) {
...
int http_client_simple_reconnect(http_client_t* hc,
    const url_t*                                u,
    http_ver_t                                  ver) {
  http_arg_list_t h;
  tvhpoll_t*      efd;
  ...
  if (tvh_str_default(u->scheme, NULL) == NULL ||
      tvh_str_default(u->host, NULL) == NULL || u->port < 0) {
...
static int http_client_redirected(http_client_t* hc) {
  ...
  if (location[0] == '\0' || location[0] == '/') {
    size_t size2 = strlen(hc->hc_scheme) + 3 + strlen(hc->hc_host) + 12 +
        strlen(location) + 1;
...
void http_client_ssl_peer_verify(http_client_t* hc, int verify) {
  ...
  if (hc->hc_verify_peer < 0) {
    ...
    tvherror(LS_HTTPC,
        "%04X: SSL peer verification method must be set only once",
        shortid(hc));
  }
}
...
static int http_client_reconnect(http_client_t* hc,
  ...
  hc->hc_fd =
      tcp_connect(host, port, hc->hc_bindaddr, errbuf, sizeof(errbuf), -1);
  if (hc->hc_fd < 0) {
    tvherror(LS_HTTPC,
        "%04X: Unable to connect to %s:%i - %s",
        shortid(hc),
        host,
        port,
        errbuf);
...
static int http_client_testsuite_hdr_received(http_client_t* hc) {
  ...
  fprintf(stderr,
      "HTTPCTS: Received header from %s:%i\n",
      hc->hc_host,
      hc->hc_port);
  ...
  while (fgets(line, sizeof(line), fp) != NULL && tvheadend_is_running()) {
    ...
    rep:
      ...
      while (tvheadend_is_running()) {
        ...
        fprintf(stderr,
            "HTTPCTS: Leave Poll: %i (%s)\n",
            r,
            r < 0 ? val2str(-r, ERRNO_tab) : "OK");
