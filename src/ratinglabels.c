 ...
 */
ratinglabel_t*
ratinglabel_create(const char* uuid, htsmsg_t* conf, const char* name, const char* src) {
...
void ratinglabel_delete(ratinglabel_t* rl) {
  ...
  if (!rl->rl_shield) {
    ...
    // Read through all of the EPG entries and set the rating label pointer to NULL for matching
    // labels. If this is not done, if an EPG entry is called that has the deleted RL, then the
    // pointer will point to rubbish and TVH will most likely crash. Note: In order to delete the RL
    // object, the mutex is already locked.
...
static htsmsg_t* ratinglabel_class_save(idnode_t* self, char* filename, size_t fsize) {
  ...
  // This is a workaround so that I can evaluate retVal and not have the compiler warning kill my
  // buzz. If the RL has changed, the EPG entry will be updated, I just need a variable to hold the
  // return code from epg_broadcast_set_age_rating even though I'm not going to use it for anything.
  retVal = 0;
  ...
  LIST_FOREACH (de, &dvrentries, de_global_link) {
    if (dvr_entry_is_upcoming(de)) {
      if (de->de_rating_label == rl) {
        tvhtrace(LS_RATINGLABELS,
            "Updating rating label for scheduled recording '%s', from '%s' / '%d' to '%s' / '%d'.",
            de->de_title->first->str,
...
static void
ratinglabel_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
  ratinglabel_t* rl = (ratinglabel_t*)self;
