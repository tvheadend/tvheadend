...
ratinglabel_t* ratinglabel_find_from_xmltv(const char* authority,
    const char*                                        label);
...
ratinglabel_t* ratinglabel_create_placeholder(int enabled,
    const char*                                   country,
    int                                           age,
    int                                           display_age,
 ...
 *
 */
static int _rl_cmp(const void* a, const void* b) {
  return 1;
}
...
ratinglabel_t* ratinglabel_find_from_uuid(const char* string_uuid) {
  ...
  if (string_uuid[0] == 0) {
    tvhtrace(LS_RATINGLABELS,
        "Empty UUID when matching rating label, exiting.");
    return NULL;
  }
  ...
  if (!uuid_set(&binary_uuid, string_uuid)) {
    ...
    RB_FOREACH (temp_rl, &ratinglabels, rl_link) {
      if (!memcmp((const void*)&binary_uuid,
              (const void*)(temp_rl->rl_id).in_uuid.bin,
              UUID_BIN_SIZE)) {
        rl = temp_rl;
        if (rl->rl_enabled) {
          tvhdebug(LS_RATINGLABELS,
              "Matched UUID '%s' with rating label '%s' / '%d'.",
              string_uuid,
              rl->rl_display_label,
              rl->rl_display_age);
          return rl;
  ...
  // To get here, either the UUID was invalid or there was no matching ENABLED
  // ratinglabel.
  return NULL;
}
...
ratinglabel_t* ratinglabel_find_from_xmltv(const char* authority,
  ...
  RB_FOREACH (temp_rl, &ratinglabels, rl_link) {
    ...
    if ((!authority && !temp_rl->rl_authority) ||
        !strcasecmp(temp_rl->rl_authority, authority)) {
  ...
  if (rl) {
    ...
    rl = ratinglabel_create_placeholder(1,
        "???",
        0,
        0,
        tmpLabel,
...
ratinglabel_t* ratinglabel_find_from_eit(char* country, int age) {
  tvhdebug(LS_RATINGLABELS,
      "Looking for '%s', '%d'.  Count: '%d'.",
      country,
      age,
      ratinglabels.entries);
  ...
  if (rl) {
    ...
    tvhtrace(LS_RATINGLABELS,
        "Not found, creating placeholder '%s' / '%d'.  Count: '%d' ",
        country,
        age,
        ratinglabels.entries);
...
ratinglabel_t* ratinglabel_create_placeholder(int enabled,
  ...
  if (rl_new) {
    tvhtrace(LS_RATINGLABELS,
        "Success: Created placeholder '%s' / '%d' : '%s / '%s'.  Count: '%d'",
        country,
        age,
        label,
        authority,
 ...
 */
static void ratinglabel_free(ratinglabel_t* rl) {
 ...
 */
ratinglabel_t* ratinglabel_create(const char* uuid,
    htsmsg_t*                                 conf,
    const char*                               name,
    const char*                               src) {
 ...
 */
static void ratinglabel_destroy(ratinglabel_t* rl) {
...
void ratinglabel_delete(ratinglabel_t* rl) {
  ...
  if (!rl->rl_shield) {
    hts_settings_remove("epggrab/ratinglabel/%s",
        idnode_uuid_as_str(&rl->rl_id, ubuf));
    ...
    // Read through all of the EPG entries and set the rating label pointer to
    // NULL for matching labels. If this is not done, if an EPG entry is called
    // that has the deleted RL, then the pointer will point to rubbish and TVH
    // will most likely crash. Note: In order to delete the RL object, the mutex
    // is already locked.
    ...
    tvhtrace(LS_RATINGLABELS,
        "Found '%d' EPG entries when deleting rating label.",
        foundCount);

    // Now check for any upcomming recordings that use this RL and remove their
    // RL UUID.
    foundCount = 0;
    ...
    LIST_FOREACH (de, &dvrentries, de_global_link) {
      if (dvr_entry_is_upcoming(de)) {
        if (de->de_rating_label == rl) {
          tvhtrace(LS_RATINGLABELS,
              "Removing rating label for scheduled recording '%s'.",
              de->de_title->first->str);
          de->de_rating_label =
              NULL; // Set the rating label UUID for this recording to null
    ...
    tvhtrace(LS_RATINGLABELS,
        "Found '%d' upcomming recordings when deleting rating label.",
        foundCount);
...
ratinglabel_class_save(idnode_t* self, char* filename, size_t fsize) {
  ...
  // This is a workaround so that I can evaluate retVal and not have the
  // compiler warning kill my buzz. If the RL has changed, the EPG entry will be
  // updated, I just need a variable to hold the return code from
  // epg_broadcast_set_age_rating even though I'm not going to use it for
  // anything.
  retVal = 0;
  ...
  tvhtrace(LS_RATINGLABELS,
      "Found '%d' EPG entries when updating rating label.",
      foundCount);

  foundCount = 0;

  // Check for upcomming recordings that need their saved RL details to be
  // updated.
  LIST_FOREACH (de, &dvrentries, de_global_link) {
    if (dvr_entry_is_upcoming(de)) {
      if (de->de_rating_label == rl) {
        tvhtrace(LS_RATINGLABELS,
            "Updating rating label for scheduled recording '%s', from '%s' / "
            "'%d' to '%s' / '%d'.",
            de->de_title->first->str,
            de->de_rating_label_saved,
            de->de_age_rating,
            rl->rl_display_label,
            rl->rl_display_age);
        ...
        // If this RL has an icon, save that, else, ensure that the recording's
        // RL icon is empty.
        if (rl->rl_icon) {
        ...
        dvr_entry_changed(de); // Save this recording and push it out to
                               // clients.
        foundCount++;
  ...
  tvhtrace(LS_RATINGLABELS,
      "Found '%d' upcomming recordings when updating rating label.",
      foundCount);

  return c;
}

static void ratinglabel_class_delete(idnode_t* self) {
...
static void ratinglabel_class_get_title(idnode_t* self,
    const char*                                   lang,
    char*                                         dst,
    size_t                                        dstsize) {
  ratinglabel_t* rl = (ratinglabel_t*)self;
...
static void ratinglabel_class_enabled_notify(void* obj, const char* lang) {
  int            a  = 1;
...
const idclass_t ratinglabel_class = {.ic_class = "ratinglabel",
    ...
    .ic_properties = (const property_t[]){
        ...
        },
        {
            ...
            .name = N_("Label"),
            .desc = N_(
                "XML 'rating' tag value to match events received via XMLTV."),
        ...
        },
        {
            ...
            .name = N_("Authority"),
            .desc = N_(
                "XMLTV 'system' attribute to match events received via XMLTV."),
...
void ratinglabel_init(void) {
  ...
  if ((c = hts_settings_load("epggrab/ratinglabel")) != NULL) {
    HTSMSG_FOREACH(f, c) {
      ...
      if (rl) {
        tvhtrace(LS_RATINGLABELS,
            "Loaded label: '%s' / '%d', enabled: '%d', label count: '%d'",
            rl->rl_display_label,
            rl->rl_display_age,
            rl->rl_enabled,
            ratinglabels.entries);
