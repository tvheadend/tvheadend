...
static int _str_cmp(void* a, void* b) {
  return strcmp(((dvr_inotify_entry_t*)a)->path,
      ((dvr_inotify_entry_t*)b)->path);
}
...
void dvr_inotify_init(void) {
  ...
  tvh_thread_create(&dvr_inotify_tid,
      NULL,
      _dvr_inotify_thread,
      NULL,
      "dvr-inotify");
...
static void dvr_inotify_add_one(dvr_entry_t* de, htsmsg_t* m) {
  ...
  if (!dvr_inotify_exists(e, de)) {
    ...
    if (e->fd < 0) {
      tvherror(LS_DVR,
          "failed to add inotify watch to %s (err=%s)",
          e->path,
          strerror(errno));
      dvr_inotify_del(de);
    } else {
 ...
 */
static void
_dvr_inotify_moved(int from_fd, const char* from, const char* to, int to_fd) {
  dvr_inotify_filename_t* dif;
