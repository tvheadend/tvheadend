 ...
 * Title
 */
static const char* dvr_timerec_title(dvr_timerec_entry_t* dte, struct tm* start) {
  size_t len;
...
static htsmsg_t* dvr_timerec_entry_class_save(idnode_t* self, char* filename, size_t fsize) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)self;
...
static int dvr_timerec_entry_class_perm(idnode_t* self, access_t* a, htsmsg_t* msg_to_write) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)self;
...
static void
dvr_timerec_entry_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)self;
  const char*          s   = "";
...
static char* dvr_timerec_entry_class_config_name_rend(void* o, const char* lang) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  if (dte->dte_config)
...
static uint32_t dvr_timerec_entry_class_owner_opts(void* o, uint32_t opts) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  if (dte && dte->dte_id.in_access && !access_verify2(dte->dte_id.in_access, ACCESS_ADMIN))
    return PO_ADVANCED;
