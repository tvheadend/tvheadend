 ...
 *
 */
static time_t dvr_timerec_timecorrection(time_t clk, int hm, struct tm* tm) {
  time_t r;
  int    isdst;
 ...
 */
static void dvr_timerec_purge_spawn(dvr_timerec_entry_t* dte, int delconf) {
 ...
 * Title
 */
static const char* dvr_timerec_title(dvr_timerec_entry_t* dte,
    struct tm*                                            start) {
  size_t len;
 ...
 */
void dvr_timerec_check(dvr_timerec_entry_t* dte) {
  dvr_entry_t* de;
  ...
  if (dte->dte_enabled == 0 || dte->dte_weekdays == 0)
    goto fail;
  if (dte->dte_start < 0 || dte->dte_start >= 24 * 60 || dte->dte_stop < 0 ||
      dte->dte_stop >= 24 * 60)
    goto fail;
 ...
 *
 */
dvr_timerec_entry_t* dvr_timerec_create(const char* uuid, htsmsg_t* conf) {
...
dvr_timerec_entry_t* dvr_timerec_create_htsp(htsmsg_t* conf) {
  dvr_timerec_entry_t* dte;
...
void dvr_timerec_update_htsp(dvr_timerec_entry_t* dte, htsmsg_t* conf) {
  ...
  idnode_changed(&dte->dte_id);
  tvhinfo(LS_DVR,
      "timerec \"%s\" on \"%s\": Updated",
      dte->dte_title ? dte->dte_title : "",
      (dte->dte_channel && dte->dte_channel->ch_name)
          ? dte->dte_channel->ch_name
          : "any channel");
}
 ...
 *
 */
static void timerec_entry_destroy(dvr_timerec_entry_t* dte, int delconf) {
  ...
  if (delconf)
    hts_settings_remove("dvr/timerec/%s",
...
static void dvr_timerec_entry_class_changed(idnode_t* self) {
...
static void dvr_timerec_entry_class_delete(idnode_t* self) {
  timerec_entry_destroy((dvr_timerec_entry_t*)self, 1);
}

static int dvr_timerec_entry_class_perm(idnode_t* self,
    access_t*                                     a,
...
static void dvr_timerec_entry_class_get_title(idnode_t* self,
    const char*                                         lang,
    char*                                               dst,
    size_t                                              dstsize) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)self;
...
static int dvr_timerec_entry_class_channel_set(void* o, const void* v) {
  ...
  if (ch == NULL) {
    ...
    if (dte->dte_id.in_access && !channel_access(ch, dte->dte_id.in_access, 1))
      return 0;
    if (dte->dte_channel)
      LIST_REMOVE(dte, dte_channel_link);
    dte->dte_channel = ch;
...
static const void* dvr_timerec_entry_class_channel_get(void* o) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  if (dte->dte_channel)
...
static char* dvr_timerec_entry_class_channel_rend(void* o, const char* lang) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  if (dte->dte_channel)
...
static int dvr_timerec_entry_class_time_set(void* o, const void* v, int* tm) {
  const char* s = v;
  int         t;
...
static int dvr_timerec_entry_class_start_set(void* o, const void* v) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  return dvr_timerec_entry_class_time_set(o, v, &dte->dte_start);
}

static int dvr_timerec_entry_class_stop_set(void* o, const void* v) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  return dvr_timerec_entry_class_time_set(o, v, &dte->dte_stop);
}

static const void* dvr_timerec_entry_class_time_get(void* o, int tm) {
  if (tm >= 0)
...
static const void* dvr_timerec_entry_class_start_get(void* o) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  return dvr_timerec_entry_class_time_get(o, dte->dte_start);
}

static const void* dvr_timerec_entry_class_stop_get(void* o) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  return dvr_timerec_entry_class_time_get(o, dte->dte_stop);
}

static htsmsg_t* dvr_timerec_entry_class_time_list(void* o, const char* lang) {
  return dvr_autorec_entry_class_time_list(o,
      tvh_gettext_lang(lang, N_("Invalid")));
}

static int dvr_timerec_entry_class_config_name_set(void* o, const void* v) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
...
static const void* dvr_timerec_entry_class_config_name_get(void* o) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  if (dte->dte_config)
...
static char* dvr_timerec_entry_class_config_name_rend(void* o,
    const char*                                             lang) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  if (dte->dte_config)
...
static int dvr_timerec_entry_class_weekdays_set(void* o, const void* v) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  htsmsg_field_t*      f;
  uint32_t             u32, bits = 0;
...
static const void* dvr_timerec_entry_class_weekdays_get(void* o) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  return dvr_autorec_entry_class_weekdays_get(dte->dte_weekdays);
}

static htsmsg_t* dvr_timerec_entry_class_weekdays_default(void) {
  return dvr_autorec_entry_class_weekdays_get(0x7f);
}

static char* dvr_timerec_entry_class_weekdays_rend(void* o, const char* lang) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
  return dvr_autorec_entry_class_weekdays_rend(dte->dte_weekdays, lang);
}

static uint32_t dvr_timerec_entry_class_owner_opts(void* o, uint32_t opts) {
  dvr_timerec_entry_t* dte = (dvr_timerec_entry_t*)o;
...
const idclass_t dvr_timerec_entry_class = {.ic_class = "dvrtimerec",
    ...
    .ic_properties = (const property_t[]){
        ...
        {.type     = PT_STR,
            .id    = "title",
            .name  = N_("Title"),
            .desc  = N_("Title of the recording - this is used to generate the "
                       "filename."),
        ...
        },
        {
            ...
            .name = N_("Start"),
            .desc =
                N_("Time to start the recording/time the recording started."),
...
static void dvr_timerec_timer_cb(void* aux) {
  dvr_timerec_entry_t* dte;

  tvhtrace(LS_DVR, "timerec update");

  /* check all entries */
  TAILQ_FOREACH (dte, &timerec_entries, dte_link) { dvr_timerec_check(dte); }

  /* load the timer */
  mtimer_arm_rel(&dvr_timerec_timer,
      dvr_timerec_timer_cb,
      NULL,
      sec2mono(3550));
}
 ...
 *
 */
void timerec_destroy_by_config(dvr_config_t* kcfg, int delconf) {
  dvr_timerec_entry_t* dte;
