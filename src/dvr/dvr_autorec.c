...
uint32_t dvr_autorec_get_max_sched_count(dvr_autorec_entry_t* dae) {
  ...
  if (max_count == 0)
    max_count =
        dae->dae_config ? dae->dae_config->dvr_autorec_max_sched_count : 0;
  return max_count;
}
...
void dvr_autorec_purge_obsolete_timers(void) {
  ...
  if (num_purged)
    tvhinfo(LS_DVR,
        "Purged %d autorec entries that no longer match schedule",
        num_purged);
}
...
int dvr_autorec_cmp(dvr_autorec_entry_t* dae, epg_broadcast_t* e) {
  ...
  if (dae->dae_channel == NULL && dae->dae_channel_tag == NULL &&
      ...
      dae->dae_minyear <= 0 && dae->dae_maxyear <= 0 &&
      dae->dae_minseason <= 0 && dae->dae_maxseason <= 0 &&
      dae->dae_serieslink_uri == NULL)
    return 0; // Avoid super wildcard match

  if (dae->dae_serieslink_uri) {
    if (!e->serieslink ||
        strcmp(dae->dae_serieslink_uri ?: "", e->serieslink->uri))
      return 0;
  }
  ...
  if (e->category) {
    ...
    if (dae->dae_cat3 && *dae->dae_cat3 &&
      ...
      (dae->dae_cat2 && *dae->dae_cat2) || (dae->dae_cat3 && *dae->dae_cat3)) {
...
dvr_autorec_entry_t* dvr_autorec_create(const char* uuid, htsmsg_t* conf) {
  ...
  dvr_config_t* c =
      dvr_config_find_by_uuid(htsmsg_get_str(conf, "config_name"));
...
static void autorec_entry_destroy(dvr_autorec_entry_t* dae, int delconf) {
  ...
  if (delconf)
    hts_settings_remove("dvr/autorec/%s",
...
static int dvr_autorec_entry_class_perm(idnode_t* self,
    access_t*                                     a,
    htsmsg_t*                                     msg_to_write) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)self;
...
static void dvr_autorec_entry_class_get_title(idnode_t* self,
    const char*                                         lang,
    char*                                               dst,
    size_t                                              dstsize) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)self;
...
static htsmsg_t* dvr_autorec_entry_class_minduration_list(void* o,
    const char*                                                 lang) {
  const char* msg = N_("Any");
  return dvr_entry_class_duration_list(o,
      tvh_gettext_lang(lang, msg),
...
static htsmsg_t* dvr_autorec_entry_class_maxduration_list(void* o,
    const char*                                                 lang) {
  const char* msg = N_("Any");
  return dvr_entry_class_duration_list(o,
      tvh_gettext_lang(lang, msg),
...
static char* dvr_autorec_entry_class_config_name_rend(void* o,
    const char*                                             lang) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  if (dae->dae_config)
...
static htsmsg_t* dvr_autorec_entry_class_star_rating_list(void* o,
    const char*                                                 lang) {
  htsmsg_t* m = htsmsg_create_list();
...
static htsmsg_t* dvr_autorec_entry_class_content_type_list(void* o,
    const char*                                                  lang) {
  htsmsg_t* m = htsmsg_create_map();
...
htsmsg_t* dvr_autorec_entry_class_dedup_list(void* o, const char* lang) {
...
#define CATEGORY_SELECTION_PROP(NUM)                                          \
  .type = PT_STR, .id = "cat" #NUM,                                           \
  .name = N_("Category " #NUM " (selection)"),                                \
  .desc = N_(                                                                 \
      "The category of the program to look for. The xmltv "                   \
...
const idclass_t dvr_autorec_entry_class = {.ic_class = "dvrautorec",
    ...
    .ic_properties = (const property_t[]){
        ...
        {CATEGORY_SELECTION_PROP(3)},
        {
            ...
            .name = N_("Full-text"),
            .desc =
                N_("When the fulltext is checked, the title pattern is "
        ...
        },
        {
            ...
            .name = N_("Star rating"),
            .desc =
                N_("The minimum number of stars the broadcast should have - in "
        ...
        },
        {
            ...
            .desc = N_("The earliest year for the programme. Programmes must "
                       "be equal to or later than this year."),
        ...
        },
        {
            ...
            .desc = N_("The latest year for the programme. Programmes must be "
                       "equal to or earlier than this year."),
            .list = dvr_autorec_entry_class_year_list,
        ...
        },
        {
            ...
            .desc = N_("The earliest season for the programme. Programmes must "
                       "be equal to or later than this season."),
        ...
        },
        {
            ...
            .desc = N_("The latest season for the programme. Programmes must "
                       "be equal to or earlier than this season."),
...
void dvr_autorec_async_reschedule(void) {
   ...
   * rule.
   */
  mtimer_arm_rel(&reschedule_timer,
      dvr_autorec_async_reschedule_cb,
      NULL,
      sec2mono(60));
}
