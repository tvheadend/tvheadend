 ...
 */
static epg_broadcast_t** dvr_autorec_purge_spawns(dvr_autorec_entry_t* dae, int del, int disabled) {
  dvr_entry_t*      de;
...
void dvr_autorec_purge_obsolete_timers(void) {
  ...
  LIST_FOREACH (de, &dvrentries, de_global_link) {
    if (dvr_autorec_entry_can_be_purged(de)) {
      ...
      tvhinfo(LS_DVR,
          "Entry %s can be purged for \"%s\" start %s stop %s",
...
void dvr_autorec_completed(dvr_autorec_entry_t* dae, int error_code) {
  ...
  while (1) {
    ...
    if (de_prev) {
      tvhinfo(LS_DVR,
          "autorec %s removing recordings %s (allowed count %u total %u)",
          dae->dae_name,
          idnode_uuid_as_str(&de_prev->de_id, ubuf),
          max_count,
          total);
...
int dvr_autorec_cmp(dvr_autorec_entry_t* dae, epg_broadcast_t* e) {
  ...
  if (dae->dae_channel == NULL && dae->dae_channel_tag == NULL && dae->dae_content_type == 0 &&
      (dae->dae_title == NULL || dae->dae_title[0] == '\0') &&
      (dae->dae_cat1 == NULL || *dae->dae_cat1 == 0) &&
      (dae->dae_cat2 == NULL || *dae->dae_cat2 == 0) &&
      (dae->dae_cat3 == NULL || *dae->dae_cat3 == 0) && dae->dae_minduration <= 0 &&
      (dae->dae_maxduration <= 0 || dae->dae_maxduration > 24 * 3600) && dae->dae_minyear <= 0 &&
      dae->dae_maxyear <= 0 && dae->dae_minseason <= 0 && dae->dae_maxseason <= 0 &&
      dae->dae_serieslink_uri == NULL)
    return 0; // Avoid super wildcard match

  if (dae->dae_serieslink_uri) {
    if (!e->serieslink || strcmp(dae->dae_serieslink_uri ?: "", e->serieslink->uri))
      return 0;
  }
  ...
  if (dae->dae_channel != NULL) {
    if (dae->dae_channel != e->channel && dae->dae_channel->ch_enabled)
      return 0;
  ...
  if (e->category) {
    ...
    if (dae->dae_cat3 && *dae->dae_cat3 && !string_list_contains_string(e->category, dae->dae_cat3))
      return 0;
  } else if ((dae->dae_cat1 && *dae->dae_cat1) || (dae->dae_cat2 && *dae->dae_cat2) ||
      (dae->dae_cat3 && *dae->dae_cat3)) {
    /* No category in event but autorec has category, so no match. */
    return 0;
  }

  if (dae->dae_start >= 0 && dae->dae_start_window >= 0 && dae->dae_start < 24 * 60 &&
  ...
  if (dae->dae_minduration > 0) {
    if (duration < dae->dae_minduration)
      return 0;
  }

  if (dae->dae_maxduration > 0) {
    if (duration > dae->dae_maxduration)
      return 0;
  }
  ...
  if (e->copyright_year && dae->dae_minyear > 0) {
    if (e->copyright_year < dae->dae_minyear)
      return 0;
  }

  if (e->copyright_year && dae->dae_maxyear > 0) {
    if (e->copyright_year > dae->dae_maxyear)
      return 0;
  }

  if (e->epnum.s_num && dae->dae_minseason > 0) {
    if (e->epnum.s_num < dae->dae_minseason)
      return 0;
  }

  if (e->epnum.s_num && dae->dae_maxseason > 0) {
    if (e->epnum.s_num > dae->dae_maxseason)
      return 0;
  }
  ...
  if ((dae->dae_serieslink_uri == NULL || dae->dae_serieslink_uri[0] == '\0') &&
    ...
    if (!dae->dae_fulltext) {
      ...
      RB_FOREACH (ls, e->title, link)
        if (!regex_match(&dae->dae_title_regex, ls->str))
          break;
    } else {
      ls = NULL;
      if (e->title)
 ...
 *
 */
dvr_autorec_entry_t* dvr_autorec_create(const char* uuid, htsmsg_t* conf) {
  ...
  if (c && c->dvr_autorec_dedup)
    dae->dae_record = c->dvr_autorec_dedup;
...
dvr_autorec_entry_t* dvr_autorec_create_htsp(htsmsg_t* conf) {
  dvr_autorec_entry_t* dae;
...
void dvr_autorec_update_htsp(dvr_autorec_entry_t* dae, htsmsg_t* conf) {
  ...
  idnode_changed(&dae->dae_id);
  tvhinfo(LS_DVR,
      "autorec \"%s\" on \"%s\": Updated",
      dae->dae_title ? dae->dae_title : "",
 ...
 *
 */
dvr_autorec_entry_t* dvr_autorec_add_series_link(const char* dvr_config_name,
...
static void dvr_autorec_entry_class_changed(idnode_t* self) {
...
static htsmsg_t* dvr_autorec_entry_class_save(idnode_t* self, char* filename, size_t fsize) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)self;
...
static void dvr_autorec_entry_class_delete(idnode_t* self) {
  autorec_entry_destroy((dvr_autorec_entry_t*)self, 1);
}

static int dvr_autorec_entry_class_perm(idnode_t* self, access_t* a, htsmsg_t* msg_to_write) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)self;
...
static void
dvr_autorec_entry_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)self;
  const char*          s   = "";
...
static int dvr_autorec_entry_class_channel_set(void* o, const void* v) {
  ...
  if (ch == NULL) {
    ...
    if (dae->dae_id.in_access && !channel_access(ch, dae->dae_id.in_access, 1))
      return 0;
    if (dae->dae_channel)
      LIST_REMOVE(dae, dae_channel_link);
    dae->dae_channel = ch;
...
static const void* dvr_autorec_entry_class_channel_get(void* o) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  if (dae->dae_channel)
...
static char* dvr_autorec_entry_class_channel_rend(void* o, const char* lang) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  if (dae->dae_channel)
...
static int dvr_autorec_entry_class_title_set(void* o, const void* v) {
  dvr_autorec_entry_t* dae   = (dvr_autorec_entry_t*)o;
  const char*          title = v ?: "";
...
static int dvr_autorec_entry_class_tag_set(void* o, const void* v) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  channel_tag_t*       tag = v ? channel_tag_find_by_uuid(v) : NULL;
  if (tag == NULL)
    tag = v ? channel_tag_find_by_name(v, 0) : NULL;
...
static const void* dvr_autorec_entry_class_tag_get(void* o) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  if (dae->dae_channel_tag)
...
static char* dvr_autorec_entry_class_tag_rend(void* o, const char* lang) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  if (dae->dae_channel_tag)
...
static int dvr_autorec_entry_class_time_set(void* o, const void* v, int* tm) {
  const char* s = v;
  int         t;
...
static int dvr_autorec_entry_class_start_set(void* o, const void* v) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  return dvr_autorec_entry_class_time_set(o, v, &dae->dae_start);
}

static int dvr_autorec_entry_class_start_window_set(void* o, const void* v) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  return dvr_autorec_entry_class_time_set(o, v, &dae->dae_start_window);
}

static const void* dvr_autorec_entry_class_time_get(void* o, int tm) {
  if (tm >= 0)
...
static const void* dvr_autorec_entry_class_start_get(void* o) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  return dvr_autorec_entry_class_time_get(o, dae->dae_start);
}

static const void* dvr_autorec_entry_class_start_window_get(void* o) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
...
static htsmsg_t* dvr_autorec_entry_class_time_list_(void* o, const char* lang) {
  const char* msg = N_("Any");
  return dvr_autorec_entry_class_time_list(o, tvh_gettext_lang(lang, msg));
}

static htsmsg_t* dvr_autorec_entry_class_extra_list(void* o, const char* lang) {
...
static htsmsg_t* dvr_autorec_entry_class_minduration_list(void* o, const char* lang) {
  const char* msg = N_("Any");
  return dvr_entry_class_duration_list(o, tvh_gettext_lang(lang, msg), 24 * 60, 60, lang);
}

static htsmsg_t* dvr_autorec_entry_class_maxduration_list(void* o, const char* lang) {
  const char* msg = N_("Any");
  return dvr_entry_class_duration_list(o, tvh_gettext_lang(lang, msg), 24 * 60, 60, lang);
}

static int dvr_autorec_entry_class_config_name_set(void* o, const void* v) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  dvr_config_t*        cfg = v ? dvr_config_find_by_uuid(v) : NULL;
  if (cfg == NULL)
    cfg = v ? dvr_config_find_by_name_default(v) : NULL;
...
static const void* dvr_autorec_entry_class_config_name_get(void* o) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  if (dae->dae_config)
...
static char* dvr_autorec_entry_class_config_name_rend(void* o, const char* lang) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  if (dae->dae_config)
...
static int dvr_autorec_entry_class_weekdays_set(void* o, const void* v) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  htsmsg_field_t*      f;
  uint32_t             u32, bits = 0;
...
static htsmsg_t* dvr_autorec_entry_class_weekdays_default(void) {
  return dvr_autorec_entry_class_weekdays_get(0x7f);
}

static const void* dvr_autorec_entry_class_weekdays_get_(void* o) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
...
htsmsg_t* dvr_autorec_entry_class_weekdays_list(void* o, const char* lang) {
...
static char* dvr_autorec_entry_class_weekdays_rend_(void* o, const char* lang) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  return dvr_autorec_entry_class_weekdays_rend(dae->dae_weekdays, lang);
}

/** Validate star rating is in range */
static int dvr_autorec_entry_class_star_rating_set(void* o, const void* v) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
...
static htsmsg_t* dvr_autorec_entry_class_star_rating_list(void* o, const char* lang) {
  htsmsg_t* m = htsmsg_create_list();
...
static htsmsg_t* dvr_autorec_entry_class_year_list(void* o, const char* lang) {
  htsmsg_t* m = htsmsg_create_list();
  htsmsg_t* e = htsmsg_create_map();
  htsmsg_add_u32(e, "key", 0);
...
static htsmsg_t* dvr_autorec_entry_class_content_type_list(void* o, const char* lang) {
  htsmsg_t* m = htsmsg_create_map();
...
static htsmsg_t* dvr_autorec_entry_class_dedup_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("Use DVR configuration"), DVR_AUTOREC_RECORD_DVR_PROFILE},
      {N_("Record all"), DVR_AUTOREC_RECORD_ALL},
      {N_("All: Record if EPG/XMLTV indicates it is a unique programme"),
          DVR_AUTOREC_RECORD_UNIQUE},
      {N_("All: Record if different episode number"), DVR_AUTOREC_RECORD_DIFFERENT_EPISODE_NUMBER},
      {N_("All: Record if different subtitle"), DVR_AUTOREC_RECORD_DIFFERENT_SUBTITLE},
      {N_("All: Record if different description"), DVR_AUTOREC_RECORD_DIFFERENT_DESCRIPTION},
      {N_("All: Record once per month"), DVR_AUTOREC_RECORD_ONCE_PER_MONTH},
      {N_("All: Record once per week"), DVR_AUTOREC_RECORD_ONCE_PER_WEEK},
      {N_("All: Record once per day"), DVR_AUTOREC_RECORD_ONCE_PER_DAY},
      {N_("Local: Record if different episode number"),
          DVR_AUTOREC_LRECORD_DIFFERENT_EPISODE_NUMBER},
      {N_("Local: Record if different title"), DVR_AUTOREC_LRECORD_DIFFERENT_TITLE},
      {N_("Local: Record if different subtitle"), DVR_AUTOREC_LRECORD_DIFFERENT_SUBTITLE},
      {N_("Local: Record if different description"), DVR_AUTOREC_LRECORD_DIFFERENT_DESCRIPTION},
      {N_("Local: Record once per month"), DVR_AUTOREC_LRECORD_ONCE_PER_MONTH},
      {N_("Local: Record once per week"), DVR_AUTOREC_LRECORD_ONCE_PER_WEEK},
      {N_("Local: Record once per day"), DVR_AUTOREC_LRECORD_ONCE_PER_DAY},
  };
  return strtab2htsmsg(tab, 1, lang);
}

static htsmsg_t* dvr_autorec_entry_class_btype_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("Any"), DVR_AUTOREC_BTYPE_ALL},
      {N_("New / premiere / unknown"), DVR_AUTOREC_BTYPE_NEW_OR_UNKNOWN},
      {N_("New / premiere"), DVR_AUTOREC_BTYPE_NEW},
      {N_("Repeated"), DVR_AUTOREC_BTYPE_REPEAT},
  };
  return strtab2htsmsg(tab, 1, lang);
}

static htsmsg_t* dvr_autorec_entry_category_list(void* o, const char* lang) {
  htsmsg_t* m = htsmsg_create_map();
  htsmsg_add_str(m, "type", "api");
...
static uint32_t dvr_autorec_entry_class_owner_opts(void* o, uint32_t opts) {
  dvr_autorec_entry_t* dae = (dvr_autorec_entry_t*)o;
  if (dae && dae->dae_id.in_access && !access_verify2(dae->dae_id.in_access, ACCESS_ADMIN))
    return PO_ADVANCED;
...
#define CATEGORY_SELECTION_PROP(NUM)                                                 \
  .type = PT_STR, .id = "cat" #NUM, .name = N_("Category " #NUM " (selection)"),     \
  ...
  .off = offsetof(dvr_autorec_entry_t, dae_cat##NUM), .opts = PO_EXPERT,             \
  .list = dvr_autorec_entry_category_list

const idclass_t dvr_autorec_entry_class = {.ic_class = "dvrautorec",
    ...
    .ic_properties                                   = (const property_t[]){{
        ...
        },
                                          {
                                              ...
                                              .desc = N_("The earliest year for the programme. Programmes must be equal to or later "
                                                         "than this year."),
        ...
        },
                                          {
                                              ...
                                              .desc = N_("The latest year for the programme. Programmes must be equal to or earlier "
                                                         "than this year."),
                                              .list = dvr_autorec_entry_class_year_list,
        ...
        },
                                          {
                                              ...
                                              .desc = N_("The earliest season for the programme. Programmes must be equal to or "
                                                         "later than this season."),
        ...
        },
                                          {
                                              ...
                                              .desc = N_("The latest season for the programme. Programmes must be equal to or "
...
static void dvr_autorec_async_reschedule_cb(void* ignored) {
...
void dvr_autorec_async_reschedule(void) {
   ...
   * rule.
   */
  mtimer_arm_rel(&reschedule_timer, dvr_autorec_async_reschedule_cb, NULL, sec2mono(60));
}
 ...
 *
 */
void autorec_destroy_by_config(dvr_config_t* kcfg, int delconf) {
  dvr_autorec_entry_t* dae;
