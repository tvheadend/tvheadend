 ...
 */
static int
dvr_parse_edl(const char* line, dvr_cutpoint_t* cutpoint, float* frame) {
  int   action = 0;
 ...
 *
 */
static int dvr_parse_comskip(const char* line,
  ...
  if (start < 0 || end < 0 || end < start || start == end) {
    tvherror(LS_DVR,
        "Insane EDL entry: start=%d, end=%d. Skipping.",
        start,
        end);
    return 1;
  }
 ...
 */
static int
dvr_parse_file(const char* path, dvr_cutpoint_list_t* cut_list, void* p) {
...
dvr_cutpoint_list_t* dvr_get_cutpoint_list(dvr_entry_t* de) {
  ...
  for (i = 0; i < ARRAY_SIZE(dvr_cutpoint_parsers); i++) {
    ...
    /* Try parsing */
    if (dvr_cutpoint_parsers[i].parse(path,
            cuts,
...
void dvr_cutpoint_delete_files(const char* s) {
  ...
  for (i = 0; i < ARRAY_SIZE(dvr_cutpoint_parsers); i++) {
    ...
    if (unlink(path))
      tvherror(LS_DVR,
          "unable to remove cutpoint file '%s'",
          (const char*)path);
  }
}
