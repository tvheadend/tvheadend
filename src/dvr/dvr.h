...
typedef struct dvr_entry {
  ...
  char* de_rating_icon_saved;     /* Saved rating icon full path (not image cache) for once the
...
#define DVR_CH_NAME(e)                            \
  ((e)->de_channel == NULL ? (e)->de_channel_name \
                           : channel_get_name((e)->de_channel, channel_blank_name))

typedef enum {
  DVR_AUTOREC_RECORD_ALL    = 0,
  DVR_AUTOREC_RECORD_UNIQUE = 14, /// Unique episode in EPG/XMLTV, typically used for movies/series,
                                  /// and not useful for news or sport.
...
static inline int dvr_entry_is_completed_ok(dvr_entry_t* de) {
  ...
  return de->de_last_error == SM_CODE_OK || de->de_last_error == SM_CODE_FORCE_OK ||
...
static inline int dvr_autorec_entry_verify(dvr_autorec_entry_t* dae, access_t* a, int readonly) {
...
static inline int dvr_timerec_entry_verify(dvr_timerec_entry_t* dte, access_t* a, int readonly) {
  if (readonly && !access_verify2(a, ACCESS_ALL_RECORDER))
    return 0;
  if (!access_verify2(a, ACCESS_ALL_RW_RECORDER))
    return 0;
...
#define dvr_entry_trace_time2(de, t1name, t1, t2name, t2, fmt, ...)                               \
  do {                                                                                            \
    if (tvhtrace_enabled())                                                                       \
      dvr_entry_trace_time2_(__FILE__, __LINE__, de, t1name, t1, t2name, t2, fmt, ##__VA_ARGS__); \
  } while (0)
