...
#define DVR_FINISHED_REMOVED_SUCCESS \
  (1 << 3) /* Removed recording, was succesful before */
#define DVR_FINISHED_REMOVED_FAILED \
  (1 << 4) /* Removed recording, was failed before */
...
typedef struct dvr_entry {
  ...
  time_t de_segment_stop_extra; /* Automatic extra time for segmented prog (from
                                   EPG) */
  ...
  char* de_rating_label_saved; /* Saved rating label for once the recording has
                                  been completed*/
  char* de_rating_icon_saved;  /* Saved rating icon full path (not image cache)
                                  for once the recording has been completed*/
...
#define DVR_CH_NAME(e)           \
  ((e)->de_channel == NULL       \
          ? (e)->de_channel_name \
          : channel_get_name((e)->de_channel, channel_blank_name))

typedef enum {
  DVR_AUTOREC_RECORD_ALL = 0,
  DVR_AUTOREC_RECORD_UNIQUE =
      14, /// Unique episode in EPG/XMLTV, typically used for movies/series, and
          /// not useful for news or sport.
...
typedef struct dvr_autorec_entry {
  ...
  uint16_t dae_star_rating; /** Minimum star rating: we use u16 instead of u8
                               since no PT_U8 type */
...
static inline dvr_autorec_entry_t* dvr_autorec_find_by_uuid(const char* uuid) {
  return (
      dvr_autorec_entry_t*)idnode_find(uuid, &dvr_autorec_entry_class, NULL);
}
...
/* Purge any autorec timers that are obsolete since they no longer match any
 * events. */
void dvr_autorec_purge_obsolete_timers(void);
/* @return 1 if entry is an autorec and can be purged since it no longer matches
 * its event. */
...
static inline dvr_timerec_entry_t* dvr_timerec_find_by_uuid(const char* uuid) {
  return (
      dvr_timerec_entry_t*)idnode_find(uuid, &dvr_timerec_entry_class, NULL);
}
...
static inline int
dvr_timerec_entry_verify(dvr_timerec_entry_t* dte, access_t* a, int readonly) {
  if (readonly && !access_verify2(a, ACCESS_ALL_RECORDER))
    return 0;
  if (!access_verify2(a, ACCESS_ALL_RW_RECORDER))
    return 0;
...
typedef struct dvr_cutpoint {
  ...
  uint64_t dc_end_ms;
  enum { DVR_CP_CUT, DVR_CP_MUTE, DVR_CP_SCENE, DVR_CP_COMM } dc_type;
} dvr_cutpoint_t;
...
#define dvr_entry_trace_time2(de, t1name, t1, t2name, t2, fmt, ...) \
  do {                                                              \
    if (tvhtrace_enabled())                                         \
      dvr_entry_trace_time2_(__FILE__,                              \
          __LINE__,                                                 \
          de,                                                       \
          t1name,                                                   \
          t1,                                                       \
