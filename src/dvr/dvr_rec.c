...
int dvr_rec_subscribe(dvr_entry_t* de) {
  ...
  if (de->de_owner && de->de_owner[0] != '\0') {
    ...
    tvherror(LS_DVR,
        "unable to find access (owner '%s', creator '%s')",
        de->de_owner,
        de->de_creator);
    ret = -EPERM;
    goto _return;
  ...
  if (aa->aa_conn_limit || aa->aa_conn_limit_dvr) {
    ...
    if (c1 && c2) {
      tvherror(LS_DVR,
          "multiple connections are not allowed for user '%s' from '%s' "
          "(limit %u, dvr limit %u, active DVR %u, streaming %u)",
          aa->aa_username ?: "",
          aa->aa_representative ?: "",
          aa->aa_conn_limit,
          aa->aa_conn_limit_dvr,
          rec_count,
          net_count);
      ret = -EOVERFLOW;
      goto _return;
  ...
  if (stat_ret != 0) {
    tvherror(LS_DVR,
        "Directory '%s' not accessible: %s",
        de->de_config->dvr_storage,
        strerror(errno));
    ret = -EIO;
    goto _return;
  }
  ...
  if (profile_chain_open(prch, &de->de_config->dvr_muxcnf, NULL, 0, 0)) {
    ...
    tvherror(LS_DVR,
        "unable to create new channel streaming chain '%s' for '%s', using "
        "default",
    ...
    if (profile_chain_open(prch, &de->de_config->dvr_muxcnf, NULL, 0, 0)) {
      tvherror(LS_DVR,
          "unable to create channel streaming default chain '%s' for '%s'",
          profile_get_name(pro),
          channel_get_name(de->de_channel, channel_blank_name));
      ret = -EINVAL;
      goto _return;
  ...
  de->de_s = subscription_create_from_channel(prch,
      NULL,
      weight,
      buf,
      prch->prch_flags,
  ...
  if (de->de_s == NULL) {
    tvherror(LS_DVR,
        "unable to create new channel subcription for '%s' profile '%s'",
        channel_get_name(de->de_channel, channel_blank_name),
        profile_get_name(pro));
    ret = -EINVAL;
    goto _return;
 ...
 */
static char* cleanup_filename(dvr_config_t* cfg, char* s, int dosubs) {
  int   len = strlen(s);
  char *s1, *p;
  ...
  if (s1 == NULL) {
    ...
    s1 = intlconv_utf8safestr(intlconv_charset_id("ASCII", 1, 1), s, len * 2);
    if (s1 == NULL)
      return NULL;
  }
  ...
  for (s = s1; *s; s++) {
    ...
    else if (cfg->dvr_whitespace_in_title && (*s == ' ' || *s == '\t') &&
        dosubs)
      *s = '-';

    else if (cfg->dvr_clean_title &&
        ((*s < 32) || (*s > 122) || (strchr("/:\\<>|*?\"", *s) != NULL)) &&
        dosubs)
      *s = '_';

    else if (cfg->dvr_windows_compatible_filenames &&
        (strchr("/:\\<>|*?\"", *s) != NULL) && dosubs)
      *s = '_';
  }
 ...
 *
 */
static char* dvr_clean_directory_separator(char* s, char* tmp, size_t tmplen) {
  char *p, *end;
...
static const char* dvr_do_prefix(const char* id,
    const char*                              fmt,
    const char*                              s,
    char*                                    tmp,
    size_t                                   tmplen) {
...
static const char* dvr_sub_title(const char* id,
    const char*                              fmt,
    const void*                              aux,
    char*                                    tmp,
    size_t                                   tmplen) {
  return dvr_do_prefix(id,
      fmt,
      lang_str_get(((dvr_entry_t*)aux)->de_title, NULL),
      tmp,
      tmplen);
}

static const char* dvr_sub_subtitle_or_summary(const char* id,
    const char*                                            fmt,
    const void*                                            aux,
    char*                                                  tmp,
    size_t                                                 tmplen) {
  ...
  if (s == NULL)
    s = lang_str_get(de->de_summary, NULL);
  return dvr_do_prefix(id, fmt, s, tmp, tmplen);
}

static const char* dvr_sub_subtitle(const char* id,
    const char*                                 fmt,
    const void*                                 aux,
    char*                                       tmp,
    size_t                                      tmplen) {
  return dvr_do_prefix(id,
      fmt,
      lang_str_get(((dvr_entry_t*)aux)->de_subtitle, NULL),
      tmp,
      tmplen);
}

static const char* dvr_sub_summary(const char* id,
    const char*                                fmt,
    const void*                                aux,
    char*                                      tmp,
    size_t                                     tmplen) {
  return dvr_do_prefix(id,
      fmt,
      lang_str_get(((dvr_entry_t*)aux)->de_summary, NULL),
      tmp,
      tmplen);
}

static const char* dvr_sub_description(const char* id,
    const char*                                    fmt,
    const void*                                    aux,
    char*                                          tmp,
    size_t                                         tmplen) {
  return dvr_do_prefix(id,
      fmt,
      lang_str_get(((dvr_entry_t*)aux)->de_desc, NULL),
      tmp,
      tmplen);
}

static const char* dvr_sub_uuid(const char* id,
    const char*                             fmt,
    const void*                             aux,
    char*                                   tmp,
    size_t                                  tmplen) {
...
static const char* dvr_sub_episode(const char* id,
    const char*                                fmt,
    const void*                                aux,
    char*                                      tmp,
    size_t                                     tmplen) {
...
static const char* _dvr_get_tvmovies_subdir(const dvr_entry_t* de) {
...
static const char* _dvr_get_tvshows_subdir(const dvr_entry_t* de) {
  dvr_config_t* config = de->de_config;
...
typedef enum {
  DVR_SF_WITHOUT_SUBDIR =
      0x0, /*< X.ts and Y.ts - movies and tvshows not in subdir */
  DVR_SF_WITH_GENRE_SUBDIR = 0x1, /*< tvmovies and tvshows */
  DVR_SF_WITH_PER_SEASON_SUBDIR =
      0x2, /*< X/Season S/Y.ts - separate dir per-season */
  DVR_SF_WITH_PER_MOVIE_SUBDIR = 0x4 /*< X/X.ts - separate dir per-movie */
} dvr_sf_t;

static const char* _dvr_sub_scraper_friendly(const char* id,
    const char*                                          fmt,
    const void*                                          aux,
    char*                                                tmp,
    size_t                                               tmplen,
    dvr_sf_t                                             subdir_type) {
  /* Directory to use for no season/special season when using
   * per-season directories. Kodi "naming tv shows/Special Episodes".
   */
  static const char special_season_dir[] =
      "Season 0"; /* Deliberately not localized. */
  const dvr_entry_t* de  = aux;
  ...
  *tmp                 = 0;
  ...
  if (subtitle)
    dvr_clean_directory_separator((char*)subtitle,
        subtitle_buf,
        subtitle_buf_size);
  ...
  if (fmt && *fmt == '1') /* Force to be a movie */
    ...
    if (ebc && ebc->category) {
      /* We've parsed categories from xmltv. So check if it has the movie
       * category. */
      is_movie = string_list_contains_string(ebc->category, "movie") ||
  ...
  tvhdebug(LS_DVR,
      "fmt = %s is_movie = %d content_type = %d",
      fmt ?: "<none>",
      is_movie,
      de->de_content_type);
  ...
  if (is_movie) {
     ...
     *   "tvmovies/title (yyyy)/title (yyyy)" (with genre_subdir + per movie
     * subdir) "title (yyyy)"                       (without genre_subdir)
    ...
    /* For 'per movie subdir' we want "title (YYYY)/" or "title/" if no year but
     * don't want "(YYYY)" (no title) */
    if ((subdir_type & DVR_SF_WITH_PER_MOVIE_SUBDIR) ==
     ...
     *   "tvshows/title/Season 1/title - S01E02 - subtitle" (with genre and per
     * season subdir) "title - S01E02 - subtitle"               (without
     * genre_subdir) "title - subtitle_2001-05-04"             (without
     * genre_subdir, long running show) "title - subtitle" (without
     * genre_subdir, no epg info on show)
     */
    if ((subdir_type & DVR_SF_WITH_GENRE_SUBDIR) == DVR_SF_WITH_GENRE_SUBDIR)
      tvh_strlcatf(tmp, tmplen, offset, "%s/", _dvr_get_tvshows_subdir(de));

    if ((subdir_type & DVR_SF_WITH_PER_SEASON_SUBDIR) ==
            DVR_SF_WITH_PER_SEASON_SUBDIR &&
        !strempty(title_buf)) {
      ...
      epg_broadcast_epnumber_format(ebc,
          ...
          NULL,
          NULL);
      /* E.g., "Simpsons/Season 1/Simpsons", to which we'll later add " -
       * S01E02" Or "Simpsons/Season 0/Simpsons" for a tv special.
       */
      tvh_strlcatf(tmp,
          tmplen,
          offset,
          "%s/%s/%s",
          title_buf,
          strempty(season_dir) ? special_season_dir : season_dir,
          title_buf);
...
static const char* dvr_sub_scraper_friendly_with_genre_subdir(const char* id,
    const char*                                                           fmt,
    const void*                                                           aux,
    char*                                                                 tmp,
    size_t tmplen) {
  return _dvr_sub_scraper_friendly(id,
      fmt,
      aux,
      tmp,
      tmplen,
      DVR_SF_WITH_GENRE_SUBDIR);
}

static const char* dvr_sub_scraper_friendly_with_genre_subdir_full(
    const char* id,
    const char* fmt,
    const void* aux,
    char*       tmp,
  ...
  return _dvr_sub_scraper_friendly(id,
      ...
      tmp,
      tmplen,
      DVR_SF_WITH_GENRE_SUBDIR | DVR_SF_WITH_PER_SEASON_SUBDIR |
          DVR_SF_WITH_PER_MOVIE_SUBDIR);
}

static const char* dvr_sub_scraper_friendly_without_genre_subdir(const char* id,
    const char* fmt,
    const void* aux,
    char*       tmp,
    size_t      tmplen) {
...
static const char* dvr_sub_scraper_friendly_without_genre_subdir_full(
    const char* id,
    const char* fmt,
    const void* aux,
    char*       tmp,
  ...
  return _dvr_sub_scraper_friendly(id,
      ...
      tmp,
      tmplen,
      DVR_SF_WITHOUT_SUBDIR | DVR_SF_WITH_PER_SEASON_SUBDIR |
          DVR_SF_WITH_PER_MOVIE_SUBDIR);
}

static const char* dvr_sub_channel(const char* id,
    const char*                                fmt,
    const void*                                aux,
    char*                                      tmp,
    size_t                                     tmplen) {
...
static const char* dvr_sub_genre(const char* id,
    const char*                              fmt,
    const void*                              aux,
    char*                                    tmp,
    size_t                                   tmplen) {
...
static const char* dvr_sub_owner(const char* id,
    const char*                              fmt,
    const void*                              aux,
    char*                                    tmp,
    size_t                                   tmplen) {
...
static const char* dvr_sub_creator(const char* id,
    const char*                                fmt,
    const void*                                aux,
    char*                                      tmp,
    size_t                                     tmplen) {
...
static const char* dvr_sub_last_error(const char* id,
    const char*                                   fmt,
    const void*                                   aux,
    char*                                         tmp,
    size_t                                        tmplen) {
  return dvr_do_prefix(id,
      fmt,
      streaming_code2txt(((dvr_entry_t*)aux)->de_last_error),
      tmp,
      tmplen);
}

static const char* dvr_sub_start(const char* id,
    const char*                              fmt,
    const void*                              aux,
    char*                                    tmp,
    size_t                                   tmplen) {
  ...
  snprintf(buf,
      sizeof(buf),
      "%" PRItime_t,
      (time_t)dvr_entry_get_start_time((dvr_entry_t*)aux, 0));
  return dvr_do_prefix(id, fmt, buf, tmp, tmplen);
}

static const char* dvr_sub_errors(const char* id,
    const char*                               fmt,
    const void*                               aux,
    char*                                     tmp,
    size_t                                    tmplen) {
  ...
  snprintf(buf,
      sizeof(buf),
      "%" PRIu32,
      (uint32_t)((dvr_entry_t*)aux)->de_errors);
  return dvr_do_prefix(id, fmt, buf, tmp, tmplen);
}

static const char* dvr_sub_data_errors(const char* id,
    const char*                                    fmt,
    const void*                                    aux,
    char*                                          tmp,
    size_t                                         tmplen) {
  ...
  snprintf(buf,
      sizeof(buf),
      "%" PRIu32,
      (uint32_t)((dvr_entry_t*)aux)->de_data_errors);
  return dvr_do_prefix(id, fmt, buf, tmp, tmplen);
}

static const char* dvr_sub_stop(const char* id,
    const char*                             fmt,
    const void*                             aux,
    char*                                   tmp,
    size_t                                  tmplen) {
  ...
  snprintf(buf,
      sizeof(buf),
      "%" PRItime_t,
      (time_t)dvr_entry_get_stop_time((dvr_entry_t*)aux));
  return dvr_do_prefix(id, fmt, buf, tmp, tmplen);
}

static const char* dvr_sub_comment(const char* id,
    const char*                                fmt,
    const void*                                aux,
    char*                                      tmp,
    size_t                                     tmplen) {
...
static const char* dvr_sub_strftime(const char* id,
    const char*                                 fmt,
    const void*                                 aux,
    char*                                       tmp,
    size_t                                      tmplen) {
...
static htsstr_substitute_t dvr_subs_time[] = {
    {.id = "a",
        .getval =
            dvr_sub_strftime}, /* The abbreviated name of the day of the week */
    {.id        = "A",
        .getval = dvr_sub_strftime}, /* The full name of the day of the week */
    {.id = "b", .getval = dvr_sub_strftime}, /* The abbreviated month name */
    {.id = "B", .getval = dvr_sub_strftime}, /* The full month name */
    {.id = "c",
        .getval =
            dvr_sub_strftime}, /* The preferred date and time representation */
    {.id = "C", .getval = dvr_sub_strftime}, /* The century number (year/100) as
                                                a 2-digit integer */
    {.id        = "d",
        .getval = dvr_sub_strftime}, /* The day of the month as a decimal number
                                        (range 01 to 31) */
    ...
    {.id = "G",
        .getval =
            dvr_sub_strftime}, /* The ISO 8601 week-based year with century */
    ...
    {.id = "j",
        .getval =
            dvr_sub_strftime}, /* The day of the year (range 000 to 366) */
    {.id        = "k",
        .getval = dvr_sub_strftime}, /* The hour (range 0 to 23) - with space */
    {.id        = "l",
        .getval = dvr_sub_strftime}, /* The hour (range 1 to 12) - with space */
    ...
    {.id        = "s",
        .getval = dvr_sub_strftime}, /* The number of seconds since the Epoch */
    {.id = "S", .getval = dvr_sub_strftime}, /* The seconds (range 00 to 60) */
    {.id = "T", .getval = dvr_sub_strftime}, /* %H:%M:%S */
    {.id        = "u",
        .getval = dvr_sub_strftime}, /* The day of the week as a decimal, range
                                        1 to 7, Monday being 1 */
    {.id = "U",
        .getval =
            dvr_sub_strftime}, /* The week number of the current year as a
                                  decimal number, range 00 to 53 (Sunday) */
    {.id = "V", .getval = dvr_sub_strftime}, /* The  ISO 8601  week  number
                                                (range 01 to 53) */
    {.id        = "w",
        .getval = dvr_sub_strftime}, /* The day of the week as a decimal, range
                                        0 to 6, Sunday being 0 */
    {.id = "W",
        .getval =
            dvr_sub_strftime}, /* The week number of the current year as a
                                  decimal number, range 00 to 53 (Monday) */
    {.id        = "x",
        .getval = dvr_sub_strftime}, /* The preferred date representation */
    {.id        = "X",
        .getval = dvr_sub_strftime}, /* The preferred time representation */
    {.id        = "y",
        .getval = dvr_sub_strftime}, /* The year as a decimal number without a
                                        century (range 00 to 99) */
    {.id = "Y", .getval = dvr_sub_strftime}, /* The year as a decimal number
                                                including the century */
    {.id        = "z",
        .getval = dvr_sub_strftime}, /* The +hhmm or -hhmm numeric timezone */
    {.id        = "Z",
        .getval = dvr_sub_strftime}, /* The timezone name or abbreviation */

    {.id = NULL, .getval = NULL}};

static const char* dvr_sub_str(const char* id,
    const char*                            fmt,
    const void*                            aux,
    char*                                  tmp,
    size_t                                 tmplen) {
...
static const char* dvr_sub_str_separator(const char* id,
    const char*                                      fmt,
    const void*                                      aux,
    char*                                            tmp,
    size_t                                           tmplen) {
...
static const char* dvr_sub_basename(const char* id,
    const char*                                 fmt,
    const void*                                 aux,
    char*                                       tmp,
    size_t                                      tmplen) {
...
static const char* dvr_sub_basic_info(const char* id,
    const char*                                   fmt,
    const void*                                   aux,
    char*                                         tmp,
    size_t                                        tmplen) {
 ...
 *
 */
static int pvr_generate_filename(dvr_entry_t* de, const streaming_start_t* ss) {
  ...
  dir_dosubs = de->de_directory == NULL ||
      (de->de_directory[0] == '$' && de->de_directory[1] == '$');
  ...
  if (de->de_directory) {
    ...
    if (dir_dosubs) {
      htsstr_substitute(de->de_directory + 2,
          ptmp,
          sizeof(ptmp),
          '$',
          dvr_subs_entry,
  ...
  if (makedirs(LS_DVR,
          filename,
          cfg->dvr_muxcnf.m_directory_permissions,
          0,
          -1,
          -1) != 0)
    return -1;
  ...
  while (1) {
    ...
    if (l + k > max) {
      ...
      cut1:
        ...
        x = (char*)htsstr_escape_find(filename + j,
            s - (filename + j) - ((l + k) - max));
        if (x == NULL)
          goto cut1;
        k = strlen(s);
        memmove(x, s, k);
    ...
    if (lastpath) {
      if (strcmp(path, lastpath) == 0) {
        free(lastpath);
        tvherror(LS_DVR,
            "unable to create unique name (missing $n in format string?)");
        return -1;
      }
    }
 ...
 *
 */
static void dvr_rec_fatal_error(dvr_entry_t* de, const char* fmt, ...) {
  char msgbuf[256];
 ...
 *
 */
static void dvr_notify(dvr_entry_t* de) {
 ...
 *
 */
static int dvr_rec_start(dvr_entry_t* de, const streaming_start_t* ss) {
  ...
  if (stat(cfg->dvr_storage, &st) || !S_ISDIR(st.st_mode)) {
    dvr_rec_fatal_error(de,
        "Unable to create file in directory '%s",
        cfg->dvr_storage);
    return -1;
  }
 ...
 *
 */
static inline int dvr_thread_global_lock(dvr_entry_t* de, int* run) {
 ...
 *
 */
static inline void dvr_thread_global_unlock(dvr_entry_t* de) {
 ...
 *
 */
static void dvr_streaming_restart(dvr_entry_t* de, int* run) {
 ...
 *
 */
static int dvr_thread_pkt_stats(dvr_entry_t* de, th_pkt_t* pkt, int payload) {
 ...
 *
 */
static int dvr_thread_mpegts_stats(dvr_entry_t* de, void* sm_data) {
  th_subscription_t* ts;
  pktbuf_t*          pb  = sm_data;
  int                ret = 0;
 ...
 *
 */
static int dvr_thread_rec_start(dvr_entry_t** _de,
  ...
  if (!*started) {
     ...
     */
    de->de_rating_label = NULL; // Forget the rating label pointer and only rely
                                // on the saved values from here on.
    ...
    if (code == 0) {
      ...
      *started = 1;
      ...
      dvr_stop_recording(de,
          code == SM_CODE_NO_SPACE ? SM_CODE_NO_SPACE : SM_CODE_INVALID_TARGET,
          1,
          0);
    dvr_thread_global_unlock(de);
  }
 ...
 *
 */
static inline int dts_pts_valid(th_pkt_t* pkt, int64_t dts_offset) {
  if (pkt->pkt_dts == PTS_UNSET || pkt->pkt_pts == PTS_UNSET ||
      dts_offset == PTS_UNSET || pkt->pkt_dts < dts_offset ||
 ...
 *
 */
static int64_t get_dts_ref(th_pkt_t* pkt, streaming_start_t* ss) {
  const streaming_start_component_t* ssc;
...
static void* dvr_thread(void* aux) {
  ...
  postproc =
      de->de_config->dvr_postproc ? strdup(de->de_config->dvr_postproc) : NULL;
  running_disabled = dvr_entry_get_epg_running(de) <= 0;
  real_start       = dvr_entry_get_start_time(de, 0);
  tvhtrace(LS_DVR,
      "%s - recoding thread started for \"%s\"",
      idnode_uuid_as_str(&de->de_id, ubuf),
  ...
  while (run) {
    ...
    if (running_disabled) {
      ...
      if (running_start > 0) {
        ...
        if (epg_running &&
            atomic_get_time_t(&de->de_running_pause) >= running_start)
          epg_running = 2;
    ...
    if (epg_running != old_epg_running)
      tvhtrace(LS_DVR,
          "%s - running flag changed from %d to %d",
          idnode_uuid_as_str(&de->de_id, ubuf),
          old_epg_running,
          epg_running);
 ...
 *
 */
void dvr_spawn_cmd(dvr_entry_t* de,
    const char*                 cmd,
    const char*                 filename,
    int                         pre) {
  ...
  /* Substitute DVR entry formatters */
  htsstr_substitute(cmd,
      buf1,
      sizeof(buf1),
      '%',
      dvr_subs_postproc_entry,
      de,
      tmp,
      sizeof(tmp));
  ...
  if (!pre) {
    htsstr_substitute(buf2,
        buf1,
        sizeof(buf1),
        '%',
        dvr_subs_postproc_filename,
        filename,
        tmp,
        sizeof(tmp));
    buf2 = tvh_strdupa(buf1);
