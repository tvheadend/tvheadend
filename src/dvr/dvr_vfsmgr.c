 ...
 *
 */
static dvr_vfs_t* dvr_vfs_find(dvr_vfs_t* old, tvh_fsid_t* id) {
  dvr_vfs_t* dv;
...
static dvr_vfs_t* dvr_vfs_find1(dvr_vfs_t* old, htsmsg_t* m) {
  int64_t     v;
  tvh_fsid_t  fsid;
  const char* s;
...
/**
 * Cleanup old recordings for this config until the dvr_cleanup_threshold is
 * reached Only "Keep until space needed" recordings are deleted, starting with
 * the oldest one Return -1 on failure, -2 if disk stats unstable (no action
 * taken), otherwise number of bytes cleaned
 */
static int64_t dvr_disk_space_cleanup(dvr_config_t* cfg, int include_active) {
  ...
  /* When deleting a file from the disk, the system needs some time to actually
   * do this */
  /* If calling this function too soon after the previous call, statvfs might be
   * wrong/not updated yet */
  /* So return a "disk stats unreliable" status */
  if (dvr_disk_space_config_lastdelete + sec2mono(10) > mclk()) {
    tvhtrace(LS_DVR,
        "disk space cleanup called <10s after last call - ignoring");
    return -2;
  }
  ...
  configName    = cfg != dvr_config_find_by_name(NULL) ? cfg->dvr_config_name
                                                    : "Default profile";

  if (diskBytes < requiredBytes) {
    tvhwarn(LS_DVR,
        "disk space cleanup for config \"%s\", required free space \"%" PRId64
        " MiB\" is smaller than the total disk size!",
        configName,
        TOMIB(requiredBytes));
  ...
  tvhtrace(LS_DVR,
      "disk space cleanup for config \"%s\", required/current free space "
      "\"%" PRId64 "/%" PRId64 " MiB\", required/current used space \"%" PRId64
      "/%" PRId64 " MiB\"",
      configName,
      TOMIB(requiredBytes),
      TOMIB(availBytes),
  ...
  while (availBytes < requiredBytes ||
      ((maximalBytes < usedBytes) && cfg->dvr_cleanup_threshold_used)) {
    oldest   = NULL;
    stoptime = gclk();
    ...
    LIST_FOREACH (de, &dvrentries, de_global_link) {
      ...
      if (dvr_entry_get_removal_days(de) !=
          DVR_REM_SPACE) // only remove the allowed ones
        continue;

      if (dvr_get_filename(de) == NULL ||
          dvr_get_filesize(de, DVR_FILESIZE_TOTAL) <= 0)
        continue;

      if (tvh_vfs_fsid_build(dvr_get_filename(de), NULL, &fsid2))
        continue;

      /* Checking for the same config is useless as it's storage path might be
       * changed meanwhile */
    ...
    if (oldest) {
      ...
      if (strftime(tbuf, sizeof(tbuf), "%F %T", &tm) <= 0)
        *tbuf = 0;
      tvhinfo(LS_DVR,
          "Delete \"until space needed\" recording \"%s\" with stop time "
          "\"%s\" and file size \"%" PRId64 " MB\"",
          lang_str_get(oldest->de_title, NULL),
          tbuf,
          TOMIB(fileSize));

      dvr_disk_space_config_lastdelete = mclk();
      dvr_entry_cancel_remove(oldest,
          0); /* Remove stored files and mark as "removed" */
    } else {
      /* Stop active recordings if cleanup is not possible */
      if (loops == 0 && include_active) {
        tvhwarn(LS_DVR,
            "No \"until space needed\" recordings found for config \"%s\", "
            "aborting active recordings now!",
            configName);
        LIST_FOREACH (de, &dvrentries, de_global_link) {
          if (de->de_sched_state != DVR_RECORDING || !de->de_config ||
              de->de_config != cfg)
            continue;
    ...
    if (loops >= 10) {
      tvhwarn(LS_DVR,
          "Not able to clear the required disk space after deleting %i \"until "
          "space needed\" recordings...",
          loops);
      goto finish;
    }
  }
...
finish:
  tvhtrace(LS_DVR,
      "disk space cleanup for config \"%s\", cleared \"%" PRId64
      " MB\" of disk space, new free disk space \"%" PRId64
      " MiB\", new used disk space \"%" PRId64 " MiB\"",
      configName,
      TOMIB(clearedBytes),
      TOMIB(availBytes),
      TOMIB(usedBytes));
...
/**
 * Check for each dvr config if the free disk size is below the
 * dvr_cleanup_threshold If so and we are using the dvr config ATM (active
 * recording), we start the cleanup procedure
 */
static void dvr_disk_space_check() {
  dvr_config_t*  cfg;
  dvr_entry_t*   de;
  struct statvfs diskdata;
  ...
  LIST_FOREACH (cfg, &dvrconfigs, config_link) {
    ...
    if (availBytes < requiredBytes ||
      ...
      LIST_FOREACH (de, &dvrentries, de_global_link) {
        ...
        if (availBytes < requiredBytes) {
          tvhwarn(LS_DVR,
              "running out of free disk space for dvr config \"%s\", required "
              "free space \"%" PRId64 " MiB\", current free space \"%" PRId64
              " MiB\"",
              cfg != dvr_config_find_by_name(NULL) ? cfg->dvr_config_name
                                                   : "Default profile",
              TOMIB(requiredBytes),
              TOMIB(availBytes));
        } else {
          tvhwarn(LS_DVR,
              "running out of used disk space for dvr config \"%s\", required "
              "used space \"%" PRId64 " MiB\", current used space \"%" PRId64
              " MiB\"",
              cfg != dvr_config_find_by_name(NULL) ? cfg->dvr_config_name
                                                   : "Default profile",
              TOMIB(maximalBytes),
        ...
        /* only cleanup one directory at the time as the system needs time to
    ...
    checking:
      tvhtrace(LS_DVR,
          "checking free and used disk space for config \"%s\" : OK",
          cfg != dvr_config_find_by_name(NULL) ? cfg->dvr_config_name
                                               : "Default profile");
    }
  }
 ...
 *
 */
static void dvr_get_disk_space_update(const char* path, int locked) {
  struct statvfs diskdata;
  dvr_vfs_t*     dvfs;
  tvh_fsid_t     fsid;
 ...
 *
 */
static void dvr_get_disk_space_tcb(void* opaque, int dearmed) {
  if (!dearmed) {
...
static void dvr_get_disk_space_cb(void* aux) {
  dvr_config_t* cfg;
  char*         path;
  ...
  mtimer_arm_rel(&dvr_disk_space_timer,
      dvr_get_disk_space_cb,
      NULL,
      sec2mono(15));
}
 ...
 * If '0' (= error or below configured minimum), a new recording should not be
 * started.
 */
  ...
  if (availBytes < requiredBytes ||
    ...
    if (cleanedBytes == -1)
      return 0;
        ...
        -2) // Disk stats may be unreliable, continue anyway
      return 1;
