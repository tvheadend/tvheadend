 ...
 */
dvr_config_t* dvr_config_find_by_list(htsmsg_t* uuids, const char* name) {
 ...
 *
 */
static int dvr_charset_update(dvr_config_t* cfg, const char* charset) {
  const char *s, *id;
...
dvr_config_create(const char* name, const char* uuid, htsmsg_t* conf) {
  ...
  cfg->dvr_cleanup_threshold_free =
      1000; // keep 1000 MiB of free space on disk by default
...
static void dvr_config_destroy(dvr_config_t* cfg, int delconf) {
  ...
  if (delconf) {
    ...
    hts_settings_remove("dvr/config/%s",
 ...
 *
 */
static void dvr_config_storage_check(dvr_config_t* cfg) {
 ...
 *
 */
static int dvr_filename_index(dvr_config_t* cfg) {
 ...
 *
 */
static int dvr_match_fmtstr(dvr_config_t* cfg, const char* needle, int nodir) {
  char *      str = cfg->dvr_pathname, *p;
  const char* x;
 ...
 *
 */
static void dvr_insert_fmtstr(dvr_config_t* cfg, int idx, const char* str) {
 ...
 *
 */
static void dvr_insert_fmtstr_before_extension(dvr_config_t* cfg,
    const char*                                              str) {
 ...
 *
 */
static void dvr_remove_fmtstr(dvr_config_t* cfg, int idx, int len) {
 ...
 *
 */
static void dvr_match_and_insert_or_remove(dvr_config_t* cfg,
    const char*                                          str,
    int                                                  val,
    int                                                  idx) {
 ...
 *
 */
static void dvr_update_pathname_safe(dvr_config_t* cfg) {
 ...
 *
 */
static void dvr_update_pathname_from_fmtstr(dvr_config_t* cfg) {
...
void dvr_config_changed(dvr_config_t* cfg) {
  ...
  if (cfg->dvr_cleanup_threshold_free < 50)
    cfg->dvr_cleanup_threshold_free =
        50; // as checking is only periodically, lower is not save
  ...
  if (cfg->dvr_profile &&
      !strcmp(profile_get_name(cfg->dvr_profile),
          "htsp")) // htsp is for streaming only
    cfg->dvr_profile = profile_find_by_name("pass", NULL);
}
...
static void dvr_config_class_changed(idnode_t* self) {
...
static void dvr_config_class_delete(idnode_t* self) {
  dvr_config_t* cfg = (dvr_config_t*)self;
  if (!dvr_config_is_default(cfg))
    dvr_config_destroy(cfg, 1);
}
...
static int dvr_config_class_enabled_set(void* o, const void* v) {
...
static uint32_t dvr_config_class_enabled_opts(void* o, uint32_t opts) {
  dvr_config_t* cfg = (dvr_config_t*)o;
...
static int dvr_config_class_name_set(void* o, const void* v) {
  dvr_config_t* cfg = (dvr_config_t*)o;
  if (dvr_config_is_default(cfg) && dvr_config_is_valid(cfg))
    return 0;
...
static int dvr_config_class_profile_set(void* o, const void* v) {
  dvr_config_t* cfg = (dvr_config_t*)o;
  profile_t*    pro;
...
static const void* dvr_config_class_profile_get(void* o) {
  dvr_config_t* cfg = (dvr_config_t*)o;
  if (cfg->dvr_profile)
...
static char* dvr_config_class_profile_rend(void* o, const char* lang) {
  dvr_config_t* cfg = (dvr_config_t*)o;
  if (cfg->dvr_profile)
...
static void dvr_config_class_get_title(idnode_t* self,
    const char*                                  lang,
    char*                                        dst,
    size_t                                       dstsize) {
  dvr_config_t* cfg = (dvr_config_t*)self;
  if (!dvr_config_is_default(cfg)) {
    ...
    snprintf(dst,
        dstsize,
        "%s",
        tvh_gettext_lang(lang, N_("(Default profile)")));
  }
}

static int dvr_config_class_charset_set(void* o, const void* v) {
  dvr_config_t* cfg = (dvr_config_t*)o;
  return dvr_charset_update(cfg, v);
}

static htsmsg_t* dvr_config_class_charset_list(void* o, const char* lang) {
  htsmsg_t* m = htsmsg_create_map();
...
static htsmsg_t* dvr_config_class_cache_list(void* o, const char* lang) {
  static struct strtab tab[] = {{N_("Unknown"), MC_CACHE_UNKNOWN},
...
static htsmsg_t* dvr_config_class_removal_list(void* o, const char* lang) {
...
static htsmsg_t* dvr_config_class_remove_after_playback_list(void* o,
    const char*                                                    lang) {
  enum { ONE_MINUTE = 60, ONE_HOUR = ONE_MINUTE * 60, ONE_DAY = ONE_HOUR * 24 };
...
static htsmsg_t* dvr_config_class_retention_list(void* o, const char* lang) {
  static const struct strtab_u32 tab[] = {
      {N_("1 day"), DVR_RET_REM_1DAY},
      {N_("3 days"), DVR_RET_REM_3DAY},
      {N_("5 days"), DVR_RET_REM_5DAY},
...
static htsmsg_t* dvr_config_class_extra_list(void* o, const char* lang) {
...
static htsmsg_t* dvr_config_entry_class_update_window_list(void* o,
    const char*                                                  lang) {
  return dvr_entry_class_duration_list(o,
...
static int dvr_config_class_pathname_set(void* o, const void* v) {
  dvr_config_t* cfg = (dvr_config_t*)o;
  const char*   s   = v;
    ...
    .ic_properties =
        (const property_t[]){{
            ...
            },
            {
                ...
                .name = N_("Priority"),
                .desc =
                    N_("Priority of the entry, higher-priority entries will "
            ...
            },
            {
                ...
                .desc = N_("Days to retain information about recordings. Once "
                           "this period is exceeded, duplicate detection will "
                           "not be possible."),
            ...
            },
            {
                ...
                .id   = "rerecord-errors",
                .name = N_(
                    "Try re-scheduling recording if more errors than (0=off)"),
            ...
            },
            {
                ...
                .desc =
                    N_("When scheduling an autorec, this option attempts "
                       "to schedule at the earliest time and on the 'best' "
                       "channel (such as channel with the most failover "
                       "services). "
            ...
            },
            {
                ...
                .desc = N_(
                    "Fetch additional artwork from installed providers. "
            ...
            },
            {
                ...
                .desc = N_(
                    "Artwork fetching requires broadcasts to have good quality "
            ...
            },
            {
                ...
                .name  = N_("Additional command line options when fetching "
                           "artwork for new recordings."),
                .desc  = N_("Some artwork providers require additional "
                           "arguments such as "
                           "'--tmdb-key my_key_from_website'. These can be "
                           "specified here. "
            ...
            },
            {
                ...
                .off =
                    offsetof(dvr_config_t, dvr_muxcnf.m_directory_permissions),
                .opts    = PO_EXPERT,
                .def.u32 = 0775,
                .group   = 2,
            },
            ...
            },
            {
                ...
                .desc  = N_("Create a directory per channel when "
                           "storing recordings. If both this and the "
                           "'directory "
            ...
            },
            {
                ...
                .desc = N_("Subdirectory to use for tvmovies when using the $q "
                           "specifier. "
                           "This can contain any alphanumeric "
                           "characters (A-Za-z0-9). Other characters may be "
                           "supported depending "
            ...
            },
            {
                ...
                .desc  = N_("Subdirectory to use for tvshows when using the $q "
                           "specifier. "
                           "This can contain any alphanumeric "
                           "characters (A-Za-z0-9). Other characters may be "
                           "supported depending "
            ...
            },
            {
                ...
                .desc    = N_("Maximum allowed difference between event start "
                           "time when "
            ...
            },
            {
                ...
                .desc  = N_("The maximum number of entries that can be "
                           "matched."),
            ...
            },
            {
                ...
                .desc  = N_("The maximum number of recordings that can be "
                           "scheduled."),
            ...
            },
            {
                ...
                .desc  = N_("Commercials will be dropped from the "
                           "recordings. Commercial detection works using "
                           "EITp/f "
                           "(EPG running state) and for the Swedish channel "
                           "TV4 "
                           "(using teletext info)."),
            ...
            },
            {
                ...
                .desc  = N_("Script/program to run when a recording starts "
                           "(service is subscribed but no filename "
                           "available)."),
            ...
            },
            {
                ...
                .desc  = N_("Script/program to run when a recording gets "
                           "removed."),
