...
typedef struct tvhcsa {
  ...
  int csa_keylen;
  void (*csa_descramble)(struct tvhcsa* csa, struct mpegts_service* s, const uint8_t* tsb, int len);
...
static inline int tvhcsa_set_type(tvhcsa_t* csa, struct mpegts_service* s, int type) {
  return -1;
}
...
#if ENABLE_DVBCSA
typedef void* (*dvbcsa_dl_bs_key_set_type)(const unsigned char ecm,
