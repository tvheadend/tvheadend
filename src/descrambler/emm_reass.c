 ...
 * conax emm handler
 */
static void emm_conax(emm_reass_t* ra,
    const uint8_t*                 data,
    int                            len,
    void*                          mux,
    emm_send_t                     send,
 ...
 */
static void emm_irdeto(emm_reass_t* ra,
    const uint8_t*                  data,
    int                             len,
    void*                           mux,
    emm_send_t                      send,
 ...
 * inspired by opensasc-ng, https://opensvn.csie.org/traccgi/opensascng/
 */
static void emm_seca(emm_reass_t* ra,
    const uint8_t*                data,
    int                           len,
    void*                         mux,
    emm_send_t                    send,
...
static void emm_viaccess(emm_reass_t* ra,
    const uint8_t*                    data,
    int                               len,
    void*                             mux,
    emm_send_t                        send,
  ...
  if (data[0] == 0x8c || data[0] == 0x8d) {
    ...
    if (match) {
      ...
      if (ass == NULL)
        return;
      l = EP_VIACCESS(ep, idx).shared_len -
          (ass - EP_VIACCESS(ep, idx).shared_emm);
 ...
 * dre emm handler
 */
static void emm_dre(emm_reass_t* ra,
    const uint8_t*               data,
    int                          len,
    void*                        mux,
    emm_send_t                   send,
...
static void emm_nagra(emm_reass_t* ra,
    const uint8_t*                 data,
    int                            len,
    void*                          mux,
    emm_send_t                     send,
...
static void emm_nds(emm_reass_t* ra,
    const uint8_t*               data,
    int                          len,
    void*                        mux,
    emm_send_t                   send,
...
static void emm_streamguard(emm_reass_t* ra,
  ...
  tvhinfo(ra->subsys,
      "emm_streamguard streamguard card data emm get,here lots of works "
      "todo...");
...
static void emm_bulcrypt(emm_reass_t* ra,
    const uint8_t*                    data,
    int                               len,
    void*                             mux,
    emm_send_t                        send,
...
static void emm_griffin(emm_reass_t* ra,
    const uint8_t*                   data,
    int                              len,
    void*                            mux,
    emm_send_t                       send,
 ...
 *   gcc -o emmra -DEMM_REASS_TEST -Isrc -Ibuild.linux -Wall
 * src/descrambler/emm_reass.c The output binary is named 'emmra' .
 *
