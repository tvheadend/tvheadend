 ...
 *
 */
static void emm_cache_insert(emm_reass_t* ra, uint32_t crc) {
...
static int emm_cache_lookup(emm_reass_t* ra, uint32_t crc) {
  int i, off;
 ...
 */
static void
emm_conax(emm_reass_t* ra, const uint8_t* data, int len, void* mux, emm_send_t send, void* aux) {
  emm_provider_t* ep;
  int             i, match = 0;
 ...
 */
static void
emm_irdeto(emm_reass_t* ra, const uint8_t* data, int len, void* mux, emm_send_t send, void* aux) {
 ...
 */
static void
emm_seca(emm_reass_t* ra, const uint8_t* data, int len, void* mux, emm_send_t send, void* aux) {
  int             i, match = 0;
  emm_provider_t* ep;
 ...
 * inspired by opensasc-ng, https://opensvn.csie.org/traccgi/opensascng/
 */
static const uint8_t* nano_start(const uint8_t* data, int len) {
  int l;
...
static const uint8_t* nano_checknano90fromnano(const uint8_t* data, int len) {
  if (data && len > 4 && data[0] == 0x90 && data[1] == 0x03)
    return data;
  return NULL;
}

static const uint8_t* nano_checknano90(const uint8_t* data, int len) {
...
static int sort_nanos(uint8_t* dest, const uint8_t* src, int len) {
  int w = 0, c = -1;
...
static void
emm_viaccess(emm_reass_t* ra, const uint8_t* data, int len, void* mux, emm_send_t send, void* aux) {
  ...
  if (data[0] == 0x8c || data[0] == 0x8d) {
    ...
    PROVIDERS_FOREACH(ra, i, ep) {
      ...
      if ((data[6] & 2))
        continue;
      for (idx = 0; idx < 2; idx++)
        if (EP_VIACCESS(ep, idx).shared_emm && EP_VIACCESS(ep, idx).shared_mux == mux) {
          id = via_provider_id(EP_VIACCESS(ep, idx).shared_emm, EP_VIACCESS(ep, idx).shared_len);
          if (id == ep->id) {
            match = 1;
            break;
          }
    ...
    if (match) {
      ...
      if ((data[6] & 2) == 0) {
        ...
        tmp[len++]  = 0x9e;
        tmp[len++]  = addrlen;
        memcpy(&tmp[len], &ass[0], addrlen);
        len += addrlen;
        tmp[len++] = 0xf0;
        tmp[len++] = 0x08;
        memcpy(&tmp[len], &ass[addrlen], 8);
        len += 8;
      } else {
        memcpy(tmp, ass, len);
      }
      ass = nano_start(EP_VIACCESS(ep, idx).shared_emm, EP_VIACCESS(ep, idx).shared_len);
      if (ass == NULL)
        return;
      l = EP_VIACCESS(ep, idx).shared_len - (ass - EP_VIACCESS(ep, idx).shared_emm);
      memcpy(&tmp[len], ass, l);
      len += l;
 ...
 */
static void
emm_dre(emm_reass_t* ra, const uint8_t* data, int len, void* mux, emm_send_t send, void* aux) {
  int             i, match = 0;
  emm_provider_t* ep;
...
static void
emm_nagra(emm_reass_t* ra, const uint8_t* data, int len, void* mux, emm_send_t send, void* aux) {
  int     match = 0;
  uint8_t hexserial[4];
...
static void
emm_nds(emm_reass_t* ra, const uint8_t* data, int len, void* mux, emm_send_t send, void* aux) {
 ...
 */
static void emm_streamguard(emm_reass_t* ra,
    const uint8_t*                       data,
    int                                  len,
    void*                                mux,
    emm_send_t                           send,
...
static void emm_cryptoworks(emm_reass_t* ra,
    const uint8_t*                       data,
    int                                  len,
    void*                                mux,
    emm_send_t                           send,
    ...
    case 0x86: /* emm-sb */
      ...
      if (RA_CRYPTOWORKS(ra).shared_emm && RA_CRYPTOWORKS(ra).shared_mux == mux) {
        ...
        if (composed) {
          ...
          memcpy(tmp + len - 5,
              RA_CRYPTOWORKS(ra).shared_emm + 12,
...
static void
emm_bulcrypt(emm_reass_t* ra, const uint8_t* data, int len, void* mux, emm_send_t send, void* aux) {
  int match = 0;
...
static void
emm_griffin(emm_reass_t* ra, const uint8_t* data, int len, void* mux, emm_send_t send, void* aux) {
  emm_provider_t* ep;
  int             i;
...
void emm_filter(emm_reass_t* ra,
    const uint8_t*           data,
    int                      len,
    void*                    mux,
    emm_send_t               send,
...
void emm_reass_init(emm_reass_t* ra, int subsys, uint16_t caid) {
  ...
  switch (ra->type) {
    ...
    case CARD_IRDETO:
      ra->do_emm = emm_irdeto;
      break;
    case CARD_SECA:
      ra->do_emm = emm_seca;
      break;
    case CARD_VIACCESS:
      ra->do_emm = emm_viaccess;
      break;
    case CARD_DRE:
      ra->do_emm = emm_dre;
      break;
    ...
    case CARD_CRYPTOWORKS:
      ra->do_emm = emm_cryptoworks;
      break;
    case CARD_BULCRYPT:
      ra->do_emm = emm_bulcrypt;
      break;
    case CARD_STREAMGUARD:
      ra->do_emm = emm_streamguard;
      break;
    case CARD_GRIFFIN:
      ra->do_emm = emm_griffin;
      break;
    default:
      break;
  }
...
static char* emms[] = {"8C 70 0C 90 03 04 08 21 A9 05 46 8B 46 BA 04",
...
static int decode_hexa(uint8_t* dst, int len, const char* src) {
  int res = 0, i;
...
static void emm_result(void* aux, const uint8_t* radata, int ralen, void* mux) {
...
int main(int argc, char* argv[]) {
  ...
  tvhinfo("config",
      "UA = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
      ra.ua[0],
      ra.ua[1],
      ra.ua[2],
      ra.ua[3],
  ...
  sa = ra.providers->sa;
  tvhinfo("config",
      "SA = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
      sa[0],
      sa[1],
      sa[2],
      sa[3],
