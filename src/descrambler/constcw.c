 ...
 *
 */
static const char* constcw_name(constcw_t* ccw) {
  return idnode_get_title(&ccw->cac_id, NULL, ccw->ccw_name, sizeof(ccw->ccw_name));
}

/**
 *
 */
static int constcw_algo(caclient_t* cac) {
 ...
 *
 */
static int constcw_key_size(caclient_t* cac) {
  constcw_t* ccw = (constcw_t*)cac;
 ...
 *
 */
static int constcw_ecm_reset(th_descrambler_t* th) {
  return 1;
}
 ...
 */
static void constcw_service_destroy(th_descrambler_t* td) {
 ...
 */
static void constcw_service_start(caclient_t* cac, service_t* t) {
  constcw_t*           ccw = (constcw_t*)cac;
  ...
  if (!mt->s_dvb_forcecaid) {
    ...
    TAILQ_FOREACH (st, &t->s_components.set_filter, es_filter_link) {
      LIST_FOREACH (c, &st->es_caids, link) {
        if (c->use && c->caid == ccw->ccw_caid && c->providerid == ccw->ccw_providerid)
          break;
      }
      if (c)
        break;
  ...
  descrambler_keys(td, constcw_algo(cac), 0, ccw->ccw_key_even, ccw->ccw_key_odd);
}

/**
 *
 */
static void constcw_free(caclient_t* cac) {
  constcw_t*         ccw = (constcw_t*)cac;
  constcw_service_t* ct;
 ...
 *
 */
static void constcw_conf_changed(caclient_t* cac) {
  if (cac->cac_enabled) {
 ...
 *
 */
static int constcw_class_key_set(void* o, const void* v, uint8_t* dkey) {
  const char* s       = v ?: "";
...
static int constcw_class_key_even_set(void* o, const void* v) {
  constcw_t* ccw = o;
  return constcw_class_key_set(o, v, ccw->ccw_key_even);
}

static int constcw_class_key_odd_set(void* o, const void* v) {
  constcw_t* ccw = o;
  return constcw_class_key_set(o, v, ccw->ccw_key_odd);
}

static const void* constcw_class_key_get(void* o, const uint8_t* key) {
...
static const void* constcw_class_key_even_get(void* o) {
  constcw_t* ccw = o;
  return constcw_class_key_get(o, ccw->ccw_key_even);
}

static const void* constcw_class_key_odd_get(void* o) {
  constcw_t* ccw = o;
...
const idclass_t caclient_ccw_csa_cbc_class = {.ic_super = &caclient_class,
...
const idclass_t caclient_ccw_des_ncb_class = {.ic_super = &caclient_class,
...
const idclass_t caclient_ccw_aes_ecb_class = {.ic_super = &caclient_class,
...
const idclass_t caclient_ccw_aes128_ecb_class = {.ic_super = &caclient_class,
