...
static void tvhcsa_empty_flush(tvhcsa_t* csa, struct mpegts_service* s) {
  /* empty - no queue */
}

static void
tvhcsa_aes_ecb_descramble(tvhcsa_t* csa, struct mpegts_service* s, const uint8_t* tsb, int len) {
...
static void
tvhcsa_aes128_ecb_descramble(tvhcsa_t* csa, struct mpegts_service* s, const uint8_t* tsb, int len) {
  const uint8_t *tsb2, *end2;
...
static void
tvhcsa_des_ncb_descramble(tvhcsa_t* csa, struct mpegts_service* s, const uint8_t* tsb, int len) {
  const uint8_t *tsb2, *end2;
...
static void tvhcsa_csa_cbc_flush(tvhcsa_t* csa, struct mpegts_service* s) {
#if ENABLE_DVBCSA
  tvhtrace(LS_CSA,
      "%p: CSA flush - descramble packets for service \"%s\" MAX=%d even=%d odd=%d fill=%d",
      csa,
      ((mpegts_service_t*)s)->s_dvb_svcname,
      csa->csa_cluster_size,
      csa->csa_fill_even,
      csa->csa_fill_odd,
      csa->csa_fill);
...
static void tvhcsa_csa_cbc_descramble(tvhcsa_t* csa,
      ...
      if (pkt[3] & 0x20) {    // incomplete packet
        ...
        if (offset >= 188) { // invalid offset (residue handling?)
          if (tvhlog_limit(&csa->tvhcsa_loglimit, 30))
            tvhtrace(LS_CSA,
                "invalid payload offset in packet for service \"%s\" (offset=%d pkt[3]=0x%02x "
                "pkt[4]=0x%02x)",
                ((mpegts_service_t*)s)->s_dvb_svcname,
                (int)offset,
                pkt[3],
...
int tvhcsa_set_type(tvhcsa_t* csa, struct mpegts_service* s, int type) {
  ...
  switch (type) {
    case DESCRAMBLER_CSA_CBC:
      ...
      tvhtrace(LS_CSA,
          "%p: service \"%s\" using CSA batch size = %d for decryption",
          csa,
          ((mpegts_service_t*)s)->s_dvb_svcname,
          csa->csa_cluster_size);
...
#if ENABLE_DVBCSA
      csa->csa_tsbbatch_even =
          malloc((csa->csa_cluster_size + 1) * sizeof(struct dvbcsa_bs_batch_s));
      csa->csa_tsbbatch_odd =
          malloc((csa->csa_cluster_size + 1) * sizeof(struct dvbcsa_bs_batch_s));
...
void tvhcsa_init(tvhcsa_t* csa) {
  ...
  if (!dvbcsa_dl_scanned) {
    ...
    dvbcsa_dl_bs_key_set_ecm =
        (dvbcsa_dl_bs_key_set_type)dlsym(RTLD_DEFAULT, "dvbcsa_bs_key_set_ecm");
    tvhinfo(LS_DESCRAMBLER,
        "dvbcsa_bs_key_set_ecm() function%s detected in libdvbcsa",
        dvbcsa_dl_bs_key_set_ecm ? "" : " not");
#else
    dvbcsa_dl_bs_key_set_ecm = (dvbcsa_dl_bs_key_set_type)NULL;
    tvhinfo(LS_DESCRAMBLER,
        "can not detect dvbcsa_bs_key_set_ecm() function: RTLD_DEFAULT not defined on this system");
#endif
  }
#endif
  csa->csa_type   = 0;
...
#if ENABLE_DVBCSA
void dvbcsa_bs_key_set_wrap(const unsigned char ecm,
    const dvbcsa_cw_t                           cw,
