...
static void tvhcsa_aes_ecb_descramble(tvhcsa_t* csa,
...
static void tvhcsa_aes128_ecb_descramble(tvhcsa_t* csa,
    struct mpegts_service*                         s,
    const uint8_t*                                 tsb,
    int                                            len) {
  const uint8_t *tsb2, *end2;
...
static void tvhcsa_des_ncb_descramble(tvhcsa_t* csa,
    struct mpegts_service*                      s,
    const uint8_t*                              tsb,
    int                                         len) {
  const uint8_t *tsb2, *end2;
...
#if ENABLE_DVBCSA
  tvhtrace(LS_CSA,
      "%p: CSA flush - descramble packets for service \"%s\" MAX=%d even=%d "
      "odd=%d fill=%d",
      csa,
      ...
      if (pkt[3] & 0x20) {    // incomplete packet
        ...
        if (offset >= 188) { // invalid offset (residue handling?)
          if (tvhlog_limit(&csa->tvhcsa_loglimit, 30))
            tvhtrace(LS_CSA,
                "invalid payload offset in packet for service \"%s\" "
                "(offset=%d pkt[3]=0x%02x pkt[4]=0x%02x)",
                ((mpegts_service_t*)s)->s_dvb_svcname,
                (int)offset,
                pkt[3],
                pkt[4]);
...
#if ENABLE_DVBCSA
      csa->csa_tsbbatch_even = malloc(
          (csa->csa_cluster_size + 1) * sizeof(struct dvbcsa_bs_batch_s));
      csa->csa_tsbbatch_odd = malloc(
          (csa->csa_cluster_size + 1) * sizeof(struct dvbcsa_bs_batch_s));
...
void tvhcsa_init(tvhcsa_t* csa) {
  ...
  if (!dvbcsa_dl_scanned) {
    ...
    tvhinfo(LS_DESCRAMBLER,
        "can not detect dvbcsa_bs_key_set_ecm() function: RTLD_DEFAULT not "
