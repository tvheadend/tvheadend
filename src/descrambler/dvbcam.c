 ...
 *
 */
static int dvbcam_service_check_caid(dvbcam_active_service_t* as,
    uint16_t                                                  caid) {
  uint16_t caid1;
  int      i;
...
static void dvbcam_unregister_ddci(dvbcam_active_cam_t* ac,
 ...
 *
 */
static int dvbcam_ca_lookup(dvbcam_active_cam_t* ac,
...
#if ENABLE_DDCI
static int dvbcam_descramble_ddci(service_t* t,
      ...
      LIST_FOREACH (c, &st->es_caids, link) {
        ...
        TAILQ_FOREACH (ac, &dvbcam_active_cams, global_link) {
          if (dvbcam_ca_lookup(ac,
            ...
            lcat = ac->ca->lca_transport;
            if (lcat->lddci &&
                linuxdvb_ddci_do_not_assign(lcat->lddci, t, dc->multi))
              continue;
#endif
            tvhtrace(LS_DVBCAM,
      ...
      TAILQ_FOREACH (st, &t->s_components.set_filter, es_link) {
        ...
        LIST_FOREACH (c, &st->es_caids, link) {
          if (i >= ARRAY_SIZE(as->caid_list)) {
            tvherror(LS_DVBCAM,
                "%s/%p: CAID service list overflow",
                ac->ca->lca_name,
                t);
            break;
          }
  ...
  if (as && as->lddci) {
    ...
    if (mi) {
      ...
      for (i = 0; i < ecm_to_open.count; i++)
        mpegts_input_open_pid(mi,
            mm,
            ecm_to_open.pids[i].pid,
            MPS_SERVICE,
            MPS_WEIGHT_CA,
            t,
            0);
      for (i = 0; i < ecm_to_close.count; i++)
        mpegts_input_close_pid(mi,
            mm,
            ecm_to_close.pids[i].pid,
            MPS_SERVICE,
            t);
      tvh_mutex_unlock(&t->s_stream_mutex);
...
#if ENABLE_DDCI
static void
dvbcam_cat_entry(void* _aux, uint16_t caid, uint32_t prov, uint16_t pid) {
  __cat_entry_t* aux = _aux;
  if (!mpegts_pid_rexists(aux->pids, pid)) {
    mpegts_pid_add(aux->pids, pid, 0);
    tvhtrace(LS_DVBCAM,
        "%p: add EMM PID %d (%04X) for CAID %04X",
        aux->service,
        pid,
        pid,
        caid);
 ...
 *
 */
static void dvbcam_cat_update(caclient_t* cac,
    mpegts_mux_t*                         mux,
    const uint8_t*                        data,
    int                                   len) {
#if ENABLE_DDCI
 ...
 *
 */
static htsmsg_t* caclient_dvbcam_class_caid_selection_list(void* o,
    const char*                                                  lang) {
    ...
    .ic_properties = (const property_t[]){
        {
            ...
            .desc =
                N_("Limit of concurrent descrambled services (per one CAM)."),
        ...
        },
        {
            ...
            .desc =
                N_("A list of allowed CAIDs (hexa format, comma separated). "
                   "E.g. '0D00,0F00,0100'."),
