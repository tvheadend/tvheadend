...
cccam_decrypt(struct cccam_crypt_block* block, uint8_t* data, int32_t len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    z       = data[i];
    data[i] = z ^
        block->keytable[(block->keytable[block->counter] +
                            block->keytable[block->sum]) &
            0xff] ^
        block->state;
    z = data[i];
    block->state ^= z;
...
static int cccam_running_reply(cccam_t* cccam, uint8_t* buf, int len) {
  ...
  tvhtrace(cccam->cc_subsys,
      "%s: response msg type=%d, response",
      cccam->cc_name,
      buf[1]);
  ...
  switch (buf[1]) {
    ...
    case MSG_CARD_REMOVED:
      if (len >= 8) {
        ...
        tvhtrace(cccam->cc_subsys,
            "%s: del card %08X message received",
            cccam->cc_name,
            cardid);
    ...
    case MSG_CLI_DATA:
      tvhinfo(cccam->cc_subsys,
          "%s: CCcam server authentication completed",
          cccam->cc_name);
      break;
    default:
 ...
 *
 */
static int cccam_read_message0(cccam_t* cccam,
    const char*                         state,
    sbuf_t*                             rbuf,
    int                                 timeout) {
  uint16_t                 msglen;
...
static int cccam_send_login(cccam_t* cccam) {
  ...
  if (cc_read((cclient_t*)cccam, data, 20, 5000)) {
    tvherror(cccam->cc_subsys,
        "%s: login failed, pwd ack not received",
        cccam->cc_name);
    return -2;
  }
...
static int cccam_init_session(void* cc) {
  ...
  if ((r = cc_read(cc, buf, 16, 5000))) {
    tvhinfo(cccam->cc_subsys,
        "%s: init error (no init seed received)",
        cccam->cc_name);
    return -1;
  }
...
static htsmsg_t* caclient_cccam_class_cccam_extended_list(void* o,
    const char*                                                 lang) {
...
static htsmsg_t* caclient_cccam_class_cccam_version_list(void* o,
    const char*                                                lang) {
  static const struct strtab tab[] = {
    ...
    .ic_properties                                = (const property_t[]){
        {
            ...
            .name = N_("Node ID"),
            .desc = N_(
                "Client node ID. Leave field empty to generate a random ID."),
