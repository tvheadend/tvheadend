 ...
 *
 */
static inline void uint8_swap(uint8_t* p1, uint8_t* p2) {
  uint8_t tmp = *p1;
  *p1         = *p2;
  *p2         = tmp;
}
 ...
 *
 */
static void cccam_crypt_xor(uint8_t* buf) {
  uint8_t i;
 ...
 *
 */
static void cccam_crypt_init(struct cccam_crypt_block* block, uint8_t* key, int32_t len) {
  uint32_t i = 0;
  uint8_t  j = 0;
 ...
 *
 */
static void cccam_decrypt(struct cccam_crypt_block* block, uint8_t* data, int32_t len) {
  int32_t i;
  uint8_t z;
  ...
  for (i = 0; i < len; i++) {
    ...
    z       = data[i];
    data[i] = z ^
        block->keytable[(block->keytable[block->counter] + block->keytable[block->sum]) & 0xff] ^
        block->state;
    z = data[i];
    block->state ^= z;
  }
 ...
 *
 */
static void cccam_encrypt(struct cccam_crypt_block* block, uint8_t* data, int32_t len) {
  int32_t i;
  uint8_t z;
  for (i = 0; i < len; i++) {
    block->counter++;
...
static void cccam_decrypt_cw(uint8_t* nodeid, uint32_t card_id, uint8_t* cws) {
  uint8_t  tmp, i;
 ...
 *
 */
static int cccam_oscam_check(cccam_t* cccam, uint8_t* buf) {
  if (!cccam->cccam_oscam) {
    ...
    for (i = 0; i < 14; i++)
      sum += buf[i];
    tvhtrace(cccam->cc_subsys,
        "%s: oscam check sum %04X recv sum %04X",
        cccam->cc_name,
        sum,
        recv_sum);
 ...
 *
 */
static int cccam_oscam_nodeid_check(cccam_t* cccam, uint8_t* buf) {
  if (!cccam->cccam_oscam) {
    uint16_t sum      = 0x1234;
    ...
    for (i = 0; i < 6; i++)
      sum += buf[i];
    tvhtrace(cccam->cc_subsys,
        "%s: oscam nodeid check sum %04X recv sum %04X",
        cccam->cc_name,
        sum,
        recv_sum);
    cccam->cccam_oscam = sum == recv_sum;
 ...
 *
 */
static inline uint8_t* cccam_set_ua(uint8_t* dst, uint8_t* src) {
  /* FIXME */
 ...
 *
 */
static inline uint8_t* cccam_set_sa(uint8_t* dst, uint8_t* src) {
 ...
 */
static int cccam_decode_card_data_reply(cccam_t* cccam, uint8_t* msg) {
  ...
  pcard = cc_new_card((cclient_t*)cccam,
      (msg[12] << 8) | msg[13],
 ...
 *
 */
static void cccam_handle_keys(cccam_t* cccam,
    cc_service_t*                      ct,
    cc_ecm_section_t*                  es,
    uint8_t*                           buf,
    int                                len,
 ...
 *
 */
static void cccam_handle_partner(cccam_t* cccam, uint8_t* msg) {
  char* saveptr;
  char* p;
  ...
  tvhinfo(cccam->cc_subsys,
      "%s: server supports extended capabilities%s%s%s",
      cccam->cc_name,
 ...
 * cc_mutex is held
 */
static int cccam_running_reply(cccam_t* cccam, uint8_t* buf, int len) {
  cc_service_t*     ct;
  cc_ecm_section_t* es;
  uint32_t          cardid;
  uint8_t           seq;
  ...
  tvhtrace(cccam->cc_subsys, "%s: response msg type=%d, response", cccam->cc_name, buf[1]);
  ...
  switch (buf[1]) {
    ...
    case MSG_SRV_DATA:
      if (len == 0x4c) {
        ...
        tvhtrace(cccam->cc_subsys, "%s: SRV_DATA: unknown length %d", cccam->cc_name, len);
      }
      break;
    case MSG_CLI_DATA:
      tvhinfo(cccam->cc_subsys, "%s: CCcam server authentication completed", cccam->cc_name);
      break;
    default:
      tvhwarn(cccam->cc_subsys, "%s: Unknown message received", cccam->cc_name);
      break;
  }
  return 0;
}
 ...
 *
 */
static int cccam_read_message0(cccam_t* cccam, const char* state, sbuf_t* rbuf, int timeout) {
  uint16_t                 msglen;
  uint8_t                  hdr[4];
 ...
 *
 */
static int cccam_send_msg(cccam_t* cccam,
    cccam_msg_type_t               cmd,
    uint8_t*                       buf,
    size_t                         len,
    int                            enq,
    uint8_t                        seq,
 ...
 * Send keep alive
 */
static void cccam_send_ka(void* cc) {
  cccam_t* cccam = cc;
  uint8_t  buf[4];
...
static void cccam_send_oscam_extended(cccam_t* cccam) {
  ...
  snprintf(buf,
      ...
      cccam_get_version_str(cccam),
      cccam_get_build_str(cccam),
      "unknown");
 ...
 *
 */
static void sha1_make_login_key(cccam_t* cccam, uint8_t* buf) {
  SHA_CTX sha1;
 ...
 * Login command
 */
static int cccam_send_login(cccam_t* cccam) {
 ...
 *
 */
static void cccam_send_cli_data(cccam_t* cccam) {
 ...
 *
 */
static void cccam_oscam_update_idnode(cccam_t* cccam) {
  uint8_t  p[8];
  uint16_t sum = 0x1234;
  int32_t  i;
 ...
 *
 */
static int cccam_init_session(void* cc) {
  cccam_t* cccam = cc;
  uint8_t  buf[256];
  int      r;
 ...
 *
 */
static int cccam_send_ecm(void* cc,
    cc_service_t*               ct,
    cc_ecm_section_t*           es,
    cc_card_data_t*             pcard,
    const uint8_t*              msg,
  ...
  if (cccam_set_busy(cccam)) {
    tvhinfo(cccam->cc_subsys,
        "%s: Ignore ECM request %02X (server is busy)",
        cccam->cc_name,
        msg[0]);
    return -1;
  }
 ...
 *
 */
static void cccam_send_emm(void* cc,
    cc_service_t*                ct,
    cc_card_data_t*              pcard,
    uint32_t                     provid,
    const uint8_t*               msg,
  ...
  if (len > 255) {
    tvherror(cccam->cc_subsys, "%s: EMM too big (%d bytes)", cccam->cc_name, len);
    return;
  }

  if (cccam_set_busy(cccam)) {
    tvhinfo(cccam->cc_subsys,
        "%s: Ignore EMM request %02X (server is busy)",
        cccam->cc_name,
        msg[0]);
    return;
  }
 ...
 *
 */
static int cccam_read(void* cc, sbuf_t* rbuf) {
  cccam_t*  cccam       = cc;
 ...
 *
 */
static void cccam_no_services(void* cc) {
 ...
 *
 */
static void cccam_conf_changed(caclient_t* cac) {
 ...
 *
 */
static int caclient_cccam_nodeid_set(void* o, const void* v) {
  cccam_t*    cccam = o;
  const char* s     = v ?: "";
  uint8_t     key[8];
  int         i, u, l;
...
static const void* caclient_cccam_nodeid_get(void* o) {
  cccam_t* cccam = o;
  snprintf(prop_sbuf,
      PROP_SBUF_LEN,
...
static htsmsg_t* caclient_cccam_class_cccam_extended_list(void* o, const char* lang) {
...
static htsmsg_t* caclient_cccam_class_cccam_version_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
const idclass_t caclient_cccam_class = {.ic_super = &caclient_cc_class,
