...
typedef struct cc_ecm_section {
  ...
  enum { ES_UNKNOWN, ES_RESOLVED, ES_FORBIDDEN, ES_IDLE } es_keystate;
...
typedef struct cc_service {
   ...
   * ECM Status
   */
  enum { ECM_INIT, ECM_VALID, ECM_RESET } ecm_state;
...
typedef struct cclient {
  ...
  int (*cc_send_ecm)(void* cc,
      cc_service_t*        ct,
      cc_ecm_section_t*    es,
      cc_card_data_t*      pcard,
      const uint8_t*       data,
      int                  len);
  void (*cc_send_emm)(void* cc,
      cc_service_t*         ct,
      cc_card_data_t*       pcard,
      uint32_t              provid,
      const uint8_t*        data,
...
cc_ecm_section_t* cc_find_pending_section(cclient_t* cc, uint32_t seq, cc_service_t** ct);
