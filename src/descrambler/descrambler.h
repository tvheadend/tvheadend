...
typedef struct th_descrambler_runtime {
  ...
  int (
      *dr_descramble)(struct service* t, struct elementary_stream* st, const uint8_t* tsb, int len);
  int                   dr_ca_count;
  int                   dr_ca_resolved;
  int                   dr_ca_failed;
  int                   dr_ca_fatal;
...
typedef void (*descrambler_section_callback_t)(void* opaque,
    int                                              pid,
...
void        descrambler_notify(th_descrambler_t* t,
           ...
           const char*                           from,
           const char*                           protocol);
int  descrambler_pass(struct service* t, struct elementary_stream* st, const uint8_t* tsb, int len);
int  descrambler_descramble(struct service* t,
     struct elementary_stream*              st,
     const uint8_t*                         tsb,
     int                                    len);
void descrambler_flush_table_data(struct service* t);
int  descrambler_open_pid(struct mpegts_mux* mux,
     void*                                   opaque,
     int                                     pid,
     descrambler_section_callback_t          callback,
...
int  descrambler_open_emm(struct mpegts_mux* mux,
     void*                                   opaque,
     int                                     caid,
     int                                     provid,
     descrambler_section_callback_t          callback);
int  descrambler_close_emm(struct mpegts_mux* mux, void* opaque, int caid, int provid);
