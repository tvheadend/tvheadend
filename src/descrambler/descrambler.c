...
descrambler_data_destroy(th_descrambler_runtime_t* dr, th_descrambler_data_t* dd, int skip) {
  if (dd) {
    if (skip && dr->dr_skip)
      ts_skip_packet2((mpegts_service_t*)dr->dr_service, dd->dd_sbuf.sb_data, dd->dd_sbuf.sb_ptr);
...
static void descrambler_data_append(th_descrambler_runtime_t* dr, const uint8_t* tsb, int len) {
...
static int descrambler_data_key_check(th_descrambler_runtime_t* dr, uint8_t key, int len) {
  th_descrambler_data_t* dd;
  int                    off = 0, l;
  uint_fast8_t           ki;
...
static int
descrambler_data_analyze(th_descrambler_runtime_t* dr, th_descrambler_data_t* dd, uint8_t ki) {
...
static void descrambler_load_hints(htsmsg_t* m) {
  ...
  HTSMSG_FOREACH(f, m) {
    ...
    hint.dh_ecmparity =
        str2val_def(htsmsg_get_str(e, "ecmparity"), ecmparitytab, ECM_PARITY_DEFAULT);
...
static void descrambler_notify_deliver(mpegts_service_t* t, descramble_info_t* di) {
...
cont:
  ...
  if (even && memcmp(empty, even, tk->key_csa.csa_keylen)) {
    ...
    tk->key_pid = pid;
    changed |= 1;
    if (tk->key_timestamp[0] == 0 || descrambler_data_key_check(dr, 0x80, dr->dr_queue_total) >= 0)
      insert |= 1;
  ...
  if (odd && memcmp(empty, odd, tk->key_csa.csa_keylen)) {
    ...
    tk->key_pid = pid;
    changed |= 2;
    if (tk->key_timestamp[1] == 0 || descrambler_data_key_check(dr, 0xc0, dr->dr_queue_total) >= 0)
      insert |= 2;
...
key_changed(th_descrambler_runtime_t* dr, th_descrambler_key_t* tk, uint8_t ki, int64_t timestamp) {
  return tk->key_index != (ki & 0x40) && tk->key_start + dr->dr_ecm_key_margin < timestamp;
}
...
key_find_struct(th_descrambler_runtime_t* dr, const uint8_t* tsb, service_t* t) {
  th_descrambler_key_t* tk;
...
int descrambler_pass(service_t* t, elementary_stream_t* st, const uint8_t* tsb, int len) {
  if ((tsb[3] & 0x80) == 0)
    ts_recv_packet0((mpegts_service_t*)t, st, tsb, len);
  return 1;
}

int descrambler_descramble(service_t* t, elementary_stream_t* st, const uint8_t* tsb, int len) {
  ...
  if (dr->dr_ca_resolved > 0) {
    ...
    for (dd = TAILQ_FIRST(&dr->dr_queue); dd; dd = dd_next) {
      ...
      if (dr->dr_key_multipid) {
        ...
        if (tk == NULL) {
          if (t->s_start_time + 3000000 < mclk() && tvhlog_limit(&dr->dr_loglimit_key, 10))
            tvhwarn(LS_DESCRAMBLER,
...
static int
descrambler_table_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  ...
  TAILQ_FOREACH (ds, &dt->sections, link) {
    ...
    if (des->last_data == NULL || len != des->last_data_len || memcmp(des->last_data, ptr, len)) {
      free(des->last_data);
        ...
        if ((t = mt->mt_service) != NULL) {
          ...
          if (dr) {
            if (!dr->dr_quick_ecm && !des->quick_ecm_called) {
              ...
              if (dr->dr_quick_ecm)
                tvhdebug(LS_DESCRAMBLER, "quick ECM enabled for service '%s'", t->s_dvb_svcname);
            }
                ...
                TAILQ_FOREACH (st, &mt->mt_service->s_components.set_filter, es_filter_link) {
                  ...
                  LIST_FOREACH (ca, &st->es_caids, link) {
                    if (ca->use == 0)
                      continue;
                    tk->key_csa.csa_ecm =
                        (caid_is_videoguard(ca->caid) && (ptr[4] != 0 && (ptr[2] - ptr[4]) == 4))
                        ? 4
                        : 0;
                    tvhtrace(LS_DESCRAMBLER,
                        "key ecm=%X (caid=%04X)",
            ...
            tvhtrace(LS_DESCRAMBLER,
                "ECM message %02x:%02x (section %d, len %d, pid %d) for service \"%s\"",
                ptr[0],
                ptr[1],
                des->number,
                len,
...
static void descrambler_cat_entry(void* _mux, uint16_t caid, uint32_t prov, uint16_t pid) {
