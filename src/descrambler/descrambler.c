 ...
 *
 */
static void descrambler_data_destroy(th_descrambler_runtime_t* dr,
...
static void descrambler_data_append(th_descrambler_runtime_t* dr,
    const uint8_t*                                            tsb,
    int                                                       len) {
  th_descrambler_data_t* dd;
  const uint8_t*         tsb0;
  ...
  if (dd && (tsb0 = dd->dd_sbuf.sb_data) != NULL) {
    ...
    if (dd && monocmpfastsec(dd->dd_timestamp, mclk()) &&
        ...
        pid1 == pid2) {
      debug2("%p: data append %d, timestamp %ld, %s[%d]",
          dr,
          len,
          dd->dd_timestamp,
          keystr(tsb0),
  ...
  debug2("%p: data append2 %d, timestamp %ld, %s[%d]",
      dr,
      len,
      dd->dd_timestamp,
      keystr(tsb),
...
static void descrambler_data_add_key(th_descrambler_runtime_t* dr,
  ...
  debug2("%p: data %s key %d, timestamp %ld",
      dr,
      head ? "insert" : "append",
      tk->key_pid,
      dd->dd_timestamp);
...
static void descrambler_data_cut(th_descrambler_runtime_t* dr, int len) {
  th_descrambler_data_t* dd;

  while (len > 0) {
    TAILQ_FOREACH (dd, &dr->dr_queue, dd_link)
      if (dd && dd->dd_sbuf.sb_data)
        break;
    if (dd == NULL)
      return;
    if (dr->dr_skip)
...
static int descrambler_data_analyze(th_descrambler_runtime_t* dr,
    th_descrambler_data_t*                                    dd,
    uint8_t                                                   ki) {
  ...
  for (dd2 = TAILQ_NEXT(dd, dd_link); dd2; dd2 = TAILQ_NEXT(dd2, dd_link)) {
    ...
    if (tsb0 == NULL || dd->dd_sbuf.sb_ptr == 0)
      continue;
 ...
 *
 */
static void descrambler_destroy_ecmsec(descrambler_ecmsec_t* des) {
...
static void descrambler_destroy_all_ecmsecs(descrambler_section_t* ds) {
  descrambler_ecmsec_t* des;
  while ((des = LIST_FIRST(&ds->ecmsecs)) != NULL)
    descrambler_destroy_ecmsec(des);
}

static void descrambler_destroy_section(descrambler_section_t* ds, int emm) {
  ds->callback(ds->opaque, -1, NULL, 0, emm);
  descrambler_destroy_all_ecmsecs(ds);
  free(ds);
}

static void descrambler_destroy_table_(descrambler_table_t* dt) {
  mpegts_table_destroy(dt->table);
  free(dt);
}

static void descrambler_destroy_table(descrambler_table_t* dt, int emm) {
 ...
 *
 */
static void descrambler_load_hints(htsmsg_t* m) {
  ...
  HTSMSG_FOREACH(f, m) {
    if (!(e = htsmsg_field_get_map(f)))
      continue;
    if ((s = htsmsg_get_str(e, "caid")) == NULL)
      continue;
    ...
    hint.dh_ecmparity   = str2val_def(htsmsg_get_str(e, "ecmparity"),
        ecmparitytab,
        ECM_PARITY_DEFAULT);
    tvhinfo(LS_DESCRAMBLER,
        "adding CAID %04X/%04X as%s%s%s interval %ums pc %d ep %s (%s)",
        hint.dh_caid,
        hint.dh_mask,
 ...
 */
static int descrambler_quick_ecm(mpegts_service_t* t, int pid) {
 ...
 */
void descrambler_service_start(service_t* t) {
        ...
        TAILQ_FOREACH (hint, &ca_hints, dh_link) {
          if (hint->dh_caid == (ca->caid & hint->dh_mask)) {
            ...
            if (hint->dh_multipid)
              multipid = 1;
            if (hint->dh_interval)
              interval = hint->dh_interval;
            if (hint->dh_paritycheck)
              paritycheck = hint->dh_paritycheck;
...
void descrambler_service_stop(service_t* t) {
  th_descrambler_t*         td;
  th_descrambler_runtime_t* dr;
  th_descrambler_key_t*     tk;
  th_descrambler_data_t*    dd;
...
void descrambler_caid_changed(service_t* t) {
  th_descrambler_t* td;
...
static void descrambler_notify_deliver(mpegts_service_t* t,
...
static void descrambler_notify_nokey(th_descrambler_runtime_t* dr) {
...
void descrambler_notify(th_descrambler_t* td,
    uint16_t                              caid,
    uint32_t                              provid,
...
int descrambler_resolved(service_t* t, th_descrambler_t* ignore) {
  th_descrambler_t* td;
...
int descrambler_multi_pid(th_descrambler_t* td) {
...
void descrambler_change_keystate(th_descrambler_t* td,
    th_descrambler_keystate_t                      keystate,
    int                                            lock) {
  service_t*                t = td->td_service;
  th_descrambler_runtime_t* dr;
  ...
  tvhtrace(LS_DESCRAMBLER,
      "%s: key state changed from %s to %s for \"%s\"",
      td->td_nicename,
  ...
  tvhtrace(LS_DESCRAMBLER,
      "service \"%s\": %d descramblers (%d ok %d failed %d fatal)",
      t->s_nicename,
      count,
      resolved,
      failed,
...
void descrambler_keys(th_descrambler_t* td,
    int                                 type,
    uint16_t                            pid,
    const uint8_t*                      even,
    const uint8_t*                      odd) {
  ...
  if (tvhcsa_set_type(&tk->key_csa, (mpegts_service_t*)t, type) < 0) {
    if (tk->key_type_overwritten)
      goto end;
    if (type == DESCRAMBLER_CSA_CBC &&
        tk->key_csa.csa_type == DESCRAMBLER_DES_NCB) {
      tvhwarn(LS_DESCRAMBLER,
          "Keep key%s type %s (requested %s) for service \"%s\", check your "
          "caclient",
          pidname,
          descrambler_keytype2str(tk->key_csa.csa_type),
          ktype,
...
cont:
  ...
  if (even && memcmp(empty, even, tk->key_csa.csa_keylen)) {
    ...
    if (dr->dr_ecm_start[0] < dr->dr_ecm_start[1]) {
      ...
      tvhdebug(LS_DESCRAMBLER,
          "Both keys received, marking ECM start for even key%s for service "
          "\"%s\"",
          pidname,
          ((mpegts_service_t*)t)->s_dvb_svcname);
    }
  ...
  if (odd && memcmp(empty, odd, tk->key_csa.csa_keylen)) {
    ...
    if (dr->dr_ecm_start[1] < dr->dr_ecm_start[0]) {
      ...
      tvhdebug(LS_DESCRAMBLER,
          "Both keys received, marking ECM start for odd key%s for service "
          "\"%s\"",
          pidname,
          ((mpegts_service_t*)t)->s_dvb_svcname);
    }
  ...
  if (changed) {
    ...
    if (tk->key_csa.csa_keylen == 8) {
      ...
      tvhtrace(LS_DESCRAMBLER,
          "Unknown keys%s pid %04X from %s for for service \"%s\"",
          pidname,
          pid,
          td->td_nicename,
          ((mpegts_service_t*)t)->s_dvb_svcname);
...
void descrambler_flush_table_data(service_t* t) {
  ...
  if (mux == NULL)
    return;
  tvhtrace(LS_DESCRAMBLER,
      "flush table data for service \"%s\"",
      ms->s_dvb_svcname);
...
static inline void key_update(service_t* t,
    th_descrambler_key_t*                tk,
    uint8_t                              key,
    int64_t                              timestamp) {
  ...
  if (tk->key_start) {
    ...
    if (tk->key_initial_interval > 2000000) {
      tk->key_interval = tk->key_start + sec2mono(50) < timestamp
          ? tk->key_initial_interval
          : MAX(5000000, timestamp - tk->key_start);
      tvhtrace(LS_DESCRAMBLER,
          "update key[%d] interval for \"%s\" to %ldms",
          tk->key_pid,
          t->s_nicename,
          (long)(tk->key_interval / 1000));
    }
...
static inline int key_changed(th_descrambler_runtime_t* dr,
    th_descrambler_key_t*                               tk,
    uint8_t                                             ki,
    int64_t                                             timestamp) {
...
static inline int key_valid(th_descrambler_key_t* tk, uint8_t ki) {
  /* 0x40 (for even) or 0x80 (for odd) */
  uint8_t mask = ((ki & 0x40) + 0x40);
  return tk && (tk->key_valid & mask);
}

static inline int key_late(th_descrambler_runtime_t* dr,
    th_descrambler_key_t*                            tk,
    uint8_t                                          ki,
    int64_t                                          timestamp) {
  ...
  if (dr->dr_ecm_last_key_time + dr->dr_ecm_key_margin < tk->key_start &&
      (!dr->dr_quick_ecm ||
          dr->dr_ecm_start[kidx] + ms2mono(10) < tk->key_start)) {
  late:
...
static inline int key_started(th_descrambler_runtime_t* dr, uint8_t ki) {
  uint8_t kidx = (ki & 0x40) >> 6;
  return mclk() - dr->dr_ecm_start[kidx] < dr->dr_ecm_key_margin * 2;
}

static void old_key_flush(th_descrambler_runtime_t* dr, service_t* t) {
...
static void key_flush(th_descrambler_runtime_t* dr,
    th_descrambler_key_t*                       tk,
    uint8_t                                     changed,
    service_t*                                  t) {
  if (!changed)
...
static th_descrambler_key_t* key_find_struct(th_descrambler_runtime_t* dr,
    const uint8_t*                                                     tsb,
    service_t*                                                         t) {
  th_descrambler_key_t* tk;
...
static int ecm_reset(service_t* t, th_descrambler_runtime_t* dr) {
  th_descrambler_t*     td;
  th_descrambler_key_t* tk;
  int                   ret = 0, i;
...
int descrambler_pass(service_t* t,
    elementary_stream_t*        st,
    const uint8_t*              tsb,
    int                         len) {
  if ((tsb[3] & 0x80) == 0)
...
int descrambler_descramble(service_t* t,
    elementary_stream_t*              st,
    const uint8_t*                    tsb,
    int                               len) {
  ...
  if (dr->dr_ca_resolved > 0) {
    ...
    for (dd = TAILQ_FIRST(&dr->dr_queue); dd; dd = dd_next) {
      ...
      if ((tsb2[3] & 0x80) == 0) {
        ...
        if (tk) {
          debug2("%p: descramble1 %d, %s[%d]",
              dr,
              len2,
              keystr(tsb2),
              extractpid(tsb2));
          tk->key_csa.csa_descramble(&tk->key_csa,
              (mpegts_service_t*)t,
              tsb2,
              len2);
          dr->dr_key_last = tk;
        } else {
          debug2("%p: direct1 %d, %s[%d]",
              dr,
              len2,
              keystr(tsb2),
              extractpid(tsb2));
      ...
      if (dr->dr_key_multipid) {
        ...
        if (tk == NULL) {
          if (t->s_start_time + 3000000 < mclk() &&
            ...
            tvhwarn(LS_DESCRAMBLER,
                "%s stream key[%d] is not available",
                ((mpegts_service_t*)t)->s_dvb_svcname,
                extractpid(tsb2));
          goto next;
        }
...
#ifdef DEBUG2
      {
        ...
        debug2("%p: timestamp %ld thres %ld now %ld (interval %ldms) %s[%d]",
            dr,
            t1,
            now - t2,
            (now - t1) / 1000,
        ...
        if ((ki & 0x80) != 0x00) {
          ...
          if (key_changed(dr, tk, ki, dd->dd_timestamp)) {
            ...
            if (r == 0) {
              ...
              goto doit;
            tvhtrace(LS_DESCRAMBLER,
                "stream key[%d] changed to %s for service \"%s\"",
                tk->key_pid,
                (ki & 0x40) ? "odd" : "even",
                ((mpegts_service_t*)t)->s_dvb_svcname);
    ...
    if ((ki & 0x80) != 0x00) {
      ...
      if (tk->key_start == 0) {
        ...
        while (dr->dr_queue_total > dr->dr_initial_paritycheck) {
          if (descrambler_data_key_check(dr,
                  ki & 0xc0,
                  dr->dr_initial_paritycheck) == 0) {
            tvhtrace(LS_DESCRAMBLER,
                "initial stream key[%d] set to %s for service \"%s\"",
                tk->key_pid,
                (ki & 0x40) ? "odd" : "even",
                ((mpegts_service_t*)t)->s_dvb_svcname);
    ...
    if (dr->dr_ca_count != dr->dr_ca_failed) {
      ...
      if (dr->dr_queue_total >= dbuflen * 188) {
        ...
        if (dr->dr_last_err + sec2mono(10) < mclk()) {
          dr->dr_last_err = mclk();
          tvherror(LS_DESCRAMBLER,
              "cannot decode packets for service \"%s\"",
              ((mpegts_service_t*)t)->s_dvb_svcname);
        } else {
          tvhtrace(LS_DESCRAMBLER,
              "cannot decode packets for service \"%s\"",
...
static int descrambler_table_callback(mpegts_table_t* mt,
    const uint8_t*                                    ptr,
    int                                               len,
    int                                               tableid) {
  ...
  LIST_FOREACH (des, &sections, active_link) {
    if (des->changed == 2) {
      ...
      if (!emm) { /* ECM */
        if ((t = mt->mt_service) != NULL) {
          ...
          if (dr) {
            if (!dr->dr_quick_ecm && !des->quick_ecm_called) {
              ...
              dr->dr_quick_ecm =
                  descrambler_quick_ecm(mt->mt_service, mt->mt_pid);
              if (dr->dr_quick_ecm)
                tvhdebug(LS_DESCRAMBLER,
                    "quick ECM enabled for service '%s'",
                    t->s_dvb_svcname);
            }
            if ((ptr[0] & 0xfe) == 0x80) { /* 0x80 = even, 0x81 = odd */
              ...
              for (i = 0; i < DESCRAMBLER_MAX_KEYS; i++) {
                ...
                if (dr->dr_quick_ecm)
                  tk->key_valid &= ~ki;
                TAILQ_FOREACH (st,
                    &mt->mt_service->s_components.set_filter,
                    es_filter_link) {
                  if (st->es_pid != mt->mt_pid)
                    continue;
                  LIST_FOREACH (ca, &st->es_caids, link) {
                    if (ca->use == 0)
                      continue;
                    tk->key_csa.csa_ecm =
                        (caid_is_videoguard(ca->caid) &&
                            (ptr[4] != 0 && (ptr[2] - ptr[4]) == 4))
                        ? 4
                        : 0;
                    tvhtrace(LS_DESCRAMBLER,
                        "key ecm=%X (caid=%04X)",
            ...
            tvhtrace(LS_DESCRAMBLER,
                "ECM message %02x:%02x (section %d, len %d, pid %d) for "
                "service \"%s\"",
                ptr[0],
                ptr[1],
                des->number,
          ...
          tvhtrace(LS_DESCRAMBLER,
              "Unknown fast table message %02x (section %d, len %d, pid %d)",
              ptr[0],
              des->number,
              len,
              mt->mt_pid);
        ...
        const char* s;
        if (mt->mt_pid == DVB_PAT_PID)
          s = "PAT";
        else if (mt->mt_pid == DVB_CAT_PID)
          s = "CAT";
        else
          s = "EMM";
        if (len >= 18)
          tvhtrace(LS_DESCRAMBLER_EMM,
              "%s message "
              "%02x:{%02x:%02x}:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%"
              "02x:%02x:%02x:%02x:%02x:%02x (len %d, pid %d)",
              s,
              ptr[0],
              ptr[1],
              ptr[2],
          ...
          tvhtrace(LS_DESCRAMBLER_EMM,
              "%s message %02x:{%02x:%02x}:%02x:%02x:%02x (len %d, pid %d)",
              s,
              ptr[0],
              ptr[1],
              ptr[2],
          ...
          tvhtrace(LS_DESCRAMBLER_EMM,
              "%s message %02x:{%02x:%02x}:%02x (len %d, pid %d)",
              s,
              ptr[0],
              ptr[1],
              ptr[2],
          ...
          for (i = 0; i < DESCRAMBLER_MAX_KEYS; i++) {
            ...
            for (j = 0; j < 2; j++) {
              ...
              if (clk3 > 0 && clk3 >= clk2 && clk3 + ms2mono(200) <= clk) {
                ...
                tvhtrace(LS_DESCRAMBLER,
                    "ECM: %s key[%d] for service \"%s\" still valid",
                    j == 0 ? "Even" : "Odd",
                    tk->key_pid,
                    t->s_dvb_svcname);
              }
...
static int descrambler_open_pid_(mpegts_mux_t* mux,
    void*                                      opaque,
    int                                        pid,
  ...
  if (!dt) {
    ...
    dt->table = mpegts_table_add(mux,
        0,
        0,
        descrambler_table_callback,
        dt,
        (flags & MT_FAST) ? "ecm" : "emm",
        LS_TBL_CSA,
        MT_FULL | MT_DEFER | flags,
        pid,
        MPS_WEIGHT_CA);
    if (dt->table)
  ...
  tvhtrace(LS_DESCRAMBLER,
      "mux %p open pid %04X (%i) (flags 0x%04x) for %p",
      mux,
      pid,
      pid,
      flags,
...
int descrambler_open_pid(mpegts_mux_t* mux,
    void*                              opaque,
    int                                pid,
    descrambler_section_callback_t     callback,
    service_t*                         service) {
...
static int descrambler_close_pid_(mpegts_mux_t* mux, void* opaque, int pid) {
  descrambler_table_t*   dt;
  descrambler_section_t* ds;
  int                    flags;
  ...
  TAILQ_FOREACH (dt, &mux->mm_descrambler_tables, link) {
    ...
    TAILQ_FOREACH (ds, &dt->sections, link) {
      if (ds->opaque == opaque) {
        ...
        tvhtrace(LS_DESCRAMBLER,
            "mux %p close pid %04X (%i) (flags 0x%04x) for %p",
            mux,
            pid,
            pid,
            flags,
...
int descrambler_close_pid(mpegts_mux_t* mux, void* opaque, int pid) {
  int res;
...
void descrambler_flush_tables(mpegts_mux_t* mux) {
  descrambler_table_t* dt;
...
static void
descrambler_cat_entry(void* _mux, uint16_t caid, uint32_t prov, uint16_t pid) {
  ...
  TAILQ_FOREACH (emm, &mux->mm_descrambler_emms, link)
    if (emm->caid == caid && emm->prov == prov) {
      ...
      if (emm->pid == EMM_PID_UNKNOWN) {
        tvhtrace(LS_DESCRAMBLER,
            "attach emm caid %04X (%i) prov %06X (%i) pid %04X (%i)",
            caid,
            caid,
            prov,
            prov,
  ...
  TAILQ_FOREACH (emm, &mux->mm_descrambler_emms, link)
    if (emm->to_be_removed) {
      if (emm->pid != EMM_PID_UNKNOWN) {
        ...
        pid  = emm->pid;
        tvhtrace(LS_DESCRAMBLER,
            "close emm caid %04X (%i) prov %06X (%i) pid %04X (%i)",
            caid,
            caid,
            prov,
            prov,
...
void descrambler_cat_data(mpegts_mux_t* mux, const uint8_t* data, int len) {
  descrambler_emm_t* emm;
...
int descrambler_open_emm(mpegts_mux_t* mux,
    void*                              opaque,
    int                                caid,
    int                                prov,
    descrambler_section_callback_t     callback) {
...
int descrambler_close_emm(mpegts_mux_t* mux, void* opaque, int caid, int prov) {
  descrambler_emm_t* emm;
  int                pid;
  ...
  if (pid != EMM_PID_UNKNOWN) {
    ...
    prov = emm->prov;
    tvhtrace(LS_DESCRAMBLER,
        "close emm caid %04X (%i) prov %06X (%i) pid %04X (%i) - direct",
        caid,
        caid,
        prov,
        prov,
