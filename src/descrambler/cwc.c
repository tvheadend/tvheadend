...
static int cwc_decode_card_data_reply(cwc_t* cwc, uint8_t* msg, int len) {
  ...
  if (plen < 14) {
    tvhinfo(cwc->cc_subsys,
        "%s: Invalid card data reply (message)",
        cwc->cc_name);
    return -1;
  }
  ...
  if (plen < nprov * 11) {
    tvhinfo(cwc->cc_subsys,
        "%s: Invalid card data reply (provider list)",
        cwc->cc_name);
    return -1;
  }
 ...
 *
 */
static void handle_ecm_reply(cc_service_t* ct,
    cc_ecm_section_t*                      es,
    uint8_t*                               msg,
    int                                    len,
    int                                    seq) {
  ...
  if (len < 19) {
    ...
    if (len <= 22) {
      ...
      tvherror(cwc->cc_subsys,
          "%s: wrong ECM reply length %d",
          cwc->cc_name,
          len);
      return;
    }
        ...
        if (caid_is_betacrypt(caid) || caid_is_irdeto(caid)) {
          ...
          if (provid != 0) {
            tvhdebug(cwc->cc_subsys,
                "%s: Bad provider for %s-card [CAID:%04X Provider:0x%06X], "
                "using zero",
                cwc->cc_name,
                n,
                caid,
 ...
 *
 */
static int cwc_read_message0(cwc_t* cwc,
    const char*                     state,
    sbuf_t*                         rbuf,
    int*                            rsize,
    int                             timeout) {
 ...
 *
 */
static int cwc_read_message(cwc_t* cwc,
    const char*                    state,
    uint8_t*                       buf,
    int                            len,
    int                            timeout) {
...
static int cwc_init_session(void* cc) {
  ...
  if ((r = cwc_read_message(cwc, "Request card data", buf, sizeof(buf), 5000)) <
      0)
    return -1;
