 ...
 *
 */
static void des_key_spread(uint8_t* normal, uint8_t* spread) {
 ...
 *
 */
static int des_encrypt(uint8_t* buffer, int len, cwc_t* cwc) {
 ...
 *
 */
static int des_decrypt(uint8_t* buffer, int len, cwc_t* cwc) {
 ...
 *
 */
static void des_make_login_key(cwc_t* cwc, uint8_t* k) {
 ...
 *
 */
static void des_make_session_key(cwc_t* cwc) {
 ...
 * the ID)
 */
static int cwc_send_msg(void* cc,
    const uint8_t*            msg,
    size_t                    len,
    int                       sid,
    int                       enq,
    uint16_t                  st_caid,
    uint32_t                  st_provider,
    uint16_t*                 _seq) {
  cwc_t*        cwc = cc;
  cc_message_t* cm;
  uint8_t*      buf;
 ...
 * Card data command
 */
static void cwc_send_data_req(cwc_t* cwc) {
  uint8_t buf[3];
 ...
 * Send keep alive
 */
static void cwc_send_ka(void* cc) {
  cwc_t*  cwc = cc;
  uint8_t buf[3];
 ...
 */
static int cwc_decode_card_data_reply(cwc_t* cwc, uint8_t* msg, int len) {
  int          plen, i;
  unsigned int nprov;
  ...
  if (plen < nprov * 11) {
    tvhinfo(cwc->cc_subsys, "%s: Invalid card data reply (provider list)", cwc->cc_name);
    return -1;
  }
  ...
  cc_new_card((cclient_t*)cwc, (msg[4] << 8) | msg[5], 0, msg + 6, nprov, pid, psa, 0);
 ...
 * Login command
 */
static int cwc_send_login(cwc_t* cwc) {
  uint8_t buf[CWS_NETMSGSIZE];
  size_t  ul, pl;

  if (cwc->cc_username == NULL || cwc->cwc_password_salted == NULL)
    return 1;
 ...
 */
static void
handle_ecm_reply(cc_service_t* ct, cc_ecm_section_t* es, uint8_t* msg, int len, int seq) {
  ...
  if (len < 19) {
    ...
    if (len <= 22) {
      ...
      tvherror(cwc->cc_subsys, "%s: wrong ECM reply length %d", cwc->cc_name, len);
      return;
    }
 ...
 * cwc_mutex is held
 */
static int cwc_running_reply(cwc_t* cwc, uint8_t msgtype, uint8_t* msg, int len) {
  cc_service_t*     ct;
  cc_ecm_section_t* es;
  uint16_t          seq;
  int               plen;
  ...
  switch (msgtype) {
    ...
    case 0x81:
      if (len < 12) {
        tvherror(cwc->cc_subsys, "%s: wrong 0x%02X length %d", cwc->cc_name, msgtype, len);
        return -1;
      }
      seq = (msg[2] << 8) | msg[3];
      len -= 12;
        ...
        if (caid_is_betacrypt(caid) || caid_is_irdeto(caid)) {
          ...
          if (provid != 0) {
            tvhdebug(cwc->cc_subsys,
                "%s: Bad provider for %s-card [CAID:%04X Provider:0x%06X], using zero",
                cwc->cc_name,
                n,
                caid,
                provid);
 ...
 *
 */
static int cwc_read_message0(cwc_t* cwc, const char* state, sbuf_t* rbuf, int* rsize, int timeout) {
  int msglen;
  ...
  *rsize = msglen + 2;
  if ((msglen = des_decrypt(rbuf->sb_data, msglen + 2, cwc)) < 15) {
    tvhinfo(cwc->cc_subsys, "%s: %s: Decrypt failed", cwc->cc_name, state);
    return -1;
  }
 ...
 *
 */
static int cwc_read_message(cwc_t* cwc, const char* state, uint8_t* buf, int len, int timeout) {
  int msglen, r;

  if ((r = cc_read((cclient_t*)cwc, buf, 2, timeout))) {
    if (tvheadend_is_running())
      tvhinfo(cwc->cc_subsys, "%s: %s: Read error (header): %s", cwc->cc_name, state, strerror(r));
    return -1;
  }
  ...
  if (msglen > len) {
    if (tvheadend_is_running())
      tvhinfo(cwc->cc_subsys, "%s: %s: Invalid message size: %d", cwc->cc_name, state, msglen);
    return -1;
  }
  ...
  if ((r = cc_read((cclient_t*)cwc, buf + 2, msglen, 1000))) {
    if (tvheadend_is_running())
      tvhinfo(cwc->cc_subsys, "%s: %s: Read error: %s", cwc->cc_name, state, strerror(r));
    return -1;
  }
 ...
 *
 */
static int cwc_init_session(void* cc) {
  cwc_t*  cwc = cc;
  uint8_t buf[CWS_NETMSGSIZE];
  int     r;
  ...
  if ((r = cc_read((cclient_t*)cwc, buf, 14, 5000))) {
    tvhinfo(cwc->cc_subsys, "%s: No login key received: %s", cwc->cc_name, strerror(r));
    return -1;
  }
 ...
 *
 */
static int cwc_read(void* cc, sbuf_t* rbuf) {
  cwc_t*    cwc         = cc;
 ...
 *
 */
static int cwc_send_ecm(void* cc,
    cc_service_t*             ct,
    cc_ecm_section_t*         es,
 ...
 *
 */
static void cwc_send_emm(void* cc,
    cc_service_t*              ct,
    cc_card_data_t*            pcard,
    uint32_t                   provid,
    const uint8_t*             data,
 ...
 *
 */
static void cwc_free(caclient_t* cac) {
  cwc_t* cwc    = (cwc_t*)cac;
 ...
 *
 */
static void cwc_conf_changed(caclient_t* cac) {
  cwc_t* cwc = (cwc_t*)cac;

  free(cwc->cwc_password_salted);
  cwc->cwc_password_salted = cwc->cc_password ? crypt_md5(cwc->cc_password, "$1$abcdefgh$") : NULL;
 ...
 *
 */
static int caclient_cwc_class_deskey_set(void* o, const void* v) {
  cwc_t*      cwc = o;
  const char* s   = v ?: "";
  uint8_t     key[14];
  int         i, u, l;
...
static const void* caclient_cwc_class_deskey_get(void* o) {
  cwc_t* cwc = o;
  snprintf(prop_sbuf,
      PROP_SBUF_LEN,
...
const idclass_t caclient_cwc_class = {.ic_super = &caclient_cc_class,
