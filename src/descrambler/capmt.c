...
#define CAPMT_LIST_MORE \
  0x00 // append a 'MORE' CAPMT object the list and start receiving the next object
#define CAPMT_LIST_FIRST \
  0x01 // clear the list when a 'FIRST' CAPMT object is received, and start receiving the next
       // object
#define CAPMT_LIST_LAST \
  0x02 // append a 'LAST' CAPMT object to the list and start working with the list
#define CAPMT_LIST_ONLY \
  0x03 // clear the list when an 'ONLY' CAPMT object is received, and start working with the object
#define CAPMT_LIST_ADD \
  0x04 // append an 'ADD' CAPMT object to the current list and start working with the updated list
#define CAPMT_LIST_UPDATE \
  0x05 // replace an entry in the list with an 'UPDATE' CAPMT object, and start working with the
       // updated list
...
static void capmt_table_input(void* opaque, int pid, const uint8_t* data, int len, int emm);
static void capmt_send_client_info(capmt_t* capmt);

/**
 *
 */
static inline const char* capmt_name(capmt_t* capmt) {
  return capmt->capmt_name;
}

static inline int capmt_oscam_so_wrapper(capmt_t* capmt) {
...
static inline int capmt_oscam_new(capmt_t* capmt) {
...
static inline int capmt_oscam_network(capmt_t* capmt) {
...
static inline int capmt_oscam_socket(capmt_t* capmt) {
...
static inline int capmt_oscam_netproto(capmt_t* capmt) {
  int oscam = capmt->capmt_oscam;
  return oscam == CAPMT_OSCAM_NET_PROTO || oscam == CAPMT_OSCAM_UNIX_SOCKET_NP;
}

static inline int capmt_include_elementary_stream(streaming_component_type_t type) {
  return SCT_ISAV(type) || type == SCT_DVBSUB || type == SCT_TELETEXT;
}

static int capmt_poll_add(capmt_t* capmt, int fd, void* ptr) {
  if (capmt->capmt_poll == NULL)
    return 0;
  return tvhpoll_add1(capmt->capmt_poll, fd, TVHPOLL_IN, ptr);
}

static int capmt_poll_rem(capmt_t* capmt, int fd) {
  if (capmt->capmt_poll == NULL)
    return 0;
  return tvhpoll_rem1(capmt->capmt_poll, fd);
}

static void capmt_pid_add(capmt_t* capmt, int adapter, int pid, mpegts_service_t* s) {
  ...
  for (i = 0; i < MAX_PIDS; i++) {
    ...
    if (t->pid == pid && t->ecm == ecm) {
      t->pid_refs++;
      tvhtrace(LS_CAPMT,
          "%s: adding pid %d adapter %d - reusing (%d)",
          capmt_name(capmt),
          pid,
          adapter,
          t->pid_refs);
  ...
  if (o) {
    ...
    tvhtrace(LS_CAPMT,
        "%s: adding pid %d adapter %d, tuner %p, mmi %p, mux %p",
        capmt_name(capmt),
        pid,
        adapter,
        ca->ca_tuner,
...
static void capmt_pid_remove(capmt_t* capmt, int adapter, int pid, uint32_t flags) {
  capmt_adapter_t*       ca = &capmt->capmt_adapters[adapter];
  capmt_opaque_t*        o;
  mpegts_mux_instance_t* mmi;
  mpegts_mux_t*          mux;
...
static void capmt_pid_flush(capmt_t* capmt) {
  int adapter;
 ...
 *
 */
static int capmt_connect(capmt_t* capmt, int i) {
  int fd;
  ...
  if (capmt_oscam_network(capmt)) {
    ...
    fd = tcp_connect(capmt->capmt_sockfile, capmt->capmt_port, NULL, errbuf, sizeof(errbuf), 2);
...
#endif
  {
    ...
    if (fd < 0 || connect(fd, (const struct sockaddr*)&serv_addr_un, sizeof(serv_addr_un)) != 0) {
 ...
 *
 */
static void capmt_socket_close(capmt_t* capmt, int sock_idx) {
...
static void capmt_socket_close_lock(capmt_t* capmt, int sock_idx) {
  tvh_mutex_lock(&capmt->capmt_mutex);
 ...
 *
 */
static int capmt_write_msg(capmt_t* capmt, int adapter, int sid, const uint8_t* buf, size_t len) {
  int     i = 0, found = 0, fd;
  ssize_t res;
  ...
  if (!capmt_oscam_so_wrapper(capmt)) {
    ...
    if (capmt->capmt_oscam == CAPMT_OSCAM_OLD) {
      ...
      for (i = 0; i < MAX_SOCKETS; i++)
        tvhdebug(LS_CAPMT,
            "%s: %s: SOCKETS TABLE DUMP [%d]: sid=%d socket=%d",
            capmt_name(capmt),
            __FUNCTION__,
            i,
            capmt->sids[i],
      ...
      if (sid == 0) {
        tvhdebug(LS_CAPMT,
            "%s: %s: got empty SID - returning from function",
            capmt_name(capmt),
            __FUNCTION__);
        return -1;
      }
      ...
      if (found)
        tvhdebug(LS_CAPMT,
            "%s: %s: found sid, reusing socket, i=%d",
            capmt_name(capmt),
            __FUNCTION__,
            i);
    ...
    if (capmt->capmt_sock[i] < 0) {
      ...
      caclient_set_status((caclient_t*)capmt,
          fd >= 0 ? CACLIENT_STATUS_CONNECTED : CACLIENT_STATUS_READY);
      if (fd >= 0)
...
#if ENABLE_ANDROID
    tvhdebug(LS_CAPMT,
        "%s: Message send failed to socket %i (%li)",
        capmt_name(capmt),
        fd,
        (long int)res); // Android bug, ssize_t is long int
#else
 ...
 */
static void
capmt_queue_msg(capmt_t* capmt, int adapter, int sid, const uint8_t* buf, size_t len, int flags) {
  ...
  if (flags & CAPMT_MSG_CLEAR) {
    for (msg = TAILQ_FIRST(&capmt->capmt_writeq); msg; msg = msg2) {
      ...
      if ((adapter == 0xff || msg->cm_adapter == adapter) && (sid == 0 || msg->cm_sid == sid)) {
  ...
  if (flags & CAPMT_MSG_NODUP) {
    TAILQ_FOREACH (msg, &capmt->capmt_writeq, cm_link)
      if (msg->cm_sb.sb_ptr == len && memcmp(msg->cm_sb.sb_data, buf, len) == 0)
        return;
  }
 ...
 *
 */
static void capmt_flush_queue(capmt_t* capmt, int del_only) {
  capmt_message_t* msg;
  ...
  while (1) {
    ...
    if (!del_only)
      capmt_write_msg(capmt, msg->cm_adapter, msg->cm_sid, msg->cm_sb.sb_data, msg->cm_sb.sb_ptr);
    sbuf_free(&msg->cm_sb);
    free(msg);
  }
}
 ...
 *
 */
static void capmt_send_stop(capmt_service_t* t) {
  ...
  if (capmt->capmt_oscam == CAPMT_OSCAM_OLD) {
    ...
    tvhdebug(LS_CAPMT,
        "%s: %s: closing socket i=%d, socket_fd=%d",
        capmt_name(capmt),
        __FUNCTION__,
        i,
        capmt->capmt_sock[i]);
  ...
  if (capmt->capmt_oscam == CAPMT_OSCAM_SO_WRAPPER) { // standard old capmt mode
    ...
    capmt_queue_msg(capmt, t->ct_adapter, service_id16(s), buf, pos, CAPMT_MSG_CLEAR);
  }
#endif
}
 ...
 *
 */
static void capmt_send_stop_descrambling(capmt_t* capmt, uint8_t demuxer) {
  uint8_t buf[8] = {
      0x9F,
      0x80,
      0x3F,
 ...
 *
 */
static void capmt_init_demuxes(capmt_t* capmt) {
  int i, j;
 ...
 * global_lock is held
 */
static void capmt_service_destroy(th_descrambler_t* td) {
...
static void capmt_filter_data(capmt_t* capmt,
    uint8_t                            adapter,
    uint8_t                            demux_index,
...
static void capmt_set_filter(capmt_t* capmt, int adapter, sbuf_t* sb, int offset) {
  ...
  tvhtrace(LS_CAPMT,
      "%s: setting filter: adapter=%d, demux=%d, filter=%d, pid=%d",
      capmt_name(capmt),
      adapter,
      demux_index,
      filter_index,
      pid);
  if (adapter >= MAX_CA || demux_index >= MAX_INDEX || filter_index >= MAX_FILTER || pid > 8191)
    return;
  tvh_mutex_lock(&capmt->capmt_mutex);
...
service_found:
  if (t) {
    ...
    f0 = pf->filter[0];
    m0 = pf->filter[1];
    if ((f0 & 0xf0) == 0x80 && (m0 & 0xf0) == 0xf0)
      goto cont;
    if (caid_is_dvn(caid) && f0 == 0x50 && m0 == 0xff)
      goto cont; /* DVN */
  }
cont:
  if (t)
...
static void capmt_stop_filter(capmt_t* capmt, int adapter, sbuf_t* sb, int offset) {
  uint8_t          demux_index  = sbuf_peek_u8(sb, offset + 0);
  uint8_t          filter_index = sbuf_peek_u8(sb, offset + 1);
  int16_t          pid;
  uint32_t         flags;
  ...
  tvhtrace(LS_CAPMT,
      "%s: stopping filter: adapter=%d, demux=%d, filter=%d, pid=%d",
      capmt_name(capmt),
      adapter,
      demux_index,
      filter_index,
      pid);
  if (adapter >= MAX_CA || demux_index >= MAX_INDEX || filter_index >= MAX_FILTER)
    return;
  tvh_mutex_lock(&capmt->capmt_mutex);
  cf     = &capmt->capmt_demuxes.filters[demux_index];
  filter = &cf->dmx[filter_index];
...
#if CONFIG_LINUXDVB
#ifdef CAPMT_OSCAM_SO_WRAPPER
static void capmt_abort(capmt_t* capmt, int keystate) {
  mpegts_service_t* t;
  capmt_service_t*  ct;
  ...
  LIST_FOREACH (ct, &capmt->capmt_services, ct_link) {
    ...
    if (ct->td_keystate != keystate) {
      tvherror(LS_CAPMT,
          ...
          capmt_name(capmt),
          t->s_dvb_svcname,
          keystate == DS_FORBIDDEN ? "access denied" : "connection close");
...
static int capmt_ecm_reset(th_descrambler_t* th) {
  descrambler_change_keystate(th, DS_READY, 1);
  return 0;
}

static void capmt_process_key(capmt_t* capmt,
    uint8_t                            adapter,
    ca_info_t*                         cai,
    int                                type,
    const uint8_t*                     even,
...
static void capmt_send_key(capmt_t* capmt) {
...
static void capmt_process_notify(capmt_t* capmt,
    uint8_t                               adapter,
    uint16_t                              sid,
    uint16_t                              caid,
    uint32_t                              provid,
    const char*                           cardsystem,
    uint16_t                              pid,
    uint32_t                              ecmtime,
    uint16_t                              hops,
    const char*                           reader,
...
static int capmt_msg_size(capmt_t* capmt, sbuf_t* sb, int offset) {
  uint32_t cmd;
  ...
  if (capmt_oscam_netproto(capmt)) {
    ...
    if (!sb->sb_bswap && !sb->sb_err) {
      if (cmd == CA_SET_PID_X || cmd == CA_SET_DESCR_X || cmd == CA_SET_DESCR_AES_X ||
          cmd == DMX_SET_FILTER_X || cmd == DMX_STOP_X) {
        sb->sb_bswap = 1;
        cmd          = sbuf_peek_u32(sb, offset);
      }
    }
...
static char* capmt_peek_str(sbuf_t* sb, int* offset) {
...
static void capmt_analyze_cmd(capmt_t* capmt, uint32_t cmd, int adapter, sbuf_t* sb, int offset) {
  if (cmd == CA_SET_PID_) {
    ...
    tvhdebug(LS_CAPMT,
        "%s: CA_SET_PID adapter %d index %d pid %d (0x%04x)",
        capmt_name(capmt),
        adapter,
        index,
        pid,
    ...
    if (adapter < MAX_CA && index >= 0 && index < MAX_INDEX) {
      ...
      tvherror(LS_CAPMT,
          "%s: Invalid index %d in CA_SET_PID (%d) for adapter %d",
          capmt_name(capmt),
          index,
          MAX_INDEX,
          adapter);
    ...
    tvhdebug(LS_CAPMT,
        "%s: CA_SET_DESCR adapter %d par %d idx %d %02x%02x%02x%02x%02x%02x%02x%02x",
        capmt_name(capmt),
        adapter,
        parity,
        index,
    ...
    if (parity > 1) {
      tvherror(LS_CAPMT,
          "%s: Invalid parity %d in CA_SET_DESCR for adapter%d",
          capmt_name(capmt),
          parity,
          adapter);
      return;
    ...
    tvhdebug(LS_CAPMT,
        "%s: CA_SET_DESCR_AES adapter %d par %d idx %d "
    ...
    if (parity == 0) {
      ...
      tvherror(LS_CAPMT,
          "%s: Invalid parity %d in CA_SET_DESCR_AES for adapter%d",
          capmt_name(capmt),
          parity,
          adapter);
    ...
    tvhdebug(LS_CAPMT,
        "%s: CA_SET_DESCR_MODE adapter %d index %d algo %d cipher mode %d",
        capmt_name(capmt),
        adapter,
        index,
        algo,
    ...
    tvhdebug(LS_CAPMT,
        "%s: ECM_INFO: adapter=%d sid=%d caid=%04X(%s) pid=%04X provid=%06X ecmtime=%d hops=%d "
    ...
    tvhinfo(LS_CAPMT,
        "%s: Connected to server '%s' (protocol version %d)",
        capmt_name(capmt),
        info,
        protover);
    if (rev)
...
static void show_connection(capmt_t* capmt, const char* what) {
  if (capmt_oscam_network(capmt)) {
    tvhinfo(LS_CAPMT,
...
#if CONFIG_LINUXDVB
static void handle_ca0(capmt_t* capmt) {
...
static void handle_single(capmt_t* capmt) {
...
#if CONFIG_LINUXDVB
#ifdef CAPMT_OSCAM_SO_WRAPPER
static void handle_ca0_wrapper(capmt_t* capmt) {
  uint8_t    buffer[18];
  uint32_t   index;
  ca_info_t* cai;
  int        ret;
  ...
  while (atomic_get(&capmt->capmt_running)) {
    ...
    if (ret < 0) {
      ...
      tvhtrace(LS_CAPMT,
          "%s: Received message from socket %i",
          capmt_name(capmt),
      ...
      if (index < MAX_INDEX) {
        ...
        capmt_process_key(capmt, 0, cai, DESCRAMBLER_CSA_CBC, buffer + 2, buffer + 10, ret == 18);
      }
    }
  }
...
#if ENABLE_LINUXDVB
static int capmt_create_udp_socket(capmt_t* capmt, int* socket, int port) {
 ...
 *
 */
static void* capmt_thread(void* aux) {
  capmt_t*         capmt = aux;
  capmt_adapter_t* ca;
  capmt_opaque_t*  t;
  int              d, i, j, fatal;
  ...
  while (atomic_get(&capmt->capmt_running)) {
    ...
    if (capmt->capmt_sock[0] >= 0) {
      ...
      if (capmt_oscam_new(capmt)) {
        ...
        if (capmt->capmt_oscam == CAPMT_OSCAM_SO_WRAPPER) {
          bind_ok =
              capmt_create_udp_socket(capmt, &capmt->capmt_adapters[0].ca_sock, capmt->capmt_port);
          if (bind_ok)
...
#else
      if (capmt_oscam_network(capmt) || capmt_oscam_socket(capmt)) {
        handle_single(capmt);
      } else {
        tvherror(LS_CAPMT,
            "%s: Only modes 3,4,5,6 are supported for non-linuxdvb devices",
            capmt_name(capmt));
        fatal = 1;
      }
#endif
...
static void capmt_caid_add(capmt_service_t* ct, mpegts_service_t* t, int pid, caid_t* c) {
  capmt_caid_ecm_t* cce;

  tvhdebug(LS_CAPMT,
      "%s: New caid 0x%04X:0x%06X (pid 0x%04X) for service \"%s\"",
      capmt_name(ct->ct_capmt),
      c->caid,
      c->providerid,
      pid,
      t->s_dvb_svcname);
...
static int capmt_update_elementary_stream(capmt_service_t* ct, int* _i, elementary_stream_t* st) {
  uint8_t type;
  int     i = *_i;
  ...
  *_i = i + 1;
  if (st->es_pid != ct->ct_pids[i] || type != ct->ct_types[i]) {
    ...
    ct->ct_type_sok[i] =
        SCT_ISVIDEO(st->es_type) || SCT_ISAUDIO(st->es_type) || st->es_type == SCT_DVBSUB;
    return 1;
  }
...
static void capmt_caid_change(th_descrambler_t* td) {
  ...
  TAILQ_FOREACH (st, &t->s_components.set_filter, es_filter_link) {
    ...
    if (t->s_dvb_prefcapid_lock == PREFCAPID_FORCE && t->s_dvb_prefcapid != st->es_pid)
      continue;
  ...
  LIST_FOREACH (cce, &ct->ct_caid_ecm, cce_link) {
    if (t->s_dvb_prefcapid_lock == PREFCAPID_FORCE && cce->cce_ecmpid != t->s_dvb_prefcapid) {
      st = NULL;
    } else {
      TAILQ_FOREACH (st, &t->s_components.set_filter, es_filter_link) {
        LIST_FOREACH (c, &st->es_caids, link)
...
static void capmt_ok_timer_cb(void* aux) {
...
static void capmt_send_request(capmt_service_t* ct, int lm) {
  ...
  LIST_FOREACH (cce2, &ct->ct_caid_ecm, cce_link) {
    ...
    if (cce2->cce_providerid) { // we need to add provider ID to the data
      if (cce2->cce_caid >> 8 == 0x01) {
        ...
        tvhwarn(LS_CAPMT,
            "%s: Unknown CAID type, don't know where to put provider ID",
            capmt_name(capmt));
      }
    }
    pos = pos2;
    tvhdebug(LS_CAPMT,
        "%s: adding ECMPID=0x%X (%d), "
  ...
  if (ct->td_keystate != DS_RESOLVED)
    tvhdebug(LS_CAPMT,
        "%s: Trying to obtain key for service \"%s\"",
        capmt_name(capmt),
        t->s_dvb_svcname);
...
static void capmt_enumerate_services(capmt_t* capmt, int force) {
  ...
  if (!all_srv_count) {
    // closing socket (oscam handle this as event and stop decrypting)
    tvhdebug(LS_CAPMT,
        "%s: %s: no subscribed services, closing socket, fd=%d",
        capmt_name(capmt),
        __FUNCTION__,
        capmt->capmt_sock[0]);
    if (capmt->capmt_sock[0] >= 0)
 ...
 * global_lock is held
 */
static void capmt_service_start(caclient_t* cac, service_t* s) {
  ...
  if (capmt_oscam_network(capmt)) {
    snprintf(buf, sizeof(buf), "capmt-%s-%i", capmt->capmt_sockfile, capmt->capmt_port);
  } else {
 ...
 *
 */
static void capmt_free(caclient_t* cac) {
  capmt_t* capmt = (capmt_t*)cac;
  tvhinfo(LS_CAPMT,
      "%s: mode %i %s %s port %i destroyed",
      capmt_name(capmt),
      capmt->capmt_oscam,
...
static void capmt_conf_changed(caclient_t* cac) {
  capmt_t*  capmt = (capmt_t*)cac;
  pthread_t tid;

  idnode_get_title(&capmt->cac_id, NULL, capmt->capmt_name, sizeof(capmt->capmt_name));

  if (val2str(capmt->capmt_oscam, caclient_capmt_oscam_mode_tab) == NULL) {
    tvherror(LS_CAPMT,
        "Unknown mode %d, disabling capmt client %s",
        capmt->capmt_oscam,
        capmt->capmt_name);
    capmt->cac_enabled = 0;
  }
...
static htsmsg_t* caclient_capmt_class_oscam_mode_list(void* o, const char* lang) {
  return strtab2htsmsg(caclient_capmt_oscam_mode_tab, 1, lang);
}

static htsmsg_t* caclient_capmt_class_cwmode_list(void* o, const char* lang) {
...
static htsmsg_t* caclient_capmt_class_pmtmode_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
const idclass_t caclient_capmt_class = {.ic_super = &caclient_class,
