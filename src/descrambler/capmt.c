...
#define CAPMT_LIST_MORE \
  0x00 // append a 'MORE' CAPMT object the list and start receiving the next
       // object
#define CAPMT_LIST_FIRST \
  0x01 // clear the list when a 'FIRST' CAPMT object is received, and start
       // receiving the next object
#define CAPMT_LIST_LAST \
  0x02 // append a 'LAST' CAPMT object to the list and start working with the
       // list
#define CAPMT_LIST_ONLY \
  0x03 // clear the list when an 'ONLY' CAPMT object is received, and start
       // working with the object
#define CAPMT_LIST_ADD \
  0x04 // append an 'ADD' CAPMT object to the current list and start working
       // with the updated list
#define CAPMT_LIST_UPDATE \
  0x05 // replace an entry in the list with an 'UPDATE' CAPMT object, and start
       // working with the updated list
...
typedef struct ca_info {
  uint16_t pids[MAX_PIDS]; // elementary stream pids (was: sequence / service id
                           // number)
  enum {
    CA_ALGO_DVBCSA,
    CA_ALGO_DES,
...
static inline int capmt_include_elementary_stream(
...
static int capmt_connect(capmt_t* capmt, int i) {
  ...
  if (capmt_oscam_network(capmt)) {
    ...
    fd = tcp_connect(capmt->capmt_sockfile,
        capmt->capmt_port,
        NULL,
        errbuf,
        sizeof(errbuf),
...
#endif
  {
    ...
    if (fd < 0 ||
        connect(fd,
            (const struct sockaddr*)&serv_addr_un,
 ...
 *
 */
static int capmt_write_msg(capmt_t* capmt,
    int                             adapter,
    int                             sid,
    const uint8_t*                  buf,
    size_t                          len) {
  ...
  if (fd <= 0) {
    tvhtrace(LS_CAPMT,
        "%s: Unable to send message for sid %i",
        capmt_name(capmt),
        sid);
    return -1;
  }
  ...
  tvhtrace(LS_CAPMT,
      "%s: Sending message to socket %i (sid %i)",
      capmt_name(capmt),
      fd,
      sid);
...
#else
    tvhdebug(LS_CAPMT,
        "%s: Message send failed to socket %i (%zi)",
        capmt_name(capmt),
        fd,
        res);
#endif
 ...
 *
 */
static void capmt_queue_msg(capmt_t* capmt,
    int                              adapter,
    int                              sid,
    const uint8_t*                   buf,
    size_t                           len,
...
static void capmt_send_client_info(capmt_t* capmt) {
  ...
  *(uint16_t*)(buf + 4) =
      htons(DVBAPI_PROTOCOL_VERSION); // supported protocol version
...
capmt_set_filter(capmt_t* capmt, int adapter, sbuf_t* sb, int offset) {
  ...
  tvhtrace(LS_CAPMT,
      ...
      filter_index,
      pid);
  if (adapter >= MAX_CA || demux_index >= MAX_INDEX ||
      filter_index >= MAX_FILTER || pid > 8191)
    return;
  tvh_mutex_lock(&capmt->capmt_mutex);
...
capmt_stop_filter(capmt_t* capmt, int adapter, sbuf_t* sb, int offset) {
  ...
  while (
      demux_index != 255 && capmt->capmt_demuxes.filters[demux_index].max == 0)
    demux_index--;
...
static void capmt_send_key(capmt_t* capmt) {
  ...
  switch (cai->algo) {
    ...
    default:
      tvherror(LS_CAPMT,
          "unknown crypto algorithm %d (mode %d)",
          cai->algo,
          cai->cipher_mode);
      return;
  }
...
static int capmt_msg_size(capmt_t* capmt, sbuf_t* sb, int offset) {
  ...
  if (capmt_oscam_netproto(capmt)) {
    /* we need to take into account the adapter index byte which is now after
     * the cmd */
    adapter_byte = 1;
  } else {
    if (!sb->sb_bswap && !sb->sb_err) {
      if (cmd == CA_SET_PID_X || cmd == CA_SET_DESCR_X ||
          cmd == CA_SET_DESCR_AES_X || cmd == DMX_SET_FILTER_X ||
          cmd == DMX_STOP_X) {
        sb->sb_bswap = 1;
        cmd          = sbuf_peek_u32(sb, offset);
      }
...
static void capmt_analyze_cmd(capmt_t* capmt,
    uint32_t                           cmd,
    int                                adapter,
    sbuf_t*                            sb,
    int                                offset) {
  if (cmd == CA_SET_PID_) {
    ...
    tvhdebug(LS_CAPMT,
        "%s: CA_SET_DESCR adapter %d par %d idx %d "
    ...
    if (adapter >= MAX_CA || index >= MAX_INDEX) {
      tvherror(LS_CAPMT,
          "%s: Invalid adapter %d or index %d",
          capmt_name(capmt),
          adapter,
          index);
      return;
    ...
    if (capmt->capmt_cwmode !=
        CAPMT_CWMODE_OE22SW) /* wait for CA_SET_DESCR_MODE */
    ...
    cai = &capmt->capmt_adapters[adapter].ca_info[index];
    if (parity == 0) {
      capmt_process_key(capmt,
          adapter,
          cai,
          DESCRAMBLER_AES128_ECB,
          cw,
          NULL,
          1);
    } else if (parity == 1) {
    ...
    tvhdebug(LS_CAPMT,
        "%s: ECM_INFO: adapter=%d sid=%d caid=%04X(%s) pid=%04X provid=%06X "
        "ecmtime=%d hops=%d reader=%s from=%s protocol=%s",
        capmt_name(capmt),
        adapter,
        sid,
        caid,
...
static int capmt_create_udp_socket(capmt_t* capmt, int* socket, int port) {
  *socket = tvh_socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
  ...
  if (bind(*socket, (const struct sockaddr*)&serv_addr, sizeof(serv_addr)) !=
      0) {
    tvherror(LS_CAPMT,
        "%s: failed to bind to ca0 (port %d)",
        capmt_name(capmt),
        port);
    return 0;
  } else
...
static void* capmt_thread(void* aux) {
  ...
  while (atomic_get(&capmt->capmt_running)) {
      ...
      if (capmt_oscam_new(capmt)) {
...
#endif
        {
          ...
          for (i = 0; i < is->is_count; i++) {
            ...
            if (n < 0 || n >= MAX_CA) {
              tvherror(LS_CAPMT,
                  "%s: adapter number > MAX_CA",
                  capmt_name(capmt));
              continue;
            }
            tvhinfo(LS_CAPMT,
                "%s: created UDP socket %d",
                capmt_name(capmt),
                n);
            bind_ok = capmt_create_udp_socket(capmt,
    ...
    tvhinfo(LS_CAPMT,
        "%s: Automatic reconnection attempt in %d seconds",
        capmt_name(capmt),
        d);
 ...
 *
 */
static void capmt_table_input(void* opaque,
    int                             pid,
    const uint8_t*                  data,
    int                             len,
    int                             emm) {
    ...
    for (filter_index = 0; filter_index < cf->max; filter_index++) {
      ...
      if (df->pid == pid && (df->flags & CAPMT_MSG_FAST) == flags) {
        ...
        if (i >= DMX_FILTER_SIZE || i + 2 == len) {
          tvhtrace(LS_CAPMT,
              "filter match pid %d len %d emm %d",
              pid,
              len,
              emm);
...
static int capmt_update_elementary_stream(capmt_service_t* ct,
    int*                                                   _i,
  ...
  *_i = i + 1;
  if (st->es_pid != ct->ct_pids[i] || type != ct->ct_types[i]) {
    ...
    ct->ct_type_sok[i] = SCT_ISVIDEO(st->es_type) || SCT_ISAUDIO(st->es_type) ||
    ...
    if (cce2->cce_providerid) { // we need to add provider ID to the data
      if (cce2->cce_caid >> 8 == 0x01) {
        ...
        buf[pos2++]  = cce2->cce_providerid & 0xff;
          ...
          (cce2->cce_caid == 0x2710)) {
        if (cce2->cce_caid == 0x4AE0 || cce2->cce_caid == 0x4AE1 ||
            cce2->cce_caid == 0x2710) {
          buf[pos + 1] = 10;
          buf[pos2++]  = cce2->cce_providerid & 0xff;
          buf[pos2++]  = 0x00;
...
static void capmt_service_start(caclient_t* cac, service_t* s) {
  ...
  if (tuner < 0 && !capmt_oscam_network(capmt) && !capmt_oscam_socket(capmt)) {
    tvhwarn(LS_CAPMT,
        "%s: Virtual adapters are supported only in modes 3, 4, 5 and 6 "
        "(service \"%s\")",
        capmt_name(capmt),
        t->s_dvb_svcname);
    goto fin;
...
static void capmt_conf_changed(caclient_t* cac) {
  ...
  idnode_get_title(&capmt->cac_id,
      NULL,
      capmt->capmt_name,
      sizeof(capmt->capmt_name));
...
static htsmsg_t* caclient_capmt_class_oscam_mode_list(void* o,
    const char*                                             lang) {
