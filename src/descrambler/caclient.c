...
static const idclass_t* caclient_class_find(const char* name) {
  const idclass_t** r;
...
static void caclient_reindex(void) {
  caclient_t* cac;
  int         i = 1;
...
static int cac_cmp(caclient_t* a, caclient_t* b) {
  return a->cac_index - b->cac_index;
}

caclient_t* caclient_create(const char* uuid, htsmsg_t* conf, int save) {
  caclient_t*      cac = NULL;
...
#endif
#if ENABLE_CONSTCW
  if (c == &caclient_ccw_csa_cbc_class || c == &caclient_ccw_des_ncb_class ||
      c == &caclient_ccw_aes_ecb_class || c == &caclient_ccw_aes128_ecb_class)
    cac = constcw_create();
#endif
  if (cac == NULL) {
...
static void caclient_delete(caclient_t* cac, int delconf) {
...
static void caclient_class_changed(idnode_t* in) {
  caclient_t* cac = (caclient_t*)in;
  cac->cac_conf_changed(cac);
}

static htsmsg_t* caclient_class_save(idnode_t* in, char* filename, size_t fsize) {
  char      ubuf[UUID_HEX_SIZE];
...
static void caclient_class_get_title(idnode_t* in, const char* lang, char* dst, size_t dstsize) {
  caclient_t* cac = (caclient_t*)in;
  if (cac->cac_name && cac->cac_name[0]) {
    snprintf(dst, dstsize, "%s", cac->cac_name);
  } else {
    snprintf(dst, dstsize, tvh_gettext_lang(lang, N_("CA client %i")), cac->cac_index);
  }
}

static void caclient_class_delete(idnode_t* self) {
  caclient_t* cac = (caclient_t*)self;
  caclient_delete(cac, 1);
}

static void caclient_class_moveup(idnode_t* self) {
  caclient_t* cac  = (caclient_t*)self;
...
static void caclient_class_movedown(idnode_t* self) {
  caclient_t* cac  = (caclient_t*)self;
...
static const void* caclient_class_class_get(void* o) {
  caclient_t*        cac = o;
...
static int caclient_class_class_set(void* o, const void* v) {
  /* just ignore, create fcn does the right job */
  return 0;
}

static const void* caclient_class_status_get(void* o) {
  caclient_t* cac = o;
...
const idclass_t caclient_class = {.ic_class = "caclient",
...
void caclient_cat_update(struct mpegts_mux* mux, const uint8_t* data, int len) {
  caclient_t* cac;
...
const char* caclient_get_status(caclient_t* cac) {
  switch (cac->cac_status) {
    case CACLIENT_STATUS_NONE:
      return "caclientNone";
    case CACLIENT_STATUS_READY:
      return "caclientReady";
    case CACLIENT_STATUS_CONNECTED:
      return "caclientConnected";
    case CACLIENT_STATUS_DISCONNECTED:
      return "caclientDisconnected";
    default:
      return "caclientUnknown";
  }
}
