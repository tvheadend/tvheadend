...
cc_card_data_t* cc_new_card(cclient_t* cc,
  ...
  if (add && !allocated) {
    ...
    providers =
        calloc(start_index + add_pcount, sizeof(pcard->cs_ra.providers[0]));
    memcpy(providers,
        pcard->cs_ra.providers,
        start_index * sizeof(pcard->cs_ra.providers[0]));
    pid = add_pid;
    psa = add_psa;
  } else {
...
skip_providers:
  ...
  buf[0] = '\0';
  i      = start_index;
  for (j = c = 0, ep = pcard->cs_ra.providers + start_index; i < pcount;
       i++, ep++) {
...
cc_find_pending_section(cclient_t* cc, uint32_t seq, cc_service_t** _ct) {
  ...
  tvhwarn(cc->cc_subsys,
      "%s: Got unexpected ECM reply (seqno: %d)",
      cc->cc_name,
      seq);
  return NULL;
}
...
static void cc_session(cclient_t* cc) {
  ...
  while (!cc_must_break(cc)) {
    ...
    if ((cm = TAILQ_FIRST(&cc->cc_writeq)) != NULL) {
      ...
      tvhtrace(cc->cc_subsys,
          "%s: sending queued message len %u",
          cc->cc_name,
          cm->cm_len);
...
found:
  ...
  if (ecm) {
    ...
    if (ct->cs_capid != 0xffff && ct->cs_capid > 0 && pid > 0 &&
        ct->cs_capid != pid) {
      tvhdebug(cc->cc_subsys,
...
void cc_caid_update(caclient_t* cac,
  ...
  tvhtrace(cc->cc_subsys,
      "%s: caid update event - client %s mux %p caid %04x (%i) prov %06x (%i) "
      "pid %04x (%i) valid %i",
      cc->cc_name,
      cac->cac_name,
      mux,
...
const idclass_t caclient_cc_class = {.ic_super = &caclient_class,
    ...
    .ic_properties                             = (const property_t[]){
        ...
        },
        {
            ...
            .desc  = N_("Enable/disable offering of Entitlement Management "
                       "Message updates."),
        ...
        },
        {
            ...
            .desc =
                N_("Update Entitlement Management Messages from one mux only."),
