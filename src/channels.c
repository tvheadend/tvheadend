...
static int ch_id_cmp(channel_t* a, channel_t* b) {
  return channel_get_id(a) - channel_get_id(b);
}

static void channel_load_icon(channel_t* ch) {
  (void)imagecache_get_id(channel_get_icon(ch));
}

static void channel_tag_load_icon(channel_tag_t* tag) {
...
static void channel_class_changed(idnode_t* self) {
...
static htsmsg_t* channel_class_save(idnode_t* self, char* filename, size_t fsize) {
  channel_t* ch = (channel_t*)self;
  htsmsg_t*  c  = NULL;
...
static void channel_class_delete(idnode_t* self) {
  channel_delete((channel_t*)self, 1);
}

static void channel_class_notify_enabled(void* obj, const char* lang) {
  channel_t* ch = (channel_t*)obj;
  if (!ch->ch_enabled)
    channel_remove_subscriber(ch, SM_CODE_CHN_NOT_ENABLED);
}

static int channel_class_autoname_set(void* obj, const void* p) {
  channel_t*  ch = (channel_t*)obj;
  const char* s;
  int         b = *(int*)p;
  if (ch->ch_autoname != b) {
...
static const void* channel_class_services_get(void* obj) {
  channel_t* ch = obj;
  return idnode_list_get2(&ch->ch_services);
}

static char* channel_class_services_rend(void* obj, const char* lang) {
  channel_t* ch = obj;
  return idnode_list_get_csv2(&ch->ch_services, lang);
}

static int channel_class_services_set(void* obj, const void* p) {
  channel_t* ch = obj;
  return idnode_list_set2(&ch->ch_id,
      &ch->ch_services,
      &service_class,
      (htsmsg_t*)p,
...
static htsmsg_t* channel_class_services_enum(void* obj, const char* lang) {
...
static const void* channel_class_tags_get(void* obj) {
  channel_t* ch = obj;
  return idnode_list_get2(&ch->ch_ctms);
}

static char* channel_class_tags_rend(void* obj, const char* lang) {
  channel_t* ch = obj;
  return idnode_list_get_csv2(&ch->ch_ctms, lang);
}

static int channel_class_tags_set_cb(idnode_t* in1, idnode_t* in2, void* origin) {
  return channel_tag_map((channel_tag_t*)in1, (channel_t*)in2, origin);
}

static int channel_class_tags_set(void* obj, const void* p) {
  channel_t* ch = obj;
  return idnode_list_set2(&ch->ch_id,
      &ch->ch_ctms,
      &channel_tag_class,
...
static void channel_class_icon_notify(void* obj, const char* lang) {
  channel_t* ch = obj;
  if (!ch->ch_load)
...
static void channel_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
  const char* s = channel_get_name((channel_t*)self, tvh_gettext_lang(lang, channel_blank_name));
...
static int channel_class_set_name(void* o, const void* p) {
  channel_t*  ch = o;
  const char* s  = p;
  if (ch->ch_load)
...
static const void* channel_class_get_name(void* o) {
  prop_ptr = channel_get_name(o, channel_blank_name);
  return &prop_ptr;
}

static const void* channel_class_get_number(void* o) {
  static int64_t i;
  i = channel_get_number(o);
  return &i;
}

static const void* channel_class_epggrab_get(void* o) {
  channel_t* ch = o;
  return idnode_list_get2(&ch->ch_epggrab);
}

static int channel_class_epggrab_set(void* o, const void* v) {
  channel_t* ch = o;
  return idnode_list_set2(&ch->ch_id,
      &ch->ch_epggrab,
      &epggrab_channel_class,
      (htsmsg_t*)v,
      epggrab_channel_map);
}

static htsmsg_t* channel_class_epggrab_list(void* o, const char* lang) {
  htsmsg_t *e, *m = htsmsg_create_map();
  htsmsg_add_str(m, "type", "api");
...
static const void* channel_class_bouquet_get(void* o) {
  channel_t* ch = o;
  if (ch->ch_bouquet)
...
static int channel_class_bouquet_set(void* o, const void* v) {
  channel_t* ch = o;
...
static int channel_class_epg_parent_set_noupdate(void* o, const void* v, channel_t** parent) {
  channel_t*  ch   = o;
  const char* uuid = v;
...
static int channel_class_epg_parent_set(void* o, const void* v) {
...
static htsmsg_t* channel_class_epg_running_list(void* o, const char* lang) {
...
const idclass_t channel_class = {.ic_class = "channel",
...
/// buffer
static char* channel_make_fuzzy_name(const char* name) {
  if (!name)
    return NULL;
...
channel_t* channel_find_by_id(uint32_t i) {
  channel_t skel;
...
int channel_access(channel_t* ch, access_t* a, int disabled) {
  ...
  if (!ch) {
    ...
    if (a->aa_chrange == NULL && a->aa_chtags == NULL && a->aa_chtags_exclude == NULL)
      return 1;
    return 0;
  }
  ...
  if (a->aa_chtags_exclude) {
    ...
    LIST_FOREACH (ilm, &ch->ch_ctms, ilm_in2_link)
      if (!strcmp(htsmsg_field_get_str(f) ?: "", idnode_uuid_as_str(ilm->ilm_in1, ubuf)))
        return 0;
  }
  if (a->aa_chtags) {
...
char* channel_get_ename(channel_t* ch,
    char*                          dst,
    size_t                         dstlen,
    const char*                    blank,
    uint32_t                       flags) {
  ...
  if (flags & CHANNEL_ENAME_NUMBERS) {
    ...
    if (number > 0) {
      if (number % CHANNEL_SPLIT) {
        tvh_strlcatf(dst, dstlen, l, "%u.%u", channel_get_major(number), channel_get_minor(number));
      } else {
...
static char* svcnamepicons(const char* svcname) {
  const char* s;
  char *      r, *d;
  int         count;
  char        c;
...
static int check_file(const char* url) {
...
const char* channel_get_icon(channel_t* ch) {
  ...
  idnode_list_mapping_t* ilm;
  const char *chicon = config.chicon_path, *picon = config.picon_path, *icon = ch->ch_icon, *chname,
             *icn;
  ...
  if (icon == NULL) {
    LIST_FOREACH (ilm, &ch->ch_services, ilm_in2_link) {
      if (!(icn = service_get_channel_icon((service_t*)ilm->ilm_in1)))
        continue;
      if (strncmp(icn, "picon://", 8) == 0)
        continue;
      if (check_file(icn)) {
  ...
  for (i = 0; icon == NULL && i < 4; i++) {
    ...
    if (!pick && chicon && chicon[0] >= ' ' && chicon[0] <= 122 &&
      ...
      if ((send = strstr(chi, "%C")) || (send = strstr(chi, "%c"))) {
        sname =
            intlconv_utf8safestr(intlconv_charset_id("ASCII", 1, 1), chname, strlen(chname) * 2);
        if (sname == NULL)
          sname = strdup(chname);
...
channel_t* channel_create0(channel_t* ch,
    const idclass_t*                  idc,
    const char*                       uuid,
    htsmsg_t*                         conf,
    const char*                       name) {
 ...
 * sorting
 */
static int channel_cmp1(const void* a, const void* b) {
  int        r;
...
static int channel_cmp2(const void* a, const void* b) {
  channel_t *c1 = *(channel_t**)a, *c2 = *(channel_t**)b;
...
channel_t** channel_get_sorted_list(const char* sort_type, int all, int* _count) {
  int        count = 0;
...
channel_t**
channel_get_sorted_list_for_tag(const char* sort_type, channel_tag_t* tag, int* _count) {
  int                    count = 0;
  channel_t *            ch, **chlist = malloc(channels_count * sizeof(channel_t*));
  idnode_list_mapping_t* ilm;
...
static memoryinfo_t channels_memoryinfo = {.my_name = "Channels",
...
int channel_tag_map(channel_tag_t* ct, channel_t* ch, void* origin) {
  ...
  ilm = idnode_list_link(&ct->ct_id, &ct->ct_ctms, &ch->ch_id, &ch->ch_ctms, origin, 2);
  if (ilm) {
 ...
 */
int channel_tag_access(channel_tag_t* ct, access_t* a, int disabled) {
  htsmsg_field_t* f;
  char            ubuf[UUID_HEX_SIZE];
...
static void channel_tag_class_changed(idnode_t* self) {
  /* HTSP */
  htsp_tag_update((channel_tag_t*)self);
}

static htsmsg_t* channel_tag_class_save(idnode_t* self, char* filename, size_t fsize) {
...
static void channel_tag_class_delete(idnode_t* self) {
  channel_tag_destroy((channel_tag_t*)self, 1);
}

static void
channel_tag_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
  channel_tag_t* ct = (channel_tag_t*)self;
  snprintf(dst, dstsize, "%s", ct->ct_name ?: "");
}

static void channel_tag_class_icon_notify(void* obj, const char* lang) {
  channel_tag_load_icon(obj);
}

static const void* channel_tag_class_get_icon(void* obj) {
...
const idclass_t channel_tag_class = {.ic_class = "channeltag",
 ...
 *
 */
static int channel_tag_cmp1(const void* a, const void* b) {
...
static int channel_tag_cmp2(const void* a, const void* b) {
  channel_tag_t *ct1 = *(channel_tag_t**)a, *ct2 = *(channel_tag_t**)b;
...
channel_tag_t** channel_tag_get_sorted_list(const char* sort_type, int* _count) {
  int            count = 0;
...
static memoryinfo_t channel_tags_memoryinfo = {.my_name = "Channel tags",
