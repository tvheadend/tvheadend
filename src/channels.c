...
static void channel_tag_mapping_destroy(idnode_list_mapping_t* ilm,
    void*                                                      origin);
...
static void channel_class_get_title(idnode_t* self,
    const char*                               lang,
    char*                                     dst,
    size_t                                    dstsize) {
...
static int channel_class_epg_parent_set_noupdate(void* o,
    const void*                                        v,
    channel_t**                                        parent) {
  channel_t*  ch   = o;
  const char* uuid = v;
...
const idclass_t channel_class = {.ic_class = "channel",
    ...
    .ic_properties                         = (const property_t[]){
        ...
        },
        {
            ...
            .notify = channel_class_icon_notify, /* try to re-render default
                                                    icon path */
        },
        {
            .type     = PT_S64,
...
channel_t* channel_find_by_name_and_bouquet(const char* name,
    const struct bouquet*                               bq) {
  channel_t*  ch;
  const char* s;
...
channel_t* channel_find_by_name_bouquet_fuzzy(const char* name,
    const struct bouquet*                                 bq) {
  channel_t*  ch;
  const char* s;
...
int64_t channel_get_number(const channel_t* ch) {
  ...
  if (ch->ch_number) {
    ...
    if (ch->ch_bouquet) {
      LIST_FOREACH (ilm, &ch->ch_services, ilm_in2_link) {
        v = bouquet_get_channel_number(ch->ch_bouquet,
            (service_t*)ilm->ilm_in1);
        if (v > 0 && (n == 0 || v < n))
          n = v;
      }
    }
...
const char* channel_get_icon(channel_t* ch) {
  ...
  idnode_list_mapping_t* ilm;
  const char *chicon = config.chicon_path, *picon = config.picon_path,
             *icon = ch->ch_icon, *chname, *icn;
        ...
        if (config.chicon_scheme == CHICON_SVCNAME) {
          ...
          while (s && *s) {
            ...
            if (send[1] == 'C' && (c > 122 || strchr(":<>|*?'\"", c) != NULL))
              *(char*)s = '_';
            else if (config.chicon_scheme == CHICON_LOWERCASE && c >= 'A' &&
                c <= 'Z')
              *(char*)s = c - 'A' + 'a';
            s++;
          }
...
void channel_delete(channel_t* ch, int delconf) {
  ...
  /* Settings */
  if (delconf)
    hts_settings_remove("channel/config/%s",
        idnode_uuid_as_str(&ch->ch_id, ubuf));
...
channel_t** channel_get_sorted_list_for_tag(const char* sort_type,
    channel_tag_t*                                      tag,
    int*                                                _count) {
  int        count = 0;
  channel_t *ch, **chlist = malloc(channels_count * sizeof(channel_t*));
...
channel_tag_class_save(idnode_t* self, char* filename, size_t fsize) {
  ...
  if (filename)
    snprintf(filename,
        fsize,
        "channel/tag/%s",
        idnode_uuid_as_str(&ct->ct_id, ubuf));
  return c;
}
...
static void channel_tag_class_get_title(idnode_t* self,
    const char*                                   lang,
    char*                                         dst,
    size_t                                        dstsize) {
  channel_tag_t* ct = (channel_tag_t*)self;
...
channel_tag_t** channel_tag_get_sorted_list(const char* sort_type,
    int*                                                _count) {
  int            count = 0;
