...
#define tvh_transcoder_log(self, level, fmt, ...)                                        \
  do {                                                                                   \
    tvhlog((level), LS_TRANSCODE, "%04X: " fmt, (((self)->id) & 0xffff), ##__VA_ARGS__); \
  } while (0)

#define _stream_log(level, fmt, transcoder, id, type, ...) \
  do {                                                     \
    tvh_transcoder_log((transcoder),                       \
        (level),                                           \
        "%02d:%s: " fmt,                                   \
        (id),                                              \
...
#define tvh_ssc_log(ssc, level, fmt, transcoder, ...)                                        \
  do {                                                                                       \
    _stream_log((level), fmt, (transcoder), (ssc)->es_index, (ssc)->es_type, ##__VA_ARGS__); \
  } while (0)

#define tvh_stream_log(self, level, fmt, ...)                                               \
  do {                                                                                      \
    _stream_log((level), fmt, (self)->transcoder, (self)->id, (self)->type, ##__VA_ARGS__); \
  } while (0)
