...
static int tvh_stream_is_copy(TVHCodecProfile* profile, tvh_ssc_t* ssc, const char* src_codecs) {
  const char* txtname;
  htsmsg_t*   list;
  int         r;
...
static int tvh_stream_setup(TVHStream* self, TVHCodecProfile* profile, tvh_ssc_t* ssc) {
  ...
  if (!icodec && !(icodec = avcodec_find_decoder(icodec_id))) {
    tvh_stream_log(self,
        LOG_ERR,
        "failed to find decoder for '%s'",
        streaming_component_type2txt(ssc->es_type));
    return -1;
  }
  if (!(ocodec = tvh_codec_profile_get_avcodec(profile))) {
    tvh_stream_log(self, LOG_ERR, "profile '%s' is disabled", tvh_codec_profile_get_name(profile));
    return -1;
  }
  ...
  if (!(self->context = tvh_context_create(self, profile, icodec, ocodec, ssc->ssc_gh))) {
    return -1;
  }
  ...
  if (ssc->es_type == SCT_UNKNOWN) {
    tvh_stream_log(self,
        LOG_ERR,
        "unable to translate AV type %s [%d] to SCT!",
        ocodec->name,
        ocodec->id);
  }
  ssc->ssc_gh = NULL;
