...
static void tvh_context_type_register(TVHContextType* self) {
  if (self->media_type <= AVMEDIA_TYPE_UNKNOWN || self->media_type >= AVMEDIA_TYPE_NB ||
      !self->ship) {
    tvherror(LS_TRANSCODE, "incomplete/wrong definition for context type");
    return;
  }
  SLIST_INSERT_HEAD(&tvh_context_types, self, link);
  tvhinfo(LS_TRANSCODE, "'%s' context type registered", av_get_media_type_string(self->media_type));
}

static TVHContextType* tvh_context_type_find(enum AVMediaType media_type) {
...
static void _context_print_opts(TVHContext* self, AVDictionary* opts) {
  char* buffer = NULL;
...
static int _context_filters_apply_sink_options(TVHContext* self, va_list ap) {
  ...
  while ((opt_name = va_arg(ap, const char*))) {
    ...
    if ((ret = av_opt_set_bin(self->oavfltctx,
             opt_name,
             opt_val,
             opt_size,
             AV_OPT_SEARCH_CHILDREN))) {
      tvh_context_log(self, LOG_ERR, "filters: failed to set option: '%s'", opt_name);
      break;
    }
  }
  return ret;
...
static int _context_open(TVHContext* self, TVHOpenPhase phase, AVDictionary** opts) {
  return (self->type->open) ? self->type->open(self, phase, opts) : 0;
}

static int _context_decode(TVHContext* self, AVPacket* avpkt) {
  return (self->type->decode) ? self->type->decode(self, avpkt) : 0;
}

static int _context_encode(TVHContext* self, AVFrame* avframe) {
...
static int _context_meta(TVHContext* self, AVPacket* avpkt, th_pkt_t* pkt) {
...
static int _context_wrap(TVHContext* self, AVPacket* avpkt, th_pkt_t* pkt) {
  return (self->type->wrap) ? self->type->wrap(self, avpkt, pkt) : 0;
}

// creation

static AVCodecContext* tvh_context_alloc_avctx(TVHContext* context, const AVCodec* avcodec) {
...
static int tvh_context_setup(TVHContext* self, const AVCodec* iavcodec, const AVCodec* oavcodec) {
  ...
  if (!(self->iavctx = tvh_context_alloc_avctx(self, iavcodec)) ||
      ...
      !(self->iavframe = av_frame_alloc()) || !(self->oavframe = av_frame_alloc())) {
    tvh_stream_log(self->stream, LOG_ERR, "failed to allocate AVCodecContext/AVFrame");
    return -1;
  }
  return 0;
}
...
static int tvh_context_open(TVHContext* self, TVHOpenPhase phase) {
  AVCodecContext*   avctx  = NULL;
  ...
  if (!ret) {
    ...
    ret = tvh_context_get_int_opt(&opts, "tvh_require_meta", &self->require_meta);
...
static th_pkt_t* tvh_context_pack(TVHContext* self, AVPacket* avpkt) {
  th_pkt_t* pkt = NULL;
...
static int tvh_context_ship(TVHContext* self, AVPacket* avpkt) {
  th_pkt_t* pkt = NULL;
  int       ret = -1;

  if (((ret = self->type->ship(self, avpkt)) > 0) && (pkt = tvh_context_pack(self, avpkt))) {
...
static int tvh_context_receive_packet(TVHContext* self) {
  AVPacket avpkt;
  int      ret = -1;
...
static int tvh_context_encode_frame(TVHContext* self, AVFrame* avframe) {
  int ret = -1;
...
static int tvh_context_pull_frame(TVHContext* self, AVFrame* avframe) {
  int ret = -1;
...
static int tvh_context_push_frame(TVHContext* self, AVFrame* avframe) {
  int ret = -1;
...
static int tvh_context_encode(TVHContext* self, AVFrame* avframe) {
  int ret = 0;
...
static int tvh_context_receive_frame(TVHContext* self, AVFrame* avframe) {
  int ret = -1;
...
static int tvh_context_decode_packet(TVHContext* self, const AVPacket* avpkt) {
  int ret = -1;
...
static int tvh_context_decode(TVHContext* self, AVPacket* avpkt) {
  int ret = 0;
...
static void tvh_context_flush(TVHContext* self) {
...
int tvh_context_get_int_opt(AVDictionary** opts, const char* key, int* value) {
  AVDictionaryEntry* entry = NULL;

  if (*opts && (entry = av_dict_get(*opts, key, NULL, AV_DICT_MATCH_CASE))) {
...
int tvh_context_get_str_opt(AVDictionary** opts, const char* key, char** value) {
  AVDictionaryEntry* entry = NULL;
...
void tvh_context_close(TVHContext* self, int flush) {
  if (flush) {
...
int tvh_context_open_filters(TVHContext* self,
  ...
  if (!(self->avfltgraph = avfilter_graph_alloc()) || !(iavfltio = avfilter_inout_alloc()) ||
      !(oavfltio = avfilter_inout_alloc()) || !(par = av_buffersrc_parameters_alloc())) {
    ret = AVERROR(ENOMEM);
    goto finish;
  }
  ...
  ret = avfilter_graph_create_filter(&self->iavfltctx,
      iavflt,
      "in",
      source_args,
      NULL,
  ...
  if (!strcmp("buffer", source_name) && self->iavctx->hw_frames_ctx) { // hmm...
    ...
    if (ret < 0) {
      tvh_context_log(self, LOG_ERR, "filters: failed to set additional parameters");
      goto finish;
    }
  }
  ...
  // filters
  ret = avfilter_graph_parse_ptr(self->avfltgraph, filters, &iavfltio, &oavfltio, NULL);
  if (ret < 0) {
    tvh_context_log(self, LOG_ERR, "filters: failed to add filters: '%s'", filters);
    goto finish;
  }
  ...
  if (!strcmp("buffer", source_name) && self->hw_device_octx) {
    for (i = 0; i < self->avfltgraph->nb_filters; i++) {
      if (!(self->avfltgraph->filters[i]->hw_device_ctx = av_buffer_ref(self->hw_device_octx))) {
        ret = -1;
        goto finish;
      }
    }
...
int tvh_context_deliver(TVHContext* self, th_pkt_t* pkt) {
  return tvh_stream_deliver(self->stream, pkt);
}

int tvh_context_handle(TVHContext* self, th_pkt_t* pkt) {
  int      ret  = 0;
  uint8_t* data = NULL;
  size_t   size = 0;
  AVPacket avpkt;
...
TVHContext* tvh_context_create(TVHStream* stream,
    TVHCodecProfile*                      profile,
    const AVCodec*                        iavcodec,
    const AVCodec*                        oavcodec,
    pktbuf_t*                             input_gh) {
...
void tvh_context_destroy(TVHContext* self) {
  if (self) {
