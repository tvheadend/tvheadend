...
static void tvh_context_type_register(TVHContextType* self) {
  if (self->media_type <= AVMEDIA_TYPE_UNKNOWN ||
      self->media_type >= AVMEDIA_TYPE_NB || !self->ship) {
...
static int _context_filters_apply_sink_options(TVHContext* self, va_list ap) {
  ...
  while ((opt_name = va_arg(ap, const char*))) {
    ...
    if ((ret = av_opt_set_bin(self->oavfltctx,
      ...
      tvh_context_log(self,
          LOG_ERR,
          "filters: failed to set option: '%s'",
          opt_name);
      break;
    }
  }
...
static int _context_meta(TVHContext* self, AVPacket* avpkt, th_pkt_t* pkt) {
  ...
  if (self->require_meta) {
    if (self->helper && self->helper->meta) {
      ...
      pkt->pkt_meta =
          pktbuf_alloc(self->oavctx->extradata, self->oavctx->extradata_size);
...
static AVCodecContext* tvh_context_alloc_avctx(TVHContext* context,
...
static int tvh_context_setup(TVHContext* self,
...
static int tvh_context_open(TVHContext* self, TVHOpenPhase phase) {
  ...
  if (!ret) {
    ...
    ret =
        tvh_context_get_int_opt(&opts, "tvh_require_meta", &self->require_meta);
    if (!ret && !(ret = _context_open(self, phase, &opts)) && // pre open
        !(ret = (helper && helper->open) ? helper->open(self, &opts)
                                         : 0) &&      // pre open
...
static th_pkt_t* tvh_context_pack(TVHContext* self, AVPacket* avpkt) {
  ...
  if (self->helper && self->helper->pack) {
    ...
    pkt = pkt_alloc(self->stream->type,
        avpkt->data,
        avpkt->size,
        avpkt->pts,
        avpkt->dts,
...
static int tvh_context_receive_packet(TVHContext* self) {
  ...
  while (
      (ret = avcodec_receive_packet(self->oavctx, &avpkt)) != AVERROR(EAGAIN)) {
...
static int tvh_context_encode_frame(TVHContext* self, AVFrame* avframe) {
  ...
  if (!(ret = avframe ? _context_encode(self, avframe) : 0)) {
    while (
        (ret = avcodec_send_frame(self->oavctx, avframe)) == AVERROR(EAGAIN)) {
...
static int tvh_context_pull_frame(TVHContext* self, AVFrame* avframe) {
  ...
  av_frame_unref(avframe);
  ret = av_buffersink_get_frame_flags(self->oavfltctx,
      avframe,
      TVH_BUFFERSINK_FLAGS);
  return (ret > 0) ? 0 : ret;
}
...
static int tvh_context_push_frame(TVHContext* self, AVFrame* avframe) {
  ...
  ret = av_buffersrc_add_frame_flags(self->iavfltctx,
      avframe,
      TVH_BUFFERSRC_FLAGS);
  return (ret > 0) ? 0 : ret;
}
...
static int tvh_context_encode(TVHContext* self, AVFrame* avframe) {
  ...
  if (!ret && !(ret = tvh_context_push_frame(self, avframe))) {
    while ((ret = tvh_context_pull_frame(self, self->oavframe)) !=
        AVERROR(EAGAIN)) {
...
static int tvh_context_receive_frame(TVHContext* self, AVFrame* avframe) {
  ...
  while (
      (ret = avcodec_receive_frame(self->iavctx, avframe)) != AVERROR(EAGAIN)) {
...
int tvh_context_get_str_opt(AVDictionary** opts,
    const char*                            key,
    char**                                 value) {
  AVDictionaryEntry* entry = NULL;
...
int tvh_context_open_filters(TVHContext* self,
  ...
  if (!(iavflt = avfilter_get_by_name(source_name))) {
    tvh_context_log(self,
        LOG_ERR,
        "filters: source filter'%s' not found",
        source_name);
  ...
  if (!(oavflt = avfilter_get_by_name(sink_name))) {
    tvh_context_log(self,
        LOG_ERR,
        "filters: sink filter '%s' not found",
        sink_name);
    ret = AVERROR_FILTER_NOT_FOUND;
    goto finish;
  }
  ret = avfilter_graph_create_filter(&self->oavfltctx,
      oavflt,
      "out",
      NULL,
      NULL,
  ...
  // filters
  ret = avfilter_graph_parse_ptr(self->avfltgraph,
      filters,
      &iavfltio,
      &oavfltio,
      NULL);
  if (ret < 0) {
    tvh_context_log(self,
        LOG_ERR,
        "filters: failed to add filters: '%s'",
        filters);
    goto finish;
  }
...
int tvh_context_handle(TVHContext* self, th_pkt_t* pkt) {
  ...
  if ((size = pktbuf_len(pkt->pkt_payload)) && pktbuf_ptr(pkt->pkt_payload)) {
    if (size >= TVH_INPUT_BUFFER_MAX_SIZE) {
      ...
      av_init_packet(&avpkt);
      if ((ret = av_packet_from_data(&avpkt,
               data,
               size))) { // takes ownership of data
