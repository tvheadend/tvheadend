...
static enum AVMediaType ssc_get_media_type(tvh_ssc_t* ssc) {
  if (ssc->ssc_disabled) {
...
static TVHCodecProfile* _find_profile(const char* name) {
  if (!strcmp(name, "copy")) {
...
static TVHAudioCodecProfile* _audio_profile(TVHCodecProfile* profile) {
  if (profile && idnode_is_instance(&profile->idnode, (idclass_t*)&codec_profile_audio_class)) {
    return (TVHAudioCodecProfile*)profile;
  }
  return NULL;
}

static int lang_match(const char* lang, tvh_ssc_t* ssc, int* index, int value) {
  if (*index >= 0)
    return 0;
...
static void tvh_transcoder_handle(TVHTranscoder* self, th_pkt_t* pkt) {
...
static tvh_ss_t* tvh_transcoder_start(TVHTranscoder* self, tvh_ss_t* ss_src) {
  tvh_ss_t*             ss;
...
static void tvh_transcoder_stop(TVHTranscoder* self, int flush) {
  TVHStream* stream = NULL;
...
static void tvh_transcoder_stream(void* opaque, tvh_sm_t* msg) {
...
static int
tvh_transcoder_setup(TVHTranscoder* self, const char** profiles, const char** src_codecs) {
  ...
  for (i = 0; i < AVMEDIA_TYPE_NB; i++) {
    if ((profile = profiles[i]) && strlen(profile)) {
      if (!(self->profiles[i] = _find_profile(profile))) {
        tvh_transcoder_log(self, LOG_ERR, "failed to find codec profile: '%s'", profile);
        return -1;
      }
      if (src_codecs[i])
...
static htsmsg_t* tvh_transcoder_info(void* opaque, htsmsg_t* list) {
  TVHTranscoder*      self = opaque;
...
static streaming_ops_t tvh_transcoder_ops = {.st_cb = tvh_transcoder_stream,
