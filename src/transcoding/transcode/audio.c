...
tvh_audio_context_decode(TVHContext *self, AVPacket *avpkt)
{
    ...
    int64_t drift = (prev_pts != AV_NOPTS_VALUE) ? new_pts - prev_pts : INT64_MAX;

    if (drift < 0)
        drift = -drift;

    // Tolerance: 3 AC3 frames at 48 kHz (3 x 1536 = 4608 samples), expressed
    // in the 90 kHz packet timebase: 4608 samples / 48000 Hz * 90000 = 8640.
    // IPTV streams often have slightly irregular packet durations causing
    // sub-frame PTS drift of up to 2 AC3 frames.  Rather than
    ...
    if (prev_pts != AV_NOPTS_VALUE && drift <= 8640) {
