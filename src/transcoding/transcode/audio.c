...
static enum AVSampleFormat _audio_context_sample_fmt(TVHContext* self, AVDictionary** opts) {
  const enum AVSampleFormat* sample_fmts = tvh_codec_profile_audio_get_sample_fmts(self->profile);
  ...
  if (!tvh_context_get_int_opt(opts, "sample_fmt", &ofmt) && ofmt == AV_SAMPLE_FMT_NONE) {
    if (sample_fmts) {
  ...
  if (tvhtrace_enabled())
    tvh_context_log(self,
        LOG_TRACE,
        "audio format selection: old %s, alt %s, in %s",
...
static int _audio_context_sample_rate(TVHContext* self, AVDictionary** opts) {
  const int* sample_rates = tvh_codec_profile_audio_get_sample_rates(self->profile);
  int        irate = self->iavctx->sample_rate, altrate = 0, orate = 0, i;

  if (!tvh_context_get_int_opt(opts, "sample_rate", &orate) && !orate && sample_rates) {
  ...
  if (tvhtrace_enabled())
    tvh_context_log(self,
        LOG_TRACE,
        "audio rate selection: old %i, alt %i, in %i",
        orate,
        altrate,
        irate);
...
static uint64_t _audio_context_channel_layout(TVHContext* self, AVDictionary** opts) {
  const uint64_t* channel_layouts = tvh_codec_profile_audio_get_channel_layouts(self->profile);
  ...
  if (!tvh_context_get_int_opt(opts, "channel_layout", &tmp) && !(olayout = tmp) &&
      channel_layouts) {
  ...
  if (tvhtrace_enabled()) {
    ...
    tvh_context_log(self,
        LOG_TRACE,
        "audio layout selection: old %s, alt %s, in %s",
        obuf,
        abuf,
        ibuf);
  }
...
static int tvh_audio_context_open_encoder(TVHContext* self, AVDictionary** opts) {
  ...
  if (self->oavctx->sample_fmt == AV_SAMPLE_FMT_NONE) {
    tvh_context_log(self, LOG_ERR, "audio encoder has no suitable sample format");
    return -1;
  }
  // sample_rate
  self->oavctx->sample_rate = _audio_context_sample_rate(self, opts);
  if (!self->oavctx->sample_rate) {
    tvh_context_log(self, LOG_ERR, "audio encoder has no suitable sample rate");
    return -1;
  }
  ...
  if (!self->oavctx->channel_layout) {
    tvh_context_log(self, LOG_ERR, "audio encoder has no suitable channel layout");
    return -1;
  }
  self->oavctx->channels = av_get_channel_layout_nb_channels(self->oavctx->channel_layout);
  return 0;
}

static int tvh_audio_context_open_filters(TVHContext* self, AVDictionary** opts) {
  ...
  av_get_channel_layout_string(layout,
      sizeof(layout),
      self->iavctx->channels,
      self->iavctx->channel_layout);
  if (str_snprintf(source_args,
          sizeof(source_args),
  ...
  if (str_snprintf(filters, sizeof(filters), "%s", (resample) ? "aresample" : "anull")) {
    return -1;
  }
  ...
  int ret = tvh_context_open_filters(self,
      ...
      "abuffersink", // sink
      "channel_layouts",
      &self->oavctx->channel_layout, // sink option: channel_layout
      sizeof(self->oavctx->channel_layout),
      "sample_fmts",
      &self->oavctx->sample_fmt, // sink option: sample_fmt
      sizeof(self->oavctx->sample_fmt),
      "sample_rates",
      &self->oavctx->sample_rate, // sink option: sample_rate
...
static int tvh_audio_context_open(TVHContext* self, TVHOpenPhase phase, AVDictionary** opts) {
  switch (phase) {
    case OPEN_ENCODER:
...
static int tvh_audio_context_decode(TVHContext* self, AVPacket* avpkt) {
  ...
  if (prev_pts != (self->pts = new_pts) && prev_pts > 12000) {
    tvh_context_log(self,
        LOG_WARNING,
        "Detected framedrop in audio (%" PRId64 " != %" PRId64 ")",
        prev_pts,
        new_pts);
  }
...
static int tvh_audio_context_encode(TVHContext* self, AVFrame* avframe) {
...
static int tvh_audio_context_ship(TVHContext* self, AVPacket* avpkt) {
  return (avpkt->pts >= 0);
}

static int tvh_audio_context_wrap(TVHContext* self, AVPacket* avpkt, th_pkt_t* pkt) {
