...
static int _video_filters_hw_pix_fmt(enum AVPixelFormat pix_fmt) {
  const AVPixFmtDescriptor* desc;

  if ((desc = av_pix_fmt_desc_get(pix_fmt)) && (desc->flags & AV_PIX_FMT_FLAG_HWACCEL)) {
    return 1;
  }
  return 0;
}

static int _video_filters_get_filters(TVHContext* self, AVDictionary** opts, char** filters) {
  char download[48];
  char deint[8];
  char hw_deint[64];
  char scale[24];
  ...
  if (filter_scale) {
    ...
    if (ihw) {
      ...
      // sw scale
      if (str_snprintf(scale, sizeof(scale), "scale=w=-2:h=%d", self->oavctx->height)) {
        return -1;
      }
    }
  }
  ...
  if (filter_sharpness) {
    ...
    if (ihw) {
      // hw scale
      hwaccels_get_sharpness_filter(self->iavctx,
          filter_sharpness,
          hw_sharpness,
          sizeof(hw_sharpness));
    }
...
static int tvh_video_context_open_decoder(TVHContext* self, AVDictionary** opts) {
#if ENABLE_HWACCELS
  int hwaccel = -1;
...
static int tvh_video_context_notify_gh(TVHContext* self) {
...
static int tvh_video_context_open_encoder(TVHContext* self, AVDictionary** opts) {
  ...
  self->oavctx->ticks_per_frame = (90000 * self->iavctx->framerate.den) /
      self->iavctx->framerate.num; // We assume 90kHz as timebase which is mandatory for MPEG-TS
  ...
  self->oavctx->gop_size =
      ceil(av_q2d(av_inv_q(av_mul_q(self->oavctx->time_base, ticks_per_frame))));
...
static int tvh_video_context_open_filters(TVHContext* self, AVDictionary** opts) {
  ...
  int ret = tvh_context_open_filters(self,
      ...
      "buffersink",                       // sink
      "pix_fmts",
      &self->oavctx->pix_fmt, // sink option: pix_fmt
...
static int tvh_video_context_open(TVHContext* self, TVHOpenPhase phase, AVDictionary** opts) {
  switch (phase) {
    case OPEN_DECODER:
...
static int tvh_video_context_encode(TVHContext* self, AVFrame* avframe) {
  ...
  if (avframe->interlaced_frame) {
    self->oavctx->field_order = avframe->top_field_first ? AV_FIELD_TB : AV_FIELD_BT;
  } else {
...
static int tvh_video_context_ship(TVHContext* self, AVPacket* avpkt) {
...
static int tvh_video_context_wrap(TVHContext* self, AVPacket* avpkt, th_pkt_t* pkt) {
  ...
  switch (pict_type) {
    ...
    case AV_PICTURE_TYPE_NONE:
      break;
    default:
      tvh_context_log(self, LOG_WARNING, "unknown picture type: %d", pict_type);
      break;
  }
...
static void tvh_video_context_close(TVHContext* self) {
#if ENABLE_HWACCELS
