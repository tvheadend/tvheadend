...
static int _video_filters_get_filters(TVHContext* self,
    AVDictionary**                                opts,
    char**                                        filters) {
  char download[48];
  char deint[8];
  ...
  if (filter_scale) {
    ...
    if (ihw) {
      // hw scale
      hwaccels_get_scale_filter(self->iavctx,
          self->oavctx,
          hw_scale,
          sizeof(hw_scale));
    } else {
      // sw scale
      if (str_snprintf(scale,
              sizeof(scale),
              "scale=w=-2:h=%d",
              self->oavctx->height)) {
        return -1;
  ...
  if (filter_denoise) {
    ...
    if (ihw) {
      // hw scale
      hwaccels_get_denoise_filter(self->iavctx,
          filter_denoise,
          hw_denoise,
          sizeof(hw_denoise));
    }
...
static int tvh_video_context_open_decoder(TVHContext* self,
    AVDictionary**                                    opts) {
#if ENABLE_HWACCELS
  int hwaccel = -1;
  ...
  self->iavctx->time_base =
      av_make_q(1, 90000); // MPEG-TS uses a fixed timebase of 90kHz
  return 0;
}
...
static int tvh_video_context_open_encoder(TVHContext* self,
    AVDictionary**                                    opts) {
  ...
  self->oavctx->ticks_per_frame = (90000 * self->iavctx->framerate.den) /
      self->iavctx->framerate
          .num; // We assume 90kHz as timebase which is mandatory for MPEG-TS
  ...
  self->oavctx->gop_size = ceil(
      av_q2d(av_inv_q(av_mul_q(self->oavctx->time_base, ticks_per_frame))));
...
static int tvh_video_context_open_filters(TVHContext* self,
    AVDictionary**                                    opts) {
...
static int tvh_video_context_open(TVHContext* self,
    TVHOpenPhase                              phase,
    AVDictionary**                            opts) {
  switch (phase) {
    case OPEN_DECODER:
