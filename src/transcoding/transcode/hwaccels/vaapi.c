...
static AVBufferRef* tvhva_init(const char* device) {
  TVHVADevice* vad;
  ...
  if (av_hwdevice_ctx_create(&vad->hw_device_ref, AV_HWDEVICE_TYPE_VAAPI, device, NULL, 0)) {
    tvherror(LS_VAAPI, "failed to create a context for device: %s", device);
    return NULL;
  }
...
static void tvhva_context_destroy(TVHVAContext* self) {
  if (self) {
...
static VADisplay* tvhva_context_display(TVHVAContext* self, AVCodecContext* avctx) {
  TVHContext*        ctx           = avctx->opaque;
  AVHWDeviceContext* hw_device_ctx = NULL;

  if (!ctx->hw_device_ref && !(ctx->hw_device_ref = tvhva_init(ctx->hw_accel_device))) {
    return NULL;
  }
...
static VAProfile tvhva_context_profile(TVHVAContext* self, AVCodecContext* avctx) {
  ...
  if (check != VAProfileNone && (profiles_max = vaMaxNumProfiles(self->display)) > 0 &&
    ...
    if (profiles_max == 0) {
      tvherror(LS_VAAPI,
          "%s: vaMaxNumProfiles() returned %d; vaapi doesn't have any profiles available, run: $ "
          "vainfo",
          self->logpref,
          profiles_max);
    }
    va_res = vaQueryConfigProfiles(self->display, profiles, &profiles_len);
    if (va_res == VA_STATUS_SUCCESS) {
      ...
      tvherror(LS_VAAPI,
          "%s: va_res != VA_STATUS_SUCCESS; Failed to query profiles: %d (%s), run: $ vainfo",
          self->logpref,
          va_res,
          vaErrorStr(va_res));
    }
...
static int tvhva_context_check_profile(TVHVAContext* self, VAProfile profile) {
  VAStatus      va_res      = VA_STATUS_ERROR_UNKNOWN;
  ...
  if (entrypoints_max > 0) {
    ...
    if (va_res == VA_STATUS_SUCCESS) {
      ...
      tvherror(LS_VAAPI,
          "%s: va_res != VA_STATUS_SUCCESS; Failed to query entrypoints: %d (%s), run: $ vainfo",
          self->logpref,
          va_res,
          vaErrorStr(va_res));
    }
    ...
    tvherror(LS_VAAPI,
        "%s: vaMaxNumEntrypoints() returned %d; vaapi doesn't have any entrypoints available, run: "
        "$ vainfo",
        self->logpref,
        entrypoints_max);
  }
...
static unsigned int tvhva_get_format(enum AVPixelFormat pix_fmt) {
  switch (pix_fmt) {
...
static int tvhva_context_config(TVHVAContext* self, VAProfile profile, unsigned int format) {
  VAStatus       va_res = VA_STATUS_ERROR_UNKNOWN;
  VAConfigAttrib attrib = {VAConfigAttribRTFormat, VA_ATTRIB_NOT_SUPPORTED};

  // vaCreateConfig
  va_res = vaGetConfigAttributes(self->display, profile, self->entrypoint, &attrib, 1);
  if (va_res != VA_STATUS_SUCCESS) {
    tvherror(LS_VAAPI, "%s: vaGetConfigAttributes: %s", self->logpref, vaErrorStr(va_res));
    return -1;
  }
  ...
  attrib.value = format;
  va_res = vaCreateConfig(self->display, profile, self->entrypoint, &attrib, 1, &self->config_id);
  if (va_res != VA_STATUS_SUCCESS) {
    tvherror(LS_VAAPI, "%s: vaCreateConfig: %s", self->logpref, vaErrorStr(va_res));
    return -1;
  }
  return 0;
}

static int tvhva_context_check_constraints(TVHVAContext* self) {
  ...
  hw_constraints = av_hwdevice_get_hwframe_constraints(self->hw_device_ref, va_config);
  if (!hw_constraints) {
  ...
  if (self->io_format != AV_PIX_FMT_NONE) {
    ...
    if (self->sw_format == AV_PIX_FMT_NONE && (io_desc = av_pix_fmt_desc_get(self->io_format))) {
      for (i = 0; hw_constraints->valid_sw_formats[i] != AV_PIX_FMT_NONE; i++) {
        sw_format = hw_constraints->valid_sw_formats[i];
  ...
  if (self->sw_format == AV_PIX_FMT_NONE) {
    tvherror(LS_VAAPI,
        "%s: VAAPI hardware does not support pixel format: %s",
        self->logpref,
  ...
  if (self->width < hw_constraints->min_width || self->height < hw_constraints->min_height ||
      self->width > hw_constraints->max_width || self->height > hw_constraints->max_height) {
    tvherror(LS_VAAPI,
        "%s: VAAPI hardware does not support image "
        "size %dx%d (constraints: width %d-%d height %d-%d).",
        self->logpref,
        self->width,
        self->height,
        hw_constraints->min_width,
        hw_constraints->max_width,
        hw_constraints->min_height,
        hw_constraints->max_height);
    ret = AVERROR(EINVAL);
  }
...
static int tvhva_context_setup(TVHVAContext* self, AVCodecContext* avctx) {
  ...
  if (profile == VAProfileNone) {
    tvherror(LS_VAAPI,
        "%s: tvhva_context_profile() returned VAProfileNone for %s",
        self->logpref,
        avctx->codec->name);
    return -1;
  }
  ...
  if (tvhva_context_check_profile(self, profile)) {
    tvherror(LS_VAAPI,
        "%s: tvhva_context_check_profile() check failed for codec: %s --> codec not available",
        self->logpref,
        avctx->codec->name);
    return -1;
  }
  ...
  if (tvhva_context_config(self, profile, format) || tvhva_context_check_constraints(self)) {
    return -1;
  }

  if (!(self->hw_frames_ref = av_hwframe_ctx_alloc(self->hw_device_ref))) {
    tvherror(LS_VAAPI, "%s: failed to create VAAPI frame context.", self->logpref);
    return AVERROR(ENOMEM);
  }
  ...
  if (av_hwframe_ctx_init(self->hw_frames_ref) < 0) {
    tvherror(LS_VAAPI, "%s: failed to initialise VAAPI frame context", self->logpref);
    return -1;
  }
  ...
  if (self->entrypoint == VAEntrypointVLD) { // decode only
    ...
    va_res    = vaCreateContext(self->display,
        self->config_id,
        self->width,
        self->height,
        VA_PROGRESSIVE,
        ...
        &self->context_id);
    if (va_res != VA_STATUS_SUCCESS) {
      tvherror(LS_VAAPI, "%s: vaCreateContext: %s", self->logpref, vaErrorStr(va_res));
      return -1;
    }
  }
...
static TVHVAContext*
tvhva_context_create(const char* logpref, AVCodecContext* avctx, VAEntrypoint entrypoint) {
  ...
  pix_fmt          = self->entrypoint == VAEntrypointVLD ? avctx->sw_pix_fmt : avctx->pix_fmt;
  if (pix_fmt == AV_PIX_FMT_YUV420P || pix_fmt == AV_PIX_FMT_VAAPI) {
  ...
  if (self->io_format == AV_PIX_FMT_NONE) {
    tvherror(LS_VAAPI,
        "%s: failed to get pix_fmt for vaapi context "
...
static int vaapi_get_buffer2(AVCodecContext* avctx, AVFrame* avframe, int flags) {
...
int vaapi_decode_setup_context(AVCodecContext* avctx) {
  TVHContext* ctx = avctx->opaque;

  if (!(ctx->hw_accel_ictx = tvhva_context_create("decode", avctx, VAEntrypointVLD))) {
    return -1;
  }
...
void vaapi_decode_close_context(AVCodecContext* avctx) {
  TVHContext* ctx = avctx->opaque;

  tvhva_context_destroy(ctx->hw_accel_ictx);
}

int vaapi_get_scale_filter(AVCodecContext* iavctx,
...
int vaapi_get_deint_filter(AVCodecContext* avctx, char* filter, size_t filter_len) {
  snprintf(filter, filter_len, "deinterlace_vaapi");
  return 0;
}

int vaapi_get_denoise_filter(AVCodecContext* avctx, int value, char* filter, size_t filter_len) {
  snprintf(filter, filter_len, "denoise_vaapi=%d", value);
  return 0;
}

int vaapi_get_sharpness_filter(AVCodecContext* avctx, int value, char* filter, size_t filter_len) {
...
int vaapi_encode_setup_context(AVCodecContext* avctx, int low_power) {
  TVHContext*   ctx             = avctx->opaque;
  ...
  if (!(hwaccel_context = tvhva_context_create("encode", avctx, entrypoint))) {
    return -1;
  }
...
void vaapi_encode_close_context(AVCodecContext* avctx) {
  TVHContext* ctx = avctx->opaque;
