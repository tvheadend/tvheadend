...
static AVBufferRef* tvhva_init(const char* device) {
  ...
  if (av_hwdevice_ctx_create(&vad->hw_device_ref,
...
static VADisplay* tvhva_context_display(TVHVAContext* self,
...
static VAProfile tvhva_context_profile(TVHVAContext* self,
    AVCodecContext*                                  avctx) {
  ...
  if (check != VAProfileNone &&
    ...
    if (profiles_max == 0) {
      tvherror(LS_VAAPI,
          "%s: vaMaxNumProfiles() returned %d; vaapi doesn't have any profiles "
          "available, run: $ vainfo",
          self->logpref,
          profiles_max);
    }
    ...
    if (va_res == VA_STATUS_SUCCESS) {
      ...
      tvherror(LS_VAAPI,
          "%s: va_res != VA_STATUS_SUCCESS; Failed to query profiles: %d (%s), "
          "run: $ vainfo",
          self->logpref,
          va_res,
          vaErrorStr(va_res));
...
static int tvhva_context_check_profile(TVHVAContext* self, VAProfile profile) {
  ...
  if (entrypoints_max > 0) {
    ...
    va_res      = vaQueryConfigEntrypoints(self->display,
        profile,
        entrypoints,
        &entrypoints_len);
    if (va_res == VA_STATUS_SUCCESS) {
      ...
      tvherror(LS_VAAPI,
          "%s: va_res != VA_STATUS_SUCCESS; Failed to query entrypoints: %d "
          "(%s), run: $ vainfo",
          self->logpref,
          va_res,
          vaErrorStr(va_res));
    ...
    if (res != 0) {
      // before giving up we swap VAEntrypointEncSliceLP with
      // VAEntrypointEncSlice or viceversa
    ...
    tvherror(LS_VAAPI,
        "%s: vaMaxNumEntrypoints() returned %d; vaapi doesn't have any "
        "entrypoints available, run: $ vainfo",
        self->logpref,
        entrypoints_max);
  }
  return res;
...
static int tvhva_context_config(TVHVAContext* self,
    VAProfile                                 profile,
    unsigned int                              format) {
  VAStatus       va_res = VA_STATUS_ERROR_UNKNOWN;
  ...
  // vaCreateConfig
  va_res = vaGetConfigAttributes(self->display,
      profile,
      self->entrypoint,
      &attrib,
      1);
  ...
  attrib.value = format;
  va_res       = vaCreateConfig(self->display,
      profile,
      self->entrypoint,
      &attrib,
      1,
...
static int tvhva_context_setup(TVHVAContext* self, AVCodecContext* avctx) {
  ...
  if (tvhva_context_check_profile(self, profile)) {
    tvherror(LS_VAAPI,
        "%s: tvhva_context_check_profile() check failed for codec: %s --> "
        "codec not available",
        self->logpref,
        avctx->codec->name);
    return -1;
  ...
  if (!(self->hw_frames_ref = av_hwframe_ctx_alloc(self->hw_device_ref))) {
    tvherror(LS_VAAPI,
        "%s: failed to create VAAPI frame context.",
        self->logpref);
    return AVERROR(ENOMEM);
  }
  ...
  if (av_hwframe_ctx_init(self->hw_frames_ref) < 0) {
    tvherror(LS_VAAPI,
        "%s: failed to initialise VAAPI frame context",
        self->logpref);
    return -1;
  }
...
static TVHVAContext* tvhva_context_create(const char* logpref,
    AVCodecContext*                                   avctx,
    VAEntrypoint                                      entrypoint) {
  ...
  pix_fmt =
      self->entrypoint == VAEntrypointVLD ? avctx->sw_pix_fmt : avctx->pix_fmt;
...
int vaapi_get_deint_filter(AVCodecContext* avctx,
    char*                                  filter,
    size_t                                 filter_len) {
...
int vaapi_get_denoise_filter(AVCodecContext* avctx,
    int                                      value,
    char*                                    filter,
    size_t                                   filter_len) {
...
int vaapi_get_sharpness_filter(AVCodecContext* avctx,
    int                                        value,
    char*                                      filter,
    size_t                                     filter_len) {
