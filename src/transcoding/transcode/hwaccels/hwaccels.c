...
static inline int check_pix_fmt(AVCodecContext* avctx,
    enum AVPixelFormat                          pix_fmt) {
  return find_hwaccel(avctx->codec_id, pix_fmt) == NULL;
}
#else
static const AVCodecHWConfig* find_hwconfig(const AVCodec* codec,
    enum AVPixelFormat                                     pix_fmt) {
...
static int hwaccels_decode_setup_context(AVCodecContext* avctx,
  ...
  if (check_pix_fmt(avctx, pix_fmt)) {
    ...
    tvherror(LS_TRANSCODE,
        "no HWAccel for the pixel format '%s'",
        desc ? desc->name : "<unk>");
...
enum AVPixelFormat hwaccels_decode_get_format(AVCodecContext* avctx,
  ...
  for (i = 0; pix_fmts[i] != AV_PIX_FMT_NONE; i++) {
    ...
    if ((desc = av_pix_fmt_desc_get(pix_fmt))) {
      tvhtrace(LS_TRANSCODE,
          "hwaccels: [%s] trying pix_fmt: %s",
          avctx->codec->name,
          desc->name);
...
int hwaccels_get_deint_filter(AVCodecContext* avctx,
    char*                                     filter,
    size_t                                    filter_len) {
  TVHContext* ctx = avctx->opaque;
...
int hwaccels_get_denoise_filter(AVCodecContext* avctx,
    int                                         value,
    char*                                       filter,
    size_t                                      filter_len) {
  TVHContext* ctx = avctx->opaque;
