...
tvh_mpeg2video_meta(TVHContext* self, AVPacket* avpkt, th_pkt_t* pkt) {
  ...
  if ((avpkt->flags & AV_PKT_FLAG_KEY)) {
    ...
    // sequence extension
    if (avpkt->size < size + 10 ||
        RB32(data + size) != 0x000001b5) { // EXT_START_CODE
      tvh_context_log(self,
          LOG_ERR,
    ...
    // group of pictures
    if (avpkt->size < size + 8 ||
        RB32(data + size) != 0x000001b8) { // GOP_START_CODE
...
static th_pkt_t* tvh_aac_pack(TVHContext* self, AVPacket* avpkt) {
  ...
  if ((pkt_size = avpkt->size + header_size) > max_size) {
    ...
    if ((pkt = pkt_alloc(self->stream->type,
             NULL,
             pkt_size,
             avpkt->pts,
             avpkt->dts,
