...
#define tvh_context_helper_register(list, helper, kind)                                \
  do {                                                                                 \
    if ((helper)->type <= AVMEDIA_TYPE_UNKNOWN || (helper)->type >= AVMEDIA_TYPE_NB) { \
      tvherror(LS_TRANSCODE, "incomplete/wrong definition for " #helper " helper");    \
      return;                                                                          \
    }                                                                                  \
...
static TVHContextHelper* tvh_context_helper_find(struct TVHContextHelpers* list,
    const AVCodec*                                                         codec) {
...
static int tvh_extradata_open(TVHContext* self, AVDictionary** opts) {
  ...
  if (!(self->iavctx->extradata = av_mallocz(extradata_size + AV_INPUT_BUFFER_PADDING_SIZE))) {
...
static void tvh_decoder_helpers_register() {
...
/* MPEG2VIDEO */
static int tvh_mpeg2video_meta(TVHContext* self, AVPacket* avpkt, th_pkt_t* pkt) {
  ...
  if ((avpkt->flags & AV_PKT_FLAG_KEY)) {
    ...
    if (avpkt->size < size || RB32(data) != 0x000001b3) { // SEQ_START_CODE
      tvh_context_log(self, LOG_ERR, "MPEG2 header: missing sequence header");
      return -1;
    }
    // load intra quantiser matrix
    if (data[size - 1] & 0x02) {
      if (avpkt->size < size + 64) {
        tvh_context_log(self, LOG_ERR, "MPEG2 header: missing load intra quantiser matrix");
        return -1;
      }
      size += 64;
    }
    ...
    if (data[size - 1] & 0x01) {
      if (avpkt->size < size + 64) {
        tvh_context_log(self, LOG_ERR, "MPEG2 header: missing load non-intra quantiser matrix");
        return -1;
      }
      size += 64;
    }
    ...
    if (avpkt->size < size + 10 || RB32(data + size) != 0x000001b5) { // EXT_START_CODE
      tvh_context_log(self, LOG_ERR, "MPEG2 header: missing sequence extension");
      return -1;
    }
    size += 10;
    ...
    if (RB32(data + size) == 0x000001b5) { // EXT_START_CODE
      if (avpkt->size < size + 12) {
        tvh_context_log(self, LOG_ERR, "MPEG2 header: missing sequence display extension");
        return -1;
      }
      size += 12;
    }
    ...
    if (avpkt->size < size + 8 || RB32(data + size) != 0x000001b8) { // GOP_START_CODE
      tvh_context_log(self, LOG_ERR, "MPEG2 header: missing group of pictures");
      return -1;
    }
    size += 8;
...
/* H264 */
static int tvh_h264_meta(TVHContext* self, AVPacket* avpkt, th_pkt_t* pkt) {
  if ((avpkt->flags & AV_PKT_FLAG_KEY) && (avpkt->size > 6) &&
    ...
    for (;;) {
      ...
      if ((nal_type == H264_NAL_SPS && nal_len >= 4 && nal_len <= UINT16_MAX) ||
          (nal_type == H264_NAL_PPS && nal_len <= UINT16_MAX)) {
        size      = nal_end - p;
        nal_start = nal_end;
        continue;
      }
...
/* HEVC */
static int tvh_hevc_meta(TVHContext* self, AVPacket* avpkt, th_pkt_t* pkt) {

  if ((avpkt->flags & AV_PKT_FLAG_KEY) && (avpkt->size >= 6) && (*avpkt->data != 1) &&
...
/* AAC */
static void tvh_aac_pack_adts_header(TVHContext* self, pktbuf_t* pb) {
...
static th_pkt_t* tvh_aac_pack(TVHContext* self, AVPacket* avpkt) {
  ...
  if ((pkt_size = avpkt->size + header_size) > max_size) {
    ...
    if ((pkt = pkt_alloc(self->stream->type, NULL, pkt_size, avpkt->pts, avpkt->dts, 0))) {
      ...
      memcpy(pktbuf_ptr(pkt->pkt_payload) + header_size, avpkt->data, avpkt->size);
    }
  } else {
...
TVHContextHelper* tvh_decoder_helper_find(const AVCodec* codec) {
  return tvh_context_helper_find(&tvh_decoder_helpers, codec);
}

TVHContextHelper* tvh_encoder_helper_find(const AVCodec* codec) {
  return tvh_context_helper_find(&tvh_encoder_helpers, codec);
}

void tvh_context_helpers_register() {
  tvh_decoder_helpers_register();
  tvh_encoder_helpers_register();
}

void tvh_context_helpers_forget() {
