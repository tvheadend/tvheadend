...
typedef enum { OPEN_DECODER, OPEN_DECODER_POST, OPEN_ENCODER, OPEN_ENCODER_POST } TVHOpenPhase;
...
int tvh_transcoder_deliver(TVHTranscoder* self, th_pkt_t* pkt);

TVHTranscoder*
tvh_transcoder_create(tvh_st_t* output, const char** profiles, const char** src_codecs);

void tvh_transcoder_destroy(TVHTranscoder* self);
...
void tvh_stream_stop(TVHStream* self, int flush);

int tvh_stream_handle(TVHStream* self, th_pkt_t* pkt);

int tvh_stream_deliver(TVHStream* self, th_pkt_t* pkt);

TVHStream* tvh_stream_create(TVHTranscoder* transcoder,
    TVHCodecProfile*                        profile,
    tvh_ssc_t*                              ssc,
    const char*                             src_codecs);

void tvh_stream_destroy(TVHStream* self);
...
void tvh_context_types_register(void);

void tvh_context_types_forget(void);
...
TVHContextHelper* tvh_decoder_helper_find(const AVCodec* avcodec);

TVHContextHelper* tvh_encoder_helper_find(const AVCodec* avcodec);

void tvh_context_helpers_register(void);

void tvh_context_helpers_forget(void);
