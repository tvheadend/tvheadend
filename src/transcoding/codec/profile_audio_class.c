...
tvh_codec_profile_audio_open(TVHAudioCodecProfile *self, AVDictionary **opts)
{
    ...
    if (self->channel_layout) {
#if LIBAVCODEC_VERSION_MAJOR > 59
        AV_DICT_SET_INT(LST_AUDIO, opts, "ch_layout_u_mask", (int) self->channel_layout, AV_DICT_DONT_OVERWRITE);
#else
        AV_DICT_SET_INT(LST_NONE, opts, "channel_layout", (int) self->channel_layout, AV_DICT_DONT_OVERWRITE);
#endif
    }
    return 0;
}
