...
static htsmsg_t* tvh_codec_audio_get_list_sample_fmts(TVHAudioCodec* self) {
  ...
  if (sample_fmts && (list = htsmsg_create_list())) {
    if (!(map = htsmsg_create_map())) {
      ...
      for (i = 0; (f = sample_fmts[i]) != AV_SAMPLE_FMT_NONE; i++) {
        if (!(f_str = av_get_sample_fmt_name(f)) || !(map = htsmsg_create_map())) {
          htsmsg_destroy(list);
          list = NULL;
          break;
        }
...
static htsmsg_t* tvh_codec_audio_get_list_sample_rates(TVHAudioCodec* self) {
  htsmsg_t * list = NULL, *map = NULL;
...
static htsmsg_t* tvh_codec_audio_get_list_channel_layouts(TVHAudioCodec* self) {
  htsmsg_t *      list = NULL, *map = NULL;
...
static int tvh_codec_profile_audio_is_copy(TVHAudioCodecProfile* self, tvh_ssc_t* ssc) {
  // TODO: fix me
...
static int tvh_codec_profile_audio_open(TVHAudioCodecProfile* self, AVDictionary** opts) {
  if (self->sample_fmt != AV_SAMPLE_FMT_NONE) {
    AV_DICT_SET_INT(opts, "sample_fmt", self->sample_fmt, AV_DICT_DONT_OVERWRITE);
  }
  if (self->sample_rate) {
    AV_DICT_SET_INT(opts, "sample_rate", self->sample_rate, AV_DICT_DONT_OVERWRITE);
  }
  if (self->channel_layout) {
...
static htsmsg_t* codec_profile_audio_class_language_list(void* obj, const char* lang) {
...
static uint32_t codec_profile_audio_class_sample_fmt_get_opts(void* obj, uint32_t opts) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
  return tvh_codec_audio_get_opts(codec, sample_fmts, opts);
}

static htsmsg_t* codec_profile_audio_class_sample_fmt_list(void* obj, const char* lang) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
...
static uint32_t codec_profile_audio_class_sample_rate_get_opts(void* obj, uint32_t opts) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
  return tvh_codec_audio_get_opts(codec, sample_rates, opts);
}

static htsmsg_t* codec_profile_audio_class_sample_rate_list(void* obj, const char* lang) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
...
static uint32_t codec_profile_audio_class_channel_layout_get_opts(void* obj, uint32_t opts) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
  return tvh_codec_audio_get_opts(codec, channel_layouts, opts);
}

static htsmsg_t* codec_profile_audio_class_channel_layout_list(void* obj, const char* lang) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
