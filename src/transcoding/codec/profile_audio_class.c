...
static int tvh_codec_profile_audio_is_copy(TVHAudioCodecProfile* self,
    tvh_ssc_t*                                                   ssc) {
  // TODO: fix me
  ...
  if ((self->channel_layout &&
          ssc_channels !=
              av_get_channel_layout_nb_channels(self->channel_layout)) ||
...
static int tvh_codec_profile_audio_open(TVHAudioCodecProfile* self,
    AVDictionary**                                            opts) {
...
static htsmsg_t* codec_profile_audio_class_language_list(void* obj,
    const char*                                                lang) {
  ...
  while (lc->code2b) {
    ...
    if (!strcmp(lc->code2b, "und")) {
      map =
          htsmsg_create_key_val("", tvh_gettext_lang(lang, N_("Use original")));
    } else {
      memset(lc_buf, 0, sizeof(lc_buf));
      if (!str_snprintf(lc_buf,
              sizeof(lc_buf),
              "%s (%s)",
              lc->desc,
              lc->code2b)) {
...
static uint32_t codec_profile_audio_class_sample_fmt_get_opts(void* obj,
    uint32_t                                                        opts) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
  return tvh_codec_audio_get_opts(codec, sample_fmts, opts);
}

static htsmsg_t* codec_profile_audio_class_sample_fmt_list(void* obj,
    const char*                                                  lang) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
...
static uint32_t codec_profile_audio_class_sample_rate_get_opts(void* obj,
    uint32_t                                                         opts) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
  return tvh_codec_audio_get_opts(codec, sample_rates, opts);
}

static htsmsg_t* codec_profile_audio_class_sample_rate_list(void* obj,
    const char*                                                   lang) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
...
static uint32_t codec_profile_audio_class_channel_layout_get_opts(void* obj,
    uint32_t                                                            opts) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
  return tvh_codec_audio_get_opts(codec, channel_layouts, opts);
}

static htsmsg_t* codec_profile_audio_class_channel_layout_list(void* obj,
    const char*                                                      lang) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
            ...
            (const property_t[]){
                {
                    ...
                    .desc = N_(
                        "Use only defined number of audio tracks at maximum."),
                    .group = 2,
                ...
                },
                {
                    ...
                    .opts  = PO_ADVANCED | PO_PHIDDEN,
                    .get_opts =
                        codec_profile_audio_class_channel_layout_get_opts,
