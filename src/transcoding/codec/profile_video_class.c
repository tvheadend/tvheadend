...
static int tvh_codec_profile_video_setup(TVHVideoCodecProfile* self,
    tvh_ssc_t*                                                 ssc) {
  ...
  switch (self->scaling_mode) {
    case 0:
      ...
      if (self->height) {
        ...
        self->size.num = (int)((double)self->size.den *
            ((double)ssc->es_width / (double)ssc->es_height));
...
static int tvh_codec_profile_video_is_copy(TVHVideoCodecProfile* self,
    tvh_ssc_t*                                                   ssc) {
  return (!self->deinterlace && (self->size.den == ssc->es_height));
}

static int tvh_codec_profile_video_open(TVHVideoCodecProfile* self,
    AVDictionary**                                            opts) {
...
static int codec_profile_video_class_deinterlace_set(void* obj,
    const void*                                            val) {
...
static uint32_t codec_profile_video_class_pix_fmt_get_opts(void* obj,
    uint32_t                                                     opts) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
  return tvh_codec_video_get_opts(codec, pix_fmts, opts);
}

static htsmsg_t* codec_profile_video_class_pix_fmt_list(void* obj,
    const char*                                               lang) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
    ...
    {.ic_super         = (idclass_t*)&codec_profile_class,
        ...
        .ic_properties = (const property_t
                    ...
                    },
            {
                ...
                .desc = N_(
                    "Height of the output video stream. Horizontal resolution "
            ...
            },
            {
                ...
                .desc =
                    N_("Use hardware acceleration for decoding if available."),
                .group = 2,
