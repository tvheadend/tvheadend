...
{
    AV_DICT_SET_INT(LS_TRANSCODE, opts, "tvh_filter_deint", self->deinterlace, 0);
    // video_size
    AV_DICT_SET_INT(LS_TRANSCODE, opts, "width", self->size.num, 0);
    AV_DICT_SET_INT(LS_TRANSCODE, opts, "height", self->size.den, 0);
    // crf
    if (self->crf) {
        AV_DICT_SET_INT(LS_TRANSCODE, opts, "crf", self->crf, AV_DICT_DONT_OVERWRITE);
    }
    // pix_fmt
    AV_DICT_SET_PIX_FMT(LS_TRANSCODE, opts, self->pix_fmt, AV_PIX_FMT_YUV420P);
    return 0;
}
