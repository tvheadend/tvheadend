...
static htsmsg_t* scaling_mode_get_list(void* o, const char* lang) {
...
static htsmsg_t* tvh_codec_video_get_list_pix_fmts(TVHVideoCodec* self) {
  ...
  if (pix_fmts && (list = htsmsg_create_list())) {
    if (!(map = htsmsg_create_map())) {
      ...
      for (i = 0; (f = pix_fmts[i]) != AV_PIX_FMT_NONE; i++) {
        if (!(f_str = av_get_pix_fmt_name(f)) || !(map = htsmsg_create_map())) {
          htsmsg_destroy(list);
          list = NULL;
          break;
        }
...
static int tvh_codec_profile_video_setup(TVHVideoCodecProfile* self, tvh_ssc_t* ssc) {
  ...
  switch (self->scaling_mode) {
    case 0:
      ...
      if (self->height) {
        ...
        self->size.num =
            (int)((double)self->size.den * ((double)ssc->es_width / (double)ssc->es_height));
...
static int tvh_codec_profile_video_is_copy(TVHVideoCodecProfile* self, tvh_ssc_t* ssc) {
  return (!self->deinterlace && (self->size.den == ssc->es_height));
}

static int tvh_codec_profile_video_open(TVHVideoCodecProfile* self, AVDictionary** opts) {
...
static int codec_profile_video_class_deinterlace_set(void* obj, const void* val) {
  TVHVideoCodecProfile* self    = (TVHVideoCodecProfile*)obj;
  const AVCodec*        avcodec = NULL;

  if (self && (avcodec = tvh_codec_profile_get_avcodec((TVHCodecProfile*)self))) {
...
static uint32_t codec_profile_video_class_pix_fmt_get_opts(void* obj, uint32_t opts) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
  return tvh_codec_video_get_opts(codec, pix_fmts, opts);
}

static htsmsg_t* codec_profile_video_class_pix_fmt_list(void* obj, const char* lang) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
