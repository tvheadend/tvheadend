...
static int tvh_codec_profile_base_is_copy(TVHCodecProfile* self, tvh_ssc_t* ssc) {
  return (!(self->bit_rate || self->qscale));
}

static int tvh_codec_profile_base_open(TVHCodecProfile* self, AVDictionary** opts) {
...
static htsmsg_t* codec_profile_class_save(idnode_t* idnode, char* filename, size_t fsize) {
...
static void codec_profile_class_delete(idnode_t* idnode) {
  tvh_codec_profile_remove((TVHCodecProfile*)idnode, 1);
}

/* codec_profile_class.name */

static int codec_profile_class_name_set(void* obj, const void* val) {
  TVHCodecProfile *self = (TVHCodecProfile*)obj, *other = NULL;
  const char*      name = (const char*)val;

  if (self) {
    if (name && name[0] != '\0' && strcmp(name, "copy") && strlen(name) < TVH_NAME_LEN &&
...
static const void* codec_profile_class_name_get(void* obj) {
...
static const void* codec_profile_class_type_get(void* obj) {
  static const char* type;
...
static const void* codec_profile_class_enabled_get(void* obj) {
  static int enabled;
...
static htsmsg_t* codec_profile_class_profile_list(void* obj, const char* lang) {
  TVHCodec* codec = tvh_codec_profile_get_codec(obj);
