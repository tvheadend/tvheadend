...
static TVHCodecProfile* tvh_codec_profile_alloc(TVHCodec* codec, htsmsg_t* conf) {
...
static void tvh_codec_profile_free(TVHCodecProfile* self) {
  TVHCodec* codec;
...
static void tvh_codec_profile_load(htsmsg_field_t* config) {
  htsmsg_t*   conf = NULL;
  ...
  if ((conf = htsmsg_field_get_map(config)) &&
    ...
    tvherror(LS_CODEC,
        "unable to load codec profile: '%s'",
        (name = htsmsg_get_str(conf, "name")) ? name : "<unknown>");
  }
}

static void tvh_codec_profile_create2(htsmsg_field_t* config) {
  htsmsg_t*   conf = NULL;
  const char* name = NULL;
  ...
  if ((conf = htsmsg_field_get_map(config)) != NULL) {
    ...
    if (tvh_codec_profile_create(conf, NULL, 1))
      tvherror(LS_CODEC,
          "unable to create codec profile from config tree: '%s'",
          (name = htsmsg_get_str(conf, "name")) ? name : "<unknown>");
  }
}

static int tvh_codec_profile_setup(TVHCodecProfile* self, tvh_ssc_t* ssc) {
  ...
  while (idclass) {
    ...
    if (codec_profile_class->setup && (ret = codec_profile_class->setup(self, ssc))) {
      break;
    }
...
int tvh_codec_profile_create(htsmsg_t* conf, const char* uuid, int save) {
  ...
  if ((!(profile_name = htsmsg_get_str(conf, "name")) || profile_name[0] == '\0' ||
          !strcmp(profile_name, "copy") || strlen(profile_name) >= TVH_NAME_LEN) ||
      (!(codec_name = htsmsg_get_str(conf, "codec_name")) || codec_name[0] == '\0')) {
...
const char* tvh_codec_profile_get_status(TVHCodecProfile* self) {
...
const char* tvh_codec_profile_get_name(TVHCodecProfile* self) {
  return self->name;
}

const char* tvh_codec_profile_get_title(TVHCodecProfile* self) {
  static __thread char profile_title[TVH_TITLE_LEN];

  memset(profile_title, 0, sizeof(profile_title));
  if (str_snprintf(profile_title,
...
const AVCodec* tvh_codec_profile_get_avcodec(TVHCodecProfile* self) {
  TVHCodec* codec = tvh_codec_profile_get_codec(self);
  return codec ? tvh_codec_get_codec(codec) : NULL;
}

/* transcode api */
int tvh_codec_profile_is_copy(TVHCodecProfile* self, tvh_ssc_t* ssc) {
  ...
  if (out_type == ssc->es_type) {
    ...
    while (idclass) {
      codec_profile_class = (codec_profile_class_t*)idclass;
      if (codec_profile_class->is_copy && !codec_profile_class->is_copy(self, ssc)) {
        return 0;
      }
      idclass = idclass->ic_super;
    }
...
int tvh_codec_profile_open(TVHCodecProfile* self, AVDictionary** opts) {
  const idclass_t*             idclass             = (&self->idnode)->in_class;
  const codec_profile_class_t* codec_profile_class = NULL;
  int                          ret                 = 0;
  ...
  while (idclass) {
    codec_profile_class = (codec_profile_class_t*)idclass;
    if (codec_profile_class->open && (ret = codec_profile_class->open(self, opts))) {
      break;
    }
    idclass = idclass->ic_super;
  }
...
/* video */
int tvh_codec_profile_video_init(TVHCodecProfile* _self, htsmsg_t* conf) {
  TVHVideoCodecProfile* self = (TVHVideoCodecProfile*)_self;
  self->pix_fmt              = AV_PIX_FMT_NONE;
  return 0;
}

void tvh_codec_profile_video_destroy(TVHCodecProfile* _self) {}

int tvh_codec_profile_video_get_hwaccel(TVHCodecProfile* self) {
  TVHCodec* codec = tvh_codec_profile_get_codec(self);
  if (codec && tvh_codec_is_enabled(codec) && tvh_codec_get_type(codec) == AVMEDIA_TYPE_VIDEO) {
    return ((TVHVideoCodecProfile*)self)->hwaccel;
  }
  return -1;
}

const enum AVPixelFormat* tvh_codec_profile_video_get_pix_fmts(TVHCodecProfile* self) {
  TVHCodec* codec = tvh_codec_profile_get_codec(self);
  return tvh_codec_video_getattr(codec, pix_fmts);
}

/* audio */
int tvh_codec_profile_audio_init(TVHCodecProfile* _self, htsmsg_t* conf) {
...
void tvh_codec_profile_audio_destroy(TVHCodecProfile* _self) {
  TVHAudioCodecProfile* self = (TVHAudioCodecProfile*)_self;
  free(self->language1);
  free(self->language2);
  free(self->language3);
...
const int* tvh_codec_profile_audio_get_sample_rates(TVHCodecProfile* self) {
  TVHCodec* codec = tvh_codec_profile_get_codec(self);
  return tvh_codec_audio_getattr(codec, sample_rates);
}

const uint64_t* tvh_codec_profile_audio_get_channel_layouts(TVHCodecProfile* self) {
  TVHCodec* codec = tvh_codec_profile_get_codec(self);
  return tvh_codec_audio_getattr(codec, channel_layouts);
}

/* internal api */
void tvh_codec_profile_remove(TVHCodecProfile* self, int delete) {
...
TVHCodec* tvh_codec_profile_get_codec(TVHCodecProfile* self) {
  return self ? self->codec : NULL;
}

TVHCodecProfile* tvh_codec_profile_find(const char* name) {
  TVHCodecProfile* profile = NULL;
...
void tvh_codec_profiles_remove() {
