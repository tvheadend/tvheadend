...
static TVHCodecProfile* tvh_codec_profile_alloc(TVHCodec* codec,
    htsmsg_t*                                             conf) {
...
const char* tvh_codec_profile_get_title(TVHCodecProfile* self) {
  ...
  if (str_snprintf(profile_title,
          ...
          (self->description && strcmp(self->description, "")) ? "%s (%s)"
                                                               : "%s%s",
          self->name,
...
int tvh_codec_profile_is_copy(TVHCodecProfile* self, tvh_ssc_t* ssc) {
  ...
  if ((out_type = codec_id2streaming_component_type(avcodec->id)) ==
      SCT_UNKNOWN) {
...
const enum AVPixelFormat* tvh_codec_profile_video_get_pix_fmts(
    TVHCodecProfile* self) {
  TVHCodec* codec = tvh_codec_profile_get_codec(self);
...
const enum AVSampleFormat* tvh_codec_profile_audio_get_sample_fmts(
    TVHCodecProfile* self) {
  TVHCodec* codec = tvh_codec_profile_get_codec(self);
...
const uint64_t* tvh_codec_profile_audio_get_channel_layouts(
    TVHCodecProfile* self) {
  TVHCodec* codec = tvh_codec_profile_get_codec(self);
