...
static enum AVMediaType codec_get_type(const AVCodec* self) {
  return self->type;
}

static const char* codec_get_type_string(const AVCodec* self) {
...
static void tvh_codec_video_init(TVHVideoCodec* self, const AVCodec* codec) {
  if (!self->pix_fmts) {
    self->pix_fmts = codec->pix_fmts;
  }
}

static void tvh_codec_audio_init(TVHAudioCodec* self, const AVCodec* codec) {
  static int default_sample_rates[] = {44100, 48000, 96000, 192000, 0};
...
static void tvh_codec_init(TVHCodec* self, const AVCodec* codec) {
  if (!self->profiles) {
...
static void tvh_codec_register(TVHCodec* self) {
  static const size_t min_size = sizeof(TVHCodecProfile);
  const AVCodec*      codec    = NULL;

  if (tvh_str_default(self->name, NULL) == NULL || self->size < min_size || !self->idclass) {
    tvherror(LS_CODEC,
        "incomplete/wrong definition for '%s' codec",
