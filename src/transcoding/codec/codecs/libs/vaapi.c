...
static htsmsg_t* platform_get_list(void* o, const char* lang) {
...
static htsmsg_t* rc_mode_get_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static htsmsg_t* h264_level_get_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static htsmsg_t* hevc_tier_get_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static htsmsg_t* hevc_level_get_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static int probe_vaapi_device(const char* device, char* name, size_t namelen) {
  drm_version_t dv;
  char          dname[128];
  int           fd;
...
static htsmsg_t* tvh_codec_profile_vaapi_device_list(void* obj,
    const char*                                            lang) {
...
static int tvh_codec_profile_vaapi_open(tvh_codec_profile_vaapi_t* self,
    AVDictionary**                                                 opts) {
  // pix_fmt
    ...
    {.ic_super         = (idclass_t*)&codec_profile_video_class,
        ...
        .ic_properties = (const property_t
            ...
            },
            {
                ...
                .name = N_("Low Power"),
                .desc = N_(
                    "Set low power mode.[if disabled will not send parameter "
                    "to libav, if enabled codec will disable B frames]"),
                .group    = 3,
                .opts     = PO_EXPERT,
                .get_opts = codec_profile_class_get_opts,
            ...
            },
            {
                ...
                .desc = N_(
                    "Set maximum process in parallel (0=skip, "
                    "2=default).[driver must implement vaSyncBuffer function]"),
                .group    = 3,
                .get_opts = codec_profile_class_get_opts,
            ...
            },
            {
                ...
                .desc     = N_("Maximum B-frame reference depth (from 1 to 3) "
                           "(default 1)"),
                .group    = 3,
                .get_opts = codec_profile_class_get_opts,
            ...
            },
            {
                ...
                .name  = N_("Constant QP"),
                .group = 3,
                .desc  = N_("Fixed QP of P frames (from 0 to 52, 0=skip).[if "
                           "disabled will not send paramter to libav]"),
                .get_opts = codec_profile_class_get_opts,
            ...
            },
            {
                ...
                .desc = N_("Target bitrate (0=skip).[if disabled will not send "
                           "paramter to libav]"),
                .group    = 3,
                .get_opts = codec_profile_class_get_opts,
            ...
            },
            {
                ...
                .desc     = N_("Maximum bitrate (0=skip).[if disabled will not "
                           "send paramter to libav]"),
                .group    = 3,
                .get_opts = codec_profile_class_get_opts,
            ...
            },
            {
                ...
                .desc     = N_("Size of transcoding buffer "
                           "(buffer=bitrate*2048*factor). Good factor is 3."),
                .group    = 3,
                .get_opts = codec_profile_class_get_opts,
            ...
            },
            {
                ...
                .desc = N_("Bitrate & Max bitrate scaler with resolution (0=no "
                           "scale; 1=proportional_change). Relative to 480."),
                .group    = 3,
                .get_opts = codec_profile_class_get_opts,
                .off =
                    offsetof(tvh_codec_profile_vaapi_t, bit_rate_scale_factor),
                .def.d = 0,
            },
            {
                .type  = PT_INT,
                ...
                .name  = N_("Denoise"),
                .group = 2,
                .desc  = N_("Denoise only available with Hardware Acceleration "
                           "(from 0 to 64, 0=skip, 0 default)"),
                .get_opts = codec_profile_class_get_opts,
            ...
            },
            {
                ...
                .name     = N_("Sharpness"),
                .group    = 2,
                .desc     = N_("Sharpness only available with Hardware "
                           "Acceleration (from 0 to 64, 0=skip, 44 default)"),
                .get_opts = codec_profile_class_get_opts,
...
static int tvh_codec_profile_vaapi_h264_open(tvh_codec_profile_vaapi_t* self,
    AVDictionary**                                                      opts) {
  // to avoid issues we have this check:
  if (self->buff_factor <= 0) {
    self->buff_factor = 3;
  }
  int int_bitrate = (int)((double)(self->bit_rate) * 1024.0 *
      (1.0 +
          (self->bit_rate_scale_factor * ((double)(self->size.den) - 480.0) /
              480.0)));
  int int_buffer_size =
      (int)((double)(self->bit_rate) * 2048.0 * self->buff_factor *
          (1.0 +
              self->bit_rate_scale_factor * ((double)(self->size.den) - 480.0) /
                  480));
  int int_max_bitrate = (int)((double)(self->max_bit_rate) * 1024.0 *
      (1.0 +
          (self->bit_rate_scale_factor * ((double)(self->size.den) - 480.0) /
              480.0)));
  tvhinfo(LS_VAAPI,
      "Bitrate = %d kbps; Buffer size = %d kbps; Max bitrate = %d kbps",
      int_bitrate / 1024,
      int_buffer_size / 1024,
      int_max_bitrate / 1024);
  ...
  // -low_power         <boolean>    E..V....... Use low-power encoding mode
  // (only available on some platforms; may not support all encoding features)
  // (default false) -idr_interval      <int>        E..V....... Distance (in
  // I-frames) between IDR frames (from 0 to INT_MAX) (default 0) -b_depth <int>
  // E..V....... Maximum B-frame reference depth (from 1 to INT_MAX) (default 1)
  // -rc_mode           <int>        E..V....... Set rate control mode (from 0
  // to 6) (default auto)
  //    auto            0            E..V....... Choose mode automatically based
  //    on other parameters CQP             1            E..V.......
  //    Constant-quality CBR             2            E..V.......
  //    Constant-bitrate VBR             3            E..V.......
  //    Variable-bitrate ICQ             4            E..V....... Intelligent
  //    constant-quality QVBR            5            E..V.......
  //    Quality-defined variable-bitrate AVBR            6 E..V....... Average
  //    variable-bitrate
  // -qp                <int>        E..V....... Constant QP (for P-frames;
  // scaled by qfactor/qoffset for I/B) (from 0 to 52) (default 0) -quality
  // <int>        E..V....... Set encode quality (trades off against speed,
  // higher is faster) (from -1 to INT_MAX) (default -1) -coder <int>
  // E..V....... Entropy coder type (from 0 to 1) (default cabac)
  //    cavlc           0            E..V.......
  //    cabac           1            E..V.......
  //    vlc             0            E..V.......
  //    ac              1            E..V.......
  // -aud               <boolean>    E..V....... Include AUD (default false)
  // -sei               <flags>      E..V....... Set SEI to include (default
  // identifier+timing+recovery_point)
  //    identifier                   E..V....... Include encoder version
  //    identifier timing                       E..V....... Include timing
  //    parameters (buffering_period and pic_timing) recovery_point E..V.......
  //    Include recovery points where appropriate
  // -profile           <int>        E..V....... Set profile (profile_idc and
  // constraint_set*_flag) (from -99 to 65535) (default -99)
  //    constrained_baseline 578          E..V.......
  //    main            77           E..V.......
  //    high            100          E..V.......
  // -level             <int>        E..V....... Set level (level_idc) (from -99
  // to 255) (default -99)
  //    1               10           E..V.......
  //    1.1             11           E..V.......
  //    1.2             12           E..V.......
  ...
  switch (self->platform) {
    case 0:
      // Uncontrained --> will allow any combination of parameters (valid or
      // invalid) this mode is usefull fur future platform and for debugging.
      if ((self->low_power == 0) && (self->desired_b_depth)) {
        AV_DICT_SET_INT(opts,
            "b_depth",
            self->desired_b_depth,
    ...
    case 2:
      ...
      // Is only going to override bf to 0 (as highlited by the previous
      // implementation)
      if (self->bit_rate) {
        AV_DICT_SET_INT(opts, "b", int_bitrate, AV_DICT_DONT_OVERWRITE);
        AV_DICT_SET_INT(opts,
...
static int tvh_codec_profile_vaapi_hevc_open(tvh_codec_profile_vaapi_t* self,
    AVDictionary**                                                      opts) {
  // to avoid issues we have this check:
  if (self->buff_factor <= 0) {
    self->buff_factor = 3;
  ...
  //    variable-bitrate
  // -qp                <int>        E..V....... Constant QP (for P-frames;
  // scaled by qfactor/qoffset for I/B) (from 0 to 52) (default 0) -aud
  // <boolean>    E..V....... Include AUD (default false) -profile <int>
  // E..V....... Set profile (general_profile_idc) (from -99 to 255) (default
  // -99)
  //    main            1            E..V.......
  //    main10          2            E..V.......
  //    rext            4            E..V.......
  ...
  //    main            0            E..V.......
  //    high            1            E..V.......
  // -level             <int>        E..V....... Set level (general_level_idc)
  ...
  // -sei               <flags>      E..V....... Set SEI to include (default
  // hdr)
  //    hdr                          E..V....... Include HDR metadata for
  //    mastering display colour volume and content light level information
  ...
  switch (self->platform) {
    case 0:
      // Unconstrained --> will allow any combination of parameters (valid or
      // invalid) this mode is usefull fur future platform and for debugging.
      if ((self->low_power == 0) && (self->desired_b_depth)) {
        AV_DICT_SET_INT(opts,
            "b_depth",
            ...
            (const property_t[]){
                {
                    ...
                    .name     = N_("Tier"),
                    .desc     = N_("Set tier (general_tier_flag) (from 0 to 1) "
                               "(default main)"),
                    .group    = 5,
                    .opts     = PO_EXPERT,
                    .get_opts = codec_profile_class_get_opts,
...
static int tvh_codec_profile_vaapi_vp8_open(tvh_codec_profile_vaapi_t* self,
    AVDictionary**                                                     opts) {
  // to avoid issues we have this check:
  if (self->buff_factor <= 0) {
    self->buff_factor = 3;
  ...
  //    Quality-defined variable-bitrate AVBR            6 E..V....... Average
  //    variable-bitrate
  // -loop_filter_level <int>        E..V....... Loop filter level (from 0 to
  // 63) (default 16) -loop_filter_sharpness <int>        E..V....... Loop
  // filter sharpness (from 0 to 15) (default 4)
        ...
        .ic_properties =
            (const property_t[]){
                ...
                },
                {
                    ...
                    .desc  = N_("Set Loop filter level (-1=skip from 0 to 63) "
                               "[default 16]"),
                    .group = 5,
                    .opts  = PO_EXPERT,
                    .get_opts = codec_profile_class_get_opts,
                    .off =
                        offsetof(tvh_codec_profile_vaapi_t, loop_filter_level),
                ...
                },
                {
                    ...
                    .desc  = N_("Set Loop filter sharpness (-1=skip from 0 to "
                               "15) [default 4]"),
                    .group = 5,
                    .opts  = PO_EXPERT,
                    .get_opts = codec_profile_class_get_opts,
                    .off      = offsetof(tvh_codec_profile_vaapi_t,
                        loop_filter_sharpness),
...
static int tvh_codec_profile_vaapi_vp9_open(tvh_codec_profile_vaapi_t* self,
    AVDictionary**                                                     opts) {
  // to avoid issues we have this check:
  if (self->buff_factor <= 0) {
    self->buff_factor = 3;
  ...
  switch (self->platform) {
    case 0:
      ...
      if (self->super_frame) {
        ...
        AV_DICT_SET(opts,
            "bsf",
            "vp9_raw_reorder,vp9_superframe",
            AV_DICT_DONT_OVERWRITE);
      }
      if (self->bit_rate) {
        AV_DICT_SET_INT(opts, "b", int_bitrate, AV_DICT_DONT_OVERWRITE);
    ...
    {.ic_super         = (idclass_t*)&codec_profile_vaapi_class,
        ...
        .ic_properties = (const property_t
            ...
            },
            {
                ...
                .id    = "loop_filter_sharpness", // Don't change
                .name  = N_("Loop filter sharpness"),
                .desc  = N_("Set Loop filter sharpness (-1=skip from 0 to 15) "
                           "[default 4]"),
                .group = 5,
                .opts  = PO_EXPERT,
                .get_opts = codec_profile_class_get_opts,
                .off =
                    offsetof(tvh_codec_profile_vaapi_t, loop_filter_sharpness),
                .intextra = INTEXTRA_RANGE(-1, 15, 1),
                .def.i    = 4,
            },
            {
                ...
                .name = N_("Super Frame"),
                .desc =
                    N_("Enable Super Frame for vp9 encoder.[default disabled]"),
                .group    = 5,
                .opts     = PO_EXPERT,
                .get_opts = codec_profile_class_get_opts,
