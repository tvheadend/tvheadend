...
static htsmsg_t* tvh_codec_profile_vaapi_device_list(void* obj, const char* lang) {
...
static int tvh_codec_profile_vaapi_open(tvh_codec_profile_vaapi_t* self, AVDictionary** opts) {
  // pix_fmt
        ...
        .ic_properties =
            (const property_t[]){{
                ...
                },
                {
                    ...
                    .name  = N_("Low Power"),
                    .desc  = N_("Set low power mode.[if disabled will not send parameter to libav, "
                                "if enabled codec will disable B frames]"),
                    .group = 3,
                    .opts  = PO_EXPERT,
                    .get_opts = codec_profile_class_get_opts,
                ...
                },
                {
                    ...
                    .desc  = N_("Set maximum process in parallel (0=skip, 2=default).[driver must "
                                "implement vaSyncBuffer function]"),
                    .group = 3,
                    .get_opts = codec_profile_class_get_opts,
                ...
                },
                {
                    ...
                    .name  = N_("Constant QP"),
                    .group = 3,
                    .desc  = N_("Fixed QP of P frames (from 0 to 52, 0=skip).[if disabled will not "
                ...
                },
                {
                    ...
                    .desc =
                        N_("Target bitrate (0=skip).[if disabled will not send paramter to libav]"),
                    .group    = 3,
                    .get_opts = codec_profile_class_get_opts,
                ...
                },
                {
                    ...
                    .desc = N_(
                        "Maximum bitrate (0=skip).[if disabled will not send paramter to libav]"),
                    .group    = 3,
                    .get_opts = codec_profile_class_get_opts,
                ...
                },
                {
                    ...
                    .desc     = N_("Size of transcoding buffer (buffer=bitrate*2048*factor). Good "
                                   "factor is 3."),
                    .group    = 3,
                    .get_opts = codec_profile_class_get_opts,
                ...
                },
                {
                    ...
                    .desc     = N_("Bitrate & Max bitrate scaler with resolution (0=no scale; "
                                   "1=proportional_change). Relative to 480."),
                    .group    = 3,
                    .get_opts = codec_profile_class_get_opts,
                ...
                },
                {
                    ...
                    .name  = N_("Denoise"),
                    .group = 2,
                    .desc  = N_("Denoise only available with Hardware Acceleration (from 0 to 64, "
                                "0=skip, 0 default)"),
                    .get_opts = codec_profile_class_get_opts,
                ...
                },
                {
                    ...
                    .name  = N_("Sharpness"),
                    .group = 2,
                    .desc = N_("Sharpness only available with Hardware Acceleration (from 0 to 64, "
                               "0=skip, 44 default)"),
                    .get_opts = codec_profile_class_get_opts,
...
static int tvh_codec_profile_vaapi_h264_open(tvh_codec_profile_vaapi_t* self, AVDictionary** opts) {
  // to avoid issues we have this check:
  if (self->buff_factor <= 0) {
    self->buff_factor = 3;
  }
  int int_bitrate     = (int)((double)(self->bit_rate) * 1024.0 *
      (1.0 + (self->bit_rate_scale_factor * ((double)(self->size.den) - 480.0) / 480.0)));
  int int_buffer_size = (int)((double)(self->bit_rate) * 2048.0 * self->buff_factor *
      (1.0 + self->bit_rate_scale_factor * ((double)(self->size.den) - 480.0) / 480));
  ...
  // -low_power         <boolean>    E..V....... Use low-power encoding mode (only available on some
  // platforms; may not support all encoding features) (default false) -idr_interval      <int>
  // E..V....... Distance (in I-frames) between IDR frames (from 0 to INT_MAX) (default 0) -b_depth
  // <int>        E..V....... Maximum B-frame reference depth (from 1 to INT_MAX) (default 1)
  ...
  // -qp                <int>        E..V....... Constant QP (for P-frames; scaled by
  // qfactor/qoffset for I/B) (from 0 to 52) (default 0) -quality           <int>        E..V.......
  // Set encode quality (trades off against speed, higher is faster) (from -1 to INT_MAX) (default
  // -1) -coder             <int>        E..V....... Entropy coder type (from 0 to 1) (default
  // cabac)
  //    cavlc           0            E..V.......
  //    cabac           1            E..V.......
  //    vlc             0            E..V.......
  ...
  //    timing                       E..V....... Include timing parameters (buffering_period and
  //    pic_timing) recovery_point               E..V....... Include recovery points where
  //    appropriate
  // -profile           <int>        E..V....... Set profile (profile_idc and constraint_set*_flag)
  // (from -99 to 65535) (default -99)
  //    constrained_baseline 578          E..V.......
  //    main            77           E..V.......
  //    high            100          E..V.......
  // -level             <int>        E..V....... Set level (level_idc) (from -99 to 255) (default
  // -99)
  //    1               10           E..V.......
  //    1.1             11           E..V.......
  //    1.2             12           E..V.......
...
static int tvh_codec_profile_vaapi_hevc_open(tvh_codec_profile_vaapi_t* self, AVDictionary** opts) {
  // to avoid issues we have this check:
  if (self->buff_factor <= 0) {
    self->buff_factor = 3;
  }
  ...
  //    AVBR            6            E..V....... Average variable-bitrate
  // -qp                <int>        E..V....... Constant QP (for P-frames; scaled by
  // qfactor/qoffset for I/B) (from 0 to 52) (default 0) -aud               <boolean>    E..V.......
  // Include AUD (default false) -profile           <int>        E..V....... Set profile
  // (general_profile_idc) (from -99 to 255) (default -99)
  //    main            1            E..V.......
  //    main10          2            E..V.......
  //    rext            4            E..V.......
  // -tier              <int>        E..V....... Set tier (general_tier_flag) (from 0 to 1) (default
  // main)
  //    main            0            E..V.......
  //    high            1            E..V.......
  // -level             <int>        E..V....... Set level (general_level_idc) (from -99 to 255)
  // (default -99)
  //    1               30           E..V.......
  //    2               60           E..V.......
  //    2.1             63           E..V.......
  ...
  //    hdr                          E..V....... Include HDR metadata for mastering display colour
  //    volume and content light level information
...
static int tvh_codec_profile_vaapi_vp8_open(tvh_codec_profile_vaapi_t* self, AVDictionary** opts) {
  // to avoid issues we have this check:
  if (self->buff_factor <= 0) {
    self->buff_factor = 3;
  }
  ...
  // -loop_filter_sharpness <int>        E..V....... Loop filter sharpness (from 0 to 15) (default
  // 4)
...
static int tvh_codec_profile_vaapi_vp9_open(tvh_codec_profile_vaapi_t* self, AVDictionary** opts) {
  // to avoid issues we have this check:
  if (self->buff_factor <= 0) {
    self->buff_factor = 3;
  }
