...
tvh_codec_profile_libopus_open(tvh_codec_profile_libopus_t *self,
                               AVDictionary **opts)
{
    AV_DICT_SET_BIT_RATE(LS_LIBOPUS, opts, self->bit_rate);
    AV_DICT_SET_INT(LS_LIBOPUS, opts, "vbr", self->vbr, 0);
    AV_DICT_SET_INT(LS_LIBOPUS, opts, "application", self->application, 0);
    AV_DICT_SET_INT(LS_LIBOPUS, opts, "compression_level", self->complexity, 0);
    return 0;
}
