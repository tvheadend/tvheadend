...
tvh_codec_profile_libx26x_open(tvh_codec_profile_libx26x_t *self,
                               AVDictionary **opts)
{
    AV_DICT_SET(LS_LIBX26X, opts, "preset", self->preset, 0);
    AV_DICT_SET(LS_LIBX26X, opts, "tune", self->tune, 0);
    return 0;
}
                               ...
                               AVDictionary **opts)
{
    // bit_rate or crf
    if (self->bit_rate) {
        AV_DICT_SET_BIT_RATE(LS_LIBX26X, opts, self->bit_rate);
    }
    else {
        AV_DICT_SET_CRF(LS_LIBX26X, opts, self->crf, 15);
    }
    // params
    if (self->params && strlen(self->params)) {
        AV_DICT_SET(LS_LIBX26X, opts, "x264-params", self->params, 0);
    }
    return 0;
}
                               ...
                               AVDictionary **opts)
{
    ...
    if (self->bit_rate) {
        ...
        AV_DICT_SET_CRF(LS_LIBX26X, opts, self->crf, 18);
    }
    // params
    if (self->params && strlen(self->params)) {
        AV_DICT_SET(LS_LIBX26X, opts, "x265-params", self->params, 0);
    }
    return 0;
}
