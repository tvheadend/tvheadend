...
{
    AV_DICT_SET_FLAGS_GLOBAL_HEADER(LS_OMX, opts);
    // bit_rate
    if (self->bit_rate) {
        AV_DICT_SET_BIT_RATE(LS_OMX, opts, self->bit_rate);
    }
    if (self->libname && strlen(self->libname)) {
        AV_DICT_SET(LS_OMX, opts, "omx_libname", self->libname, 0);
    }
    if (self->libprefix && strlen(self->libprefix)) {
        AV_DICT_SET(LS_OMX, opts, "omx_libprefix", self->libprefix, 0);
    }
    AV_DICT_SET_INT(LS_OMX, opts, "zerocopy", self->zerocopy, 0);
    return 0;
}
