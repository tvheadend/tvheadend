...
#define AV_DICT_SET_CQ(d, v, a) \
    AV_DICT_SET_INT(LS_NVENC, (d), "cq", (v) ? (v) : (a), AV_DICT_DONT_OVERWRITE)
                             ...
                             AVDictionary **opts)
{
    ...
    AV_DICT_SET_INT(LS_NVENC, opts, "gpu", MINMAX(self->devicenum, 0, 15), 0);
    if (self->preset != PRESET_DEFAULT &&
        (s = val2str(self->profile, presettab)) != NULL) {
        AV_DICT_SET(LS_NVENC, opts, "preset", s, 0);
    }
    if (self->rc != NV_ENC_PARAMS_RC_AUTO &&
        (s = val2str(self->rc, rctab)) != NULL) {
        AV_DICT_SET(LS_NVENC, opts, "rc", s, 0);
    }
    if (self->bit_rate) {
        AV_DICT_SET_BIT_RATE(LS_NVENC, opts, self->bit_rate);
    }
    AV_DICT_SET_INT(LS_NVENC, opts, "quality", self->quality, 0);
    return 0;
}
                                  ...
                                  AVDictionary **opts)
{
    ...
    if (self->level != NV_ENC_LEVEL_AUTOSELECT &&
        ...
        AV_DICT_SET(LS_NVENC, opts, "level", s, 0);
    }

    if (self->nvenc_profile != FF_AV_PROFILE_UNKNOWN &&
        (s = val2str(self->nvenc_profile, profiletab)) != NULL) {
        AV_DICT_SET(LS_NVENC, opts, "profile", s, 0);
    }
    
    // ------ Set Defaults ---------
    AV_DICT_SET_INT(LS_NVENC, opts, "qmin", -1, 0);
    AV_DICT_SET_INT(LS_NVENC, opts, "qmax", -1, 0);
    AV_DICT_SET_INT(LS_NVENC, opts, "qdiff", -1, 0);
    AV_DICT_SET_INT(LS_NVENC, opts, "qblur", -1, 0);
    AV_DICT_SET_INT(LS_NVENC, opts, "qcomp", -1, 0);
    AV_DICT_SET_INT(LS_NVENC, opts, "g", 250, 0);
    AV_DICT_SET_INT(LS_NVENC, opts, "bf", 0, 0);
    AV_DICT_SET_INT(LS_NVENC, opts, "refs", 0, 0);
    return 0;
}
