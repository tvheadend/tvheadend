...
strtab2htsmsg0(const struct strtab tab[], int n, int i18n, const char* lang) {
  ...
  for (i = 0; i < n; i++) {
    ...
    htsmsg_add_str(e,
        "val",
        i18n ? tvh_gettext_lang(lang, tab[i].str) : tab[i].str);
    htsmsg_add_msg(l, NULL, e);
  }
  return l;
}

#define strtab2htsmsg(tab, i18n, lang) \
  strtab2htsmsg0(tab, sizeof(tab) / sizeof(tab[0]), i18n, lang)

static inline htsmsg_t* strtab2htsmsg0_u32(const struct strtab_u32 tab[],
    uint32_t                                                       n,
    int                                                            i18n,
    const char*                                                    lang) {
  uint32_t  i;
...
static inline htsmsg_t* strtab2htsmsg0_str(const struct strtab_str tab[],
    uint32_t                                                       n,
    int                                                            i18n,
    const char*                                                    lang) {
  uint32_t  i;
  htsmsg_t *e, *l = htsmsg_create_list();
  for (i = 0; i < n; i++) {
    e = htsmsg_create_key_val(tab[i].val,
        i18n ? tvh_gettext_lang(lang, tab[i].str) : tab[i].str);
    htsmsg_add_msg(l, NULL, e);
  }
  return l;
