...
static void _epg_object_getref(void* o) {
  epg_object_t* eo = o;
  tvhtrace(LS_EPG, "eo [%p, %u, %d] getref %d", eo, eo->id, eo->type, eo->refcount + 1);
  if (eo->refcount == 0)
    LIST_REMOVE(eo, un_link);
  eo->refcount++;
}
...
static int _epg_object_putref(void* o) {
  epg_object_t* eo = o;
  tvhtrace(LS_EPG, "eo [%p, %u, %d] putref %d", eo, eo->id, eo->type, eo->refcount - 1);
  assert(eo->refcount > 0);
  eo->refcount--;
  if (!eo->refcount) {
    eo->ops->destroy(eo);
...
htsmsg_t* epg_episode_epnum_serialize(epg_episode_num_t* num) {
  ...
  if (!num)
    return NULL;
  if (!num->e_num && !num->e_cnt && !num->s_num && !num->s_cnt && !num->p_num && !num->p_cnt &&
      !num->text)
    return NULL;
  m = htsmsg_create_map();
  if (num->e_num)
...
static void
_epg_channel_rem_broadcast(channel_t* ch, epg_broadcast_t* ebc, epg_broadcast_t* ebc_new) {
  ...
  *save |= 1;
  ...
  while ((ebc = RB_PREV(ret, sched_link)) != NULL) {
    ...
    if (!_epg_object_can_remove(ebc, ret)) {
      tvhtrace(LS_EPG,
          "grabber %s for event %u has higher priority than overlap (b), removing added",
          ebc->grabber->id,
          ebc->id);
  ...
  while ((ebc = RB_NEXT(ret, sched_link)) != NULL) {
    ...
    if (!_epg_object_can_remove(ebc, ret)) {
      tvhtrace(LS_EPG,
          "grabber %s for event %u has higher priority than overlap (a), removing added",
          ebc->grabber->id,
          ebc->id);
      _epg_channel_rem_broadcast(ch, ret, NULL);
      return NULL;
...
epg_set_t* epg_set_broadcast_find_by_uri(epg_set_tree_t* tree, const char* uri) {
  epg_set_t* dummy;
...
epg_set_t* epg_set_broadcast_insert(epg_set_tree_t* tree, epg_broadcast_t* b, const char* uri) {
  epg_set_t*      es;
  epg_set_item_t* item;
...
void epg_set_broadcast_remove(epg_set_tree_t* tree, epg_set_t* set, epg_broadcast_t* b) {
  epg_set_item_t* item;
...
int epg_broadcast_change_finish(epg_broadcast_t* broadcast, epg_changes_t changes, int merge) {
  int save = 0;
  if (merge)
    return 0;
...
epg_broadcast_t* epg_broadcast_clone(channel_t* channel, epg_broadcast_t* src, int* save) {
  epg_broadcast_t* ebc;
  epg_changes_t    changes = 0;

  if (!src)
    return NULL;
  ebc = epg_broadcast_find_by_time(channel, src->grabber, src->start, src->stop, 1, save, &changes);
  if (ebc) {
    /* Copy metadata */
...
int epg_broadcast_set_running(epg_broadcast_t* broadcast, epg_running_t running) {
  int save = 0;
...
int epg_broadcast_set_dvb_eid(epg_broadcast_t* b, uint16_t dvb_eid, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u16(b, &b->dvb_eid, dvb_eid, changed, EPG_CHANGED_DVB_EID);
}

int epg_broadcast_set_is_widescreen(epg_broadcast_t* b, uint8_t ws, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_widescreen, ws, changed, EPG_CHANGED_IS_WIDESCREEN);
}

int epg_broadcast_set_is_hd(epg_broadcast_t* b, uint8_t hd, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_hd, hd, changed, EPG_CHANGED_IS_HD);
}

int epg_broadcast_set_lines(epg_broadcast_t* b, uint16_t lines, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u16(b, &b->lines, lines, changed, EPG_CHANGED_LINES);
}

int epg_broadcast_set_aspect(epg_broadcast_t* b, uint16_t aspect, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u16(b, &b->aspect, aspect, changed, EPG_CHANGED_ASPECT);
}

int epg_broadcast_set_is_deafsigned(epg_broadcast_t* b, uint8_t ds, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_deafsigned, ds, changed, EPG_CHANGED_DEAFSIGNED);
}

int epg_broadcast_set_is_subtitled(epg_broadcast_t* b, uint8_t st, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_subtitled, st, changed, EPG_CHANGED_SUBTITLED);
}

int epg_broadcast_set_is_audio_desc(epg_broadcast_t* b, uint8_t ad, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_audio_desc, ad, changed, EPG_CHANGED_AUDIO_DESC);
}

int epg_broadcast_set_is_new(epg_broadcast_t* b, uint8_t n, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_new, n, changed, EPG_CHANGED_IS_NEW);
}

int epg_broadcast_set_is_repeat(epg_broadcast_t* b, uint8_t r, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_repeat, r, changed, EPG_CHANGED_IS_REPEAT);
}

int epg_broadcast_set_is_bw(epg_broadcast_t* b, uint8_t bw, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_bw, bw, changed, EPG_CHANGED_IS_BW);
}

int epg_broadcast_set_star_rating(epg_broadcast_t* b, uint8_t stars, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->star_rating, stars, changed, EPG_CHANGED_STAR_RATING);
}

int epg_broadcast_set_title(epg_broadcast_t* b, const lang_str_t* title, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_lang_str(b, &b->title, title, changed, EPG_CHANGED_TITLE);
}
...
int epg_broadcast_set_subtitle(epg_broadcast_t* b,
  ...
  if (!b)
    return 0;
  return _epg_object_set_lang_str(b, &b->subtitle, subtitle, changed, EPG_CHANGED_SUBTITLE);
}

int epg_broadcast_set_summary(epg_broadcast_t* b, const lang_str_t* str, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_lang_str(b, &b->summary, str, changed, EPG_CHANGED_SUMMARY);
}
...
int epg_broadcast_set_description(epg_broadcast_t* b,
  ...
  if (!b)
    return 0;
  return _epg_object_set_lang_str(b, &b->description, str, changed, EPG_CHANGED_DESCRIPTION);
}

int epg_broadcast_set_credits(epg_broadcast_t* b, const htsmsg_t* credits, epg_changes_t* changed) {
  if (!b)
    return 0;
...
int epg_broadcast_set_image(epg_broadcast_t* b, const char* image, epg_changes_t* changed) {
  int save;
  if (!b)
    return 0;
...
int epg_broadcast_set_epnumber(epg_broadcast_t* b, uint16_t number, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u16(b, &b->epnum.e_num, number, changed, EPG_CHANGED_EPNUM_NUM);
}
...
int epg_broadcast_set_eppart(epg_broadcast_t* b,
  ...
  if (!b)
    return 0;
  save |= _epg_object_set_u16(b, &b->epnum.p_num, part, changed, EPG_CHANGED_EPPAR_NUM);
  save |= _epg_object_set_u16(b, &b->epnum.p_cnt, count, changed, EPG_CHANGED_EPPAR_CNT);
  return save;
}

int epg_broadcast_set_epnum(epg_broadcast_t* b, epg_episode_num_t* num, epg_changes_t* changed) {
  int                      save  = 0;
  ...
  if (!num)
    num = &_zero;
  if (num->s_num)
    save |= _epg_object_set_u16(b, &b->epnum.s_num, num->s_num, changed, EPG_CHANGED_EPSER_NUM);
  if (num->s_cnt)
    save |= _epg_object_set_u16(b, &b->epnum.s_cnt, num->s_cnt, changed, EPG_CHANGED_EPSER_CNT);
  if (num->e_num)
    save |= _epg_object_set_u16(b, &b->epnum.e_num, num->e_num, changed, EPG_CHANGED_EPNUM_NUM);
  if (num->e_cnt)
    save |= _epg_object_set_u16(b, &b->epnum.e_cnt, num->e_cnt, changed, EPG_CHANGED_EPNUM_CNT);
  if (num->p_num)
    save |= _epg_object_set_u16(b, &b->epnum.p_num, num->p_num, changed, EPG_CHANGED_EPPAR_NUM);
  if (num->p_cnt)
    save |= _epg_object_set_u16(b, &b->epnum.p_cnt, num->p_cnt, changed, EPG_CHANGED_EPPAR_CNT);
  if (num->text)
    save |= _epg_object_set_str(b, &b->epnum.text, num->text, changed, EPG_CHANGED_EPTEXT);
  return save;
}

int epg_broadcast_set_genre(epg_broadcast_t* b, epg_genre_list_t* genre, epg_changes_t* changed) {
  int          save = 0;
  epg_genre_t *g1, *g2;
...
int epg_broadcast_set_copyright_year(epg_broadcast_t* b, uint16_t year, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u16(b, &b->copyright_year, year, changed, EPG_CHANGED_COPYRIGHT_YEAR);
}

int epg_broadcast_set_age_rating(epg_broadcast_t* b, uint8_t age, epg_changes_t* changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->age_rating, age, changed, EPG_CHANGED_AGE_RATING);
}
...
int epg_broadcast_set_first_aired(epg_broadcast_t* b, time_t aired, epg_changes_t* changed) {
  if (!b)
    return 0;
  if (changed)
...
int epg_genre_list_contains(epg_genre_list_t* list, epg_genre_t* genre, int partial) {
  uint8_t      mask = 0xFF;
  epg_genre_t* g;
  if (!list || !genre)
    return 0;
...
static void _eq_add(epg_query_t* eq, epg_broadcast_t* e) {
  ...
  if (fulltext) {
    if ((s = epg_broadcast_get_title(e, lang)) == NULL || regex_match(&eq->stitle_re, s)) {
      if ((s = epg_broadcast_get_subtitle(e, lang)) == NULL || regex_match(&eq->stitle_re, s)) {
        if ((s = epg_broadcast_get_summary(e, lang)) == NULL || regex_match(&eq->stitle_re, s)) {
...
epg_broadcast_t** epg_query(epg_query_t* eq, access_t* perm) {
  ...
  channel = channel_find_by_uuid(eq->channel) ?: channel_find_by_name(eq->channel);

  tag = channel_tag_find_by_uuid(eq->channel_tag) ?: channel_tag_find_by_name(eq->channel_tag, 0);
