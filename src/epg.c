...
void epg_updated(void) {
  ...
  while ((eo = LIST_FIRST(&epg_object_unref))) {
    tvhtrace(LS_EPG, "unref'd object %u created during update", eo->id);
...
static void _epg_object_destroy(epg_object_t* eo, epg_object_tree_t* tree) {
  ...
  tvhtrace(LS_EPG, "eo [%p, %u, %d] destroy", eo, eo->id, eo->type);
  if (eo->_updated)
    LIST_REMOVE(eo, up_link);
...
static void _epg_object_set_updated0(void* o) {
  epg_object_t* eo = o;
  tvhtrace(LS_EPG, "eo [%p, %u, %d] updated", eo, eo->id, eo->type);
  eo->_updated = 1;
  eo->updated  = gclk();
...
static int _epg_object_can_remove(void* _old, void* _new) {
  ...
  if (ograb == NULL || ograb == ngrab)
    return 1;
  if (ngrab->priority > ograb->priority)
    return 1;
  return 0;
}
...
static int _epg_object_set_grabber(void* o, epggrab_module_t* ngrab) {
  ...
  if (!ngrab)
    return 1; // grab=NULL is override
  ograb = ((epg_object_t*)o)->grabber;
  if (ograb == ngrab)
    return 1;
  if (ograb && ograb->priority >= ngrab->priority)
    return 0;
...
static void _epg_object_create(void* o) {
  ...
  if (!eo->id)
    eo->id = ++_epg_object_idx;
  tvhtrace(LS_EPG, "eo [%p, %u, %d] created", eo, eo->id, eo->type);
...
static htsmsg_t* _epg_object_serialize(void* o) {
  htsmsg_t*     m;
  epg_object_t* eo = o;
  tvhtrace(LS_EPG, "eo [%p, %u, %d] serialize", eo, eo->id, eo->type);
  if (!eo->id || !eo->type)
    return NULL;
  m = htsmsg_create_map();
...
static epg_object_t* _epg_object_deserialize(htsmsg_t* m, epg_object_t* eo) {
  ...
  uint32_t    u32;
  const char* s;
  if (htsmsg_get_u32(m, "id", &eo->id))
    return NULL;
  if (htsmsg_get_u32(m, "tp", &u32))
    return NULL;
  if (u32 != eo->type)
    return NULL;
  ...
  tvhtrace(LS_EPG,
      "eo [%p, %u, %d, %s, %s] deserialize",
      eo,
      eo->id,
      eo->type,
      s,
      eo->grabber ? eo->grabber->id : NULL);
  return eo;
}

static int _epg_object_set_str(void* o,
    char**                           old,
    const char*                      newstr,
...
EPG_OBJECT_SET_FN(_epg_object_set_lang_str,
    ...
    lang_str_compare,
    lang_str_copy)
EPG_OBJECT_SET_FN(_epg_object_set_string_list,
    string_list_t,
    string_list_destroy,
    string_list_cmp,
    string_list_copy)
EPG_OBJECT_SET_FN(_epg_object_set_htsmsg,
    htsmsg_t,
    htsmsg_destroy,
    htsmsg_cmp,
    htsmsg_copy)
...
htsmsg_t* epg_episode_epnum_serialize(epg_episode_num_t* num) {
  ...
  if (!num)
    return NULL;
  if (!num->e_num && !num->e_cnt && !num->s_num && !num->s_cnt && !num->p_num &&
      !num->p_cnt && !num->text)
    return NULL;
  m = htsmsg_create_map();
  if (num->e_num)
...
void epg_episode_epnum_deserialize(htsmsg_t* m, epg_episode_num_t* num) {
  ...
  if (!htsmsg_get_u32(m, "enum", &u32) || !htsmsg_get_u32(m, "e_num", &u32))
    num->e_num = u32;
  if (!htsmsg_get_u32(m, "ecnt", &u32) || !htsmsg_get_u32(m, "e_cnt", &u32))
    num->e_cnt = u32;
  if (!htsmsg_get_u32(m, "snum", &u32) || !htsmsg_get_u32(m, "s_num", &u32))
    num->s_num = u32;
  if (!htsmsg_get_u32(m, "scnt", &u32) || !htsmsg_get_u32(m, "s_cnt", &u32))
    num->s_cnt = u32;
  if (!htsmsg_get_u32(m, "pnum", &u32) || !htsmsg_get_u32(m, "p_num", &u32))
    num->p_num = u32;
  if (!htsmsg_get_u32(m, "pcnt", &u32) || !htsmsg_get_u32(m, "p_cnt", &u32))
    num->p_cnt = u32;
  if ((str = htsmsg_get_str(m, "text")))
    num->text = strdup(str);
}

size_t epg_episode_num_format(epg_episode_num_t* epnum,
    char*                                        buf,
    size_t                                       len,
    const char*                                  pre,
    const char*                                  sfmt,
  ...
  if (epnum->e_num || epnum->s_num) {
    if (pre)
      tvh_strlcatf(buf, len, i, "%s", pre);
    if (sfmt && epnum->s_num) {
      tvh_strlcatf(buf, len, i, sfmt, epnum->s_num);
      if (cfmt && epnum->s_cnt)
        tvh_strlcatf(buf, len, i, cfmt, epnum->s_cnt);
      if (sep && efmt && epnum->e_num)
        tvh_strlcatf(buf, len, i, "%s", sep);
    }
    if (efmt && epnum->e_num)
...
int epg_episode_number_cmp(const epg_episode_num_t* a,
    const epg_episode_num_t*                        b) {
  if (a->e_num) {
    if (a->s_num != b->s_num) {
...
int epg_episode_number_cmpfull(const epg_episode_num_t* a,
    const epg_episode_num_t*                            b) {
...
static void _epg_channel_rem_broadcast(channel_t* ch,
    epg_broadcast_t*                              ebc,
    epg_broadcast_t*                              ebc_new) {
  RB_REMOVE(&ch->ch_epg_schedule, ebc, sched_link);
  if (ch->ch_epg_now == ebc)
    ch->ch_epg_now = NULL;
  if (ch->ch_epg_next == ebc)
    ch->ch_epg_next = NULL;
  if (ebc_new) {
...
static void _epg_channel_timer_callback(void* p) {
  ...
  if ((cur = ch->ch_epg_now)) {
    if (cur->running != EPG_RUNNING_STOP &&
        cur->running != EPG_RUNNING_NOTSET) {
  ...
  if (next) {
    tvhdebug(LS_EPG,
        "arm channel timer @ %s for %s",
        gmtime2local(next, tm1, sizeof(tm1)),
        channel_get_name(ch, channel_blank_name));
...
static epg_broadcast_t* _epg_channel_add_broadcast(channel_t* ch,
  ...
  if (!src) {
    tvherror(LS_EPG,
        "skipped event (!grabber) %u (%s) on %s @ %s to %s",
        (*bcast)->id,
        epg_broadcast_get_title(*bcast, NULL),
        channel_get_name(ch, channel_blank_name),
  ...
  if (!create) {
    ...
    ret = RB_INSERT_SORTED(&ch->ch_epg_schedule,
        *bcast,
        sched_link,
        _ebc_start_cmp);
    ...
    if (!ret) {
      ...
      *bcast = NULL;
      ...
      ret->grabber = src;
      tvhtrace(LS_EPG,
          "added event %u (%s) on %s @ %s to %s (grabber %s)",
          ret->id,
          epg_broadcast_get_title(ret, NULL),
          channel_get_name(ch, channel_blank_name),
      ...
      if (ret->stop == (*bcast)->stop) {
        ...
        tvhtrace(LS_EPG,
            "updated event %u (%s) on %s @ %s to %s (grabber %s)",
            ret->id,
            epg_broadcast_get_title(ret, NULL),
            channel_get_name(ch, channel_blank_name),
            gmtime2local(ret->start, tm1, sizeof(tm1)),
  ...
  *save |= 1;
  ...
  while ((ebc = RB_PREV(ret, sched_link)) != NULL) {
    if (ebc->stop <= ret->start)
      break;
    if (!_epg_object_can_remove(ebc, ret)) {
      tvhtrace(LS_EPG,
          "grabber %s for event %u has higher priority than overlap (b), "
          "removing added",
          ebc->grabber->id,
          ebc->id);
    ...
    if (config.epg_cut_window &&
        ebc->stop - ebc->start > config.epg_cut_window * 2 &&
        ebc->stop - ret->start <= config.epg_cut_window) {
      tvhtrace(LS_EPG,
          "cut stop for overlap (b) event %u (%s) on %s @ %s to %s",
          ebc->id,
          epg_broadcast_get_title(ebc, NULL),
          channel_get_name(ch, channel_blank_name),
    ...
    tvhtrace(LS_EPG,
        "remove overlap (b) event %u (%s) on %s @ %s to %s",
        ebc->id,
        epg_broadcast_get_title(ebc, NULL),
        channel_get_name(ch, channel_blank_name),
        gmtime2local(ebc->start, tm1, sizeof(tm1)),
  ...
  while ((ebc = RB_NEXT(ret, sched_link)) != NULL) {
    if (ebc->start >= ret->stop)
      break;
    if (!_epg_object_can_remove(ebc, ret)) {
      tvhtrace(LS_EPG,
          "grabber %s for event %u has higher priority than overlap (a), "
          "removing added",
          ebc->grabber->id,
          ebc->id);
      _epg_channel_rem_broadcast(ch, ret, NULL);
    ...
    if (config.epg_cut_window &&
        ret->stop - ret->start > config.epg_cut_window * 2 &&
        ret->stop - ebc->start <= config.epg_cut_window) {
      tvhtrace(LS_EPG,
          "cut stop for overlap (a) event %u (%s) on %s @ %s to %s",
          ebc->id,
          epg_broadcast_get_title(ebc, NULL),
          channel_get_name(ch, channel_blank_name),
          gmtime2local(ebc->start, tm1, sizeof(tm1)),
    ...
    tvhtrace(LS_EPG,
        "remove overlap (a) event %u (%s) on %s @ %s to %s",
        ebc->id,
        epg_broadcast_get_title(ebc, NULL),
        channel_get_name(ch, channel_blank_name),
        gmtime2local(ebc->start, tm1, sizeof(tm1)),
  ...
  if (RB_FIRST(&ch->ch_epg_schedule) == ret) {
    timer = 1;
  } else if (ch->ch_epg_now && RB_NEXT(ch->ch_epg_now, sched_link) == ret) {
    timer = 1;
  }
...
epg_set_t* epg_set_broadcast_find_by_uri(epg_set_tree_t* tree,
    const char*                                          uri) {
  epg_set_t* dummy;
...
epg_set_t* epg_set_broadcast_insert(epg_set_tree_t* tree,
    epg_broadcast_t*                                b,
    const char*                                     uri) {
  epg_set_t*      es;
  epg_set_item_t* item;
...
void epg_set_broadcast_remove(epg_set_tree_t* tree,
    epg_set_t*                                set,
    epg_broadcast_t*                          b) {
  epg_set_item_t* item;
...
static void _epg_broadcast_destroy(void* eo) {
  ...
  if (ebc->title)
    lang_str_destroy(ebc->title);
  if (ebc->subtitle)
    lang_str_destroy(ebc->subtitle);
  if (ebc->summary)
    lang_str_destroy(ebc->summary);
  if (ebc->description)
    lang_str_destroy(ebc->description);
  ...
  free(ebc->epnum.text);
  if (ebc->credits)
    htsmsg_destroy(ebc->credits);
  if (ebc->credits_cached)
    lang_str_destroy(ebc->credits_cached);
  if (ebc->category)
    string_list_destroy(ebc->category);
  if (ebc->keyword)
    string_list_destroy(ebc->keyword);
  if (ebc->keyword_cached)
    lang_str_destroy(ebc->keyword_cached);
...
epg_broadcast_t* epg_broadcast_find_by_time(channel_t* channel,
    epggrab_module_t*                                  src,
    time_t                                             start,
    time_t                                             stop,
    int                                                create,
...
int epg_broadcast_change_finish(epg_broadcast_t* broadcast,
    epg_changes_t                                changes,
    int                                          merge) {
  int save = 0;
  if (merge)
    return 0;
  if (changes & EPG_CHANGED_CREATE)
    return 0;
  ...
  if (!(changes & EPG_CHANGED_EPTEXT))
    save |=
        _epg_object_set_str(broadcast, &broadcast->epnum.text, NULL, NULL, 0);
...
epg_broadcast_t*
epg_broadcast_clone(channel_t* channel, epg_broadcast_t* src, int* save) {
  epg_broadcast_t* ebc;
  epg_changes_t    changes = 0;

  if (!src)
    return NULL;
  ebc = epg_broadcast_find_by_time(channel,
      src->grabber,
      src->start,
      src->stop,
      1,
  ...
  if (ebc) {
    ...
    *save |= epg_broadcast_set_serieslink_uri(ebc,
        src->serieslink ? src->serieslink->uri : NULL,
        &changes);
    *save |= epg_broadcast_set_episodelink_uri(ebc,
        src->episodelink ? src->episodelink->uri : NULL,
        &changes);
    *save |= epg_broadcast_set_first_aired(ebc, src->first_aired, &changes);
    *save |=
        epg_broadcast_set_copyright_year(ebc, src->copyright_year, &changes);
...
epg_broadcast_t* epg_broadcast_find_by_eid(channel_t* ch, uint16_t eid) {
  ...
  RB_FOREACH (e, &ch->ch_epg_schedule, sched_link) {
    if (e->dvb_eid == eid && e->stop > gclk())
      return e;
  }
  return NULL;
}

int epg_broadcast_set_running(epg_broadcast_t* broadcast,
    epg_running_t                              running) {
  int save = 0;
...
static int _epg_broadcast_set_set(epg_broadcast_t* ebc,
    const char*                                    uri,
    epg_set_tree_t*                                tree,
    epg_set_t**                                    set) {
  if (*set == NULL) {
...
int epg_broadcast_set_serieslink_uri(epg_broadcast_t* ebc,
    const char*                                       uri,
    epg_changes_t*                                    changed) {
  if (!ebc)
    return 0;
  if (changed)
    *changed |= EPG_CHANGED_SERIESLINK;
  return _epg_broadcast_set_set(ebc, uri, &epg_serieslinks, &ebc->serieslink);
}

int epg_broadcast_set_episodelink_uri(epg_broadcast_t* ebc,
    const char*                                        uri,
    epg_changes_t*                                     changed) {
  if (!ebc)
    return 0;
...
int epg_broadcast_set_dvb_eid(epg_broadcast_t* b,
    uint16_t                                   dvb_eid,
    epg_changes_t*                             changed) {
  if (!b)
    return 0;
  return _epg_object_set_u16(b,
      &b->dvb_eid,
      dvb_eid,
      changed,
      EPG_CHANGED_DVB_EID);
...
int epg_broadcast_set_is_widescreen(epg_broadcast_t* b,
    uint8_t                                          ws,
    epg_changes_t*                                   changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b,
      &b->is_widescreen,
      ws,
      changed,
      EPG_CHANGED_IS_WIDESCREEN);
}

int epg_broadcast_set_is_hd(epg_broadcast_t* b,
    uint8_t                                  hd,
    epg_changes_t*                           changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_hd, hd, changed, EPG_CHANGED_IS_HD);
}

int epg_broadcast_set_lines(epg_broadcast_t* b,
    uint16_t                                 lines,
    epg_changes_t*                           changed) {
  if (!b)
    return 0;
  return _epg_object_set_u16(b, &b->lines, lines, changed, EPG_CHANGED_LINES);
}

int epg_broadcast_set_aspect(epg_broadcast_t* b,
    uint16_t                                  aspect,
    epg_changes_t*                            changed) {
  if (!b)
    return 0;
...
int epg_broadcast_set_is_deafsigned(epg_broadcast_t* b,
    uint8_t                                          ds,
    epg_changes_t*                                   changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b,
      ...
      ds,
      changed,
      EPG_CHANGED_DEAFSIGNED);
}

int epg_broadcast_set_is_subtitled(epg_broadcast_t* b,
    uint8_t                                         st,
    epg_changes_t*                                  changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b,
      ...
      st,
      changed,
      EPG_CHANGED_SUBTITLED);
}

int epg_broadcast_set_is_audio_desc(epg_broadcast_t* b,
    uint8_t                                          ad,
    epg_changes_t*                                   changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b,
      ...
      ad,
      changed,
      EPG_CHANGED_AUDIO_DESC);
}

int epg_broadcast_set_is_new(epg_broadcast_t* b,
    uint8_t                                   n,
    epg_changes_t*                            changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_new, n, changed, EPG_CHANGED_IS_NEW);
}

int epg_broadcast_set_is_repeat(epg_broadcast_t* b,
    uint8_t                                      r,
    epg_changes_t*                               changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b,
      ...
      r,
      changed,
      EPG_CHANGED_IS_REPEAT);
}

int epg_broadcast_set_is_bw(epg_broadcast_t* b,
    uint8_t                                  bw,
    epg_changes_t*                           changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b, &b->is_bw, bw, changed, EPG_CHANGED_IS_BW);
}

int epg_broadcast_set_star_rating(epg_broadcast_t* b,
    uint8_t                                        stars,
    epg_changes_t*                                 changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b,
      ...
      stars,
      changed,
      EPG_CHANGED_STAR_RATING);
}

int epg_broadcast_set_title(epg_broadcast_t* b,
    const lang_str_t*                        title,
    epg_changes_t*                           changed) {
  if (!b)
    return 0;
  return _epg_object_set_lang_str(b,
      &b->title,
      title,
      changed,
      EPG_CHANGED_TITLE);
...
int epg_broadcast_set_subtitle(epg_broadcast_t* b,
    const lang_str_t*                           subtitle,
    epg_changes_t*                              changed) {
  if (!b)
    return 0;
  return _epg_object_set_lang_str(b,
      ...
      subtitle,
      changed,
      EPG_CHANGED_SUBTITLE);
}

int epg_broadcast_set_summary(epg_broadcast_t* b,
    const lang_str_t*                          str,
    epg_changes_t*                             changed) {
  if (!b)
    return 0;
  return _epg_object_set_lang_str(b,
...
int epg_broadcast_set_description(epg_broadcast_t* b,
    const lang_str_t*                              str,
    epg_changes_t*                                 changed) {
  if (!b)
    return 0;
  return _epg_object_set_lang_str(b,
      ...
      str,
      changed,
      EPG_CHANGED_DESCRIPTION);
}

int epg_broadcast_set_credits(epg_broadcast_t* b,
    const htsmsg_t*                            credits,
    epg_changes_t*                             changed) {
  if (!b)
    return 0;
  const int mod = _epg_object_set_htsmsg(b,
      &b->credits,
      credits,
      changed,
      EPG_CHANGED_CREDITS);
...
int epg_broadcast_set_category(epg_broadcast_t* b,
    const string_list_t*                        msg,
    epg_changes_t*                              changed) {
  if (!b)
    return 0;
  return _epg_object_set_string_list(b,
      &b->category,
      msg,
      changed,
      EPG_CHANGED_CATEGORY);
}

int epg_broadcast_set_keyword(epg_broadcast_t* b,
    const string_list_t*                       msg,
    epg_changes_t*                             changed) {
  if (!b)
    return 0;
  const int mod = _epg_object_set_string_list(b,
      &b->keyword,
      msg,
      changed,
      EPG_CHANGED_KEYWORD);
...
int epg_broadcast_set_image(epg_broadcast_t* b,
    const char*                              image,
    epg_changes_t*                           changed) {
  int save;
  if (!b)
    return 0;
  save = _epg_object_set_str(b, &b->image, image, changed, EPG_CHANGED_IMAGE);
  if (save)
    imagecache_get_id(image);
  return save;
}

int epg_broadcast_set_epnumber(epg_broadcast_t* b,
    uint16_t                                    number,
    epg_changes_t*                              changed) {
  if (!b)
    return 0;
  return _epg_object_set_u16(b,
      ...
      number,
      changed,
      EPG_CHANGED_EPNUM_NUM);
}

int epg_broadcast_set_eppart(epg_broadcast_t* b,
    uint16_t                                  part,
    uint16_t                                  count,
    epg_changes_t*                            changed) {
  int save = 0;
  if (!b)
    return 0;
  save |= _epg_object_set_u16(b,
      &b->epnum.p_num,
      part,
      changed,
      EPG_CHANGED_EPPAR_NUM);
  save |= _epg_object_set_u16(b,
      &b->epnum.p_cnt,
      count,
      changed,
      EPG_CHANGED_EPPAR_CNT);
  return save;
}

int epg_broadcast_set_epnum(epg_broadcast_t* b,
  ...
  if (num->s_num)
    save |= _epg_object_set_u16(b,
        ...
        num->s_num,
        changed,
        EPG_CHANGED_EPSER_NUM);
  if (num->s_cnt)
    save |= _epg_object_set_u16(b,
        &b->epnum.s_cnt,
        num->s_cnt,
        changed,
        EPG_CHANGED_EPSER_CNT);
  if (num->e_num)
    save |= _epg_object_set_u16(b,
        &b->epnum.e_num,
        num->e_num,
...
int epg_broadcast_set_genre(epg_broadcast_t* b,
...
int epg_broadcast_set_copyright_year(epg_broadcast_t* b,
    uint16_t                                          year,
    epg_changes_t*                                    changed) {
  if (!b)
    return 0;
  return _epg_object_set_u16(b,
      ...
      year,
      changed,
      EPG_CHANGED_COPYRIGHT_YEAR);
}

int epg_broadcast_set_age_rating(epg_broadcast_t* b,
    uint8_t                                       age,
    epg_changes_t*                                changed) {
  if (!b)
    return 0;
  return _epg_object_set_u8(b,
      ...
      age,
      changed,
      EPG_CHANGED_AGE_RATING);
}

int epg_broadcast_set_rating_label(epg_broadcast_t* b,
  ...
  if (rating_label != b->rating_label) {
    ...
    if (changed)
      *changed |= EPG_CHANGED_RATING_LABEL;
    return 1;
  }
...
int epg_broadcast_set_first_aired(epg_broadcast_t* b,
    time_t                                         aired,
    epg_changes_t*                                 changed) {
  if (!b)
    return 0;
...
const char* epg_broadcast_get_title(const epg_broadcast_t* b,
    const char*                                            lang) {
  if (!b || !b->title)
    return NULL;
...
const char* epg_broadcast_get_credits_cached(epg_broadcast_t* b,
    const char*                                               lang) {
  if (!b || !b->credits_cached)
    return NULL;
  return lang_str_get(b->credits_cached, lang);
}

const char* epg_broadcast_get_keyword_cached(epg_broadcast_t* b,
    const char*                                               lang) {
  if (!b || !b->keyword_cached)
    return NULL;
  return lang_str_get(b->keyword_cached, lang);
}

const char* epg_broadcast_get_description(epg_broadcast_t* b,
    const char*                                            lang) {
  if (!b || !b->description)
    return NULL;
...
size_t epg_broadcast_epnumber_format(epg_broadcast_t* b,
    char*                                             buf,
    size_t                                            len,
    const char*                                       pre,
    const char*                                       sfmt,
  ...
  return epg_episode_num_format(&num, buf, len, pre, sfmt, sep, efmt, cfmt);
}
...
htsmsg_t* epg_broadcast_serialize(epg_broadcast_t* broadcast) {
  ...
  if (!broadcast)
    return NULL;
  if (!(m = _epg_object_serialize((epg_object_t*)broadcast)))
    return NULL;
...
epg_broadcast_t* epg_broadcast_deserialize(htsmsg_t* m, int create, int* save) {
  channel_t*        ch = NULL;
  ...
  /* Create */
  ebc = _epg_channel_add_broadcast(ch,
      skel,
      (*skel)->grabber,
      create,
      save,
  ...
  if ((str = htsmsg_get_str(m, "ratlab"))) {
    ...
    *save |= epg_broadcast_set_rating_label(ebc,
        ratinglabel_find_from_uuid(str),
        &changes);
  }
...
static const char** _epg_genre_names[16][16] = {{/* 00 */
                                                    C_{"", NULL}},
    {
        /* 01 */
...
int epg_genre_list_add_by_str(epg_genre_list_t* list,
    const char*                                 str,
    const char*                                 lang) {
  epg_genre_t g;
...
int epg_genre_list_contains(epg_genre_list_t* list,
    epg_genre_t*                              genre,
    int                                       partial) {
  uint8_t      mask = 0xFF;
  epg_genre_t* g;
  if (!list || !genre)
    return 0;
  if (partial && !(genre->code & 0x0F))
    mask = 0xF0;
  LIST_FOREACH (g, list, link) {
    if ((g->code & mask) == genre->code)
      break;
  }
  return g ? 1 : 0;
}
...
htsmsg_t*
epg_genres_list_all(int major_only, int major_prefix, const char* lang) {
  int         i, j;
  htsmsg_t *  e, *m;
  const char* s;
...
static inline int _eq_comp_num(epg_filter_num_t* f, int64_t val) {
  switch (f->comp) {
    case EC_EQ:
      return val != f->val1;
    case EC_LT:
    ...
    case EC_GT:
      return val < f->val1;
    case EC_RG:
      return val < f->val1 || val > f->val2;
    default:
      return 0;
  }
}

static inline int _eq_comp_str(epg_filter_str_t* f, const char* str) {
  if (!str)
    return 0;
  switch (f->comp) {
    case EC_EQ:
      return strcmp(str, f->str);
    case EC_LT:
      return strcmp(str, f->str) > 0;
    case EC_GT:
      return strcmp(str, f->str) < 0;
    case EC_IN:
      return strstr(str, f->str) != NULL;
    case EC_RE:
      return regex_match(&f->re, str) != 0;
    default:
      return 0;
  }
}

static void _eq_add(epg_query_t* eq, epg_broadcast_t* e) {
  ...
  if (e->stop < gclk())
    return;
  if (_eq_comp_num(&eq->start, e->start))
    return;
  if (_eq_comp_num(&eq->stop, e->stop))
    return;
  if (eq->duration.comp != EC_NO) {
    int64_t duration = (int64_t)e->stop - (int64_t)e->start;
    if (_eq_comp_num(&eq->duration, duration))
      return;
  }
  if (eq->stars.comp != EC_NO)
    if (_eq_comp_num(&eq->stars, e->star_rating))
      return;
  if (eq->age.comp != EC_NO)
    if (_eq_comp_num(&eq->age, e->age_rating))
      return;
  if (eq->channel_num.comp != EC_NO)
    if (_eq_comp_num(&eq->channel_num, channel_get_number(e->channel)))
      return;
  if (eq->channel_name.comp != EC_NO)
    if (_eq_comp_str(&eq->channel_name, channel_get_name(e->channel, NULL)))
      return;
  if (eq->cat1 && *eq->cat1) {
  ...
  if (eq->genre_count) {
    ...
    for (i = 0; i < eq->genre_count; i++) {
      ...
      if (genre.code == 0)
        continue;
      if (epg_genre_list_contains(&e->genre, &genre, 1))
        r++;
    }
    if (!r)
      return;
  ...
  if (eq->title.comp != EC_NO || (eq->stitle && !fulltext)) {
    if ((s = epg_broadcast_get_title(e, lang)) == NULL)
      return;
    if (eq->stitle && !fulltext && regex_match(&eq->stitle_re, s))
      return;
    if (eq->title.comp != EC_NO && _eq_comp_str(&eq->title, s))
      return;
  }
  if (eq->subtitle.comp != EC_NO) {
    if ((s = epg_broadcast_get_subtitle(e, lang)) == NULL)
      return;
    if (_eq_comp_str(&eq->subtitle, s))
      return;
  }
  if (eq->summary.comp != EC_NO) {
    if ((s = epg_broadcast_get_summary(e, lang)) == NULL)
      return;
    if (_eq_comp_str(&eq->summary, s))
      return;
  }
  if (eq->description.comp != EC_NO) {
    if ((s = epg_broadcast_get_description(e, lang)) == NULL)
      return;
    if (_eq_comp_str(&eq->description, s))
      return;
  }
...
static void _eq_add_channel(epg_query_t* eq, channel_t* ch) {
  epg_broadcast_t* ebc;
  RB_FOREACH (ebc, &ch->ch_epg_schedule, sched_link)
    _eq_add(eq, ebc);
}

static int _eq_init_str(epg_filter_str_t* f) {
  if (f->comp != EC_RE)
    return 0;
  return regex_compile(&f->re, f->str, TVHREGEX_CASELESS, LS_EPG);
}

static void _eq_done_str(epg_filter_str_t* f) {
  if (f->comp == EC_RE)
    regex_free(&f->re);
  free(f->str);
  f->str = NULL;
...
static int
_epg_sort_duration_ascending(const void* a, const void* b, void* eq) {
  return _epg_sort_duration(*(epg_broadcast_t**)a) -
      _epg_sort_duration(*(epg_broadcast_t**)b);
}

static int
_epg_sort_duration_descending(const void* a, const void* b, void* eq) {
  return _epg_sort_duration(*(epg_broadcast_t**)b) -
      _epg_sort_duration(*(epg_broadcast_t**)a);
}

static int _epg_sort_title_ascending(const void* a, const void* b, void* eq) {
  const char* s1 =
      epg_broadcast_get_title(*(epg_broadcast_t**)a, ((epg_query_t*)eq)->lang);
  const char* s2 =
      epg_broadcast_get_title(*(epg_broadcast_t**)b, ((epg_query_t*)eq)->lang);
  if (s1 == NULL && s2 == NULL)
    return 0;
  if (s1 == NULL && s2)
    return 1;
  ...
  int r = strcmp(s1, s2);
  if (r == 0) {
    s1 = epg_broadcast_get_subtitle(*(epg_broadcast_t**)a,
        ((epg_query_t*)eq)->lang);
    s2 = epg_broadcast_get_subtitle(*(epg_broadcast_t**)b,
        ((epg_query_t*)eq)->lang);
    if (s1 == NULL && s2 == NULL)
      return 0;
    if (s1 == NULL && s2)
...
static int
_epg_sort_subtitle_ascending(const void* a, const void* b, void* eq) {
  const char* s1 = epg_broadcast_get_subtitle(*(epg_broadcast_t**)a,
      ((epg_query_t*)eq)->lang);
  const char* s2 = epg_broadcast_get_subtitle(*(epg_broadcast_t**)b,
      ((epg_query_t*)eq)->lang);
  if (s1 == NULL && s2 == NULL)
    return 0;
  if (s1 == NULL && s2)
...
static int
_epg_sort_subtitle_descending(const void* a, const void* b, void* eq) {
  return _epg_sort_subtitle_ascending(a, b, eq) * -1;
}

static int _epg_sort_summary_ascending(const void* a, const void* b, void* eq) {
  const char* s1 = epg_broadcast_get_summary(*(epg_broadcast_t**)a,
      ((epg_query_t*)eq)->lang);
  const char* s2 = epg_broadcast_get_summary(*(epg_broadcast_t**)b,
      ((epg_query_t*)eq)->lang);
  if (s1 == NULL && s2 == NULL)
    return 0;
  if (s1 == NULL && s2)
...
static int
_epg_sort_summary_descending(const void* a, const void* b, void* eq) {
  return _epg_sort_summary_ascending(a, b, eq) * -1;
}

static int
_epg_sort_description_ascending(const void* a, const void* b, void* eq) {
  const char* s1 = epg_broadcast_get_description(*(epg_broadcast_t**)a,
      ((epg_query_t*)eq)->lang);
  const char* s2 = epg_broadcast_get_description(*(epg_broadcast_t**)b,
      ((epg_query_t*)eq)->lang);
  if (s1 == NULL && s2 == NULL)
    return 0;
  if (s1 == NULL && s2)
...
static int
_epg_sort_description_descending(const void* a, const void* b, void* eq) {
  return _epg_sort_description_ascending(a, b, eq) * -1;
}

static int
_epg_sort_extratext_ascending(const void* a, const void* b, void* eq) {
...
static int
_epg_sort_extratext_descending(const void* a, const void* b, void* eq) {
...
static int
_epg_sort_channel_descending(const void* a, const void* b, void* eq) {
  return _epg_sort_description_ascending(a, b, eq) * -1;
}

static int
_epg_sort_channel_num_ascending(const void* a, const void* b, void* eq) {
...
static int
_epg_sort_channel_num_descending(const void* a, const void* b, void* eq) {
...
static int _epg_sort_stars_ascending(const void* a, const void* b, void* eq) {
  return (*(epg_broadcast_t**)a)->star_rating -
      (*(epg_broadcast_t**)b)->star_rating;
}

static int _epg_sort_stars_descending(const void* a, const void* b, void* eq) {
  return (*(epg_broadcast_t**)b)->star_rating -
      (*(epg_broadcast_t**)a)->star_rating;
}

static int _epg_sort_age_ascending(const void* a, const void* b, void* eq) {
  return (*(epg_broadcast_t**)a)->age_rating -
      (*(epg_broadcast_t**)b)->age_rating;
}

static int _epg_sort_age_descending(const void* a, const void* b, void* eq) {
  return (*(epg_broadcast_t**)b)->age_rating -
      (*(epg_broadcast_t**)a)->age_rating;
}
...
epg_broadcast_t** epg_query(epg_query_t* eq, access_t* perm) {
  ...
  /* Setup exp */
  if (_eq_init_str(&eq->title))
    goto fin;
  if (_eq_init_str(&eq->subtitle))
    goto fin;
  if (_eq_init_str(&eq->summary))
    goto fin;
  if (_eq_init_str(&eq->description))
    goto fin;
  if (_eq_init_str(&eq->extratext))
    goto fin;
  if (_eq_init_str(&eq->channel_name))
    goto fin;
  ...
  channel =
      channel_find_by_uuid(eq->channel) ?: channel_find_by_name(eq->channel);
  ...
  switch (eq->sort_dir) {
    case ES_ASC:
      switch (eq->sort_key) {
        case ESK_START:
          fcn = _epg_sort_start_ascending;
          break;
        case ESK_STOP:
          fcn = _epg_sort_stop_ascending;
          break;
        case ESK_DURATION:
          fcn = _epg_sort_duration_ascending;
          break;
        case ESK_TITLE:
          fcn = _epg_sort_title_ascending;
          break;
        case ESK_SUBTITLE:
          fcn = _epg_sort_subtitle_ascending;
          break;
        case ESK_SUMMARY:
          fcn = _epg_sort_summary_ascending;
          break;
        case ESK_DESCRIPTION:
          fcn = _epg_sort_description_ascending;
          break;
        case ESK_EXTRATEXT:
          fcn = _epg_sort_extratext_ascending;
          break;
        case ESK_CHANNEL:
          fcn = _epg_sort_channel_ascending;
          break;
        case ESK_CHANNEL_NUM:
          fcn = _epg_sort_channel_num_ascending;
          break;
        case ESK_STARS:
          fcn = _epg_sort_stars_ascending;
          break;
        case ESK_AGE:
          fcn = _epg_sort_age_ascending;
          break;
        case ESK_GENRE:
          fcn = _epg_sort_genre_ascending;
          break;
      }
      break;
    case ES_DSC:
      switch (eq->sort_key) {
        case ESK_START:
          fcn = _epg_sort_start_descending;
          break;
        case ESK_STOP:
          fcn = _epg_sort_stop_descending;
          break;
        case ESK_DURATION:
          fcn = _epg_sort_duration_descending;
          break;
        case ESK_TITLE:
          fcn = _epg_sort_title_descending;
          break;
        case ESK_SUBTITLE:
          fcn = _epg_sort_subtitle_descending;
          break;
        case ESK_SUMMARY:
          fcn = _epg_sort_summary_descending;
          break;
        case ESK_DESCRIPTION:
          fcn = _epg_sort_description_descending;
          break;
        case ESK_EXTRATEXT:
          fcn = _epg_sort_extratext_descending;
          break;
        case ESK_CHANNEL:
          fcn = _epg_sort_channel_descending;
          break;
        case ESK_CHANNEL_NUM:
          fcn = _epg_sort_channel_num_descending;
          break;
        case ESK_STARS:
          fcn = _epg_sort_stars_descending;
          break;
        case ESK_AGE:
          fcn = _epg_sort_age_descending;
          break;
        case ESK_GENRE:
          fcn = _epg_sort_genre_descending;
          break;
      }
      break;
  }
