 ...
 *
 */
static size_t streaming_message_data_size(streaming_message_t* sm) {
 ...
 *
 */
static htsmsg_t* streaming_queue_info(void* opaque, htsmsg_t* list) {
...
void streaming_queue_init(streaming_queue_t* sq, int reject_filter, size_t maxsize) {
  static streaming_ops_t ops = {.st_cb = streaming_queue_deliver, .st_info = streaming_queue_info};
...
streaming_start_t* streaming_start_copy(const streaming_start_t* src) {
  int    i;
  size_t siz =
      sizeof(streaming_start_t) + sizeof(streaming_start_component_t) * src->ss_num_components;
 ...
 *
 */
streaming_start_component_t* streaming_start_component_find_by_index(streaming_start_t* ss,
    int                                                                                 idx) {
...
const char* streaming_component_audio_type2desc(int audio_type) {
  ...
  switch (audio_type) {
    case 0:
      return ""; /* "Undefined" in the standard, but used for normal audio */
    case 1:
      return N_("Clean effects");
    case 2:
      return N_("Hearing impaired");
    case 3:
      return N_("Visually impaired commentary/audio description");
  }
