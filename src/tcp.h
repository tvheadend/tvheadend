...
#define IP_IN_ADDR(storage)                                                         \
  ((storage).ss_family == AF_INET6 ? &((struct sockaddr_in6*)&(storage))->sin6_addr \
...
#define IP_PORT(storage)                                                           \
  ((storage).ss_family == AF_INET6 ? ((struct sockaddr_in6*)&(storage))->sin6_port \
                                   : ((struct sockaddr_in*)&(storage))->sin_port)
#define IP_PORT_SET(storage, port)                            \
  do {                                                        \
    if ((storage).ss_family == AF_INET6)                      \
      ((struct sockaddr_in6*)&(storage))->sin6_port = (port); \
    else                                                      \
...
typedef struct tcp_server_ops {
  void (
      *start)(int fd, void** opaque, struct sockaddr_storage* peer, struct sockaddr_storage* self);
...
static inline int ip_check_equal_v4(const struct sockaddr_storage* a,
...
static inline int ip_check_equal_v6(const struct sockaddr_storage* a,
    const struct sockaddr_storage*                                 b) {
  return memcmp(IP_AS_V6(a, addr).s6_addr,
             IP_AS_V6(b, addr).s6_addr,
             sizeof(IP_AS_V6(a, addr).s6_addr)) == 0;
}

static inline int ip_check_equal(const struct sockaddr_storage* a,
    const struct sockaddr_storage*                              b) {
  return a->ss_family == b->ss_family && a->ss_family == AF_INET
      ? ip_check_equal_v4(a, b)
...
static inline int ip_check_in_network_v6(const struct sockaddr_storage* network,
  ...
  for (i = 0; i < sizeof(IP_AS_V6(address, addr).s6_addr); ++i)
    if (((IP_AS_V6(address, addr).s6_addr)[i] & (IP_AS_V6(mask, addr).s6_addr)[i]) !=
        ((IP_AS_V6(network, addr).s6_addr)[i] & (IP_AS_V6(mask, addr).s6_addr)[i]))
      return 0;
  return 1;
}
...
static inline int ip_check_in_network(const struct sockaddr_storage* network,
  ...
  return address->ss_family == AF_INET
      ? ip_check_in_network_v4(network, mask, address)
...
static inline int ip_check_is_any_v6(const struct sockaddr_storage* address) {
  return memcmp(IP_AS_V6(address, addr).s6_addr,
             in6addr_any.s6_addr,
             sizeof(in6addr_any.s6_addr)) == 0;
}

static inline int ip_check_is_any(const struct sockaddr_storage* address) {
  return address->ss_family == AF_INET
      ? ip_check_is_any_v4(address)
...
typedef void(tcp_server_callback_t)(int fd,
    void*                               opaque,
    struct sockaddr_storage*            peer,
    struct sockaddr_storage*            self);

void* tcp_server_create(int subsystem,
    const char*             name,
    const char*             bindaddr,
    int                     port,
...
int tcp_read_data(int fd, char* buf, const size_t bufsize, htsbuf_queue_t* spill);
...
void*    tcp_connection_launch(int fd,
       int                         streaming,
