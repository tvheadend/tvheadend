...
static inline int ip_check_in_network_v6(const struct sockaddr_storage* network,
  ...
  for (i = 0; i < sizeof(IP_AS_V6(address, addr).s6_addr); ++i)
    if (((IP_AS_V6(address, addr).s6_addr)[i] &
            (IP_AS_V6(mask, addr).s6_addr)[i]) !=
        ((IP_AS_V6(network, addr).s6_addr)[i] &
            (IP_AS_V6(mask, addr).s6_addr)[i]))
      return 0;
  return 1;
}
...
static inline int ip_check_in_network(const struct sockaddr_storage* network,
  ...
  return address->ss_family == AF_INET
      ...
      : (address->ss_family == AF_INET6
                ? ip_check_in_network_v6(network, mask, address)
                : 0);
}
...
char* tcp_get_str_from_ip(const struct sockaddr_storage* sa,
    char*                                                dst,
    size_t                                               maxlen);

struct sockaddr_storage* tcp_get_ip_from_str(const char* str,
