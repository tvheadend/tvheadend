...
static void spawn_reaper(void) {
  ...
  LIST_FOREACH (s, &spawns, link)
    if (s->killed && s->killed < mclk()) {
      ...
      if (r && errno == EPERM)
        tvherror(LS_SPAWN,
            "Unable to kill task pid %d (not enough permissions)",
            s->pid);
    }
  tvh_mutex_unlock(&spawn_mutex);
}
 ...
 *
 */
int spawn_parse_args(char*** argv,
    int                      argc,
    const char*              cmd,
    const char**             replace) {
  char *       s, *f, *p, *a;
...
/**
 * Execute the given program and return its standard output as file-descriptor
 * (pipe).
 */
  ...
  if (p == 0) {
    ...
    if ((selfd = opendir("/dev/fd")) != NULL) {
      while ((selfd_ent = readdir(selfd)) != NULL) {
        ...
        if (open_fd >= 3) {
          if (close(open_fd) == -1)
            spawn_error("failed to close fd '%d': %s",
                open_fd,
                strerror(errno));
        }
      }
...
/**
 * Execute the given program and return its standard input as file-descriptor
 * (pipe). The standard output file-decriptor (od) must be valid, too.
 */
