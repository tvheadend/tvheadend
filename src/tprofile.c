...
static void tprofile_time_replace(tprofile_time_t* tpt, const char* id, uint64_t t) {
  free(tpt->id);
  tpt->id = strdup(id);
  tpt->t  = t;
}

static void tprofile_avg_update(tprofile_avg_t* tpa, uint64_t val) {
...
static void qprofile_time_replace(qprofile_time_t* qpt, const char* id, uint64_t t, uint64_t pos) {
  free(qpt->id);
  qpt->id  = strdup(id);
  qpt->t   = t;
  qpt->pos = pos;
...
static inline void qprofile_avg_update(qprofile_avg_t* tpa, uint64_t val) {
...
static void tprofile_log_tstats(void) {
  ...
  for (tprof = LIST_FIRST(&tprofile_all); tprof; tprof = tprof_next) {
    ...
    if (tprof->changed == 0)
      continue;
    tvhtrace(LS_TPROF,
        "%s: max/avg/cnt=%" PRIu64 "ms/%" PRIu64 "ms/%" PRIu64 " (max=%s)%s",
        tprof->name,
        mono2ms(tprof->tmax.t),
        mono2ms(tprof->tavg.avg),
        tprof->tavg.count,
        tprof->tmax.id ?: "?",
...
static void qprofile_log_qstats(void) {
  ...
  for (qprof = LIST_FIRST(&qprofile_all); qprof; qprof = qprof_next) {
    ...
    if (diff == 0)
      diff = 1;
    tvhtrace(LS_QPROF,
        "%s: max/avg/cnt/drop/dropcnt=%" PRIu64 "/%" PRIu64 "/%" PRIu64 "/%" PRIu64 "/%" PRIu64
