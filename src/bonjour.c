...
static void bonjour_callback(CFNetServiceRef theService, CFStreamError* error, void* info) {
  if (error->error) {
    tvherror(LS_BONJOUR, "callback error (domain = %ld, error =%d)", error->domain, error->error);
  }
}

static void bonjour_start_service(CFNetServiceRef* svc,
  ...
  str = CFStringCreateWithCStringNoCopy(kCFAllocatorDefault,
      service_type,
  ...
  *svc = CFNetServiceCreate(NULL, CFSTR(""), str, CFSTR("Tvheadend"), port);
  ...
  CFNetServiceScheduleWithRunLoop(*svc, CFRunLoopGetCurrent(), kCFRunLoopCommonModes);
  ...
  if (!CFNetServiceRegisterWithOptions(*svc, 0, &error))
    tvherror(LS_BONJOUR,
        "registration failed (service type = %s, "
        "domain = %ld, error =%d)",
        service_type,
        error.domain,
        error.error);
  else
    tvherror(LS_BONJOUR, "service '%s' successfully established", service_type);
}

static void bonjour_stop_service(CFNetServiceRef* svc) {
  CFNetServiceUnscheduleFromRunLoop(*svc, CFRunLoopGetCurrent(), kCFRunLoopCommonModes);
  CFNetServiceSetClient(*svc, NULL, NULL);
  CFRelease(*svc);
}

void bonjour_init(void) {
  txt_rec_t txt_rec_http[] = {{"path", tvheadend_webroot ? tvheadend_webroot : "/"}, {.key = NULL}};

  if (tvheadend_webui_port > 0)
    bonjour_start_service(&svc_http, "_http._tcp", tvheadend_webui_port, txt_rec_http);
