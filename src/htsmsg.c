 ...
 *
 */
static void htsmsg_field_data_destroy(htsmsg_field_t* f) {
  switch (f->hmf_type) {
    case HMF_MAP:
    case HMF_LIST:
...
#if ENABLE_SLOW_MEMORYINFO
  memoryinfo_free(&htsmsg_field_memoryinfo, sizeof(htsmsg_field_t) + f->hmf_edata_size);
#endif
  free(f);
}
...
htsmsg_field_add(htsmsg_t* msg, const char* name, int type, int flags, size_t esize) {
  ...
  memoryinfo_alloc(&htsmsg_field_memoryinfo, sizeof(htsmsg_field_t) + f->hmf_edata_size);
#endif
  return f;
}
 ...
 *
 */
void htsmsg_concat(htsmsg_t* msg, htsmsg_t* sub) {
  if (sub == NULL)
    return;
 ...
 *
 */
static int htsmsg_field_set_uuid(htsmsg_field_t* f, tvh_uuid_t* u) {
 ...
 *
 */
static htsmsg_t* htsmsg_field_set_msg(htsmsg_field_t* f, htsmsg_t* sub) {
...
htsmsg_t* htsmsg_add_msg(htsmsg_t* msg, const char* name, htsmsg_t* sub) {
  ...
  f = htsmsg_field_add(msg, name, sub->hm_islist ? HMF_LIST : HMF_MAP, 0, sizeof(htsmsg_t));
...
int bool_check(const char* str) {
  if (str &&
      (!strcmp(str, "yes") || !strcmp(str, "true") || !strcmp(str, "on") || !strcmp(str, "1")))
    return 1;
  return 0;
}
 ...
 *
 */
int32_t htsmsg_get_s32_or_default(htsmsg_t* msg, const char* name, int32_t def) {
  int32_t s32;
 ...
 *
 */
int htsmsg_get_bin(htsmsg_t* msg, const char* name, const void** binp, size_t* lenp) {
  htsmsg_field_t* f;
 ...
 *
 */
static void htsmsg_print0(htsmsg_t* msg, int indent) {
  htsmsg_field_t* f;
  int             i;
...
static void htsmsg_copy_f(htsmsg_t* dst, const htsmsg_field_t* f, const char* name) {
  htsmsg_t* sub;
  ...
  switch (f->hmf_type) {
    ...
    case HMF_MAP:
    case HMF_LIST:
      sub = f->hmf_type == HMF_LIST ? htsmsg_create_list() : htsmsg_create_map();
...
void htsmsg_copy_field(htsmsg_t* dst,
    const char*                  dstname,
    const htsmsg_t*              src,
    const char*                  srcname) {
  htsmsg_field_t* f;
 ...
 */
static void htsmsg_add_str_ap(htsmsg_t* msg, const char* name, const char* fmt, va_list ap0) {
...
void htsmsg_add_str_printf(htsmsg_t* msg, const char* name, const char* fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
