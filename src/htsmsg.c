 ...
 *
 */
htsmsg_field_t* htsmsg_field_add(htsmsg_t* msg,
    const char*                            name,
    int                                    type,
    int                                    flags,
    size_t                                 esize) {
 ...
 *
 */
void htsmsg_add_bin(htsmsg_t* msg,
    const char*               name,
    const void*               bin,
    size_t                    len) {
 ...
 *
 */
void htsmsg_add_bin_alloc(htsmsg_t* msg,
    const char*                     name,
    const void*                     bin,
    size_t                          len) {
 ...
 *
 */
void htsmsg_add_bin_ptr(htsmsg_t* msg,
    const char*                   name,
    const void*                   bin,
    size_t                        len) {
  htsmsg_field_t* f = htsmsg_field_add(msg, name, HMF_BIN, 0, 0);
...
htsmsg_t* htsmsg_add_msg(htsmsg_t* msg, const char* name, htsmsg_t* sub) {
  ...
  f = htsmsg_field_add(msg,
      name,
      sub->hm_islist ? HMF_LIST : HMF_MAP,
      0,
      sizeof(htsmsg_t));
...
int bool_check(const char* str) {
  if (str &&
      (!strcmp(str, "yes") || !strcmp(str, "true") || !strcmp(str, "on") ||
          !strcmp(str, "1")))
    return 1;
  return 0;
}
 ...
 *
 */
int htsmsg_get_bin(htsmsg_t* msg,
    const char*              name,
    const void**             binp,
    size_t*                  lenp) {
  htsmsg_field_t* f;
...
htsmsg_copy_f(htsmsg_t* dst, const htsmsg_field_t* f, const char* name) {
  ...
  switch (f->hmf_type) {
    ...
    case HMF_LIST:
      sub =
          f->hmf_type == HMF_LIST ? htsmsg_create_list() : htsmsg_create_map();
 ...
 */
static void htsmsg_add_str_ap(htsmsg_t* msg,
    const char*                         name,
    const char*                         fmt,
    va_list                             ap0) {
...
void htsmsg_add_str_printf(htsmsg_t* msg,
    const char*                      name,
    const char*                      fmt,
    ...) {
  va_list ap;
