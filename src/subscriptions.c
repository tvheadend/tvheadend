...
static void subscription_link_service(th_subscription_t* s, service_t* t) {
  ...
  tvhtrace(LS_SUBSCRIPTION,
      "%04X: linking sub %p to svc %p type %i",
      shortid(s),
      s,
      t,
      t->s_type);
  ...
  if (elementary_set_has_streams(&t->s_components, 1) ||
      t->s_type != STYPE_STD) {
...
static void subscription_show_none(th_subscription_t* s) {
  ...
  if (s->ths_channel)
    tvh_strlcatf(buf,
        ...
        l,
        " to channel \"%s\"",
        s->ths_channel ? channel_get_name(s->ths_channel, channel_blank_name)
                       : "none");
#if ENABLE_MPEGTS
 ...
 *
 */
static service_instance_t* subscription_start_instance(th_subscription_t* s,
    int* error) {
  ...
  si = service_find_instance(s->ths_service,
      ...
      s->ths_flags,
      s->ths_timeout,
      mclk() > s->ths_postpone_end ? 0
                                   : mono2sec(s->ths_postpone_end - mclk()));
  if (si && (s->ths_flags & SUBSCRIPTION_CONTACCESS) == 0)
    mtimer_arm_rel(&s->ths_ca_check_timer,
        subscription_ca_check_cb,
        s,
        s->ths_ca_timeout);
  ...
  LIST_FOREACH (s, &subscriptions, ths_global_link) {
    ...
    if (si == NULL) {
      if (s->ths_last_error != error ||
        ...
        tvhtrace(LS_SUBSCRIPTION,
            "%04X: instance not available, retrying",
            shortid(s));
        if (s->ths_last_error != error)
...
static void subscription_input(void* opaque, streaming_message_t* sm) {
  ...
  if (sm->sm_type == SMT_SERVICE_STATUS &&
      (sm->sm_code &
          (TSS_TUNING | TSS_TIMEOUT | TSS_NO_DESCRAMBLER | TSS_CA_CHECK))) {
    error = tss2errcode(sm->sm_code);
    if (error != SM_CODE_OK)
      if (error != SM_CODE_NO_ACCESS ||
          (s->ths_flags & SUBSCRIPTION_CONTACCESS) == 0) {
 ...
 *
 */
static th_subscription_t* subscription_create_from_channel_or_service(
  ...
  *error = 0;
  ...
  if (tvhtrace_enabled()) {
    const char* pro_name =
        prch->prch_pro ? profile_get_name(prch->prch_pro) : "<none>";
    if (ch)
      tvhtrace(LS_SUBSCRIPTION,
      ...
      tvhtrace(LS_SUBSCRIPTION,
          "%04X: creating subscription for service %s weight %d using profile "
          "%s",
          shortid(s),
          service->s_nicename,
          weight,
...
static void subscription_status_callback(void* p) {
  ...
  LIST_FOREACH (s, &subscriptions, ths_global_link) {
    ...
    uint64_t in_prev =
        atomic_exchange_u64(&s->ths_total_bytes_in_prev, in_curr);
    uint64_t out_curr = atomic_get_u64(&s->ths_total_bytes_out);
    uint64_t out_prev =
        atomic_exchange_u64(&s->ths_total_bytes_out_prev, out_curr);
...
static streaming_ops_t dummy_ops = {.st_cb = dummy_callback,
    .st_info                               = dummy_info};
...
void subscription_dummy_join(const char* id, int first) {
  ...
  if (t == NULL) {
    tvherror(LS_SUBSCRIPTION,
        "Unable to dummy join %s, service not found, retrying...",
        id);
