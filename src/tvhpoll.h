...
static inline tvhpoll_event_t*
tvhpoll_event(tvhpoll_event_t* ev, int fd, uint32_t events, void* ptr) {
  ev->fd     = fd;
  ev->events = events;
  ev->ptr    = ptr;
  return ev;
...
static inline tvhpoll_event_t* tvhpoll_event1(tvhpoll_event_t* ev, int fd) {
  ev->fd     = fd;
  ev->events = 0;
  ev->ptr    = NULL;
  return ev;
