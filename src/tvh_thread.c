...
static TAILQ_HEAD(, tvh_mutex) thrwatch_mutexes = TAILQ_HEAD_INITIALIZER(
    thrwatch_mutexes);
static int64_t tvh_thread_crash_time;
#endif

#if ENABLE_TRACE
static void tvh_thread_mutex_failed(tvh_mutex_t* mutex,
    const char*                                  reason,
    const char*                                  filename,
    int                                          lineno);
#endif
...
#if ENABLE_TRACE
static void
tvh_mutex_check_magic(tvh_mutex_t* mutex, const char* filename, int lineno) {
  if (mutex && mutex->magic1 == TVH_THREAD_MUTEX_MAGIC1 &&
...
#if ENABLE_TRACE
static void
tvh_mutex_add_to_list(tvh_mutex_t* mutex, const char* filename, int lineno) {
...
int tvh_cond_timedwait(tvh_cond_t* cond,
    tvh_mutex_t*                   mutex,
    int64_t                        monoclock) {
  int r;
...
int tvh_cond_timedwait_ts(tvh_cond_t* cond,
    tvh_mutex_t*                      mutex,
    struct timespec*                  ts) {
  int r;
...
static void tvh_thread_deadlock_write(htsbuf_queue_t* q) {
  ...
  int       fd =
      hts_settings_open_file(HTS_SETTINGS_OPEN_WRITE | HTS_SETTINGS_OPEN_DIRECT,
          "mutex-deadlock.txt");
  if (fd < 0)
    fd = fd_stderr;
...
static void* tvh_thread_watch_thread(void* aux) {
  ...
  while (!atomic_get(&tvhwatch_done)) {
    ...
    pthread_mutex_unlock(&thrwatch_mutex);
    if (tvh_thread_debug == 12345678 &&
        tvh_thread_crash_time < getfastmonoclock()) {
      tvh_thread_debug--;
...
void tvh_thread_init(int debug_level) {
  ...
  if (debug_level > 0) {
    ...
    tvh_thread_create(&thrwatch_tid,
        NULL,
        tvh_thread_watch_thread,
        NULL,
        "thrwatch");
  }
#endif
