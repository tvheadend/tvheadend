...
typedef struct service_mapper_conf {
  ...
  int merge_same_name_fuzzy; ///< Merge entries with the same name with fuzzy
                             ///< matching (ignore case, etc)
  ...
  int network_tags;  ///< Create tags based on network name (useful for multi
                     ///< adapter equipments)
...
// Process one service
struct channel* service_mapper_process(const service_mapper_conf_t* conf,
    struct service*                                                 s,
    struct bouquet*                                                 bq);
