...
typedef struct epggrab_channel {
  ...
  idnode_list_head_t
      channels; ///< Mapped channels (1 = epggrab channel, 2 = channel)
...
void epggrab_channel_link_delete(epggrab_channel_t* ec,
    struct channel*                                 ch,
    int                                             delconf);
int  epggrab_channel_link(epggrab_channel_t* ec,
     struct channel*                         ch,
     void*                                   origin);
...
struct epggrab_module_int {
  ...
  int xmltv_scrape_onto_desc; ///< Include scraped actors
                              ///< and extra details on to programme description
                              ///< for viewing by legacy clients.
  int xmltv_use_category_not_genre; ///< Use category tags and don't map to DVB
                                    ///< genres.
