...
typedef void(parse_callback_t)(parser_t* t,
    parser_es_t*                         st,
    const uint8_t*                       data,
    int                                  len,
    int                                  start);
...
static inline int64_t getpts(const uint8_t* p) {
  int a = p[0];
  int b = (p[1] << 8) | p[2];
  int c = (p[3] << 8) | p[4];
  ...
  if ((a & 1) && (b & 1) && (c & 1)) {
    ...
    return ((int64_t)((a >> 1) & 0x07) << 30) | ((int64_t)((b >> 1)) << 15) |
        ((int64_t)((c >> 1)));
