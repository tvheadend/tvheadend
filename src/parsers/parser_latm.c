...
static int read_audio_specific_config(parser_es_t* st,
    latm_private_t*                                latm,
    bitstream_t*                                   bs) {
  int aot, sr, sri;
  ...
  if (sr < 7350 || sr > 96000 || latm->channel_config == 0 ||
      latm->channel_config > 7)
    return -1;
  ...
  if (aot == AOT_SBR ||
      (aot == AOT_PS &&
          !(show_bits(bs, 3) & 3 && !(show_bits(bs, 9) & 0x3f)))) {
