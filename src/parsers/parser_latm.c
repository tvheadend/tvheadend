...
static int read_audio_specific_config(parser_es_t* st, latm_private_t* latm, bitstream_t* bs) {
  int aot, sr, sri;
  ...
  if (sr < 7350 || sr > 96000 || latm->channel_config == 0 || latm->channel_config > 7)
    return -1;

  st->es_frame_duration = 1024 * 90000 / sr;

  latm->ext_sri = 0;
  if (aot == AOT_SBR || (aot == AOT_PS && !(show_bits(bs, 3) & 3 && !(show_bits(bs, 9) & 0x3f)))) {
...
static int read_stream_mux_config(parser_es_t* st, latm_private_t* latm, bitstream_t* bs) {
 ...
 * Parse AAC LATM
 */
th_pkt_t* parse_latm_audio_mux_element(parser_t* t, parser_es_t* st, const uint8_t* data, int len) {
  latm_private_t* latm;
  bitstream_t     bs, out;
  int             slot_len, tmp, i;
  uint8_t*        buf;
  ...
  th_pkt_t* pkt =
      pkt_alloc(st->es_type, NULL, slot_len + 7, st->es_curdts, st->es_curdts, t->prs_current_pcr);
