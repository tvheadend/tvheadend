...
static inline int is_ssc(uint32_t sc) {
  return (sc & ~0x0f) == 0x1e0;
}

static inline int pts_is_backlog(int64_t pts) {
  return pts != PTS_UNSET && (pts & PTS_BACKLOG) != 0;
}

static inline int64_t pts_no_backlog(int64_t pts) {
  return pts_is_backlog(pts) ? (pts & ~PTS_BACKLOG) : pts;
}

typedef int(packet_parser_t)(parser_t* t,
    parser_es_t*                       st,
    size_t                             len,
    uint32_t                           next_startcode,
    int                                sc_offset);
 ...
 *
 */
static th_pkt_t* parser_error_packet(parser_t* t, parser_es_t* st, int error) {
  th_pkt_t* pkt;

  pkt          = pkt_alloc(st->es_type, NULL, 0, PTS_UNSET, PTS_UNSET, t->prs_current_pcr);
  pkt->pkt_err = error;
  return pkt;
}
 ...
 * prs_rstlog
 */
static void parser_rstlog(parser_t* t, th_pkt_t* pkt) {
...
static void parser_deliver(parser_t* t, parser_es_t* st, th_pkt_t* pkt) {
  ...
  if (d > diff || d == PTS_UNSET) {
    if (d != PTS_UNSET && tvhlog_limit(&st->es_pcr_log, 2))
      tvhwarn(LS_PARSER,
          "%s: DTS and PCR diff is very big (%" PRId64 ")",
          st->es_nicename,
          pts_diff(pkt->pkt_pcr, pkt->pkt_pts));
...
static void parser_do_backlog(parser_t* t,
    parser_es_t*                        st,
 ...
 */
static int parse_pes_header(parser_t* t, parser_es_t* st, const uint8_t* buf, size_t len) {
...
err:
  ...
  if (tvhlog_limit(&st->es_pes_log, 10))
    tvhwarn(LS_TS, "%s Corrupted PES header (errors %zi)", st->es_nicename, st->es_pes_log.count);
  return -1;
}
 ...
 */
static void parse_pes(parser_t* t,
    parser_es_t*                st,
    const uint8_t*              data,
    int                         len,
    int                         start,
  ...
  if (start) {
    ...
    if (data[0] != 0 && data[1] != 0 && data[2] != 1)
      if (tvhlog_limit(&st->es_pes_log, 10))
        tvhwarn(LS_TS,
            "%s: Invalid start code %02x:%02x:%02x",
            st->es_nicename,
            data[0],
            data[1],
            data[2]);
  ...
  for (i = 0; i < len;) {
    ...
    if (st->es_header_mode) {
      ...
      if (off > 2)
        sc = st->es_buf.sb_data[off - 3] << 16 | st->es_buf.sb_data[off - 2] << 8 |
 ...
 */
static int
depacketize(parser_t* t, parser_es_t* st, size_t len, uint32_t next_startcode, int sc_offset) {
 ...
 *
 */
static void makeapkt(parser_t* t,
    parser_es_t*               st,
    const void*                buf,
    int                        len,
    int64_t                    dts,
...
static const int aac_sample_rates[16] = {96000,
    88200,
    64000,
    48000,
    44100,
 ...
 * Inspect ADTS header
 */
static int mp4a_valid_frame(const uint8_t* buf) {
 ...
 */
static void parse_aac(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
  int       l, muxlen, p, latm;
  th_pkt_t* pkt;
...
static inline int mpa_valid_frame(uint32_t h) {
  if ((h & 0xffe00000) != 0xffe00000)
    return 0; /* start bits */
  if ((h & (3 << 17)) == 0)
    return 0; /* unknown MPEG audio layer */
  if ((h & (15 << 12)) == (15 << 12))
...
static int parse_mpa123(parser_t* t, parser_es_t* st) {
  ...
  for (i = fsize2 = 0; i < len - 4; i++) {
    if (!mpa_valid_frame(h = RB32(buf + i)))
      continue;
    ...
    switch (layer) {
      case 1:
        fsize = (((br * 12000) / sr) + pad) * 4;
        break;
      case 2:
        fsize = ((br * 144000) / sr) + pad;
        break;
      case 3:
        fsize = ((br * 144000) / (sr << lsf)) + pad;
        break;
      default:
        fsize = 0;
    }
    ...
    ok:
      if (st->es_audio_version < layer) {
        tvhtrace(LS_PARSER,
            "mpeg audio version change %02d: val=%d (old=%d)",
            st->es_index,
            layer,
            st->es_audio_version);
      ...
      makeapkt(t, st, buf + i, fsize, dts, duration, channels, mpa_sri[(buf[i + 2] >> 2) & 3]);
      i += fsize - 1;
      fsize2 = fsize;
    }
  }
 ...
 */
static int
parse_mpa(parser_t* t, parser_es_t* st, size_t ilen, uint32_t next_startcode, int sc_offset) {
  int r;
...
static void parse_pes_mpa(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
 ...
 */
static int ac3_valid_frame(const uint8_t* buf) {
  uint_fast8_t bs;
...
static int
parse_ac3(parser_t* t, parser_es_t* st, size_t ilen, uint32_t next_startcode, int sc_offset) {
...
again:
  ...
  for (i = count = fsize2 = 0; i < len - 6; i++) {
    if (!(ver = ac3_valid_frame(p = buf + i)))
      continue;
    versions[ver - 1]++;
...
static void parse_pes_ac3(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
 ...
 */
static int
parse_mpeg2video_pic_start(parser_t* t, parser_es_t* st, int* frametype, bitstream_t* bs) {
  int pct;
...
void parser_set_stream_vparam(parser_es_t* st, int width, int height, int duration) {
  ...
  tvhtrace(LS_PARSER,
      "vparam %02d: w=%d h=%d d=%d (old w=%d h=%d d=%d meta=%d)",
      st->es_index,
      width,
      height,
      duration,
 ...
 */
static int parse_mpeg2video_seq_start(parser_t* t, parser_es_t* st, bitstream_t* bs) {
 ...
 *
 */
static int drop_trailing_zeroes(const uint8_t* buf, size_t len) {
 ...
 *
 */
static void parser_global_data_move(parser_es_t* st, const uint8_t* data, size_t len, int reset) {
  if (reset) {
    ...
    st->es_global_data = realloc(st->es_global_data, st->es_global_data_len + len2);
 ...
 */
static int
parse_mpeg2video(parser_t* t, parser_es_t* st, size_t len, uint32_t next_startcode, int sc_offset) {
  const uint8_t* buf = st->es_buf.sb_data + sc_offset;
  bitstream_t    bs;
  int            frametype;
  th_pkt_t*      pkt;
    ...
    case 0x01 ... 0xaf:
      ...
      if (next_startcode == 0x100 || next_startcode > 0x1af) {
        ...
        if (st->es_global_data) {
          pkt->pkt_meta      = pktbuf_make(st->es_global_data, metalen = st->es_global_data_len);
          st->es_global_data = NULL;
          st->es_global_data_len = 0;
        }
        ...
        if (metalen) {
          ...
          pkt->pkt_payload = pktbuf_make(st->es_buf.sb_data, st->es_buf.sb_ptr - 4);
...
static void
parse_pes_mpeg2video(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
 ...
 *
 */
static void parse_h264_backlog(parser_t* t, parser_es_t* st, th_pkt_t* pkt) {
...
static void parse_h264_deliver(parser_t* t, parser_es_t* st, th_pkt_t* pkt) {
...
static int
parse_h264(parser_t* t, parser_es_t* st, size_t len, uint32_t next_startcode, int sc_offset) {
  const uint8_t* buf = st->es_buf.sb_data + sc_offset;
  uint32_t       sc  = st->es_startcode;
...
static void parse_pes_h264(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
 ...
 */
static int
parse_hevc(parser_t* t, parser_es_t* st, size_t len, uint32_t next_startcode, int sc_offset) {
  const uint8_t* buf      = st->es_buf.sb_data + sc_offset;
  uint32_t       sc       = st->es_startcode;
  ...
  switch (nal_type) {
    ...
    case HEVC_NAL_CRA_NUT:
      ...
      st->es_curpkt =
          pkt_alloc(st->es_type, NULL, 0, st->es_curpts, st->es_curdts, t->prs_current_pcr);
  ...
  if (is_ssc(next_startcode) || ((next_startcode >> 1) & 0x3f) == HEVC_NAL_AUD) {
    /* Complete frame - new start code or delimiter */
    if (st->es_incomplete)
      return PARSER_HEADER;
    th_pkt_t* pkt     = st->es_curpkt;
...
static void parse_pes_hevc(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
 ...
 */
static void parse_subtitles(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
  th_pkt_t*      pkt;
  int            psize, hlen;
  const uint8_t* buf;
  const uint8_t* d;
  ...
  if (psize >= 6) {
    ...
    if (buf[psize - 1] == 0xff) {
      pkt =
          pkt_alloc(st->es_type, buf, psize - 1, st->es_curpts, st->es_curdts, t->prs_current_pcr);
      pkt->pkt_commercial = t->prs_tt_commercial_advice;
      pkt->pkt_err        = st->es_buf.sb_err;
      parser_deliver(t, st, pkt);
      sbuf_reset(&st->es_buf, 4000);
 ...
 * Teletext parser
 */
static void parse_teletext(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
  th_pkt_t*      pkt;
  int            psize, hlen;
  const uint8_t* buf;
  const uint8_t* d;
 ...
 */
static void parse_hbbtv(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
  th_pkt_t* pkt =
      pkt_alloc(st->es_type, data, len, t->prs_current_pcr, t->prs_current_pcr, t->prs_current_pcr);
  pkt->pkt_err = st->es_buf.sb_err;
  parser_deliver(t, st, pkt);
}
 ...
 * RDS parser
 */
static void parse_rds(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {
  th_pkt_t*      pkt;
  int            psize, hlen;
  const uint8_t* buf;
  const uint8_t* d;
...
#else
static inline int data_noise(const uint8_t* data, int len) {
  return 0;
}
#endif

static void parse_none(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start) {}

/**
 * Parse raw mpeg data - single TS packet
 */
void parse_mpeg_ts(parser_t* t, parser_es_t* st, const uint8_t* data, int len, int start, int err) {
  if (err) {
    if (start)
      parser_deliver_error(t, st);
