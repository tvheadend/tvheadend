...
static th_pkt_t* parser_error_packet(parser_t* t, parser_es_t* st, int error) {
  ...
  pkt =
      pkt_alloc(st->es_type, NULL, 0, PTS_UNSET, PTS_UNSET, t->prs_current_pcr);
  pkt->pkt_err = error;
  return pkt;
}
  ...
  while ((sm = TAILQ_FIRST(&st->es_backlog)) != NULL) {
    ...
    if (meta) {
      if (pkt->pkt_meta == NULL) {
        ...
        npkt->pkt_payload =
            pktbuf_alloc(NULL, metalen + pktbuf_len(pkt->pkt_payload));
 ...
 */
static int
parse_pes_header(parser_t* t, parser_es_t* st, const uint8_t* buf, size_t len) {
...
err:
  ...
  if (tvhlog_limit(&st->es_pes_log, 10))
    tvhwarn(LS_TS,
        "%s Corrupted PES header (errors %zi)",
        st->es_nicename,
        st->es_pes_log.count);
  return -1;
}
    ...
    if (st->es_header_mode) {
      ...
      if (off > 2)
        sc = st->es_buf.sb_data[off - 3] << 16 |
            st->es_buf.sb_data[off - 2] << 8 | st->es_buf.sb_data[off - 1];
 ...
 *
 */
static int depacketize(parser_t* t,
    parser_es_t*                 st,
    size_t                       len,
    uint32_t                     next_startcode,
    int                          sc_offset) {
...
static void makeapkt(parser_t* t,
    ...
    int                        sri) {
  th_pkt_t* pkt =
      pkt_alloc(st->es_type, buf, len, dts, dts, t->prs_current_pcr);
 ...
 */
static void parse_aac(parser_t* t,
    parser_es_t*                st,
    const uint8_t*              data,
    int                         len,
    int                         start) {
  ...
  while ((l = st->es_buf.sb_ptr - p) > 3) {
    ...
    if (d[0] == 0 && d[1] == 0 && d[2] == 1) {
      ...
      muxlen =
          ((uint16_t)(d[3] & 0x03) << 11) | ((uint16_t)d[4] << 3) | (d[5] >> 5);
      if (l < muxlen)
        break;
  ...
  for (i = fsize2 = 0; i < len - 4; i++) {
    ...
    ok:
      if (st->es_audio_version < layer) {
        ...
        service_update_elementary_stream(st->es_service,
 ...
 *
 */
static int parse_mpa(parser_t* t,
    parser_es_t*               st,
    size_t                     ilen,
    uint32_t                   next_startcode,
    int                        sc_offset) {
  ...
  if ((r = depacketize(t, st, ilen, next_startcode, sc_offset)) !=
      PARSER_APPEND)
    return r;
  return parse_mpa123(t, st);
}

static void parse_pes_mpa(parser_t* t,
    parser_es_t*                    st,
    const uint8_t*                  data,
    int                             len,
    int                             start) {
...
static int parse_ac3(parser_t* t,
    parser_es_t*               st,
    size_t                     ilen,
    uint32_t                   next_startcode,
    int                        sc_offset) {
  ...
  if ((i = depacketize(t, st, ilen, next_startcode, sc_offset)) !=
      PARSER_APPEND)
    return i;
...
static void parse_pes_ac3(parser_t* t,
    parser_es_t*                    st,
    const uint8_t*                  data,
    int                             len,
    int                             start) {
 ...
 */
static int parse_mpeg2video_pic_start(parser_t* t,
    parser_es_t*                                st,
    int*                                        frametype,
    bitstream_t*                                bs) {
  int pct;
 ...
 *
 */
void parser_set_stream_vparam(parser_es_t* st,
    int                                    width,
    int                                    height,
    int                                    duration) {
  int need_save = 0;
 ...
 */
static int
parse_mpeg2video_seq_start(parser_t* t, parser_es_t* st, bitstream_t* bs) {
 ...
 *
 */
static void parser_global_data_move(parser_es_t* st,
    const uint8_t*                               data,
    size_t                                       len,
    int                                          reset) {
  if (reset) {
    ...
    st->es_global_data =
        realloc(st->es_global_data, st->es_global_data_len + len2);
 ...
 *
 */
static int parse_mpeg2video(parser_t* t,
    parser_es_t*                      st,
    size_t                            len,
    uint32_t                          next_startcode,
    int                               sc_offset) {
    ...
    case 0x01 ... 0xaf:
      ...
      if (next_startcode == 0x100 || next_startcode > 0x1af) {
        ...
        if (st->es_global_data) {
          pkt->pkt_meta =
              pktbuf_make(st->es_global_data, metalen = st->es_global_data_len);
          st->es_global_data     = NULL;
          st->es_global_data_len = 0;
        }
        ...
        if (metalen) {
          pkt->pkt_payload =
              pktbuf_alloc(NULL, metalen + st->es_buf.sb_ptr - 4);
          memcpy(pktbuf_ptr(pkt->pkt_payload),
              pktbuf_ptr(pkt->pkt_meta),
              metalen);
          memcpy(pktbuf_ptr(pkt->pkt_payload) + metalen,
              st->es_buf.sb_data,
              st->es_buf.sb_ptr - 4);
        } else {
          pkt->pkt_payload =
              pktbuf_make(st->es_buf.sb_data, st->es_buf.sb_ptr - 4);
...
static void parse_pes_mpeg2video(parser_t* t,
    parser_es_t*                           st,
    const uint8_t*                         data,
    int                                    len,
    int                                    start) {
...
static int parse_h264(parser_t* t,
    parser_es_t*                st,
    size_t                      len,
    uint32_t                    next_startcode,
    int                         sc_offset) {
    ...
    if (len >= 9) {
      ...
      if (pkt) {
        if (l2 < len - 6 && l2 + 7 != len) {
          ...
          pkt->pkt_payload =
              pktbuf_append(pkt->pkt_payload, buf + 6 + l2, len - 6 - l2);
        }
...
static void parse_pes_h264(parser_t* t,
    parser_es_t*                     st,
    const uint8_t*                   data,
    int                              len,
    int                              start) {
 ...
 *
 */
static int parse_hevc(parser_t* t,
    parser_es_t*                st,
    size_t                      len,
    uint32_t                    next_startcode,
    int                         sc_offset) {
  ...
  switch (nal_type) {
    ...
    case HEVC_NAL_CRA_NUT:
      ...
      st->es_curpkt                  = pkt_alloc(st->es_type,
          NULL,
          0,
          st->es_curpts,
          st->es_curdts,
...
static void parse_pes_hevc(parser_t* t,
    parser_es_t*                     st,
    const uint8_t*                   data,
    int                              len,
    int                              start) {
 ...
 */
static void parse_subtitles(parser_t* t,
    parser_es_t*                      st,
    const uint8_t*                    data,
    int                               len,
    int                               start) {
 ...
 * Teletext parser
 */
static void parse_teletext(parser_t* t,
    parser_es_t*                     st,
    const uint8_t*                   data,
    int                              len,
    int                              start) {
 ...
 */
static void parse_hbbtv(parser_t* t,
    parser_es_t*                  st,
    const uint8_t*                data,
    int                           len,
    int                           start) {
  th_pkt_t* pkt = pkt_alloc(st->es_type,
      data,
      len,
      t->prs_current_pcr,
      t->prs_current_pcr,
 ...
 * RDS parser
 */
static void parse_rds(parser_t* t,
    parser_es_t*                st,
    const uint8_t*              data,
    int                         len,
    int                         start) {
...
static void parse_none(parser_t* t,
    parser_es_t*                 st,
    const uint8_t*               data,
    int                          len,
    int                          start) {}
