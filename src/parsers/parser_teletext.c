...
#define bitreverse(b) (((b) * 0x0202020202ULL & 0x010884422010ULL) % 1023)
 ...
 */
static const uint16_t teletext_subset[SUBSET_LAST + 1][SUBSET_CHARMAP_COUNT] = {{0x0023,
                                                                                    0x0024,
                                                                                    0x0040,
                                                                                    0x005B,
                                                                                    0x005C,
 ...
 */
static const uint16_t latin_g2[96] = {
    /* 0x20 */ 0x00A0,
    0x00A1,
    0x00A2,
    0x00A3,
 ...
 */
static const uint16_t cyrillic_1_g0[64] = {
    /* 0x40 */ 0x0427,
    0x0410,
    0x0411,
    0x0426,
 ...
 */
static const uint16_t cyrillic_2_g0[64] = {
    /* 0x40 */ 0x042E,
    0x0410,
    0x0411,
    0x0426,
 ...
 */
static const uint16_t cyrillic_3_g0[64] = {
    /* 0x40 */ 0x042E,
    0x0410,
    0x0411,
    0x0426,
 ...
 */
static const uint16_t cyrillic_g2[96] = {
    /* 0x20 */ 0x00A0,
    0x00A1,
    0x00A2,
    0x00A3,
 ...
 */
static const uint16_t greek_g0[64] = {
    /* 0x40 */ 0x0390,
    0x0391,
    0x0392,
    0x0393,
 ...
 */
static const uint16_t greek_g2[96] = {
    /* 0x20 */ 0x00A0,
    0x0061,
    0x0062,
    0x00A3,
 ...
 */
static const uint16_t arabic_g0[96] = {
    /* 0x20 */ 0x0020,
    0x0021,
    0x0022,
    0x00A3,
 ...
 */
static const uint16_t arabic_g2[96] = {
    /* 0x20 */ 0x0000,
    0x0000,
    0x0000,
    0x0000,
 ...
 */
static const uint16_t hebrew_g0[37] = {
    /* 0x5b */ 0x2190,
    0x00BD,
    0x2192,
    0x2191,
 ...
 */
static uint16_t teletext_to_ucs2(uint8_t cs, uint8_t ss, uint8_t c) {
  int i;
...
static inline uint8_t ham_decode8(uint8_t a) {
  return hamtable[a] & 0xf;
}

static uint8_t ham_decode16(uint8_t* p) {
  return (ham_decode8(p[1]) << 4) | ham_decode8(p[0]);
}

static uint32_t ham_decode24(uint8_t* p) {
...
static int is_tt_clock(const uint8_t* str) {
  return isdigit(str[0]) && isdigit(str[1]) && str[2] == ':' && isdigit(str[3]) &&
      isdigit(str[4]) && str[5] == ':' && isdigit(str[6]) && isdigit(str[7]);
}

/**
 *
 */
static int update_tt_clock(parser_t* t, const uint8_t* buf) {
  uint8_t str[10];
  int     i;
  time_t  ti;
...
static uint8_t* get_cset(uint8_t off) {
...
static int extract_subtitle(parser_t* t, parser_es_t* st, tt_mag_t* ttm, int64_t pts) {
  int      i, j, start, off                                   = 0;
...
static void tt_subtitle_deliver(parser_t* t, parser_es_t* parent, tt_mag_t* ttm) {
  ...
  TAILQ_FOREACH (es, &t->prs_components.set_filter, es_filter_link) {
    ...
    if (st->es_type != SCT_TEXTSUB)
      continue;
    if (parent->es_pid == st->es_parent_pid && ttm->ttm_curpage == st->es_pid - PID_TELETEXT_BASE) {
 ...
 *
 */
static void tt_decode_line(parser_t* t, parser_es_t* st, uint8_t* buf) {
  ...
  switch (line) {
    case 0:
      ...
      ttm->ttm_national = ((c >> 7) & 1) | ((c >> 5) & 2) | ((c >> 3) & 4);
    ...
    case 28:
    case 29:
      if (ham_decode8(buf[2]) != 0)
        break; /* designation must be 0 */
...
static void teletext_rundown_scan(parser_t* prs, tt_private_t* ttp) {
  int                 i;
  uint8_t*            l;
  mpegts_service_t*   t;
  ...
  if (t->s_dvb_svcname && strcmp("TV4", t->s_dvb_svcname) && strcmp("TV4 HD", t->s_dvb_svcname))
    return;
