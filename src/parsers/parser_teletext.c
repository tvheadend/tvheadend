 ...
 */
static const uint16_t teletext_subset[SUBSET_LAST + 1][SUBSET_CHARMAP_COUNT] = {
    {0x0023,
        0x0024,
        0x0040,
        0x005B,
...
static const uint8_t* COLOR_MAP[8] = {
    // black (grey for visibility), red, green, yellow, blue, magenta, cyan,
    // white
...
static int is_tt_clock(const uint8_t* str) {
  return isdigit(str[0]) && isdigit(str[1]) && str[2] == ':' &&
      isdigit(str[3]) && isdigit(str[4]) && str[5] == ':' && isdigit(str[6]) &&
      isdigit(str[7]);
}
...
static void tt_decode_line(parser_t* t, parser_es_t* st, uint8_t* buf) {
  ...
  switch (line) {
    case 0:
      ...
      ttm->ttm_current_pts =
          t->prs_current_pcr + (int64_t)st->es_service->s_pts_shift * 900;
      break;
    ...
    case 29:
      ...
      /* extract the character set designation and national option selection
       * bits */
 ...
 *
 */
void teletext_input(parser_t* t,
    parser_es_t*              st,
    const uint8_t*            data,
    int                       len) {
  int     j;
...
static void teletext_rundown_scan(parser_t* prs, tt_private_t* ttp) {
  ...
  if (t->s_dvb_svcname && strcmp("TV4", t->s_dvb_svcname) &&
