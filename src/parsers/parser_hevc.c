...
static int
hvcc_parse_ptl(bitstream_t* gb, HEVCDecoderConfigurationRecord* hvcc, unsigned int max_sub_layers) {
  unsigned int         i;
...
static int
skip_hrd_parameters(bitstream_t* gb, uint8_t cprms_present_flag, unsigned int max_sub_layers) {
  unsigned int i;
...
static void
hvcc_parse_vui(bitstream_t* gb, HEVCDecoderConfigurationRecord* hvcc, unsigned int max_sub_layers) {
  ...
  if (read_bits1(gb)) { // bitstream_restriction_flag
    ...
    hvcc->min_spatial_segmentation_idc =
        MIN(hvcc->min_spatial_segmentation_idc, min_spatial_segmentation_idc);
...
static int hvcc_parse_vps(bitstream_t* gb, HEVCDecoderConfigurationRecord* hvcc) {
...
static int parse_rps(bitstream_t* gb,
...
static int hvcc_parse_sps(bitstream_t* gb, HEVCDecoderConfigurationRecord* hvcc) {
...
static int hvcc_parse_pps(bitstream_t* gb, HEVCDecoderConfigurationRecord* hvcc) {
...
static int
hvcc_add_nal_unit(uint8_t* nal_buf, uint32_t nal_size, HEVCDecoderConfigurationRecord* hvcc) {
  int         ret = 0;
  bitstream_t bs;
  uint8_t     nal_type;
  uint8_t*    rbsp_buf;
...
static int hvcc_write(sbuf_t* pb, HEVCDecoderConfigurationRecord* hvcc) {
  ...
  tvhtrace(LS_HEVC, "configurationVersion:                %" PRIu8, hvcc->configurationVersion);
  tvhtrace(LS_HEVC, "general_profile_space:               %" PRIu8, hvcc->general_profile_space);
  tvhtrace(LS_HEVC, "general_tier_flag:                   %" PRIu8, hvcc->general_tier_flag);
  tvhtrace(LS_HEVC, "general_profile_idc:                 %" PRIu8, hvcc->general_profile_idc);
  tvhtrace(LS_HEVC,
  ...
  tvhtrace(LS_HEVC, "general_level_idc:                   %" PRIu8, hvcc->general_level_idc);
  tvhtrace(LS_HEVC,
      "min_spatial_segmentation_idc:        %" PRIu16,
      hvcc->min_spatial_segmentation_idc);
  tvhtrace(LS_HEVC, "parallelismType:                     %" PRIu8, hvcc->parallelismType);
  tvhtrace(LS_HEVC, "chromaFormat:                        %" PRIu8, hvcc->chromaFormat);
  tvhtrace(LS_HEVC, "bitDepthLumaMinus8:                  %" PRIu8, hvcc->bitDepthLumaMinus8);
  tvhtrace(LS_HEVC, "bitDepthChromaMinus8:                %" PRIu8, hvcc->bitDepthChromaMinus8);
  tvhtrace(LS_HEVC, "avgFrameRate:                        %" PRIu16, hvcc->avgFrameRate);
  tvhtrace(LS_HEVC, "constantFrameRate:                   %" PRIu8, hvcc->constantFrameRate);
  tvhtrace(LS_HEVC, "numTemporalLayers:                   %" PRIu8, hvcc->numTemporalLayers);
  tvhtrace(LS_HEVC, "temporalIdNested:                    %" PRIu8, hvcc->temporalIdNested);
  tvhtrace(LS_HEVC, "lengthSizeMinusOne:                  %" PRIu8, hvcc->lengthSizeMinusOne);
  tvhtrace(LS_HEVC, "numOfArrays:                         %" PRIu8, hvcc->numOfArrays);
  for (i = 0; i < hvcc->numOfArrays; i++) {
    tvhtrace(LS_HEVC,
        "NAL_unit_type[%" PRIu8 "]:                    %" PRIu8,
        i,
        hvcc->array[i].NAL_unit_type);
    tvhtrace(LS_HEVC,
        "numNalus[%" PRIu8 "]:                         %" PRIu16,
        i,
        hvcc->array[i].numNalus);
  ...
  if (!vps_count || vps_count > MAX_VPS_COUNT || !sps_count || sps_count > MAX_SPS_COUNT ||
   ...
   */
  sbuf_put_byte(pb,
      hvcc->constantFrameRate << 6 | hvcc->numTemporalLayers << 3 | hvcc->temporalIdNested << 2 |
  ...
  for (i = 0; i < hvcc->numOfArrays; i++) {
     ...
     */
    sbuf_put_byte(pb, (0 << 7) | (hvcc->array[i].NAL_unit_type & 0x3f));
    ...
    for (j = 0; j < hvcc->array[i].numNalus; j++) {
      ...
      sbuf_append(pb, hvcc->array[i].nalUnit[j], hvcc->array[i].nalUnitLength[j]);
    }
  }
...
static int hevc_decode_vui(hevc_vui_t* vui, bitstream_t* bs) {
...
int hevc_decode_sps(parser_es_t* st, bitstream_t* bs) {
  ...
  if (!vps->num_units_in_tick && !vps->time_scale && !sps->vui.num_units_in_tick &&
      !sps->vui.time_scale)
    return -1;
  ...
  log2_ctb_size = log2_min_cb_size + log2_diff_max_min_coding_block_size;
  if (log2_ctb_size > 18)
    return -1;
