...
static int skip_hrd_parameters(bitstream_t* gb,
  ...
  for (i = 0; i < max_sub_layers; i++) {
    ...
    if (nal_hrd_parameters_present_flag)
      skip_sub_layer_hrd_parameters(gb,
          cpb_cnt,
...
static int hvcc_parse_sps(bitstream_t* gb,
  ...
  unsigned int num_short_term_ref_pic_sets,
      num_delta_pocs[MAX_SHORT_TERM_RPS_COUNT];
...
static uint8_t*
nal_unit_extract_rbsp(const uint8_t* src, uint32_t src_len, uint32_t* dst_len) {
  uint8_t* dst;
  uint32_t i, len;
...
static int hvcc_add_nal_unit(uint8_t* nal_buf,
    uint32_t                          nal_size,
    HEVCDecoderConfigurationRecord*   hvcc) {
  int         ret = 0;
  bitstream_t bs;
...
static int hvcc_write(sbuf_t* pb, HEVCDecoderConfigurationRecord* hvcc) {
  ...
  if (!vps_count || vps_count > MAX_VPS_COUNT || !sps_count ||
      sps_count > MAX_SPS_COUNT || !pps_count || pps_count > MAX_PPS_COUNT)
    return -1;
   ...
   */
  sbuf_put_byte(pb,
      hvcc->general_profile_space << 6 | hvcc->general_tier_flag << 5 |
          hvcc->general_profile_idc);
   ...
   */
  sbuf_put_byte(pb,
      hvcc->constantFrameRate << 6 | hvcc->numTemporalLayers << 3 |
...
th_pkt_t* hevc_convert_pkt(th_pkt_t* src) {
  ...
  avc_parse_nal_units(&payload,
      pktbuf_ptr(src->pkt_payload),
...
int hevc_decode_slice_header(parser_es_t* st, bitstream_t* bs, int* pkttype) {
  ...
  if (sps->vui.time_scale) {
    d = 180000 * (uint64_t)sps->vui.num_units_in_tick /
        (uint64_t)sps->vui.time_scale;
