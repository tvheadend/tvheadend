...
static void ts_recv_pcr(parser_t* t, const uint8_t* tsb) {
  ...
  if (t->prs_current_pcr == PTS_UNSET || t->prs_current_pcr_guess ||
      ...
      (t->prs_candidate_pcr != PTS_UNSET &&
          pts_diff(t->prs_candidate_pcr, pcr) <=
              (int64_t)t->prs_pcr_boundary)) {
 ...
 */
static void
ts_recv_pcr_audio(parser_t* t, parser_es_t* st, const uint8_t* buf, int len) {
  int64_t dts, pts, d;
  int     hdr, flags, hlen;
...
static void parser_input_mpegts(parser_t* prs, pktbuf_t* pb) {
  ...
  for (; len > 0; tsb += 188, len -= 188) {
    ...
    if (pusi && !error && off < 188 - 16 && tsb[off] == 0 &&
        tsb[off + 1] == 0 && tsb[off + 2] == 1 && SCT_ISAUDIO(pes->es_type)) {
      ts_recv_pcr_audio(prs, pes, tsb + off, 188 - off);
    } else if (pes->es_type == SCT_HBBTV) {
      dvb_table_parse(&pes->es_psi,
          "parser",
          tsb,
          188,
          1,
...
static streaming_ops_t parser_input_ops = {.st_cb = parser_input,
 ...
 * Parser create
 */
streaming_target_t* parser_create(streaming_target_t* output,
