 ...
 *
 */
static inline parser_es_t* parser_find_es(parser_t* prs, int pid) {
  ...
  if (t->prs_current_pcr == PTS_UNSET || t->prs_current_pcr_guess ||
    ...
    if (pcr != t->prs_current_pcr) {
      if (t->prs_current_pcr == PTS_UNSET)
        ...
        tvhtrace(LS_PCR,
            "%s: change   : %" PRId64 "%s",
            service_nicename(t->prs_service),
            pcr,
 ...
 */
static void ts_recv_pcr_audio(parser_t* t, parser_es_t* st, const uint8_t* buf, int len) {
  int64_t dts, pts, d;
  int     hdr, flags, hlen;
...
set:
  ...
  tvhtrace(LS_PCR,
      "%s: audio DTS: %" PRId64 " dts %" PRId64 " [%s/%d]",
      service_nicename(t->prs_service),
      t->prs_current_pcr,
      dts,
 ...
 * HBBTV
 */
static void ts_recv_hbbtv1_cb(mpegts_psi_table_t* mt, const uint8_t* buf, int len) {
...
static void parser_input_mpegts(parser_t* prs, pktbuf_t* pb) {
  ...
  for (; len > 0; tsb += 188, len -= 188) {
    ...
    if (tsb[3] & 0x20) {
      off = tsb[4] + 5;
      if (pes->es_pid == prs->prs_pcr_pid && !error && off > 10 && (tsb[5] & 0x10) != 0 &&
          off <= 188)
        ts_recv_pcr(prs, tsb);
    } else {
      off = 4;
    ...
    if (pusi && !error && off < 188 - 16 && tsb[off] == 0 && tsb[off + 1] == 0 &&
        tsb[off + 2] == 1 && SCT_ISAUDIO(pes->es_type)) {
...
static void parser_init_es(parser_es_t* pes, parser_t* prs) {
  ...
  if (pes->es_type == SCT_HBBTV && pes->es_psi.mt_name == NULL)
    dvb_table_parse_init(&pes->es_psi,
        "hbbtv",
        LS_TS,
        pes->es_pid,
        DVB_HBBTV_BASE,
...
static void parser_do_rstlog(parser_t* prs) {
 ...
 *
 */
static void parser_input(void* opaque, streaming_message_t* sm) {
  parser_t* prs = opaque;
...
static htsmsg_t* parser_input_info(void* opaque, htsmsg_t* list) {
  parser_t*           prs = opaque;
 ...
 * Parser create
 */
streaming_target_t* parser_create(streaming_target_t* output, th_subscription_t* ts) {
