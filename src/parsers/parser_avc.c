...
int isom_write_avcc(sbuf_t* sb, const uint8_t* data, int len) {
  ...
  if (RB32(data) != 0x00000001 && RB24(data) != 0x000001) {
  ...
  while (end - buf > 4) {
    ...
    if ((nal_type == H264_NAL_SPS) && (size >= 4) && (size <= UINT16_MAX)) {
      ...
      sps_size_array =
          realloc(sps_size_array, sizeof(uint32_t) * (sps_count + 1));
      sps_array[sps_count]      = buf;
      sps_size_array[sps_count] = size;
      sps_count++;
    } else if ((nal_type == H264_NAL_PPS) && (size <= UINT16_MAX)) {
      pps_size_array =
          realloc(pps_size_array, sizeof(uint32_t) * (pps_count + 1));
  ...
  sbuf_put_byte(sb,
      0xe0 +
          sps_count); /* 3 bits reserved (111) + 5 bits number of sps (00001) */
