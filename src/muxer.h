...
#define MC_CAP_ANOTHER_SERVICE \
  (1 << 0) /* I can stream another service (SID must match!) */
...
typedef struct muxer {
  ...
  int (*m_open_file)(struct muxer*,
      const char* filename);           /* Open for file storage */
...
static inline int
muxer_init(muxer_t* m, struct streaming_start* ss, const char* name) {
  if (m && ss)
    return m->m_init(m, ss, name);
  return -1;
}

static inline int muxer_reconfigure(muxer_t* m,
...
static inline int
muxer_write_meta(muxer_t* m, struct epg_broadcast* eb, const char* comment) {
  if (m)
    return m->m_write_meta(m, eb, comment);
  return -1;
}
...
static inline const char* muxer_mime(muxer_t* m,
    const struct streaming_start*             ss) {
  if (m && ss)
