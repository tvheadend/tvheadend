...
typedef struct htsp_connection {
  ...
  int64_t htsp_epg_window; // only send async epg updates within this window
                           // (seconds)
...
static void htsp_trace(htsp_connection_t* htsp,
    int                                   subsystem,
    const char*                           prefix,
    htsmsg_t*                             m) {
  htsbuf_queue_t q;
...
static void htsp_disable_stream(htsp_subscription_t* hs, unsigned int id) {
  if (id < NUM_FILTERED_STREAMS)
    hs->hs_filtered_streams[id / 64] |= 1 << (id & 63);
}

static void htsp_enable_stream(htsp_subscription_t* hs, unsigned int id) {
  if (id < NUM_FILTERED_STREAMS)
    hs->hs_filtered_streams[id / 64] &= ~(1 << (id & 63));
}

static inline int htsp_is_stream_enabled(htsp_subscription_t* hs,
    unsigned int                                              id) {
  if (id < NUM_FILTERED_STREAMS)
...
static inline int htsp_anonymize(htsp_connection_t* htsp) {
 ...
 *
 */
static void htsp_update_logname(htsp_connection_t* htsp) {
  char buf[100];
 ...
 *
 */
static void htsp_msg_destroy(htsp_msg_t* hm) {
 ...
 *
 */
static void htsp_init_queue(htsp_msg_q_t* hmq, int strict_prio) {
 ...
 *
 */
static const char* htsp_image(htsp_connection_t* htsp,
    const char*                                  image,
    char*                                        buf,
    size_t                                       buflen,
    int                                          version) {
 ...
 *
 */
static void htsp_subscription_destroy(htsp_connection_t* htsp,
 ...
 *
 */
static void htsp_subscription_free(htsp_connection_t* htsp,
    htsp_subscription_t*                              hs) {
  LIST_REMOVE(hs, hs_link);
  htsp_flush_queue(htsp, &hs->hs_q, 1);
  free(hs);
 ...
 *
 */
static void htsp_send(htsp_connection_t* htsp,
    htsmsg_t*                            m,
    pktbuf_t*                            pb,
    htsp_msg_q_t*                        hmq,
    int                                  payloadsize) {
 ...
 *
 */
static void htsp_send_subscription(htsp_connection_t* htsp,
    htsmsg_t*                                         m,
    pktbuf_t*                                         pb,
    htsp_subscription_t*                              hs,
    int                                               payloadsize) {
 ...
 */
static htsmsg_t* htsp_error(htsp_connection_t* htsp, const char* errstr) {
 ...
 */
static htsmsg_t* htsp_success(void) {
  htsmsg_t* r = htsmsg_create_map();
 ...
 *
 */
static void htsp_reply(htsp_connection_t* htsp, htsmsg_t* in, htsmsg_t* out) {
  uint32_t seq;
 ...
 * Update challenge
 */
static int htsp_generate_challenge(htsp_connection_t* htsp) {
  int fd, n;
 ...
 */
static inline int htsp_user_access_channel(htsp_connection_t* htsp,
    channel_t*                                                ch) {
  if (!ch || !ch->ch_enabled ||
      LIST_FIRST(&ch->ch_services) ==
          NULL) /* Don't pass unplayable channels to clients */
    return 0;
  if (!htsp)
    return 1;
...
static const char* htsp_dvr_config_name(htsp_connection_t* htsp,
  ...
  if (!cfg && perm->aa_username)
    tvhinfo(LS_HTSP,
        "User '%s' has no valid dvr config in ACL, using default...",
        perm->aa_username);
 ...
 */
static htsmsg_t* htsp_serierec_convert(htsp_connection_t* htsp,
    htsmsg_t*                                             in,
    channel_t*                                            ch,
    int                                                   autorec,
    int                                                   add) {
  ...
  if (autorec) { // autorec specific
    ...
    if (!(retval = htsmsg_get_s64(in, "startExtra", &s64)) || add)
      htsmsg_add_s64(conf,
          "start_extra",
          !retval ? (s64 < 0 ? 0 : s64) : 0); // 0 = dvr config
    ...
    if (add) { // for add, stay compatible with older "approxTime
      ...
      htsmsg_add_s32(conf,
          "start_window",
          start_window >= 0 ? start_window : -1); // -1 = any duration
 ...
 *
 */
static void htsp_serialize_epnum(htsmsg_t* out,
...
static channel_tag_t* htsp_channel_tag_find_by_id(htsp_connection_t* htsp,
    uint32_t                                                         id) {
  channel_tag_t* ct;
 ...
 *
 */
static htsmsg_t* htsp_file_open(htsp_connection_t* htsp,
    const char*                                    path,
    int                                            fd,
    dvr_entry_t*                                   de) {
  struct stat st;
  ...
  if (de) {
    ...
    hf->hf_subscription = subscription_create_from_file("HTSP",
        charset,
        path,
        htsp->htsp_peername,
        htsp->htsp_granted_access->aa_representative,
 ...
 *
 */
static htsp_file_t* htsp_file_find(const htsp_connection_t* htsp,
    htsmsg_t*                                               in) {
  htsp_file_t* hf;
 ...
 *
 */
static void htsp_file_destroy(htsp_file_t* hf) {
...
static void htsp_file_update_stats(htsp_file_t* hf, size_t len) {
...
htsp_build_channel(channel_t* ch, const char* method, htsp_connection_t* htsp) {
  ...
  if ((icon = channel_get_icon(ch)))
    htsmsg_add_str(out,
        "channelIcon",
        htsp_image(htsp, icon, buf, sizeof(buf), 8));
  ...
  LIST_FOREACH (ilm, &ch->ch_services, ilm_in2_link) {
    ...
    htsmsg_add_u32(svcmsg,
        "content",
        service_is_tv(t) ? 0x01 : (service_is_radio(t) ? 0x02 : 0x00));

    if (service_is_encrypted(t)) {
      htsmsg_add_u32(svcmsg, "caid", 65535);
      htsmsg_add_str(svcmsg,
          "caname",
          tvh_gettext_lang(htsp->htsp_language, N_("Encrypted service")));
    }
 ...
 *
 */
static htsmsg_t* htsp_build_tag(htsp_connection_t* htsp,
    channel_tag_t*                                 ct,
    const char*                                    method,
  ...
  if (!strempty(icon))
    htsmsg_add_str(out,
        "tagIcon",
        htsp_image(htsp, icon, buf, sizeof(buf), 34));
 ...
 *
 */
static htsmsg_t* htsp_build_dvrentry(htsp_connection_t* htsp,
    dvr_entry_t*                                        de,
    const char*                                         method,
    const char*                                         lang,
    int                                                 statsonly) {
  ...
  if (!statsonly) {
    ...
    if (de->de_autorec)
      htsmsg_add_str(out,
          "autorecId",
          idnode_uuid_as_str(&de->de_autorec->dae_id, ubuf));

    if (de->de_timerec)
      htsmsg_add_str(out,
          "timerecId",
          idnode_uuid_as_str(&de->de_timerec->dte_id, ubuf));
    ...
    if (htsp->htsp_version > 24)
      ...
      htsmsg_add_u32(out,
          "retention",
          dvr_entry_get_retention_days(de) == DVR_RET_ONREMOVE
    ...
    if (htsp->htsp_version > 36) {
      ...
      if (epggrab_conf.epgdb_processparentallabels) {
        // If this is still scheduled (in the future) then send the current
        // values, if not, send the 'saved' values.
        ...
        if (de->de_sched_state == DVR_SCHEDULED) {
          if (de->de_rating_label) {
            if (de->de_rating_label->rl_display_label) {
              htsmsg_add_str(out,
                  "ratingLabel",
                  de->de_rating_label->rl_display_label);
            }
    ...
    if (!strempty(image))
      htsmsg_add_str(out,
          "image",
          htsp_image(htsp, image, buf, sizeof(buf), 34));
 ...
 *
 */
static htsmsg_t* htsp_build_autorecentry(htsp_connection_t* htsp,
  ...
  if (htsp->htsp_version > 24)
    ...
    htsmsg_add_u32(out,
        "retention",
        dvr_autorec_get_retention_days(dae) == DVR_RET_ONREMOVE
            ? dvr_autorec_get_removal_days(dae)
            : dvr_autorec_get_retention_days(dae));
  ...
  htsmsg_add_s32(out,
      "approxTime",
      dae->dae_start >= 0 && tad >= 0 ? ((dae->dae_start + tad / 2) % (24 * 60))
                                      : -1);
  htsmsg_add_s32(out, "start", dae->dae_start >= 0 ? dae->dae_start : -1);
  htsmsg_add_s32(out,
      "startWindow",
      dae->dae_start_window >= 0 ? dae->dae_start_window : -1);
 ...
 *
 */
static htsmsg_t* htsp_build_timerecentry(htsp_connection_t* htsp,
  ...
  if (htsp->htsp_version > 24)
    ...
    htsmsg_add_u32(out,
        "retention",
        dvr_timerec_get_retention_days(dte) == DVR_RET_ONREMOVE
            ? dvr_timerec_get_removal_days(dte)
            : dvr_timerec_get_retention_days(dte));
 ...
 *
 */
static htsmsg_t* htsp_build_event(epg_broadcast_t* e,
    const char*                                    method,
    const char*                                    lang,
    time_t                                         update,
  ...
  if (!strempty(e->image))
    htsmsg_add_str(out,
        "image",
        htsp_image(htsp, e->image, buf, sizeof(buf), 34));
 ...
 */
static htsmsg_t* htsp_method_hello(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t*   r;
  uint32_t    v;
  ...
  tvhinfo(LS_HTSP,
      "%s: Welcomed client software: %s (HTSPv%d)",
      htsp->htsp_logname,
      name,
      v);
 ...
 * Try to authenticate
 */
static htsmsg_t* htsp_method_authenticate(htsp_connection_t* htsp,
    htsmsg_t*                                                in) {
  htsmsg_t* r = htsmsg_create_map();
  ...
  if (!(htsp->htsp_granted_access->aa_rights & HTSP_PRIV_MASK))
    ...
    htsmsg_add_u32(r,
        "admin",
        htsp->htsp_granted_access->aa_rights & ACCESS_ADMIN ? 1 : 0);
    htsmsg_add_u32(r,
        "streaming",
        htsp->htsp_granted_access->aa_rights & ACCESS_HTSP_STREAMING ? 1 : 0);
    htsmsg_add_u32(r,
        "dvr",
        htsp->htsp_granted_access->aa_rights & ACCESS_HTSP_RECORDER ? 1 : 0);
    htsmsg_add_u32(r,
        "faileddvr",
        htsp->htsp_granted_access->aa_rights & ACCESS_FAILED_RECORDER ? 1 : 0);
    htsmsg_add_u32(r,
        "anonymous",
        htsp->htsp_granted_access->aa_rights & ACCESS_HTSP_ANONYMIZE ? 1 : 0);
    ...
    htsmsg_add_u32(r,
        "uilevel",
        htsp->htsp_granted_access->aa_uilevel == UILEVEL_DEFAULT
            ? config.uilevel
            : htsp->htsp_granted_access->aa_uilevel);
    htsmsg_add_str(r,
        "uilanguage",
        htsp->htsp_granted_access->aa_lang_ui
            ? htsp->htsp_granted_access->aa_lang_ui
            : (config.language_ui ? config.language_ui : ""));
  }
 ...
 * Try to authenticate
 */
static htsmsg_t* htsp_method_api(htsp_connection_t* htsp, htsmsg_t* in) {
 ...
 */
static htsmsg_t* htsp_method_getDiskSpace(htsp_connection_t* htsp,
    htsmsg_t*                                                in) {
  htsmsg_t* out;
 ...
 */
static htsmsg_t* htsp_method_getSysTime(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t*       out;
  struct timeval  tv;
  struct timezone tz;
  int             tz_offset;
 ...
 */
static htsmsg_t* htsp_method_async(htsp_connection_t* htsp, htsmsg_t* in) {
  channel_t*           ch;
  channel_tag_t*       ct;
  dvr_entry_t*         de;
  dvr_autorec_entry_t* dae;
  ...
  TAILQ_FOREACH (dae, &autorec_entries, dae_link)
    if (!dvr_autorec_entry_verify(dae, htsp->htsp_granted_access, 1))
      htsp_send_message(htsp,
          htsp_build_autorecentry(htsp, dae, "autorecEntryAdd"),
          NULL);
  ...
  TAILQ_FOREACH (dte, &timerec_entries, dte_link)
    if (!dvr_timerec_entry_verify(dte, htsp->htsp_granted_access, 1))
      htsp_send_message(htsp,
          htsp_build_timerecentry(htsp, dte, "timerecEntryAdd"),
          NULL);
  ...
  LIST_FOREACH (de, &dvrentries, de_global_link)
    if (!dvr_entry_verify(de, htsp->htsp_granted_access, 1))
      htsp_send_message(htsp,
          htsp_build_dvrentry(htsp, de, "dvrEntryAdd", htsp->htsp_language, 0),
          NULL);
 ...
 */
static htsmsg_t* htsp_method_getChannel(htsp_connection_t* htsp, htsmsg_t* in) {
  uint32_t   channelId;
  channel_t* ch = NULL;
 ...
 * Get information about the given event
 */
static htsmsg_t* htsp_method_getEvent(htsp_connection_t* htsp, htsmsg_t* in) {
  uint32_t         eventId;
  epg_broadcast_t* e;
  const char*      lang;
 ...
 * n following events
 */
static htsmsg_t* htsp_method_getEvents(htsp_connection_t* htsp, htsmsg_t* in) {
  ...
  if (e || ch) {
    ...
    if (!e)
      e = ch->ch_epg_now ?: ch->ch_epg_next;
 ...
 * do an epg query
 */
static htsmsg_t* htsp_method_epgQuery(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t *     out, *array;
  const char*    query;
  int            i;
  uint32_t       u32, full;
  ...
  tvhtrace(LS_HTSP,
      "min_duration %d and max_duration %d",
      min_duration,
      max_duration);
...
static htsmsg_t* htsp_method_getEpgObject(htsp_connection_t* htsp,
    htsmsg_t*                                                in) {
  uint32_t          id, u32;
 ...
 *
 */
static htsmsg_t* htsp_method_getDvrConfigs(htsp_connection_t* htsp,
    htsmsg_t*                                                 in) {
  htsmsg_t *      out, *l, *c;
  htsmsg_field_t* f;
  dvr_config_t*   cfg;
 ...
 * add a Dvrentry
 */
static htsmsg_t* htsp_method_addDvrEntry(htsp_connection_t* htsp,
    htsmsg_t*                                               in) {
  htsmsg_t *              conf, *out;
 ...
 * Find DVR entry
 */
static dvr_entry_t* htsp_findDvrEntry(htsp_connection_t* htsp,
    htsmsg_t*                                            in,
    htsmsg_t**                                           out,
    int                                                  readonly) {
  uint32_t     dvrEntryId;
 ...
 * update a Dvrentry
 */
static htsmsg_t* htsp_method_updateDvrEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                  in) {
  htsmsg_t*      out = NULL;
  uint32_t       u32;
  dvr_entry_t*   de;
  ...
  dvr_config_name =
      htsp_dvr_config_name(htsp, htsmsg_get_str(in, "configName"));
  ...
  rating_label =
      NULL; // Rating labels not supported for manually created DVR entries
 ...
 * stop a Dvrentry
 */
static htsmsg_t* htsp_method_stopDvrEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                in) {
  htsmsg_t*    out = NULL;
  dvr_entry_t* de;
 ...
 * cancel a Dvrentry
 */
static htsmsg_t* htsp_method_cancelDvrEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                  in) {
  htsmsg_t*    out = NULL;
  dvr_entry_t* de;
 ...
 * delete a Dvrentry
 */
static htsmsg_t* htsp_method_deleteDvrEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                  in) {
  htsmsg_t*    out;
  dvr_entry_t* de;
 ...
 */
static htsmsg_t* htsp_method_addAutorecEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                   in) {
  htsmsg_t*            out;
  dvr_autorec_entry_t* dae;
  const char*          str;
  ...
  if (htsp->htsp_version > 24) {
    if (!htsmsg_get_s64(in,
            "channelId",
            &s64)) { // not sending or -1 = any channel
      if (s64 >= 0)
 ...
 */
static htsmsg_t* htsp_method_updateAutorecEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                      in) {
  const char*          daeId;
  dvr_autorec_entry_t* dae;
  int64_t              s64;
 ...
 */
static htsmsg_t* htsp_method_deleteAutorecEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                      in) {
  const char*          daeId;
  dvr_autorec_entry_t* dae;
 ...
 */
static htsmsg_t* htsp_method_addTimerecEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                   in) {
  htsmsg_t*            out;
  dvr_timerec_entry_t* dte;
  const char*          str;
 ...
 */
static htsmsg_t* htsp_method_updateTimerecEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                      in) {
  const char*          dteId;
  dvr_timerec_entry_t* dte;
  int64_t              s64;
 ...
 */
static htsmsg_t* htsp_method_deleteTimerecEntry(htsp_connection_t* htsp,
    htsmsg_t*                                                      in) {
  const char*          dteId;
  dvr_timerec_entry_t* dte;
 ...
 *                                      3=Commercial break.
 **/
static htsmsg_t* htsp_method_getDvrCutpoints(htsp_connection_t* htsp,
    htsmsg_t*                                                   in) {
  uint32_t     dvrEntryId;
  dvr_entry_t* de;
  if (htsmsg_get_u32(in, "id", &dvrEntryId))
 ...
 */
static htsmsg_t* htsp_method_getTicket(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t*    out;
  uint32_t     id;
  char         path[255];
...
static void _bytes_out_cb(void* aux) {
  ...
  if (hs->hs_s) {
    subscription_add_bytes_out(hs->hs_s,
        atomic_exchange(&hs->hs_s_bytes_out, 0));
 ...
 */
static htsmsg_t* htsp_method_subscribe(htsp_connection_t* htsp, htsmsg_t* in) {
  ...
  hs->hs_queue_depth =
      htsmsg_get_u32_or_default(in, "queueDepth", HTSP_DEFAULT_QUEUE_DEPTH);
  ...
  if (ch->ch_remote_timeshift) {
    ...
    if (timeshiftPeriod != 0) {
      if (timeshiftPeriod == ~0)
        ...
        tvhdebug(LS_HTSP,
            "using timeshift buffer (%u mins)",
            timeshiftPeriod / 60);
    }
  }
#endif

  pro = profile_find_by_list(htsp->htsp_granted_access->aa_profiles,
      profile_id,
      "htsp",
      SUBSCRIPTION_PACKET | SUBSCRIPTION_HTSP);
  profile_chain_init(&hs->hs_prch, pro, ch, 1);
  if (profile_chain_work(&hs->hs_prch,
          &hs->hs_input,
          timeshiftPeriod,
          ch->ch_remote_timeshift ? PROFILE_WORK_REMOTE_TS
                                  : PROFILE_WORK_NONE)) {
    tvherror(LS_HTSP,
        "unable to create profile chain '%s'",
  ...
  if (hs->hs_s)
    htsmsg_add_u32(rep,
        "weight",
        hs->hs_s->ths_weight >= 0 ? hs->hs_s->ths_weight : 0);
  ...
  tvhdebug(LS_HTSP,
      "%s - subscribe to %s using profile %s",
      htsp->htsp_logname,
      channel_get_name(ch, channel_blank_name),
      profile_get_name(pro));
  hs->hs_s = subscription_create_from_channel(&hs->hs_prch,
      NULL,
      weight,
      htsp->htsp_logname,
 ...
 */
static htsmsg_t* htsp_method_unsubscribe(htsp_connection_t* htsp,
    htsmsg_t*                                               in) {
 ...
 */
static htsmsg_t* htsp_method_change_weight(htsp_connection_t* htsp,
    htsmsg_t*                                                 in) {
  htsp_subscription_t* hs;
  uint32_t             sid, weight;
 ...
 * Skip stream
 */
static htsmsg_t* htsp_method_skip(htsp_connection_t* htsp, htsmsg_t* in) {
  htsp_subscription_t* hs;
  uint32_t             sid, abs;
  int64_t              s64;
 ...
 * Set stream speed
 */
static htsmsg_t* htsp_method_speed(htsp_connection_t* htsp, htsmsg_t* in) {
  htsp_subscription_t* hs;
  uint32_t             sid;
  int32_t              speed;
 ...
 * Revert to live
 */
static htsmsg_t* htsp_method_live(htsp_connection_t* htsp, htsmsg_t* in) {
  htsp_subscription_t* hs;
  uint32_t             sid;
  streaming_skip_t     skip;
 ...
 */
static htsmsg_t* htsp_method_filter_stream(htsp_connection_t* htsp,
    htsmsg_t*                                                 in) {
  htsp_subscription_t* hs;
  uint32_t             sid;
  htsmsg_t*            l;
 ...
 * Open file
 */
static htsmsg_t* htsp_method_file_open(htsp_connection_t* htsp, htsmsg_t* in) {
  const char *str, *s2;
 ...
 *
 */
static htsmsg_t* htsp_method_file_read(htsp_connection_t* htsp, htsmsg_t* in) {
 ...
 *
 */
static htsmsg_t* htsp_method_file_close(htsp_connection_t* htsp, htsmsg_t* in) {
  htsp_file_t* hf = htsp_file_find(htsp, in);
  uint32_t     u32;
  dvr_entry_t* de;
  ...
  if (hf->hf_de_id > 0 && (de = dvr_entry_find_by_id(hf->hf_de_id))) {
    int save = 0;
    /* Only allow incrementing playcount on file close, the rest can be done
     * with "updateDvrEntry" */
    if (htsp->htsp_version < 27 ||
 ...
 *
 */
static htsmsg_t* htsp_method_file_stat(htsp_connection_t* htsp, htsmsg_t* in) {
  htsp_file_t* hf = htsp_file_find(htsp, in);
  htsmsg_t*    rep;
  struct stat  st;
  int          fd;
 ...
 *
 */
static htsmsg_t* htsp_method_file_seek(htsp_connection_t* htsp, htsmsg_t* in) {
  htsp_file_t* hf = htsp_file_find(htsp, in);
  htsmsg_t*    rep;
  const char*  str;
  int64_t      off;
 ...
 *
 */
static htsmsg_t* htsp_method_getProfiles(htsp_connection_t* htsp,
    htsmsg_t*                                               in) {
  htsmsg_t *out, *l;
...
static int htsp_verify_callback(void* aux, const char* passwd) {
  struct htsp_verify_struct* v = aux;
  uint8_t                    d[20];

  if (v->digest == NULL || v->challenge == NULL)
    return 0;
 ...
 */
static int htsp_authenticate(htsp_connection_t* htsp, htsmsg_t* m) {
  ...
  if (!htsmsg_get_bin(m, "digest", &digest, &digestlen)) {
    ...
    rights = access_get(htsp->htsp_peer, username, htsp_verify_callback, &vs);
    ...
    privgain = (rights->aa_rights | htsp->htsp_granted_access->aa_rights) !=
        htsp->htsp_granted_access->aa_rights;

    tvhinfo(LS_HTSP,
        "%s: Identified as user '%s'",
        htsp->htsp_logname,
        username);
    ...
    tvhinfo(LS_HTSP,
        "%s: Identified as user '%s' (unverified)",
        htsp->htsp_logname,
        username);
    tvh_str_update(&htsp->htsp_username, username);
    htsp_update_logname(htsp);
 ...
 * Status callback
 */
static void htsp_server_status(void* opaque, htsmsg_t* m) {
 ...
 *
 */
static int htsp_read_loop(htsp_connection_t* htsp) {
  ...
  if (htsp_generate_challenge(htsp)) {
    tvherror(LS_HTSP, "%s: Unable to generate challenge", htsp->htsp_logname);
    return 1;
  }
  ...
  tcp_id = tcp_connection_launch(htsp->htsp_fd,
      streaming,
      htsp_server_status,
 ...
 *
 */
static void* htsp_write_scheduler(void* aux) {
...
static void htsp_serve(int   fd,
  ...
  *opaque = &htsp;
  ...
  tvh_thread_create(&htsp.htsp_writer_thread,
      NULL,
      htsp_write_scheduler,
      &htsp,
      "htsp-write");
 ...
 * Cancel callback
 */
static void htsp_server_cancel(void* opaque) {
  htsp_connection_t* htsp = opaque;
...
void htsp_init(const char* bindaddr) {
  ...
  static tcp_server_ops_t ops = {.start = htsp_serve,
      .stop                             = NULL,
      .cancel                           = htsp_server_cancel};
  if (tvheadend_htsp_port > 0)
    htsp_server = tcp_server_create(LS_HTSP,
        "HTSP",
        bindaddr,
        tvheadend_htsp_port,
        &ops,
  ...
  if (tvheadend_htsp_port_extra > 0)
    htsp_server_2 = tcp_server_create(LS_HTSP,
        "HTSP2",
        bindaddr,
        tvheadend_htsp_port_extra,
        &ops,
        NULL);
}
 ...
 *
 */
static void htsp_async_send(htsmsg_t* m, int mode, void* aux) {
  htsp_connection_t* htsp;
...
static void htsp_async_send_cb(http_async_send_cb_t cb, int mode, void* aux) {
  htsp_connection_t* htsp;
  htsmsg_t*          m;
...
_htsp_channel_update(channel_t* ch, const char* method, htsmsg_t* msg) {
  ...
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_ON)
      if (htsp_user_access_channel(htsp, ch)) {
        htsmsg_t* m =
            msg ? htsmsg_copy(msg) : htsp_build_channel(ch, method, htsp);
        htsp_send_message(htsp, m, NULL);
      }
  }
  htsmsg_destroy(msg);
 ...
 *
 */
static htsmsg_t* htsp_tag_update_msg(htsp_connection_t* htsp, void* tag) {
...
void htsp_dvr_entry_update_stats(dvr_entry_t* de) {
  ...
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_ON) {
      if (!dvr_entry_verify(de, htsp->htsp_granted_access, 1)) {
        htsmsg_t* m = htsp_build_dvrentry(htsp,
            de,
            "dvrEntryUpdate",
            htsp->htsp_language,
            htsp->htsp_version <= 25 ? 0 : 1);
        htsp_send_message(htsp, m, NULL);
      }
    }
  }
 ...
 */
static void _htsp_autorec_entry_update(dvr_autorec_entry_t* dae,
    const char*                                             method,
    htsmsg_t*                                               msg) {
  htsp_connection_t* htsp;
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_ON) {
      if (!dvr_autorec_entry_verify(dae, htsp->htsp_granted_access, 1)) {
        htsmsg_t* m =
            msg ? htsmsg_copy(msg) : htsp_build_autorecentry(htsp, dae, method);
        htsp_send_message(htsp, m, NULL);
      }
    }
  }
 ...
 */
static void _htsp_timerec_entry_update(dvr_timerec_entry_t* dte,
    const char*                                             method,
    htsmsg_t*                                               msg) {
  htsp_connection_t* htsp;
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_ON) {
      if (!dvr_timerec_entry_verify(dte, htsp->htsp_granted_access, 1)) {
        htsmsg_t* m =
            msg ? htsmsg_copy(msg) : htsp_build_timerecentry(htsp, dte, method);
        htsp_send_message(htsp, m, NULL);
      }
    }
  }
 ...
 */
static void htsp_epg_send_waiting(htsp_connection_t* htsp, int64_t mintime) {
  epg_broadcast_t* ebc;
  channel_t*       ch;
  int64_t          maxtime;
  ...
  CHANNEL_FOREACH(ch) {
    ...
    RB_FOREACH (ebc, &ch->ch_epg_schedule, sched_link) {
      ...
      if (htsp->htsp_epg_window && ebc->start > maxtime)
        break;
      htsmsg_t* e =
          htsp_build_event(ebc, "eventAdd", htsp->htsp_language, 0, htsp);
      if (e)
        htsp_send_message(htsp, e, NULL);
    }
  }

  /* Keep the epg window up to date */
  if (htsp->htsp_epg_window)
    mtimer_arm_rel(&htsp->htsp_epg_timer,
        htsp_epg_window_cb,
        htsp,
        sec2mono(HTSP_ASYNC_EPG_INTERVAL));
}
...
_htsp_event_update(epg_broadcast_t* ebc, const char* method, htsmsg_t* msg) {
  ...
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_EPG) {
      /* Use last update instead of window time as we do not want to push an
       * update for an event we still have to send with "htsp_epg_window_cb" */
 ...
 */
static void htsp_stream_deliver(htsp_subscription_t* hs, th_pkt_t* pkt) {
  htsmsg_t*          m;
  htsp_msg_t*        hm;
  ...
  if (pkt->pkt_pts != PTS_UNSET) {
    int64_t pts =
        hs->hs_90khz ? pkt->pkt_pts : ts_rescale(pkt->pkt_pts, 1000000);
    htsmsg_add_s64(m, "pts", pts);
  }

  if (pkt->pkt_dts != PTS_UNSET) {
    int64_t dts =
        hs->hs_90khz ? pkt->pkt_dts : ts_rescale(pkt->pkt_dts, 1000000);
    htsmsg_add_s64(m, "dts", dts);
  }

  uint32_t dur =
      hs->hs_90khz ? pkt->pkt_duration : ts_rescale(pkt->pkt_duration, 1000000);
 ...
 */
static void htsp_subscription_start(htsp_subscription_t* hs,
  ...
  for (i = 0; i < ss->ss_num_components; i++) {
    ...
    if (SCT_ISVIDEO(ssc->es_type)) {
      ...
      if (ssc->es_frame_duration)
        htsmsg_add_u32(c,
            "duration",
            hs->hs_90khz ? ssc->es_frame_duration
 ...
 */
static void htsp_subscription_stop(htsp_subscription_t* hs,
    const char*                                         err,
    const char*                                         subscriptionErr) {
  htsmsg_t* m = htsmsg_create_map();
 ...
 */
static void htsp_subscription_grace(htsp_subscription_t* hs, int grace) {
  htsmsg_t* m = htsmsg_create_map();
  htsmsg_add_str(m, "method", "subscriptionGrace");
  htsmsg_add_u32(m, "subscriptionId", hs->hs_sid);
  htsmsg_add_u32(m, "graceTimeout", grace);
  tvhdebug(LS_HTSP,
      "%s - subscription grace %i seconds",
 ...
 */
static void htsp_subscription_status(htsp_subscription_t* hs,
    const char*                                           err,
    const char*                                           subscriptionErr) {
  htsmsg_t* m = htsmsg_create_map();
 ...
 *
 */
static void htsp_subscription_service_status(htsp_subscription_t* hs,
    int                                                           status) {
 ...
 *
 */
static void htsp_subscription_signal_status(htsp_subscription_t* hs,
    signal_status_t*                                             sig) {
  htsmsg_t* m = htsmsg_create_map();
  ...
  if (!htsp_anonymize(hs->hs_htsp)) {
    ...
    if ((sig->signal != -2) &&
        (sig->signal_scale == SIGNAL_STATUS_SCALE_RELATIVE))
 ...
 *
 */
static void htsp_subscription_descramble_info(htsp_subscription_t* hs,
 ...
 *
 */
static void htsp_subscription_speed(htsp_subscription_t* hs, int speed) {
  htsmsg_t* m = htsmsg_create_map();
 ...
 */
#if ENABLE_TIMESHIFT
static void htsp_subscription_timeshift_status(htsp_subscription_t* hs,
  ...
  htsmsg_add_s64(m,
      "shift",
      hs->hs_90khz ? status->shift : ts_rescale(status->shift, 1000000));
  if (status->pts_start != PTS_UNSET)
    htsmsg_add_s64(m,
        "start",
        hs->hs_90khz ? status->pts_start
                     : ts_rescale(status->pts_start, 1000000));
  if (status->pts_end != PTS_UNSET)
    htsmsg_add_s64(m,
        "end",
        hs->hs_90khz ? status->pts_end : ts_rescale(status->pts_end, 1000000));
  htsp_send_subscription(hs->hs_htsp, m, NULL, hs, 0);
}
#endif
 ...
 *
 */
static void htsp_subscription_skip(htsp_subscription_t* hs,
    streaming_skip_t*                                   skip) {
  htsmsg_t* m = htsmsg_create_map();
  ...
  if (skip->type == SMT_SKIP_ERROR)
    ...
    htsmsg_add_s64(m,
        "time",
        hs->hs_90khz ? skip->time : ts_rescale(skip->time, 1000000));
