...
static streaming_ops_t htsp_streaming_input_ops = {.st_cb = htsp_streaming_input,
...
static void htsp_trace(htsp_connection_t* htsp, int subsystem, const char* prefix, htsmsg_t* m) {
  htsbuf_queue_t q;
  char*          s;
...
static inline int htsp_is_stream_enabled(htsp_subscription_t* hs, unsigned int id) {
  if (id < NUM_FILTERED_STREAMS)
 ...
 *
 */
static void htsp_flush_queue(htsp_connection_t* htsp, htsp_msg_q_t* hmq, int dead) {
  htsp_msg_t* hm;
 ...
 */
static const char*
htsp_image(htsp_connection_t* htsp, const char* image, char* buf, size_t buflen, int version) {
 ...
 *
 */
static void htsp_subscription_destroy(htsp_connection_t* htsp, htsp_subscription_t* hs) {
 ...
 *
 */
static void htsp_subscription_free(htsp_connection_t* htsp, htsp_subscription_t* hs) {
  LIST_REMOVE(hs, hs_link);
  htsp_flush_queue(htsp, &hs->hs_q, 1);
  free(hs);
}
 ...
 */
static void
htsp_send(htsp_connection_t* htsp, htsmsg_t* m, pktbuf_t* pb, htsp_msg_q_t* hmq, int payloadsize) {
 ...
 *
 */
static void htsp_send_message(htsp_connection_t* htsp, htsmsg_t* m, htsp_msg_q_t* hmq) {
  if (tvhtrace_enabled()) {
 ...
 */
static inline int htsp_user_access_channel(htsp_connection_t* htsp, channel_t* ch) {
  if (!ch || !ch->ch_enabled ||
      LIST_FIRST(&ch->ch_services) == NULL) /* Don't pass unplayable channels to clients */
    return 0;
  if (!htsp)
    return 1;
...
static const char* htsp_dvr_config_name(htsp_connection_t* htsp, const char* config_name) {
 ...
 *
 */
static void htsp_serialize_epnum(htsmsg_t* out, epg_episode_num_t* epnum, const char* textname) {
  if (epnum->s_num) {
...
static channel_tag_t* htsp_channel_tag_find_by_id(htsp_connection_t* htsp, uint32_t id) {
  channel_tag_t* ct;
 ...
 *
 */
static htsp_file_t* htsp_file_find(const htsp_connection_t* htsp, htsmsg_t* in) {
  htsp_file_t* hf;
 ...
 *
 */
static htsmsg_t* htsp_build_channel(channel_t* ch, const char* method, htsp_connection_t* htsp) {
...
static htsmsg_t* htsp_build_dvrentry(htsp_connection_t* htsp,
  ...
  if (!statsonly) {
    ...
    if (htsp->htsp_version > 24)
      ...
      htsmsg_add_u32(out,
          "retention",
          dvr_entry_get_retention_days(de) == DVR_RET_ONREMOVE ? dvr_entry_get_removal_days(de)
...
htsp_build_autorecentry(htsp_connection_t* htsp, dvr_autorec_entry_t* dae, const char* method) {
  ...
  htsmsg_add_s32(out,
      "approxTime",
      dae->dae_start >= 0 && tad >= 0 ? ((dae->dae_start + tad / 2) % (24 * 60)) : -1);
 ...
 * Try to authenticate
 */
static htsmsg_t* htsp_method_authenticate(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t* r = htsmsg_create_map();
  ...
  if (!(htsp->htsp_granted_access->aa_rights & HTSP_PRIV_MASK))
    ...
    htsmsg_add_str(r,
        "uilanguage",
        htsp->htsp_granted_access->aa_lang_ui ? htsp->htsp_granted_access->aa_lang_ui
 ...
 */
static htsmsg_t* htsp_method_getDiskSpace(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t* out;
...
static htsmsg_t* htsp_method_epgQuery(htsp_connection_t* htsp, htsmsg_t* in) {
  ...
  if (eq.entries) {
    ...
    for (i = 0; i < eq.entries; ++i) {
      if (full)
        htsmsg_add_msg(array, NULL, htsp_build_event(eq.result[i], NULL, lang, 0, htsp));
      else
...
static htsmsg_t* htsp_method_getEpgObject(htsp_connection_t* htsp, htsmsg_t* in) {
  uint32_t          id, u32;
 ...
 *
 */
static htsmsg_t* htsp_method_getDvrConfigs(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t *      out, *l, *c;
  htsmsg_field_t* f;
  dvr_config_t*   cfg;
  const char *    uuid, *s;
 ...
 * add a Dvrentry
 */
static htsmsg_t* htsp_method_addDvrEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t *              conf, *out;
  ...
  if (!e) {
    ...
    /* Required attributes */
    if (htsmsg_get_s64(in, "start", &start) || htsmsg_get_s64(in, "stop", &stop) ||
 ...
 * update a Dvrentry
 */
static htsmsg_t* htsp_method_updateDvrEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t*      out = NULL;
  uint32_t       u32;
  dvr_entry_t*   de;
 ...
 * stop a Dvrentry
 */
static htsmsg_t* htsp_method_stopDvrEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t*    out = NULL;
  dvr_entry_t* de;
 ...
 * cancel a Dvrentry
 */
static htsmsg_t* htsp_method_cancelDvrEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t*    out = NULL;
  dvr_entry_t* de;
 ...
 * delete a Dvrentry
 */
static htsmsg_t* htsp_method_deleteDvrEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t*    out;
  dvr_entry_t* de;
 ...
 */
static htsmsg_t* htsp_method_addAutorecEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t*            out;
  dvr_autorec_entry_t* dae;
  const char*          str;
  uint32_t             u32;
 ...
 */
static htsmsg_t* htsp_method_updateAutorecEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  const char*          daeId;
  dvr_autorec_entry_t* dae;
  int64_t              s64;
  channel_t*           ch = NULL;
 ...
 */
static htsmsg_t* htsp_method_deleteAutorecEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  const char*          daeId;
  dvr_autorec_entry_t* dae;
 ...
 */
static htsmsg_t* htsp_method_addTimerecEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t*            out;
  dvr_timerec_entry_t* dte;
  const char*          str;
  channel_t*           ch = NULL;
 ...
 */
static htsmsg_t* htsp_method_updateTimerecEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  const char*          dteId;
  dvr_timerec_entry_t* dte;
  int64_t              s64;
  channel_t*           ch = NULL;
 ...
 */
static htsmsg_t* htsp_method_deleteTimerecEntry(htsp_connection_t* htsp, htsmsg_t* in) {
  const char*          dteId;
  dvr_timerec_entry_t* dte;
 ...
 *                                      3=Commercial break.
 **/
static htsmsg_t* htsp_method_getDvrCutpoints(htsp_connection_t* htsp, htsmsg_t* in) {
  uint32_t     dvrEntryId;
  dvr_entry_t* de;
  if (htsmsg_get_u32(in, "id", &dvrEntryId))
    return htsp_error(htsp, N_("Invalid arguments"));
...
static htsmsg_t* htsp_method_subscribe(htsp_connection_t* htsp, htsmsg_t* in) {
  ...
  hs->hs_queue_depth = htsmsg_get_u32_or_default(in, "queueDepth", HTSP_DEFAULT_QUEUE_DEPTH);
  ...
  if (profile_chain_work(&hs->hs_prch,
          &hs->hs_input,
          timeshiftPeriod,
          ch->ch_remote_timeshift ? PROFILE_WORK_REMOTE_TS : PROFILE_WORK_NONE)) {
 ...
 */
static htsmsg_t* htsp_method_unsubscribe(htsp_connection_t* htsp, htsmsg_t* in) {
 ...
 */
static htsmsg_t* htsp_method_change_weight(htsp_connection_t* htsp, htsmsg_t* in) {
  htsp_subscription_t* hs;
  uint32_t             sid, weight;
 ...
 */
static htsmsg_t* htsp_method_filter_stream(htsp_connection_t* htsp, htsmsg_t* in) {
  htsp_subscription_t* hs;
  uint32_t             sid;
  htsmsg_t*            l;
  if (htsmsg_get_u32(in, "subscriptionId", &sid))
...
static htsmsg_t* htsp_method_file_open(htsp_connection_t* htsp, htsmsg_t* in) {
  ...
  if ((s2 = tvh_strbegins(str, "dvr/")) != NULL || (s2 = tvh_strbegins(str, "dvrfile/")) != NULL) {
 ...
 *
 */
static htsmsg_t* htsp_method_getProfiles(htsp_connection_t* htsp, htsmsg_t* in) {
  htsmsg_t *out, *l;
...
static int htsp_authenticate(htsp_connection_t* htsp, htsmsg_t* m) {
  ...
  if (!htsmsg_get_bin(m, "digest", &digest, &digestlen)) {
    ...
    tvhinfo(LS_HTSP, "%s: Identified as user '%s'", htsp->htsp_logname, username);
    ...
    tvhinfo(LS_HTSP, "%s: Identified as user '%s' (unverified)", htsp->htsp_logname, username);
    tvh_str_update(&htsp->htsp_username, username);
    htsp_update_logname(htsp);
  }
 ...
 */
static int htsp_read_message(htsp_connection_t* htsp, htsmsg_t** mp, int timeout) {
  int      v;
  uint32_t len;
  uint8_t  data[4];
  void*    buf;
...
static void* htsp_write_scheduler(void* aux) {
  ...
  while (htsp->htsp_writer_run) {
    ...
    if (r) {
      tvhinfo(LS_HTSP, "%s: Write error -- %s", htsp->htsp_logname, strerror(errno));
      break;
    }
  }
 ...
 */
static void
htsp_serve(int fd, void** opaque, struct sockaddr_storage* source, struct sockaddr_storage* self) {
  htsp_connection_t    htsp;
  char                 buf[50];
  htsp_subscription_t* s;
  ...
  *opaque = &htsp;
  ...
  tvh_thread_create(&htsp.htsp_writer_thread, NULL, htsp_write_scheduler, &htsp, "htsp-write");
...
void htsp_init(const char* bindaddr) {
  ...
  static tcp_server_ops_t ops = {.start = htsp_serve, .stop = NULL, .cancel = htsp_server_cancel};
  ...
  if (tvheadend_htsp_port_extra > 0)
    htsp_server_2 =
        tcp_server_create(LS_HTSP, "HTSP2", bindaddr, tvheadend_htsp_port_extra, &ops, NULL);
}
 ...
 */
static void _htsp_channel_update(channel_t* ch, const char* method, htsmsg_t* msg) {
  htsp_connection_t* htsp;
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_ON)
      if (htsp_user_access_channel(htsp, ch)) {
        htsmsg_t* m = msg ? htsmsg_copy(msg) : htsp_build_channel(ch, method, htsp);
        htsp_send_message(htsp, m, NULL);
      }
  }
  htsmsg_destroy(msg);
 ...
 */
static void _htsp_dvr_entry_update(dvr_entry_t* de, const char* method, htsmsg_t* msg) {
  htsp_connection_t* htsp;
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_ON)
      if (!dvr_entry_verify(de, htsp->htsp_granted_access, 1)) {
        htsmsg_t* m =
            msg ? htsmsg_copy(msg) : htsp_build_dvrentry(htsp, de, method, htsp->htsp_language, 0);
        htsp_send_message(htsp, m, NULL);
      }
  }
  htsmsg_destroy(msg);
...
_htsp_autorec_entry_update(dvr_autorec_entry_t* dae, const char* method, htsmsg_t* msg) {
  ...
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_ON) {
      if (!dvr_autorec_entry_verify(dae, htsp->htsp_granted_access, 1)) {
        htsmsg_t* m = msg ? htsmsg_copy(msg) : htsp_build_autorecentry(htsp, dae, method);
        htsp_send_message(htsp, m, NULL);
      }
    }
  }
...
_htsp_timerec_entry_update(dvr_timerec_entry_t* dte, const char* method, htsmsg_t* msg) {
  ...
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_ON) {
      if (!dvr_timerec_entry_verify(dte, htsp->htsp_granted_access, 1)) {
        htsmsg_t* m = msg ? htsmsg_copy(msg) : htsp_build_timerecentry(htsp, dte, method);
        htsp_send_message(htsp, m, NULL);
      }
    }
  }
 ...
 */
static void _htsp_event_update(epg_broadcast_t* ebc, const char* method, htsmsg_t* msg) {
  htsp_connection_t* htsp;
  LIST_FOREACH (htsp, &htsp_async_connections, htsp_async_link) {
    if (htsp->htsp_async_mode & HTSP_ASYNC_EPG) {
      ...
      if (!htsp->htsp_epg_window || ebc->start <= htsp->htsp_epg_lastupdate) {
        if (htsp_user_access_channel(htsp, ebc->channel)) {
          htsmsg_t* m =
              msg ? htsmsg_copy(msg) : htsp_build_event(ebc, method, htsp->htsp_language, 0, htsp);
          htsp_send_message(htsp, m, NULL);
        }
      }
    }
 ...
 */
static void htsp_subscription_start(htsp_subscription_t* hs, const streaming_start_t* ss) {
  ...
  for (i = 0; i < ss->ss_num_components; i++) {
    ...
    if (SCT_ISVIDEO(ssc->es_type)) {
      ...
      if (ssc->es_frame_duration)
        htsmsg_add_u32(c,
            "duration",
            hs->hs_90khz ? ssc->es_frame_duration : ts_rescale(ssc->es_frame_duration, 1000000));
    ...
    if (ssc->ssc_gh)
      htsmsg_add_bin_ptr(m, "meta", pktbuf_ptr(ssc->ssc_gh), pktbuf_len(ssc->ssc_gh));
  ...
  if (!uuid_empty(&si->si_adapter_uuid))
    htsmsg_add_str(sourceinfo, "adapter_uuid", uuid_get_hex(&si->si_adapter_uuid, ubuf));
  if (!uuid_empty(&si->si_mux_uuid))
    htsmsg_add_str(sourceinfo, "mux_uuid", uuid_get_hex(&si->si_mux_uuid, ubuf));
  if (!uuid_empty(&si->si_network_uuid))
    htsmsg_add_str(sourceinfo, "network_uuid", uuid_get_hex(&si->si_network_uuid, ubuf));
 ...
 *
 */
static void htsp_subscription_service_status(htsp_subscription_t* hs, int status) {
 ...
 *
 */
static void htsp_subscription_signal_status(htsp_subscription_t* hs, signal_status_t* sig) {
  htsmsg_t* m = htsmsg_create_map();
...
static void htsp_subscription_timeshift_status(htsp_subscription_t* hs,
  ...
  if (status->pts_start != PTS_UNSET)
    htsmsg_add_s64(m,
        "start",
        hs->hs_90khz ? status->pts_start : ts_rescale(status->pts_start, 1000000));
 ...
 *
 */
static void htsp_subscription_skip(htsp_subscription_t* hs, streaming_skip_t* skip) {
  htsmsg_t* m = htsmsg_create_map();
