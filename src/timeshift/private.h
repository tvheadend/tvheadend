...
typedef TAILQ_HEAD(timeshift_index_iframe_list,
    timeshift_index_iframe) timeshift_index_iframe_list_t;
...
typedef TAILQ_HEAD(timeshift_index_data_list,
    timeshift_index_data) timeshift_index_data_list_t;
...
typedef struct timeshift {
  ...
  uint8_t
      audio_packet_counter; ///< Counter for audio packets in audio-only streams
...
void timeshift_packet_log0(const char* prefix,
    timeshift_t*                       ts,
    streaming_message_t*               sm);

static inline void timeshift_packet_log(const char* prefix,
    timeshift_t*                                    ts,
    streaming_message_t*                            sm) {
...
timeshift_file_t*
timeshift_filemgr_prev(timeshift_file_t* ts, int* end, int keep);
timeshift_file_t*
     timeshift_filemgr_next(timeshift_file_t* ts, int* end, int keep);
