...
static timeshift_seek_t*
_seek_set_file(timeshift_seek_t* seek, timeshift_file_t* tsf, off_t roff) {
  seek->file  = tsf;
  seek->frame = NULL;
  if (tsf)
    tsf->roff = roff;
...
static ssize_t
_read_buf(timeshift_file_t* tsf, int fd, void* buf, size_t size) {
  ssize_t r;
  size_t  ret;

  if (tsf && tsf->ram) {
    if (tsf->roff == tsf->woff)
      return 0;
    if (tsf->roff + size > tsf->woff)
      return -1;
...
static ssize_t
_read_msg(timeshift_file_t* tsf, int fd, streaming_message_t** sm) {
  ssize_t                  r, cnt = 0;
  size_t                   sz;
...
static int64_t _timeshift_first_time(timeshift_t* ts, int* active) {
  int64_t                   ret = 0;
  int                       end;
...
static int _timeshift_skip(timeshift_t* ts,
    int64_t                             req_time,
    int64_t                             cur_time,
  ...
  if (!tsi) {
    while (tsf && !end) {
      if (back) {
        ...
        if ((tsf->time >= sec) && (tsi = TAILQ_FIRST(&tsf->iframes)))
          break;
        tsf = timeshift_filemgr_next(tsf, &end, 0);
      }
      tsi = NULL;
 ...
 *
 */
static int _timeshift_do_skip(timeshift_t* ts,
    int64_t                                req_time,
    int64_t                                last_time,
    timeshift_seek_t*                      seek) {
  timeshift_seek_t nseek;
  ...
  tvhdebug(LS_TIMESHIFT,
      "ts %d skip to %" PRId64 " from %" PRId64,
      ts->id,
      req_time,
      last_time);
  ...
  *seek = nseek;
  if (nseek.file != NULL) {
    ...
    tvhtrace(LS_TIMESHIFT,
        "do skip seek->file %p roff %" PRId64,
        nseek.file,
        (int64_t)nseek.file->roff);
  }
 ...
 * Output packet
 */
static int _timeshift_read(timeshift_t* ts,
    timeshift_seek_t*                   seek,
  ...
  *sm = NULL;
  ...
  if (tsf) {
    ...
    if (tsf->rfd >= 0)
      if ((off = lseek(tsf->rfd, tsf->roff, SEEK_SET)) != tsf->roff)
        tvherror(LS_TIMESHIFT,
            "ts %d seek to %s failed (off %" PRId64 " != %" PRId64 "): %s",
            ts->id,
            tsf->path,
            (int64_t)tsf->roff,
            (int64_t)off,
    ...
    if (r < 0) {
      streaming_message_t* e =
          streaming_msg_create_code(SMT_STOP, SM_CODE_UNDEFINED_ERROR);
      streaming_target_deliver2(ts->output, e);
      tvhtrace(LS_TIMESHIFT,
          "ts %d seek to %jd (woff %jd) (fd %i)",
          ts->id,
          (intmax_t)off,
          (intmax_t)tsf->woff,
          tsf->rfd);
    ...
    tvhtrace(LS_TIMESHIFT,
        "ts %d seek to %jd (fd %i) read msg %p/%" PRId64 " (%" PRId64 ")",
        ts->id,
        (intmax_t)off,
        tsf->rfd,
        *sm,
 ...
 */
static int
_timeshift_flush_to_live(timeshift_t* ts, timeshift_seek_t* seek, int* wait) {
  streaming_message_t* sm;
 ...
 */
static void timeshift_fill_status(timeshift_t* ts,
  ...
  status->full = ts->full;
  tvhtrace(LS_TIMESHIFT,
      "ts %d status start %" PRId64 " end %" PRId64 " current %" PRId64
      " state %d",
      ts->id,
      start,
      end,
...
static void timeshift_status(timeshift_t* ts, int64_t current_time) {
...
void* timeshift_reader(void* p) {
  ...
  while (run) {
    ...
    // Note: Previously we allowed unlimited wait, but we now must wake
    // periodically
        ...
        if (ctrl->sm_type == SMT_EXIT) {
          ...
          if (cur_speed != speed) {
            ...
            if (speed != 100 || state != TS_LIVE) {
              ...
              tvhtrace(LS_TIMESHIFT,
                  "reader - set %s",
                  speed == 0 ? "TS_PAUSE" : "TS_PLAY");
            }
            if ((ts->state == TS_PLAY && state != TS_PLAY) ||
                (speed != cur_speed)) {
              mono_play_time = mono_now;
              tvhtrace(LS_TIMESHIFT,
                  "update play time TS_LIVE - %" PRId64
                  " play buffer from %" PRId64,
                  mono_now,
                  pause_time);
              if (speed != cur_speed)
          ...
          switch (skip->type) {
            ...
            case SMT_SKIP_REL_TIME:
              ...
              tvhdebug(LS_TIMESHIFT,
                  "ts %d skip %" PRId64 " requested %" PRId64,
                  ts->id,
                  skip_time,
                  skip->time);
              ...
              if (ts->state == TS_LIVE) {
                ...
                if (tmp_file &&
                    (seek->file = timeshift_filemgr_get(ts, i64)) != NULL) {
                  seek->file->roff = seek->file->size;
              ...
              if (ts->state == TS_LIVE) {
                if (skip_time >= ts->buf_time - TIMESHIFT_PLAY_BUF) {
                  tvhdebug(LS_TIMESHIFT,
                      "ts %d skip ignored, already live",
                      ts->id);
                  skip = NULL;
                } else {
                  ts->state = TS_PLAY;
              ...
              if (skip) {
                ...
                if (seek->frame) {
                  ...
                  tvhtrace(LS_TIMESHIFT,
                      "ts %d skip - play buffer from %" PRId64
                      " last_time %" PRId64,
                      ts->id,
                      pause_time,
                      last_time);
                  ...
                  if (mono_play_time != mono_now)
                    tvhtrace(LS_TIMESHIFT,
                        "ts %d update play time skip - %" PRId64,
                        ts->id,
                        mono_now);
                  mono_play_time = mono_now;
            ...
            default:
              tvherror(LS_TIMESHIFT,
                  "ts %d invalid/unsupported skip type: %d",
                  ts->id,
                  skip->type);
              skip = NULL;
              break;
    ...
    tvhtrace(LS_TIMESHIFT,
        "speed %d now %" PRId64 " play_time %" PRId64 " deliver %" PRId64
        " deliver0 %" PRId64,
        cur_speed,
        mono_now,
        mono_play_time,
    ...
    if (sm &&
      ...
      if (wait == 0)
        wait = 1;
      tvhtrace(LS_TIMESHIFT,
          "ts %d wait %d speed %d sm_time %" PRId64 " deliver %" PRId64,
          ts->id,
          wait,
          cur_speed,
          sm->sm_time,
      ...
      if ((end == 1 && !ts->full) || !seek->file) {
        ...
        if (cur_speed <= 0) {
          ...
          if (ts->state != TS_PLAY) {
            ...
            if (mono_play_time != mono_now)
              tvhtrace(LS_TIMESHIFT,
                  "update play time (pause) - %" PRId64,
                  mono_now);
            mono_play_time = mono_now;
          }
        }
        tvhdebug(LS_TIMESHIFT,
            "ts %d sob speed %d last time %" PRId64,
            ts->id,
            cur_speed,
            last_time);
        pause_time = last_time;
