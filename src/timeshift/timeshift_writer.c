 ...
 */
static ssize_t _write_fd(int fd, const void* buf, size_t count) {
  ssize_t r;
  size_t  n = 0;
  while (n < count) {
...
static ssize_t _write(timeshift_file_t* tsf, const void* buf, size_t count) {
  uint8_t* ram;
  size_t   alloc;
  ssize_t  ret;
  if (tsf->ram) {
 ...
 * Write message
 */
static ssize_t _write_msg(timeshift_file_t* tsf,
...
static ssize_t _write_msg_fd(int fd,
    streaming_message_type_t     type,
    int64_t                      time,
    const void*                  buf,
    size_t                       len) {
 ...
 * Write signal status
 */
ssize_t timeshift_write_sigstat(timeshift_file_t* tsf,
    int64_t                                       time,
    signal_status_t*                              sigstat) {
  return _write_msg(tsf,
      SMT_SIGNAL_STATUS,
      time,
      sigstat,
 ...
 */
ssize_t
timeshift_write_packet(timeshift_file_t* tsf, int64_t time, th_pkt_t* pkt) {
  ssize_t ret = 0, err;
 ...
 */
ssize_t
timeshift_write_mpegts(timeshift_file_t* tsf, int64_t time, void* data) {
 ...
 */
static void _handle_sstart(timeshift_t* ts,
 ...
 */
static void add_frame_to_index(timeshift_t* ts,
    timeshift_file_t*                       tsf,
    streaming_message_t*                    sm) {
  ...
  if ((pkt->pkt_componentindex == ts->vididx &&
    ...
    if (ts->vididx != -1 || ts->audio_packet_counter > 100) {
      timeshift_index_iframe_t* ti =
          calloc(1, sizeof(timeshift_index_iframe_t));
      memoryinfo_alloc(&timeshift_memoryinfo, sizeof(*ti));
      ti->pos  = tsf->size;
      ti->time = sm->sm_time;
...
_process_msg0(timeshift_t* ts, timeshift_file_t* tsf, streaming_message_t* sm) {
  ssize_t err;
...
static void _process_msg(timeshift_t* ts, streaming_message_t* sm, int* run) {
  int               err, teletext = 0;
  timeshift_file_t* tsf;
  th_pkt_t*         pkt;
