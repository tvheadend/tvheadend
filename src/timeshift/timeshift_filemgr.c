      ...
      if (rmdir(dpath) == -1)
        if (errno != ENOTEMPTY)
          tvherror(LS_TIMESHIFT,
              "failed to remove %s [e=%s]",
              dpath,
              strerror(errno));
    } else {
      tvhtrace(LS_TIMESHIFT,
          "remove RAM segment (time %" PRId64 ", size %" PRId64 ")",
          tsf->time,
          (int64_t)tsf->size);
    }
...
static void timeshift_reaper_remove(timeshift_file_t* tsf) {
  if (tvhtrace_enabled()) {
    if (tsf->path)
      ...
      tvhtrace(LS_TIMESHIFT,
          "queue file for removal - RAM segment time %li",
          (long)tsf->time);
  }
  tvh_mutex_lock(&timeshift_reaper_lock);
...
void timeshift_filemgr_dump0(timeshift_t* ts) {
  ...
  TAILQ_FOREACH (tsf, &ts->files, link) {
    tvhtrace(LS_TIMESHIFT,
        "ts %d (full=%d) file dump tsf %p time %4" PRId64 " last %10" PRId64
        " bad %d refcnt %d",
        ts->id,
        ts->full,
        tsf,
 ...
 */
static int timeshift_filemgr_get_root(char* buf, size_t len) {
 ...
 * Remove file
 */
void timeshift_filemgr_remove(timeshift_t* ts,
    timeshift_file_t*                      tsf,
    int                                    force) {
  if (tsf->wfd >= 0)
    close(tsf->wfd);
  ...
  if (tvhtrace_enabled()) {
    if (tsf->path)
      tvhdebug(LS_TIMESHIFT,
          "ts %d remove %s (size %" PRId64 ")",
          ts->id,
          tsf->path,
          (int64_t)tsf->size);
    else
      tvhdebug(LS_TIMESHIFT,
          "ts %d RAM segment remove time %" PRId64 " (size %" PRId64
          ", alloc size %" PRId64 ")",
          ts->id,
          tsf->time,
          (int64_t)tsf->size,
          (int64_t)tsf->ram_size);
  }
 ...
 *
 */
static timeshift_file_t* timeshift_filemgr_file_init(timeshift_t* ts,
    int64_t                                                       start_time) {
  timeshift_file_t* tsf;
...
timeshift_file_t* timeshift_filemgr_get(timeshift_t* ts, int64_t start_time) {
  ...
  if (!tsf_tl || tsf_tl->time < time ||
    ...
    /* Check period */
    if (!timeshift_conf.unlimited_period && ts->max_time && tsf_hd && tsf_tl) {
    ...
    if (!timeshift_conf.unlimited_size &&
        atomic_pre_add_u64(&timeshift_conf.total_size, 0) >=
            timeshift_conf.max_size) {
    ...
    if (!ts->full) {
      ...
      tvhtrace(LS_TIMESHIFT,
          "ts %d RAM total %" PRId64 " requested %" PRId64 " segment %" PRId64,
          ts->id,
          atomic_pre_add_u64(&timeshift_total_ram_size, 0),
          timeshift_conf.ram_size,
          timeshift_conf.ram_segment_size);
      while (1) {
        if (timeshift_conf.ram_size >= 8 * 1024 * 1024 &&
          ...
          if (!tsf_tmp->ram) {
            ...
            tvhtrace(LS_TIMESHIFT,
                "ts %d create RAM segment with %" PRId64 " bytes (time %" PRId64
                ")",
                ts->id,
                tsf_tmp->ram_size,
                start_time);
          ...
          if (timeshift_conf.ram_fit && tsf_hd && !tsf_hd->refcount &&
            ...
            tvhtrace(LS_TIMESHIFT,
                "ts %d remove RAM segment %" PRId64 " (fit)",
                ts->id,
                tsf_hd->time);
            timeshift_filemgr_remove(ts, tsf_hd, 0);
          } else {
      ...
      if (tsf_tmp && tsf_tl) {
        ...
        if ((ti = TAILQ_LAST(&tsf_tl->sstart, timeshift_index_data_list)) ||
            ts->smt_start) {
          tvhtrace(LS_TIMESHIFT,
              "ts %d copy smt_start to new file%s",
              ts->id,
              ti ? " (from last file)" : "");
          timeshift_index_data_t* ti2 =
              calloc(1, sizeof(timeshift_index_data_t));
          if (ti) {
            memoryinfo_alloc(&timeshift_memoryinfo,
                sizeof(timeshift_index_data_t));
...
timeshift_file_t*
timeshift_filemgr_next(timeshift_file_t* tsf, int* end, int keep) {
...
timeshift_file_t*
timeshift_filemgr_prev(timeshift_file_t* tsf, int* end, int keep) {
...
void timeshift_filemgr_init(void) {
  ...
  tvh_thread_create(&timeshift_reaper_thread,
      NULL,
      timeshift_reaper_callback,
      NULL,
      "tshift-reap");
}
