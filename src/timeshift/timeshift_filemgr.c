...
void timeshift_filemgr_dump0(timeshift_t* ts) {
  ...
  TAILQ_FOREACH (tsf, &ts->files, link) {
    tvhtrace(LS_TIMESHIFT,
        "ts %d (full=%d) file dump tsf %p time %4" PRId64 " last %10" PRId64 " bad %d refcnt %d",
        ts->id,
        ts->full,
        tsf,
        tsf->time,
...
void timeshift_filemgr_remove(timeshift_t* ts, timeshift_file_t* tsf, int force) {
  ...
  if (tvhtrace_enabled()) {
    if (tsf->path)
      ...
      tvhdebug(LS_TIMESHIFT,
          "ts %d RAM segment remove time %" PRId64 " (size %" PRId64 ", alloc size %" PRId64 ")",
          ts->id,
          tsf->time,
          (int64_t)tsf->size,
 ...
 *
 */
static timeshift_file_t* timeshift_filemgr_file_init(timeshift_t* ts, int64_t start_time) {
  timeshift_file_t* tsf;
    ...
    if (!ts->full) {
      ...
      while (1) {
        if (timeshift_conf.ram_size >= 8 * 1024 * 1024 &&
          ...
          tsf_hd = TAILQ_FIRST(&ts->files);
          if (timeshift_conf.ram_fit && tsf_hd && !tsf_hd->refcount && tsf_hd->ram &&
              ts->file_segments == 0) {
            tvhtrace(LS_TIMESHIFT,
...
timeshift_file_t* timeshift_filemgr_next(timeshift_file_t* tsf, int* end, int keep) {
...
timeshift_file_t* timeshift_filemgr_prev(timeshift_file_t* tsf, int* end, int keep) {
...
void timeshift_filemgr_init(void) {
  ...
  tvh_thread_create(&timeshift_reaper_thread, NULL, timeshift_reaper_callback, NULL, "tshift-reap");
}
