...
struct epg_broadcast {
  ...
  epg_episode_num_t
      epnum; ///< Episode numbering; NOTE: use the accessor routine!

  htsmsg_t* credits;          ///< Cast/Credits map of name -> role type (actor,
                              ///< presenter, director, etc).
  lang_str_t* credits_cached; ///< Comma separated cast (for regex searching in
                              ///< GUI/autorec). Kept in sync with cast_map
  string_list_t*
      category; ///< Extra categories (typically from xmltv) such as "Western"
                ///< or "Sumo Wrestling". These extra categories are often a
                ///< superset of our EN 300 468 DVB genre. Used with drop-down
                ///< lists in the GUI.
  string_list_t* keyword;     ///< Extra keywords (typically from xmltv) such as
                              ///< "Wild West" or "Unicorn".
  ...
  uint16_t
      copyright_year; ///< xmltv DTD gives a tag "date" (separate to
                      ///< previously-shown/first aired). This is the date
                      ///< programme was "finished...probably the copyright
                      ///< date." We'll call it copyright_year since words like
                      ///< "complete" and "finished" sound too similar to dvr
                      ///< recorded functionality. We'll only store the year
                      ///< since we only get year not month and day.
};
...
/* Post-modify */
int epg_broadcast_change_finish(epg_broadcast_t* b,
    epg_changes_t                                changed,
    int merge) __attribute__((warn_unused_result));

/* Special */
epg_broadcast_t*
epg_broadcast_clone(struct channel* channel, epg_broadcast_t* src, int* save);
...
int epg_broadcast_set_keyword(epg_broadcast_t* b,
    const string_list_t*                       msg,
    epg_changes_t* changed) __attribute__((warn_unused_result));
int epg_broadcast_set_serieslink_uri(epg_broadcast_t* b,
    const char*                                       uri,
    epg_changes_t* changed) __attribute__((warn_unused_result));
int epg_broadcast_set_episodelink_uri(epg_broadcast_t* b,
    const char*                                        uri,
    epg_changes_t* changed) __attribute__((warn_unused_result));
...
void   epg_broadcast_get_epnum(const epg_broadcast_t* b,
