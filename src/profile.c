...
static void profile_class_get_title(idnode_t* in,
    const char*                               lang,
    char*                                     dst,
    size_t                                    dstsize) {
  profile_t* pro = (profile_t*)in;
...
profile_t* profile_find_by_list(htsmsg_t* uuids,
  ...
  if (!res) {
    res = profile_find_by_name((sflags & SUBSCRIPTION_HTSP) ? "htsp" : NULL,
        NULL);
    if (!profile_verify(res, sflags))
      tvherror(LS_PROFILE,
          "unable to select a working profile (asked '%s' alt '%s')",
          name,
          alt);
  }
  return res;
 ...
 *
 */
static void profile_sharer_deliver(profile_chain_t* prch,
  ...
  if (sm->sm_type == SMT_PACKET) {
    ...
    if (pkt->pkt_pts >= prch->prch_ts_delta &&
      ...
      pkt_trace(LS_PROFILE,
          pkt,
          "packet drop (delta %" PRId64 ")",
          prch->prch_ts_delta);
      streaming_msg_free(sm);
      return;
    }
...
static profile_sharer_t* profile_sharer_find(profile_chain_t* prch) {
  ...
  if (!prsh) {
    ...
    streaming_target_init(&prsh->prsh_input,
        &profile_sharer_input_ops,
        prsh,
        0);
...
static int profile_sharer_postinit(profile_sharer_t* prsh) {
  ...
  r                    = tvh_thread_create(&prsh->prsh_queue_thread,
      NULL,
      profile_sharer_thread,
      prsh,
      "sharer");
  if (r) {
 ...
 *
 */
static int profile_sharer_create(profile_sharer_t* prsh,
 ...
 *
 */
void profile_chain_init(profile_chain_t* prch,
    profile_t*                           pro,
    void*                                id,
    int                                  queue) {
 ...
 *
 */
int profile_chain_raw_open(profile_chain_t* prch,
    void*                                   id,
    size_t                                  qsize,
    int                                     muxer) {
  muxer_config_t c;
...
const idclass_t profile_mpegts_spawn_class = {.ic_super = &profile_class,
    ...
    .ic_properties = (const property_t[]){
        {.type    = PT_STR,
            ...
            .desc =
                N_("Command line to run a task which accepts MPEG-TS stream"
...
static htsmsg_t* profile_class_codec_profiles_list(enum AVMediaType media_type,
    const char*                                                     lang) {
...
static int profile_class_src_codec_set(profile_transcode_t* pro,
    char**                                                  str,
    const void*                                             p) {
...
static const void* profile_class_src_codec_get(profile_transcode_t* pro,
    char*                                                           str) {
...
static int profile_transcode_can_share(profile_chain_t* prch,
...
static int profile_transcode_work(profile_chain_t* prch,
  ...
  if (!prsh->prsh_transcoder) {
    ...
    dst = prsh->prsh_transcoder =
        transcoder_create(&prsh->prsh_input, profiles, src_codecs);
    if (!dst)
      goto fail;
...
void profile_init(void) {
  ...
  profile_register(&profile_libav_matroska_class,
      profile_libav_matroska_builder);
    ...
    HTSMSG_FOREACH(f, c) {
      ...
      transcoder_create:
        ...
        (void)profile_create(NULL, e, 1);
          ...
          idnode_is_instance(&pro->pro_id, &profile_transcode_class)) {
  ...
  if (!profile_default) {
    ...
    if (pro == NULL)
      tvhabort(LS_PROFILE,
          "no default streaming profile! reinstall data files");
    profile_default = pro;
  }
}
