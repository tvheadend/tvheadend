...
static profile_build_t* profile_class_find(const char* name) {
  profile_build_t* pb;
...
profile_t* profile_create(const char* uuid, htsmsg_t* conf, int save) {
  profile_t*       pro = NULL;
...
static void profile_delete(profile_t* pro, int delconf) {
...
static htsmsg_t* profile_class_save(idnode_t* in, char* filename, size_t fsize) {
...
static void profile_class_get_title(idnode_t* in, const char* lang, char* dst, size_t dstsize) {
  profile_t* pro = (profile_t*)in;
...
static void profile_class_delete(idnode_t* self) {
...
static uint32_t profile_class_enabled_opts(void* o, uint32_t opts) {
  profile_t* pro = o;
  uint32_t   r   = 0;
...
static const void* profile_class_class_get(void* o) {
  profile_t*         pro = o;
...
static int profile_class_class_set(void* o, const void* v) {
  /* just ignore, create fcn does the right job */
  return 0;
}

static const void* profile_class_default_get(void* o) {
  static int res;
  res = o == profile_default;
  return &res;
}

static int profile_class_default_set(void* o, const void* v) {
  profile_t *pro = o, *old;
...
static uint32_t profile_class_name_opts(void* o, uint32_t opts) {
  profile_t* pro = o;
  uint32_t   r   = 0;
...
static htsmsg_t* profile_class_priority_list(void* o, const char* lang) {
...
static htsmsg_t* profile_class_svfilter_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
const idclass_t profile_class = {.ic_class = "profile",
    ...
    .ic_groups                             = (const property_group_t[]){{
                                    ...
                                    {}},
    .ic_properties                         = (const property_t[]){{.type     = PT_STR,
                                                                      .id    = "class",
                                                                      .name  = N_("Class"),
 ...
 *
 */
static profile_t* profile_find_by_name2(const char* name, const char* alt, int all) {
  profile_t* pro;
 ...
 *
 */
profile_t* profile_find_by_list(htsmsg_t* uuids, const char* name, const char* alt, int sflags) {
 ...
 *
 */
static void profile_deliver(profile_chain_t* prch, streaming_message_t* sm) {
  if (prch->prch_start_pending) {
    ...
    sm2 = streaming_msg_create_data(SMT_START, streaming_start_copy(prsh->prsh_start_msg));
...
static htsmsg_t* profile_input_info(void* opaque, htsmsg_t* list) {
...
static streaming_ops_t profile_input_ops = {.st_cb = profile_input, .st_info = profile_input_info};

/*
 *
 */
static void profile_input_queue(void* opaque, streaming_message_t* sm) {
  profile_chain_t*          prch = opaque;
  profile_sharer_t*         prsh = prch->prch_sharer;
...
static htsmsg_t* profile_input_queue_info(void* opaque, htsmsg_t* list) {
  htsmsg_add_str(list, NULL, "profile queue input");
  profile_input_info(opaque, list);
  return list;
}

static streaming_ops_t profile_input_queue_ops = {.st_cb = profile_input_queue,
    .st_info                                             = profile_input_queue_info};

/*
 *
 */
static void profile_sharer_deliver(profile_chain_t* prch, streaming_message_t* sm) {
  if (sm->sm_type == SMT_PACKET) {
     ...
     */
    if (pkt->pkt_pts >= prch->prch_ts_delta && pkt->pkt_dts >= prch->prch_ts_delta &&
 ...
 *
 */
static void profile_sharer_input(void* opaque, streaming_message_t* sm) {
...
static htsmsg_t* profile_sharer_input_info(void* opaque, htsmsg_t* list) {
  htsmsg_add_str(list, NULL, "profile sharer input");
  return list;
}

static streaming_ops_t profile_sharer_input_ops = {.st_cb = profile_sharer_input,
    .st_info                                              = profile_sharer_input_info};

/*
 *
 */
static profile_sharer_t* profile_sharer_find(profile_chain_t* prch) {
 ...
 *
 */
static void* profile_sharer_thread(void* aux) {
 ...
 *
 */
static int profile_sharer_postinit(profile_sharer_t* prsh) {
  int r;
  ...
  r = tvh_thread_create(&prsh->prsh_queue_thread, NULL, profile_sharer_thread, prsh, "sharer");
  if (r) {
 ...
 */
static int
profile_sharer_create(profile_sharer_t* prsh, profile_chain_t* prch, streaming_target_t* dst) {
 ...
 *
 */
static void profile_sharer_destroy(profile_chain_t* prch) {
  profile_sharer_t*         prsh = prch->prch_sharer;
  ...
  if (LIST_EMPTY(&prsh->prsh_chains)) {
    ...
    if (prsh->prsh_queue_run) {
      ...
      for (psm = TAILQ_FIRST(&prsh->prsh_queue); psm; psm = psm2) {
        ...
        if (psm->psm_prch != prch)
          continue;
        if (psm->psm_sm->sm_type == SMT_PACKET || psm->psm_sm->sm_type == SMT_MPEGTS)
          streaming_msg_free(psm->psm_sm);
        else
          profile_input(psm->psm_prch, psm->psm_sm);
        TAILQ_REMOVE(&prsh->prsh_queue, psm, psm_link);
 ...
 *
 */
int profile_chain_work(profile_chain_t* prch,
    struct streaming_target*            dst,
    uint32_t                            timeshift_period,
    profile_work_flags_t                flags) {
...
static const int prio2weight[] = {[PROFILE_SPRIO_DVR_IMPORTANT] = 525,
 ...
 *  HTSP Profile Class
 */
const idclass_t profile_htsp_class = {.ic_super = &profile_class,
    .ic_class                                   = "profile-htsp",
    .ic_caption                                 = N_("HTSP Stream Profile"),
    .ic_properties = (const property_t[]){/* Ready for future extensions */
        {}}};

static int profile_htsp_work(profile_chain_t* prch,
  ...
  streaming_target_init(&prch->prch_input,
      prsh->prsh_do_queue ? &profile_input_queue_ops : &profile_input_ops,
      prch,
      0);
...
static profile_t* profile_htsp_builder(void) {
...
static int profile_pass_rewrite_sid_set(void* in, const void* v) {
...
static int profile_pass_int_set(void* in, const void* v, int* prop) {
  profile_mpegts_t* pro = (profile_mpegts_t*)in;
  int               val = *(int*)v;
...
static int profile_pass_rewrite_pmt_set(void* in, const void* v) {
  return profile_pass_int_set(in, v, &((profile_mpegts_t*)in)->pro_rewrite_pmt);
}

static int profile_pass_rewrite_pat_set(void* in, const void* v) {
  return profile_pass_int_set(in, v, &((profile_mpegts_t*)in)->pro_rewrite_pat);
}

static int profile_pass_rewrite_sdt_set(void* in, const void* v) {
  return profile_pass_int_set(in, v, &((profile_mpegts_t*)in)->pro_rewrite_sdt);
}

static int profile_pass_rewrite_nit_set(void* in, const void* v) {
  return profile_pass_int_set(in, v, &((profile_mpegts_t*)in)->pro_rewrite_nit);
}

static int profile_pass_rewrite_eit_set(void* in, const void* v) {
  return profile_pass_int_set(in, v, &((profile_mpegts_t*)in)->pro_rewrite_eit);
}

const idclass_t profile_mpegts_pass_class = {.ic_super = &profile_class,
...
static int profile_mpegts_pass_reopen(profile_chain_t* prch,
    muxer_config_t*                                    m_cfg,
    muxer_hints_t*                                     hints,
    int                                                flags) {
...
static int profile_mpegts_pass_open(profile_chain_t* prch,
    muxer_config_t*                                  m_cfg,
    muxer_hints_t*                                   hints,
    int                                              flags,
    size_t                                           qsize) {
...
static profile_t* profile_mpegts_pass_builder(void) {
...
static int profile_spawn_rewrite_sid_set(void* in, const void* v) {
...
static int profile_spawn_int_set(void* in, const void* v, int* prop) {
  profile_mpegts_spawn_t* pro = (profile_mpegts_spawn_t*)in;
  int                     val = *(int*)v;
  if (pro->pro_rewrite_sid > 0)
    val = 1;
...
static int profile_mpegts_spawn_reopen(profile_chain_t* prch,
    muxer_config_t*                                     m_cfg,
    muxer_hints_t*                                      hints,
    int                                                 flags) {
...
static int profile_mpegts_spawn_open(profile_chain_t* prch,
    muxer_config_t*                                   m_cfg,
    muxer_hints_t*                                    hints,
    int                                               flags,
    size_t                                            qsize) {
...
static void profile_mpegts_spawn_free(profile_t* _pro) {
  profile_mpegts_spawn_t* pro = (profile_mpegts_spawn_t*)_pro;
  free(pro->pro_cmdline);
  free(pro->pro_mime);
}

static profile_t* profile_mpegts_spawn_builder(void) {
...
const idclass_t profile_matroska_class = {.ic_super = &profile_class,
    ...
    .ic_groups                                      = (const property_group_t[]){{
                                             ...
                                             {}},
    .ic_properties                                  = (const property_t[]){{.type     = PT_BOOL,
                                                                               .id    = "webm",
                                                                               .name  = N_("WEBM"),
...
static int profile_matroska_reopen(profile_chain_t* prch,
    muxer_config_t*                                 m_cfg,
    muxer_hints_t*                                  hints,
    int                                             flags) {
...
static int profile_matroska_open(profile_chain_t* prch,
    muxer_config_t*                               m_cfg,
    muxer_hints_t*                                hints,
    int                                           flags,
    size_t                                        qsize) {
...
static profile_t* profile_matroska_builder(void) {
...
static htsmsg_t* profile_class_mc_audio_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("Any"), MC_UNKNOWN},
      {
          N_("MPEG-2 audio"),
...
const idclass_t profile_audio_class = {.ic_super = &profile_class,
    .ic_class                                    = "profile-audio",
    .ic_caption                                  = N_("Audio stream"),
    .ic_properties =
        (const property_t[]){{.type     = PT_INT,
                                 .id    = "type",
                                 .name  = N_("Audio type"),
...
static int profile_audio_reopen(profile_chain_t* prch,
    muxer_config_t*                              m_cfg,
    muxer_hints_t*                               hints,
    int                                          flags) {
  muxer_config_t   c;
...
static int profile_audio_open(profile_chain_t* prch,
    muxer_config_t*                            m_cfg,
    muxer_hints_t*                             hints,
    int                                        flags,
    size_t                                     qsize) {
...
static profile_t* profile_audio_builder(void) {
...
const idclass_t profile_libav_mpegts_class = {.ic_super = &profile_class,
    .ic_class                                           = "profile-libav-mpegts",
    .ic_caption                                         = N_("MPEG-TS/av-lib"),
    .ic_properties                                      = (const property_t[]){{}}};

static int profile_libav_mpegts_reopen(profile_chain_t* prch,
    muxer_config_t*                                     m_cfg,
    muxer_hints_t*                                      hints,
    int                                                 flags) {
  muxer_config_t c;
...
static int profile_libav_mpegts_open(profile_chain_t* prch,
    muxer_config_t*                                   m_cfg,
    muxer_hints_t*                                    hints,
    int                                               flags,
    size_t                                            qsize) {
...
static profile_t* profile_libav_mpegts_builder(void) {
...
const idclass_t profile_libav_matroska_class = {.ic_super = &profile_class,
...
static int profile_libav_matroska_reopen(profile_chain_t* prch,
    muxer_config_t*                                       m_cfg,
    muxer_hints_t*                                        hints,
    int                                                   flags) {
...
static int profile_libav_matroska_open(profile_chain_t* prch,
    muxer_config_t*                                     m_cfg,
    muxer_hints_t*                                      hints,
    int                                                 flags,
    size_t                                              qsize) {
...
static profile_t* profile_libav_matroska_builder(void) {
...
static int profile_libav_mp4_reopen(profile_chain_t* prch,
    muxer_config_t*                                  m_cfg,
    muxer_hints_t*                                   hints,
    int                                              flags) {
  muxer_config_t c;
...
static int profile_libav_mp4_open(profile_chain_t* prch,
    muxer_config_t*                                m_cfg,
    muxer_hints_t*                                 hints,
    int                                            flags,
    size_t                                         qsize) {
...
static profile_t* profile_libav_mp4_builder(void) {
...
static htsmsg_t* profile_class_mc_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("Not set"), MC_UNKNOWN},
      {
          N_("Matroska (mkv)/built-in"),
          MC_MATROSKA,
      },
      {
          N_("WEBM/built-in"),
...
static int htsmsg_add_entry(htsmsg_t* list, const char* key, const char* val) {
  htsmsg_t* map = NULL;
...
static htsmsg_t* profile_class_codec_profile_make_list(const char* lang) {
  htsmsg_t* list = NULL;
...
static htsmsg_t* profile_class_codec_profiles_list(enum AVMediaType media_type, const char* lang) {
...
static htsmsg_t* profile_class_pro_vcodec_list(void* o, const char* lang) {
  return profile_class_codec_profiles_list(AVMEDIA_TYPE_VIDEO, lang);
}

static int profile_class_src_codec_set(profile_transcode_t* pro, char** str, const void* p) {
...
static const void* profile_class_src_codec_get(profile_transcode_t* pro, char* str) {
...
static int profile_class_src_vcodec_set(void* obj, const void* p) {
  profile_transcode_t* pro = (profile_transcode_t*)obj;
  return profile_class_src_codec_set(pro, &pro->pro_src_vcodec, p);
}

static const void* profile_class_src_vcodec_get(void* obj) {
  profile_transcode_t* pro = (profile_transcode_t*)obj;
  return profile_class_src_codec_get(pro, pro->pro_src_vcodec);
}

static htsmsg_t* profile_class_src_vcodec_list(void* o, const char* lang) {
  return strtab2htsmsg_str(profile_class_src_vcodec_tab, 1, lang);
}

static htsmsg_t* profile_class_pro_acodec_list(void* o, const char* lang) {
...
static int profile_class_src_acodec_set(void* obj, const void* p) {
  profile_transcode_t* pro = (profile_transcode_t*)obj;
  return profile_class_src_codec_set(pro, &pro->pro_src_acodec, p);
}

static const void* profile_class_src_acodec_get(void* obj) {
  profile_transcode_t* pro = (profile_transcode_t*)obj;
  return profile_class_src_codec_get(pro, pro->pro_src_acodec);
}

static htsmsg_t* profile_class_src_acodec_list(void* o, const char* lang) {
  return strtab2htsmsg_str(profile_class_src_acodec_tab, 1, lang);
}

static htsmsg_t* profile_class_pro_scodec_list(void* o, const char* lang) {
...
static int profile_class_src_scodec_set(void* obj, const void* p) {
  profile_transcode_t* pro = (profile_transcode_t*)obj;
  return profile_class_src_codec_set(pro, &pro->pro_src_scodec, p);
}

static const void* profile_class_src_scodec_get(void* obj) {
  profile_transcode_t* pro = (profile_transcode_t*)obj;
  return profile_class_src_codec_get(pro, pro->pro_src_scodec);
}

static htsmsg_t* profile_class_src_scodec_list(void* o, const char* lang) {
  return strtab2htsmsg_str(profile_class_src_scodec_tab, 1, lang);
}

const idclass_t profile_transcode_class = {.ic_super = &profile_class,
...
static int profile_transcode_can_share(profile_chain_t* prch, profile_chain_t* joiner) {
...
static int profile_transcode_work(profile_chain_t* prch,
    streaming_target_t*                            dst,
    uint32_t                                       timeshift_period,
    profile_work_flags_t                           flags) {
  profile_sharer_t*    prsh;
  ...
  if (!prsh->prsh_transcoder) {
    ...
    dst = prsh->prsh_transcoder = transcoder_create(&prsh->prsh_input, profiles, src_codecs);
    if (!dst)
      goto fail;
...
static int profile_transcode_mc_valid(int mc) {
  switch (mc) {
    case MC_MATROSKA:
    case MC_WEBM:
    case MC_MPEGTS:
...
static int profile_transcode_reopen(profile_chain_t* prch,
    muxer_config_t*                                  m_cfg,
    muxer_hints_t*                                   hints,
    int                                              flags) {
  profile_transcode_t* pro = (profile_transcode_t*)prch->prch_pro;
...
static int profile_transcode_open(profile_chain_t* prch,
    muxer_config_t*                                m_cfg,
    muxer_hints_t*                                 hints,
    int                                            flags,
    size_t                                         qsize) {
...
static void profile_transcode_free(profile_t* _pro) {
...
static profile_t* profile_transcode_builder(void) {
