 ...
 */
static char*
idnode_get_display(idnode_t* self, const property_t* p, const char* lang) {
  char* r = NULL;
  if (p) {
    if (p->rend)
      r = p->rend(self, lang);
      ...
      if (l && !idnode_get_u32(self, p->id, (uint32_t*)&v))
        HTSMSG_FOREACH(f, l) {
          ...
          if (!htsmsg_get_s32(m, "key", &k) &&
              (s = htsmsg_get_str(m, "val")) != NULL && v == k) {
            r = strdup(s);
            break;
          }
        }
...
void* idnode_find0(tvh_uuid_t* uuid,
    const idclass_t*           idc,
    const idnodes_rb_t*        domain) {
  char     buf[UUID_HEX_SIZE];
  idnode_t skel;
  skel.in_uuid = *uuid;
  tvhtrace(LS_IDNODE,
      "find node %s class %s",
      uuid_get_hex(uuid, buf),
      idc ? idc->ic_class : NULL);
  return idnode_find_(&skel, idc, domain);
}

void* idnode_find(const char* uuid,
    const idclass_t*          idc,
    const idnodes_rb_t*       domain) {
  idnode_t skel;
  tvhtrace(LS_IDNODE,
    ...
    if (f->type == IF_NUM) {
      ...
      if (p) {
        if (p->type == PT_U32 || p->type == PT_S64 || p->type == PT_TIME) {
          int64_t v = f->u.n.n;
          if (INTEXTRA_IS_SPLIT(p->intextra) &&
              p->intextra != f->u.n.intsplit) {
...
int idnode_filter(idnode_t* in, idnode_filter_t* filter, const char* lang) {
  ...
  LIST_FOREACH (f, filter, link) {
    ...
    if (f->type == IF_STR) {
      ...
      int         r = 1;
      str           = strdisp =
          idnode_get_display(in, idnode_find_prop(in, f->key), lang);
      if (!str)
...
void idnode_filter_add_str(idnode_filter_t* filt,
    const char*                             key,
    const char*                             val,
    int                                     comp) {
...
void idnode_filter_add_dbl(idnode_filter_t* filt,
    const char*                             key,
    double                                  dbl,
    int                                     comp) {
  idnode_filter_ele_t* ele = calloc(1, sizeof(idnode_filter_ele_t));
...
void idnode_filter_add_bool(idnode_filter_t* filt,
    const char*                              key,
    int                                      val,
    int                                      comp) {
  idnode_filter_ele_t* ele = calloc(1, sizeof(idnode_filter_ele_t));
...
void idnode_set_add(idnode_set_t* is,
    idnode_t*                     in,
...
static int idnode_class_write_values(idnode_t* self,
    const idclass_t*                           idc,
    htsmsg_t*                                  c,
    int                                        optmask) {
  int save = 0;
...
void idnode_read0(idnode_t* self,
    htsmsg_t*               c,
    htsmsg_t*               list,
    int                     optmask,
    const char*             lang) {
...
static htsmsg_t* idnode_params(const idclass_t* idc,
    idnode_t*                                   self,
    htsmsg_t*                                   list,
    int                                         optmask,
    const char*                                 lang) {
...
static htsmsg_t* idclass_get_property_groups(const idclass_t* idc,
    const char*                                               lang) {
 ...
 */
htsmsg_t* idclass_serialize0(const idclass_t* idc,
    htsmsg_t*                                 list,
    int                                       optmask,
    const char*                               lang) {
  const char* s;
 ...
 *
 */
htsmsg_t* idnode_serialize0(idnode_t* self,
    htsmsg_t*                         list,
    int                               optmask,
    const char*                       lang) {
  const idclass_t* idc = self->in_class;
  ...
  htsmsg_add_str(m,
      "text",
      idnode_get_title(self, lang, buf, sizeof(buf)) ?: "");
  if ((s = idclass_get_caption(idc, lang)))
    htsmsg_add_str(m, "caption", s);
  if ((s = idclass_get_class(idc)))
    htsmsg_add_str(m, "class", s);
...
idnode_slist_enum(idnode_t* in, idnode_slist_t* options, const char* lang) {
  ...
  for (; options->id; options++) {
    m = htsmsg_create_key_val(options->id,
...
int idnode_slist_set(idnode_t* in,
    idnode_slist_t*            options,
    const htsmsg_t*            vals) {
  idnode_slist_t* o;
  htsmsg_field_t* f;
  int *           ip, changed = 0;
...
char* idnode_slist_rend(idnode_t* in,
    idnode_slist_t*               options,
    const char*                   lang) {
  int*   ip;
  size_t l = 0;
...
char* idnode_list_get_csv1(idnode_list_head_t* in1_list, const char* lang) {
  ...
  LIST_FOREACH (ilm, in1_list, ilm_in1_link)
    htsmsg_add_str(l,
        NULL,
        idnode_get_title(ilm->ilm_in2, lang, buf, sizeof(buf)));
...
char* idnode_list_get_csv2(idnode_list_head_t* in2_list, const char* lang) {
  ...
  LIST_FOREACH (ilm, in2_list, ilm_in2_link)
    htsmsg_add_str(l,
        NULL,
        idnode_get_title(ilm->ilm_in1, lang, buf, sizeof(buf)));
