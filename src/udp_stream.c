...
udp_stream_t* create_udp_stream(udp_connection_t* uc, const char* hint) {
...
void delete_udp_stream(udp_stream_t* ustream) {
  tvh_mutex_lock(&udp_streams_mutex);
...
udp_stream_t* find_udp_stream_by_hint(const char* hint) {
  udp_stream_t* us;
  int           found = 0;
...
udp_stream_t* find_udp_stream_by_url(const char* url) {
  udp_stream_t* us;
  int           found = 0;
...
static void* udp_stream_thread(void* aux) {
...
int udp_stream_shutdown(udp_stream_t* us) {
  if (!atomic_get(&us->us_running)) {
