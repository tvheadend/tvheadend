...
static inline int gh_require_meta(int type) {
  return type == SCT_HEVC || type == SCT_H264 || type == SCT_MPEG2VIDEO ||
      type == SCT_MP4A || type == SCT_AAC || type == SCT_VORBIS ||
      type == SCT_THEORA;
}
...
static int header_complete(streaming_start_component_t* ssc, int not_so_picky) {
  ...
  if (is_video) {
    if (!not_so_picky &&
        (ssc->es_aspect_num == 0 || ssc->es_aspect_den == 0 ||
...
static int headers_complete(globalheaders_t* gh) {
  ...
  for (i = 0; i < ss->ss_num_components; i++) {
    ...
    qd[i] =
        gh_is_audiovideo(ssc->es_type) ? gh_queue_delay(gh, ssc->es_index) : 0;
    if (qd[i] > qd_max)
      qd_max = qd[i];
  }
    ...
    if (!header_complete(ssc, threshold)) {
      ...
      if (threshold || (qd[i] <= 0 && qd_max > (MAX_NOPKT_TIME * 90))) {
        ...
        tvhdebug(LS_GLOBALHEADERS,
            "gh disable stream %d %s%s%s (PID %i) threshold %d qd %" PRId64
            " qd_max %" PRId64,
            ssc->es_index,
...
static void gh_hold(globalheaders_t* gh, streaming_message_t* sm) {
  ...
  switch (sm->sm_type) {
    case SMT_PACKET:
      ...
      if (ssc == NULL) {
        tvherror(LS_GLOBALHEADERS,
            "Unable to find component %d",
            pkt->pkt_componentindex);
        streaming_msg_free(sm);
        return;
      }
      ...
      sm =
          streaming_msg_create_data(SMT_START, streaming_start_copy(gh->gh_ss));
