 ...
 *
 */
static inline int gh_require_meta(int type) {
  return type == SCT_HEVC || type == SCT_H264 || type == SCT_MPEG2VIDEO || type == SCT_MP4A ||
      type == SCT_AAC || type == SCT_VORBIS || type == SCT_THEORA;
}

/**
 *
 */
static inline int gh_is_audiovideo(int type) {
 ...
 *
 */
static void gh_flush(globalheaders_t* gh) {
  if (gh->gh_ss != NULL) {
 ...
 *
 */
static void apply_header(streaming_start_component_t* ssc, th_pkt_t* pkt) {
...
static int header_complete(streaming_start_component_t* ssc, int not_so_picky) {
  ...
  if (is_video) {
    if (!not_so_picky &&
        (ssc->es_aspect_num == 0 || ssc->es_aspect_den == 0 || ssc->es_width == 0 ||
            ssc->es_height == 0))
      return 0;
  }
 ...
 *
 */
static int64_t gh_queue_delay(globalheaders_t* gh, int index) {
  ...
  while (f != l) {
    if (f->pr_pkt->pkt_dts != PTS_UNSET) {
      ssc = streaming_start_component_find_by_index(gh->gh_ss, f->pr_pkt->pkt_componentindex);
  ...
  while (l != f) {
    if (l->pr_pkt->pkt_dts != PTS_UNSET) {
      ssc = streaming_start_component_find_by_index(gh->gh_ss, l->pr_pkt->pkt_componentindex);
      if (ssc && ssc->es_index == index)
        break;
    }
 ...
 *
 */
static int headers_complete(globalheaders_t* gh) {
  ...
  for (i = 0; i < ss->ss_num_components; i++) {
    ...
    qd[i] = gh_is_audiovideo(ssc->es_type) ? gh_queue_delay(gh, ssc->es_index) : 0;
    if (qd[i] > qd_max)
      qd_max = qd[i];
  }
    ...
    if (!header_complete(ssc, threshold)) {
      ...
      if (threshold || (qd[i] <= 0 && qd_max > (MAX_NOPKT_TIME * 90))) {
        ...
        tvhdebug(LS_GLOBALHEADERS,
            "gh disable stream %d %s%s%s (PID %i) threshold %d qd %" PRId64 " qd_max %" PRId64,
            ssc->es_index,
 ...
 *
 */
static void gh_start(globalheaders_t* gh, streaming_message_t* sm) {
 ...
 *
 */
static void gh_hold(globalheaders_t* gh, streaming_message_t* sm) {
  th_pkt_t*                    pkt;
  streaming_start_component_t* ssc;
  ...
  switch (sm->sm_type) {
    case SMT_PACKET:
      pkt = sm->sm_data;
      ssc = streaming_start_component_find_by_index(gh->gh_ss, pkt->pkt_componentindex);
      if (ssc == NULL) {
      ...
      sm = streaming_msg_create_data(SMT_START, streaming_start_copy(gh->gh_ss));
 ...
 *
 */
static void gh_pass(globalheaders_t* gh, streaming_message_t* sm) {
  th_pkt_t* pkt;
  switch (sm->sm_type) {
    case SMT_START:
      /* stop */
 ...
 *
 */
static void globalheaders_input(void* opaque, streaming_message_t* sm) {
...
static htsmsg_t* globalheaders_input_info(void* opaque, htsmsg_t* list) {
  globalheaders_t*    gh = opaque;
...
static streaming_ops_t globalheaders_input_ops = {.st_cb = globalheaders_input,
 ...
 *
 */
void globalheaders_destroy(streaming_target_t* pad) {
