...
normalize_ts(tsfix_t *tf, tfstream_t *tfs, th_pkt_t *pkt, int backlog)
{
  ...
  if (tfs->tfs_last_dts_norm == PTS_UNSET) {
     ...
     * range. Only the backlog window can legitimately precede it, and
     * the reference is pulled back by at most 160000 ticks to cover
     * that, so keep the rest of the range available to streams whose
     * first packet arrives late in a long session. */
    if (dts > PTS_MASK - 2 * 90000 || pkt->pkt_err) {
