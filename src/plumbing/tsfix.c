 ...
 */
static int64_t tsfix_ts_diff(int64_t ts1, int64_t ts2) {
  int64_t r;
  ts1 &= PTS_MASK;
  ts2 &= PTS_MASK;
 ...
 *
 */
static void tsfix_destroy_streams(tsfix_t* tf) {
  tfstream_t* tfs;
...
static tfstream_t* tfs_find(tsfix_t* tf, th_pkt_t* pkt) {
  tfstream_t* tfs;
 ...
 *
 */
static tfstream_t* tsfix_add_stream(tsfix_t* tf, int index, streaming_component_type_t type) {
 ...
 *
 */
static void tsfix_start(tsfix_t* tf, streaming_start_t* ss) {
 ...
 *
 */
static void tsfix_stop(tsfix_t* tf) {
 ...
 *
 */
static void tsfix_packet_drop(tfstream_t* tfs, th_pkt_t* pkt, const char* reason) {
  if (tvhtrace_enabled()) {
    char buf[64];
 ...
 *
 */
static void normalize_ts(tsfix_t* tf, tfstream_t* tfs, th_pkt_t* pkt, int backlog) {
 ...
 *
 */
static inline int txfix_need_to_update_ref(tsfix_t* tf, tfstream_t* tfs, th_pkt_t* pkt) {
  return tfs->tfs_local_ref == PTS_UNSET && tf->tf_tsref != PTS_UNSET && pkt->pkt_dts != PTS_UNSET;
}

/**
 *
 */
static int tsfix_update_ref(tsfix_t* tf, tfstream_t* tfs, th_pkt_t* pkt) {
  tfstream_t* tfs2;
  int64_t     diff;
  ...
  if (tfs->tfs_audio) {
    ...
    if (diff > 3 * 90000) {
      tvhwarn(LS_TSFIX,
          "The timediff for %s is big (%" PRId64 "), using current dts",
          streaming_component_type2txt(tfs->tfs_type),
          diff);
    ...
    LIST_FOREACH (tfs2, &tf->tf_streams, tfs_link)
      if (tfs2->tfs_audio && tfs2->tfs_last_dts_in != PTS_UNSET) {
        ...
        if (diff > 6 * 90000) {
          tvhwarn(LS_TSFIX,
              "The timediff for %s is big (%" PRId64 "), using audio dts",
              streaming_component_type2txt(tfs->tfs_type),
              diff);
          tfs->tfs_parent    = tfs2;
 ...
 *
 */
static void tsfix_backlog(tsfix_t* tf) {
  th_pkt_t*   pkt;
  tfstream_t* tfs;
  int         r;
 ...
 *
 */
static int64_t tsfix_backlog_diff(tsfix_t* tf) {
  th_pkt_t*    pkt;
  th_pktref_t* pr;
  tfstream_t*  tfs;
  int64_t      res = 0;
  ...
  PKTREF_FOREACH(pr, &tf->tf_backlog) {
    ...
    if (pkt->pkt_dts == PTS_UNSET)
      continue;
    if (pkt->pkt_dts >= tf->tf_tsref)
      continue;
    if (tf->tf_tsref > (PTS_MASK * 3) / 4 && pkt->pkt_dts < PTS_MASK / 4)
      continue;
    tfs = tfs_find(tf, pkt);
    if (!tfs->tfs_audio && !tfs->tfs_video)
      continue;
 ...
 */
static void recover_pts(tsfix_t* tf, tfstream_t* tfs, th_pkt_t* pkt) {
  th_pktref_t* srch;
  int          total;
        ...
        switch (pkt->v.pkt_frametype) {
          case PKT_B_FRAME:
            if (pkt->pkt_pts == PTS_UNSET) {
              ...
              tvhtrace(LS_TSFIX,
                  "%-12s PTS b-frame set to %" PRId64 " (old %" PRId64 ")",
                  streaming_component_type2txt(tfs->tfs_type),
                  pkt->pkt_dts,
                  pkt->pkt_pts);
            ...
            if (pkt->pkt_pts == PTS_UNSET) {
              ...
              PKTREF_FOREACH(srch, &tf->tf_ptsq) {
                ...
                if (srch->pr_pkt->v.pkt_frametype <= PKT_P_FRAME &&
                  ...
                  tvhtrace(LS_TSFIX,
                      "%-12s PTS *-frame set to %" PRId64 " (old %" PRId64 "), DTS %" PRId64,
                      streaming_component_type2txt(tfs->tfs_type),
                      srch->pr_pkt->pkt_dts,
                      pkt->pkt_pts,
                      pkt->pkt_dts);
 ...
 */
static void compute_pts(tsfix_t* tf, tfstream_t* tfs, th_pkt_t* pkt) {
  // If PTS is missing, set it to DTS if not video
  if (pkt->pkt_pts == PTS_UNSET && !tfs->tfs_video) {
    pkt->pkt_pts = pkt->pkt_dts;
    tvhtrace(LS_TSFIX,
        "%-12s PTS set to %" PRId64,
        streaming_component_type2txt(tfs->tfs_type),
        pkt->pkt_pts);
  }
 ...
 *
 */
static void tsfix_input_packet(tsfix_t* tf, streaming_message_t* sm) {
  th_pkt_t*   pkt;
  tfstream_t *tfs, *tfs2;
  ...
  if (tf->tf_tsref == PTS_UNSET && pkt->pkt_dts != PTS_UNSET &&
    ...
    if (diff > threshold) {
      ...
      tvhtrace(LS_TSFIX,
          "reference clock set to %" PRId64 " (dts %" PRId64 " pcr %" PRId64 " backlog %" PRId64
          ")",
          tf->tf_tsref,
          pkt->pkt_dts,
          pkt->pkt_pcr,
  ...
  if (pkt->pkt_dts == PTS_UNSET) {
    ...
    if (pkt->pkt_payload != NULL || pkt->pkt_pts != PTS_UNSET) {
      ...
      tvhtrace(LS_TSFIX,
          "%-12s DTS set to last %" PRId64 " +%d == %" PRId64 ", PTS = %" PRId64,
          streaming_component_type2txt(tfs->tfs_type),
          tfs->tfs_last_dts_in,
          pdur,
          pkt->pkt_dts,
 ...
 *
 */
static void tsfix_input(void* opaque, streaming_message_t* sm) {
  tsfix_t* tf = opaque;
...
static htsmsg_t* tsfix_input_info(void* opaque, htsmsg_t* list) {
  tsfix_t*            tf = opaque;
...
static streaming_ops_t tsfix_input_ops = {.st_cb = tsfix_input, .st_info = tsfix_input_info};
 ...
 *
 */
void tsfix_destroy(streaming_target_t* pad) {
