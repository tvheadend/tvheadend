...
txfix_need_to_update_ref(tsfix_t* tf, tfstream_t* tfs, th_pkt_t* pkt) {
  return tfs->tfs_local_ref == PTS_UNSET && tf->tf_tsref != PTS_UNSET &&
...
static int tsfix_update_ref(tsfix_t* tf, tfstream_t* tfs, th_pkt_t* pkt) {
  ...
  if (tfs->tfs_audio) {
    ...
    if (diff > 2 * 90000) {
      tvhwarn(LS_TSFIX,
          "The timediff for TELETEXT is big (%" PRId64 "), using current dts",
          diff);
      tfs->tfs_local_ref = pkt->pkt_dts;
    } else {
      tfs->tfs_local_ref = tf->tf_tsref;
            ...
            if (pkt->pkt_pts == PTS_UNSET) {
              ...
              PKTREF_FOREACH(srch, &tf->tf_ptsq) {
                ...
                if (srch->pr_pkt->v.pkt_frametype <= PKT_P_FRAME &&
                    pts_is_greater_or_equal(pkt->pkt_dts,
                        srch->pr_pkt->pkt_dts) > 0 &&
                    pts_diff(pkt->pkt_dts, srch->pr_pkt->pkt_dts) <
                        10 * 90000) {
                  tvhtrace(LS_TSFIX,
                      "%-12s PTS *-frame set to %" PRId64 " (old %" PRId64
                      "), DTS %" PRId64,
                      streaming_component_type2txt(tfs->tfs_type),
                      srch->pr_pkt->pkt_dts,
                      pkt->pkt_pts,
...
static void tsfix_input_packet(tsfix_t* tf, streaming_message_t* sm) {
  ...
  if (tf->tf_tsref == PTS_UNSET && pkt->pkt_dts != PTS_UNSET &&
    ...
    threshold = 22500;
    LIST_FOREACH (tfs2, &tf->tf_streams, tfs_link)
      if (tfs != tfs2 && (tfs2->tfs_audio || tfs2->tfs_video) &&
          !tfs2->tfs_seen) {
        threshold = 90000;
        break;
      }
    ...
    if (diff > threshold) {
      ...
      tvhtrace(LS_TSFIX,
          "reference clock set to %" PRId64 " (dts %" PRId64 " pcr %" PRId64
          " backlog %" PRId64 ")",
          tf->tf_tsref,
          pkt->pkt_dts,
          pkt->pkt_pcr,
  ...
  if (pkt->pkt_dts == PTS_UNSET) {
    ...
    if (pkt->pkt_payload != NULL || pkt->pkt_pts != PTS_UNSET) {
      ...
      tvhtrace(LS_TSFIX,
          "%-12s DTS set to last %" PRId64 " +%d == %" PRId64
          ", PTS = %" PRId64,
          streaming_component_type2txt(tfs->tfs_type),
          tfs->tfs_last_dts_in,
          pdur,
...
static streaming_ops_t tsfix_input_ops = {.st_cb = tsfix_input,
