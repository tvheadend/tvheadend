 ...
 * CRC32
 */
static uint32_t crc_tab[256] = {0x00000000,
    0x04c11db7,
    0x09823b6e,
    0x0d4326d9,
    0x130476dc,
 ...
 *
 */
static const int sample_rates[16] = {96000,
    88200,
    64000,
    48000,
    44100,
...
/* ---------------- private code */
static const uint8_t map2[] = {0x3e,
    0xff,
    0xff,
    0xff,
    0x3f,
...
uint32_t sbuf_peek_u32(sbuf_t* sb, int off) {
  ...
  if (ENDIAN_SWAP_COND(sb->sb_bswap))
    ...
    return (((uint16_t)p[0]) << 24) | (((uint16_t)p[1]) << 16) | (((uint16_t)p[2]) << 8) | p[3];
}
...
static uint8_t* openssl_hash(uint8_t* hash, const uint8_t* msg, size_t msglen, const EVP_MD* md) {
  EVP_MD_CTX* mdctx;
...
static char* openssl_hash_hexstr(const char* str, int lowercase, const EVP_MD* md, int len) {
  int         i;
  uint8_t     hash[len];
...
int makedirs(int subsys, const char* inpath, int mode, int mstrict, gid_t gid, uid_t uid) {
  int         err, ok;
  size_t      x;
  struct stat st;
  char*       path;
  ...
  while (ok) {
    ...
    if (path[x] == '/' || !path[x]) {
      ...
      if (stat(path, &st)) {
        ...
        if (!err && !stat(path, &st) && FILE_MODE_BITS(mode) != FILE_MODE_BITS(st.st_mode)) {
          err = chmod(path, mode); /* override umode */
          if (!mstrict) {
            err = 0;
            tvhwarn(subsys,
                "Unable to change directory permissions "
                "to \"%o\" for \"%s\" (keeping \"%o\")",
                mode,
                path,
                FILE_MODE_BITS(st.st_mode));
            mode = FILE_MODE_BITS(st.st_mode);
          }
        }
        tvhtrace(subsys,
            "Creating directory \"%s\" with octal permissions "
            "\"%o\" gid %d uid %d",
            path,
            mode,
            gid,
      ...
      if (err) {
        tvhalert(subsys, "Unable to create dir \"%s\": %s", path, strerror(errno));
        return -1;
      }
      path[x] = '/';
    }
...
int mpegts_word_count(const uint8_t* tsb, int len, uint32_t mask) {
  ...
  while (len >= 188) {
    if (len >= 4 * 188 && (mpegts_word32(tsb + 0 * 188) & mask) == val &&
...
static void deferred_unlink_dir_cb(void* s, int dearmed) {
