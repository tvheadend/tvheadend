...
static uint8_t* openssl_hash(uint8_t* hash,
    const uint8_t*                    msg,
    size_t                            msglen,
    const EVP_MD*                     md) {
  EVP_MD_CTX* mdctx;
...
char* sha256sum_base64(const char* str) {
  ...
  if (out == NULL)
    return NULL;
  if (openssl_hash(hash, (const uint8_t*)str, strlen(str), EVP_sha256()) ==
      NULL) {
    free(out);
    return NULL;
  }
...
htsmsg_t* network_interfaces_enum(void* obj, const char* lang) {
  ...
  if (ifnames) {
    ...
    for (ifname = ifnames; ifname->if_name; ifname++)
      htsmsg_add_msg(list,
          NULL,
          htsmsg_create_key_val(ifname->if_name, ifname->if_name));
