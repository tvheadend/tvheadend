...
tvh_iconv(iconv_t cd, char** inbuf, size_t* inbytesleft, char** outbuf, size_t* outbytesleft) {
  return iconv(cd, inbuf, inbytesleft, outbuf, outbytesleft);
}

static void intlconv_test(void) {
  ...
  if (s == NULL ||
      (strcmp(s, "ZlutouckyKun") && strcmp(s, "Zlutouck'yKun") && strcmp(s, "?lu?ou?k?K??"))) {
...
static int intlconv_cmp(intlconv_cache_t* a, intlconv_cache_t* b) {
...
char* intlconv_charset_id(const char* charset, int transil, int ignore_bad_chars) {
  ...
  if (charset == NULL || charset[0] == '\0' || strcasecmp(charset, "UTF8") == 0 ||
      strcasecmp(charset, "UTF-8") == 0)
    return NULL;
  delim = index(charset, '/') ? (charset[strlen(charset) - 1] != '/' ? "/" : "") : "//";
...
ssize_t
intlconv_utf8(char* dst, size_t dst_size, const char* dst_charset_id, const char* src_utf8) {
  ...
  tvh_mutex_lock(&intlconv_lock);
  if (intlconv_last_ic && strcmp(intlconv_last_ic->ic_charset_id, dst_charset_id) == 0) {
    ic = intlconv_last_ic;
    goto found;
  }
...
ssize_t intlconv_to_utf8(char* dst,
  ...
  if (intlconv_last_src_ic && strcmp(intlconv_last_src_ic->ic_charset_id, src_charset_id) == 0) {
...
const char* intlconv_charsets[] = {"437",
    "500",
    "500V1",
    "850",
    "851",
