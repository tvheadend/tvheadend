...
static inline size_t tvh_iconv(iconv_t cd,
    char**                             inbuf,
    size_t*                            inbytesleft,
    char**                             outbuf,
    size_t*                            outbytesleft) {
...
static void intlconv_test(void) {
  ...
  if (s == NULL ||
      (strcmp(s, "ZlutouckyKun") && strcmp(s, "Zlutouck'yKun") &&
          strcmp(s, "?lu?ou?k?K??"))) {
    tvherror(LS_MAIN,
        "iconv() routine is not working properly (%s), aborting!",
        s);
...
char* intlconv_charset_id(const char* charset,
  ...
  if (charset == NULL || charset[0] == '\0' ||
      strcasecmp(charset, "UTF8") == 0 || strcasecmp(charset, "UTF-8") == 0)
    return NULL;
