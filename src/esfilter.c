...
struct esfilter_entry_queue esfilters[ESF_CLASS_LAST];
...
esfilter_class_moveup(idnode_t *self)
{
  ...
  tvhlog(LOG_INFO, "test", "moveup %s", idnode_uuid_as_str(self));
}
...
esfilter_class_movedown(idnode_t *self)
{
  ...
  tvhlog(LOG_INFO, "test", "movedown %s", idnode_uuid_as_str(self));
}
...
esfilter_class_service_set(void *o, const void *v)
{
  ...
  const char *s = v;
  int save = 0;
  if (strncmp(esf->esf_service, s, UUID_STR_LEN)) {
    strncpy(esf->esf_service, s, UUID_STR_LEN);
    esf->esf_service[UUID_STR_LEN-1] = '\0';
    save = 1;
  }
  return save;
}
