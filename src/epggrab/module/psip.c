...
static int _psip_eit_callback(mpegts_table_t* mt,
    const uint8_t*                            ptr,
    int                                       len,
    int                                       tableid);
static int _psip_ett_callback(mpegts_table_t* mt,
    const uint8_t*                            ptr,
    int                                       len,
    int                                       tableid);
...
typedef struct psip_table {
  ...
  mpegts_table_t*
           pt_table; /* pointer to mpegts_table to receive EIT/ETT tables/sections */
...
static void psip_status_destroy(mpegts_table_t* mt) {
...
static psip_table_t* psip_find_table(psip_status_t* ps, uint16_t pid) {
  psip_table_t* pt;
...
static mpegts_table_t* psip_activate_table(psip_status_t* ps,
    psip_table_t*                                         pt) {
  mpegts_table_t* mt;

  if (IS_EIT(pt->pt_type)) {
    /* This is an EIT table */
    mt = mpegts_table_add(ps->ps_mm,
        DVB_ATSC_EIT_BASE,
        DVB_ATSC_EIT_MASK,
        _psip_eit_callback,
        ps,
  ...
  pt->pt_table   = mt;
  tvhtrace(LS_PSIP,
      "table activated - pid 0x%04X type 0x%04X",
      mt->mt_pid,
      pt->pt_type);
  return mt;
}

static void psip_reschedule_tables(psip_status_t* ps) {
  ...
  TAILQ_FOREACH (pt, &ps->ps_tables, pt_link) {
    ...
    if (pt->pt_table) {
      tvhtrace(LS_PSIP,
          "table late: pid = 0x%04X, type = 0x%04X\n",
          pt->pt_pid,
          pt->pt_type);
      mpegts_table_destroy(pt->pt_table);
      pt->pt_table = NULL;
...
static int _psip_eit_callback_channel(psip_status_t* ps,
    channel_t*                                       ch,
    const uint8_t*                                   ptr,
    int                                              len,
    int                                              count) {
  ...
  for (i = 0; len >= 12 && i < count; len -= size, ptr += size, i++) {
    ...
    tvhtrace(LS_PSIP,
        "  %03d: [%s] eventid 0x%04x at %" PRItime_t
        ", duration %d, etmlocation %x, title: '%s' (%d bytes)",
        i,
        ...
        length,
        etmlocation,
        lang_str_get(title, NULL),
        titlelen);

    save2 = changes2 = 0;

    ebc =
        epg_broadcast_find_by_time(ch, mod, start, stop, 1, &save2, &changes2);
    tvhtrace(LS_PSIP,
        "  eid=%5d, start=%" PRItime_t ", stop=%" PRItime_t ", ebc=%p",
        eventid,
        start,
        stop,
        ebc);
...
static int _psip_eit_callback(mpegts_table_t* mt,
  ...
  if (!svc) {
    tvhtrace(LS_PSIP,
        "EIT with no associated channel found (tsid 0x%04x)",
        tsid);
    return -1;
  }
...
static int _psip_ett_callback(mpegts_table_t* mt,
  ...
  if (!svc) {
    tvhtrace(LS_PSIP,
        "ETT with no associated channel found (sourceid 0x%04x)",
        sourceid);
    return -1;
  }
  ...
  if (!isevent) {
    tvhtrace(LS_PSIP,
        "0x%04x: channel ETT tableid 0x%04X [%s], ver %d",
        mt->mt_pid,
        tsid,
        svc->s_dvb_svcname,
        ver);
    ...
    LIST_FOREACH (ilm, &svc->s_channels, ilm_in1_link) {
      ...
      if (ebc && ebc->grabber == mod) {
        ...
        tvhtrace(LS_PSIP,
            "0x%04x: ETT tableid 0x%04X [%s], eventid 0x%04X (%d) ['%s'], ver "
            "%d",
            mt->mt_pid,
            tsid,
            svc->s_dvb_svcname,
    ...
    if (found == 0) {
      tvhtrace(LS_PSIP,
          "0x%04x: ETT tableid 0x%04X [%s], eventid 0x%04X (%d), ver %d - no "
          "matching broadcast found [%.80s]",
          mt->mt_pid,
          tsid,
          svc->s_dvb_svcname,
          eventid,
...
static int _psip_mgt_callback(mpegts_table_t* mt,
    const uint8_t*                            ptr,
    int                                       len,
    int                                       tableid) {
  int                       r;
  ...
  tvhtrace(LS_PSIP,
      "0x%04x: MGT tsid %04X (%d), ver %d, count %d",
      mt->mt_pid,
      tsid,
      tsid,
      ver,
  ...
  for (i = 0; i < count && len >= 11; i++) {
    ...
    tvhdebug(LS_PSIP,
        "table %d - type 0x%04X, pid 0x%04X, ver 0x%04X, size 0x%08X",
        i,
        type,
        tablepid,
        tablever,
...
static int _psip_start(epggrab_ota_map_t* map, mpegts_mux_t* dm) {
  mpegts_table_t* mt;
  psip_status_t*  ps;
...
static int _psip_stop(epggrab_ota_map_t* map, mpegts_mux_t* dm) {
...
static int
_psip_tune(epggrab_ota_map_t* map, epggrab_ota_mux_t* om, mpegts_mux_t* mm) {
  int                     r = 0;
  mpegts_service_t*       s;
...
void psip_init(void) {
  ...
  epggrab_module_ota_create(NULL,
      "psip",
      LS_PSIP,
      NULL,
      "PSIP: ATSC Grabber",
