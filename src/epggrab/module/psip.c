...
static psip_table_t* psip_update_table(psip_status_t* ps, uint16_t pid, int type) {
  psip_table_t* pt;
...
static mpegts_table_t* psip_activate_table(psip_status_t* ps, psip_table_t* pt) {
  mpegts_table_t* mt;
  ...
  if (!isevent) {
    ...
    LIST_FOREACH (ilm, &svc->s_channels, ilm_in1_link) {
      ...
      if (ebc && ebc->grabber == mod) {
        ...
        tvhtrace(LS_PSIP,
            "0x%04x: ETT tableid 0x%04X [%s], eventid 0x%04X (%d) ['%s'], ver %d",
            mt->mt_pid,
            tsid,
            svc->s_dvb_svcname,
            eventid,
    ...
    if (found == 0) {
      tvhtrace(LS_PSIP,
          "0x%04x: ETT tableid 0x%04X [%s], eventid 0x%04X (%d), ver %d - no matching broadcast "
          "found [%.80s]",
          mt->mt_pid,
          tsid,
          svc->s_dvb_svcname,
...
static int _psip_mgt_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  int                       r;
  int                       sect, last, ver;
  int                       count, i;
  uint16_t                  tsid;
...
static int _psip_tune(epggrab_ota_map_t* map, epggrab_ota_mux_t* om, mpegts_mux_t* mm) {
  int                     r = 0;
  mpegts_service_t*       s;
...
void psip_init(void) {
  ...
  epggrab_module_ota_create(NULL, "psip", LS_PSIP, NULL, "PSIP: ATSC Grabber", 1, NULL, &ops);
}
