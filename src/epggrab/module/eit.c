...
static void _eit_dtag_dump(epggrab_module_t* mod,
    uint8_t                                  dtag,
    uint8_t                                  dlen,
    const uint8_t*                           buf) {
#if APS_DEBUG
...
static dvb_string_conv_t _eit_freesat_conv[2] = {{0x1f, freesat_huffman_decode},
    {0x00, NULL}};
 ...
 * Short Event - 0x4d
 */
static int _eit_desc_short_event(epggrab_module_t* mod,
    const uint8_t*                                 ptr,
    int                                            len,
    eit_event_t*                                   ev) {
  int  r;
  ...
  /* Title */
  if ((r = _eit_get_string_with_len(mod,
           buf,
           sizeof(buf),
           ptr,
           len,
 ...
 * Extended Event - 0x4e
 */
static int _eit_desc_ext_event(epggrab_module_t* mod,
    const uint8_t*                               ptr,
    int                                          len,
    eit_event_t*                                 ev) {
  int            r, ilen;
...
static int _eit_desc_component(epggrab_module_t* mod,
    const uint8_t*                               ptr,
    int                                          len,
    eit_event_t*                                 ev) {
  uint8_t c, t;
  ...
  if (c == 0x1) {
    if (t > 0x08 && t < 0x11) {
      ...
      if (t != 0x09 && t != 0x0d)
        ev->ws = 1;
    } else if (t == 0x02 || t == 0x03 || t == 0x04 || t == 0x06 || t == 0x07 ||
        t == 0x08) {
      ev->ws = 1;
    }
...
static int _eit_desc_content(epggrab_module_t* mod,
    const uint8_t*                             ptr,
    int                                        len,
    eit_event_t*                               ev) {
  uint8_t tempPtr =
      0; // Temporary variable to hold a (potentially) changed *ptr value.
  tempPtr = *ptr;
  while (len > 1) {
    // If the genre translation table has been loaded, it will not be a null
    // pointer.
    ...
    if (tempPtr == 0xb1) // 0xB1 is the genre code for 'Black and White'
      ev->bw = 1;
        ...
        0xb0) { // 0xB0 is the start of the 'Special Characteristics' block.
      if (!ev->genre)
        ev->genre = calloc(1, sizeof(epg_genre_list_t));
      epg_genre_list_add_by_eit(ev->genre,
          (const uint8_t)tempPtr); // Cast as a 'const'
    }
    len -= 2;
    ptr += 2;
  }
 ...
 */
static int _eit_desc_parental(epggrab_module_t* mod,
    const uint8_t*                              ptr,
    int                                         len,
    eit_event_t*                                ev) {
  int cnt = 0, sum = 0, i = 3;
...
static int _eit_desc_crid(epggrab_module_t* mod,
  ...
  while (len > 3) {
    ...
    if ((*ptr & 0x3) == 0) {
      ...
      r = _eit_get_string_with_len(mod,
          buf,
          sizeof(buf),
          ptr + 1,
          len - 1,
 ...
 */
static void
_eit_scrape_episode(lang_str_t* str, eit_module_t* eit_mod, eit_event_t* ev) {
  lang_str_ele_t* se;
  char            buffer[2048];
  ...
  RB_FOREACH (se, str, link) {
    if (eit_pattern_apply_list(buffer,
      ...
      if ((ev->en.s_num = positive_atoi(buffer))) {
        tvhtrace(LS_TBL_EIT,
            "  extract season number %d using %s",
            ev->en.s_num,
            eit_mod->id);
        break;
      }
  ...
  RB_FOREACH (se, str, link) {
    if (eit_pattern_apply_list(buffer,
      ...
      if ((ev->en.e_num = positive_atoi(buffer))) {
        tvhtrace(LS_TBL_EIT,
            "  extract episode number %d using %s",
            ev->en.e_num,
            eit_mod->id);
        break;
      }
...
static int _eit_process_event_one(epggrab_module_t* mod,
  ...
  if (rsonly) {
    ...
    _ebc.stop    = stop;
    _ebc.episodelink =
        epg_set_broadcast_find_by_uri(&epg_episodelinks, ev->uri);
  ...
  *save |= epg_broadcast_set_is_bw(ebc, ev->bw, &changes);
  ...
  if (ev->copyright_year > 0)
    *save |=
        epg_broadcast_set_copyright_year(ebc, ev->copyright_year, &changes);
...
static int
_eit_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  int                       r, sect, last, ver;
  uint8_t                   seg;
  ...
  if ((hacks & EIT_HACK_EXTRAMUXLOOKUP) != 0 &&
      (tableid == 0x50 || tableid == 0x4E)) {
...
static int _eit_activate(void* m, int e) {
  eit_module_t* mod = m;
  tvhtrace(LS_TBL_EIT,
      "_eit_activate %s change to %d from %d with scrape_episode of %d and "
      "scrape_subtitle of %d",
      mod->id,
      e,
      mod->active,
      mod->scrape_episode,
...
static int
_eit_tune(epggrab_ota_map_t* map, epggrab_ota_mux_t* om, mpegts_mux_t* mm) {
  int                     r = 0;
  mpegts_service_t*       s;
...
static void
eit_queue_priv(mpegts_mux_t* dm, const char* src, eit_private_t* priv) {
  epggrab_ota_mux_t*           om;
...
void eit_nit_callback(mpegts_table_t* mt,
    uint16_t                          nbid,
    const char*                       name,
    uint32_t                          nitpriv) {
  ...
  tvhtrace(LS_TBL_EIT,
      "NIT - tsid %04X (%d) onid %04X (%d) nbid %04X (%d) network name '%s' "
      "private %08X",
      dm->mm_tsid,
      dm->mm_tsid,
      dm->mm_onid,
...
static void _eit_module_load_config(eit_module_t* mod) {
  ...
  tvhinfo(LS_TBL_EIT,
      "scraper %s attempt to load config \"%s\"",
      mod->id,
      config_file);
  ...
  if (m) {
    ...
    if (r > 0)
      tvhinfo(LS_TBL_EIT,
          "scraper %s loaded config \"%s\"",
          mod->id,
          config_file);
    else
      tvhwarn(LS_TBL_EIT,
          "scraper %s failed to load config \"%s\"",
          mod->id,
          config_file);
    htsmsg_destroy(m);
  } else {
    ...
    .ic_properties = (const property_t[]){
        {
            ...
            .desc  = N_("Set the short EIT destription to given target "
                       "(subtitle, summary or both)."),
...
static eit_module_t* eit_module_ota_create(const char* id,
    ...
    epggrab_ota_module_ops_t*                          ops) {
  eit_module_t* mod =
      (eit_module_t*)epggrab_module_ota_create(calloc(1, sizeof(eit_module_t)),
          id,
          subsys,
          saveid,
          name,
...
static void eit_parse_list(htsmsg_t* conf,
    const char*                      fname,
    uint16_t*                        list,
    int                              list_len,
    int*                             count) {
...
static void eit_init_one(const char* id, htsmsg_t* conf) {
  ...
  priv->name = name_str;
  if (name_str) {
    priv->module = (epggrab_module_ota_t*)eit_module_ota_create(id,
        LS_TBL_EIT,
        NULL,
