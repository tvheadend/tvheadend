...
static void _eit_dtag_dump(epggrab_module_t* mod, uint8_t dtag, uint8_t dlen, const uint8_t* buf) {
#if APS_DEBUG
  int  i = 0, j = 0;
  char tmp[100];
...
static dvb_string_conv_t _eit_freesat_conv[2] = {{0x1f, freesat_huffman_decode}, {0x00, NULL}};

/*
 * Get string
 */
static int _eit_get_string_with_len(epggrab_module_t* mod,
    char*                                             dst,
    size_t                                            dstlen,
    const uint8_t*                                    src,
    size_t                                            srclen,
 ...
 */
static int
_eit_desc_short_event(epggrab_module_t* mod, const uint8_t* ptr, int len, eit_event_t* ev) {
  int  r;
  char lang[4];
  char buf[512];
  ...
  /* Title */
  if ((r = _eit_get_string_with_len(mod, buf, sizeof(buf), ptr, len, ev->default_charset)) < 0) {
    return -1;
  } else if (r > 1) {
    if (!ev->title)
      ev->title = lang_str_create();
 ...
 */
static int
_eit_desc_ext_event(epggrab_module_t* mod, const uint8_t* ptr, int len, eit_event_t* ev) {
  int            r, ilen;
  ...
  while (ilen) {
    ...
    /* Key */
    if ((r = _eit_get_string_with_len(mod, ikey, sizeof(ikey), iptr, ilen, ev->default_charset)) <
        0)
      break;
    ...
    /* Value */
    if ((r = _eit_get_string_with_len(mod, ival, sizeof(ival), iptr, ilen, ev->default_charset)) <
        0)
      break;
  ...
  /* Description */
  if (_eit_get_string_with_len(mod, buf, sizeof(buf), ptr, len, ev->default_charset) > 1) {
    if (!ev->desc)
      ev->desc = lang_str_create();
...
static int
_eit_desc_component(epggrab_module_t* mod, const uint8_t* ptr, int len, eit_event_t* ev) {
  uint8_t c, t;
...
static int _eit_desc_content(epggrab_module_t* mod, const uint8_t* ptr, int len, eit_event_t* ev) {
  ...
  while (len > 1) {
    ...
    if (epggrab_ota_genre_translation) {
      ...
      if (tempPtr != *ptr) {
        tvhtrace(LS_TBL_EIT,
            "Translating '%d' (0x%02x) to '%d' (0x%02x)",
            *ptr,
            *ptr,
            tempPtr,
            tempPtr);
 ...
 */
static int _eit_desc_parental(epggrab_module_t* mod, const uint8_t* ptr, int len, eit_event_t* ev) {
  int cnt = 0, sum = 0, i = 3;
 ...
 * Content ID - 0x76
 */
static int _eit_desc_crid(epggrab_module_t* mod,
    const uint8_t*                          ptr,
    int                                     len,
    eit_event_t*                            ev,
    eit_data_t*                             ed) {
  ...
  while (len > 3) {
    ...
    if ((*ptr & 0x3) == 0) {
      ...
      r = _eit_get_string_with_len(mod, buf, sizeof(buf), ptr + 1, len - 1, ev->default_charset);
      if (r < 0)
        return -1;
      if (r == 0)
        continue;
 ...
 */
static void _eit_scrape_episode(lang_str_t* str, eit_module_t* eit_mod, eit_event_t* ev) {
  lang_str_ele_t* se;
  char            buffer[2048];
 ...
 * @param ev - [out] modified event data.
 */
static void _eit_scrape_text(eit_module_t* eit_mod, eit_event_t* ev) {
  lang_str_ele_t* se;
  char            buffer[2048];
  ...
  if (ev->title && eit_mod->scrape_title) {
    ...
    RB_FOREACH (se, ev->title, link) {
      snprintf(title_summary,
          sizeof(title_summary),
          "%s %% %s",
          se->str,
          lang_str_get(ev->summary, se->lang));
      if (eit_pattern_apply_list(buffer,
              sizeof(buffer),
              title_summary,
              se->lang,
              &eit_mod->p_scrape_title)) {
        tvhtrace(LS_TBL_EIT,
            "  scrape title '%s' from '%s' using %s",
            buffer,
            title_summary,
            eit_mod->id);
  ...
  if (eit_mod->scrape_subtitle) {
    RB_FOREACH (se, ev->summary, link) {
      if (eit_pattern_apply_list(buffer,
              ...
              se->str,
              se->lang,
              &eit_mod->p_scrape_subtitle)) {
        tvhtrace(LS_TBL_EIT,
            "  scrape subtitle '%s' from '%s' using %s",
            buffer,
            se->str,
            eit_mod->id);
  ...
  if (eit_mod->scrape_summary) {
    ...
    RB_FOREACH (se, ev->summary, link) {
      if (eit_pattern_apply_list(buffer,
              ...
              se->str,
              se->lang,
              &eit_mod->p_scrape_summary)) {
        tvhtrace(LS_TBL_EIT,
            "  scrape summary '%s' from '%s' using %s",
            buffer,
            se->str,
            eit_mod->id);
        lang_str_set(&ls, buffer, se->lang);
...
static int _eit_process_event_one(epggrab_module_t* mod,
    int                                             tableid,
    int                                             sect,
  ...
  stop  = start + bcdtoint(ptr[7] & 0xff) * 3600 + bcdtoint(ptr[8] & 0xff) * 60 +
      bcdtoint(ptr[9] & 0xff);
  ...
  tvhtrace(LS_TBL_EIT,
      "svc='%s', ch='%s', eid=%5d, tbl=%02x, running=%d, start=%s,"
      " stop=%s, ebc=%p",
      ...
      gmtime2local(stop, tm2, sizeof(tm2)),
      ebc);
  if (!ebc) {
    if (tableid == 0x4e)
      rsonly = 1;
  ...
  *save |= epg_broadcast_set_dvb_eid(ebc, eid, &changes);
  ...
  /* Summary/Description */
  if (ev->summary)
    if (short_target != 0 || (ev->subtitle && lang_str_compare(ev->summary, ev->subtitle)))
...
running:
  ...
  if (ebc && running && tableid == 0x4e) {
    if (sect == 0) {
      switch (running) {
        case 2:
          run = EPG_RUNNING_WARM;
          break;
        case 3:
          run = EPG_RUNNING_PAUSE;
          break;
        case 4:
          run = EPG_RUNNING_NOW;
          break;
        default:
          run = EPG_RUNNING_STOP;
          break;
      }
...
static int _eit_process_event(epggrab_module_t* mod,
    eit_data_t*                                 ed,
    const uint8_t*                              ptr0,
    int                                         len0,
    int*                                        save,
        ...
        if (epggrab_conf.epgdb_processparentallabels) {
          if (ev.rating_label) {
            tvhtrace(mod->subsys,
                "RATINGLABEL '%d'  '%s'",
                ev.parental,
                ev.rating_label->rl_display_label);
          } else {
  ...
  if (svc && eit_mod->opaque) {
    LIST_FOREACH (ilm, &svc->s_channels, ilm_in1_link) {
      ...
      if (!ch->ch_enabled || ch->ch_epg_parent)
        continue;
      if (_eit_process_event_one(mod, tableid, sect, svc, ch, &ev, ptr0, len0, local, save) < 0)
        break;
    }
  }
  if (lock)
...
#if TODO_ADD_EXTRA
  if (ev.extra)
    htsmsg_destroy(ev.extra);
#endif
  if (ev.genre)
    epg_genre_list_destroy(ev.genre);
  if (ev.title)
    lang_str_destroy(ev.title);
  if (ev.subtitle)
    lang_str_destroy(ev.subtitle);
  if (ev.summary)
    lang_str_destroy(ev.summary);
  if (ev.desc)
    lang_str_destroy(ev.desc);
...
static void _eit_process_data(void* m, void* data, uint32_t len) {
  int         save = 0, r;
  size_t      hlen;
  eit_data_t* ed = data;
...
static void _eit_process_immediate(void* m, const void* ptr, uint32_t len, eit_data_t* ed) {
  int save = 0, r;
...
static int _eit_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  int                       r, sect, last, ver;
  uint8_t                   seg;
  ...
  tvhtrace(LS_TBL_EIT,
      "%s: sid %i tsid %04x onid %04x seg %02x",
      mt->mt_name,
      sid,
      tsid,
      onid,
  ...
  if (tableid == 0x4f || tableid >= 0x60) {
    ...
    if ((mm->mm_tsid != tsid || mm->mm_onid != onid) && !mm->mm_eit_tsid_nocheck) {
      if (mm->mm_onid != MPEGTS_ONID_NONE && mm->mm_tsid != MPEGTS_TSID_NONE)
        tvhtrace(LS_TBL_EIT,
...
static int _eit_start(epggrab_ota_map_t* map, mpegts_mux_t* dm) {
  return 0;
}

static void _eit_install_one_handler(mpegts_mux_t* dm, epggrab_ota_map_t* map) {
...
static void _eit_install_handlers(epggrab_ota_map_t* _map, mpegts_mux_t* dm) {
...
static int _eit_activate(void* m, int e) {
  eit_module_t* mod = m;
  tvhtrace(LS_TBL_EIT,
      "_eit_activate %s change to %d from %d with scrape_episode of %d and scrape_subtitle of %d",
      mod->id,
      e,
      mod->active,
      mod->scrape_episode,
...
static int _eit_tune(epggrab_ota_map_t* map, epggrab_ota_mux_t* om, mpegts_mux_t* mm) {
  int                     r = 0;
  mpegts_service_t*       s;
...
static void eit_queue_priv(mpegts_mux_t* dm, const char* src, eit_private_t* priv) {
  epggrab_ota_mux_t*           om;
...
void eit_nit_callback(mpegts_table_t* mt, uint16_t nbid, const char* name, uint32_t nitpriv) {
  ...
  tvhtrace(LS_TBL_EIT,
      "NIT - tsid %04X (%d) onid %04X (%d) nbid %04X (%d) network name '%s' private %08X",
      dm->mm_tsid,
      dm->mm_tsid,
      dm->mm_onid,
      dm->mm_onid,
...
void eit_sdt_callback(mpegts_table_t* mt, uint32_t sdtpriv) {
  ...
  tvhtrace(LS_TBL_EIT,
      "SDT - tsid %04X (%d) onid %04X (%d) private %08X",
      dm->mm_tsid,
      dm->mm_tsid,
      dm->mm_onid,
      dm->mm_onid,
...
static void _eit_module_load_config(eit_module_t* mod) {
  ...
  const char* config_file =
      mod->scrape_config && *mod->scrape_config ? mod->scrape_config : mod->id;
...
static htsmsg_t* epggrab_mod_eit_class_short_list(void* o, const char* lang) {
  static const struct strtab tab[] = {{N_("Subtitle"), 0},
      {N_("Summary"), 1},
      {N_("Subtitle and summary"), 2}};
  return strtab2htsmsg(tab, 1, lang);
}

static const idclass_t epggrab_mod_eit_class = {.ic_super = &epggrab_mod_ota_scraper_class,
    ...
    .ic_properties                                        = (const property_t[]){
        {
                                                   ...
                                                   .desc =
                N_("Set the short EIT destription to given target (subtitle, summary or both)."),
...
static eit_module_t* eit_module_ota_create(const char* id,
    int                                                subsys,
    const char*                                        saveid,
    const char*                                        name,
    int                                                priority,
    epggrab_ota_module_ops_t*                          ops) {
  eit_module_t* mod = (eit_module_t*)epggrab_module_ota_create(calloc(1, sizeof(eit_module_t)),
      id,
      subsys,
      saveid,
      name,
      priority,
      &epggrab_mod_eit_class,
      ops);
  return mod;
}

static void
eit_parse_list(htsmsg_t* conf, const char* fname, uint16_t* list, int list_len, int* count) {
...
static void eit_init_one(const char* id, htsmsg_t* conf) {
  ...
  if (map) {
    HTSMSG_FOREACH(f, map) {
      if (strcmp(htsmsg_field_name(f), "slave") == 0) {
        ...
        if (s)
          strncpy(priv->slave, s, sizeof(priv->slave) - 1);
  ...
  if (name_str) {
    priv->module = (epggrab_module_ota_t*)
        eit_module_ota_create(id, LS_TBL_EIT, NULL, lang_str_get(name_str, NULL), prio, ops);
