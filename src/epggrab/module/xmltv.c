...
static const char*
xmltv_ns_get_parse_num(const char* s, uint16_t* ap, uint16_t* bp) {
  int a = -1, b = -1;
...
static void parse_xmltv_ns_episode(const char* s, epg_episode_num_t* epnum) {
...
static void parse_xmltv_dd_progid(epggrab_module_t* mod,
    const char*                                     s,
    char**                                          uri,
    char**                                          suri,
 ...
 *
 */
static void get_episode_info(epggrab_module_t* mod,
    htsmsg_t*                                  tags,
    char**                                     uri,
    char**                                     suri,
    epg_episode_num_t*                         epnum) {
 ...
 *       job
 */
static int xmltv_parse_vid_quality(epg_broadcast_t* ebc,
    htsmsg_t*                                       m,
    int8_t*                                         bw,
 ...
 */
int xmltv_parse_accessibility(epg_broadcast_t* ebc,
    htsmsg_t*                                  m,
    epg_changes_t*                             changes) {
  int             save = 0;
  htsmsg_t*       tag;
 ...
 * Previously shown
 */
static int _xmltv_parse_previously_shown(epg_broadcast_t* ebc,
    time_t*                                               first_aired,
    htsmsg_t*                                             tag,
    epg_changes_t*                                        changes) {
  int         ret;
 ...
 */
static int _xmltv_parse_date_finished(epg_broadcast_t* ebc,
    htsmsg_t*                                          tag,
    epg_changes_t*                                     changes) {
  if (!ebc || !tag)
    return 0;
 ...
 *   </star-rating>
 */
static int _xmltv_parse_star_rating(epg_broadcast_t* ebc,
    htsmsg_t*                                        body,
    epg_changes_t*                                   changes) {
  double      a, b;
  htsmsg_t *  stars, *tags;
  ...
  if (!ebc || !body)
    return 0;
  if (!(stars = htsmsg_get_map(body, "star-rating")))
    return 0;
  if (!(tags = htsmsg_get_map(stars, "tags")))
    return 0;
 ...
 */
static int _xmltv_parse_age_rating(epg_broadcast_t* ebc,
    htsmsg_t*                                       body,
    epg_changes_t*                                  changes) {
  uint8_t     age = 0;
  ...
  const char* rating_system; // System attribute from the 'rating' tag : <rating
                             // system="ACMA">
  ...
  if (epggrab_conf.epgdb_processparentallabels) {
    HTSMSG_FOREACH(f, body) {
      if (!strcmp(htsmsg_field_name(f), "rating") &&
          (rating = htsmsg_get_map_by_field(f))) {
        ...
        if ((tags = htsmsg_get_map(rating, "tags"))) {
          ...
          if ((s1 = htsmsg_xml_get_cdata_str(tags, "value"))) {
            // tvherror(LS_RATINGLABELS, "XMLTV got system: '%s' / '%s'",
            // rating_system, s1);

            rl = ratinglabel_find_from_xmltv(rating_system, s1);

            if (rl) {
              tvhtrace(LS_RATINGLABELS,
                  "Found label: '%s' / '%s' / '%s' / '%d'",
                  rl->rl_authority,
                  rl->rl_label,
                  rl->rl_country,
              ...
              if (rl->rl_display_age >= 0 && rl->rl_display_age < 22) {
                return epg_broadcast_set_age_rating(ebc,
                    rl->rl_display_age,
                    changes);
              } // END age sanity test
 ...
 * Parse category list
 */
static epg_genre_list_t* _xmltv_parse_categories(htsmsg_t* tags) {
  htsmsg_t*         e;
  htsmsg_field_t*   f;
  epg_genre_list_t* egl = NULL;
  HTSMSG_FOREACH(f, tags) {
    if (!strcmp(htsmsg_field_name(f), "category") &&
...
_xmltv_parse_lang_str(lang_str_t** ls, htsmsg_t* tags, const char* tname) {
  ...
  HTSMSG_FOREACH(f, tags) {
    if (!strcmp(htsmsg_field_name(f), tname) &&
        (e = htsmsg_get_map_by_field(f))) {
      if (!*ls)
        *ls = lang_str_create();
      lang = NULL;
...
/// that have tagname.
__attribute__((warn_unused_result)) static string_list_t*
_xmltv_make_str_list_from_matching(htsmsg_t* tags, const char* tagname) {
  ...
  HTSMSG_FOREACH(f, tags) {
    if (!strcmp(htsmsg_field_name(f), tagname) &&
        (e = htsmsg_get_map_by_field(f))) {
      const char* str = htsmsg_get_str(e, "cdata");
      if (str && *str) {
        if (!tag_list)
          tag_list = string_list_create();
...
_xmltv_parse_credits(htsmsg_t** out_credits, htsmsg_t* tags) {
  ...
  HTSMSG_FOREACH(f, credits_tags) {
    ...
    if ((!strcmp(fname, "actor") || !strcmp(fname, "director") ||
            !strcmp(fname, "guest") || !strcmp(fname, "presenter") ||
            !strcmp(fname, "writer")) &&
        (e = htsmsg_get_map_by_field(f))) {
      const char* str = htsmsg_get_str(e, "cdata");
      ...
      if (strstr(str, "|") == 0) {
        ...
        if (!credits_names)
          credits_names = string_list_create();
        string_list_insert(credits_names, str);

        if (!*out_credits)
          *out_credits = htsmsg_create_map();
 ...
 */
static void xmltv_appendit(lang_str_t** _desc,
    string_list_t*                      list,
    const char*                         name,
    lang_str_t*                         summary) {
 ...
 */
static int _xmltv_parse_programme_tags(epggrab_module_t* mod,
    channel_t*                                           ch,
    htsmsg_t*                                            tags,
    time_t                                               start,
    time_t                                               stop,
  ...
  const int scrape_onto_desc =
      ((epggrab_module_int_t*)mod)->xmltv_scrape_onto_desc;
  const int use_category_not_genre =
      ((epggrab_module_int_t*)mod)->xmltv_use_category_not_genre;
  int               save    = 0;
  ...
  if (scrape_extra || scrape_onto_desc) {
    ...
    string_list_t* keyword =
        _xmltv_make_str_list_from_matching(tags, "keyword");
    ...
    if (credits)
      htsmsg_destroy(credits);
    if (credits_names)
      string_list_destroy(credits_names);
    if (category)
      string_list_destroy(category);
    if (keyword)
      string_list_destroy(keyword);
...
#undef APPENDIT
  ...
  /* Quality metadata */
  save |= xmltv_parse_vid_quality(ebc,
      htsmsg_get_map(tags, "video"),
      &bw,
      &changes);
  ...
  /* Misc */
  save |= _xmltv_parse_previously_shown(ebc,
      &first_aired,
  ...
  if (subtitle)
    lang_str_destroy(subtitle);
  if (desc)
    lang_str_destroy(desc);
  if (summary)
 ...
 */
static int _xmltv_parse_programme(epggrab_module_t* mod,
    htsmsg_t*                                       body,
    epggrab_stats_t*                                stats) {
  int                    chsave = 0, save = 0;
  ...
  LIST_FOREACH (ilm, &ec->channels, ilm_in1_link) {
    ...
    if (!ch->ch_enabled || ch->ch_epg_parent)
      continue;
    save |=
        _xmltv_parse_programme_tags(mod, ch, tags, start, stop, icon, stats);
  }
  return save;
}
 ...
 */
static int _xmltv_parse_channel(epggrab_module_t* mod,
    htsmsg_t*                                     body,
    epggrab_stats_t*                              stats) {
  ...
  if ((tags = htsmsg_get_map(body, "tags")) == NULL)
    return 0;
  if ((ch = epggrab_channel_find(mod, id, 1, &save)) == NULL)
    return 0;
  ch->laststamp = gclk();
  ...
  HTSMSG_FOREACH(f, tags) {
    if (!(subtag = htsmsg_field_get_map(f)))
      continue;
 ...
 */
static int
_xmltv_parse_tv(epggrab_module_t* mod, htsmsg_t* body, epggrab_stats_t* stats) {
  int             gsave = 0, save;
  htsmsg_t*       tags;
  htsmsg_field_t* f;
...
static int _xmltv_parse(void* mod, htsmsg_t* data, epggrab_stats_t* stats) {
  htsmsg_t *tags, *tv;
...
#define SCRAPE_EXTRA_DESC                                                     \
  N_("Obtain list of credits (actors, etc.), keywords and extra information " \
     "from the xml tags (if available). "                                     \
     "Some xmltv providers supply a list of actors and additional keywords "  \
     "to "                                                                    \
     "describe programmes. This option will retrieve this additional "        \
     "information. "                                                          \
...
#define SCRAPE_ONTO_DESC_NAME \
  N_("Alter programme description to include detailed information")
#define SCRAPE_ONTO_DESC_DESC                                                \
  N_("If enabled then this will alter the programme descriptions to "        \
     "include information about actors, keywords and categories (if "        \
     "available from the xmltv file). "                                      \
     "This is useful for legacy clients that can not parse newer Tvheadend " \
     "messages "                                                             \
     "containing this information or do not display the information. "       \
     "For example the modified description might include 'Starring: Lorem "  \
     "Ipsum'. "                                                              \
     "The description is altered for all clients, both legacy, modern, and " \
     "GUI. "                                                                 \
     "Enabling scraping of detailed information can use significant "        \
     "resources (memory and CPU). "                                          \
     "You should not enable this if you use 'duplicate detect if different " \
     "description' "                                                         \
...
static htsmsg_t* xmltv_dn_chnum_list(void* o, const char* lang) {
  ...
  if (outlen > 0) {
    ...
    while (i < outlen) {
      if (outbuf[i] == '\n' || outbuf[i] == '\0') {
        ...
        epggrab_module_int_create(NULL,
            &epggrab_mod_int_xmltv_class,
            &outbuf[p],
            LS_XMLTV,
            "xmltv",
            name,
    ...
    char  bin[PATH_MAX];
    char* argv[] = {NULL, (char*)"--description", NULL};
    path         = strdup(tmp);
    ...
    while (tmp) {
      ...
      if ((dir = opendir(tmp))) {
        while ((de = readdir(dir))) {
          if (strstr(de->d_name, XMLTV_GRAB) != de->d_name)
            continue;
...
void xmltv_init(void) {
  /* External module */
  epggrab_module_ext_create(NULL,
      &epggrab_mod_ext_xmltv_class,
      "xmltv",
      LS_XMLTV,
      "xmltv",
      "XMLTV",
