...
static epggrab_channel_t* _opentv_find_epggrab_channel(opentv_module_t* mod,
    int                                                                 cid,
    int                                                                 create,
    int*                                                                save) {
  char chid[256];
...
/* Find event entry */
static opentv_entry_t*
opentv_find_entry(opentv_status_t* sta, uint16_t eid, int cid) {
...
static char*
_opentv_parse_string(opentv_module_t* prov, const uint8_t* buf, int len) {
  int   ok = 0;
  char *ret, *tmp;
        ...
        if (rlen >= 7) {
          ...
          if (!ev->title) {
            ...
            if (!strcmp(prov->dict->id, "skynz")) {
              if ((strlen(ev->title) >= 6) && (ev->title[0] == '[') &&
                  (ev->title[1] == '[') && (ev->title[4] == ']') &&
                  (ev->title[5] == ']'))
...
static int opentv_parse_event_section_one(opentv_module_t* mod,
  ...
  while (i < len) {
    ...
    /* Find broadcast */
    if (ev.start && ev.stop) {
      ebc = epg_broadcast_find_by_time(ch,
          src,
          ev.start,
          ev.stop,
          1,
...
static int opentv_parse_event_section(opentv_module_t* mod,
    int                                                cid,
    int                                                mjd,
    const uint8_t*                                     buf,
    int                                                len) {
      ...
      if (mod->bouquetid != nbid) {
        if (mod->bouquet_auto) {
          if (nbid < mod->bouquetid) {
            tvhwarn(LS_OPENTV,
                "%s: bouquet id set to %d, report this!",
                mt->mt_name,
                nbid);
            mod->bouquetid = nbid;
          } else
...
static int opentv_table_callback(mpegts_table_t* mt,
    const uint8_t*                               buf,
    int                                          len,
    int                                          tableid) {
  int                       r = 1;
...
static int opentv_bat_callback(mpegts_table_t* mt,
    const uint8_t*                             buf,
    int                                        len,
    int                                        tableid) {
  int *              t, r;
...
static int
_opentv_tune(epggrab_ota_map_t* map, epggrab_ota_mux_t* om, mpegts_mux_t* mm) {
  opentv_module_t* mod = (opentv_module_t*)map->om_module;
...
static int _opentv_prov_load_one(const char* id, htsmsg_t* m) {
  ...
  mod = (opentv_module_t*)epggrab_module_ota_create(
      calloc(1, sizeof(opentv_module_t)),
      ibuf,
      LS_OPENTV,
      NULL,
      nbuf,
