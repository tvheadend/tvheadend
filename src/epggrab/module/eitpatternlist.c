...
void eit_pattern_compile_list(eit_pattern_list_t* list,
    htsmsg_t*                                     l,
    int                                           flags) {
...
void eit_pattern_compile_named_list(eit_pattern_list_t* list,
    htsmsg_t*                                           m,
    const char*                                         key) {
  ...
  TAILQ_FOREACH (p, l, p_links) {
    ...
    if (!regex_match(&p->compiled, text) &&
      ...
      for (matchno = 2;; ++matchno) {
        if (regex_match_substring(&p->compiled,
                matchno,
                matchbuf,
                sizeof(matchbuf)))
          break;
