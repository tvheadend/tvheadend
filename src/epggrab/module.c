...
static void epggrab_mod_class_title(idnode_t* self,
    const char*                               lang,
    char*                                     dst,
    size_t                                    dstsize) {
...
static void epggrab_mod_class_changed(idnode_t* self) {
  epggrab_module_t* mod = (epggrab_module_t*)self;
...
static htsmsg_t* epggrab_module_ota_scrapper_config_list(void* o,
    const char*                                                lang) {
  ...
  htsmsg_add_str(e,
      "val",
      tvh_gettext_lang(lang, N_("Use default configuration")));
...
const idclass_t epggrab_mod_class = {.ic_class = "epggrab_mod",
...
const idclass_t epggrab_mod_int_class = {.ic_super = &epggrab_mod_class,
...
const idclass_t epggrab_mod_ext_class = {.ic_super = &epggrab_mod_class,
...
const idclass_t epggrab_mod_ota_class = {.ic_super = &epggrab_mod_class,
    ...
    .ic_properties = (const property_t[]){
        {
            ...
            .desc =
                N_("Configuration containing regular expressions to use for "
        ...
        },
        {
            ...
            .desc = N_(
                "Enable/disable scraping episode details using the grabber."),
        ...
        },
        {
            ...
            .desc = N_("Enable/disable scraping title from the programme title "
                       "and description. "
                       "Some broadcasters can split the title over the "
                       "separate title, "
                       "and summary fields. This allows scraping of common "
                       "split title formats "
                       "from within the broadcast title and summary field if "
                       "supported by the "
                       "configuration file."),
        ...
        },
        {
            ...
            .desc = N_(
                "Enable/disable scraping subtitle from the programme "
                "description. "
                "Some broadcasters do not send separate title, subtitle, "
                "description, "
                "and summary fields. This allows scraping of common subtitle "
                "formats "
        ...
        },
        {
            ...
            .desc = N_(
                "Enable/disable scraping summary from the programme "
                "description. "
                "Some broadcasters do not send separate title, subtitle, "
                "description, "
                "and summary fields. This allows scraping of a modified "
                "summary "
                "from within the broadcast summary field if supported by the "
                "configuration file."),
...
epggrab_module_t* epggrab_module_create(epggrab_module_t* skel,
    const idclass_t*                                      cls,
    const char*                                           id,
    int                                                   subsys,
    const char*                                           saveid,
...
void epggrab_module_parse(void* m, htsmsg_t* data) {
  ...
  /* Debug stats */
  tvhinfo(mod->subsys,
      "%s: parse took %" PRId64 " seconds",
      mod->id,
      mono2sec(tm2 - tm1));
  tvhinfo(mod->subsys,
      "%s:  channels   tot=%5d new=%5d mod=%5d",
      mod->id,
      stats.channels.total,
      stats.channels.created,
      stats.channels.modified);
  tvhinfo(mod->subsys,
      "%s:  brands     tot=%5d new=%5d mod=%5d",
      mod->id,
      stats.brands.total,
      stats.brands.created,
      stats.brands.modified);
  tvhinfo(mod->subsys,
      "%s:  seasons    tot=%5d new=%5d mod=%5d",
      mod->id,
      stats.seasons.total,
      stats.seasons.created,
      stats.seasons.modified);
  tvhinfo(mod->subsys,
      "%s:  episodes   tot=%5d new=%5d mod=%5d",
      mod->id,
      stats.episodes.total,
      stats.episodes.created,
      stats.episodes.modified);
  tvhinfo(mod->subsys,
      "%s:  broadcasts tot=%5d new=%5d mod=%5d",
      mod->id,
  ...
  if (save && epggrab_conf.epgdb_saveafterimport) {
     ...
     */
    gtimer_arm_rel(&epggrab_save_timer, epg_save_callback, NULL, 60 * 2);
...
static void epggrab_module_int_done(void* m) {
  epggrab_module_int_t* mod = m;
  mod->active               = 0;
  free((char*)mod->path);
  mod->path = NULL;
...
epggrab_module_int_t* epggrab_module_int_create(epggrab_module_int_t* skel,
    const idclass_t*                                                  cls,
    const char*                                                       id,
    int                                                               subsys,
    const char*                                                       saveid,
  ...
  /* Allocate data */
  if (!skel)
    skel = calloc(1, sizeof(epggrab_module_int_t));
...
htsmsg_t* epggrab_module_trans_xml(void* m, char* c) {
  ...
  if (!ret)
    tvherror(mod->subsys,
        "%s: htsmsg_xml_deserialize error %s",
        mod->id,
        errbuf);
  return ret;
}
...
static void _epggrab_socket_handler(epggrab_module_ext_t* mod, int s) {
  ...
  if (outlen)
    data = mod->trans(mod, outbuf);
  time(&tm2);

  /* Process */
  if (data) {
    tvhinfo(mod->subsys,
        "%s: grab took %" PRItime_t " seconds",
        mod->id,
        tm2 - tm1);
 ...
 */
static void epggrab_module_done_socket(void* m) {
 ...
 */
static int epggrab_module_activate_socket(void* m, int a) {
  ...
  if (!a) {
    ...
    tvh_thread_create(&mod->tid,
        &tattr,
        _epggrab_socket_thread,
        mod,
        "epggrabso");
  }
  return 1;
 ...
 * Create a module
 */
epggrab_module_ext_t* epggrab_module_ext_create(epggrab_module_ext_t* skel,
    const idclass_t*                                                  cls,
    const char*                                                       id,
    int                                                               subsys,
    const char*                                                       saveid,
  ...
  /* Allocate data */
  if (!skel)
    skel = calloc(1, sizeof(epggrab_module_ext_t));
...
epggrab_module_ota_t* epggrab_module_ota_create(epggrab_module_ota_t* skel,
    const char*                                                       id,
    int                                                               subsys,
    const char*                                                       saveid,
    const char*                                                       name,
  ...
  if (!skel)
    skel = calloc(1, sizeof(epggrab_module_ota_t));
