...
/* Delete ilm */
static void _epgggrab_channel_link_delete(idnode_list_mapping_t* ilm, int delconf) {
...
int epggrab_channel_link(epggrab_channel_t* ec, channel_t* ch, void* origin) {
  ...
  ilm = idnode_list_link(&ec->idnode, &ec->channels, &ch->ch_id, &ch->ch_epggrab, origin, 1);
  if (ilm == NULL)
    return 0;
...
/* Create new entry */
epggrab_channel_t*
epggrab_channel_create(epggrab_module_t* owner, htsmsg_t* conf, const char* uuid) {
...
epggrab_channel_t*
epggrab_channel_find(epggrab_module_t* mod, const char* id, int create, int* save) {
  char*              s;
  epggrab_channel_t* ec;
...
static void
epggrab_channel_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
  epggrab_channel_t* ec = (epggrab_channel_t*)self;

  snprintf(dst, dstsize, "%s: %s (%s)", ec->name ?: ec->id, ec->id, ec->mod->name);
}

static htsmsg_t* epggrab_channel_class_save(idnode_t* self, char* filename, size_t fsize) {
  epggrab_channel_t* ec = (epggrab_channel_t*)self;
...
static htsmsg_t* epggrab_channel_class_update_enum(void* obj, const char* lang) {
