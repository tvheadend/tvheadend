...
static inline int is_paired(epggrab_channel_t* ec) {
  return ec->only_one && LIST_FIRST(&ec->channels);
}

static inline int epggrab_channel_check(epggrab_channel_t* ec, channel_t* ch) {
...
int epggrab_channel_match_number(epggrab_channel_t* ec, channel_t* ch) {
  ...
  if (ec->lcn && ec->lcn == channel_get_number(ch))
    return 1;
  return 0;
}

/* Delete ilm */
static void _epgggrab_channel_link_delete(idnode_list_mapping_t* ilm,
    int                                                          delconf) {
  epggrab_channel_t* ec = (epggrab_channel_t*)ilm->ilm_in1;
  channel_t*         ch = (channel_t*)ilm->ilm_in2;
  tvhdebug(ec->mod->subsys,
      "%s: unlinking %s from %s",
      ec->mod->id,
      ec->id,
...
/* Destroy */
void epggrab_channel_link_delete(epggrab_channel_t* ec,
    channel_t*                                      ch,
    int                                             delconf) {
...
/* Do update */
static void epggrab_channel_sync(epggrab_channel_t* ec, channel_t* ch) {
  int save = 0;
  ...
  if (ec->update_chnum && ec->lcn > 0 && epggrab_conf.channel_renumber)
    save |= channel_set_number(ch,
        ec->lcn / CHANNEL_SPLIT,
        ec->lcn % CHANNEL_SPLIT);
...
int epggrab_channel_link(epggrab_channel_t* ec, channel_t* ch, void* origin) {
  ...
  ilm = idnode_list_link(&ec->idnode,
      &ec->channels,
      &ch->ch_id,
      &ch->ch_epggrab,
      origin,
...
int epggrab_channel_set_number(epggrab_channel_t* ec, int major, int minor) {
  ...
  if (ec->lcn != lcn) {
    ...
    if (epggrab_conf.channel_renumber) {
      LIST_FOREACH (ilm, &ec->channels, ilm_in1_link) {
        ch = (channel_t*)ilm->ilm_in2;
        if (channel_set_number(ch, lcn / CHANNEL_SPLIT, lcn % CHANNEL_SPLIT))
          idnode_changed(&ch->ch_id);
      }
    }
    save = 1;
...
static int epggrab_channel_autolink_one(epggrab_channel_t* ec, channel_t* ch) {
...
/* Autolink EPG channel to channel */
static int epggrab_channel_autolink(epggrab_channel_t* ec) {
  channel_t* ch;
...
static int _ch_id_cmp(void* a, void* b) {
  return strcmp(((epggrab_channel_t*)a)->id, ((epggrab_channel_t*)b)->id);
}

/* Create new entry */
epggrab_channel_t* epggrab_channel_create(epggrab_module_t* owner,
    htsmsg_t*                                               conf,
    const char*                                             uuid) {
  ...
  if (RB_INSERT_SORTED(&owner->channels, ec, link, _ch_id_cmp)) {
    tvherror(LS_EPGGRAB,
        "removing duplicate channel id '%s' (uuid '%s')",
        ec->id,
        uuid);
...
epggrab_channel_t* epggrab_channel_find(epggrab_module_t* mod,
    const char*                                           id,
    int                                                   create,
    int*                                                  save) {
  char*              s;
  ...
  if (!create) {
    ...
    ec = RB_INSERT_SORTED(&mod->channels,
        epggrab_channel_skel,
        link,
        _ch_id_cmp);
    if (!ec) {
...
void epggrab_channel_destroy(epggrab_channel_t* ec,
    int                                         delconf,
    int                                         rb_remove) {
  ...
  if (delconf)
    hts_settings_remove("epggrab/%s/channels/%s",
        ec->mod->saveid,
        idnode_uuid_as_str(&ec->idnode, ubuf));
...
void epggrab_channel_flush(epggrab_module_t* mod, int delconf) {
  epggrab_channel_t* ec;
...
const char* epggrab_channel_get_id(epggrab_channel_t* ec) {
...
static void epggrab_channel_class_get_title(idnode_t* self,
    const char*                                       lang,
    char*                                             dst,
    size_t                                            dstsize) {
...
static void epggrab_channel_class_delete(idnode_t* self) {
  epggrab_channel_destroy((epggrab_channel_t*)self, 1, 1);
}

static const void* epggrab_channel_class_modid_get(void* obj) {
  epggrab_channel_t* ec = obj;
  snprintf(prop_sbuf, PROP_SBUF_LEN, "%s", ec->mod->id ?: "");
  return &prop_sbuf_ptr;
}

static int epggrab_channel_class_modid_set(void* obj, const void* p) {
  return 0;
}

static const void* epggrab_channel_class_module_get(void* obj) {
  epggrab_channel_t* ec = obj;
  snprintf(prop_sbuf, PROP_SBUF_LEN, "%s", ec->mod->name ?: "");
  return &prop_sbuf_ptr;
}

static const void* epggrab_channel_class_path_get(void* obj) {
  epggrab_channel_t* ec = obj;
  if (ec->mod->type == EPGGRAB_INT || ec->mod->type == EPGGRAB_EXT)
    snprintf(prop_sbuf,
        PROP_SBUF_LEN,
        "%s",
        ((epggrab_module_int_t*)ec->mod)->path ?: "");
  else
    prop_sbuf[0] = '\0';
  return &prop_sbuf_ptr;
}

static const void* epggrab_channel_class_names_get(void* obj) {
  epggrab_channel_t* ec = obj;
...
static int epggrab_channel_class_names_set(void* obj, const void* p) {
...
static void epggrab_channel_class_enabled_notify(void* obj, const char* lang) {
  epggrab_channel_t* ec = obj;
  if (!ec->enabled) {
    epggrab_channel_links_delete(ec, 1);
  } else {
...
static const void* epggrab_channel_class_channels_get(void* obj) {
  epggrab_channel_t* ec = obj;
  return idnode_list_get1(&ec->channels);
}

static int epggrab_channel_class_channels_set(void* obj, const void* p) {
  epggrab_channel_t* ec = obj;
  return idnode_list_set1(&ec->idnode,
      &ec->channels,
      &channel_class,
      (htsmsg_t*)p,
      epggrab_channel_map);
...
static char* epggrab_channel_class_channels_rend(void* obj, const char* lang) {
  epggrab_channel_t* ec = obj;
...
static htsmsg_t* epggrab_channel_class_update_enum(void* obj,
    const char*                                          lang) {
  return idnode_slist_enum(obj, epggrab_channel_class_update_slist, lang);
}

static const void* epggrab_channel_class_update_get(void* obj) {
  return idnode_slist_get(obj, epggrab_channel_class_update_slist);
}

static char* epggrab_channel_class_update_rend(void* obj, const char* lang) {
  return idnode_slist_rend(obj, epggrab_channel_class_update_slist, lang);
}

static int epggrab_channel_class_update_set(void* obj, const void* p) {
  return idnode_slist_set(obj, epggrab_channel_class_update_slist, p);
}

static void epggrab_channel_class_update_notify(void* obj, const char* lang) {
  epggrab_channel_t*     ec = obj;
  channel_t*             ch;
  idnode_list_mapping_t* ilm;
...
static void epggrab_channel_class_only_one_notify(void* obj, const char* lang) {
  epggrab_channel_t*     ec = obj;
...
const idclass_t epggrab_channel_class = {.ic_class = "epggrab_channel",
