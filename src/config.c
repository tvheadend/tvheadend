 ...
 */
static const char* config_migrate_v1_chn_id_to_uuid(htsmsg_t* chns,
    uint32_t                                                  id) {
  htsmsg_field_t* f;
  htsmsg_t*       e;
  uint32_t        u32;
  HTSMSG_FOREACH(f, chns) {
    if (!(e = htsmsg_field_get_map(f)))
      continue;
    if (htsmsg_get_u32(e, "channelid", &u32))
      continue;
    if (u32 == id) {
 ...
 */
static const char* config_migrate_v1_chn_name_to_uuid(htsmsg_t* chns,
    const char*                                                 name) {
 ...
 */
static void config_migrate_v1_chn_add_svc(htsmsg_t* chns,
    const char*                                     chnname,
    const char*                                     svcuuid) {
 ...
 */
static void config_migrate_v1_dvb_svcs(const char* name,
    const char*                                    netu,
    const char*                                    muxu,
    htsmsg_t*                                      channels) {
  tvh_uuid_t      svcu;
  ...
  if ((c = hts_settings_load_r(1, "dvbtransports/%s", name))) {
    HTSMSG_FOREACH(f, c) {
      ...
      hts_settings_save(svc,
          "input/linuxdvb/networks/%s/muxes/%s/services/%s",
          netu,
          muxu,
          uuid_get_hex(&svcu, ubuf));
 ...
 */
static void config_migrate_v1_dvb_network(const char* name,
    htsmsg_t*                                         c,
    htsmsg_t*                                         channels) {
  int             i;
  ...
  const char *    str, *type;
  uint32_t        u32;
  const char*     mux_str_props[] = {"bandwidth",
      "consellation",
      "transmission_mode",
      "guard_interval",
      "hierarchy",
  ...
  if (!(tun = hts_settings_load("dvbadapters/%s", name)))
    return;
  ...
  HTSMSG_FOREACH(f, c) {
    ...
    if ((str = htsmsg_get_str(e, "delivery_system")) && strlen(str) > 4)
    ...
    /* Save */
    uuid_set(&muxu, NULL);
    hts_settings_save(mux,
        "input/linuxdvb/networks/%s/muxes/%s/config",
  ...
  hts_settings_save(net,
      "input/linuxdvb/networks/%s/config",
      uuid_get_hex(&netu, ubuf));
  htsmsg_destroy(tun);
  htsmsg_destroy(net);
}
 ...
 */
static void config_migrate_v1_dvr(const char* path, htsmsg_t* channels) {
  htsmsg_t *      c, *e, *m;
  htsmsg_field_t* f;
  const char*     str;
 ...
 */
static void config_migrate_v1_epggrab(const char* path, htsmsg_t* channels) {
 ...
 */
static void config_migrate_v1(void) {
  ...
  if ((c = hts_settings_load_r(1, "iptvservices"))) {
    ...
    HTSMSG_FOREACH(f, c) {
      if (!(e = htsmsg_field_get_map(f)))
        continue;
      if (!(str = htsmsg_get_str(e, "group")))
        continue;
      if (htsmsg_get_u32(e, "port", &u32))
        continue;
      ...
      hts_settings_save(m,
          "input/iptv/networks/%s/muxes/%s/config",
          ubufn,
          uuid_get_hex(&muxu, ubufm));
      htsmsg_destroy(m);
      ...
      hts_settings_save(m,
          "input/iptv/networks/%s/muxes/%s/services/%s",
          ubufn,
          ubufm,
          ubufs);
      htsmsg_destroy(m);
 ...
 */
static void config_migrate_v2(void) {
  htsmsg_t*  m;
  tvh_uuid_t u;
  char       ubuf[UUID_HEX_SIZE];
  ...
  if (hts_settings_exists("input/iptv/muxes")) {
    ...
    /* Move muxes */
    hts_settings_buildpath(src, sizeof(src), "input/iptv/muxes");
    hts_settings_buildpath(dst,
        sizeof(dst),
        "input/iptv/networks/%s/muxes",
        ubuf);
    rename(src, dst);
  }
}
 ...
 */
static void config_migrate_v3(void) {
  char src[1024], dst[1024];
 ...
 */
static void config_migrate_v5(void) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
  const char*     str;
 ...
 */
static void config_migrate_v6(void) {
  htsmsg_t *      c, *m;
  htsmsg_field_t* f;
  const char*     str;
  uint32_t        interval;
    ...
    if (xchs) {
      HTSMSG_FOREACH(f, xchs) {
        if ((xc = htsmsg_get_map_by_field(f))) {
          hts_settings_save(xc,
              "epggrab/xmltv/channels/%s",
              htsmsg_field_name(f));
        }
      }
      htsmsg_destroy(xchs);
 ...
 */
static void config_migrate_simple(const char* dir,
    htsmsg_t*                                 list,
...
static void config_migrate_v7(void) {
...
static void config_migrate_v8(void) {
  htsmsg_t *      ch, *e, *m;
  htsmsg_field_t* f;
...
static void config_modify_autorec(htsmsg_t* c,
    uint32_t                                id,
    const char*                             uuid,
    const void*                             aux) {
  uint32_t u32;
...
static void config_modify_dvr_log(htsmsg_t* c,
    uint32_t                                id,
    const char*                             uuid,
    const void*                             aux) {
...
static void config_migrate_v9(void) {
  ...
  if ((c = hts_settings_load("dvr")) != NULL) {
    ...
    HTSMSG_FOREACH(f, c) {
      if (!(e = htsmsg_field_get_map(f)))
        continue;
      if (strcmp(htsmsg_field_name(f), "config"))
        continue;
    ...
    HTSMSG_FOREACH(f, c) {
      if (!(e = htsmsg_field_get_map(f)))
        continue;
      if (strcmp(htsmsg_field_name(f), "config") == 0)
        continue;
      if (strncmp(htsmsg_field_name(f), "config", 6))
        continue;
      htsmsg_add_str(e, "name", htsmsg_field_name(f) + 6);
      uuid_set(&u, NULL);
      hts_settings_remove("dvr/%s", htsmsg_field_name(f));
...
static void config_migrate_move(const char* dir, const char* newdir) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
...
static void config_migrate_v10(void) {
...
static void config_modify_acl_dvallcfg(htsmsg_t* c, htsmsg_t* dvr_config) {
  uint32_t    a;
...
static void config_modify_acl_tag_only(htsmsg_t* c, htsmsg_t* channel_tag) {
  uint32_t    a;
...
static void config_modify_dvr_config_name(htsmsg_t* c, htsmsg_t* dvr_config) {
...
static void config_migrate_v11(void) {
  htsmsg_t*       dvr_config;
...
static void config_modify_caclient(htsmsg_t* c,
    uint32_t                                 id,
    const char*                              uuid,
    const void*                              aux) {
  uint32_t u;
...
static void config_migrate_v12(void) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;

  config_migrate_simple("cwc", NULL, config_modify_caclient, "caclient_cwc");
  config_migrate_simple("capmt",
      NULL,
      config_modify_caclient,
      "caclient_capmt");
...
static void config_migrate_v13(void) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
  int             i;
...
static const char* config_migrate_v14_codec(int i) {
  switch (i) {
    case 1:
      return "mpeg2video";
    case 2:
...
static void config_migrate_v14(void) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
  int             i;
...
static void config_migrate_v15(void) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
  int             i;
...
static int config_dvr_autorec_start_set(const char* s, int* tm) {
  int t;
...
static void config_modify_dvrauto(htsmsg_t* c) {
  int  tm = -1, tw = -1;
  char buf[16];

  if (config_dvr_autorec_start_set(htsmsg_get_str(c, "start"), &tm) > 0 &&
      tm >= 0) {
    tm -= 15;
    if (tm < 0)
      tm += 24 * 60;
...
static void config_migrate_v16(void) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
...
static void config_migrate_v17(void) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
  int             i, p;
...
static void config_migrate_v18(void) {
  htsmsg_t *      c, *e, *l, *m;
  htsmsg_field_t* f;
  const char*     filename;
...
static void config_migrate_v19(void) {
  htsmsg_t *      c, *e, *m;
  htsmsg_field_t* f;
...
static void config_migrate_v20_helper(htsmsg_t* e, const char* fname) {
  htsmsg_t*   l;
  const char* str;
  char*       p;
...
static void config_migrate_v20(void) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
 ...
 */
static void config_migrate_v21(void) {
  htsmsg_t *      c, *m, *e, *a;
  htsmsg_field_t* f;
  const char*     str;
  int64_t         s64;
 ...
 */
static void config_migrate_v22(void) {
  htsmsg_t* c;
  uint32_t  u32;
 ...
 */
static void config_migrate_v23_one(const char* modname) {
  htsmsg_t *      c, *m, *n;
  htsmsg_field_t* f;
  uint32_t        maj, min;
  int64_t         num;
...
static void config_migrate_v23(void) {
...
static void config_migrate_v24(void) {
  htsmsg_t *         c, *e;
  htsmsg_field_t*    f;
  static const char* streaming_list[] = {"streaming",
      "basic",
      "adv_streaming",
      "advanced",
      "htsp_streaming",
      "htsp",
      NULL};
  static const char* dvr_list[]       = {"dvr",
      "basic",
      "htsp_dvr",
      "htsp",
      "all_dvr",
 ...
 * Perform backup
 */
static void dobackup(const char* oldver) {
  char         outfile[PATH_MAX], cwd[PATH_MAX];
  const char*  argv[] = {"/usr/bin/tar",
      "cjf",
      outfile,
      "--exclude",
      "backup",
  ...
  tvhinfo(LS_CONFIG,
      "backup: migrating config from %s (running %s)",
      oldver,
      tvheadend_version);
  ...
  snprintf(outfile, sizeof(outfile), "%s/backup/%s.tar.bz2", root, oldver);
  ...
  if (code) {
    ...
    tvherror(LS_CONFIG,
        "please DON'T report this as an error, you may use --nobackup to skip");
    tvherror(LS_CONFIG,
        "... or run the above command in the printed directory");
    tvherror(LS_CONFIG,
        "... using the same user/group as for the Tvheadend executable");
 ...
 * Migration table
 */
static const config_migrate_t config_migrate_table[] = {config_migrate_v1,
    config_migrate_v2,
    config_migrate_v3,
 ...
 */
static int config_migrate(int backup) {
  uint32_t    v;
  const char* s;
 ...
 *
 */
static void config_check_one(const char* dir) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
...
static char* config_get_dir(uid_t uid) {
  char        hts_home[PATH_MAX +
      sizeof("/.hts/tvheadend")]; /* Must be largest of the 3 config strings! */
  ...
  if (stat(hts_home, &st) == 0) {
    if (S_ISLNK(st.st_mode)) {
      ...
      if ((readlink(hts_home, hts_home_link, sizeof(hts_home_link)) == -1) ||
        ...
        tvherror(LS_CONFIG,
            ".hts/tvheadend is inaccessible: %s",
            strerror(errno));
        return NULL;
      }
    ...
    tvhwarn(LS_CONFIG,
        "Found legacy '.hts/tvheadend', consider moving this to '.config/hts' "
        "instead.");
...
void config_boot(const char* path,
  ...
  if (stat(config.confdir, &st)) {
    ...
    if (makedirs(LS_CONFIG, config.confdir, 0700, 1, gid, uid)) {
      tvhwarn(LS_START,
          "failed to create settings directory %s,"
  ...
  /* And is usable */
    ...
    tvhwarn(LS_START,
        "configuration path %s is not r/w"
        " for UID:%d GID:%d [e=%s],"
        " settings will not be saved",
        config.confdir,
        getuid(),
  ...
  if (chown(config_lock, uid, gid))
    tvhwarn(LS_CONFIG,
        "unable to chown lock file %s UID:%d GID:%d",
        config_lock,
        uid,
        gid);
...
static int config_class_cors_origin_set(void* o, const void* v) {
  const char* s = v;
  url_t       u;
  ...
  if (*s == '*') {
    ...
    if (urlparse(s, &u))
      goto wrong;
    if (u.scheme && (!strcmp(u.scheme, "http") || !strcmp(u.scheme, "https")) &&
        u.host) {
      if (u.port)
        snprintf(prop_sbuf,
            PROP_SBUF_LEN,
...
static const void* config_class_language_get(void* o) {
  return htsmsg_csv_2_list(config.language, ',');
}

static int config_class_language_set(void* o, const void* v) {
...
static htsmsg_t* config_class_language_list(void* o, const char* lang) {
  htsmsg_t* m = htsmsg_create_map();
...
static const void* config_class_info_area_get(void* o) {
  return htsmsg_csv_2_list(config.info_area, ',');
}

static int config_class_info_area_set(void* o, const void* v) {
  char* s = htsmsg_list_2_csv((htsmsg_t*)v, ',', 3);
...
static void config_class_info_area_list1(htsmsg_t* m,
    const char*                                    key,
    const char*                                    val,
    const char*                                    lang) {
...
static htsmsg_t* config_class_info_area_list(void* o, const char* lang) {
...
static htsmsg_t* config_class_dscp_list(void* o, const char* lang) {
...
static htsmsg_t* config_class_uilevel(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static htsmsg_t* config_class_chiconscheme_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static htsmsg_t* config_class_piconscheme_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static htsmsg_t* config_class_http_auth_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static htsmsg_t* config_class_http_auth_algo_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static void config_muxconfpath_notify_cb(void* opaque, int disarmed) {
  ...
  tvhinfo(LS_CONFIG,
      "scanfile (re)initialization with path %s",
      muxconf_path ?: "<none>");
...
const idclass_t config_class = {.ic_snode = &config.idnode,
    .ic_class                             = "config",
    ...
    .ic_properties                        = (const property_t[]){
        ...
        {.type    = PT_BOOL,
            ...
            .desc =
                N_("Add sources (like DVB-T string) to the channel name list"),
        ...
        {.type     = PT_U32,
            ...
            .desc  = N_("The time window to cut the stop time from the "
                       "overlapped event in seconds."),
        ...
        {.type    = PT_U32,
            ...
            .desc =
                N_("Maximum allowed difference between event start time when "
            ...
            .group = 3},
        {
            ...
            .desc = N_(
                "If both a picon and a channel-specific "
        ...
        {.type    = PT_INT,
            ...
            .desc = N_("Digest access authentication is intended as a security "
                       "trade-off. "
                       "It is intended to replace unencrypted HTTP basic "
        ...
        {.type    = PT_INT,
            ...
            .desc =
                N_("The hash algorithm type for the digest authentication."),
        ...
        {.type        = PT_U32,
            ...
            .desc  = N_("The number of seconds in which authentication tickets "
                       "generated by "
        ...
        {.type    = PT_STR,
            ...
            .desc = N_(
                "IP address of the HDHomerun device. This is needed if you "
        ...
        {.type    = PT_STR,
            ...
            .desc =
                N_("IP of the Docker host. Each HDHomeRun tuner sends data "
        ...
        {.type    = PT_INT,
            ...
            .desc =
                N_("Starting port number of the UDP listeners. The listeners "
            ...
            .group = 6},
        {
            ...
            .name =
                N_("Number of tuners to export for HDHomeRun Server Emulation"),
            .desc =
                N_("When Tvheadend is acting as an HDHomeRun Server "
        ...
        },
        {
            ...
            .desc =
                N_("The number of MPEG-TS packets Tvheadend buffers in case "
