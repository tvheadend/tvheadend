...
static const char* json_parse_value(const char* s,
    void*                                       parent,
    const char*                                 name,
 ...
 */
static char*
json_parse_string(const char* s, const char** endp, const char** failp, const char** failmsg) {
  const char* start;
  char *      r, *a, *b;
  int         l, esc = 0;
 ...
 *
 */
static void* json_parse_map(const char* s,
    const char**                        endp,
    const json_deserializer_t*          jd,
    void*                               opaque,
    const char**                        failp,
 ...
 *
 */
static void* json_parse_list(const char* s,
    const char**                         endp,
    const json_deserializer_t*           jd,
    void*                                opaque,
    const char**                         failp,
 ...
 *
 */
static const char* json_parse_double(const char* s, double* dp) {
  const char* ep;
  while (*s > 0 && *s < 33)
    s++;
 ...
 *
 */
static char* json_parse_integer(const char* s, int64_t* lp) {
  char* ep;
  while (*s > 0 && *s < 33)
    s++;
  const char* s2 = s;
...
void* json_deserialize(const char* src,
  ...
  if (c == NULL) {
    ...
    if (offset > len || offset < 0) {
      ...
      if (offset < 0)
        offset = 0;
      snprintf(errbuf, errlen, "%s at offset %d : '%.20s'", errmsg, (int)offset, src + offset);
    }
  }
  return c;
}
