...
#define LIST_LAST(headname, head, field)                                     \
  ({                                                                         \
    headname* r;                                                             \
    for (r = LIST_FIRST(head); LIST_NEXT(r, field); r = LIST_NEXT(r, field)) \
      ;                                                                      \
    r;                                                                       \
  })
...
#define LIST_PREV(elm, headname, head, field)                                              \
  ({                                                                                       \
    headname* r;                                                                           \
    for (r = LIST_FIRST(head); r && LIST_NEXT(r, field) != (elm); r = LIST_NEXT(r, field)) \
      ;                                                                                    \
    r;                                                                                     \
  })

#define LIST_ENTRY_INIT(elm, field) (elm)->field.le_next = NULL, (elm)->field.le_prev = NULL
...
#define LIST_SAFE_ENTRY(elm, field) ((elm)->field.le_next == NULL && (elm)->field.le_prev == NULL)

#define LIST_MOVE(newhead, oldhead, field)                       \
  do {                                                           \
    if ((oldhead)->lh_first) {                                   \
...
#define LIST_INSERT_SORTED(head, elm, field, cmpfunc) \
  do {                                                \
    if (LIST_EMPTY(head)) {                           \
...
#define TAILQ_INSERT_SORTED(head, elm, field, cmpfunc) \
  do {                                                 \
...
#define TAILQ_INSERT_SORTED_R(head, headname, elm, field, cmpfunc) \
  do {                                                             \
    if (TAILQ_FIRST(head) == NULL) {                               \
      TAILQ_INSERT_HEAD(head, elm, field);                         \
    } else {                                                       \
