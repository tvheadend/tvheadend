...
typedef enum { PROFILE_WORK_NONE = 0, PROFILE_WORK_REMOTE_TS } profile_work_flags_t;
...
typedef struct profile_chain {
  ...
  int (*prch_can_share)(struct profile_chain* prch, struct profile_chain* joiner);
} profile_chain_t;
...
typedef struct profile {
  ...
  int (*pro_work)(profile_chain_t* prch,
      struct streaming_target*     dst,
      uint32_t                     timeshift_period,
      profile_work_flags_t         flags);
  int (*pro_reopen)(profile_chain_t* prch, muxer_config_t* m_cfg, muxer_hints_t* hints, int flags);
...
static inline void profile_release(profile_t* pro) {
  ...
  if (--pro->pro_refcount == 0)
    profile_release_(pro);
}
