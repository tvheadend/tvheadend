 ...
 */
static const char* audioes_muxer_mime(muxer_t* m,
    const struct streaming_start*              ss) {
  muxer_container_type_t             mc = MC_UNKNOWN;
  const streaming_start_component_t* ssc;
 ...
 */
static int audioes_muxer_reconfigure(muxer_t* m,
    const struct streaming_start*             ss) {
 ...
 * NOP
 */
static int audioes_muxer_write_meta(muxer_t* m,
...
muxer_t* audioes_muxer_create(const muxer_config_t* m_cfg,
  ...
  if (m_cfg->m_type != MC_MPEG2AUDIO && m_cfg->m_type != MC_AC3 &&
      m_cfg->m_type != MC_AAC && m_cfg->m_type != MC_MP4A &&
      m_cfg->m_type != MC_VORBIS && m_cfg->m_type != MC_AC4)
    return NULL;
