 ...
 *
 */
static int audioes_muxer_type(streaming_component_type_t type) {
 ...
 *
 */
static const streaming_start_component_t* audioes_get_component(muxer_t* m,
 ...
 */
static const char* audioes_muxer_mime(muxer_t* m, const struct streaming_start* ss) {
  muxer_container_type_t             mc = MC_UNKNOWN;
  const streaming_start_component_t* ssc;
 ...
 */
static int audioes_muxer_reconfigure(muxer_t* m, const struct streaming_start* ss) {
 ...
 */
static int audioes_muxer_init(muxer_t* m, struct streaming_start* ss, const char* name) {
 ...
 */
static int audioes_muxer_open_stream(muxer_t* m, int fd) {
  audioes_muxer_t* am = (audioes_muxer_t*)m;
 ...
 */
static int audioes_muxer_open_file(muxer_t* m, const char* filename) {
  int              fd;
  audioes_muxer_t* am = (audioes_muxer_t*)m;
  ...
  tvhtrace(LS_AUDIOES,
      ...
      filename,
      am->m_config.m_file_permissions);

  fd = open(filename, O_WRONLY | O_CREAT | O_TRUNC, am->m_config.m_file_permissions);

  if (fd < 0) {
    am->am_error = errno;
    tvherror(LS_AUDIOES, "%s: Unable to create file, open failed -- %s", filename, strerror(errno));
    am->m_errors++;
    return -1;
  }
  ...
  if (fchmod(fd, am->m_config.m_file_permissions))
    tvherror(LS_AUDIOES, "%s: Unable to change permissions -- %s", filename, strerror(errno));
 ...
 */
static int audioes_muxer_write_pkt(muxer_t* m, streaming_message_type_t smt, void* data) {
  ...
  if (am->am_error) {
    ...
    if (!MC_IS_EOS_ERROR(errno)) {
      tvherror(LS_AUDIOES, "%s: Write failed -- %s", am->am_filename, strerror(errno));
    } else {
      am->m_eos = 1;
    }
    am->m_errors++;
 ...
 * NOP
 */
static int audioes_muxer_write_meta(muxer_t* m, struct epg_broadcast* eb, const char* comment) {
  return 0;
}
 ...
 * Close the muxer
 */
static int audioes_muxer_close(muxer_t* m) {
  audioes_muxer_t* am = (audioes_muxer_t*)m;

  if ((am->am_seekable) && (close(am->am_fd))) {
    am->am_error = errno;
    tvherror(LS_AUDIOES, "%s: Unable to close file -- %s", am->am_filename, strerror(errno));
    am->m_errors++;
    return -1;
  }
 ...
 */
static void audioes_muxer_destroy(muxer_t* m) {
  audioes_muxer_t* am = (audioes_muxer_t*)m;
...
muxer_t* audioes_muxer_create(const muxer_config_t* m_cfg, const muxer_hints_t* hints) {
  ...
  if (m_cfg->m_type != MC_MPEG2AUDIO && m_cfg->m_type != MC_AC3 && m_cfg->m_type != MC_AAC &&
      m_cfg->m_type != MC_MP4A && m_cfg->m_type != MC_VORBIS && m_cfg->m_type != MC_AC4)
    return NULL;
