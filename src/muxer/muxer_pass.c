...
/*
 * Rewrite a PAT packet to only include the service included in the transport
 * stream.
 */
static void
pass_muxer_pat_cb(mpegts_psi_table_t* mt, const uint8_t* buf, int len) {
  ...
  out[1] = 0x80;
  out[2] =
      13; /* section_length (number of bytes after this field, including CRC) */
 ...
 */
static const char* pass_muxer_mime(muxer_t* m,
 ...
 */
static int pass_muxer_reconfigure(muxer_t* m,
    const struct streaming_start*          ss) {
...
static int pass_muxer_open2(pass_muxer_t* pm) {
  ...
  if (cmdline && cmdline[0]) {
    ...
    if (spawn_parse_args(&argv, 64, cmdline, NULL))
      goto error;
    if (spawn_with_passthrough(argv[0],
            argv,
            NULL,
            pm->pm_ofd,
            &pm->pm_fd,
            ...
            1)) {
      tvherror(LS_PASS,
          "Unable to start pipe '%s' (wrong executable?)",
          cmdline);
      goto error;
    }
...
static void pass_muxer_write(muxer_t* m, const void* data, size_t size) {
  ...
  if (pm->m_config.m_output_chunk > 0) {
    ret =
        tvh_write_in_chunks(pm->pm_fd, data, size, pm->m_config.m_output_chunk);
  } else {
...
static void pass_muxer_write_ts(muxer_t* m, pktbuf_t* pb) {
  ...
  if (pm->m_config.u.pass.m_rewrite_pat || pm->m_config.u.pass.m_rewrite_pmt ||
    ...
    for (tsb = pktbuf_ptr(pb), len2 = pktbuf_len(pb), len = 0; len2 > 0;
         tsb += l, len2 -= l) {
 ...
 * NOP
 */
static int pass_muxer_write_meta(muxer_t* m,
...
muxer_t* pass_muxer_create(const muxer_config_t* m_cfg,
  ...
  dvb_table_parse_init(&pm->pm_nit,
      ...
      DVB_NIT_MASK,
      pm);
  dvb_table_parse_init(&pm->pm_eit,
      "pass-eit",
      LS_TBL_PASS,
      DVB_EIT_PID,
      0,
