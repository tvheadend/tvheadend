...
static void pass_muxer_write(muxer_t* m, const void* data, size_t size);

/*
 * Rewrite a PAT packet to only include the service included in the transport stream.
 */
static void pass_muxer_pat_cb(mpegts_psi_table_t* mt, const uint8_t* buf, int len) {
  pass_muxer_t* pm;
  uint8_t       out[16], *ob;
  int           ol, l;
 ...
 *
 */
static void pass_muxer_pmt_cb(mpegts_psi_table_t* mt, const uint8_t* buf, int len) {
  pass_muxer_t*                      pm;
  uint8_t                            out[1024], *ob;
  uint16_t                           sid, pid;
  int                                l, ol, i;
 ...
 *
 */
static void pass_muxer_sdt_cb(mpegts_psi_table_t* mt, const uint8_t* buf, int len) {
  pass_muxer_t* pm;
  uint8_t       out[1024], *ob;
  uint16_t      sid;
  int           l, ol;
 ...
 *
 */
static void pass_muxer_nit_cb(mpegts_psi_table_t* mt, const uint8_t* buf, int len) {
  pass_muxer_t* pm;
 ...
 *
 */
static void pass_muxer_eit_cb(mpegts_psi_table_t* mt, const uint8_t* buf, int len) {
  pass_muxer_t* pm;
  uint16_t      sid;
  uint8_t *     sbuf, *out;
  int           olen;
 ...
 */
static const char* pass_muxer_mime(muxer_t* m, const struct streaming_start* ss) {
  int                                i;
  int                                has_audio;
  int                                has_video;
 ...
 */
static int pass_muxer_reconfigure(muxer_t* m, const struct streaming_start* ss) {
  ...
  if (pm->m_config.u.pass.m_rewrite_pmt) {
    ...
    dvb_table_parse_init(&pm->pm_pmt,
        "pass-pmt",
        LS_TBL_PASS,
        pm->pm_pmt_pid,
        DVB_PMT_BASE,
 ...
 */
static int pass_muxer_init(muxer_t* m, struct streaming_start* ss, const char* name) {
 ...
 */
static int pass_muxer_open2(pass_muxer_t* pm) {
 ...
 */
static int pass_muxer_open_stream(muxer_t* m, int fd) {
  pass_muxer_t* pm = (pass_muxer_t*)m;
 ...
 */
static int pass_muxer_open_file(muxer_t* m, const char* filename) {
  int           fd;
  pass_muxer_t* pm = (pass_muxer_t*)m;
  ...
  tvhtrace(LS_PASS,
      ...
      filename,
      pm->m_config.m_file_permissions);

  fd = open(filename, O_WRONLY | O_CREAT | O_TRUNC, pm->m_config.m_file_permissions);

  if (fd < 0) {
    pm->pm_error = errno;
    tvherror(LS_PASS, "%s: Unable to create file, open failed -- %s", filename, strerror(errno));
    pm->m_errors++;
    return -1;
  }
  ...
  if (fchmod(fd, pm->m_config.m_file_permissions))
    tvherror(LS_PASS, "%s: Unable to change permissions -- %s", filename, strerror(errno));
...
static void pass_muxer_write(muxer_t* m, const void* data, size_t size) {
  ...
  if (ret) {
    ...
    if (!MC_IS_EOS_ERROR(errno))
      tvherror(LS_PASS, "%s: Write failed -- %s", pm->pm_filename, strerror(errno));
    else
 ...
 */
static void pass_muxer_write_ts(muxer_t* m, pktbuf_t* pb) {
  pass_muxer_t* pm = (pass_muxer_t*)m;
  int           l, pid;
  ...
  if (pm->m_config.u.pass.m_rewrite_pat || pm->m_config.u.pass.m_rewrite_pmt ||
    ...
    for (tsb = pktbuf_ptr(pb), len2 = pktbuf_len(pb), len = 0; len2 > 0; tsb += l, len2 -= l) {
 ...
 */
static int pass_muxer_write_pkt(muxer_t* m, streaming_message_type_t smt, void* data) {
 ...
 * NOP
 */
static int pass_muxer_write_meta(muxer_t* m, struct epg_broadcast* eb, const char* comment) {
  return 0;
}
 ...
 */
static int pass_muxer_close(muxer_t* m) {
  pass_muxer_t* pm = (pass_muxer_t*)m;

  if (pm->pm_spawn_pid > 0)
    spawn_kill(pm->pm_spawn_pid,
        tvh_kill_to_sig(pm->m_config.u.pass.m_killsig),
        pm->m_config.u.pass.m_killtimeout);
  if (pm->pm_seekable && close(pm->pm_ofd)) {
    pm->pm_error = errno;
    tvherror(LS_PASS,
        "%s: Unable to close file, close failed -- %s",
        pm->pm_filename,
        strerror(errno));
    pm->m_errors++;
    return -1;
 ...
 */
static void pass_muxer_destroy(muxer_t* m) {
  pass_muxer_t* pm = (pass_muxer_t*)m;
...
muxer_t* pass_muxer_create(const muxer_config_t* m_cfg, const muxer_hints_t* hints) {
  ...
  dvb_table_parse_init(&pm->pm_pat,
      "pass-pat",
      LS_TBL_PASS,
      DVB_PAT_PID,
      DVB_PAT_BASE,
      DVB_PAT_MASK,
      pm);
  dvb_table_parse_init(&pm->pm_pmt, "pass-pmt", LS_TBL_PASS, 100, DVB_PMT_BASE, DVB_PMT_MASK, pm);
  dvb_table_parse_init(&pm->pm_sdt,
      "pass-sdt",
      LS_TBL_PASS,
      DVB_SDT_PID,
      DVB_SDT_BASE,
      DVB_SDT_MASK,
      pm);
  dvb_table_parse_init(&pm->pm_nit,
      "pass-nit",
      LS_TBL_PASS,
      DVB_NIT_PID,
      DVB_NIT_BASE,
      DVB_NIT_MASK,
      pm);
  dvb_table_parse_init(&pm->pm_eit, "pass-eit", LS_TBL_PASS, DVB_EIT_PID, 0, 0, pm);
