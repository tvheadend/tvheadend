 ...
 *
 */
static htsbuf_queue_t* mk_build_ebmlheader(mk_muxer_t* mk) {
 ...
 */
static int mk_split_xiph_headers(uint8_t* extradata,
    int                                   extradata_size,
 ...
 *
 */
static htsbuf_queue_t* mk_build_segment_info(mk_muxer_t* mk) {
  htsbuf_queue_t* q = htsbuf_queue_alloc(0);
  char            app[128];
 ...
 *
 */
static htsbuf_queue_t* mk_build_tracks(mk_muxer_t* mk, streaming_start_t* ss) {
  ...
  for (i = 0; i < ss->ss_num_components; i++) {
    ...
    if (mk->webm && ssc->es_type != SCT_VP8 && ssc->es_type != SCT_VORBIS)
      tvhwarn(LS_MKV,
          "WEBM format supports only VP8+VORBIS streams (detected %s)",
    ...
    switch (ssc->es_type) {
      ...
      case SCT_OPUS:
        if (ssc->ssc_gh) {
          ...
          sbuf_init(&hdr);
          if (tr->avc) {
            isom_write_avcc(&hdr, pktbuf_ptr(ssc->ssc_gh), pktbuf_len(ssc->ssc_gh));
          } else if (tr->hevc) {
            isom_write_hvcc(&hdr, pktbuf_ptr(ssc->ssc_gh), pktbuf_len(ssc->ssc_gh));
          } else {
            sbuf_append(&hdr, pktbuf_ptr(ssc->ssc_gh), pktbuf_len(ssc->ssc_gh));
          }
      ...
      case SCT_VORBIS:
        if (ssc->ssc_gh) {
          ...
          if (mk_split_xiph_headers(pktbuf_ptr(ssc->ssc_gh),
    ...
    if (SCT_ISVIDEO(ssc->es_type)) {
      ...
      if (ssc->es_aspect_num && ssc->es_aspect_den) {
        if (mk->webm) {
          ...
          ebml_append_uint(vi,
              0x54b2,
              0); // DisplayUnit: pixels because DAR is not supported by webm
        } else {
 ...
 *
 */
static int mk_write_to_fd(mk_muxer_t* mk, htsbuf_queue_t* hq) {
  htsbuf_data_t* hd;
  int            i      = 0;
 ...
 *
 */
static void mk_write_queue(mk_muxer_t* mk, htsbuf_queue_t* q) {
 ...
 *
 */
static void mk_write_master(mk_muxer_t* mk, uint32_t id, htsbuf_queue_t* p) {
  htsbuf_queue_t q;
 ...
 *
 */
static htsbuf_queue_t* mk_build_segment_header(int64_t size) {
  htsbuf_queue_t* q = htsbuf_queue_alloc(0);
  uint8_t         u8[8];
 ...
 *
 */
static void mk_write_segment_header(mk_muxer_t* mk, int64_t size) {
  htsbuf_queue_t* q;
 ...
 *
 */
static htsbuf_queue_t* mk_build_one_chapter(mk_chapter_t* ch) {
  htsbuf_queue_t* q = htsbuf_queue_alloc(0);
 ...
 *
 */
static htsbuf_queue_t* mk_build_edition_entry(mk_muxer_t* mk) {
  mk_chapter_t*   ch;
  htsbuf_queue_t* q = htsbuf_queue_alloc(0);
 ...
 *
 */
static htsbuf_queue_t* mk_build_chapters(mk_muxer_t* mk) {
  htsbuf_queue_t* q = htsbuf_queue_alloc(0);
 ...
 *
 */
static htsbuf_queue_t* build_tag_string(const char* name,
    const char*                                     value,
    const char*                                     lang,
    int                                             targettype,
...
build_tag_int(const char* name, int value, int targettype, const char* targettypename) {
  char str[64];
 ...
 *
 */
static void addtag(htsbuf_queue_t* q, htsbuf_queue_t* t) {
 ...
 */
static htsbuf_queue_t*
_mk_build_metadata(const dvr_entry_t* de, const epg_broadcast_t* ebc, const char* comment) {
  htsbuf_queue_t*         q = htsbuf_queue_alloc(0);
  ...
  if (ch)
    addtag(q,
        build_tag_string("TVCHANNEL", channel_get_name(ch, channel_blank_name), NULL, 0, NULL));
  ...
  if (num.e_num)
    addtag(q, build_tag_int("PART_NUMBER", num.e_num, 0, NULL));
  if (num.s_num)
    addtag(q, build_tag_int("PART_NUMBER", num.s_num, 60, "SEASON"));
  if (num.p_num)
    addtag(q, build_tag_int("PART_NUMBER", num.p_num, 40, "PART"));
  if (num.text)
    addtag(q, build_tag_string("SYNOPSIS", num.text, NULL, 0, NULL));
 ...
 *
 */
static htsbuf_queue_t* mk_build_one_metaseek(mk_muxer_t* mk, uint32_t id, off_t pos) {
  htsbuf_queue_t* q = htsbuf_queue_alloc(0);
 ...
 *
 */
static void mk_append_master(mk_muxer_t* mk, htsbuf_queue_t* q, off_t id1, off_t id2, off_t val) {
 ...
 *
 */
static htsbuf_queue_t* mk_build_metaseek(mk_muxer_t* mk) {
  htsbuf_queue_t* q = htsbuf_queue_alloc(0);
 ...
 *
 */
static void mk_write_metaseek(mk_muxer_t* mk, int first) {
  htsbuf_queue_t q;
 ...
 *
 */
static htsbuf_queue_t* mk_build_segment(mk_muxer_t* mk, streaming_start_t* ss) {
 ...
 *
 */
static void addcue(mk_muxer_t* mk, int64_t pts, int tracknum) {
 ...
 *
 */
static void mk_add_chapter0(mk_muxer_t* mk, uint32_t uuid, int64_t ts) {
  mk_chapter_t* ch;
 ...
 *
 */
static void mk_add_chapter(mk_muxer_t* mk, int64_t ts) {
  mk_chapter_t* ch;
  int           uuid;
 ...
 *
 */
static void mk_close_cluster(mk_muxer_t* mk) {
  if (mk->cluster != NULL)
 ...
 *
 */
static void mk_write_frame_i(mk_muxer_t* mk, mk_track_t* t, th_pkt_t* pkt) {
 ...
 *
 */
static void mk_write_cues(mk_muxer_t* mk) {
  mk_cue_t*       mc;
 ...
 */
static int mk_mux_write_pkt(mk_muxer_t* mk, th_pkt_t* pkt) {
  int         i, mark;
  mk_track_t* t = NULL;
  th_pkt_t *  opkt, *tpkt;
  ...
  if (mk->dvbsub_reorder && pkt->pkt_type == SCT_DVBSUB &&
      pts_diff(pkt->pkt_pcr, pkt->pkt_pts) > 90000) {
    tvhtrace(LS_MKV,
        "insert pkt to holdq: pts %" PRId64 ", pcr %" PRId64 ", diff %" PRId64 "\n",
        pkt->pkt_pcr,
        pkt->pkt_pts,
  ...
  if (SCT_ISAUDIO(pkt->pkt_type)) {
    while ((opkt = pktref_first(&mk->holdq)) != NULL) {
      ...
      tvhtrace(LS_MKV,
          "hold push, pts %" PRId64 ", audio pts %" PRId64 "\n",
          opkt->pkt_pts,
          pkt->pkt_pts);
    ...
    if (pkt->a.pkt_channels != t->channels && pkt->a.pkt_channels) {
      mark        = 1;
      t->channels = pkt->a.pkt_channels;
    }
    if (pkt->a.pkt_sri != t->sri && pkt->a.pkt_sri) {
      mark       = 1;
      t->sri     = pkt->a.pkt_sri;
      t->ext_sri = pkt->a.pkt_ext_sri;
    }
  } else if (SCT_ISVIDEO(pkt->pkt_type)) {
    if (pkt->v.pkt_aspect_num != t->aspect_num && pkt->v.pkt_aspect_num) {
      mark          = 1;
      t->aspect_num = pkt->v.pkt_aspect_num;
    }
    if (pkt->v.pkt_aspect_den != t->aspect_den && pkt->v.pkt_aspect_den) {
      mark          = 1;
      t->aspect_den = pkt->v.pkt_aspect_den;
    }
  }
  if (pkt->pkt_commercial != t->commercial && pkt->pkt_commercial != COMMERCIAL_UNKNOWN) {
    mark          = 1;
...
static int mk_mux_close(mk_muxer_t* mk) {
  ...
  if (mk->seekable) {
    ...
    if (lseek(mk->fd, mk->segmentinfo_pos, SEEK_SET) == mk->segmentinfo_pos)
      ...
      mk->error = errno;
      tvherror(LS_MKV,
          "%s: Unable to write duration, seek failed -- %s",
          mk->filename,
          strerror(errno));
    }
    ...
    if (lseek(mk->fd, mk->segment_header_pos, SEEK_SET) == mk->segment_header_pos) {
      ...
      mk->error = errno;
      tvherror(LS_MKV,
          "%s: Unable to write total size, seek failed -- %s",
          mk->filename,
          strerror(errno));
    }
    ...
    if (close(mk->fd)) {
      mk->error = errno;
      tvherror(LS_MKV,
          "%s: Unable to close the file descriptor, close failed -- %s",
          mk->filename,
          strerror(errno));
    }
  }
 ...
 */
static const char* mkv_muxer_mime(muxer_t* m, const struct streaming_start* ss) {
  int                                i;
  int                                has_audio;
  int                                has_video;
 ...
 */
static int mkv_muxer_init(muxer_t* m, streaming_start_t* ss, const char* name) {
 ...
 */
static int mkv_muxer_reconfigure(muxer_t* m, const struct streaming_start* ss) {
  mk_muxer_t* mk = (mk_muxer_t*)m;
 ...
 */
static int mkv_muxer_open_stream(muxer_t* m, int fd) {
  mk_muxer_t* mk = (mk_muxer_t*)m;
 ...
 * Open a file
 */
static int mkv_muxer_open_file(muxer_t* m, const char* filename) {
  mk_muxer_t* mk = (mk_muxer_t*)m;
  int         fd, permissions = mk->m_config.m_file_permissions;

  tvhtrace(LS_MKV, "Creating file \"%s\" with file permissions \"%o\"", filename, permissions);

  fd = open(filename, O_WRONLY | O_CREAT | O_TRUNC, permissions);

  if (fd < 0) {
    mk->error = errno;
    tvherror(LS_MKV, "%s: Unable to create file, open failed -- %s", mk->filename, strerror(errno));
    mk->m_errors++;
    return -1;
  }
  ...
  if (fchmod(fd, permissions))
    tvherror(LS_MKV, "%s: Unable to change permissions -- %s", filename, strerror(errno));
 ...
 */
static int mkv_muxer_write_pkt(muxer_t* m, streaming_message_type_t smt, void* data) {
 ...
 */
static int mkv_muxer_write_meta(muxer_t* m, struct epg_broadcast* eb, const char* comment) {
  mk_muxer_t*    mk = (mk_muxer_t*)m;
  htsbuf_queue_t q;
 ...
 */
static int mkv_muxer_close(muxer_t* m) {
  mk_muxer_t* mk = (mk_muxer_t*)m;
 ...
 */
static void mkv_muxer_destroy(muxer_t* m) {
  mk_muxer_t*   mk = (mk_muxer_t*)m;
  mk_chapter_t* ch;
