    ...
    if (SCT_ISVIDEO(ssc->es_type)) {
      ...
      if (ssc->es_aspect_num && ssc->es_aspect_den) {
        if (mk->webm) {
          ebml_append_uint(vi,
              0x54b0,
              (ssc->es_height * ssc->es_aspect_num) / ssc->es_aspect_den);
 ...
 *
 */
static htsbuf_queue_t* build_tag_int(const char* name,
    int                                          value,
    int                                          targettype,
    const char*                                  targettypename) {
  char str[64];
 ...
 *
 */
static htsbuf_queue_t* _mk_build_metadata(const dvr_entry_t* de,
  ...
  // TODO - MKV The spec suggests that this tag can consist of multiple value
  // types. https://www.matroska.org/technical/tagging.html
  //==> Depending on the COUNTRY it’s the format of the rating of a movie
  //==> (P, R, X in the USA, an age in other countries or a URI defining a
  // logo).
 ...
 *
 */
static void mk_append_master(mk_muxer_t* mk,
    htsbuf_queue_t*                      q,
    off_t                                id1,
    off_t                                id2,
    off_t                                val) {
...
static int mk_mux_close(mk_muxer_t* mk) {
  ...
  if (mk->seekable) {
    ...
    if (lseek(mk->fd, mk->segment_header_pos, SEEK_SET) ==
        mk->segment_header_pos) {
 ...
 */
static const char* mkv_muxer_mime(muxer_t* m,
...
static int mkv_muxer_open_file(muxer_t* m, const char* filename) {
  ...
  tvhtrace(LS_MKV,
      "Creating file \"%s\" with file permissions \"%o\"",
      filename,
      permissions);
  ...
  if (fd < 0) {
    mk->error = errno;
    tvherror(LS_MKV,
        "%s: Unable to create file, open failed -- %s",
        mk->filename,
        strerror(errno));
    mk->m_errors++;
    return -1;
  ...
  if (fchmod(fd, permissions))
    tvherror(LS_MKV,
        "%s: Unable to change permissions -- %s",
        filename,
        strerror(errno));
 ...
 */
static int mkv_muxer_write_meta(muxer_t* m,
    struct epg_broadcast*                eb,
    const char*                          comment) {
  mk_muxer_t*    mk = (mk_muxer_t*)m;
  htsbuf_queue_t q;
