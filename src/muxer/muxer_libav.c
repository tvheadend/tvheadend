 ...
 */
static int lav_muxer_write(void* opaque, uint8_t* buf, int buf_size) {
  int          r;
 ...
 */
static int lav_muxer_add_stream(lav_muxer_t* lm, const streaming_start_component_t* ssc) {
  AVStream*       st;
  AVCodecContext* c;
  const AVCodec*  codec;
  int             rc = -1;
 ...
 */
static int lav_muxer_support_stream(muxer_container_type_t mc, streaming_component_type_t type) {
  int ret = 0;
 ...
 */
static const char* lav_muxer_mime(muxer_t* m, const struct streaming_start* ss) {
  int                                i;
  int                                has_audio;
  int                                has_video;
 ...
 */
static int lav_muxer_init(muxer_t* m, struct streaming_start* ss, const char* name) {
  int                          i;
  ...
  for (i = 0; i < ss->ss_num_components; i++) {
    ...
    if (lav_muxer_add_stream(lm, ssc)) {
      tvherror(LS_LIBAV, "Failed to add %s stream", streaming_component_type2txt(ssc->es_type));
      ssc->ssc_muxer_disabled = 1;
      continue;
    }
  }
  ...
  if (!lm->lm_oc->nb_streams) {
    ...
    tvherror(LS_LIBAV, "Failed to write %s header", muxer_container_type2txt(lm->m_config.m_type));
    lm->m_errors++;
    goto fail;
  }
 ...
 */
static int lav_muxer_reconfigure(muxer_t* m, const struct streaming_start* ss) {
  lav_muxer_t* lm = (lav_muxer_t*)m;
 ...
 */
static int lav_muxer_open_stream(muxer_t* m, int fd) {
  uint8_t*     buf;
  AVIOContext* pb;
  lav_muxer_t* lm = (lav_muxer_t*)m;

  buf           = av_malloc(MUX_BUF_SIZE);
  pb            = avio_alloc_context(buf, MUX_BUF_SIZE, 1, lm, NULL, lav_muxer_write, NULL);
  pb->seekable  = 0;
  lm->lm_oc->pb = pb;
  lm->lm_fd     = fd;
...
static int lav_muxer_open_file(muxer_t* m, const char* filename) {
  AVFormatContext* oc;
  lav_muxer_t*     lm = (lav_muxer_t*)m;
  char             buf[256];
  int              r;
  ...
  if (chmod(filename, lm->m_config.m_file_permissions))
    tvherror(LS_LIBAV, "%s: Unable to change permissions -- %s", filename, strerror(errno));
 ...
 */
static int lav_muxer_write_pkt(muxer_t* m, streaming_message_type_t smt, void* data) {
  int              i;
  AVFormatContext* oc;
  AVStream*        st;
  AVPacket         packet;
  ...
  for (i = 0; i < oc->nb_streams; i++) {
    ...
    packet.pts      = av_rescale_q_rnd(pkt->pkt_pts,
        mpeg_tc,
        st->time_base,
        AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
    packet.dts      = av_rescale_q_rnd(pkt->pkt_dts,
        mpeg_tc,
        st->time_base,
        AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
    packet.duration = av_rescale_q_rnd(pkt->pkt_duration,
        mpeg_tc,
        st->time_base,
        AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
 ...
 * NOP
 */
static int lav_muxer_write_meta(muxer_t* m, struct epg_broadcast* eb, const char* comment) {
  return 0;
}
 ...
 * NOP
 */
static int lav_muxer_add_marker(muxer_t* m) {
  return 0;
}
 ...
 */
static int lav_muxer_close(muxer_t* m) {
  AVFormatContext* oc;
  int              ret = 0;
  lav_muxer_t*     lm  = (lav_muxer_t*)m;

  if (lm->lm_init && av_write_trailer(lm->lm_oc) < 0) {
    tvhwarn(LS_LIBAV, "Failed to write %s trailer", muxer_container_type2txt(lm->m_config.m_type));
    lm->m_errors++;
    ret = -1;
  }
 ...
 */
static void lav_muxer_destroy(muxer_t* m) {
  int          i;
  lav_muxer_t* lm = (lav_muxer_t*)m;
