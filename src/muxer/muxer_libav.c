 ...
 */
static int lav_muxer_add_stream(lav_muxer_t* lm,
  ...
  if (SCT_ISAUDIO(ssc->es_type)) {
    ...
    if (ssc->es_type == SCT_H264) {
      if (av_bsf_alloc(lm->bsf_h264_filter, &lm->ctx)) {
        tvherror(LS_LIBAV,
            "Failed to alloc AVBitStreamFilter for h264_mp4toannexb");
        goto fail;
      }
    } else if (ssc->es_type == SCT_HEVC) {
      if (av_bsf_alloc(lm->bsf_hevc_filter, &lm->ctx)) {
        tvherror(LS_LIBAV,
            "Failed to alloc AVBitStreamFilter for hevc_mp4toannexb");
        goto fail;
      }
    } else if (ssc->es_type == SCT_VP9) {
      if (av_bsf_alloc(lm->bsf_vp9_filter, &lm->ctx)) {
        tvherror(LS_LIBAV,
            "Failed to alloc AVBitStreamFilter for vp9_superframe ");
        goto fail;
      }
    } else {
      if (av_bsf_alloc(lm->bsf_h264_filter, &lm->ctx)) {
 ...
 */
static int lav_muxer_support_stream(muxer_container_type_t mc,
 ...
 */
static const char* lav_muxer_mime(muxer_t* m,
...
static int lav_muxer_open_stream(muxer_t* m, int fd) {
  ...
  pb =
      avio_alloc_context(buf, MUX_BUF_SIZE, 1, lm, NULL, lav_muxer_write, NULL);
  pb->seekable  = 0;
  lm->lm_oc->pb = pb;
  lm->lm_fd     = fd;
 ...
 * NOP
 */
static int lav_muxer_write_meta(muxer_t* m,
