...
static int
_lang_str_add(lang_str_t* ls, const char* str, const char* lang, int cmd) {
  int             save = 0;
  lang_str_ele_t *e, *ae;
  ...
  /* Get proper code */
  if (!lang)
    lang = lang_code_preferred();
  if (!(lang = lang_code_get(lang)))
    return 0;
...
int lang_str_add(lang_str_t* ls, const char* str, const char* lang) {
...
int lang_str_set(lang_str_t** dst, const char* str, const char* lang) {
  lang_str_ele_t* e;
  int             found;
  if (*dst == NULL)
    goto change1;
  ...
  if (*dst) {
    ...
    RB_FOREACH (e, *dst, link) {
      if (found)
        goto change;
      found = strcmp(e->lang, lang) == 0 && strcmp(e->str, str) == 0;
      if (!found)
        goto change;
    }
    if (found)
...
/* Set new string with update check */
int lang_str_set_multi(lang_str_t** dst, const lang_str_t* src) {
  int             changed = 0;
  lang_str_ele_t* e;
  RB_FOREACH (e, src, link) { changed |= lang_str_set(dst, e->str, e->lang); }
  return changed;
}

/* Set new strings with update check */
int lang_str_set2(lang_str_t** dst, const lang_str_t* src) {
  if (*dst) {
...
void lang_str_serialize_one(htsmsg_t* m,
    const char*                       f,
    const char*                       str,
    const char*                       lang) {
