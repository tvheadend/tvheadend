...
static int download_fetch_complete(http_client_t* hc) {
  ...
  if (hc->hc_code == HTTP_STATUS_OK && hc->hc_result == 0 &&
      hc->hc_data_size > 0)
  ...
  while (1) {
    ...
    if (len == 0) {
      ...
      dn->process(dn->aux,
          p,
          NULL,
          (char*)dn->pipe_sbuf.sb_data,
          (size_t)dn->pipe_sbuf.sb_ptr);
...
static int download_pipe(download_t* dn, const char* args) {
  ...
  /* Grab */
  r = spawn_and_give_stdout(argv[0],
      argv,
      NULL,
      &dn->pipe_fd,
      &dn->pipe_pid,
...
static void download_fetch(void* aux) {
  ...
  hc =
      http_client_connect(dn, HTTP_VERSION_1_1, u.scheme, u.host, u.port, NULL);
  if (hc == NULL) {
