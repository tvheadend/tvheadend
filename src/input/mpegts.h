...
typedef TAILQ_HEAD(mpegts_table_feed_queue, mpegts_table_feed) mpegts_table_feed_queue_t;
...
typedef int (*mpegts_table_callback_t)(mpegts_table_t*, const uint8_t* buf, int len, int tableid);

struct mpegts_table_mux_cb {
  int tag;
  int (*cb)(mpegts_table_t*,
      mpegts_mux_t*  mm,
      uint16_t       nbid,
      const uint8_t  dtag,
      const uint8_t* dptr,
...
struct mpegts_network {
  ...
  mpegts_mux_t* (*mn_create_mux)(mpegts_network_t*,
      void*    origin,
      uint32_t onid,
      uint32_t tsid,
      void*    conf,
      int      force);
  mpegts_service_t* (*mn_create_service)(mpegts_mux_t*, uint16_t sid, uint16_t pmt_pid);
...
mpegts_input_t*
mpegts_input_create0(mpegts_input_t* mi, const idclass_t* idc, const char* uuid, htsmsg_t* c);
...
#define mpegts_input_create1(u, c) \
  mpegts_input_create0(calloc(1, sizeof(mpegts_input_t)), &mpegts_input_class, u, c)
...
void mpegts_network_register_builder(const idclass_t* idc,
    mpegts_network_t* (*build)(const idclass_t* idc, htsmsg_t* conf));

void mpegts_network_unregister_builder(const idclass_t* idc);

mpegts_network_builder_t* mpegts_network_builder_find(const char* clazz);

mpegts_network_t* mpegts_network_build(const char* clazz, htsmsg_t* conf);

mpegts_network_t* mpegts_network_create0(mpegts_network_t* mn,
    const idclass_t*                                       idc,
    const char*                                            uuid,
    const char*                                            name,
    htsmsg_t*                                              conf);
...
mpegts_mux_t*
mpegts_network_find_mux(mpegts_network_t* mn, uint32_t onid, uint32_t tsid, int check);

mpegts_service_t*
mpegts_network_find_active_service(mpegts_network_t* mn, uint16_t sid, mpegts_mux_t** rmm);
...
htsmsg_t* mpegts_network_wizard_get(mpegts_input_t* mi,
    const idclass_t*                                idc,
    mpegts_network_t*                               mn,
    const char*                                     lang);
void      mpegts_network_wizard_create(const char* clazz, htsmsg_t** nlist, const char* lang);

mpegts_mux_t* mpegts_mux_create0(mpegts_mux_t* mm,
...
#define mpegts_mux_create1(uuid, mn, onid, tsid, conf) \
  mpegts_mux_create0(calloc(1, sizeof(mpegts_mux_t)), &mpegts_mux_class, uuid, mn, onid, tsid, conf)
...
#define mpegts_mux_delete_by_uuid(u, delconf) \
  {                                           \
    mpegts_mux_t* mm = mpegts_mux_find(u);    \
    if (mm)                                   \
...
mpegts_mux_instance_t* mpegts_mux_instance_create0(mpegts_mux_instance_t* mmi,
    const idclass_t* class,
    const char*     uuid,
    mpegts_input_t* mi,
    mpegts_mux_t*   mm);
...
#define mpegts_mux_instance_create(type, uuid, mi, mm) \
  (struct type*)                                       \
      mpegts_mux_instance_create0(calloc(1, sizeof(struct type)), &type##_class, uuid, mi, mm);

void mpegts_mux_instance_delete(tvh_input_instance_t* tii);

int mpegts_mux_instance_start(mpegts_mux_instance_t** mmiptr, service_t* t, int weight);
...
int                mpegts_mux_subscribe(mpegts_mux_t* mm,
                   mpegts_input_t*                    mi,
                   const char*                        name,
                   int                                weight,
                   int                                flags);
...
static inline mpegts_pid_t* mpegts_mux_find_pid(mpegts_mux_t* mm, int pid, int create) {
...
void mpegts_input_postdemux(mpegts_input_t* mi, mpegts_mux_t* mm, uint8_t* data, int len);
...
void tsdebug_encode_keys(uint8_t* dst,
    uint16_t                      sid,
    uint16_t                      pid,
    uint8_t                       keytype,
    uint8_t                       keylen,
...
mpegts_table_t*    mpegts_table_add(mpegts_mux_t* mm,
       ...
       int                                        pid,
       int                                        weight);
mpegts_table_t*    mpegts_table_find(mpegts_mux_t* mm, const char* name, void* opaque);
...
int dvb_pat_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);
int dvb_cat_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);
int dvb_pmt_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tabelid);
int dvb_nit_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);
int dvb_bat_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);
int dvb_fs_sdt_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);
int dvb_sdt_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);
int dvb_tdt_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);
int dvb_tot_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);
int atsc_vct_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);
int atsc_stt_callback(struct mpegts_table* mt, const uint8_t* ptr, int len, int tableid);

void psi_tables_install(mpegts_input_t* mi, mpegts_mux_t* mm, dvb_fe_delivery_system_t delsys);

mpegts_service_t* mpegts_service_create0(mpegts_service_t* ms,
    const idclass_t* class,
    const char*   uuid,
    mpegts_mux_t* mm,
    uint16_t      sid,
...
#define mpegts_service_create(t, u, m, s, p, c) \
  (struct t*)mpegts_service_create0(calloc(1, sizeof(struct t)), &t##_class, u, m, s, p, c)

#define mpegts_service_create1(u, m, s, p, c) \
  mpegts_service_create0(calloc(1, sizeof(mpegts_service_t)), &mpegts_service_class, u, m, s, p, c)

mpegts_service_t* mpegts_service_create_raw(mpegts_mux_t* mm);

mpegts_service_t*
mpegts_service_find(mpegts_mux_t* mm, uint16_t sid, uint16_t pmt_pid, int create, int* save);

service_t*
mpegts_service_find_e2(uint32_t stype, uint32_t sid, uint32_t tsid, uint32_t onid, uint32_t hash);

mpegts_service_t* mpegts_service_find_by_pid(mpegts_mux_t* mm, int pid);

void mpegts_service_autoenable(mpegts_service_t* s, const char* where);

void mpegts_service_update_slave_pids(mpegts_service_t* t,
    mpegts_service_t*                                   master_filter,
    int                                                 del);
...
#define mpegts_add_listener(ml) LIST_INSERT_HEAD(&mpegts_listeners, ml, ml_link)

#define mpegts_rem_listener(ml) LIST_REMOVE(ml, ml_link)
