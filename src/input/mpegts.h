...
typedef TAILQ_HEAD(mpegts_table_feed_queue,
    mpegts_table_feed) mpegts_table_feed_queue_t;
...
static inline int
mpegts_pid_wexists(mpegts_apids_t* pids, uint16_t pid, uint16_t weight) {
...
int mpegts_pid_compare_weight(mpegts_apids_t* dst,
    ...
    mpegts_apids_t*                           add,
    mpegts_apids_t*                           del);
int mpegts_pid_weighted(mpegts_apids_t* dst,
    mpegts_apids_t*                     src,
    int                                 limit,
    int                                 mweight);
int mpegts_pid_dump(mpegts_apids_t* pids,
    char*                           buf,
    int                             len,
    int                             wflag,
    int                             raw);
...
typedef int (*mpegts_table_callback_t)(mpegts_table_t*,
    const uint8_t* buf,
    int            len,
    int            tableid);
...
struct mpegts_network {
  ...
  mpegts_mux_t* (*mn_create_mux)(mpegts_network_t*,
      ...
      int      force);
  mpegts_service_t* (
      *mn_create_service)(mpegts_mux_t*, uint16_t sid, uint16_t pmt_pid);
...
#define MM_SCAN_CHECK_OK(mm)             \
  ((mm)->mm_scan_result == MM_SCAN_OK || \
      (mm)->mm_scan_result == MM_SCAN_PARTIAL)
...
struct mpegts_input {
   ...
   */
  int (
      *mi_is_enabled)(mpegts_input_t*, mpegts_mux_t* mm, int flags, int weight);
  void (*mi_enabled_updated)(mpegts_input_t*);
  void (*mi_display_name)(mpegts_input_t*, char* buf, size_t len);
  int (
      *mi_get_weight)(mpegts_input_t*, mpegts_mux_t* mm, int flags, int weight);
  ...
  void (*mi_open_service)(mpegts_input_t*,
      mpegts_service_t*,
      int flags,
      int first,
      int weight);
...
#define mpegts_network_create(t, u, n, c) \
  (struct t*)                             \
      mpegts_network_create0(calloc(1, sizeof(struct t)), &t##_class, u, n, c)
...
mpegts_mux_t* mpegts_network_find_mux(mpegts_network_t* mn,
    uint32_t                                            onid,
    uint32_t                                            tsid,
    int                                                 check);

mpegts_service_t* mpegts_network_find_active_service(mpegts_network_t* mn,
    uint16_t                                                           sid,
    mpegts_mux_t**                                                     rmm);
...
void mpegts_network_get_type_str(mpegts_network_t* mn,
    char*                                          buf,
    size_t                                         buflen);

void mpegts_network_bouquet_trigger0(mpegts_network_t* mn, int timeout);
static inline void mpegts_network_bouquet_trigger(mpegts_network_t* mn,
    int                                                             timeout) {
  if (mn->mn_bouquet)
...
htsmsg_t* mpegts_network_wizard_get(mpegts_input_t* mi,
    ...
    mpegts_network_t*                               mn,
    const char*                                     lang);
void      mpegts_network_wizard_create(const char* clazz,
         htsmsg_t**                                nlist,
         const char*                               lang);
...
#define mpegts_mux_create1(uuid, mn, onid, tsid, conf) \
  mpegts_mux_create0(calloc(1, sizeof(mpegts_mux_t)),  \
      &mpegts_mux_class,                               \
      uuid,                                            \
      mn,                                              \
      onid,                                            \
...
void mpegts_mux_tuning_error(const char* mux_uuid,
    mpegts_mux_instance_t*               mmi_match);
...
void mpegts_mux_remove_subscriber(mpegts_mux_t* mm,
    th_subscription_t*                          s,
    int                                         reason);
...
th_subscription_t* mpegts_mux_find_subscription_by_name(mpegts_mux_t* mm,
    const char*                                                       name);
...
int dvb_pat_callback(struct mpegts_table* mt,
    const uint8_t*                        ptr,
    int                                   len,
    int                                   tableid);
int dvb_cat_callback(struct mpegts_table* mt,
    const uint8_t*                        ptr,
    int                                   len,
    int                                   tableid);
int dvb_pmt_callback(struct mpegts_table* mt,
    const uint8_t*                        ptr,
    int                                   len,
    int                                   tabelid);
int dvb_nit_callback(struct mpegts_table* mt,
    const uint8_t*                        ptr,
    int                                   len,
    int                                   tableid);
int dvb_bat_callback(struct mpegts_table* mt,
    const uint8_t*                        ptr,
    int                                   len,
    int                                   tableid);
int dvb_fs_sdt_callback(struct mpegts_table* mt,
    const uint8_t*                           ptr,
    int                                      len,
    int                                      tableid);
int dvb_sdt_callback(struct mpegts_table* mt,
    const uint8_t*                        ptr,
    int                                   len,
    int                                   tableid);
int dvb_tdt_callback(struct mpegts_table* mt,
    const uint8_t*                        ptr,
    int                                   len,
    int                                   tableid);
int dvb_tot_callback(struct mpegts_table* mt,
    const uint8_t*                        ptr,
    int                                   len,
    int                                   tableid);
int atsc_vct_callback(struct mpegts_table* mt,
    const uint8_t*                         ptr,
    int                                    len,
    int                                    tableid);
int atsc_stt_callback(struct mpegts_table* mt,
    const uint8_t*                         ptr,
    int                                    len,
    int                                    tableid);
...
mpegts_service_t* mpegts_service_find(mpegts_mux_t* mm,
    uint16_t                                        sid,
    uint16_t                                        pmt_pid,
    int                                             create,
    int*                                            save);

service_t* mpegts_service_find_e2(uint32_t stype,
    uint32_t                               sid,
    uint32_t                               tsid,
    uint32_t                               onid,
    uint32_t                               hash);
