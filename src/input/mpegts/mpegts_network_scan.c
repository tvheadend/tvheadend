...
void mpegts_network_scan_timer_cb(void* p) {
   ...
   */
  mtimer_arm_rel(&mn->mn_scan_timer,
...
static void mpegts_network_scan_mux_add(mpegts_network_t* mn,
    mpegts_mux_t*                                         mm) {
  TAILQ_INSERT_SORTED_R(&mn->mn_scan_ipend,
      mpegts_mux_queue,
      mm,
      mm_scan_link,
      mm_cmp);
...
static void mpegts_network_scan_idle_mux_add(mpegts_network_t* mn,
    mpegts_mux_t*                                              mm) {
...
/* Finished */
static inline void mpegts_network_scan_mux_done0(mpegts_mux_t* mm,
    mpegts_mux_scan_result_t                                   result,
    int                                                        weight) {
  mpegts_network_t*       mn    = mm->mm_network;
...
void mpegts_network_scan_mux_cancel(mpegts_mux_t* mm, int reinsert) {
  ...
  mpegts_network_scan_mux_done0(mm,
      MM_SCAN_NONE,
...
void mpegts_network_scan_queue_add(mpegts_mux_t* mm,
    int                                          weight,
    int                                          flags,
    int                                          delay) {
  int               requeue = 0;
  ...
  if (weight == SUBSCRIPTION_PRIO_SCAN_IDLE) {
    ...
    TAILQ_INSERT_SORTED_R(&mn->mn_scan_pend,
        mpegts_mux_queue,
        mm,
        mm_scan_link,
        mm_cmp);
...
void mpegts_mux_bouquet_rescan(const char* src, const char* extra) {
  ...
  if ((l = startswith(src, "dvb-fastscan://")) > 0) {
    ...
    if ((l = startswith(src, "DVBS2,")) > 0 ||
        (l = startswith(src, "DVBS-2,")) > 0)
      dvbs2 = 1;
    else if ((l = startswith(src, "DVBS,")) > 0 ||
        (l = startswith(src, "DVB-S,")) > 0)
      dvbs2 = 0;
    else
      return;
    src += l;
    ...
    LIST_FOREACH (mn, &mpegts_network_all, mn_global_link)
      LIST_FOREACH (mm, &mn->mn_muxes, mm_network_link)
        if (idnode_is_instance(&mm->mm_id, &dvb_mux_dvbs_class) &&
            deltaU32(((dvb_mux_t*)mm)->lm_tuning.dmc_fe_freq, freq) < 2000 &&
            ((dvb_mux_t*)mm)->lm_tuning.u.dmc_fe_qpsk.polarisation ==
                dvb_str2pol(pol) &&
            ((dvb_mux_t*)mm)->lm_tuning.u.dmc_fe_qpsk.orbital_pos == satpos) {
          tvhinfo(LS_MPEGTS,
              "fastscan mux found '%s', set scan state 'PENDING'",
              mm->mm_nicename);
          mpegts_mux_scan_state_set(mm, MM_SCAN_STATE_PEND);
          return;
        }
