...
/* Notify */
static void mpegts_network_scan_notify(mpegts_mux_t* mm) {
 ...
 */
static int mm_cmp(mpegts_mux_t* a, mpegts_mux_t* b) {
 ...
 *   3) do standard mux sorting
 */
static int mm_cmp_idle(mpegts_mux_t* a, mpegts_mux_t* b) {
  int r = b->mm_scan_weight - a->mm_scan_weight;
  if (r == 0) {
...
static void mpegts_network_scan_queue_del0(mpegts_mux_t* mm) {
...
static int mpegts_network_scan_do_mux(mpegts_mux_queue_t* q, mpegts_mux_t* mm) {
  int r, state = mm->mm_scan_state;

  assert(
      state == MM_SCAN_STATE_PEND || state == MM_SCAN_STATE_IPEND || state == MM_SCAN_STATE_ACTIVE);
  ...
  r = mpegts_mux_subscribe(mm,
      ...
      "scan",
      mm->mm_scan_weight,
      mm->mm_scan_flags | SUBSCRIPTION_ONESHOT | SUBSCRIPTION_TABLES);
  ...
  assert(state == MM_SCAN_STATE_PEND || state == MM_SCAN_STATE_IPEND);
...
static void mpegts_network_scan_mux_add(mpegts_network_t* mn, mpegts_mux_t* mm) {
  TAILQ_INSERT_SORTED_R(&mn->mn_scan_ipend, mpegts_mux_queue, mm, mm_scan_link, mm_cmp);
  mtimer_arm_rel(&mn->mn_scan_timer, mpegts_network_scan_timer_cb, mn, sec2mono(10));
}

static void mpegts_network_scan_idle_mux_add(mpegts_network_t* mn, mpegts_mux_t* mm) {
  mm->mm_scan_weight = SUBSCRIPTION_PRIO_SCAN_IDLE;
  TAILQ_INSERT_SORTED_R(&mn->mn_scan_ipend, mpegts_mux_queue, mm, mm_scan_link, mm_cmp_idle);
  mtimer_arm_rel(&mn->mn_scan_timer, mpegts_network_scan_timer_cb, mn, sec2mono(10));
}

/* Finished */
static inline void
mpegts_network_scan_mux_done0(mpegts_mux_t* mm, mpegts_mux_scan_result_t result, int weight) {
  mpegts_network_t*       mn    = mm->mm_network;
...
void mpegts_network_scan_mux_cancel(mpegts_mux_t* mm, int reinsert) {
  ...
  mpegts_network_scan_mux_done0(mm, MM_SCAN_NONE, reinsert ? mm->mm_scan_weight : 0);
}
...
void mpegts_network_scan_queue_add(mpegts_mux_t* mm, int weight, int flags, int delay) {
  ...
  tvhdebug(LS_MPEGTS,
      "adding mux %s to scan queue weight %d flags %04X",
      mm->mm_nicename,
      weight,
      flags);
  ...
  if (weight == SUBSCRIPTION_PRIO_SCAN_IDLE) {
    ...
    TAILQ_INSERT_SORTED_R(&mn->mn_scan_pend, mpegts_mux_queue, mm, mm_scan_link, mm_cmp);
  }
...
static ssize_t startswith(const char* str, const char* start) {
...
void mpegts_mux_bouquet_rescan(const char* src, const char* extra) {
  ...
  if ((l = startswith(src, "dvb-bouquet://dvbs,")) > 0) {
    ...
    LIST_FOREACH (mn, &mpegts_network_all, mn_global_link)
      LIST_FOREACH (mm, &mn->mn_muxes, mm_network_link)
        if (idnode_is_instance(&mm->mm_id, &dvb_mux_dvbs_class) && mm->mm_tsid == tsid &&
  ...
  tsid_lookup:
    LIST_FOREACH (mn, &mpegts_network_all, mn_global_link)
      LIST_FOREACH (mm, &mn->mn_muxes, mm_network_link)
        if (idnode_is_instance(&mm->mm_id, ic) && mm->mm_tsid == tsid)
          mpegts_mux_scan_state_set(mm, MM_SCAN_STATE_PEND);
    return;
  }
  ...
  if ((l = startswith(src, "dvb-fastscan://")) > 0) {
    ...
    tvhinfo(LS_MPEGTS,
        "fastscan mux not found, position:%i, frequency:%i, polarisation:%c",
        satpos,
        freq,
        pol[0]);
