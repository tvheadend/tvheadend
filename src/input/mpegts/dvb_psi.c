...
static inline uint16_t extract_2byte(const uint8_t* ptr) {
  return (((uint16_t)ptr[0]) << 8) | ptr[1];
}

static inline uint32_t extract_4byte(const uint8_t* ptr) {
  return (((uint32_t)ptr[0]) << 24) | (((uint32_t)ptr[1]) << 16) | (((uint32_t)ptr[2]) << 8) |
      ptr[3];
}

static inline uint16_t extract_tsid(const uint8_t* ptr) {
  return (ptr[0] << 8) | ptr[1];
}

static inline uint16_t extract_onid(const uint8_t* ptr) {
  return (ptr[0] << 8) | ptr[1];
}

static inline uint16_t extract_pid(const uint8_t* ptr) {
  return ((ptr[0] & 0x1f) << 8) | ptr[1];
}

static inline uint16_t extract_svcid(const uint8_t* ptr) {
  return (ptr[0] << 8) | ptr[1];
}

static inline int mpegts_mux_alive(mpegts_mux_t* mm) {
  /*
...
static uint32_t dvb_priv_lookup(mpegts_table_t* mt, const uint8_t* lptr, int llen) {
  uint32_t       priv = 0;
  uint8_t        dtag;
  int            dllen, dlen;
...
static int mpegts_mux_tsid_check(mpegts_mux_t* mm, mpegts_table_t* mt, uint16_t tsid) {
  if (tsid == 0 && !mm->mm_tsid_accept_zero_value) {
    if (tvhlog_limit(&mm->mm_tsid_loglimit, 2)) {
      tvhwarn(mt->mt_subsys,
          "%s: %s: TSID zero value detected, ignoring",
          mt->mt_name,
          mm->mm_nicename);
    }
    return 1;
  ...
  if (mm->mm_tsid != MPEGTS_TSID_NONE) {
    if (mm->mm_tsid && mm->mm_tsid != tsid) {
      if (++mm->mm_tsid_checks > 12) {
        tvhwarn(mt->mt_subsys,
            "%s: %s: TSID change detected - old %04x (%d), new %04x (%d)",
            mt->mt_name,
            mm->mm_nicename,
            mm->mm_tsid,
            mm->mm_tsid,
        ...
        if (tvhtrace_enabled()) {
          tvhtrace(mt->mt_subsys,
              "%s: %s: ignore TSID - old %04x (%d), new %04x (%d) (checks %d)",
              mt->mt_name,
              mm->mm_nicename,
              mm->mm_tsid,
              mm->mm_tsid,
...
static void dvb_bouquet_comment(bouquet_t* bq, mpegts_mux_t* mm) {
...
static mpegts_mux_t* dvb_fs_mux_find(mpegts_mux_t* mm, uint16_t onid, uint16_t tsid) {
  mpegts_mux_t* mux;
  char*         s;
  int           i;
...
static void dvb_fs_mux_add(mpegts_table_t* mt, mpegts_mux_t* mm, mpegts_mux_t* mux) {
  const char* uuid;
 ...
 */
static const dvb_fe_code_rate_t fec_tab[16] = {DVB_FEC_AUTO,
    DVB_FEC_1_2,
    DVB_FEC_2_3,
    DVB_FEC_3_4,
    DVB_FEC_5_6,
 ...
 */
static mpegts_mux_t* dvb_desc_sat_del(mpegts_table_t* mt,
    mpegts_mux_t*                                     mm,
    uint16_t                                          onid,
    uint16_t                                          tsid,
    const uint8_t*                                    ptr,
  ...
  dvb_mux_conf_init((mpegts_network_t*)mm->mm_network,
      &dmc,
  ...
  static int mtab[4]    = {DVB_MOD_NONE, DVB_MOD_QPSK, DVB_MOD_PSK_8, DVB_MOD_QAM_16};
  static int rtab[4]    = {DVB_ROLLOFF_35, DVB_ROLLOFF_25, DVB_ROLLOFF_20, DVB_ROLLOFF_AUTO};
  ...
  if (dmc.dmc_fe_delsys == DVB_SYS_DVBS && dmc.dmc_fe_rolloff != DVB_ROLLOFF_35) {
 ...
 */
static mpegts_mux_t* dvb_desc_cable_del(mpegts_table_t* mt,
    mpegts_mux_t*                                       mm,
    uint16_t                                            onid,
    uint16_t                                            tsid,
    const uint8_t*                                      ptr,
  ...
  static const dvb_fe_modulation_t qtab[16] = {DVB_MOD_QAM_AUTO,
      DVB_MOD_QAM_16,
      DVB_MOD_QAM_32,
      DVB_MOD_QAM_64,
      DVB_MOD_QAM_128,
 ...
 */
static mpegts_mux_t* dvb_desc_terr_del(mpegts_table_t* mt,
    mpegts_mux_t*                                      mm,
    uint16_t                                           onid,
    uint16_t                                           tsid,
    const uint8_t*                                     ptr,
    int                                                len) {
  static const dvb_fe_bandwidth_t      btab[8] = {DVB_BANDWIDTH_8_MHZ,
           DVB_BANDWIDTH_7_MHZ,
           DVB_BANDWIDTH_6_MHZ,
           DVB_BANDWIDTH_AUTO,
           DVB_BANDWIDTH_AUTO,
           ...
           DVB_BANDWIDTH_AUTO,
           DVB_BANDWIDTH_AUTO};
  static const dvb_fe_modulation_t     ctab[4] = {DVB_MOD_QPSK,
          DVB_MOD_QAM_16,
          DVB_MOD_QAM_64,
          DVB_MOD_QAM_AUTO};
  static const dvb_fe_guard_interval_t gtab[4] = {DVB_GUARD_INTERVAL_1_32,
      DVB_GUARD_INTERVAL_1_16,
      DVB_GUARD_INTERVAL_1_8,
      DVB_GUARD_INTERVAL_1_4};
  static const dvb_fe_transmit_mode_t  ttab[4] = {DVB_TRANSMISSION_MODE_2K,
  ...
  if (!idnode_is_instance(&mm->mm_id, &dvb_mux_dvbt_class))
    return NULL;
 ...
 */
static void dvb_desc_freq_list(mpegts_table_t* mt, mpegts_mux_t* mm, const uint8_t* ptr, int len) {
...
static int dvb_desc_service(const uint8_t* ptr,
    int                                    len,
    int*                                   stype,
    char*                                  sprov,
    size_t                                 sprov_len,
...
dvb_bat_find_service(dvb_bat_id_t* bi, mpegts_service_t* s, uint8_t lcn_dtag, uint32_t lcn) {
  dvb_bat_svc_t* bs;
...
static int dvb_desc_service_list(mpegts_table_t* mt,
    const uint8_t*                               ptr,
    int                                          len,
    mpegts_mux_t*                                mm,
    dvb_bat_id_t*                                bi) {
...
static int dvb_desc_local_channel(mpegts_table_t* mt,
    mpegts_network_t*                             mn,
    const uint8_t*                                ptr,
    int                                           len,
    uint8_t                                       dtag,
  ...
  while (len >= 4) {
    ...
    if (sid && lcn && mm) {
      ...
      if (s) {
        ...
        } else if ((!s->s_dvb_channel_dtag || s->s_dvb_channel_dtag == dtag || prefer) &&
...
static void
dvb_freesat_local_channels(dvb_bat_id_t* bi, mpegts_table_t* mt, const uint8_t* ptr, int len) {
...
static void dvb_freesat_regions(dvb_bat_id_t* bi, mpegts_table_t* mt, const uint8_t* ptr, int len) {
  uint16_t              id;
  char                  name[32];
...
static int dvb_freesat_add_service(dvb_bat_id_t* bi,
    dvb_freesat_region_t*                        fr,
    mpegts_service_t*                            s,
    uint32_t                                     lcn) {
...
static void dvb_freesat_completed(dvb_bat_t* b, dvb_bat_id_t* bi, mpegts_table_t* mt) {
  dvb_bat_svc_t*        bs;
  dvb_freesat_svc_t*    fs;
  dvb_freesat_region_t* fr;
  uint16_t              sid;
  ...
  TAILQ_FOREACH (bs, &bi->services, link) {
    ...
    TAILQ_FOREACH (fs, &bi->fservices, link)
      if (fs->sid == sid) {
        ...
        if (!fr) {
          tvhtrace(mt->mt_subsys,
              "%s: cannot find freesat region id %u",
              mt->mt_name,
              fs->regionid);
          continue;
        }
  ...
  LIST_FOREACH (fr, &bi->fregions, link) {
    regions++;
    if (TAILQ_EMPTY(&fr->services))
      continue;
    uregions++;
    TAILQ_FOREACH (fs, &fr->services, region_link) {
      if (!dvb_freesat_add_service(bi, fr, fs->svc, fs->lcn))
        break;
      TAILQ_FOREACH (bs, &bi->services, link)
    ...
    TAILQ_FOREACH (bs, &bi->services, link) {
      ...
      if ((fs = bs->fallback) != NULL) {
        if (!dvb_freesat_add_service(bi, fr, bs->svc, fs->lcn))
          break;
      } else {
        if (!dvb_freesat_add_service(bi, fr, bs->svc, 0))
          break;
      }
    }
  }
  ...
  tvhtrace(mt->mt_subsys,
      "%s: completed %s [%04X] bouquets '%s' total %u regions %u (%u)",
      mt->mt_name,
      bi->freesat ? "freesat" : "bskyb",
      bi->nbid,
      bi->name,
  ...
  tvhtrace(mt->mt_subsys,
      "%s: completed %s [%04X] bouquets '%s' update finished",
      mt->mt_name,
      bi->freesat ? "freesat" : "bskyb",
      bi->nbid,
      bi->name);
...
static void dvb_bskyb_local_channels(dvb_bat_id_t* bi,
    mpegts_table_t*                                mt,
    const uint8_t*                                 ptr,
    int                                            len,
    mpegts_mux_t*                                  mm) {
  ...
  tvhtrace(mt->mt_subsys,
      "%s:      region id %04X (%d) unknown %02X (%d)",
      mt->mt_name,
      regionid,
      regionid,
      ptr[0],
  ...
  while (len > 8) {
    ...
    tvhtrace(mt->mt_subsys,
        "%s:      sid %04X (%d) type %02X (%d) lcn %d unknown %04X (%d)",
        mt->mt_name,
        sid,
        sid,
        stype,
...
int dvb_pat_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  ...
  r    = dvb_table_begin((mpegts_psi_table_t*)mt,
      ptr,
      len,
      tableid,
      tsid,
 ...
 * CAT processing
 */
void dvb_cat_decode(const uint8_t* data,
    int                            len,
...
static void dvb_cat_entry(void* _mt, uint16_t caid, uint32_t prov, uint16_t pid) {
  mpegts_table_t* mt = _mt;
  tvhdebug(mt->mt_subsys,
      "%s:  caid %04X (%d) pid %04X (%d)",
      mt->mt_name,
      (uint16_t)caid,
      (uint16_t)caid,
      pid,
...
int dvb_cat_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  int                       r, sect, last, ver;
  mpegts_mux_t*             mm = mt->mt_mux;
  mpegts_psi_table_state_t* st = NULL;

  /* Start */
  r = dvb_table_begin((mpegts_psi_table_t*)mt, ptr, len, tableid, 0, 5, &st, &sect, &last, &ver, 0);
  if (r != 1)
    return r;
  ptr += 5;
  len -= 5;
...
static void dvb_bat_destroy_lists(mpegts_table_t* mt) {
...
void dvb_bat_destroy(mpegts_table_t* mt) {
  dvb_bat_destroy_lists(mt);
  free(mt->mt_bat);
  mt->mt_bat = NULL;
}

static void dvb_bat_completed(dvb_bat_t* b,
    mpegts_table_t*                      mt,
    int                                  tableid,
    int                                  tsid,
    int                                  nbid,
  ...
  LIST_FOREACH (bi, &b->bats, link) {
    ...
    TAILQ_FOREACH (bs, &bi->services, link) {
      services_count++;
      if (bq->bq_enabled)
        bouquet_add_service(bq, (service_t*)bs->svc, (int64_t)bs->lcn * CHANNEL_SPLIT, NULL);
    }
...
static int dvb_nit_mux(mpegts_table_t* mt,
    mpegts_mux_t*                      mux,
    mpegts_mux_t*                      mm,
    uint16_t                           onid,
    uint16_t                           tsid,
  ...
  if (!discovery || tvhtrace_enabled()) {
    tvhlog(discovery ? LOG_TRACE : LOG_DEBUG,
        mt->mt_subsys,
        "%s:  onid %04X (%d) tsid %04X (%d) mux %s%s",
        mt->mt_name,
        onid,
        onid,
        tsid,
  ...
  DVB_DESC_FOREACH(mt, lptr, llen, 4, dlptr, dllen, dtag, dlen, dptr) {
    ...
    if (discovery > 0 && dtag != DVB_DESC_SAT_DEL && dtag != DVB_DESC_CABLE_DEL &&
        dtag != DVB_DESC_TERR_DEL && dtag != DVB_DESC_FREQ_LIST)
      continue;
#endif
    ...
    switch (dtag) {
      ...
      case DVB_DESC_TERR_DEL:
        if (discovery > 0) {
          if (dtag == DVB_DESC_SAT_DEL)
            mux =
                dvb_desc_sat_del(mt, mm, onid, tsid, dptr, dlen, tableid == DVB_FASTSCAN_NIT_BASE);
      ...
      case 0x82:
        ...
        break;
      case 0x83:
        if (priv == 0 || priv == 0x28 || priv == 0x29 || priv == 0xa5 || priv == 0x212c ||
            priv == 0x233A || priv == 0x3200 || priv == 0x3201)
          goto lcn;
        break;
      case 0x86:
        if (priv == 0 || priv == 0x1a0)
          goto lcn;
        break;
      case 0x88:
        if (priv == 0x28) {
...
dvberr: tvhtrace(mt->mt_subsys, "%s:  error", mt->mt_name);
return -1;
}

int dvb_nit_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  int                       save = 0, retry = 0;
  int                       r, sect, last, ver;
  ...
  if (r == 0) {
    if (tableid == 0x4A || tableid == DVB_FASTSCAN_NIT_BASE) {
      ...
      if ((b = mt->mt_bat) != NULL) {
        if (!b->complete) {
          dvb_bat_completed(b, mt, tableid, mm->mm_tsid, nbid, mm, mt->mt_opaque);
  ...
  /* BAT ID lookup */
  if (dvb_bouquets_parse && (tableid == 0x4A || tableid == DVB_FASTSCAN_NIT_BASE)) {
    if ((b = mt->mt_bat) == NULL) {
      b          = calloc(1, sizeof(*b));
      mt->mt_bat = b;
    }
      ...
      case DVB_DESC_NETWORK_NAME:
        if (dvb_get_string(name, sizeof(name), dptr, dlen, charset, NULL))
          tvhtrace(mt->mt_subsys,
              "%s:    %s name error",
              mt->mt_name,
              dtag == DVB_DESC_BOUQUET_NAME ? "bouquet" : "network");
        break;
...
DVB_LOOP_FOREACH(mt, ptr, len, 0, lptr, llen, 6) {
  ...
  if (tableid == DVB_FASTSCAN_NIT_BASE) {
    ...
    LIST_FOREACH (mux, &mn->mn_muxes, mm_network_link) {
      if (mux->mm_onid == onid && mux->mm_tsid == tsid && (mm == mux || mpegts_mux_alive(mux))) {
        r = dvb_nit_mux(mt, mux, mm, onid, tsid, lptr, llen, tableid, bi, 0);
        if (r < 0)
          return r;
        if (priv == 0 && priv2)
 ...
 */
static int dvb_sdt_mux(mpegts_table_t* mt,
    mpegts_mux_t*                      mm,
    mpegts_mux_t*                      mm_orig,
    const uint8_t*                     ptr,
    int                                len,
    uint8_t                            tableid) {
  uint32_t          priv = 0;
  ...
  while (len >= 5) {
    ...
    *sprov = *sname = *sauth = 0;
    tvhdebug(mt->mt_subsys,
        "%s:  sid %04X (%d) running %d free_ca %d",
        mt->mt_name,
        service_id,
        service_id,
        running_status,
  ...
  tvhtrace(mt->mt_subsys,
      "%s:  type %02X (%d) name [%s] provider [%s] def_auth [%s]",
      mt->mt_name,
      stype,
      stype,
      sname,
...
int dvb_sdt_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  ...
  /* ID */
  tvhdebug(mt->mt_subsys, "%s: onid %04X (%d) tsid %04X (%d)", mt->mt_name, onid, onid, tsid, tsid);
  ...
  if (tableid == 0x42) {
    if (mm->mm_tsid != tsid)
      return 0; /* keep rolling - perhaps PAT was not parsed yet */
    ...
    LIST_FOREACH (mm, &mn->mn_muxes, mm_network_link)
      if (mm->mm_onid == onid && mm->mm_tsid == tsid && (mm == mm_orig || mpegts_mux_alive(mm))) {
 ...
 */
/* Done */
int atsc_vct_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
        ...
        for (j = 0; j < dlen;) {
          ...
          if (tag == ATSC_DESC_EXT_CHANNEL_NAME) {
            ...
            if (ls) {
              ...
              tvhdebug(mt->mt_subsys,
                  "%s:  extended channel name: '%s' (%d bytes)",
                  mt->mt_name,
                  x,
                  len);
              lang_str_destroy(ls);
 ...
 * ATSC STT processing
 */
int atsc_stt_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  int                       r, sect, last, ver, extraid;
  ...
  tvhdebug(mt->mt_subsys,
      "%s: system_time %d, gps_utc_offset %d, is DST %d",
      mt->mt_name,
      systemtime,
      gps_utc_offset,
      is_dst);
 ...
 * DVB BAT processing
 */
int dvb_bat_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  return dvb_nit_callback(mt, ptr, len, tableid);
}

#if ENABLE_MPEGTS_DVB
static int dvb_fs_sdt_mux(mpegts_table_t* mt,
    mpegts_mux_t*                         mm,
    ...
    if (discovery) {
      ...
      DVB_DESC_EACH(mt, lptr, llen, dtag, dlen, dptr) {
        switch (dtag) {
          case DVB_DESC_SAT_DEL:
            tvhtrace(mt->mt_subsys,
                "%s:    dtag %02X dlen %d (discovery) onid %04X (%d) tsid %04X (%d)",
                mt->mt_name,
                dtag,
                dlen,
                onid,
  ...
  tvhdebug(mt->mt_subsys,
      "%s:  service %04X (%d) onid %04X (%d) tsid %04X (%d)",
      mt->mt_name,
      service_id,
      service_id,
      onid,
...
tvhtrace(mt->mt_subsys, "%s:    type %d name [%s] provider [%s]", mt->mt_name, stype, sname, sprov);
 ...
 */
int dvb_fs_sdt_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  int                       r, sect, last, ver;
  uint16_t                  nbid;
  mpegts_mux_t*             mm = mt->mt_mux;
  ...
  dvb_fs_sdt_mux(mt, mm, st, ptr, len, 0, bq && bq->bq_fastscan_nit ? bq->bq_fastscan_bi : NULL);
...
int dvb_tdt_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  if (len == 5) {
 ...
 */
int dvb_tot_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  if (len >= 5) {
...
static void psi_tables_default(mpegts_mux_t* mm) {
  mpegts_table_add(mm,
      DVB_PAT_BASE,
      DVB_PAT_MASK,
      dvb_pat_callback,
...
#if ENABLE_MPEGTS_DVB
static void psi_tables_dvb_fastscan(void* aux, bouquet_t* bq, const char* name, int pid) {
  tvhtrace(LS_FASTSCAN, "adding table %04X (%i) for '%s'", pid, pid, name);
  bq->bq_fastscan_nit = 1;
  bq->bq_fastscan_sdt = 1;
  mpegts_table_add(aux,
      DVB_FASTSCAN_NIT_BASE,
      DVB_FASTSCAN_MASK,
      dvb_nit_callback,
      bq,
      "fs_nit",
...
static void psi_tables_dvb(mpegts_mux_t* mm) {
  mpegts_table_add(mm,
      DVB_NIT_BASE,
      DVB_NIT_MASK,
      dvb_nit_callback,
  ...
  if (config.tvhtime_update_enabled) {
    mpegts_table_add(mm,
        ...
        "tdt",
        LS_TBL_TIME,
        MT_ONESHOT | MT_QUICKREQ | MT_RECORD,
        DVB_TDT_PID,
        MPS_WEIGHT_TDT);
    mpegts_table_add(mm,
        DVB_TOT_BASE,
        ...
        "tot",
        LS_TBL_TIME,
        MT_ONESHOT | MT_QUICKREQ | MT_CRC | MT_RECORD,
        DVB_TDT_PID,
        MPS_WEIGHT_TDT);
  }
#if ENABLE_MPEGTS_DVB
  if (idnode_is_instance(&mm->mm_id, &dvb_mux_dvbs_class)) {
    ...
    if (mc->dmc_fe_type == DVB_TYPE_S)
      dvb_fastscan_each(mm,
          mc->u.dmc_fe_qpsk.orbital_pos,
          mc->dmc_fe_freq,
          mc->u.dmc_fe_qpsk.polarisation,
          psi_tables_dvb_fastscan);
  }
#endif
}

static void psi_tables_atsc_c(mpegts_mux_t* mm) {
  mpegts_table_add(mm,
      DVB_VCT_C_BASE,
      DVB_VCT_MASK,
      atsc_vct_callback,
...
static void psi_tables_atsc_t(mpegts_mux_t* mm) {
  mpegts_table_add(mm,
      DVB_VCT_T_BASE,
      DVB_VCT_MASK,
      atsc_vct_callback,
...
void psi_tables_install(mpegts_input_t* mi, mpegts_mux_t* mm, dvb_fe_delivery_system_t delsys) {
  if (mi == NULL || mm == NULL)
    return;
