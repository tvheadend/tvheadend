...
static mpegts_mux_t* dvb_desc_sat_del(mpegts_table_t* mt,
  ...
  static int mtab[4]    = {DVB_MOD_NONE,
      DVB_MOD_QPSK,
      DVB_MOD_PSK_8,
      DVB_MOD_QAM_16};
  static int rtab[4]    = {DVB_ROLLOFF_35,
      DVB_ROLLOFF_25,
      DVB_ROLLOFF_20,
      DVB_ROLLOFF_AUTO};
  ...
  /* Create */
  return mm->mm_network
      ->mn_create_mux(mm->mm_network, mm, onid, tsid, &dmc, force);
}
 ...
 */
static void dvb_desc_freq_list(mpegts_table_t* mt,
    mpegts_mux_t*                              mm,
    const uint8_t*                             ptr,
    int                                        len) {
...
static int dvb_desc_service(const uint8_t* ptr,
  ...
  *stype = *ptr++;
  ...
  /* Provider */
  if ((r = dvb_get_string_with_len(sprov, sprov_len, ptr, len, charset, NULL)) <
      0)
    return -1;

  /* Name */
  if (dvb_get_string_with_len(sname,
          sname_len,
          ptr + r,
          len - r,
          charset,
...
static dvb_bat_svc_t* dvb_bat_find_service(dvb_bat_id_t* bi,
    mpegts_service_t*                                    s,
    uint8_t                                              lcn_dtag,
    uint32_t                                             lcn) {
  dvb_bat_svc_t* bs;
  ...
  while (len >= 4) {
    ...
    if (sid && lcn && mm) {
      ...
      if (s) {
        ...
        } else if ((!s->s_dvb_channel_dtag || s->s_dvb_channel_dtag == dtag ||
                       prefer) &&
...
static void dvb_freesat_local_channels(dvb_bat_id_t* bi,
    mpegts_table_t*                                  mt,
    const uint8_t*                                   ptr,
    int                                              len) {
...
static void dvb_freesat_regions(dvb_bat_id_t* bi,
    mpegts_table_t*                           mt,
    const uint8_t*                            ptr,
    int                                       len) {
  uint16_t              id;
  ...
  while (len > 5) {
    ...
    if ((r = dvb_get_string_with_len(name,
             sizeof(name),
             ptr + 5,
             len - 5,
             NULL,
...
static int dvb_freesat_add_service(dvb_bat_id_t* bi,
  ...
  if (!fr->bouquet) {
    ...
    if (idnode_is_instance(&bi->mm->mm_id, &dvb_mux_dvbs_class))
      dvb_sat_position_to_str(
          ((dvb_mux_t*)bi->mm)->lm_tuning.u.dmc_fe_qpsk.orbital_pos,
          name,
          sizeof(name));
    snprintf(src,
        sizeof(src),
  ...
  bouquet_add_service(fr->bouquet,
      (service_t*)s,
      (int64_t)lcn * CHANNEL_SPLIT,
      NULL);
  return fr->bouquet->bq_enabled;
}

static void
dvb_freesat_completed(dvb_bat_t* b, dvb_bat_id_t* bi, mpegts_table_t* mt) {
  dvb_bat_svc_t*        bs;
  dvb_freesat_svc_t*    fs;
  dvb_freesat_region_t* fr;
  uint16_t              sid;
 ...
 * PAT processing
 */
int dvb_pat_callback(mpegts_table_t* mt,
    const uint8_t*                   ptr,
    int                              len,
    int                              tableid) {
  int                       r, sect, last, ver;
...
int dvb_cat_callback(mpegts_table_t* mt,
    const uint8_t*                   ptr,
    int                              len,
    int                              tableid) {
  int                       r, sect, last, ver;
...
static void dvb_bat_completed(dvb_bat_t* b,
  ...
  LIST_FOREACH (bi, &b->bats, link) {
    ...
    if (tableid == 0x4A /* BAT */) {
      ...
      if (idnode_is_instance(&mux->mm_id, &dvb_mux_dvbs_class)) {
        ...
        if (mc->u.dmc_fe_qpsk.orbital_pos != INT_MAX) {
          char buf[16];
          dvb_sat_position_to_str(mc->u.dmc_fe_qpsk.orbital_pos,
              buf,
              sizeof(buf));
          snprintf(src,
              sizeof(src),
              "dvb-bouquet://dvbs,%s,%04X,%04X",
              buf,
              tsid,
              bi->nbid);
        }
    ...
    TAILQ_FOREACH (bs, &bi->services, link) {
      services_count++;
      if (bq->bq_enabled)
        bouquet_add_service(bq,
            (service_t*)bs->svc,
            (int64_t)bs->lcn * CHANNEL_SPLIT,
            NULL);
    }
...
static int dvb_nit_mux(mpegts_table_t* mt,
  ...
  DVB_DESC_FOREACH(mt, lptr, llen, 4, dlptr, dllen, dtag, dlen, dptr) {
    ...
    if (discovery > 0 && dtag != DVB_DESC_SAT_DEL &&
        dtag != DVB_DESC_CABLE_DEL && dtag != DVB_DESC_TERR_DEL &&
    ...
    switch (dtag) {
      ...
      case 0x83:
        if (priv == 0 || priv == 0x28 || priv == 0x29 || priv == 0xa5 ||
            priv == 0x212c || priv == 0x233A || priv == 0x3200 ||
            priv == 0x3201)
          goto lcn;
        break;
      case 0x86:
...
int dvb_nit_callback(mpegts_table_t* mt,
    const uint8_t*                   ptr,
    int                              len,
    int                              tableid) {
  int                       save = 0, retry = 0;
...
int dvb_sdt_callback(mpegts_table_t* mt,
    const uint8_t*                   ptr,
    int                              len,
    int                              tableid) {
 ...
 */
/* Done */
int atsc_vct_callback(mpegts_table_t* mt,
    const uint8_t*                    ptr,
    int                               len,
    int                               tableid) {
 ...
 * ATSC STT processing
 */
int atsc_stt_callback(mpegts_table_t* mt,
    const uint8_t*                    ptr,
    int                               len,
    int                               tableid) {
  int                       r, sect, last, ver, extraid;
 ...
 * DVB BAT processing
 */
int dvb_bat_callback(mpegts_table_t* mt,
    const uint8_t*                   ptr,
    int                              len,
    int                              tableid) {
    ...
    if (discovery) {
      ...
      DVB_DESC_EACH(mt, lptr, llen, dtag, dlen, dptr) {
        switch (dtag) {
          case DVB_DESC_SAT_DEL:
            tvhtrace(mt->mt_subsys,
                "%s:    dtag %02X dlen %d (discovery) onid %04X (%d) tsid %04X "
                "(%d)",
                mt->mt_name,
                dtag,
                dlen,
 ...
 */
int dvb_fs_sdt_callback(mpegts_table_t* mt,
    const uint8_t*                      ptr,
    int                                 len,
    int                                 tableid) {
  int                       r, sect, last, ver;
...
int dvb_tdt_callback(mpegts_table_t* mt,
    const uint8_t*                   ptr,
    int                              len,
    int                              tableid) {
  if (len == 5) {
 ...
 */
int dvb_tot_callback(mpegts_table_t* mt,
    const uint8_t*                   ptr,
    int                              len,
    int                              tableid) {
  if (len >= 5) {
...
void psi_tables_install(mpegts_input_t* mi,
    mpegts_mux_t*                       mm,
