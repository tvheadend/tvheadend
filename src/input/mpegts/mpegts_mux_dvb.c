...
static void dvb_mux_delete(mpegts_mux_t* mm, int delconf);
...
#define dvb_mux_class_R(c, f, l, t, ...)                                       \
  static const void* dvb_mux_##c##_class_##l##_get(void* o) {                  \
    dvb_mux_t* lm = o;                                                         \
    prop_ptr      = dvb_##t##2str(lm->lm_tuning.dmc_fe_##f);                   \
    return &prop_ptr;                                                          \
  }                                                                            \
  static int dvb_mux_##c##_class_##l##_set(void* o, const void* v) {           \
    dvb_mux_t* lm            = o;                                              \
    lm->lm_tuning.dmc_fe_##f = dvb_str2##t((const char*)v);                    \
    return 1;                                                                  \
  }                                                                            \
  static htsmsg_t* dvb_mux_##c##_class_##l##_enum(void* o, const char* lang) { \
    static const int t[] = {__VA_ARGS__};                                      \
    int              i;                                                        \
    htsmsg_t *       m = htsmsg_create_list(), *e;                             \
    for (i = 0; i < ARRAY_SIZE(t); i++) {                                      \
      e = htsmsg_create_key_val(dvb_##t##2str(t[i]),                           \
          tvh_gettext_lang(lang, dvb_##t##2str(t[i])));                        \
...
#define MUX_PROP_STR(_id, _name, t, l, d)                                     \
  .type = PT_STR, .id = _id, .name = _name,                                   \
  .get = dvb_mux_##t##_class_##l##_get, .set = dvb_mux_##t##_class_##l##_set, \
  .list = dvb_mux_##t##_class_##l##_enum, .def.s = d

static const void* dvb_mux_class_delsys_get(void* o) {
  dvb_mux_t* lm = o;
  prop_ptr      = dvb_delsys2str(lm->lm_tuning.dmc_fe_delsys);
  return &prop_ptr;
}

static int dvb_mux_class_delsys_set(void* o, const void* v) {
  const char* s      = v;
...
const idclass_t dvb_mux_class = {.ic_super = &mpegts_mux_class,
    .ic_class                              = "dvb_mux",
...
static htsmsg_t* dvb_mux_dvbt_class_delsys_enum(void* o, const char* lang) {
...
static int dvb_mux_dvbt_class_frequency_set(void* o, const void* v) {
  dvb_mux_t* lm  = o;
...
const idclass_t dvb_mux_dvbt_class = {.ic_super = &dvb_mux_class,
    ...
    .ic_properties                              = (const property_t[]){
        ...
        {
            MUX_PROP_STR("transmission_mode",
                N_("Transmission mode"),
                dvbt,
                mode,
                N_("AUTO")),
...
static htsmsg_t* dvb_mux_dvbc_class_delsys_enum(void* o, const char* lang) {
  htsmsg_t* list = htsmsg_create_list();
...
const idclass_t dvb_mux_dvbc_class = {.ic_super = &dvb_mux_class,
    ...
    .ic_properties                              = (const property_t[]){
        ...
        },
        {
            MUX_PROP_STR("constellation",
                ...
                N_("AUTO")),
            .desc =
                N_("The quadrature amplitude modulation (QAM) used by the mux. "
                   "If you're not sure of the value leave as AUTO."),
        },
        {
...
dvb_mux_class_X(dvbs,
    ...
    polarisation,
    pol,
    DVB_POLARISATION_VERTICAL,
    DVB_POLARISATION_HORIZONTAL,
    DVB_POLARISATION_CIRCULAR_LEFT,
    DVB_POLARISATION_CIRCULAR_RIGHT);
dvb_mux_class_R(dvbs,
    pls_mode,
    pls_mode,
    plsmode,
...
static int dvb_mux_dvbs_class_frequency_set(void* o, const void* v) {
  dvb_mux_t* lm  = o;
  uint32_t   val = *(uint32_t*)v;
...
static int dvb_mux_dvbs_class_symbol_rate_set(void* o, const void* v) {
  dvb_mux_t* lm  = o;
  uint32_t   val = *(uint32_t*)v;
...
static htsmsg_t* dvb_mux_dvbs_class_delsys_enum(void* o, const char* lang) {
  htsmsg_t* list = htsmsg_create_list();
...
static const void* dvb_mux_dvbs_class_orbital_get(void* o) {
  static char buf[16], *s = buf;
  dvb_mux_t*  lm = o;
  if (lm->lm_tuning.u.dmc_fe_qpsk.orbital_pos == INT_MAX)
    buf[0] = '\0';
  else
    dvb_sat_position_to_str(lm->lm_tuning.u.dmc_fe_qpsk.orbital_pos,
        buf,
        sizeof(buf));
  return &s;
}

static int dvb_mux_dvbs_class_orbital_set(void* o, const void* s) {
  dvb_mux_t* lm = o;
  int        pos;
...
const idclass_t dvb_mux_dvbs_class = {.ic_super = &dvb_mux_class,
...
static htsmsg_t* dvb_mux_atsc_t_class_delsys_enum(void* o, const char* lang) {
  htsmsg_t* list = htsmsg_create_list();
...
const idclass_t dvb_mux_atsc_t_class = {.ic_super = &dvb_mux_class,
...
static htsmsg_t* dvb_mux_atsc_c_class_delsys_enum(void* o, const char* lang) {
  htsmsg_t* list = htsmsg_create_list();
  htsmsg_add_str(list, NULL, dvb_delsys2str(DVB_SYS_DVBC_ANNEX_B));
  return list;
}

const idclass_t dvb_mux_atsc_c_class = {.ic_super = &dvb_mux_class,
 ...
 * CableCARD
 */
const idclass_t dvb_mux_cablecard_class = {.ic_super = &dvb_mux_class,
    ...
    .ic_properties = (const property_t[]){
        ...
        },
        {
            ...
            .name = N_("Callsign"),
            .desc = N_(
                "The channel's name or callsign as set by the cable provider."),
...
static htsmsg_t* dvb_mux_isdb_t_class_delsys_enum(void* o, const char* lang) {
  htsmsg_t* list = htsmsg_create_list();
  htsmsg_add_str(list, NULL, dvb_delsys2str(DVB_SYS_ISDBT));
  return list;
}

dvb_mux_class_X(isdb_t,
    isdbt,
    bandwidth,
    bw,
    bw,
...
const idclass_t dvb_mux_isdb_t_class = {.ic_super = &dvb_mux_class,
    ...
    .ic_properties                                = (const property_t[]){
        ...
        {
            MUX_PROP_STR("layera_mod",
                N_("Layer A: Constellation"),
                isdb_t,
                isdbt_mod_a,
                N_("AUTO")),
        ...
        },
        {
            ...
            .off  = offsetof(dvb_mux_t,
                lm_tuning.u.dmc_fe_isdbt.layers[0].segment_count),
        },
        {
            .type = PT_U32,
            .id   = "layera_timint",
            ...
            .off  = offsetof(dvb_mux_t,
                lm_tuning.u.dmc_fe_isdbt.layers[0].time_interleaving),
        },
        /* Layer B */
        {
            MUX_PROP_STR("layerb_fec",
                N_("Layer B: FEC"),
                isdb_t,
                isdbt_fec_b,
                N_("AUTO")),
        ...
        {
            MUX_PROP_STR("layerb_mod",
                N_("Layer B: Constellation"),
                isdb_t,
                isdbt_mod_b,
                N_("AUTO")),
        ...
        },
        {
            ...
            .off  = offsetof(dvb_mux_t,
                lm_tuning.u.dmc_fe_isdbt.layers[1].segment_count),
        },
        {
            .type = PT_U32,
            .id   = "layerb_timint",
            ...
            .off  = offsetof(dvb_mux_t,
                lm_tuning.u.dmc_fe_isdbt.layers[1].time_interleaving),
        },
        /* Layer C */
        {
            MUX_PROP_STR("layerc_fec",
                N_("Layer C: FEC"),
                isdb_t,
                isdbt_fec_c,
                N_("AUTO")),
        ...
        {
            MUX_PROP_STR("layerc_mod",
                N_("Layer C: Constellation"),
                isdb_t,
                isdbt_mod_c,
                N_("AUTO")),
        ...
        },
        {
            ...
            .off  = offsetof(dvb_mux_t,
                lm_tuning.u.dmc_fe_isdbt.layers[2].segment_count),
        },
        {
            .type = PT_U32,
            .id   = "layerc_timint",
            ...
            .off  = offsetof(dvb_mux_t,
                lm_tuning.u.dmc_fe_isdbt.layers[2].time_interleaving),
        },
        {}}};
...
const idclass_t dvb_mux_isdb_c_class = {.ic_super = &dvb_mux_class,
...
static htsmsg_t* dvb_mux_isdb_s_class_delsys_enum(void* o, const char* lang) {
  htsmsg_t* list = htsmsg_create_list();
  htsmsg_add_str(list, NULL, dvb_delsys2str(DVB_SYS_ISDBS));
  return list;
}

const idclass_t dvb_mux_isdb_s_class = {.ic_super = &dvb_mux_class,
...
static htsmsg_t* dvb_mux_dtmb_class_delsys_enum(void* o, const char* lang) {
  htsmsg_t* list = htsmsg_create_list();
  htsmsg_add_str(list, NULL, dvb_delsys2str(DVB_SYS_DTMB));
  return list;
}

static int dvb_mux_dtmb_class_frequency_set(void* o, const void* v) {
  dvb_mux_t* lm  = o;
  uint32_t   val = *(uint32_t*)v;
...
const idclass_t dvb_mux_dtmb_class = {.ic_super = &dvb_mux_class,
...
static htsmsg_t* dvb_mux_dab_class_delsys_enum(void* o, const char* lang) {
  htsmsg_t* list = htsmsg_create_list();
  htsmsg_add_str(list, NULL, dvb_delsys2str(DVB_SYS_DAB));
  return list;
}

const idclass_t dvb_mux_dab_class = {.ic_super = &dvb_mux_class,
...
static void dvb_mux_display_name(mpegts_mux_t* mm, char* buf, size_t len) {
...
dvb_mux_t* dvb_mux_create0(dvb_network_t* ln,
    uint32_t                              onid,
    uint32_t                              tsid,
  ...
  if (c == NULL)
    c = c2 = hts_settings_load_r(1,
        "input/dvb/networks/%s/muxes/%s/services",
