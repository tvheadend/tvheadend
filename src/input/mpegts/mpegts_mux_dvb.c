...
#define MUX_PROP_STR(_id, _name, t, l, d)                                         \
  .type = PT_STR, .id = _id, .name = _name, .get = dvb_mux_##t##_class_##l##_get, \
  .set = dvb_mux_##t##_class_##l##_set, .list = dvb_mux_##t##_class_##l##_enum, .def.s = d
...
dvb_mux_class_R(dvbs,
    ...
    DVB_ROLLOFF_25,
    DVB_ROLLOFF_35);
dvb_mux_class_R(dvbs, pilot, pilot, pilot, DVB_PILOT_AUTO, DVB_PILOT_ON, DVB_PILOT_OFF);
dvb_mux_class_X(dvbs,
    qpsk,
    polarisation,
    polarisation,
...
dvb_mux_class_R(dvbs, pls_mode, pls_mode, plsmode, DVB_PLS_ROOT, DVB_PLS_GOLD, DVB_PLS_COMBO);
...
dvb_mux_class_R(atsc_t, modulation, qam, qam, DVB_MOD_QAM_AUTO, DVB_MOD_QAM_256, DVB_MOD_VSB_8);
...
static htsmsg_t* dvb_mux_config_save(mpegts_mux_t* mm, char* filename, size_t fsize) {
...
dvb_mux_t* dvb_mux_create0(dvb_network_t* ln,
  ...
  if (!(mm = mpegts_mux_create0(mm, idc, uuid, (mpegts_network_t*)ln, onid, tsid, conf))) {
    free(mm);
    return NULL;
  }
