...
static void mpegts_mux_sched_set_timer(mpegts_mux_sched_t* mms) {
  /* Update timer */
  if (!mms->mms_enabled) {
    if (mms->mms_sub)
    ...
    if (mms->mms_timeout <= 0)
      ...
      gtimer_arm_rel(&mms->mms_timer, mpegts_mux_sched_timer, mms, mms->mms_timeout);
    }
  } else {
    time_t nxt;
...
static void mpegts_mux_sched_class_changed(idnode_t* in) {
...
static htsmsg_t* mpegts_mux_sched_class_save(idnode_t* in, char* filename, size_t fsize) {
...
static void mpegts_mux_sched_class_delete(idnode_t* in) {
  mpegts_mux_sched_delete((mpegts_mux_sched_t*)in, 1);
}

static htsmsg_t* mpegts_mux_sched_class_mux_list(void* o, const char* lang) {
  htsmsg_t *m, *p;
...
static int mpegts_mux_sched_class_cron_set(void* p, const void* v) {
...
const idclass_t mpegts_mux_sched_class = {.ic_class = "mpegts_mux_sched",
...
static htsmsg_t* mpegts_mux_sched_input_info(void* p, htsmsg_t* list) {
  htsmsg_add_str(list, NULL, "mux sched input");
  return list;
}

static streaming_ops_t mpegts_mux_sched_input_ops = {.st_cb = mpegts_mux_sched_input,
...
static void mpegts_mux_sched_timer(void* p) {
  ...
  if (!mms->mms_active) {
    ...
    mms->mms_sub = subscription_create_from_mux(mms->mms_prch,
        NULL,
        mms->mms_weight,
        mms->mms_creator ?: "",
    ...
    if (mms->mms_restart &&
        (mms->mms_timeout <= 0 || mms->mms_start + mms->mms_timeout < gclk() - 60)) {
...
mpegts_mux_sched_t* mpegts_mux_sched_create(const char* uuid, htsmsg_t* conf) {
  ...
  /* Validate */
  if (!mpegts_mux_find(mms->mms_mux ?: "") || !mms->mms_cronstr) {
    mpegts_mux_sched_delete(mms, 1);
    return NULL;
  }
