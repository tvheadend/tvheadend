...
static int tvhdhomerun_frontend_get_priority(mpegts_input_t* mi, mpegts_mux_t* mm, int flags) {
  return mpegts_input_get_priority(mi, mm, flags);
}

static int tvhdhomerun_frontend_get_grace(mpegts_input_t* mi, mpegts_mux_t* mm) {
  return 15;
}

static int
tvhdhomerun_frontend_is_enabled(mpegts_input_t* mi, mpegts_mux_t* mm, int flags, int weight) {
...
static void* tvhdhomerun_frontend_input_thread(void* aux) {
  ...
  tvhpoll_event(ev + 1, hfe->hf_input_thread_pipe.rd, TVHPOLL_IN, &hfe->hf_input_thread_pipe);
  ...
  while (tvheadend_is_running()) {
    ...
    if ((r = sbuf_read(&sb, sockfd)) < 0) {
      ...
      tvherror(LS_TVHDHOMERUN, "%s - read() error %d (%s)", buf, errno, strerror(errno));
      break;
    }
...
static void tvhdhomerun_frontend_monitor_cb(void* aux) {
  ...
  /* re-arm */
  mtimer_arm_rel(&hfe->hf_monitor_timer, tvhdhomerun_frontend_monitor_cb, hfe, sec2mono(1));

  /* Get current status */
  tvh_mutex_lock(&hfe->hf_hdhomerun_device_mutex);
  res =
      hdhomerun_device_get_tuner_status(hfe->hf_hdhomerun_tuner, &tuner_status_str, &tuner_status);
  tvh_mutex_unlock(&hfe->hf_hdhomerun_device_mutex);
  if (res < 1)
  ...
  if (!hfe->hf_locked) {
    if (tuner_status.signal_present) {
      ...
      psi_tables_install(mmi->mmi_input, mm, ((dvb_mux_t*)mm)->lm_tuning.dmc_fe_delsys);
...
static int
tvhdhomerun_frontend_start_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi, int weight) {
...
static void tvhdhomerun_frontend_stop_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
  tvhdhomerun_frontend_t* hfe = (tvhdhomerun_frontend_t*)mi;
  char                    buf1[256], buf2[256];
...
/* A helper function that writes a range of pids to the 'buffer'.  This function is called more than
   once from tvhdhomerun_frontend_update_pids. */
{
  ...
  if (*pBuffer < *endBuffer) { /* Check if 'buffer' is full. */
    if (a == b)
      *pBuffer += snprintf(*pBuffer,
          *endBuffer - *pBuffer,
          "0x%04x",
          a); /* First and last pid in a range are the same, then that one pid is appended. */
    else
      *pBuffer += snprintf(*pBuffer,
          *endBuffer - *pBuffer,
          "0x%04x-0x%04x",
...
static void tvhdhomerun_frontend_update_pids(mpegts_input_t* mi, mpegts_mux_t* mm) {
  ...
  /* Temporary storage for preparing the list of pid ranges to send to the hdhomerun device by the
   * set filter command. (hdhomerun's filter format example: "0x0000-0x0001 0x0030-0x0031
   * 0x0034-0x0036 0x00a0-0x00a1 0x00a4-0x00a5 0x1ffb") */
  ...
  /* Check if the turn-on-all-pids flag is set.  If so, ask the hdhomerun to send all pids
   * ("0x0000-0x1fff").  */
  if (wpids.all) {
    tvhdebug(LS_TVHDHOMERUN, "setting PID filter full mux");
    snprintf(buffer, bufferSize, "0x0000-0x1fff");
  }
  /* Otherwise, if we have any pids, proceed to find consecutive runs of pids to group them into a
     list of ranges. */
  else {
    if (wpids.count > 0) {
      ...
      const char* endBuffer = buffer + bufferSize; /* Have the address after the end of buffer on
      ...
      int   firstDelimiter =
          -1; /* Set this to 'true' so that we can skip writing the first delimiter/space. */

      /* Walk the list of pids and keep track of runs of consecutive pids. Setup the state for the
       * first pid. */
      for (i = 1, prev = begin = wpids.pids[0].pid; i < wpids.count; i++) {
        ...
        if (prev + 1 != curr) {
          /* If the current pid is NOT +1 more than the previous pid, then this is the end of a
           * range of pids. Write out this range of consecutive pids. */
          ...
          /* Also, this is the start of a new range of pids.  Set 'begin' to the beginning of the
           * next range. */
          begin = curr;
        }
        prev = curr;              /* At bottom of the loop, current pid is now the previous pid. */
        if (pBuffer >= endBuffer) /* We have reached the end of the 'buffer', no need to continue
                                     walking through the list. */
          break;
      }
      /* We are at the end of the list of pids, write the final range of consecutive pids to the
       * 'buffer'. */
      tvhdhomerun_frontend_update_pids_appendPidRange(begin,
          prev,
          &firstDelimiter,
      ...
      if (pBuffer >= endBuffer) { /* We could not fit the list of ranges of pids into the 'buffer'
                                   * and have an incomplete/mangled 'buffer' so as a backup, we will
                                   * request all pids except NULL packets (0x1fff). */
...
const idclass_t tvhdhomerun_frontend_dvbt_class = {.ic_super = &tvhdhomerun_frontend_class,
    .ic_class                                                = "tvhdhomerun_frontend_dvbt",
    .ic_caption                                              = N_("HDHomeRun DVB-T frontend"),
    .ic_properties                                           = (const property_t[]){{}}};

const idclass_t tvhdhomerun_frontend_dvbc_class = {.ic_super = &tvhdhomerun_frontend_class,
    .ic_class                                                = "tvhdhomerun_frontend_dvbc",
    .ic_caption                                              = N_("HDHomeRun DVB-C frontend"),
    .ic_properties                                           = (const property_t[]){{}}};

const idclass_t tvhdhomerun_frontend_atsc_t_class = {.ic_super = &tvhdhomerun_frontend_class,
    .ic_class                                                  = "tvhdhomerun_frontend_atsc_t",
    .ic_caption                                                = N_("HDHomeRun ATSC-T frontend"),
    .ic_properties                                             = (const property_t[]){{}}};

const idclass_t tvhdhomerun_frontend_atsc_c_class = {.ic_super = &tvhdhomerun_frontend_class,
    .ic_class                                                  = "tvhdhomerun_frontend_atsc_c",
    .ic_caption                                                = N_("HDHomeRun ATSC-C frontend"),
    .ic_properties                                             = (const property_t[]){{}}};

const idclass_t tvhdhomerun_frontend_cablecard_class = {.ic_super = &tvhdhomerun_frontend_class,
    .ic_class      = "tvhdhomerun_frontend_cablecard",
    .ic_caption    = N_("HDHomeRun CableCARD frontend"),
    .ic_properties = (const property_t[]){{}}};

const idclass_t tvhdhomerun_frontend_isdbt_class = {.ic_super = &tvhdhomerun_frontend_class,
    .ic_class                                                 = "tvhdhomerun_frontend_isdbt",
    .ic_caption                                               = N_("HDHomeRun ISDB-T frontend"),
    .ic_properties                                            = (const property_t[]){{}}};

static mpegts_network_t* tvhdhomerun_frontend_wizard_network(tvhdhomerun_frontend_t* hfe) {
  return (mpegts_network_t*)LIST_FIRST(&hfe->mi_networks);
}

static htsmsg_t* tvhdhomerun_frontend_wizard_get(tvh_input_t* ti, const char* lang) {
...
static void tvhdhomerun_frontend_wizard_set(tvh_input_t* ti, htsmsg_t* conf, const char* lang) {
  tvhdhomerun_frontend_t* hfe   = (tvhdhomerun_frontend_t*)ti;
...
tvhdhomerun_frontend_t* tvhdhomerun_frontend_create(tvhdhomerun_device_t* hd,
  ...
  /* Default name */
  if (!hfe->mi_name ||
      (strncmp(hfe->mi_name, "HDHomeRun ", 7) == 0 && strstr(hfe->mi_name, " Tuner ") &&
          strstr(hfe->mi_name, " #"))) {
    char lname[256];
    char ip[64];
