...
static int tvhdhomerun_frontend_get_weight(mpegts_input_t* mi,
    mpegts_mux_t*                                          mm,
    int                                                    flags,
    int                                                    weight) {
...
static int tvhdhomerun_frontend_get_priority(mpegts_input_t* mi,
    mpegts_mux_t*                                            mm,
    int                                                      flags) {
  return mpegts_input_get_priority(mi, mm, flags);
}

static int tvhdhomerun_frontend_get_grace(mpegts_input_t* mi,
    mpegts_mux_t*                                         mm) {
  return 15;
}

static int tvhdhomerun_frontend_is_enabled(mpegts_input_t* mi,
    mpegts_mux_t*                                          mm,
    int                                                    flags,
    int                                                    weight) {
...
static void* tvhdhomerun_frontend_input_thread(void* aux) {
  ...
  if (setsockopt(sockfd,
          ...
          sizeof(sock_opt)) < 0) {
    close(sockfd);
    tvherror(LS_TVHDHOMERUN,
        "failed to enable broadcast on socket (%d)",
        errno);
    return NULL;
  }
  ...
  if (setsockopt(sockfd,
    ...
    close(sockfd);
    tvherror(LS_TVHDHOMERUN,
        "failed to set address reuse on socket (%d)",
        errno);
    return NULL;
  }

  /* important: we need large rx buffers to accomodate the large amount of
   * traffic */
  if (setsockopt(sockfd,
          SOL_SOCKET,
          SO_RCVBUF,
    ...
    tvhwarn(LS_TVHDHOMERUN,
        "failed set socket rx buffer size, expect CC errors (%d)",
        errno);
  }
  ...
  sock_addr.sin_port        = config.local_port == 0
      ? 0
      : htons(config.local_port + hfe->hf_tunerNumber);
  ...
  /* pretend we are smart and set video_socket; doubt that this is required
   * though */
...
static void tvhdhomerun_frontend_monitor_cb(void* aux) {
  tvhdhomerun_frontend_t* hfe = aux;
  ...
  /* Get current status */
  tvh_mutex_lock(&hfe->hf_hdhomerun_device_mutex);
  res = hdhomerun_device_get_tuner_status(hfe->hf_hdhomerun_tuner,
      &tuner_status_str,
      &tuner_status);
  tvh_mutex_unlock(&hfe->hf_hdhomerun_device_mutex);
  if (res < 1)
  ...
  if (!hfe->hf_locked) {
    if (tuner_status.signal_present) {
      ...
      if (hfe->hf_type == DVB_TYPE_CABLECARD) {
        ...
        sscanf(strstr(tuner_status.channel, ":"),
            ":%u",
      ...
      tvh_mutex_lock(&hfe->hf_input_thread_mutex);
      tvh_thread_create(&hfe->hf_input_thread,
          NULL,
...
static int tvhdhomerun_frontend_tune(tvhdhomerun_frontend_t* hfe,
    mpegts_mux_instance_t*                                   mmi) {
  hfe->hf_status      = SIGNAL_NONE;
  ...
  tvh_mutex_lock(&hfe->hf_hdhomerun_device_mutex);
  res =
      hdhomerun_device_tuner_lockkey_request(hfe->hf_hdhomerun_tuner, &perror);
  if (res < 1) {
    tvh_mutex_unlock(&hfe->hf_hdhomerun_device_mutex);
  ...
  if (hfe->hf_type == DVB_TYPE_CABLECARD)
    res = hdhomerun_device_set_tuner_vchannel(hfe->hf_hdhomerun_tuner,
        channel_buf);
  else
    res = hdhomerun_device_set_tuner_channel(hfe->hf_hdhomerun_tuner,
        channel_buf);
  tvh_mutex_unlock(&hfe->hf_hdhomerun_device_mutex);
  if (res < 1) {
...
static int tvhdhomerun_frontend_start_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                                mmi,
    int                                                   weight) {
...
static void tvhdhomerun_frontend_stop_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                                mmi) {
  tvhdhomerun_frontend_t* hfe = (tvhdhomerun_frontend_t*)mi;
  char                    buf1[256], buf2[256];
...
static void tvhdhomerun_frontend_update_pids_appendPidRange(int a,
    int                                                         b,
    int*                                                        firstDelimiter,
    char**                                                      pBuffer,
    const char**                                                endBuffer)
/* A helper function that writes a range of pids to the 'buffer'.  This function
   is called more than once from tvhdhomerun_frontend_update_pids. */
{
  if (*firstDelimiter)   /* Don't bother printing a space before the first range
                            of pids. */
    *firstDelimiter = 0; /* Set this to false the first time. */
  else {
    if (*pBuffer < *endBuffer) /* Check if 'buffer' is full. */
      *pBuffer += snprintf(*pBuffer,
          *endBuffer - *pBuffer,
          " "); /* After the first range, separate pid ranges by a space. */
  }
  if (*pBuffer < *endBuffer) { /* Check if 'buffer' is full. */
    if (a == b)
      *pBuffer += snprintf(*pBuffer,
          *endBuffer - *pBuffer,
          "0x%04x",
          a); /* First and last pid in a range are the same, then that one pid
                 is appended. */
    else
      *pBuffer += snprintf(*pBuffer,
          *endBuffer - *pBuffer,
          "0x%04x-0x%04x",
          a,
          b); /* Append a range of pids to 'buffer'. */
  }
}

static void tvhdhomerun_frontend_update_pids(mpegts_input_t* mi,
    mpegts_mux_t*                                            mm) {
  tvhdhomerun_frontend_t* hfe = (tvhdhomerun_frontend_t*)mi;
  ...
  /* The length of the buffer for list of hdhomerun formatted list of pids. MUST
   * BE GREATER THAN 14! */
  const unsigned int bufferSize = 1024;

  /* Temporary storage for preparing the list of pid ranges to send to the
   * hdhomerun device by the set filter command. (hdhomerun's filter format
   * example: "0x0000-0x0001 0x0030-0x0031 0x0034-0x0036 0x00a0-0x00a1
   * 0x00a4-0x00a5 0x1ffb") */
  ...
  buffer[0] = '\0'; /* Initialize buffer to handle the case where no pids are
                       requested. */

  /* Check if the turn-on-all-pids flag is set.  If so, ask the hdhomerun to
   * send all pids ("0x0000-0x1fff").  */
  if (wpids.all) {
    tvhdebug(LS_TVHDHOMERUN, "setting PID filter full mux");
    snprintf(buffer, bufferSize, "0x0000-0x1fff");
  }
  /* Otherwise, if we have any pids, proceed to find consecutive runs of pids to
     group them into a list of ranges. */
  else {
    if (wpids.count > 0) {
      ...
      const char* endBuffer =
          buffer + bufferSize; /* Have the address after the end of buffer on
                                  hand to help avoid writing past the buffer. */
      char* pBuffer = buffer;  /* Move this pointer through the buffer as we
                                  write formatted pids. */
      int firstDelimiter = -1; /* Set this to 'true' so that we can skip writing
                                  the first delimiter/space. */

      /* Walk the list of pids and keep track of runs of consecutive pids. Setup
       * the state for the first pid. */
      for (i = 1, prev = begin = wpids.pids[0].pid; i < wpids.count; i++) {

        curr = wpids.pids[i].pid;
        /* make sure the pid maps to a max of 0x1FFF, API will reject the call
         * otherwise */
        if (curr > 0x1FFF) {
          tvherror(LS_TVHDHOMERUN,
              "pid %d is too large, masking to API maximum of 0x1FFF",
              curr);
          curr = (curr & 0x1FFF);
        }
        ...
        if (prev + 1 != curr) {
          /* If the current pid is NOT +1 more than the previous pid, then this
           * is the end of a range of pids.
           * Write out this range of consecutive pids. */
          tvhdhomerun_frontend_update_pids_appendPidRange(begin,
              prev,
              &firstDelimiter,
              &pBuffer,
              &endBuffer);

          /* Also, this is the start of a new range of pids.  Set 'begin' to the
           * beginning of the next range. */
          begin = curr;
        }
        prev = curr; /* At bottom of the loop, current pid is now the previous
                        pid. */
        if (pBuffer >=
            endBuffer) /* We have reached the end of the 'buffer', no need to
                          continue walking through the list. */
          break;
      }
      /* We are at the end of the list of pids, write the final range of
       * consecutive pids to the 'buffer'. */
      tvhdhomerun_frontend_update_pids_appendPidRange(begin,
          prev,
          &firstDelimiter,
          &pBuffer,
      ...
      if (pBuffer >=
          endBuffer) { /* We could not fit the list of ranges of pids into the
                        * 'buffer' and have an incomplete/mangled 'buffer'
                        * so as a backup, we will request all pids except NULL
                        * packets (0x1fff). */
        tvhdebug(LS_TVHDHOMERUN,
            "pfilter list is too big for buffer[%d] = \"%s\"(truncated)",
            bufferSize,
            buffer);
...
static idnode_set_t* tvhdhomerun_frontend_network_list(mpegts_input_t* mi) {
  tvhdhomerun_frontend_t* hfe = (tvhdhomerun_frontend_t*)mi;

  return dvb_network_list_by_fe_type(hfe->hf_type);
}

static void tvhdhomerun_frontend_class_changed(idnode_t* in) {
  tvhdhomerun_device_t* la = ((tvhdhomerun_frontend_t*)in)->hf_device;
  tvhdhomerun_device_changed(la);
}

void tvhdhomerun_frontend_save(tvhdhomerun_frontend_t* hfe, htsmsg_t* fe) {
  ...
  snprintf(id,
      sizeof(id),
      "%s #%d",
      dvb_type2str(hfe->hf_type),
      hfe->hf_tunerNumber);
  htsmsg_add_msg(fe, id, m);
}

const idclass_t tvhdhomerun_frontend_class = {.ic_super = &mpegts_input_class,
...
static mpegts_network_t* tvhdhomerun_frontend_wizard_network(
    tvhdhomerun_frontend_t* hfe) {
  return (mpegts_network_t*)LIST_FIRST(&hfe->mi_networks);
}

static htsmsg_t* tvhdhomerun_frontend_wizard_get(tvh_input_t* ti,
    const char*                                               lang) {
...
static void tvhdhomerun_frontend_wizard_set(tvh_input_t* ti,
    htsmsg_t*                                            conf,
    const char*                                          lang) {
  tvhdhomerun_frontend_t* hfe   = (tvhdhomerun_frontend_t*)ti;
...
void tvhdhomerun_frontend_delete(tvhdhomerun_frontend_t* hfe) {
...
tvhdhomerun_frontend_t* tvhdhomerun_frontend_create(tvhdhomerun_device_t* hd,
    struct hdhomerun_discover_device_t* discover_info,
    htsmsg_t*                           conf,
    dvb_fe_type_t                       type,
    unsigned int                        frontend_number) {
  const idclass_t*        idc;
  ...
  hfe->hf_hdhomerun_tuner = hdhomerun_device_create(discover_info->device_id,
      discover_info->ip_addr,
      frontend_number,
      hdhomerun_debug_obj);
  ...
  hfe = (tvhdhomerun_frontend_t*)
      mpegts_input_create0((mpegts_input_t*)hfe, idc, uuid, conf);
  if (!hfe)
    return NULL;
  ...
  if (!hfe->mi_name ||
      (strncmp(hfe->mi_name, "HDHomeRun ", 7) == 0 &&
          strstr(hfe->mi_name, " Tuner ") && strstr(hfe->mi_name, " #"))) {
    char lname[256];
    char ip[64];
    tcp_get_str_from_ip(&hd->hd_info.ip_address, ip, sizeof(ip));
    snprintf(lname,
        sizeof(lname),
        "HDHomeRun %s Tuner #%i (%s)",
        dvb_type2str(type),
        hfe->hf_tunerNumber,
        ip);
    free(hfe->mi_name);
