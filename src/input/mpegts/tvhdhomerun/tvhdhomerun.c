...
#define hdhomerun_discover_find_devices_custom hdhomerun_discover_find_devices_custom_v2
#endif

static htsmsg_t* tvhdhomerun_device_class_save(idnode_t* in, char* filename, size_t fsize) {
...
static void
tvhdhomerun_device_class_get_title(idnode_t* in, const char* lang, char* dst, size_t dstsize) {
  tvhdhomerun_device_t* hd = (tvhdhomerun_device_t*)in;
  char                  ip[64];
...
static htsmsg_t* tvhdhomerun_device_class_override_enum(void* p, const char* lang) {
...
static void tvhdhomerun_device_class_override_notify(void* obj, const char* lang) {
  tvhdhomerun_device_t*              hd = obj;
...
static void tvhdhomerun_discovery_destroy(tvhdhomerun_discovery_t* d, int unlink) {
  if (d == NULL)
    return;
  if (unlink) {
...
static void tvhdhomerun_device_calc_bin_uuid(uint8_t* uuid, const uint32_t device_id) {
  SHA_CTX sha1;
...
static void tvhdhomerun_device_create(struct hdhomerun_discover_device_t* dInfo) {
  ...
  if (conf != NULL) {
    ...
    if (override_type != NULL) {
      ...
      if (!(type == DVB_TYPE_C || type == DVB_TYPE_T || type == DVB_TYPE_ATSC_T ||
              type == DVB_TYPE_ATSC_C || type == DVB_TYPE_ISDB_T || type == DVB_TYPE_ISDB_C ||
  ...
  if (!tvh_hardware_create0((tvh_hardware_t*)hd, &tvhdhomerun_device_class, uhex, conf))
    return;
