...
static idnode_set_t* tvhdhomerun_device_class_get_childs(idnode_t* in) {
  tvhdhomerun_device_t*   hd = (tvhdhomerun_device_t*)in;
...
static void tvhdhomerun_device_class_get_title(idnode_t* in,
    const char*                                          lang,
    char*                                                dst,
    size_t                                               dstsize) {
  tvhdhomerun_device_t* hd = (tvhdhomerun_device_t*)in;
  ...
  snprintf(dst, dstsize, "%s - %s", hd->hd_info.friendlyname, ip);
}

static const void* tvhdhomerun_device_class_active_get(void* obj) {
  static int              active;
...
static htsmsg_t* tvhdhomerun_device_class_override_enum(void* p,
    const char*                                               lang) {
...
static int tvhdhomerun_device_class_override_set(void* obj, const void* p) {
...
static void tvhdhomerun_device_class_override_notify(void* obj,
    const char*                                            lang) {
  tvhdhomerun_device_t*              hd = obj;
  ...
  while ((hfe = TAILQ_FIRST(&hd->hd_frontends)) != NULL) {
    ...
    discover_info.device_id =
        hdhomerun_device_get_device_id(hfe->hf_hdhomerun_tuner);
    discover_info.ip_addr =
        hdhomerun_device_get_device_ip(hfe->hf_hdhomerun_tuner);
...
static const void* tvhdhomerun_device_class_get_ip_address(void* obj) {
  tvhdhomerun_device_t* hd = obj;
  tcp_get_str_from_ip(&hd->hd_info.ip_address, prop_sbuf, PROP_SBUF_LEN);
  return &prop_sbuf_ptr;
}

const idclass_t tvhdhomerun_device_class = {.ic_class = "tvhdhomerun_client",
...
static void tvhdhomerun_discovery_destroy(tvhdhomerun_discovery_t* d,
    int                                                            unlink) {
  if (d == NULL)
    return;
  if (unlink) {
...
static void tvhdhomerun_device_calc_bin_uuid(uint8_t* uuid,
...
static void tvhdhomerun_device_calc_uuid(char* uhex, const uint32_t device_id) {
  uint8_t uuidbin[20];
...
static tvhdhomerun_device_t* tvhdhomerun_device_find(uint32_t device_id) {
  tvh_hardware_t* th;
  uint8_t         binuuid[20];
...
static void tvhdhomerun_device_create(
    struct hdhomerun_discover_device_t* dInfo) {
  ...
  hdhomerun_tuner =
      hdhomerun_device_create(dInfo->device_id, dInfo->ip_addr, 0, NULL);
  {
  ...
  if (!tvh_hardware_create0((tvh_hardware_t*)hd,
          &tvhdhomerun_device_class,
          uhex,
          conf))
    return;
  ...
  ((struct sockaddr_in*)&hd->hd_info.ip_address)->sin_addr.s_addr =
      htonl(dInfo->ip_addr);
  ...
  for (j = 0; j < dInfo->tuner_count; ++j) {
    if (tvhdhomerun_frontend_create(hd, dInfo, feconf, type, j)) {
      tvhinfo(LS_TVHDHOMERUN,
          "Created frontend %08X tuner %d",
          dInfo->device_id,
          j);
    } else {
      tvherror(LS_TVHDHOMERUN,
          "Unable to create frontend-device. ( %08x-%d )",
          dInfo->device_id,
          j);
    }
  }
...
static uint32_t tvhdhomerun_ip(void) {
  if (*config.hdhomerun_ip == 0)
    return 0;
...
static void* tvhdhomerun_device_discovery_thread(void* aux) {
  struct hdhomerun_discover_device_t result_list[MAX_HDHOMERUN_DEVICES];
  int                                numDevices, brk;

  while (tvheadend_is_running()) {

    numDevices = hdhomerun_discover_find_devices_custom(tvhdhomerun_ip(),
    ...
    if (numDevices > 0) {
      while (numDevices > 0) {
        ...
        if (cDev->device_type == HDHOMERUN_DEVICE_TYPE_TUNER) {
          ...
          tvhdhomerun_device_t* existing =
              tvhdhomerun_device_find(cDev->device_id);
          if (tvheadend_is_running()) {
            if (!existing) {
              tvhinfo(LS_TVHDHOMERUN,
                  "Found HDHomerun device %08x with %d tuners",
                  cDev->device_id,
                  cDev->tuner_count);
              tvhdhomerun_device_create(cDev);
            } else if (((struct sockaddr_in*)&existing->hd_info.ip_address)
                           ->sin_addr.s_addr != htonl(cDev->ip_addr)) {
              ...
              ((struct sockaddr_in*)&detected_dev_addr)->sin_addr.s_addr =
                  htonl(cDev->ip_addr);

              char existing_ip[64];
              tcp_get_str_from_ip(&existing->hd_info.ip_address,
                  existing_ip,
                  sizeof(existing_ip));

              char detected_ip[64];
              tcp_get_str_from_ip(&detected_dev_addr,
                  detected_ip,
                  sizeof(detected_ip));

              tvhinfo(LS_TVHDHOMERUN,
                  "HDHomerun device %08x switched IPs from %s to %s, updating",
                  cDev->device_id,
                  existing_ip,
                  detected_ip);
...
void tvhdhomerun_init(void) {
  ...
  tvh_thread_create(&tvhdhomerun_discovery_tid,
      NULL,
...
void tvhdhomerun_device_destroy(tvhdhomerun_device_t* hd) {
  tvhdhomerun_frontend_t* lfe;
