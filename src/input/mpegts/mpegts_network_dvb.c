...
void dvb_network_scanfile_set(dvb_network_t* ln, const char* id) {
  ...
  LIST_FOREACH (dmc, &sfn->sfn_muxes, dmc_link) {
    if (!(mm = dvb_network_find_mux(ln, dmc, MPEGTS_ONID_NONE, MPEGTS_TSID_NONE, 0, 0))) {
      mm = dvb_mux_create0(ln, MPEGTS_ONID_NONE, MPEGTS_TSID_NONE, dmc, NULL, NULL);
      if (mm) {
...
static htsmsg_t*
dvb_network_class_scanfile_list0(const idclass_t* clazz, dvb_network_t* ln, const char* lang) {
  const char* type;
...
#define SCANFILE_LIST(name)                                                            \
  static htsmsg_t* dvb_network_class_scanfile_list_##name(void* o, const char* lang) { \
...
static int dvb_network_check_bandwidth(int bw1, int bw2) {
  if (bw1 == DVB_BANDWIDTH_NONE || bw1 == DVB_BANDWIDTH_AUTO || bw2 == DVB_BANDWIDTH_NONE ||
      bw2 == DVB_BANDWIDTH_AUTO)
    return 0;
  return bw1 != bw2;
}

static int dvb_network_check_symbol_rate(dvb_mux_t* lm, dvb_mux_conf_t* dmc, int deltar) {
  switch (dmc->dmc_fe_type) {
    ...
    case DVB_TYPE_ATSC_C:
      return deltaU32(lm->lm_tuning.u.dmc_fe_qam.symbol_rate, dmc->u.dmc_fe_qam.symbol_rate) >
          deltar;
    case DVB_TYPE_S:
      return deltaU32(lm->lm_tuning.u.dmc_fe_qpsk.symbol_rate, dmc->u.dmc_fe_qpsk.symbol_rate) >
          deltar;
...
dvb_mux_t* dvb_network_find_mux(dvb_network_t* ln,
  ...
  LIST_FOREACH (mm, &ln->mn_muxes, mm_network_link) {
    ...
    if (onid != MPEGTS_ONID_NONE && mm->mm_onid != MPEGTS_ONID_NONE && mm->mm_onid != onid)
      continue;
    if (tsid != MPEGTS_TSID_NONE && mm->mm_tsid != MPEGTS_TSID_NONE && mm->mm_tsid != tsid)
      continue;
...
static htsmsg_t* dvb_network_config_save(mpegts_network_t* mn, char* filename, size_t fsize) {
  htsmsg_t* c = htsmsg_create_map();
  char      ubuf[UUID_HEX_SIZE];
  idnode_save(&mn->mn_id, c);
  htsmsg_add_str(c, "class", mn->mn_id.in_class->ic_class);
  if (filename)
    snprintf(filename, fsize, "input/dvb/networks/%s/config", idnode_uuid_as_str(&mn->mn_id, ubuf));
  return c;
}
...
static mpegts_mux_t* dvb_network_mux_create2(mpegts_network_t* mn, htsmsg_t* conf) {
  dvb_network_t* ln = (dvb_network_t*)mn;
  dvb_mux_t*     mm;
  mm = dvb_mux_create0(ln, MPEGTS_ONID_NONE, MPEGTS_TSID_NONE, NULL, NULL, conf);
...
dvb_network_t* dvb_network_create0(const char* uuid, const idclass_t* idc, htsmsg_t* conf) {
  dvb_network_t*  ln;
  dvb_mux_t*      lm;
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
  ...
  /* Create */
  if (!(ln = (dvb_network_t*)mpegts_network_create0((void*)ln, idc, uuid, NULL, conf)))
    return NULL;
...
static mpegts_network_t* dvb_network_builder(const idclass_t* idc, htsmsg_t* conf) {
...
void dvb_network_init(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(dvb_network_classes); i++)
    mpegts_network_register_builder(dvb_network_classes[i], dvb_network_builder);
