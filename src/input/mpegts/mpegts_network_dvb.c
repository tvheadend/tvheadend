...
static void dvb_network_class_delete(idnode_t* in) {
  mpegts_network_t* mn = (mpegts_network_t*)in;
  char              ubuf[UUID_HEX_SIZE];

  /* remove config */
  hts_settings_remove("input/dvb/networks/%s", idnode_uuid_as_str(in, ubuf));

  /* Parent delete */
  mpegts_network_delete(mn, 1);
}

static const void* dvb_network_class_scanfile_get(void* o) {
  prop_ptr = NULL;
  return &prop_ptr;
}
...
void dvb_network_scanfile_set(dvb_network_t* ln, const char* id) {
  ...
  LIST_FOREACH (dmc, &sfn->sfn_muxes, dmc_link) {
    if (!(mm = dvb_network_find_mux(ln,
              dmc,
              MPEGTS_ONID_NONE,
              MPEGTS_TSID_NONE,
              0,
              0))) {
      mm = dvb_mux_create0(ln,
          MPEGTS_ONID_NONE,
          MPEGTS_TSID_NONE,
          dmc,
          NULL,
      ...
      if (tvhtrace_enabled()) {
        ...
        tvhtrace(LS_SCANFILE,
            "mux %p %s added to network %s",
            mm,
            buf,
            ln->mn_network_name);
      }
      ...
      if (tvhtrace_enabled()) {
        ...
        dvb_mux_conf_str(dmc, buf, sizeof(buf));
        tvhtrace(LS_SCANFILE,
            "mux %p skipped %s in network %s",
            mm,
            buf,
            ln->mn_network_name);
        dvb_mux_conf_str(&((dvb_mux_t*)mm)->lm_tuning, buf, sizeof(buf));
        tvhtrace(LS_SCANFILE,
            "mux %p exists %s in network %s",
            mm,
            buf,
            ln->mn_network_name);
      }
...
static int dvb_network_class_scanfile_set(void* o, const void* s) {
  dvb_network_scanfile_set(o, s);
  return 0;
}

static htsmsg_t* dvb_network_class_scanfile_list0(const idclass_t* clazz,
    dvb_network_t*                                                 ln,
    const char*                                                    lang) {
  const char* type;
...
htsmsg_t* dvb_network_class_scanfile_list(void* o, const char* lang) {
  if (o == NULL)
    return NULL;
  return dvb_network_class_scanfile_list0(
      ((mpegts_network_t*)o)->mn_id.in_class,
      o,
      lang);
}

#define SCANFILE_LIST(name)                                              \
  static htsmsg_t* dvb_network_class_scanfile_list_##name(void* o,       \
      const char*                                               lang) {                                                \
    return dvb_network_class_scanfile_list0(&dvb_network_##name##_class, \
        o,                                                               \
        lang);                                                           \
  }
...
static const void* dvb_network_class_orbital_pos_get(void* o) {
  dvb_network_t* ln = o;
  prop_sbuf[0]      = '\0';
...
static int dvb_network_class_orbital_pos_set(void* o, const void* s) {
  dvb_network_t* ln = o;
  int            satpos;
...
static htsmsg_t* dvb_network_class_orbital_pos_list(void* o, const char* lang) {
  htsmsg_t *e, *m = htsmsg_create_map();
  htsmsg_add_str(m, "type", "api");
...
const idclass_t dvb_network_class = {.ic_super = &mpegts_network_class,
    .ic_class                                  = "dvb_network",
    .ic_caption                                = N_("LinuxDVB network"),
    .ic_delete                                 = dvb_network_class_delete,
    .ic_properties                             = (const property_t[]){{}}};

const idclass_t dvb_network_dvbt_class = {.ic_super = &dvb_network_class,
...
const idclass_t dvb_network_dvbc_class = {.ic_super = &dvb_network_class,
...
const idclass_t dvb_network_dvbs_class = {.ic_super = &dvb_network_class,
...
const idclass_t dvb_network_atsc_t_class = {.ic_super = &dvb_network_class,
...
const idclass_t dvb_network_atsc_c_class = {.ic_super = &dvb_network_class,
...
const idclass_t dvb_network_cablecard_class = {.ic_super = &dvb_network_class,
    .ic_class      = "dvb_network_cablecard",
    .ic_caption    = N_("CableCARD Network"),
    .ic_properties = (const property_t[]){{}}};

const idclass_t dvb_network_isdb_t_class = {.ic_super = &dvb_network_class,
...
const idclass_t dvb_network_isdb_c_class = {.ic_super = &dvb_network_class,
...
const idclass_t dvb_network_isdb_s_class = {.ic_super = &dvb_network_class,
...
const idclass_t dvb_network_dtmb_class = {.ic_super = &dvb_network_class,
...
const idclass_t dvb_network_dab_class = {.ic_super = &dvb_network_class,
...
static int dvb_network_check_bandwidth(int bw1, int bw2) {
...
static int dvb_network_check_orbital_pos(int satpos1, int satpos2) {
...
dvb_mux_t* dvb_network_find_mux(dvb_network_t* ln,
    dvb_mux_conf_t*                            dmc,
    uint32_t                                   onid,
    uint32_t                                   tsid,
    int                                        check,
  ...
  LIST_FOREACH (mm, &ln->mn_muxes, mm_network_link) {
    ...
    if (lm->lm_tuning.dmc_fe_type != dmc->dmc_fe_type)
      continue;

    /* Also, the system type should match (DVB-S/DVB-S2) */
    if (!approx_match && (lm->lm_tuning.dmc_fe_delsys != dmc->dmc_fe_delsys))
      continue;

    /* if ONID/TSID are a perfect match (and this is DVB-S, allow greater
     * deltaf) */
    if (lm->lm_tuning.dmc_fe_type == DVB_TYPE_S) {
      deltar = 7000;
      if (onid != MPEGTS_ONID_NONE && tsid != MPEGTS_TSID_NONE) {
        deltaf = 16000; // This is slightly crazy, but I have seen 10MHz changes
                        // in freq and remember the ONID and TSID must agree
      } else {
    ...
    /* Reject if not same frequency (some tolerance due to changes and diff in
     * NIT) */
    if (deltaU32(lm->lm_tuning.dmc_fe_freq, dmc->dmc_fe_freq) > deltaf)
      continue;

    /* Reject if not same symbol rate (some tolerance due to changes and diff in
     * NIT) */
    if (dvb_network_check_symbol_rate(lm, dmc, deltar))
      continue;

    /* Reject if not same polarisation */
    if (lm->lm_tuning.u.dmc_fe_qpsk.polarisation !=
        dmc->u.dmc_fe_qpsk.polarisation)
      continue;
    ...
    if (!approx_match && (lm->lm_tuning.dmc_fe_type == DVB_TYPE_S)) {
      ...
      if (!dvb_modulation_is_none_or_auto(lm->lm_tuning.dmc_fe_modulation) &&
          ...
          lm->lm_tuning.dmc_fe_modulation != dmc->dmc_fe_modulation)
        continue;

      /* Same FEC */
      if (lm->lm_tuning.u.dmc_fe_qpsk.fec_inner != dmc->u.dmc_fe_qpsk.fec_inner)
        continue;
    ...
    /* Same PLP/ISI */
    if (lm->lm_tuning.dmc_fe_stream_id != dmc->dmc_fe_stream_id)
      continue;

    mm_alt = mm;

    /* Reject if not same ID */
    if (onid != MPEGTS_ONID_NONE && mm->mm_onid != MPEGTS_ONID_NONE &&
        mm->mm_onid != onid)
      continue;
    if (tsid != MPEGTS_TSID_NONE && mm->mm_tsid != MPEGTS_TSID_NONE &&
        mm->mm_tsid != tsid)
      continue;
...
static mpegts_mux_t* dvb_network_create_mux(mpegts_network_t* mn,
    void*                                                     origin,
    uint32_t                                                  onid,
    uint32_t                                                  tsid,
    void*                                                     p,
  ...
  if (force && cls == &dvb_mux_dvbs_class && dmc->dmc_fe_type == DVB_TYPE_S &&
    ...
    if (dvb_network_check_orbital_pos(satpos, dmc->u.dmc_fe_qpsk.orbital_pos)) {
      LIST_FOREACH (mn, &mpegts_network_all, mn_global_link) {
        ...
        if (satpos == INT_MAX)
          continue;
        if (!dvb_network_check_orbital_pos(satpos,
                dmc->u.dmc_fe_qpsk.orbital_pos))
          break;
      }
      if (mn == NULL)
  ...
  if (!mm && (ln->mn_autodiscovery != MN_DISCOVERY_DISABLE || force)) {
    save |=
        dvb_fe_type_by_network_class(ln->mn_id.in_class) == dmc->dmc_fe_type;
    ...
    if (save) {
      ...
      if (tvhtrace_enabled()) {
        ...
        dvb_mux_conf_str(&((dvb_mux_t*)mm)->lm_tuning, buf, sizeof(buf));
        tvhtrace(LS_MPEGTS,
            "mux %p %s onid %i tsid %i added to network %s (autodiscovery)",
            mm,
            buf,
            onid,
            tsid,
            mm->mm_network->mn_network_name);
      }
    }
  } else if (mm) {
    char           buf[128];
    ...
    if (mm->mm_dmc_origin_expire > mclk() && mm->mm_dmc_origin &&
...
#define COMPARE(x, cbit)                                                \
  ({                                                                    \
    ...
    if (xr) {                                                           \
      tvhtrace(LS_MPEGTS,                                               \
          "create mux dmc->" #x " (%li) != lm->lm_tuning." #x " (%li)", \
          (long)dmc->x,                                                 \
          (long)tuning_new.x);                                          \
      tuning_new.x = dmc->x;                                            \
    }                                                                   \
...
#undef COMPAREN
    ...
    if (save) {
      ...
      tvhlog(change ? LOG_WARNING : LOG_NOTICE,
          LS_MPEGTS,
          "mux %s%s %s (%08x)",
          muxname,
          change ? " changed from" : " old params",
          buf,
          save);
...
static mpegts_mux_t* dvb_network_mux_create2(mpegts_network_t* mn,
    htsmsg_t*                                                  conf) {
  dvb_network_t* ln = (dvb_network_t*)mn;
  dvb_mux_t*     mm;
  mm =
      dvb_mux_create0(ln, MPEGTS_ONID_NONE, MPEGTS_TSID_NONE, NULL, NULL, conf);
...
dvb_network_t*
dvb_network_create0(const char* uuid, const idclass_t* idc, htsmsg_t* conf) {
  dvb_network_t*  ln;
  dvb_mux_t*      lm;
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
  ...
  /* Create */
  if (!(ln = (dvb_network_t*)
              mpegts_network_create0((void*)ln, idc, uuid, NULL, conf)))
    return NULL;
...
static mpegts_network_t* dvb_network_builder(const idclass_t* idc,
    htsmsg_t*                                                 conf) {
