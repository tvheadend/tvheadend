...
const idclass_t mpegts_mux_instance_class = {.ic_super = &tvh_input_instance_class,
...
static void mpegts_mux_scan_active(mpegts_mux_t* mm, const char* buf, mpegts_input_t* mi) {
  int t;

  /* Setup scan */
  if (mm->mm_scan_state == MM_SCAN_STATE_PEND || mm->mm_scan_state == MM_SCAN_STATE_IPEND) {
...
static int mpegts_mux_keep_exists(mpegts_input_t* mi) {
...
static int mpegts_mux_subscribe_keep(mpegts_mux_t* mm, mpegts_input_t* mi) {
  char* s;
  int   r;
...
static void mpegts_mux_subscribe_linked(mpegts_input_t* mi, mpegts_mux_t* mm) {
  ...
  LIST_FOREACH (mnl2, &mi2->mi_networks, mnl_mi_link)
    if (mnl2->mnl_network == mm->mm_network)
      LIST_FOREACH (mm2, &mnl2->mnl_network->mn_muxes, mm_network_link)
        if (!mm2->mm_active && MM_SCAN_CHECK_OK(mm) && !LIST_EMPTY(&mm2->mm_services))
...
static htsmsg_t* mpegts_mux_class_save(idnode_t* self, char* filename, size_t fsize) {
...
static void mpegts_mux_class_delete(idnode_t* self) {
  mpegts_mux_t* mm = (mpegts_mux_t*)self;
  if (mm->mm_delete)
    mm->mm_delete(mm, 1);
}
...
static void
mpegts_mux_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
  mpegts_mux_nice_name((mpegts_mux_t*)self, dst, dstsize);
}

static const void* mpegts_mux_class_get_num_svc(void* ptr) {
  static int        n;
  mpegts_mux_t*     mm = ptr;
  mpegts_service_t* s;
...
static const void* mpegts_mux_class_get_num_chn(void* ptr) {
  static int             n;
  mpegts_mux_t*          mm = ptr;
  mpegts_service_t*      s;
...
static const void* mpegts_mux_class_get_network(void* ptr) {
...
static const void* mpegts_mux_class_get_network_uuid(void* ptr) {
  mpegts_mux_t* mm = ptr;
  if (mm && mm->mm_network)
...
static const void* mpegts_mux_class_get_name(void* ptr) {
  static char   buf[512], *s = buf;
  mpegts_mux_t* mm = ptr;
...
static struct strtab scan_state_tab[] = {
...
static struct strtab scan_result_tab[] = {
...
int mpegts_mux_class_scan_state_set(void* o, const void* p) {
  mpegts_mux_t* mm    = o;
  int           state = *(int*)p;
  ...
  if (state == MM_SCAN_STATE_PEND || state == MM_SCAN_STATE_IPEND ||
    ...
    mpegts_network_scan_queue_add(mm, SUBSCRIPTION_PRIO_SCAN_USER, SUBSCRIPTION_USERSCAN, 0);
...
static htsmsg_t* mpegts_mux_class_scan_state_enum(void* p, const char* lang) {
  return strtab2htsmsg(scan_state_tab, 1, lang);
}

static htsmsg_t* mpegts_mux_class_scan_result_enum(void* p, const char* lang) {
  return strtab2htsmsg(scan_result_tab, 1, lang);
}

static void mpegts_mux_class_enabled_notify(void* p, const char* lang) {
  mpegts_mux_t* mm = p;
  if (!mm->mm_is_enabled(mm)) {
...
static htsmsg_t* mpegts_mux_enable_list(void* o, const char* lang) {
...
static htsmsg_t* mpegts_mux_epg_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
static htsmsg_t* mpegts_mux_epg_module_id_list(void* o, const char* lang) {
  return epggrab_ota_module_id_list(lang);
}

static htsmsg_t* mpegts_mux_ac3_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
...
const idclass_t mpegts_mux_class = {.ic_class = "mpegts_mux",
    ...
    .ic_properties                            = (const property_t[]){{.type      = PT_INT,
                                                                         .id     = "enabled",
                                                                         .name   = N_("Enabled"),
...
static void mpegts_mux_display_name(mpegts_mux_t* mm, char* buf, size_t len) {
  snprintf(buf, len, "Multiplex [onid:%04X tsid:%04X]", mm->mm_onid, mm->mm_tsid);
}
...
static htsmsg_t* mpegts_mux_config_save(mpegts_mux_t* mm, char* filename, size_t fsize) {
  return NULL;
}

static int mpegts_mux_is_enabled(mpegts_mux_t* mm) {
  return mm->mm_network->mn_enabled && mm->mm_enabled == MM_ENABLE;
}

static int mpegts_mux_is_epg(mpegts_mux_t* mm) {
  return mm->mm_epg;
}

static void mpegts_mux_create_instances(mpegts_mux_t* mm) {
...
static int mpegts_mux_has_subscribers(mpegts_mux_t* mm, const char* name) {
...
static void mpegts_mux_stop(mpegts_mux_t* mm, int force, int reason) {
  char*                  s;
  ...
  while ((mp = RB_FIRST(&mm->mm_pids))) {
    assert(mi);
    if (mp->mp_pid == MPEGTS_FULLMUX_PID || mp->mp_pid == MPEGTS_TABLES_PID) {
      while ((mps = LIST_FIRST(&mm->mm_all_subs))) {
        tvhdebug(LS_MPEGTS,
            "%s - close PID %s subscription [%d/%p]",
            mm->mm_nicename,
...
static void mpegts_mux_update_pids_cb(void* aux) {
  mpegts_mux_t*   mm = aux;
  mpegts_input_t* mi;
...
static void mpegts_mux_scan_service_check(mpegts_mux_t* mm) {
  ...
  for (s = LIST_FIRST(&mm->mm_services); s; s = snext) {
    ...
    if (s->s_enabled && s->s_auto != SERVICE_AUTO_OFF &&
      ...
      tvhinfo(LS_MPEGTS,
          "disabling service %s [sid %04X/%d] (missing in PAT/SDT)",
...
static void mpegts_mux_scan_timeout(void* aux) {
  int             c, q, w;
  mpegts_mux_t*   mm = aux;
  mpegts_table_t* mt;
...
mpegts_mux_t* mpegts_mux_post_create(mpegts_mux_t* mm) {
  ...
  if (mm->mm_scan_result == MM_SCAN_NONE || !mn->mn_skipinitscan)
    mpegts_network_scan_queue_add(mm, SUBSCRIPTION_PRIO_SCAN_INIT, SUBSCRIPTION_INITSCAN, 10);
  else if (mm->mm_network->mn_idlescan)
    mpegts_network_scan_queue_add(mm, SUBSCRIPTION_PRIO_SCAN_IDLE, SUBSCRIPTION_IDLESCAN, 10);
...
mpegts_service_t* mpegts_mux_find_service(mpegts_mux_t* mm, uint16_t sid) {
  mpegts_service_t* ms;
  LIST_FOREACH (ms, &mm->mm_services, s_dvb_mux_link)
...
mpegts_pid_t* mpegts_mux_find_pid_(mpegts_mux_t* mm, int pid, int create) {
  ...
  if (pid < 0 || pid > MPEGTS_TABLES_PID)
    return NULL;
...
int mpegts_mux_compare(mpegts_mux_t* a, mpegts_mux_t* b) {
  int r = uuid_cmp(&a->mm_network->mn_id.in_uuid, &b->mm_network->mn_id.in_uuid);
  if (r)
    return r;
  ...
  if (idnode_is_instance(&a->mm_id, &dvb_mux_dvbs_class) &&
    ...
    r = (int)mc1->u.dmc_fe_qpsk.polarisation - (int)mc2->u.dmc_fe_qpsk.polarisation;
    if (r == 0)
