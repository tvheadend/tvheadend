...
mpegts_mux_instance_t* mpegts_mux_instance_create0(mpegts_mux_instance_t* mmi,
  ...
  LIST_INSERT_HEAD(&mi->mi_mux_instances,
      (tvh_input_instance_t*)mmi,
      tii_input_link);
...
static void
mpegts_mux_scan_active(mpegts_mux_t* mm, const char* buf, mpegts_input_t* mi) {
  int t;
  ...
  if (mm->mm_scan_state == MM_SCAN_STATE_PEND ||
    ...
    /* Setup timeout */
    mtimer_arm_rel(&mm->mm_scan_timeout,
        mpegts_mux_scan_timeout,
        mm,
        sec2mono(t));
  }
}
...
fatal:
  ...
  tvherror(LS_MPEGTS,
      "%s - %s - linked input cannot be started (%s: %i)",
      buf1,
      buf2,
      serr,
      r);
...
void mpegts_mux_unsubscribe_linked(mpegts_input_t* mi, service_t* t) {
  ...
  if (mi) {
    ...
    for (ths = LIST_FIRST(&subscriptions); ths; ths = ths_next) {
      ...
      if (ths->ths_source == (tvh_input_t*)mi &&
          !strcmp(ths->ths_title, "keep") && ths->ths_service != t)
...
static void mpegts_mux_class_get_title(idnode_t* self,
    const char*                                  lang,
    char*                                        dst,
    size_t                                       dstsize) {
...
int mpegts_mux_class_scan_state_set(void* o, const void* p) {
  ...
  if (state == MM_SCAN_STATE_PEND || state == MM_SCAN_STATE_IPEND ||
    ...
    mpegts_network_scan_queue_add(mm,
        SUBSCRIPTION_PRIO_SCAN_USER,
        SUBSCRIPTION_USERSCAN,
        0);
...
const idclass_t mpegts_mux_class = {.ic_class = "mpegts_mux",
    ...
    .ic_properties                            = (const property_t[]){
        ...
        },
        {
            ...
            .name = N_("First scan"),
            .desc =
                N_("When the mux was successfully scanned for the first time."),
...
static void mpegts_mux_stop(mpegts_mux_t* mm, int force, int reason) {
  ...
  if (mi->mi_linked) {
    ...
    if (mi2 && (mmi2 = LIST_FIRST(&mi2->mi_mux_active)) != NULL) {
      if (mmi2 &&
          !mpegts_mux_has_subscribers(mmi2->mmi_mux,
              mmi2->mmi_mux->mm_nicename)) {
        s              = mi2->mi_linked;
        mi2->mi_linked = NULL;
        ...
        if (!force) {
          tvhtrace(LS_MPEGTS,
              "%s - keeping subscribed (linked tuner active)",
              mm->mm_nicename);
          return;
        }
      }
...
void mpegts_mux_open_table(mpegts_mux_t* mm,
    mpegts_table_t*                      mt,
    int                                  subscribe) {
  mpegts_input_t* mi;
...
void mpegts_mux_scan_done(mpegts_mux_t* mm, const char* buf, int res) {
  ...
  if (res < 0) {
    ...
    if (incomplete > 0 && total > incomplete && incomplete <= 3) {
      tvhinfo(LS_MPEGTS,
          "%s - scan complete (partial - %d/%d tables)",
          buf,
          incomplete,
          total);
...
int mpegts_mux_compare(mpegts_mux_t* a, mpegts_mux_t* b) {
  int r =
      uuid_cmp(&a->mm_network->mn_id.in_uuid, &b->mm_network->mn_id.in_uuid);
  if (r)
    return r;
