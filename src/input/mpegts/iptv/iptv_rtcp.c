...
rtcp_send_nak(rtcp_t* rtcp_info, uint32_t ssrc, uint16_t seqn, uint16_t len) {
  ...
  tvhinfo(LS_IPTV,
      "RTCP: Sending NAK report for SSRC 0x%x, missing: %d, following packets: "
      "%d",
      ssrc,
      seqn,
      len - 1);
...
int rtcp_connect(rtcp_t* info,
  ...
  if (info->connection == NULL) {
    ...
    if (rtcp_conn == NULL || rtcp_conn == UDP_FATAL_ERROR) {
      tvhwarn(LS_IPTV,
          "%s - Unable to bind, RTCP won't be available",
          nicename);
      return -1;
    }
  ...
  if (host == NULL) {
    if (urlparse(url ?: "", &rtcp_url)) {
      tvhwarn(LS_IPTV,
          "%s - invalid RTCP URL, should be rtp://HOST:PORT [%s]",
          nicename,
          url);
      goto fail;
    }
  ...
  if (udp_connect(info->connection, "rtcp", host, port)) {
    tvhwarn(LS_IPTV,
        "%s - Unable to connect, RTCP won't be available",
        nicename);
    goto fail;
  }
  urlreset(&rtcp_url);
