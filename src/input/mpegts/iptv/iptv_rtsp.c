...
static int iptv_rtsp_header(http_client_t* hc) {
  ...
  switch (hc->hc_cmd) {
    case RTSP_CMD_DESCRIBE:
      ...
      if (hc->hc_code == HTTP_STATUS_MOVED || hc->hc_code == HTTP_STATUS_FOUND ||
          hc->hc_code == HTTP_STATUS_SEE_OTHER || hc->hc_code == HTTP_STATUS_NOT_MODIFIED) {
        ...
        if (p == NULL) {
          tvherror(LS_IPTV,
              "received code %d from RTSP server but no new location given for '%s'",
              hc->hc_code,
              im->mm_iptv_url_raw);
          return -1;
        }
    ...
    case RTSP_CMD_PLAY:
      ...
      // Use the HTTP host for sending RTCP reports, NOT the hc_rtp_dest (which is where the stream
      // is sent)
      ...
      if (rtsp_play_decode(hc) == 0) {
        if (rp->start_position == 0)
          ...
          tvhdebug(LS_IPTV, "rtsp: position update: %" PRItime_t, hc->hc_rtsp_stream_start);
        }
      } else
 ...
 */
static int iptv_rtsp_start(iptv_input_t* mi, iptv_mux_t* im, const char* raw, const url_t* u) {
  rtsp_priv_t*      rp;
  http_client_t*    hc;
  udp_connection_t *rtp, *rtcp;
  int               r;

  if (!(hc = http_client_connect(im, RTSP_VERSION_1_0, u->scheme, u->host, u->port, NULL)))
 ...
 */
#if ENABLE_TIMESHIFT
static void rtsp_timeshift_fill_status(rtsp_st_t* ts, rtsp_priv_t* rp, timeshift_status_t* status) {
