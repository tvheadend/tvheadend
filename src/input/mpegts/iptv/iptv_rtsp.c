 ...
 */
static void
iptv_rtsp_stop(iptv_mux_t *im)
{
    iptv_rtsp_info_t *rtsp_info = (iptv_rtsp_info_t *) im->im_data;
...
/*
 ...
 The file descriptor will be filled with the socket number that will receive RTP packets.
 */
static int
iptv_rtsp_start(iptv_mux_t *im, const url_t *url)
{
  ...
  rtsp_info->curl = curl;
  rtsp_info->uri = url->raw;
  rtsp_info->client_addr = NULL;
  rtsp_info->is_initialized = 0;
  im->im_data = rtsp_info;
  
  result = curl_easy_setopt(curl, CURLOPT_URL, rtsp_info->uri);
  ...
  result = rtsp_options(curl, rtsp_info->uri, response);
  ...
  if(iptv_rtsp_bind(rtsp_info, &im->mm_iptv_fd, "0") == -1)
  {
    iptv_rtsp_stop(im);
    return -1;
  }
  ...
  result = rtsp_setup(curl, rtsp_info->uri, transport, response);
  ...
  iptv_rtsp_check_client_port(rtsp_info, &im->mm_iptv_fd);
  
  // TODO: check that the server answer for client_port is correct
  
  result = rtsp_play(curl, rtsp_info->uri, "0.000-", response);
  header = response->header_head.lh_first->next.le_next;
  ...
  if(result != CURLE_OK)
  {
    tvhlog(LOG_ERR, "IPTV", "RTSP initialization failed for %s", rtsp_info->uri);
    iptv_rtsp_stop(im);
    return -1;
  }
...
static ssize_t
iptv_rtsp_read ( iptv_mux_t *im, size_t *off )
{
    ssize_t len = iptv_rtp_read(im, off);
    if(len > 0) {
        iptv_rtsp_info_t *info = (iptv_rtsp_info_t *) im->im_data;
        rtcp_receiver_update(info->rtcp_info, im->mm_iptv_buffer.sb_data);
    }
    return len; 
}
