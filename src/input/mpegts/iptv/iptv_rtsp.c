 ...
 *
 */
static void iptv_rtsp_close_cb(void* aux) {
 ...
 * Alive timeout
 */
static void iptv_rtsp_alive_cb(void* aux) {
  iptv_mux_t*  im = aux;
 ...
 * Connected
 */
static int iptv_rtsp_header(http_client_t* hc) {
  ...
  if (hc->hc_cmd == RTSP_CMD_GET_PARAMETER && hc->hc_code != HTTP_STATUS_OK) {
    tvhtrace(LS_IPTV,
        "GET_PARAMETER command returned invalid error code %d for '%s', "
        "fall back to OPTIONS in keep alive loop.",
        hc->hc_code,
        im->mm_iptv_url_raw);
  ...
  if (hc->hc_cmd == RTSP_CMD_DESCRIBE && hc->hc_code != HTTP_STATUS_OK &&
    ...
    tvherror(LS_IPTV,
        "DESCRIBE request returned an invalid error code (%d) for '%s', "
        "fall back to GET_PARAMETER in keep alive loop.",
        hc->hc_code,
        im->mm_iptv_url_raw);
  ...
  switch (hc->hc_cmd) {
    case RTSP_CMD_DESCRIBE:
      ...
      if (hc->hc_code == HTTP_STATUS_MOVED ||
        ...
        if (!hc->hc_handle_location) {
          tvherror(LS_IPTV,
              "received code %d from RTSP server but redirects disabled '%s'",
              hc->hc_code,
              im->mm_iptv_url_raw);
          return -1;
        }
        ...
        if (p == NULL) {
          tvherror(LS_IPTV,
              "received code %d from RTSP server but no new location given for "
              "'%s'",
              hc->hc_code,
              im->mm_iptv_url_raw);
          return -1;
        ...
        tvhinfo(LS_IPTV,
            "received new location from RTSP server (code: %d) '%s' was '%s'",
            hc->hc_code,
            p,
            im->mm_iptv_url_raw);
      }
    ...
    case RTSP_CMD_PLAY:
      ...
      // Use the HTTP host for sending RTCP reports, NOT the hc_rtp_dest (which
      // is where the stream is sent)
      if (im->mm_iptv_ret_url) {
        if (rtcp_connect(&im->im_rtcp_info,
                im->mm_iptv_ret_url,
                NULL,
                0,
      ...
      } else if (rtcp_connect(&im->im_rtcp_info,
                     NULL,
                     hc->hc_host,
                     hc->hc_rtcp_server_port,
                     im->mm_iptv_interface,
                     im->mm_nicename) == 0) {
        im->im_use_retransmission = 1;
      }
      if (rtsp_play_decode(hc) == 0) {
        if (rp->start_position == 0)
          ...
          tvhdebug(LS_IPTV,
              "rtsp: position update: %" PRItime_t,
 ...
 * Receive data
 */
static int iptv_rtsp_data(http_client_t* hc, void* buf, size_t len) {
  rtsp_priv_t* rp;
  iptv_mux_t*  im = hc->hc_aux;
  int          r;
  ...
  switch (hc->hc_cmd) {
    case RTSP_CMD_DESCRIBE:
      ...
      if (rtsp_describe_decode(hc, buf, len) >= 0) {
        ...
        tvhdebug(LS_IPTV,
            "rtsp: buffer update, start: %" PRItime_t ", end: %" PRItime_t,
            rp->range_start,
            rp->range_end);
      }
      if (rp->play) {
    ...
    default:
      if (len > 0) {
        tvherror(LS_IPTV,
            "unknown data %zd received for '%s':\n%s",
            len,
            im->mm_iptv_url_raw,
            (char*)buf);
      }
 ...
 */
static int iptv_rtsp_start(iptv_input_t* mi,
    iptv_mux_t*                          im,
    const char*                          raw,
    const url_t*                         u) {
  rtsp_priv_t*      rp;
  ...
  hc->hc_rtsp_keep_alive_cmd =
      RTSP_CMD_DESCRIBE;    /* start keep alive loop with DESCRIBE */
 ...
 * Stop connection
 */
static void iptv_rtsp_stop(iptv_input_t* mi, iptv_mux_t* im) {
  rtsp_priv_t* rp = im->im_data;
  int          play;
...
static void iptv_rtp_header_callback(iptv_mux_t* im, uint8_t* rtp, int len) {
 ...
 * Read data
 */
static ssize_t iptv_rtsp_read(iptv_input_t* mi, iptv_mux_t* im) {
  ssize_t r;
 ...
 */
#if ENABLE_TIMESHIFT
static void rtsp_timeshift_fill_status(rtsp_st_t* ts,
    rtsp_priv_t*                                  rp,
  ...
  tvhdebug(LS_TIMESHIFT,
      "remote ts status start %" PRId64 " end %" PRId64 " current %" PRId64,
      start,
      end,
      current);
...
static void rtsp_timeshift_status(rtsp_st_t* pd, rtsp_priv_t* rp) {
...
static void rtsp_input(void* opaque, streaming_message_t* sm) {
  ...
  switch (sm->sm_type) {
    ...
    case SMT_SKIP:
      ...
      rtsp_set_position(rp->hc, rp->range_start + ts_rescale(data->time, 1));
      tvhinfo(LS_IPTV,
          "rtsp: skip: %" PRItime_t " + %" PRId64,
          rp->range_start,
...
static htsmsg_t* rtsp_input_info(void* opaque, htsmsg_t* list) {
  return list;
}

static streaming_ops_t rtsp_input_ops = {.st_cb = rtsp_input,
    .st_info                                    = rtsp_input_info};

streaming_target_t* rtsp_st_create(streaming_target_t* out,
...
void iptv_rtsp_init(void) {
  static iptv_handler_t ih[] = {{.scheme          = "rtsp",
