...
static int ih_cmp(iptv_handler_t* a, iptv_handler_t* b) {
...
void iptv_handler_register(iptv_handler_t* ih, int num) {
  ...
  while (num) {
    ...
    if (r)
      tvhwarn(LS_IPTV, "attempt to re-register handler for %s", ih->scheme);
    num--;
    ih++;
  }
}

static iptv_handler_t* iptv_handler_find(const char* scheme) {
  iptv_handler_t ih;
  ih.scheme = scheme;
...
static int iptv_input_thread_number(iptv_input_t* mi) {
...
static void
iptv_input_class_get_title(idnode_t* self, const char* lang, char* dst, size_t dstsize) {
...
const idclass_t        iptv_input_class = {.ic_super = &mpegts_input_class,
           .ic_class                                 = "iptv_input",
...
static mpegts_mux_instance_t* iptv_input_is_free(mpegts_input_t* mi,
    mpegts_mux_t*                                                mm,
    iptv_is_free_t*                                              conf,
    int                                                          weight,
    int*                                                         lweight) {
...
static int iptv_input_thread_balance(iptv_input_t* mi) {
...
static int iptv_input_is_enabled(mpegts_input_t* mi, mpegts_mux_t* mm, int flags, int weight) {
  int                    r;
...
static int iptv_input_get_weight(mpegts_input_t* mi, mpegts_mux_t* mm, int flags, int weight) {
  int                    w;
  mpegts_mux_instance_t* mmi;
...
static int iptv_input_get_grace(mpegts_input_t* mi, mpegts_mux_t* mm) {
  iptv_mux_t*     im = (iptv_mux_t*)mm;
  iptv_network_t* in = (iptv_network_t*)im->mm_network;
  return in->in_max_timeout;
}

static int iptv_input_get_priority(mpegts_input_t* mi, mpegts_mux_t* mm, int flags) {
  iptv_mux_t*     im = (iptv_mux_t*)mm;
  iptv_network_t* in = (iptv_network_t*)im->mm_network;
...
static int iptv_input_warm_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
...
static const char* iptv_sub_url_encode(iptv_mux_t* im, const char* s, char* tmp, size_t tmplen) {
  char* p;
  if (im->mm_iptv_url &&
      (!strncmp(im->mm_iptv_url, "pipe://", 7) || !strncmp(im->mm_iptv_url, "file://", 7)))
    return s;
  p = url_encode(s);
...
static htsstr_substitute_t iptv_input_subst[] = {{.id = "m", .getval = iptv_sub_mux_name},
    {.id = "n", .getval = iptv_sub_service_name},
    {.id = "w", .getval = iptv_sub_weight},
    {.id = NULL, .getval = NULL}};

static int iptv_input_start_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi, int weight) {
...
static void iptv_input_stop_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
  iptv_mux_t*         im   = (iptv_mux_t*)mmi->mmi_mux;
  iptv_thread_pool_t* pool = ((iptv_input_t*)mi)->mi_tpool;
  uint32_t            u32;
...
static void iptv_input_display_name(mpegts_input_t* mi, char* buf, size_t len) {
  snprintf(buf, len, "IPTV #%d", iptv_input_thread_number((iptv_input_t*)mi));
}

static inline int iptv_input_pause_check(iptv_mux_t* im) {
  ...
  im->im_pcr &= PTS_MASK;
  if (old != im->im_pcr)
    tvhtrace(LS_IPTV_PCR,
        "updated %" PRId64 ", time start %" PRId64 ", limit %" PRId64 ", diff %" PRId64,
        im->im_pcr,
        im->im_pcr_start,
        limit,
        im->im_pcr_end - im->im_pcr_start);
...
static void* iptv_input_thread(void* aux) {
  ...
  while (tvheadend_is_running()) {
    ...
    if (nfds < 0) {
      if (tvheadend_is_running() && !ERRNO_AGAIN(errno)) {
        tvherror(LS_IPTV, "poll() error %s, sleeping 1 second", strerror(errno));
        sleep(1);
      }
      continue;
    } else if (nfds == 0) {
...
void iptv_input_recv_flush(iptv_mux_t* im) {
  ...
  if (mmi == NULL)
    return;
  mpegts_input_recv_packets(mmi, &im->mm_iptv_buffer, MPEGTS_DATA_CC_RESTART, NULL);
}
...
int iptv_input_recv_packets(iptv_mux_t* im, ssize_t len) {
  ...
  if (mono2sec(in->in_bandwidth_clock) != mono2sec(s64)) {
    if (in->in_max_bandwidth && in->in_bps > in->in_max_bandwidth * 1024) {
      if (!in->in_bw_limited) {
        tvhinfo(LS_IPTV,
            "%s bandwidth limited exceeded",
  ...
  if (mmi) {
    ...
    mpegts_input_recv_packets(mmi,
        &im->mm_iptv_buffer,
        in->in_remove_scrambled_bits ? MPEGTS_DATA_REMOVE_SCRAMBLED : 0,
        &pcr);
    if (pcr.pcr_first != PTS_UNSET && pcr.pcr_last != PTS_UNSET) {
      ...
      if (im->im_pcr == PTS_UNSET) {
        ...
        if (s64 != PTS_UNSET) {
          ...
          tvhtrace(LS_IPTV_PCR,
              "first %" PRId64 " last %" PRId64 ", time start %" PRId64 ", end %" PRId64,
              pcr.pcr_first,
              pcr.pcr_last,
              im->im_pcr_start,
              im->im_pcr_end);
...
void iptv_input_mux_started(iptv_input_t* mi, iptv_mux_t* im, int reset) {
  ...
  if (mm->mm_active)
    psi_tables_install((mpegts_input_t*)mi, mm, im->mm_iptv_atsc ? DVB_SYS_ATSC_ALL : DVB_SYS_DVBT);
}

static int iptv_network_bouquet_source(mpegts_network_t* mn, char* source, size_t len) {
...
static int iptv_network_bouquet_comment(mpegts_network_t* mn, char* comment, size_t len) {
  iptv_network_t* in = (iptv_network_t*)mn;
...
static void iptv_network_delete(mpegts_network_t* mn, int delconf) {
  iptv_network_t* in            = (iptv_network_t*)mn;
  char*           url           = in->in_url;
  ...
  /* Remove config */
  if (delconf)
    hts_settings_remove("input/iptv/networks/%s", idnode_uuid_as_str(&in->mn_id, ubuf));
...
static void iptv_network_class_delete(idnode_t* in) {
  return iptv_network_delete((mpegts_network_t*)in, 1);
}

static int iptv_network_class_icon_url_set(void* in, const void* v) {
...
const idclass_t        iptv_network_class = {.ic_super = &mpegts_network_class,
...
static int iptv_auto_network_class_url_set(void* in, const void* v) {
  iptv_network_t* mn = in;
  return iptv_url_set(&mn->in_url, &mn->in_url_sane, v, 1, 1);
}

static void iptv_auto_network_class_notify_url(void* in, const char* lang) {
  iptv_auto_network_trigger(in);
}

static htsmsg_t* iptv_auto_network_class_charset_list(void* o, const char* lang) {
...
const idclass_t iptv_auto_network_class = {.ic_super = &iptv_network_class,
    ...
    .ic_properties                                   = (const property_t[]){{.type      = PT_STR,
                                          ...
                                          {.type    = PT_INT,
                                              ...
                                              .desc =
                                                  N_("Ignore last components in path. The defined count "
...
static mpegts_mux_t* iptv_network_create_mux2(mpegts_network_t* mn, htsmsg_t* conf) {
  iptv_mux_t* im = iptv_mux_create0((iptv_network_t*)mn, NULL, conf);
  return mpegts_mux_post_create((mpegts_mux_t*)im);
}

static void iptv_network_auto_scan(mpegts_network_t* mn) {
  iptv_auto_network_trigger((iptv_network_t*)mn);
  mpegts_network_scan(mn);
}

static mpegts_service_t*
iptv_network_create_service(mpegts_mux_t* mm, uint16_t sid, uint16_t pmt_pid) {
  return (mpegts_service_t*)iptv_service_create0((iptv_mux_t*)mm, sid, pmt_pid, NULL, NULL);
}

static const idclass_t* iptv_network_mux_class(mpegts_network_t* mm) {
  extern const idclass_t iptv_mux_class;
  return &iptv_mux_class;
}

static htsmsg_t* iptv_network_config_save(mpegts_network_t* mn, char* filename, size_t fsize) {
...
iptv_network_t* iptv_network_create0(const char* uuid, htsmsg_t* conf, const idclass_t* idc) {
  ...
  if (!mpegts_network_create0((mpegts_network_t*)in, idc, uuid, NULL, conf)) {
    free(in->in_remove_args);
    free(in);
    return NULL;
  }
...
static mpegts_network_t* iptv_network_builder(const idclass_t* idc, htsmsg_t* conf) {
  mpegts_network_t*   mn;
  iptv_thread_pool_t* pool;
...
static void iptv_network_init(void) {
  htsmsg_t *      c, *e;
  htsmsg_field_t* f;
  ...
  /* Register builders */
  mpegts_network_register_builder(&iptv_network_class, iptv_network_builder);
  mpegts_network_register_builder(&iptv_auto_network_class, iptv_network_builder);
...
static mpegts_network_t* iptv_input_wizard_network(iptv_input_t* lfe) {
  return (mpegts_network_t*)LIST_FIRST(&lfe->mi_networks);
}

static htsmsg_t* iptv_input_wizard_get(tvh_input_t* ti, const char* lang) {
...
static void iptv_input_wizard_set(tvh_input_t* ti, htsmsg_t* conf, const char* lang) {
  iptv_input_t*     mi    = (iptv_input_t*)ti;
...
static iptv_input_t* iptv_create_input(void* tpool) {
  iptv_input_t*     input = calloc(1, sizeof(iptv_input_t));
  mpegts_network_t* mn;

  /* Init Input */
  mpegts_input_create0((mpegts_input_t*)input, &iptv_input_class, NULL, NULL);
