...
static void iptv_input_class_get_title(idnode_t* self,
    const char*                                  lang,
    char*                                        dst,
    size_t                                       dstsize) {
  ...
  snprintf(dst,
      dstsize,
      "%s%d",
      tvh_gettext_lang(lang, N_("IPTV thread #")),
      num);
}
...
static mpegts_mux_instance_t* iptv_input_is_free(mpegts_input_t* mi,
  ...
  /* If we are checking if an input is free, return null if the mux's input is
   * already running */
...
static int iptv_input_is_enabled(mpegts_input_t* mi,
    mpegts_mux_t*                                mm,
    int                                          flags,
    int                                          weight) {
  int                    r;
...
static int iptv_input_get_weight(mpegts_input_t* mi,
    mpegts_mux_t*                                mm,
    int                                          flags,
    int                                          weight) {
  int                    w;
...
static int iptv_input_warm_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
  ...
  lmmi =
      iptv_input_is_free(mi, mmi->mmi_mux, &conf, mmi->mmi_start_weight, NULL);
  tvhtrace(LS_IPTV_SUB,
      "warm mux[%p]: %p (%d)",
      im,
      lmmi,
      mmi->mmi_start_weight);
  if (lmmi) {
    /* Stop */
...
static const char* iptv_sub_mux_name(const char* id,
    const char*                                  fmt,
    const void*                                  aux,
    char*                                        tmp,
    size_t                                       tmplen) {
...
static const char* iptv_sub_service_name(const char* id,
    const char*                                      fmt,
    const void*                                      aux,
    char*                                            tmp,
    size_t                                           tmplen) {
...
static const char* iptv_sub_weight(const char* id,
    const char*                                fmt,
    const void*                                aux,
    char*                                      tmp,
    size_t                                     tmplen) {
...
static int iptv_input_start_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                      mmi,
    int                                         weight) {
  ...
  if (im->mm_iptv_url_raw) {
    im->mm_active =
        mmi; // Note: must set here else mux_started call
...
static void iptv_input_stop_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                      mmi) {
  iptv_mux_t*         im   = (iptv_mux_t*)mmi->mmi_mux;
  iptv_thread_pool_t* pool = ((iptv_input_t*)mi)->mi_tpool;
  uint32_t            u32;
  ...
  if (u32 == 0)
    gtimer_arm_rel(&iptv_tpool_manage_timer,
        iptv_input_thread_manage_cb,
        NULL,
        0);
}
...
static inline int iptv_input_pause_check(iptv_mux_t* im) {
  ...
  if (old != im->im_pcr)
    tvhtrace(LS_IPTV_PCR,
        "updated %" PRId64 ", time start %" PRId64 ", limit %" PRId64
        ", diff %" PRId64,
        im->im_pcr,
        im->im_pcr_start,
        limit,
  ...
  while (tvheadend_is_running()) {
    ...
    if (nfds < 0) {
      if (tvheadend_is_running() && !ERRNO_AGAIN(errno)) {
        tvherror(LS_IPTV,
            "poll() error %s, sleeping 1 second",
            strerror(errno));
        sleep(1);
      }
      continue;
    ...
    if (r == 1) {
      ...
      if (im->mm_active)
        mtimer_arm_rel(&im->im_pause_timer,
            iptv_input_unpause,
            im,
            sec2mono(1));
    ...
    if (pcr.pcr_first != PTS_UNSET && pcr.pcr_last != PTS_UNSET) {
      ...
      if (im->im_pcr == PTS_UNSET) {
        ...
        if (s64 != PTS_UNSET) {
          ...
          tvhtrace(LS_IPTV_PCR,
              "first %" PRId64 " last %" PRId64 ", time start %" PRId64
              ", end %" PRId64,
              pcr.pcr_first,
              pcr.pcr_last,
              im->im_pcr_start,
...
void iptv_input_mux_started(iptv_input_t* mi, iptv_mux_t* im, int reset) {
  ...
  if (mm->mm_active)
    psi_tables_install((mpegts_input_t*)mi,
        mm,
...
static htsmsg_t* iptv_auto_network_class_charset_list(void* o,
    const char*                                             lang) {
        ...
        (const property_t[]){{.type      = PT_STR,
            ...
            {.type     = PT_INT,
                ...
                .desc  = N_("Ignore last components in path. The defined count "
                           ...
                           "when the identical source is compared - see Help "
...
static mpegts_mux_t* iptv_network_create_mux2(mpegts_network_t* mn,
    htsmsg_t*                                                   conf) {
...
iptv_network_t*
iptv_network_create0(const char* uuid, htsmsg_t* conf, const idclass_t* idc) {
...
static mpegts_network_t* iptv_network_builder(const idclass_t* idc,
    htsmsg_t*                                                  conf) {
  mpegts_network_t*   mn;
  iptv_thread_pool_t* pool;
