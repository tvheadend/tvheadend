...
static int _epgcfg_from_str(const char* str, char* manual, size_t manual_len) {
  static struct strtab cfgs[] = {{"0", MM_EPG_DISABLE},
      {"none", MM_EPG_DISABLE},
 ...
 *
 */
static void iptv_auto_network_process_m3u_item(iptv_network_t* in,
    const char*                                                last_url,
  ...
  if (url == NULL ||
      (strncmp(url, "file://", 7) && strncmp(url, "pipe://", 7) &&
          strncmp(url, "http://", 7) && strncmp(url, "https://", 8) &&
          strncmp(url, "rtsp://", 7) && strncmp(url, "rtsps://", 8) &&
          strncmp(url, "udp://", 6) && strncmp(url, "rtp://", 6)))
    return;

  epgid = htsmsg_get_str(item, "tvh-chnum");
  if (!epgid)
    epgid = htsmsg_get_str(item, "tvg-chno");
  if (!epgid)
    epgid = htsmsg_get_str(item, "channel-number");
  ...
  if (!tags)
    tags = htsmsg_get_str(item, "group-title");
  if (tags) {
...
skip_url:
  ...
  LIST_FOREACH (mm, &in->mn_muxes, mm_network_link) {
    ...
    if (strcmp(im->mm_iptv_url_cmpid ?: (im->mm_iptv_url ?: ""), url2) == 0) {
      ...
      if (im->mm_iptv_chnum != chnum) {
        mpegts_network_bouquet_trigger((mpegts_network_t*)in,
            0); /* propagate LCN change */
        im->mm_iptv_chnum = chnum;
        change            = 1;
      }
      if ((im->mm_iptv_muxname == NULL || im->mm_iptv_muxname[0] == '\0') &&
          n && *n) {
  ...
  if (!htsmsg_get_s64(item, "vlc-program", &vlcprog) && vlcprog > 1 &&
      vlcprog < 8191)
 ...
 *
 */
static int iptv_auto_network_process_m3u(iptv_network_t* in,
    char*                                                data,
    const char*                                          last_url,
    const char*                                          host_url,
  ...
  if (items) {
    HTSMSG_FOREACH(f, items) {
      if ((item = htsmsg_field_get_map(f)) == NULL)
        continue;
      iptv_auto_network_process_m3u_item(in,
          last_url,
          remove_args,
          ignore_args,
          ignore_path,
  ...
  if (total == 0)
    ...
    tvhinfo(LS_IPTV,
        "m3u parse: %d new mux(es) in network '%s' (total %d)",
        count,
        in->mn_network_name,
        total);
  return ret;
 ...
 *
 */
static int iptv_auto_network_process(void* aux,
    const char*                            last_url,
    const char*                            host_url,
    char*                                  data,
    size_t                                 len) {
  ...
  if (!strncmp(data, "#EXTM3U", 7))
    r = iptv_auto_network_process_m3u(in,
        data,
        last_url,
        host_url,
        &remove_args,
  ...
  if (r == 0) {
    ...
    if (count > 0)
      tvhinfo(LS_IPTV,
          "removed %d mux(es) from network '%s'",
          count,
          in->mn_network_name);
  } else {
    LIST_FOREACH (mm, &in->mn_muxes, mm_network_link)
    ...
    tvherror(LS_IPTV,
        "unknown playlist format for network '%s'",
        in->mn_network_name);
  }
 ...
 *
 */
static void iptv_auto_network_stop(void* aux) {
  auto_private_t* ap = aux;
 ...
 *
 */
static void iptv_auto_network_trigger0(void* aux) {
  auto_private_t* ap = aux;
  iptv_network_t* in = ap->in_network;

  if (in->mn_enabled)
    download_start(&ap->in_download, in->in_url, ap);
  mtimer_arm_rel(&ap->in_auto_timer,
      iptv_auto_network_trigger0,
      ap,
