...
static inline void
iptv_url_set0(char** url, char** sane_url, const char* url1, const char* sane_url1) {
  free(*url);
  free(*sane_url);
...
static int iptv_mux_url_set(void* p, const void* v) {
  iptv_mux_t* im = p;
  return iptv_url_set(&im->mm_iptv_url, &im->mm_iptv_url_sane, v, 1, 1);
}

static int iptv_mux_ret_url_set(void* p, const void* v) {
  iptv_mux_t* im = p;
  return iptv_url_set(&im->mm_iptv_ret_url, &im->mm_iptv_ret_url_sane, v, 0, 0);
}

#if ENABLE_LIBAV
static htsmsg_t* iptv_mux_libav_enum(void* o, const char* lang) {
...
const idclass_t iptv_mux_class = {.ic_super = &mpegts_mux_class,
    .ic_class                               = "iptv_mux",
...
static htsmsg_t* iptv_mux_config_save(mpegts_mux_t* mm, char* filename, size_t fsize) {
...
static void iptv_mux_free(mpegts_mux_t* mm) {
...
static void iptv_mux_delete(mpegts_mux_t* mm, int delconf) {
  char ubuf1[UUID_HEX_SIZE];
  char ubuf2[UUID_HEX_SIZE];
...
static void iptv_mux_display_name(mpegts_mux_t* mm, char* buf, size_t len) {
  iptv_mux_t* im = (iptv_mux_t*)mm;
 ...
 * Create
 */
iptv_mux_t* iptv_mux_create0(iptv_network_t* in, const char* uuid, htsmsg_t* conf) {
  htsmsg_t *      c, *c2, *e;
  htsmsg_field_t* f;
  iptv_service_t* ms;
  char            ubuf1[UUID_HEX_SIZE];
  ...
  /* Create Mux */
  iptv_mux_t* im = mpegts_mux_create(iptv_mux,
      uuid,
      (mpegts_network_t*)in,
      MPEGTS_ONID_NONE,
      MPEGTS_TSID_NONE,
  ...
  if (c == NULL)
    c = c2 = hts_settings_load_r(1,
        "input/iptv/networks/%s/muxes/%s/services",
