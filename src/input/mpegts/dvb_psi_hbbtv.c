 ...
 * Extract Hbbtv
 */
htsmsg_t* dvb_psi_parse_hbbtv(mpegts_psi_table_t* mt,
    const uint8_t*                                buf,
    int                                           len,
    int*                                          _sect) {
  ...
  DVB_DESC_FOREACH(mt, p, l, 7, dlptr, dllen, dtag, dlen, dptr) {
    ...
    switch (dtag) {
      case DVB_DESC_APP:
        ...
        while (dlen-- > 0) {
          tvhtrace(mt->mt_subsys,
              "%s:     transport protocol label %02X",
              mt->mt_name,
              dptr[0]);
          dptr++;
        }
      ...
      case DVB_DESC_APP_NAME:
        ...
        while (dlen > 4) {
          r = dvb_get_string_with_len(title,
              sizeof(title),
              dptr + 3,
              dlen - 3,
              "UTF-8",
      ...
      case DVB_DESC_APP_TRANSPORT:
        ...
        if (protocol_id == 0x0003) {
          r = dvb_get_string_with_len(name,
              sizeof(name),
              dptr,
              dlen,
              "UTF-8",
...
int dvb_hbbtv_callback(mpegts_table_t* mt,
    const uint8_t*                     buf,
    int                                len,
    int                                tableid) {
