...
static void mpegts_network_class_notify_enabled(void* obj, const char* lang) {
...
static htsmsg_t* mpegts_network_class_save(idnode_t* in, char* filename, size_t fsize) {
...
static void
mpegts_network_class_get_title(idnode_t* in, const char* lang, char* dst, size_t dstsize) {
  mpegts_network_t* mn = (mpegts_network_t*)in;
  *dst                 = 0;
...
static const void* mpegts_network_class_get_num_mux(void* ptr) {
  static int        n;
  mpegts_mux_t*     mm;
...
static const void* mpegts_network_class_get_num_svc(void* ptr) {
  static int        n;
  mpegts_mux_t*     mm;
  mpegts_service_t* s;
  mpegts_network_t* mn = ptr;
...
static const void* mpegts_network_class_get_num_chn(void* ptr) {
  static int             n;
  mpegts_mux_t*          mm;
  mpegts_service_t*      s;
  mpegts_network_t*      mn = ptr;
...
static const void* mpegts_network_class_get_scanq_length(void* ptr) {
  static int        n;
  mpegts_mux_t*     mm;
  mpegts_network_t* mn = ptr;
...
static void mpegts_network_class_idlescan_notify(void* p, const char* lang) {
  mpegts_network_t* mn = p;
  mpegts_mux_t*     mm;
  LIST_FOREACH (mm, &mn->mn_muxes, mm_network_link) {
    if (mn->mn_idlescan)
      mpegts_network_scan_queue_add(mm, SUBSCRIPTION_PRIO_SCAN_IDLE, SUBSCRIPTION_IDLESCAN, 0);
...
static htsmsg_t* mpegts_network_discovery_enum(void* o, const char* lang) {
...
static void mpegts_network_class_notify_bouquet(void* in, const char* lang) {
...
const idclass_t mpegts_network_class = {.ic_class = "mpegts_network",
...
static void mpegts_network_display_name(mpegts_network_t* mn, char* buf, size_t len) {
  strlcpy(buf, tvh_str_default(mn->mn_network_name, "unknown"), len);
}

static bouquet_t* mpegts_network_bouquet_get(mpegts_network_t* mn, int create) {
  char buf[128];
...
static void mpegts_network_bouquet_update(void* aux) {
...
void mpegts_network_bouquet_trigger0(mpegts_network_t* mn, int timeout) {
  mtimer_arm_rel(&mn->mn_bouquet_timer, mpegts_network_bouquet_update, mn, sec2mono(timeout));
}

static int mpegts_network_bouquet_source(mpegts_network_t* mn, char* source, size_t len) {
  char ubuf[UUID_HEX_SIZE];
  snprintf(source, len, "mpegts-network://%s", idnode_uuid_as_str(&mn->mn_id, ubuf));
  return 0;
}

static int mpegts_network_bouquet_comment(mpegts_network_t* mn, char* comment, size_t len) {
...
static htsmsg_t* mpegts_network_config_save(mpegts_network_t* mn, char* filename, size_t size) {
  // Nothing - leave to child classes
  return NULL;
}

static mpegts_mux_t* mpegts_network_create_mux(mpegts_network_t* mn,
    void*                                                        origin,
    uint32_t                                                     sid,
    uint32_t                                                     tsid,
    void*                                                        aux,
...
mpegts_network_create_service(mpegts_mux_t* mm, uint16_t sid, uint16_t pmt_pid) {
  return NULL;
}

static const idclass_t* mpegts_network_mux_class(mpegts_network_t* mn) {
  extern const idclass_t mpegts_mux_class;
  return &mpegts_mux_class;
}

static mpegts_mux_t* mpegts_network_mux_create2(mpegts_network_t* mn, htsmsg_t* conf) {
  return NULL;
}
...
static void mpegts_network_link_delete(mpegts_network_link_t* mnl) {
...
mpegts_network_find_mux(mpegts_network_t* mn, uint32_t onid, uint32_t tsid, int check) {
  ...
  LIST_FOREACH (mm, &mn->mn_muxes, mm_network_link) {
    if (mm->mm_onid && onid && mm->mm_onid != onid)
      continue;
    if (mm->mm_tsid == tsid) {
