...
static const void* mpegts_service_class_get_mux(void* ptr) {
...
static const void* mpegts_service_class_get_mux_uuid(void* ptr) {
  mpegts_service_t* ms = ptr;
  if (ms && ms->s_dvb_mux)
...
static const void* mpegts_service_class_get_network(void* ptr) {
  static char       buf[512], *s = buf;
  mpegts_service_t* ms = ptr;
...
static htsmsg_t* mpegts_service_pref_capid_lock_list(void* o, const char* lang) {
...
const idclass_t mpegts_service_class = {.ic_super = &service_class,
    ...
    .ic_properties                                = (const property_t[]){
        ...
        },
        {
                                           ...
                                           .desc = N_("Add this value to PTS for the teletext subtitles. The time value is in "
 ...
 */
static int mpegts_service_is_enabled(service_t* t, int flags) {
...
static int mpegts_service_is_enabled_raw(service_t* t, int flags) {
  mpegts_service_t* s  = (mpegts_service_t*)t;
  mpegts_mux_t*     mm = s->s_dvb_mux;
  return mm->mm_is_enabled(mm) ? s->s_enabled : 0;
}
 ...
 * Save
 */
static htsmsg_t* mpegts_service_config_save(service_t* t, char* filename, size_t fsize) {
  if (filename == NULL) {
 ...
 */
static int mpegts_service_enlist_raw(service_t* t,
    tvh_input_t*                                ti,
  ...
  LIST_FOREACH (mmi, &m->mm_instances, mmi_mux_link) {
    ...
    if (r == MI_IS_ENABLED_NEVER) {
      tvhtrace(LS_MPEGTS,
          "enlist: input %p not enabled for mux %p service %s weight %d flags %x",
          mi,
          mmi->mmi_mux,
          s->s_nicename,
          weight,
    ...
    if (r == MI_IS_ENABLED_RETRY) {
      ...
      tvhtrace(LS_MPEGTS,
          "enlist: input %p postponed for mux %p service %s weight %d flags %x",
          mi,
          mmi->mmi_mux,
          s->s_nicename,
          weight,
    ...
    if (mmi->mmi_mux->mm_active == mmi) {
      ...
      if (w > 0 && mi->mi_free_weight && weight >= mi->mi_free_weight && w < mi->mi_free_weight)
        w = 0;
    }
 ...
 * Service instance list
 */
static int mpegts_service_enlist(service_t* t,
    tvh_input_t*                            ti,
    struct service_instance_list*           sil,
    int                                     flags,
    int                                     weight) {
 ...
 * Start service
 */
static int mpegts_service_start(service_t* t, int instance, int weight, int flags) {
  int                    r;
  mpegts_service_t*      s = (mpegts_service_t*)t;
  mpegts_mux_t*          m = s->s_dvb_mux;
  mpegts_mux_instance_t* mmi;
 ...
 * Stop service
 */
static void mpegts_service_stop(service_t* t) {
  mpegts_service_t* s = (mpegts_service_t*)t;
 ...
 * Refresh
 */
static void mpegts_service_refresh(service_t* t) {
  mpegts_service_t* s = (mpegts_service_t*)t;
  mpegts_input_t*   i = s->s_dvb_active_input;
 ...
 * Source info
 */
static void mpegts_service_setsourceinfo(service_t* t, source_info_t* si) {
  char              buf[256];
  mpegts_service_t* s = (mpegts_service_t*)t;
  mpegts_mux_t*     m = s->s_dvb_mux;
 ...
 * Grace period
 */
static int mpegts_service_grace_period(service_t* t) {
  int               r  = 0;
...
static const char* mpegts_service_channel_name(service_t* s) {
  return ((mpegts_service_t*)s)->s_dvb_svcname;
}

static const char* mpegts_service_source(service_t* s) {
#if ENABLE_MPEGTS_DVB
  mpegts_service_t* ms      = (mpegts_service_t*)s;
  const idclass_t*  mux_idc = ms->s_dvb_mux->mm_id.in_class;
  if (mux_idc == &dvb_mux_dvbs_class)
    return "DVB-S";
  if (mux_idc == &dvb_mux_dvbc_class)
    return "DVB-C";
  if (mux_idc == &dvb_mux_dvbt_class)
    return "DVB-T";
  if (mux_idc == &dvb_mux_atsc_t_class)
    return "ATSC-T";
  if (mux_idc == &dvb_mux_atsc_c_class)
    return "ATSC-C";
  if (mux_idc == &dvb_mux_isdb_t_class)
    return "ISDB-T";
  if (mux_idc == &dvb_mux_isdb_c_class)
    return "ISDB-C";
  if (mux_idc == &dvb_mux_isdb_s_class)
    return "ISDB-S";
  if (mux_idc == &dvb_mux_dtmb_class)
    return "DTMB";
  if (mux_idc == &dvb_mux_dab_class)
    return "DAB";
#endif
  return NULL;
}

static const char* mpegts_service_provider_name(service_t* s) {
  return ((mpegts_service_t*)s)->s_dvb_provider;
}

#if ENABLE_MPEGTS_DVB
static int mpegts_picon_mux_isvalid(dvb_mux_t* mux, int orbital_position) {
  switch (mux->mm_onid) {
    case 0:
    case 0x1111:
      return 0;
...
static const char* mpegts_service_channel_icon(service_t* s) {
  /* DVB? */
#if ENABLE_MPEGTS_DVB
  mpegts_service_t*      ms = (mpegts_service_t*)s;
  extern const idclass_t dvb_mux_class;
  if (ms->s_dvb_mux && idnode_is_instance(&ms->s_dvb_mux->mm_id, &dvb_mux_class)) {
    int32_t    hash = 0;
      ...
      case DVB_TYPE_S:
        ...
        if (!mpegts_picon_mux_isvalid(mmd, pos))
          hash |= ((mmd->lm_tuning.dmc_fe_freq / 1000) & 0x7fff) |
              (mmd->lm_tuning.u.dmc_fe_qpsk.orbital_pos == DVB_POLARISATION_HORIZONTAL ? 0x8000
                                                                                       : 0);
        break;
      case DVB_TYPE_C:
...
#if ENABLE_MPEGTS_DVB
static int mpegts_service_match_network(mpegts_network_t* mn, uint32_t hash, const idclass_t* idc) {
  int pos, pos2;

  if (idc != &dvb_mux_dvbs_class)
    return 1;
  pos = hash >> 16;
  if (pos > 3600 || pos < 0)
    return 0;
...
#if ENABLE_MPEGTS_DVB
static int mpegts_service_match_mux(dvb_mux_t* mm, uint32_t hash, const idclass_t* idc) {
...
mpegts_service_find_e2(uint32_t stype, uint32_t sid, uint32_t tsid, uint32_t onid, uint32_t hash) {
  ...
  LIST_FOREACH (mn, &mpegts_network_all, mn_global_link) {
    if (!idnode_is_instance(&mn->mn_id, &dvb_network_class))
      continue;
    if (dvb_network_mux_class(mn) != idc)
      continue;
    if (!mpegts_service_match_network(mn, hash, idc))
      continue;
    LIST_FOREACH (mm, &mn->mn_muxes, mm_network_link) {
      if (!idnode_is_instance(&mm->mm_id, idc))
        continue;
      if (mm->mm_tsid != tsid || mm->mm_onid != onid)
        continue;
      if (!mpegts_service_match_mux((dvb_mux_t*)mm, hash, idc))
        continue;
...
static void mpegts_service_mapped(service_t* t) {
  epggrab_ota_queue_mux(((mpegts_service_t*)t)->s_dvb_mux);
}

void mpegts_service_unref(service_t* t) {
  mpegts_service_t* ms = (mpegts_service_t*)t;
...
void mpegts_service_delete(service_t* t, int delconf) {
...
static int mpegts_service_satip_source(service_t* t) {
  mpegts_service_t* ms = (mpegts_service_t*)t;
  mpegts_network_t* mn = ms->s_dvb_mux ? ms->s_dvb_mux->mm_network : NULL;
  return mn ? mn->mn_satip_source : -1;
}

static mpegts_apids_t* mpegts_service_pid_list_(service_t* t, void* owner) {
  mpegts_service_t* ms   = (mpegts_service_t*)t;
...
static mpegts_apids_t* mpegts_service_pid_list(service_t* t) {
  return mpegts_service_pid_list_(t, t);
}

static void mpegts_service_memoryinfo(service_t* t, int64_t* size) {
  mpegts_service_t* ms = (mpegts_service_t*)t;
  *size += sizeof(*ms);
...
static int mpegts_service_unseen(service_t* t, const char* type, time_t before) {
  mpegts_service_t* ms  = (mpegts_service_t*)t;
  int               pat = type && strcasecmp(type, "pat") == 0;
  if (pat && ms->s_auto != SERVICE_AUTO_PAT_MISSING)
    return 0;
 ...
 * Create service
 */
mpegts_service_t* mpegts_service_create0(mpegts_service_t* s,
  ...
  if (!conf) {
    if (sid)
      s->s_components.set_service_id = sid;
    if (pmt_pid)
      s->s_components.set_pmt_pid = pmt_pid;
  }

  if (service_create0((service_t*)s, STYPE_STD, class, uuid, S_MPEG_TS, conf) == NULL)
    return NULL;
...
mpegts_service_t* mpegts_service_find_by_pid(mpegts_mux_t* mm, int pid) {
  ...
  LIST_FOREACH (s, &mm->mm_services, s_dvb_mux_link) {
    tvh_mutex_lock(&s->s_stream_mutex);
    if (pid == s->s_components.set_pmt_pid || pid == s->s_components.set_pcr_pid)
      goto ok;
...
void mpegts_service_autoenable(mpegts_service_t* s, const char* where) {
  if (!s->s_enabled && s->s_auto == SERVICE_AUTO_PAT_MISSING) {
    tvhinfo(LS_MPEGTS,
        "enabling service %s [sid %04X/%d] (found in %s)",
        s->s_nicename,
        service_id16(s),
        service_id16(s),
        where);
...
static void mpegts_service_raw_setsourceinfo(service_t* t, source_info_t* si) {
...
static int mpegts_service_raw_update_pids(mpegts_service_t* t, mpegts_apids_t* pids) {
...
void mpegts_service_update_slave_pids(mpegts_service_t* s, mpegts_service_t* master, int del) {
  mpegts_service_t*    s2;
  mpegts_apids_t*      pids;
  ...
  TAILQ_FOREACH (st, &s->s_components.set_filter, es_filter_link)
    if ((is_ddci || s->s_scrambled_pass || st->es_type != SCT_CA) && st->es_pid >= 0 &&
        st->es_pid < 8192)
...
static int mpegts_service_link(mpegts_service_t* master, mpegts_service_t* slave) {
...
static int mpegts_service_unlink(mpegts_service_t* master, mpegts_service_t* slave) {
  tvh_mutex_lock(&slave->s_stream_mutex);
...
static mpegts_apids_t* mpegts_service_raw_pid_list(service_t* t) {
...
mpegts_service_t* mpegts_service_create_raw(mpegts_mux_t* mm) {
  ...
  if (service_create0((service_t*)s, STYPE_RAW, &mpegts_service_raw_class, NULL, S_MPEG_TS, NULL) ==
      NULL) {
    free(s);
    return NULL;
  }
