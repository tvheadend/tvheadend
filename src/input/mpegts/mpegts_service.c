...
static htsmsg_t* mpegts_service_pref_capid_lock_list(void* o,
    const char*                                            lang) {
...
const idclass_t mpegts_service_class = {.ic_super = &service_class,
    ...
    .ic_properties = (const property_t[]){
        ...
        },
        {
            ...
            .desc = N_("Add this value to PTS for the teletext subtitles. The "
                       "time value is in milliseconds and may be negative."),
...
static int mpegts_service_enlist_raw(service_t* t,
  ...
  LIST_FOREACH (mmi, &m->mm_instances, mmi_mux_link) {
    ...
    if (r == MI_IS_ENABLED_NEVER) {
      tvhtrace(LS_MPEGTS,
          "enlist: input %p not enabled for mux %p service %s weight %d flags "
          "%x",
          mi,
          mmi->mmi_mux,
          s->s_nicename,
    ...
    if (mmi->mmi_mux->mm_active == mmi) {
      ...
      if (w > 0 && mi->mi_free_weight && weight >= mi->mi_free_weight &&
          w < mi->mi_free_weight)
        w = 0;
    }
...
static void mpegts_service_refresh(service_t* t) {
  ...
  i->mi_open_service(i,
      s,
      s->s_dvb_subscription_flags,
      0,
      s->s_dvb_subscription_weight);
}
      ...
      case DVB_TYPE_S:
        ...
        if (!mpegts_picon_mux_isvalid(mmd, pos))
          hash |= ((mmd->lm_tuning.dmc_fe_freq / 1000) & 0x7fff) |
              (mmd->lm_tuning.u.dmc_fe_qpsk.orbital_pos ==
...
#if ENABLE_MPEGTS_DVB
static int mpegts_service_match_network(mpegts_network_t* mn,
    uint32_t                                              hash,
    const idclass_t*                                      idc) {
  int pos, pos2;
...
mpegts_service_match_mux(dvb_mux_t* mm, uint32_t hash, const idclass_t* idc) {
  ...
  pol =
      (hash & 0x8000) ? DVB_POLARISATION_HORIZONTAL : DVB_POLARISATION_VERTICAL;
  mmd = mm;
...
mpegts_service_t* mpegts_service_create0(mpegts_service_t* s,
  ...
  if (service_create0((service_t*)s, STYPE_STD, class, uuid, S_MPEG_TS, conf) ==
      NULL)
    return NULL;
...
static int mpegts_service_raw_update_pids(mpegts_service_t* t,
    mpegts_apids_t*                                         pids) {
    ...
    if (pids && !pids->all && x && x->all) {
      ...
      if (pids && pids->all) {
        ...
        if (p) {
          for (i = 0; i < p->count; i++) {
            pi = &p->pids[i];
            mpegts_input_update_pid_weight(mi,
                mm,
                pi->pid,
                MPS_RAW,
                pi->weight,
...
void mpegts_service_update_slave_pids(mpegts_service_t* s,
...
static int mpegts_service_link(mpegts_service_t* master,
...
static int mpegts_service_unlink(mpegts_service_t* master,
    mpegts_service_t*                              slave) {
  tvh_mutex_lock(&slave->s_stream_mutex);
...
mpegts_service_t* mpegts_service_create_raw(mpegts_mux_t* mm) {
  ...
  if (service_create0((service_t*)s,
          STYPE_RAW,
          &mpegts_service_raw_class,
          NULL,
          S_MPEG_TS,
          NULL) == NULL) {
    free(s);
