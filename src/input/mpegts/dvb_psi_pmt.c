 ...
 * Add a CA descriptor
 */
static int psi_desc_add_ca(mpegts_table_t* mt,
  ...
  tvhdebug(mt->mt_subsys,
      "%s:  caid %04X (%s) provider %08X pid %04X",
      mt->mt_name,
      caid,
      caid2name(caid),
      provid,
 ...
 */
static int psi_desc_ca(mpegts_table_t* mt, elementary_set_t* set, const uint8_t* buffer, int size) {
  int      r = 0;
  int      i;
  uint32_t provid = 0;
  uint16_t caid, pid;
 ...
 */
static int psi_desc_teletext(elementary_set_t* set,
    const uint8_t*                             ptr,
    int                                        size,
    int                                        parent_pid,
    int*                                       position) {
 ...
 *
 */
static void dvb_pmt_hbbtv_table_remove(mpegts_mux_t* mm, elementary_stream_t* st) {
 ...
 */
uint32_t dvb_psi_parse_pmt(mpegts_table_t* mt,
    const char*                            nicename,
    elementary_set_t*                      set,
    const uint8_t*                         ptr,
    int                                    len) {
    ...
    while (dllen > 1) {
      ...
      switch (dtag) {
        ...
        case DVB_DESC_REGISTRATION:
          if (mux->mm_pmt_ac3 != MM_AC3_PMT_N05 && dlen == 4 && ptr[0] == 'A' && ptr[1] == 'C' &&
              ptr[2] == '-' && ptr[3] == '3')
            hts_stream_type = SCT_AC3;
  ...
  if (update) {
    tvhdebug(mt->mt_subsys,
        "%s: Service \"%s\" PMT (version %d) updated"
...
int dvb_pmt_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  ...
  update = dvb_psi_parse_pmt(mt, service_nicename((service_t*)s), &s->s_components, ptr, len);
  if (update) {
  ...
  if (update) {
    if (update &
        ~(PMT_UPDATE_NEW_CA_STREAM | PMT_UPDATE_NEW_CAID | PMT_UPDATE_CA_PROVIDER_CHANGE |
  ...
  if (update &
      (PMT_UPDATE_NEW_CA_STREAM | PMT_UPDATE_NEW_CAID | PMT_UPDATE_CAID_DELETED |
          PMT_UPDATE_CAID_PID))
    descrambler_caid_changed((service_t*)s);

  TAILQ_FOREACH (es, &s->s_components.set_filter, es_filter_link) {
    if (es->es_type != SCT_HBBTV)
      continue;
    tvhdebug(mt->mt_subsys,
        "%s:    install hbbtv pid %04X (%d)",
        mt->mt_name,
        es->es_pid,
        es->es_pid);
    mpegts_table_add(mm,
