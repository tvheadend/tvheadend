 ...
 */
static int psi_desc_ca(mpegts_table_t* mt,
    elementary_set_t*                  set,
  ...
  switch (caid & 0xFF00) {
    ...
    case 0x0500: // Viaccess
      for (i = 4; i + 5 <= size;) {
        ...
        if (nano == 0x14) {
          provid =
              (buffer[i] << 16) | (buffer[i + 1] << 8) | (buffer[i + 2] & 0xf0);
          break;
        }
 ...
 *
 */
static void dvb_pmt_hbbtv_table_remove(mpegts_mux_t* mm,
    elementary_stream_t*                             st) {
    ...
    while (dllen > 1) {
      ...
      switch (dtag) {
        ...
        case DVB_DESC_REGISTRATION:
          if (mux->mm_pmt_ac3 != MM_AC3_PMT_N05 && dlen == 4 && ptr[0] == 'A' &&
              ptr[1] == 'C' && ptr[2] == '-' && ptr[3] == '3')
            hts_stream_type = SCT_AC3;
...
int dvb_pmt_callback(mpegts_table_t* mt,
    const uint8_t*                   ptr,
    int                              len,
    int                              tableid) {
  ...
  update = dvb_psi_parse_pmt(mt,
      service_nicename((service_t*)s),
      &s->s_components,
      ptr,
      len);
  if (update) {
  ...
  if (update) {
    if (update &
        ~(PMT_UPDATE_NEW_CA_STREAM | PMT_UPDATE_NEW_CAID |
            PMT_UPDATE_CA_PROVIDER_CHANGE | PMT_UPDATE_CAID_DELETED |
            PMT_UPDATE_CAID_PID)) {
