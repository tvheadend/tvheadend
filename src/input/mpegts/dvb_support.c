                    ...
                    dvb_fe_delivery_system_t delsys )
{
  ...
  dmc->dmc_fe_t2mi_pid = DVB_NO_STREAM_ID_FILTER;
	dmc->dmc_fe_t2mi_plp = 0;
...
dvb_mux_conf_str_dvbs ( dvb_mux_conf_t *dmc, char *buf, size_t bufsize )
{
  ...
  return
  snprintf(buf, bufsize,
           "%s %sfreq %d %c sym %d fec %s mod %s roff %s is_id %d t2mi_id %d t2mi_plp %d pls_mode %s pls_code %d",
           dvb_delsys2str(dmc->dmc_fe_delsys),
           satbuf,
           dmc->dmc_fe_freq,
           pol ? pol[0] : 'X',
