...
conv_gb(const uint8_t* src, size_t srclen, char* dst, size_t* dstlen) {
  ssize_t len;
...
static inline size_t
conv_UCS2(const uint8_t* src, size_t srclen, char* dst, size_t* dstlen) {
...
static inline size_t
conv_utf8(const uint8_t* src, size_t srclen, char* dst, size_t* dstlen) {
  while (srclen > 0 && (*dstlen) > 0) {
    uint_fast8_t c = *src;
    if (c <= 0x7f) {
...
static inline size_t
conv_6937(const uint8_t* src, size_t srclen, char* dst, size_t* dstlen) {
  while (srclen > 0 && (*dstlen) > 0) {
    uint_fast8_t c = *src;
    if (c <= 0x7f) {
      conv_lower(dst, dstlen, c);
 ...
 *
 * refer to "ATSC Standard: Program and System Information Protocol for
 * Terrestrial Broadcast and Cable" (Document A65/2013), Section 6.10, pages
 * 79-82
 */

lang_str_t* atsc_get_string(const uint8_t* src, size_t srclen) {
  const int bufferSize =
      255 * 3 + 1; /* Max length of a text segment (255) *
  ...
  for (i = 0; i < stringcount && srclen >= 4; i++) {
    ...
    /* Step through the list of text segments, decoding each and append them all
     * together. */
    for (j = 0; j < segmentcount && srclen >= 3; j++) {
      ...
      if (compressiontype == 0 &&
          (/* No Compression and one of these: */
              (mode >= 0x0 &&
                  mode <= 0x6) || /* Unicode range 0x0000 .. 0x06FF */
              (mode >= 0x9 &&
                  mode <= 0x10) || /* Unicode range 0x0900 .. 0x10FF */
              (mode >= 0x20 &&
                  mode <= 0x27) || /* Unicode range 0x2000 .. 0x27FF */
              (mode >= 0x30 &&
                  mode <= 0x33) /* Unicode range 0x3000 .. 0x33FF */
              )) {
        tvhtrace(LS_MPEGTS,
        ...
        for (k = 0, bufpointer = buf, remainingbufleft = bufferSize;
             k < bytecount;
             k++) {
          /* Make sure there is enough buffer left for the next (or last) UTF-8
           * character [Max # bytes in a single UTF-8 character (3) + NULL
           * terminator (1)] */
          if (remainingbufleft > (3 + 1)) {
            ...
            /* We have run out of buffer space for this text segment, then stop
             * and truncate.
        ...
        *bufpointer = '\0';
         ...
         * bytecount/2: since the function is handling both utf16 bytes in one
         * count only half the length is needed */
         ...
         * - compression type == 0x2 (Huffman-like coding with another fixed
         * codebook)
         ...
         *
         * - text mode == 0x7, 0x8, 0x11 .. 0x1F, 0x28 .. 0x2F, 0x34 .. 0x3D,
         * 0x42 .. 0x47, 0x49 .. 0xFF (reserved)
         */
        ...
        /* For text segments that are not supported, write a terse diagnostic
         * text indicating the unsupported type instead of the text segment. */
        snprintf(buf,
            bufferSize - 1,
            "[comptype=0x%02X,mode=0x%02X]",
            compressiontype,
            mode);
        if (ls == NULL)
          ls = lang_str_create();
...
void dvb_mux_conf_init(mpegts_network_t* ln,
    dvb_mux_conf_t*                      dmc,
...
dvb_mux_conf_str_dvbs(dvb_mux_conf_t* dmc, char* buf, size_t bufsize) {
  ...
  return snprintf(buf,
      bufsize,
      "%s %sfreq %d %c sym %d fec %s mod %s roff %s is_id %d pls_mode %s "
      "pls_code %d",
      dvb_delsys2str(dmc->dmc_fe_delsys),
      satbuf,
      dmc->dmc_fe_freq,
...
dvb_mux_conf_str_isdb_t(dvb_mux_conf_t* dmc, char* buf, size_t bufsize) {
  ...
  return snprintf(buf,
      bufsize,
      "%s freq %d bw %s guard %s A (%s,%s,%d,%d) B (%s,%s,%d,%d) C "
      "(%s,%s,%d,%d)",
      dvb_delsys2str(dmc->dmc_fe_delsys),
      dmc->dmc_fe_freq,
