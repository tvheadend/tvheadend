 ...
 */
void ts_recv_packet0(mpegts_service_t* t, elementary_stream_t* st, const uint8_t* tsb, int len) {
  mpegts_service_t* m;
  ...
  for (tsb2 = tsb, len2 = len; len2 > 0; tsb2 += 188, len2 -= 188) {
    ...
    if (tsb2[3] & 0x10) {
      ...
      if (st->es_cc != -1 && cc != st->es_cc) {
        ...
        if (t->s_start_time + sec2mono(1) < mclk() && tvhlog_limit(&st->es_cc_log, 10))
          tvhwarn(LS_TS,
              "%s Continuity counter error (total %zi)",
              st->es_nicename,
              st->es_cc_log.count);
        if (!error)
          errors++;
 ...
 */
static void
ts_recv_skipped0(mpegts_service_t* t, elementary_stream_t* st, const uint8_t* tsb, int len) {
  mpegts_service_t* m;
  int               len2, off, cc, pid;
  const uint8_t*    tsb2;
 ...
 */
int ts_recv_packet1(mpegts_service_t* t,
    uint64_t                          tspos,
    uint16_t                          pid,
    const uint8_t*                    tsb,
    int                               len,
  ...
  if (error) {
    ...
    if (tvhlog_limit(&t->s_tei_log, 10))
      tvhwarn(LS_TS,
          "%s Transport error indicator (total %zi)",
 ...
 */
void ts_recv_packet2(mpegts_service_t* t, const uint8_t* tsb, int len) {
  elementary_stream_t* st;
  int                  pid, len2;
 ...
 * Process transport stream packets, simple version
 */
void ts_skip_packet2(mpegts_service_t* t, const uint8_t* tsb, int len) {
  elementary_stream_t* st;
  int                  pid, len2;
 ...
 *
 */
void ts_recv_raw(mpegts_service_t* t, uint64_t tspos, const uint8_t* tsb, int len) {
  int pid, parent = 0;
 ...
 *
 */
static void ts_flush(mpegts_service_t* t, sbuf_t* sb) {
 ...
 *
 */
static void ts_remux(mpegts_service_t* t, const uint8_t* src, int len, int errors) {
  sbuf_t* sb = &t->s_tsbuf;
 ...
 *
 */
static void ts_skip(mpegts_service_t* t, const uint8_t* src, int len) {
  sbuf_t* sb = &t->s_tsbuf;
  ...
  if (monocmpfastsec(mclk(), t->s_tsbuf_last) && sb->sb_err < (TS_REMUX_BUFSIZE / 188))
    return;
...
int ts_resync(const uint8_t* tsb, int* len, int* idx) {
  ...
  while (err && (*len > 376)) {
    (*idx)++;
    (*len)--;
    err = (tsb[*idx] != 0x47) || (tsb[*idx + 188] != 0x47) || (tsb[*idx + 376] != 0x47);
  }
  return err;
}
