...
static const int dvb_servicetype_map[][2] = {
    ...
    {0x1D, ST_HDTV},  /* Advanced codec HDTV - plano-stereoscopic - NVOD
                         time-shifted */
    {0x1E, ST_HDTV},  /* Advanced codec HDTV - plano-stereoscopic - NVOID
                         reference */
...
void mpegts_psi_section_reassemble(mpegts_psi_table_t* mt,
  ...
  if (pusi) {
    ...
    if (off > 188)
      goto wrong_state;
    while (off2 < len2) {
      r = mpegts_psi_section_reassemble0(mt,
          logprefix,
          tsb + off2,
          len2,
          0,
...
static void mpegts_table_state_reset(mpegts_psi_table_t* mt,
...
static mpegts_psi_table_state_t* mpegts_table_state_find(mpegts_psi_table_t* mt,
    int      tableid,
    uint64_t extraid,
    int      last) {
...
int dvb_table_begin(mpegts_psi_table_t* mt,
  ...
  if (len < minlen) {
    tvhdebug(mt->mt_subsys,
        "%s: invalid table length %d min %d",
        mt->mt_name,
        len,
        minlen);
    return -1;
  ...
  if (sect && ret) {
    ...
    *ret = st = mpegts_table_state_find(mt, tableid, extraid, *last);
    ...
    if (interval && mt->mt_last_complete &&
      ...
      tvhtrace(mt->mt_subsys,
          "%s:  time interval exceeded, complete restart",
          mt->mt_name);
      RB_FOREACH (st2, &mt->mt_state, link)
        if (st != st2)
          mpegts_table_state_restart(mt,
              st2,
              st2->last,
              MPEGTS_PSI_VERSION_NONE);
...
void dvb_table_parse(mpegts_psi_table_t* mt,
  ...
  for (end = tsb + len; tsb < end; tsb += 188)
    mpegts_psi_section_reassemble(mt,
        logprefix,
        tsb,
        crc,
        dvb_table_parse_cb,
