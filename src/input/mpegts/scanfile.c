...
scanfile_region_create(const char* type, const char* id, const char* desc) {
  ...
  if (!reg) {
    ...
    memoryinfo_alloc(&scanfile_memoryinfo,
        sizeof(*reg) + strlen(id) + 1 + strlen(desc) + 1);
...
static int scanfile_create_network(scanfile_network_t** _net,
  ...
  *str = '\0';
  ...
  memoryinfo_alloc(&scanfile_memoryinfo,
      sizeof(*net) + strlen(buf2) + 1 + strlen(buf) + 1 + strlen(path) + 1 +
          strlen(type) + 1);
...
static scanfile_network_t* scanfile_load_file(scanfile_network_t* net,
  ...
  tvhtrace(LS_SCANFILE,
      "load file %s (processed bytes %" PRId64 ")",
      name,
      scanfile_total_load);
 ...
 */
static void
scanfile_load_dir(const char* path, const char* type, int lvl, int test) {
  char                p[PATH_MAX];
  fb_dir*             dir;
  fb_dirent*          de;
  fb_file*            fp;
...
static int scanfile_stats(const char* what, scanfile_region_list_t* list) {
  ...
  if (regions) {
    tvhinfo(LS_SCANFILE,
        "%s - loaded %i regions with %i networks",
        what,
        regions,
        networks);
    return 1;
...
void scanfile_init(const char* muxconf_path, int lock) {
  ...
  memoryinfo_alloc(&scanfile_memoryinfo,
      REGIONS * sizeof(scanfile_region_list_t));
  for (i = 0; i < REGIONS; i++) {
  ...
  if (!r) {
    ...
    tvhwarn(LS_SCANFILE,
        ...
        "http://git.linuxtv.org/cgit.cgi/dtv-scan-tables.git/tree/");
    for (i = 0; i < REGIONS; i++)
