...
static const char* scanfile_region_types[][2] = {{"dvb-s", "dvbs"},
    {"dvb-t", "dvbt"},
    {"dvb-c", "dvbc"},
    {"atsc-t", NULL},
    {"atsc-c", NULL},
...
static const char* tldcode2longname(const char* tld) {
  int i;
...
static int scanfile_load_atsc(dvb_mux_conf_t* mux, const char* line) {
  char qam[20];
  int  r;
  ...
  if (r != 2)
    return 1;
  if ((mux->dmc_fe_modulation = dvb_str2qam(qam)) == -1)
    return 1;

  return 0;
}

static int scanfile_load_dvbt(dvb_mux_conf_t* mux, const char* line) {
  ...
  if (*line == '2') {
    ...
    if (r == 2 && mux->dmc_fe_stream_id < 1000 && strstr(bw, "MHz") == 0) {
      r = sscanf(line + 1,
          "%u %u %u %10s %10s %10s %10s %10s %10s %10s",
          &mux->dmc_fe_stream_id,
          &system_id,
          &mux->dmc_fe_freq,
          bw,
          fec,
      ...
      r = sscanf(line + 1,
          "%u %10s %10s %10s %10s %10s %10s %10s %u",
          &mux->dmc_fe_freq,
          bw,
          fec,
          fec2,
  ...
  if ((mux->u.dmc_fe_ofdm.bandwidth = dvb_str2bw(bw)) == -1)
    return 1;
  if ((mux->dmc_fe_modulation = dvb_str2qam(qam)) == -1)
    return 1;
  if ((mux->u.dmc_fe_ofdm.code_rate_HP = dvb_str2fec(fec)) == -1)
    return 1;
  if ((mux->u.dmc_fe_ofdm.code_rate_LP = dvb_str2fec(fec2)) == -1)
    return 1;
  if ((mux->u.dmc_fe_ofdm.transmission_mode = dvb_str2mode(mode)) == -1)
    return 1;
  if ((mux->u.dmc_fe_ofdm.guard_interval = dvb_str2guard(guard)) == -1)
    return 1;
  if ((mux->u.dmc_fe_ofdm.hierarchy_information = dvb_str2hier(hier)) == -1)
    return 1;

  return 0;
}

static int scanfile_load_dvbs(dvb_mux_conf_t* mux, const char* line) {
  ...
  r = sscanf(line,
      ...
      &mux->dmc_fe_freq,
      pol,
      &mux->u.dmc_fe_qpsk.symbol_rate,
      fec,
      rolloff,
      qam,
      &mux->dmc_fe_stream_id,
      &mux->dmc_fe_pls_code,
      (int*)&mux->dmc_fe_pls_mode);
  if (r < (4 + v2))
    return 1;

  if ((mux->u.dmc_fe_qpsk.polarisation = dvb_str2pol(pol)) == -1)
    return 1;
  if ((mux->u.dmc_fe_qpsk.fec_inner = dvb_str2fec(fec)) == -1)
    return 1;
  if (v2) {
    if ((mux->dmc_fe_rolloff = dvb_str2rolloff(rolloff)) == -1)
      return 1;
    if ((mux->dmc_fe_modulation = dvb_str2qam(qam)) == -1)
      return 1;
    if (r < (4 + v2 + 1))
...
static int scanfile_load_dvbc(dvb_mux_conf_t* mux, const char* line) {
  char fec[20], qam[20];
  int  r;

  r = sscanf(line, "%u %u %s %s", &mux->dmc_fe_freq, &mux->u.dmc_fe_qam.symbol_rate, fec, qam);
  if (r != 4)
    return 1;

  dvb_mux_conf_init(NULL, mux, DVB_SYS_DVBC_ANNEX_A);
  if ((mux->u.dmc_fe_qam.fec_inner = dvb_str2fec(fec)) == -1)
    return 1;
  if ((mux->dmc_fe_modulation = dvb_str2qam(qam)) == -1)
    return 1;
 ...
 * Sorting
 */
static int scanfile_network_dvbs_pos(char* n, int* rpos) {
...
static int scanfile_network_cmp(scanfile_network_t* a, scanfile_network_t* b) {
  if (a->sfn_satpos == b->sfn_satpos)
    return strcmp(a->sfn_name, b->sfn_name);
  return b->sfn_satpos - a->sfn_satpos;
}
static int scanfile_region_cmp(scanfile_region_t* a, scanfile_region_t* b) {
...
scanfile_region_create(const char* type, const char* id, const char* desc) {
 ...
 *
 */
static int scanfile_create_network(scanfile_network_t** _net,
    const char*                                         type,
    const char*                                         path,
    const char*                                         name,
  ...
  *str = '\0';
  ...
  memoryinfo_alloc(&scanfile_memoryinfo,
      sizeof(*net) + strlen(buf2) + 1 + strlen(buf) + 1 + strlen(path) + 1 + strlen(type) + 1);
 ...
 * Process mux entry
 */
static int scanfile_load_one(scanfile_network_t** net,
    const char*                                   type,
    const char*                                   path,
    const char*                                   name,
    const char*                                   line,
  ...
  if (r) {
    ...
    if (*net == NULL && scanfile_create_network(net, type, path, name, mux->dmc_fe_delsys)) {
      free(mux);
      return -1;
    }
    if (!test) {
...
#define mux_fail0(r, text)       \
  do {                           \
    tvhtrace(LS_SCANFILE, text); \
    ((r) = -1);                  \
  } while (0)
#define mux_fail(r, text, val)        \
  do {                                \
...
static int scanfile_load_dvbv5(scanfile_network_t** net,
    const char*                                     type,
    const char*                                     path,
    const char*                                     name,
    char*                                           line,
  ...
  while (!fb_eof(fp)) {
    /* Get line */
    buf[sizeof(buf) - 1] = '\0';
    if (!fb_gets(fp, buf, sizeof(buf) - 1))
      break;
    s = str_trim(buf);
    if (*s == '#' || *s == '\0')
      continue;
  ...
  if (mux->dmc_fe_delsys == DVB_SYS_DVBT || mux->dmc_fe_delsys == DVB_SYS_DVBT2) {
    ...
    mux->dmc_fe_modulation = mux->dmc_fe_delsys == DVB_SYS_DVBS2 ? DVB_MOD_PSK_8 : DVB_MOD_QPSK;
    ...
    if (htsmsg_get_u32(l, "SYMBOL_RATE", &mux->u.dmc_fe_qpsk.symbol_rate))
      ...
      mux->dmc_fe_delsys == DVB_SYS_DVBC_ANNEX_B || mux->dmc_fe_delsys == DVB_SYS_DVBC_ANNEX_C) {
    ...
    mux->u.dmc_fe_isdbt.layers[0].segment_count =
        htsmsg_get_u32_or_default(l, "ISDBT_LAYERA_SEGMENT_COUNT", 0);
    mux->u.dmc_fe_isdbt.layers[1].segment_count =
        htsmsg_get_u32_or_default(l, "ISDBT_LAYERB_SEGMENT_COUNT", 0);
    mux->u.dmc_fe_isdbt.layers[2].segment_count =
        htsmsg_get_u32_or_default(l, "ISDBT_LAYERC_SEGMENT_COUNT", 0);

    mux->u.dmc_fe_isdbt.layers[0].time_interleaving =
        htsmsg_get_u32_or_default(l, "ISDBT_LAYERA_TIME_INTERLEAVING", 0);
    mux->u.dmc_fe_isdbt.layers[1].time_interleaving =
        htsmsg_get_u32_or_default(l, "ISDBT_LAYERB_TIME_INTERLEAVING", 0);
    mux->u.dmc_fe_isdbt.layers[2].time_interleaving =
        htsmsg_get_u32_or_default(l, "ISDBT_LAYERC_TIME_INTERLEAVING", 0);
 ...
 * Process a file
 */
static scanfile_network_t* scanfile_load_file(scanfile_network_t* net,
    const char*                                                   type,
    const char*                                                   path,
    fb_file*                                                      fp,
    const char*                                                   name,
  ...
  if (scanfile_total_load > SCANFILE_LIMIT)
    goto end;
 ...
 */
static void scanfile_load_dir(const char* path, const char* type, int lvl, int test) {
  char                p[PATH_MAX];
  fb_dir*             dir;
  fb_dirent*          de;
  fb_file*            fp;
 ...
 *
 */
static int scanfile_stats(const char* what, scanfile_region_list_t* list) {
  scanfile_region_t*  reg;
 ...
 * Destroy the region
 */
static void scanfile_done_region(scanfile_region_list_t* list) {
  scanfile_region_t*  reg;
  scanfile_network_t* net;
  ...
  while ((reg = LIST_FIRST(&list->srl_regions)) != NULL) {
    ...
    while ((net = LIST_FIRST(&reg->sfr_networks)) != NULL) {
      ...
      scanfile_done_muxes(net);
      memoryinfo_free(&scanfile_memoryinfo,
          sizeof(*net) + (net->sfn_id ? strlen(net->sfn_id) + 1 : 0) +
    ...
    memoryinfo_free(&scanfile_memoryinfo,
        sizeof(*reg) + (reg->sfr_id ? strlen(reg->sfr_id) + 1 : 0) +
...
void scanfile_init(const char* muxconf_path, int lock) {
  ...
  if (!r) {
    tvhwarn(LS_SCANFILE,
        "no predefined muxes found, check path '%s%s'",
        path[0] == '/' ? path : TVHEADEND_DATADIR "/",
        path[0] == '/' ? "" : path);
    tvhwarn(LS_SCANFILE,
        "expected tree structure - http://git.linuxtv.org/cgit.cgi/dtv-scan-tables.git/tree/");
    for (i = 0; i < REGIONS; i++)
