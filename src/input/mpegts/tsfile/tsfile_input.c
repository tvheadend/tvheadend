...
static void tsfile_input_stop_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                        mmi) {
  int             err;
...
static int tsfile_input_start_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                        t,
    int                                           weight) {
  struct stat            st;
  ...
  if (ti->ti_thread_pipe.rd == -1) {
    ...
    tvh_thread_create(&ti->ti_thread_id,
        NULL,
        tsfile_input_thread,
        mi,
        "tsfile");
...
tsfile_input_t* tsfile_input_create(int idx) {
  ...
  /* Create object */
  mi = (tsfile_input_t*)mpegts_input_create0(calloc(1, sizeof(tsfile_input_t)),
      &mpegts_input_class,
      NULL,
      NULL);
  mi->mi_instance  = idx;
