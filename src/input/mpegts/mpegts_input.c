...
static void mpegts_input_del_network(mpegts_network_link_t* mnl);

/*
 * DBUS
 */

static void mpegts_input_dbus_notify(mpegts_input_t* mi, int64_t subs) {
#if ENABLE_DBUS_1
  ...
  snprintf(buf,
      sizeof(buf),
      "/input/mpegts/%s",
      idnode_uuid_as_str(&mi->ti_id, ubuf));
...
static void mpegts_input_class_get_title(idnode_t* in,
    const char*                                    lang,
    char*                                          dst,
    size_t                                         dstsize) {
...
const void* mpegts_input_class_active_get(void* obj) {
  static int      active;
...
const void* mpegts_input_class_network_get(void* obj) {
...
int mpegts_input_class_network_set(void* obj, const void* p) {
  return mpegts_input_set_networks(obj, (htsmsg_t*)p);
}

htsmsg_t* mpegts_input_class_network_enum(void* obj, const char* lang) {
  htsmsg_t *p, *m;
...
char* mpegts_input_class_network_rend(void* obj, const char* lang) {
  char*                  str;
  mpegts_network_link_t* mnl;
  mpegts_network_t*      mn;
  mpegts_input_t*        mi = obj;
  ...
  LIST_FOREACH (mnl, &mi->mi_networks, mnl_mi_link) {
    ...
    htsmsg_add_str(l,
        NULL,
        idnode_get_title(&mn->mn_id, lang, buf, sizeof(buf)));
  }
...
static void mpegts_input_enabled_notify(void* p, const char* lang) {
  mpegts_input_t*        mi = p;
...
static int mpegts_input_class_linked_set(void* self, const void* val) {
...
static const void* mpegts_input_class_linked_get(void* self) {
...
static htsmsg_t* mpegts_input_class_linked_enum(void* self, const char* lang) {
  mpegts_input_t *mi = self, *mi2;
  tvh_input_t*    ti;
  char            ubuf[UUID_HEX_SIZE], buf[384];
  htsmsg_t*       m = htsmsg_create_list();
  htsmsg_t*       e =
      htsmsg_create_key_val("", tvh_gettext_lang(lang, N_("Not linked")));
...
const idclass_t mpegts_input_class = {.ic_super = &tvh_input_class,
    ...
    .ic_properties                              = (const property_t[]){
        ...
        {.type    = PT_INT,
            ...
            .name = N_("Priority"),
            .desc =
                N_("The tuner priority value (a higher value means to "
        ...
        {.type    = PT_INT,
            ...
            .desc = N_(
                "The tuner priority value for streamed channels "
        ...
        },
        {
            ...
            .desc =
                N_("If the subscription weight for the input is below "
        ...
        },
        {
            ...
            .desc =
                N_("The scrambled bits in MPEG-TS packets are always cleared. "
        ...
        },
        {
            ...
            .desc =
                N_("Wake up the linked input whenever this adapter "
...
int mpegts_input_is_enabled(mpegts_input_t* mi,
    mpegts_mux_t*                           mm,
    int                                     flags,
    int                                     weight) {
...
static int mpegts_input_start_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                        mmi,
    int                                           weight) {
  return SM_CODE_TUNING_FAILED;
}

static void mpegts_input_stop_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                        mmi) {}
...
void mpegts_input_close_pids(mpegts_input_t* mi,
    mpegts_mux_t*                            mm,
    void*                                    owner,
    int                                      all) {
  ...
  if (all)
    for (mps = LIST_FIRST(&mm->mm_all_subs); mps; mps = mps_next) {
      ...
      if (mps->mps_owner != owner)
        continue;
      pid = MPEGTS_FULLMUX_PID;
      if (mps->mps_type & MPS_TABLES)
        pid = MPEGTS_TABLES_PID;
...
mpegts_pid_t* mpegts_input_open_pid(mpegts_input_t* mi,
    mpegts_mux_t*                                   mm,
    int                                             pid,
    int                                             type,
    int                                             weight,
  ...
  if ((mp = mpegts_mux_find_pid(mm, pid, 1))) {
    ...
    if (pid == MPEGTS_FULLMUX_PID || pid == MPEGTS_TABLES_PID) {
      ...
      LIST_FOREACH (mps2, &mm->mm_all_subs, mps_svcraw_link)
        if (mps2->mps_owner == owner)
          break;
      if (mps2 == NULL) {
        LIST_INSERT_HEAD(&mm->mm_all_subs, mps, mps_svcraw_link);
        tvhdebug(LS_MPEGTS,
            "%s - open PID %s subscription [%04x/%p]",
            mm->mm_nicename,
            (type & MPS_TABLES) ? "tables" : "fullmux",
            type,
            owner);
...
int mpegts_input_close_pid(mpegts_input_t* mi,
    mpegts_mux_t*                          mm,
    int                                    pid,
    int                                    type,
    void*                                  owner) {
  ...
  if (pid == MPEGTS_FULLMUX_PID || pid == MPEGTS_TABLES_PID) {
    ...
    if (mps == NULL)
      return -1;
    tvhdebug(LS_MPEGTS,
        "%s - close PID %s subscription [%04x/%p]",
        mm->mm_nicename,
        pid == MPEGTS_TABLES_PID ? "tables" : "fullmux",
        type,
        owner);
    if (pid == MPEGTS_FULLMUX_PID)
...
mpegts_pid_t* mpegts_input_update_pid_weight(mpegts_input_t* mi,
    mpegts_mux_t*                                            mm,
    int                                                      pid,
    int                                                      type,
    int                                                      weight,
  ...
  if (pid == MPEGTS_FULLMUX_PID || pid == MPEGTS_TABLES_PID) {
    ...
    if (mps == NULL)
      return NULL;
    tvhdebug(LS_MPEGTS,
        "%s - update PID %s weight %d subscription [%04x/%p]",
        mm->mm_nicename,
        pid == MPEGTS_TABLES_PID ? "tables" : "fullmux",
        weight,
        type,
    ...
    if (mps == NULL)
      return NULL;
    tvhdebug(LS_MPEGTS,
        "%s - update PID %04X (%d) weight %d [%d/%p]",
        mm->mm_nicename,
        mp->mp_pid,
        mp->mp_pid,
        weight,
...
elementary_stream_t* mpegts_input_open_service_pid(mpegts_input_t* mi,
    mpegts_mux_t*                                                  mm,
    service_t*                                                     s,
  ...
  if (es && mm->mm_active) {
    mpegts_input_open_pid(mi, mm, pid, MPS_SERVICE, weight, s, 0);
  }
  return es;
}

static void mpegts_input_update_pids(mpegts_input_t* mi, mpegts_mux_t* mm) {
  /* nothing - override */
}
...
static void mpegts_input_cat_pass_entry(void* _aux,
    uint16_t                                  caid,
    uint32_t                                  prov,
    uint16_t                                  pid) {
  ...
  tvhdebug(LS_TBL_BASE,
      "cat:  pass: caid %04X (%d) pid %04X (%d)",
      (uint16_t)caid,
      (uint16_t)caid,
      pid,
      pid);
  es = mpegts_input_open_service_pid(aux->mi,
      aux->mm,
      aux->service,
      SCT_CAT,
      pid,
...
static int mpegts_input_cat_pass_callback(mpegts_table_t* mt,
    const uint8_t*                                        ptr,
    int                                                   len,
    int                                                   tableid) {
  int                       r, sect, last, ver;
...
void mpegts_input_open_pmt_monitor(mpegts_mux_t* mm, mpegts_service_t* s) {
  ...
  s->s_pmt_mon = mpegts_table_add(mm,
      DVB_PMT_BASE,
      DVB_PMT_MASK,
      dvb_pmt_callback,
      s,
...
void mpegts_input_open_cat_monitor(mpegts_mux_t* mm, mpegts_service_t* s) {
  ...
  s->s_cat_mon = mpegts_table_add(mm,
      DVB_CAT_BASE,
      DVB_CAT_MASK,
...
void mpegts_input_open_service(mpegts_input_t* mi,
    mpegts_service_t*                          s,
    int                                        flags,
    int                                        init,
    int                                        weight) {
  ...
  if (s->s_type == STYPE_STD) {
    ...
    TAILQ_FOREACH (st, &s->s_components.set_filter, es_filter_link)
      if ((s->s_scrambled_pass || st->es_type != SCT_CA) &&
          ...
          st->es_pid != s->s_components.set_pcr_pid && st->es_pid < 8192) {
        st->es_pid_opened = 1;
        mpegts_input_open_pid(mi,
            mm,
            st->es_pid,
            MPS_SERVICE,
            mpegts_mps_weight(st),
            s,
            reopen);
      }
...
void mpegts_input_create_mux_instance(mpegts_input_t* mi, mpegts_mux_t* mm) {
  ...
  LIST_FOREACH (tii, &mi->mi_mux_instances, tii_input_link)
    if (((mpegts_mux_instance_t*)tii)->mmi_mux == mm)
      break;
  if (!tii)
    mpegts_mux_instance_create(mpegts_mux_instance, NULL, mi, mm);
}

static void mpegts_input_started_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                           mmi) {
  ...
  if (LIST_FIRST(&mi->mi_mux_active) == NULL)
    mtimer_arm_rel(&mi->mi_status_timer,
        mpegts_input_status_timer,
        mi,
        sec2mono(1));
...
static void mpegts_input_stopping_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                            mmi) {
...
static void mpegts_input_stopped_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                           mmi) {
  char          buf[256];
  service_t *   s, *s_next;
  mpegts_mux_t* mm = mmi->mmi_mux;
...
static int mpegts_input_has_subscription(mpegts_input_t* mi, mpegts_mux_t* mm) {
  int                      ret = 0;
  const service_t*         t;
  const th_subscription_t* ths;
  tvh_mutex_lock(&mi->mi_output_lock);
...
static void mpegts_input_analyze_table_queue(mpegts_input_t* mi) {
  ...
  for (pid = 0; pid < 8192; pid++)
    if (counters[pid])
      tvhtrace(LS_MPEGTS,
          "table queue pid=%04X (%d) cnt %u len %u",
          pid,
          pid,
          counters[pid],
          sizes[pid]);
...
#else
static inline int data_noise(mpegts_packet_t* mp) {
  return 0;
}
#endif

static inline int get_pcr(const uint8_t* tsb, int64_t* rpcr) {
  int_fast64_t pcr;

  if (tsb[1] & 0x80) /* error bit */
    return 0;

  if ((tsb[3] & 0x20) == 0 || tsb[4] <= 5 || (tsb[5] & 0x10) == 0)
    return 0;
...
static inline int ts_sync_count(const uint8_t* tsb, int len) {
  const uint8_t* start = tsb;
  while (len >= 188) {
    if (len >= 1880 && tsb[0 * 188] == 0x47 && tsb[1 * 188] == 0x47 &&
        tsb[2 * 188] == 0x47 && tsb[3 * 188] == 0x47 && tsb[4 * 188] == 0x47 &&
        tsb[5 * 188] == 0x47 && tsb[6 * 188] == 0x47 && tsb[7 * 188] == 0x47 &&
...
static void mpegts_input_queue_packets(mpegts_mux_instance_t* mmi,
    mpegts_packet_t*                                          mp) {
  ...
  if (mmi->mmi_mux->mm_active == mmi) {
    if (mi->mi_input_queue_size < 50 * 1024 * 1024) {
      ...
      memoryinfo_alloc(&mpegts_input_queue_memoryinfo,
          sizeof(mpegts_packet_t) + len);
      ...
      if (tvhlog_limit(&mi->mi_input_queue_loglimit, 10))
        tvhwarn(LS_MPEGTS,
            "too much queued input data (over 50MB) for %s, discarding new",
            mi->mi_name);
...
void mpegts_input_recv_packets(mpegts_mux_instance_t* mmi,
    sbuf_t*                                           sb,
    int                                               flags,
    mpegts_pcr_t*                                     pcr) {
  mpegts_input_t*  mi = mmi->mmi_input;
...
retry:
  ...
  /* Check for sync */
  while (
      (len >= MIN_TS_SYN) && ((len2 = ts_sync_count(tsb, len)) < MIN_TS_SYN)) {
  ...
  if (len2 >= MIN_TS_SYN || (flags & MPEGTS_DATA_CC_RESTART)) {
    ...
    if (mi->mi_remove_scrambled_bits ||
        (flags & MPEGTS_DATA_REMOVE_SCRAMBLED) != 0) {
      uint8_t *tmp, *end;
...
static void mpegts_input_table_dispatch(mpegts_mux_t* mm,
...
static void
mpegts_input_table_restart(mpegts_mux_t* mm, const char* logprefix, int fast) {
  mpegts_table_t* mt;
  tvh_mutex_lock(&mm->mm_tables_lock);
  LIST_FOREACH (mt, &mm->mm_tables, mt_link) {
    if (fast && (mt->mt_flags & MT_FAST) == 0)
...
static mpegts_table_feed_t* mpegts_input_table_feed_create(mpegts_input_t* mi,
    mpegts_mux_t*                                                          mm,
    uint8_t*                                                               tsb,
    int llen) {
  mpegts_table_feed_t* mtf;
  ...
  memoryinfo_alloc(&mpegts_input_table_memoryinfo,
      sizeof(mpegts_table_feed_t) + llen);
  TAILQ_INSERT_TAIL(&mi->mi_table_queue, mtf, mtf_link);
  return mtf;
}

static void mpegts_input_table_waiting(mpegts_input_t* mi, mpegts_mux_t* mm) {
  mpegts_table_t* mt;
...
static int mpegts_input_process(mpegts_input_t* mi, mpegts_packet_t* mpkt) {
  uint16_t               pid, pid2;
  uint8_t                cc, cc2;
        ...
        if (!(tsb[1] & 0x80)) {
          ...
          if (type & MPS_TABLE) {
            if (mi->mi_table_queue_size >= 2 * 1024 * 1024) {
              if (tvhlog_limit(&mi->mi_input_queue_loglimit, 10)) {
                tvhwarn(LS_MPEGTS,
                    "too much queued table input data (over 2MB) for %s, "
                    "discarding new",
                    mi->mi_name);
                if (tvhtrace_enabled())
              ...
              if (mtf && mtf->mtf_mux == mm &&
                  mtf->mtf_len + llen <= MPEGTS_MTF_ALLOC_CHUNK) {
                pid2 = (mtf->mtf_tsb[1] << 8) | mtf->mtf_tsb[2];
                if (pid == pid2) {
                  memcpy(mtf->mtf_tsb + mtf->mtf_len, tsb, llen);
                  memoryinfo_free(&mpegts_input_table_memoryinfo,
                      sizeof(mpegts_table_feed_t) + mtf->mtf_len);
                  mtf->mtf_len += llen;
                  mi->mi_table_queue_size += llen;
                  memoryinfo_alloc(&mpegts_input_table_memoryinfo,
                      sizeof(mpegts_table_feed_t) + mtf->mtf_len);
 ...
 */
void mpegts_input_postdemux(mpegts_input_t* mi,
    mpegts_mux_t*                           mm,
    uint8_t*                                tsb,
    int                                     len) {
  uint16_t               pid;
...
static void* mpegts_input_thread(void* p) {
  mpegts_packet_t* mp;
  mpegts_input_t*  mi    = p;
  size_t           bytes = 0;
  int              update_pids;
  ...
  while (atomic_get(&mi->mi_running)) {
    ...
    memoryinfo_free(&mpegts_input_queue_memoryinfo,
        sizeof(mpegts_packet_t) + mp->mp_len);
...
static void* mpegts_input_table_thread(void* aux) {
  mpegts_table_feed_t* mtf;
  ...
  while (atomic_get(&mi->mi_running)) {
    ...
    if (atomic_get(&mi->mi_running)) {
      ...
      if (mm && mm->mm_active) {
        ...
        mpegts_input_table_dispatch(mm,
            mm->mm_nicename,
            mtf->mtf_tsb,
            mtf->mtf_len,
            0);
...
static void mpegts_input_stream_status(mpegts_mux_instance_t* mmi,
...
static void mpegts_input_get_streams(tvh_input_t* i,
    tvh_input_stream_list_t*                      isl) {
...
static void mpegts_input_clear_stats(tvh_input_t* i) {
  mpegts_input_t*        mi = (mpegts_input_t*)i;
  tvh_input_instance_t*  mmi_;
  mpegts_mux_instance_t* mmi;
...
static void mpegts_input_thread_start(void* aux) {
  mpegts_input_t* mi = aux;
  atomic_set(&mi->mi_running, 1);

  tvh_thread_create(&mi->mi_table_tid,
      NULL,
      mpegts_input_table_thread,
      mi,
      "mi-table");
  tvh_thread_create(&mi->mi_input_tid,
      NULL,
      mpegts_input_thread,
      mi,
      "mi-main");
...
static void mpegts_input_thread_stop(mpegts_input_t* mi) {
...
mpegts_input_t* mpegts_input_create0(mpegts_input_t* mi,
...
void mpegts_input_save(mpegts_input_t* mi, htsmsg_t* m) {
