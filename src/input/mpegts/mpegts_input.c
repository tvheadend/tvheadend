...
static void
mpegts_input_class_get_title(idnode_t* in, const char* lang, char* dst, size_t dstsize) {
...
static void mpegts_input_display_name(mpegts_input_t* mi, char* buf, size_t len) {
  if (mi->mi_name) {
...
static int mpegts_input_start_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi, int weight) {
  return SM_CODE_TUNING_FAILED;
}

static void mpegts_input_stop_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {}
...
void mpegts_input_close_pids(mpegts_input_t* mi, mpegts_mux_t* mm, void* owner, int all) {
...
int mpegts_input_close_pid(mpegts_input_t* mi, mpegts_mux_t* mm, int pid, int type, void* owner) {
...
static void mpegts_input_cat_pass_entry(void* _aux, uint16_t caid, uint32_t prov, uint16_t pid) {
...
static int
mpegts_input_cat_pass_callback(mpegts_table_t* mt, const uint8_t* ptr, int len, int tableid) {
  int                       r, sect, last, ver;
  ...
  if (s->s_type == STYPE_STD) {
    ...
    TAILQ_FOREACH (st, &s->s_components.set_filter, es_filter_link)
      if ((s->s_scrambled_pass || st->es_type != SCT_CA) &&
          st->es_pid != s->s_components.set_pmt_pid && st->es_pid != s->s_components.set_pcr_pid &&
          st->es_pid < 8192) {
        st->es_pid_opened = 1;
...
static void mpegts_input_started_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
...
static void mpegts_input_stopping_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
...
static void mpegts_input_stopped_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
  char          buf[256];
  service_t *   s, *s_next;
  mpegts_mux_t* mm = mmi->mmi_mux;
...
static void mpegts_input_error(mpegts_input_t* mi, mpegts_mux_t* mm, int tss_flags) {
  service_t *t, *t_next;
  tvh_mutex_lock(&mi->mi_output_lock);
...
static inline int ts_sync_count(const uint8_t* tsb, int len) {
  ...
  while (len >= 188) {
    if (len >= 1880 && tsb[0 * 188] == 0x47 && tsb[1 * 188] == 0x47 && tsb[2 * 188] == 0x47 &&
        tsb[3 * 188] == 0x47 && tsb[4 * 188] == 0x47 && tsb[5 * 188] == 0x47 &&
        tsb[6 * 188] == 0x47 && tsb[7 * 188] == 0x47 && tsb[8 * 188] == 0x47 &&
        tsb[9 * 188] == 0x47) {
      len -= 1880;
      tsb += 1880;
    } else if (*tsb == 0x47) {
...
static void mpegts_input_queue_packets(mpegts_mux_instance_t* mmi, mpegts_packet_t* mp) {
...
retry:
  ...
  /* Check for sync */
  while ((len >= MIN_TS_SYN) && ((len2 = ts_sync_count(tsb, len)) < MIN_TS_SYN)) {
...
static void mpegts_input_table_restart(mpegts_mux_t* mm, const char* logprefix, int fast) {
  mpegts_table_t* mt;
  tvh_mutex_lock(&mm->mm_tables_lock);
  LIST_FOREACH (mt, &mm->mm_tables, mt_link) {
    if (fast && (mt->mt_flags & MT_FAST) == 0)
...
static int mpegts_input_process(mpegts_input_t* mi, mpegts_packet_t* mpkt) {
  ...
  while (len > 0) {
    ...
    if ((mp = mpegts_mux_find_pid(mm, pid, 0))) {
      ...
      /* Low level CC check */
      if (tsb[3] & 0x10) {
        for (tsb2 = tsb, tsb2_end = tsb + llen, cc2 = mp->mp_cc; tsb2 < tsb2_end; tsb2 += 188) {
          cc = tsb2[3] & 0x0f;
          if (cc2 != 0xff && cc2 != cc) {
      ...
      if (type & MPS_SERVICE) {
        LIST_FOREACH (mps, &mp->mp_svc_subs, mps_svcraw_link) {
          s = mps->mps_owner;
          f = (type & (MPS_TABLE | MPS_FTABLE)) || (pid == s->s_components.set_pmt_pid) ||
        ...
        if (!(tsb[1] & 0x80)) {
          ...
          if (type & MPS_TABLE) {
            if (mi->mi_table_queue_size >= 2 * 1024 * 1024) {
              if (tvhlog_limit(&mi->mi_input_queue_loglimit, 10)) {
                tvhwarn(LS_MPEGTS,
                    "too much queued table input data (over 2MB) for %s, discarding new",
                    mi->mi_name);
                if (tvhtrace_enabled())
  ...
  /* Raw stream */
  if (tsb != mpkt->mp_data && LIST_FIRST(&mmi->mmi_streaming_pad.sp_targets) != NULL) {
 ...
 */
void mpegts_input_postdemux(mpegts_input_t* mi, mpegts_mux_t* mm, uint8_t* tsb, int len) {
  uint16_t               pid;
  int                    llen, type = 0;
  mpegts_pid_t*          mp;
  mpegts_pid_sub_t*      mps;
...
static void mpegts_input_stream_status(mpegts_mux_instance_t* mmi, tvh_input_stream_t* st) {
  int                s = 0, w = 0;
...
static void mpegts_input_get_streams(tvh_input_t* i, tvh_input_stream_list_t* isl) {
...
static void mpegts_input_thread_start(void* aux) {
  ...
  tvh_thread_create(&mi->mi_table_tid, NULL, mpegts_input_table_thread, mi, "mi-table");
  tvh_thread_create(&mi->mi_input_tid, NULL, mpegts_input_thread, mi, "mi-main");
}
...
mpegts_input_t*
mpegts_input_create0(mpegts_input_t* mi, const idclass_t* class, const char* uuid, htsmsg_t* c) {
  char buf[32];
