...
static void  linuxdvb_frontend_monitor(void* aux);
static void* linuxdvb_frontend_input_thread(void* aux);
...
static inline int lfe_group(linuxdvb_frontend_t* lfe1, linuxdvb_frontend_t* lfe2) {
...
static void linuxdvb_frontend_class_changed(idnode_t* in) {
...
const idclass_t linuxdvb_frontend_class = {.ic_super = &mpegts_input_class,
...
const idclass_t linuxdvb_frontend_dvbt_class = {.ic_super = &linuxdvb_frontend_class,
...
static idnode_set_t* linuxdvb_frontend_dvbs_class_get_childs(idnode_t* self) {
...
static int linuxdvb_frontend_dvbs_class_satconf_set(void* self, const void* str) {
...
static const void* linuxdvb_frontend_dvbs_class_satconf_get(void* self) {
  linuxdvb_frontend_t* lfe = self;
  if (lfe->lfe_satconf)
...
static htsmsg_t* linuxdvb_frontend_dvbs_class_master_enum(void* self, const char* lang) {
...
const idclass_t linuxdvb_frontend_dvbs_class = {.ic_super = &linuxdvb_frontend_class,
...
const idclass_t linuxdvb_frontend_dvbs_slave_class = {.ic_super = &linuxdvb_frontend_class,
...
const idclass_t linuxdvb_frontend_dvbc_class = {.ic_super = &linuxdvb_frontend_class,
    .ic_class                                             = "linuxdvb_frontend_dvbc",
    .ic_doc                                               = tvh_doc_linuxdvb_frontend_dvbc_class,
    .ic_caption    = N_("TV Adapters - Linux DVB-C Frontend"),
    .ic_properties = (const property_t[]){{}}};

const idclass_t linuxdvb_frontend_atsc_t_class = {.ic_super = &linuxdvb_frontend_class,
    .ic_class                                               = "linuxdvb_frontend_atsc_t",
    .ic_doc                                                 = tvh_doc_linuxdvb_frontend_dvbt_class,
    .ic_caption    = N_("TV Adapters - Linux ATSC-T Frontend"),
    .ic_properties = (const property_t[]){{}}};

const idclass_t linuxdvb_frontend_atsc_c_class = {.ic_super = &linuxdvb_frontend_class,
    .ic_class                                               = "linuxdvb_frontend_atsc_c",
    .ic_doc                                                 = tvh_doc_linuxdvb_frontend_dvbc_class,
    .ic_caption    = N_("TV Adapters - Linux ATSC-C Frontend"),
    .ic_properties = (const property_t[]){{}}};

const idclass_t linuxdvb_frontend_isdb_t_class = {.ic_super = &linuxdvb_frontend_class,
    .ic_class                                               = "linuxdvb_frontend_isdb_t",
    .ic_doc                                                 = tvh_doc_linuxdvb_frontend_dvbt_class,
    .ic_caption    = N_("TV Adapters - Linux ISDB-T Frontend"),
    .ic_properties = (const property_t[]){{}}};

const idclass_t linuxdvb_frontend_isdb_c_class = {.ic_super = &linuxdvb_frontend_class,
    .ic_class                                               = "linuxdvb_frontend_isdb_c",
    .ic_doc                                                 = tvh_doc_linuxdvb_frontend_dvbc_class,
    .ic_caption    = N_("TV Adapters - Linux ISDB-C Frontend"),
    .ic_properties = (const property_t[]){{}}};

const idclass_t linuxdvb_frontend_isdb_s_class = {.ic_super = &linuxdvb_frontend_class,
    .ic_class                                               = "linuxdvb_frontend_isdb_s",
    .ic_doc                                                 = tvh_doc_linuxdvb_frontend_dvbs_class,
    .ic_caption    = N_("TV Adapters - Linux ISDB-S Frontend (Master)"),
    .ic_properties = (const property_t[]){{}}};

const idclass_t linuxdvb_frontend_dtmb_class = {.ic_super = &linuxdvb_frontend_class,
    .ic_class                                             = "linuxdvb_frontend_dab",
    .ic_caption                                           = N_("TV Adapters - DTMB Frontend"),
    .ic_properties                                        = (const property_t[]){{}}};

const idclass_t linuxdvb_frontend_dab_class = {.ic_super = &linuxdvb_frontend_class,
    .ic_class                                            = "linuxdvb_frontend_dab",
...
static void linuxdvb_frontend_close_fd(linuxdvb_frontend_t* lfe, const char* name) {
  char buf[256];
...
static int linuxdvb_frontend_open_fd(linuxdvb_frontend_t* lfe, const char* name) {
  ...
  tvhtrace(LS_LINUXDVB, "%s - opening FE %s (%d)%s", name, lfe->lfe_fe_path, lfe->lfe_fe_fd, extra);

  return lfe->lfe_fe_fd <= 0;
}

static void linuxdvb_frontend_enabled_updated(mpegts_input_t* mi) {
  char                 buf[512];
...
static int linuxdvb_frontend_get_priority(mpegts_input_t* mi, mpegts_mux_t* mm, int flags) {
  linuxdvb_frontend_t* lfe = (linuxdvb_frontend_t*)mi;
...
static int linuxdvb_frontend_get_grace(mpegts_input_t* mi, mpegts_mux_t* mm) {
  linuxdvb_frontend_t* lfe = (linuxdvb_frontend_t*)mi;
  return lfe->lfe_satconf ? linuxdvb_satconf_get_grace(lfe->lfe_satconf, mm)
                          : lfe->lfe_grace_period;
}

static int
linuxdvb_frontend_is_enabled(mpegts_input_t* mi, mpegts_mux_t* mm, int flags, int weight) {
  linuxdvb_frontend_t *lfe = (linuxdvb_frontend_t*)mi, *lfe2;
  linuxdvb_adapter_t*  la;
  tvh_hardware_t*      th;
  char                 ubuf[UUID_HEX_SIZE];
  ...
  LIST_FOREACH (th, &tvh_hardware, th_link) {
    ...
    LIST_FOREACH (lfe2, &la->la_frontends, lfe_link) {
      ...
      if (lfe2->lfe_master && !strcmp(lfe2->lfe_master, idnode_uuid_as_str(&lfe->ti_id, ubuf)) &&
          lfe2->lfe_refcount > 0) {
...
ok:
  w = -1;
  LIST_FOREACH (lfe2, &lfe->lfe_adapter->la_frontends, lfe_link) {
    if (lfe2 == lfe || !lfe_group(lfe, lfe2))
      continue;
    w = MAX(w, mpegts_input_get_weight((mpegts_input_t*)lfe2, mm, flags, weight));
  }
  if (w >= weight)
...
static void linuxdvb_frontend_stop_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
  char                 buf1[256];
  linuxdvb_frontend_t *lfe = (linuxdvb_frontend_t*)mi, *lfe2;
...
static int linuxdvb_frontend_warm_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
...
static int linuxdvb_frontend_start_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi, int weight) {
  linuxdvb_frontend_t *lfe = (linuxdvb_frontend_t*)mi, *lfe2;
  int                  res, f;
...
static void linuxdvb_frontend_update_pids(mpegts_input_t* mi, mpegts_mux_t* mm) {
  linuxdvb_frontend_t* lfe = (linuxdvb_frontend_t*)mi;
  mpegts_pid_t*        mp;
...
static void linuxdvb_frontend_create_mux_instance(mpegts_input_t* mi, mpegts_mux_t* mm) {
  tvh_hardware_t*      th;
  linuxdvb_adapter_t*  la;
  linuxdvb_frontend_t *lfe = (linuxdvb_frontend_t*)mi, *lfe2;
  char                 ubuf[UUID_HEX_SIZE];
...
static idnode_set_t* linuxdvb_frontend_network_list(mpegts_input_t* mi) {
  linuxdvb_frontend_t* lfe = (linuxdvb_frontend_t*)mi;

  tvhtrace(LS_LINUXDVB, "%s: network list for %s", mi->mi_name ?: "", dvb_type2str(lfe->lfe_type));
...
static inline int ioctl_check(linuxdvb_frontend_t* lfe, int bit) {
  return !(lfe->lfe_ioctls & (1 << bit));
}

static inline void ioctl_bad(linuxdvb_frontend_t* lfe, int bit) {
...
static void linuxdvb_frontend_monitor(void* aux) {
  ...
  if (fe_status != lfe->lfe_status) {
    tvhdebug(LS_LINUXDVB,
        "%s - status %7s (%s%s%s%s%s%s)",
        buf,
        signal2str(status),
  ...
  if (!lfe->lfe_locked) {
    ...
    if (status == SIGNAL_GOOD) {
      ...
      tvh_mutex_lock(&lfe->lfe_dvr_lock);
      tvh_thread_create(&lfe->lfe_dvr_thread,
          NULL,
      ...
      /* Table handlers */
      psi_tables_install((mpegts_input_t*)lfe, mm, ((dvb_mux_t*)mm)->lm_tuning.dmc_fe_delsys);
  ...
  if (gotprop) {
    ...
    if (ioctl_check(lfe, 1) && fe_properties[0].u.st.len > 0) {
      if (fe_properties[0].u.st.stat[0].scale == FE_SCALE_RELATIVE) {
        ...
        mmi->tii_stats.signal =
            sig_multiply(fe_properties[0].u.st.stat[0].uvalue, lfe->lfe_sig_multiplier);
      } else if (fe_properties[0].u.st.stat[0].scale == FE_SCALE_DECIBEL) {
        mmi->tii_stats.signal_scale = SIGNAL_STATUS_SCALE_DECIBEL;
        mmi->tii_stats.signal =
            sig_multiply(fe_properties[0].u.st.stat[0].svalue, lfe->lfe_sig_multiplier);
        ...
        mmi->tii_stats.signal_scale = SIGNAL_STATUS_SCALE_UNKNOWN;
        if (logit)
          tvhwarn(LS_LINUXDVB, "Unhandled signal scale: %d", fe_properties[0].u.st.stat[0].scale);
      }
    }
    /* ERROR_BIT_COUNT */
    if (ioctl_check(lfe, 3) && fe_properties[1].u.st.len > 0) {
      if (fe_properties[1].u.st.stat[0].scale == FE_SCALE_COUNTER) {
        ...
        if (logit)
          tvhwarn(LS_LINUXDVB,
              "Unhandled ERROR_BIT_COUNT scale: %d",
    ...
    if (fe_properties[2].u.st.len > 0) {
      if (ioctl_check(lfe, 4) && fe_properties[2].u.st.stat[0].scale == FE_SCALE_COUNTER) {
        ...
        if (logit && fe_properties[2].u.st.stat[0].scale == FE_SCALE_COUNTER)
          tvhwarn(LS_LINUXDVB,
              "Unhandled TOTAL_BIT_COUNT scale: %d",
    ...
    if (ioctl_check(lfe, 6) && fe_properties[3].u.st.len > 0) {
      if (fe_properties[3].u.st.stat[0].scale == FE_SCALE_RELATIVE) {
        ...
        mmi->tii_stats.snr =
            sig_multiply(fe_properties[3].u.st.stat[0].uvalue, lfe->lfe_snr_multiplier);
      } else if (fe_properties[3].u.st.stat[0].scale == FE_SCALE_DECIBEL) {
        mmi->tii_stats.snr_scale = SIGNAL_STATUS_SCALE_DECIBEL;
        mmi->tii_stats.snr =
            sig_multiply(fe_properties[3].u.st.stat[0].svalue, lfe->lfe_snr_multiplier);
        ...
        mmi->tii_stats.snr_scale = SIGNAL_STATUS_SCALE_UNKNOWN;
        if (logit)
          tvhwarn(LS_LINUXDVB, "Unhandled SNR scale: %d", fe_properties[3].u.st.stat[0].scale);
      }
    }
    ...
    if (ioctl_check(lfe, 8) && fe_properties[4].u.st.len > 0) {
      if (fe_properties[4].u.st.stat[0].scale == FE_SCALE_COUNTER) {
        ...
        if (logit)
          tvhwarn(LS_LINUXDVB,
              "Unhandled ERROR_BLOCK_COUNT scale: %d",
    ...
    if (ioctl_check(lfe, 9) && fe_properties[5].u.st.len > 0) {
      if (fe_properties[5].u.st.stat[0].scale == FE_SCALE_COUNTER) {
        ...
        if (logit)
          tvhwarn(LS_LINUXDVB,
              "Unhandled TOTAL_BLOCK_COUNT scale: %d",
...
static void linuxdvb_frontend_open_pid0(linuxdvb_frontend_t* lfe,
    const char*                                              name,
    linuxdvb_pid_t*                                          pids,
    int                                                      pids_size,
    int                                                      pid) {
  ...
  for (; pids_size > 0 && pids->fd >= 0; pids_size--, pids++)
    ;
  if (pids_size == 0) {
    tvherror(LS_LINUXDVB, "%s - maximum PID count reached, pid %d ignored", name, pid);
    return;
  }
  ...
  if (fd == -1) {
    tvherror(LS_LINUXDVB, "%s - failed to open dmx for pid %d [e=%s]", name, pid, strerror(errno));
    return;
  }
  ...
  if (ioctl(fd, DMX_SET_PES_FILTER, &dmx_param)) {
    tvherror(LS_LINUXDVB,
        "%s - failed to config dmx for pid %d [e=%s]",
        name,
        pid,
        strerror(errno));
    close(fd);
...
static void linuxdvb_frontend_close_pid0(linuxdvb_frontend_t* lfe,
    const char*                                               name,
    linuxdvb_pid_t*                                           pids,
    int                                                       pids_size,
    int                                                       pid) {
  for (; pids_size > 0 && pids->pid != pid; pids_size--, pids++)
    ;
  if (pids_size == 0)
    return;
...
static int linuxdvb_pid_exists(linuxdvb_pid_t* pids, int pids_size, int pid) {
  int i;
...
static void linuxdvb_update_pids(linuxdvb_frontend_t* lfe,
    const char*                                       name,
...
static void* linuxdvb_frontend_input_thread(void* aux) {
  ...
  while (tvheadend_is_running()) {
    ...
    if ((n = sbuf_read(&sb, dvr)) < 0) {
      ...
      tvherror(LS_LINUXDVB, "%s - read() error %d (%s)", name, errno, strerror(errno));
      break;
    }
...
static int linuxdvb2tvh(const char* prefix, linuxdvb_tbl_t* tbl, int key, int defval) {
...
static int tvh2linuxdvb(const char* prefix, linuxdvb_tbl_t* tbl, int key, int defval) {
  while (tbl->t != TABLE_EOD) {
    if (tbl->t == key)
      return tbl->l;
    tbl++;
...
#if DVB_API_VERSION >= 5
static linuxdvb_tbl_t delsys_tbl[] = {{.t = DVB_SYS_DVBC_ANNEX_B, .l = SYS_DVBC_ANNEX_B},
...
int linuxdvb_frontend_clear(linuxdvb_frontend_t* lfe, mpegts_mux_instance_t* mmi) {
  char buf1[256];
  ...
  static struct dtv_properties clear_cmdseq = {.num = 1, .props = clear_p};
...
int linuxdvb_frontend_tune0(linuxdvb_frontend_t* lfe, mpegts_mux_instance_t* mmi, uint32_t freq) {
  static linuxdvb_tbl_t inv_tbl[] = {{.t = DVB_INVERSION_AUTO, .l = INVERSION_AUTO},
      {.t = DVB_INVERSION_OFF, .l = INVERSION_OFF},
      {.t = DVB_INVERSION_ON, .l = INVERSION_ON},
      {.t = TABLE_EOD}};
  static linuxdvb_tbl_t bw_tbl[]  = {{.t = DVB_BANDWIDTH_AUTO, .l = BANDWIDTH_AUTO},
#if DVB_VER_ATLEAST(5, 3)
...
#endif
      {.t = TABLE_EOD}};
  static linuxdvb_tbl_t fec_tbl[] = {{.t = DVB_FEC_NONE, .l = FEC_NONE},
...
#endif
      {.t = TABLE_EOD}};
  static linuxdvb_tbl_t mod_tbl[] = {{.t = DVB_MOD_AUTO, .l = QAM_AUTO},
...
#endif
      {.t = TABLE_EOD}};
  static linuxdvb_tbl_t guard_tbl[] = {{.t = DVB_GUARD_INTERVAL_AUTO, .l = GUARD_INTERVAL_AUTO},
...
#endif
      {.t = TABLE_EOD}};
  static linuxdvb_tbl_t h_tbl[] = {{.t = DVB_HIERARCHY_NONE, .l = HIERARCHY_NONE},
      ...
      {.t = TABLE_EOD}};
#if DVB_VER_ATLEAST(5, 0)
  static linuxdvb_tbl_t pilot_tbl[]   = {{.t = DVB_PILOT_AUTO, .l = PILOT_AUTO},
...
#define _OFDM(xyz) p.u.ofdm.xyz
      ...
      _OFDM(hierarchy_information) = TRU(ofdm.hierarchy_information, h_tbl, HIERARCHY_AUTO);
#undef _OFDM
      break;
    case DVB_TYPE_C:
  ...
  if (dmc->dmc_fe_type != lfe->lfe_type) {
    tvherror(LS_LINUXDVB,
        "%s - failed to tune [type does not match %i != %i]",
        buf1,
        dmc->dmc_fe_type,
        lfe->lfe_type);
    return SM_CODE_TUNING_FAILED;
...
#define S2CMD(c, d)                \
  ...
  if (lfe->lfe_type == DVB_TYPE_T || lfe->lfe_type == DVB_TYPE_DTMB) {
    ...
    /* DVB-C */
  } else if (lfe->lfe_type == DVB_TYPE_C || lfe->lfe_type == DVB_TYPE_ATSC_C ||
    ...
    r = dmc->dmc_fe_stream_id != DVB_NO_STREAM_ID_FILTER
        ? (dmc->dmc_fe_stream_id & 0xFF) | ((dmc->dmc_fe_data_slice & 0xFF) << 8)
    ...
    for (i = j = 0; i < 3; i++, j += DTV_ISDBT_LAYERB_FEC - DTV_ISDBT_LAYERA_FEC) {
      ...
      S2CMD(DTV_ISDBT_LAYERA_TIME_INTERLEAVING + j,
          dmc->u.dmc_fe_isdbt.layers[i].time_interleaving);
    }
#endif
...
int linuxdvb_frontend_tune1(linuxdvb_frontend_t* lfe, mpegts_mux_instance_t* mmi, uint32_t freq) {
  int  r = 0, i, rep;
  char buf1[256];
...
static mpegts_network_t* linuxdvb_frontend_wizard_network(linuxdvb_frontend_t* lfe) {
...
static htsmsg_t* linuxdvb_frontend_wizard_get(tvh_input_t* ti, const char* lang) {
...
static void linuxdvb_frontend_wizard_set(tvh_input_t* ti, htsmsg_t* conf, const char* lang) {
  linuxdvb_frontend_t* lfe   = (linuxdvb_frontend_t*)ti;
...
linuxdvb_frontend_t* linuxdvb_frontend_create(htsmsg_t* conf,
    linuxdvb_adapter_t*                                 la,
    int                                                 number,
    const char*                                         fe_path,
    const char*                                         dmx_path,
  ...
  if (conf) {
    if (!htsmsg_get_list(conf, "networks") && (str = htsmsg_get_str(conf, "network"))) {
...
void linuxdvb_frontend_save(linuxdvb_frontend_t* lfe, htsmsg_t* fe) {
  char      id[16];
