  ...
  if (!(*active)) {
    LIST_FOREACH (lfe2, &lfe->lfe_adapter->la_frontends, lfe_link) {
      if (!lfe_group(lfe, lfe2))
        continue;
      if (lfe2->mi_is_enabled((mpegts_input_t*)lfe2, NULL, 0, -1) !=
          MI_IS_ENABLED_NEVER) {
        *active = 1;
        return active;
      }
...
const idclass_t linuxdvb_frontend_class = {.ic_super = &mpegts_input_class,
    ...
    .ic_properties = (const property_t[]){
        ...
        {.type     = PT_BOOL,
            ...
            .desc  = N_("Allow all PIDs (no filter) when the 'Maximum PIDs' "
                       "limit is reached."),
        ...
        },
        {
            ...
            .desc =
                N_("If set, the first bytes from the MPEG-TS stream "
...
static int linuxdvb_frontend_dvbs_class_satconf_set(void* self,
    const void*                                           str) {
...
static htsmsg_t* linuxdvb_frontend_dvbs_class_master_enum(void* self,
    const char*                                                 lang) {
  ...
  LIST_FOREACH (th, &tvh_hardware, th_link) {
    ...
    LIST_FOREACH (lfe2, &la->la_frontends, lfe_link) {
      if (lfe2 != lfe && lfe2->lfe_type == lfe->lfe_type) {
        e = htsmsg_create_key_val(idnode_uuid_as_str(&lfe2->ti_id, ubuf),
            lfe2->mi_name);
        htsmsg_add_msg(m, NULL, e);
      }
    }
...
static void linuxdvb_frontend_close_fd(linuxdvb_frontend_t* lfe,
    const char*                                             name) {
  char buf[256];
...
static int linuxdvb_frontend_open_fd(linuxdvb_frontend_t* lfe,
    const char*                                           name) {
  linuxdvb_frontend_t* lfe2;
  const char*          extra = "";
...
static int linuxdvb_frontend_get_weight(mpegts_input_t* mi,
    mpegts_mux_t*                                       mm,
    int                                                 flags,
    int                                                 weight) {
  int                  w   = 0;
  ...
  LIST_FOREACH (lfe2, &lfe->lfe_adapter->la_frontends, lfe_link) {
    if (!lfe_group(lfe, lfe2))
      continue;
    w = MAX(w,
        mpegts_input_get_weight((mpegts_input_t*)lfe2, mm, flags, weight));
  }
  return w;
}

static int linuxdvb_frontend_get_priority(mpegts_input_t* mi,
    mpegts_mux_t*                                         mm,
    int                                                   flags) {
  linuxdvb_frontend_t* lfe = (linuxdvb_frontend_t*)mi;
...
static int linuxdvb_frontend_is_enabled(mpegts_input_t* mi,
    mpegts_mux_t*                                       mm,
    int                                                 flags,
    int                                                 weight) {
  linuxdvb_frontend_t *lfe = (linuxdvb_frontend_t*)mi, *lfe2;
  ...
  LIST_FOREACH (th, &tvh_hardware, th_link) {
    ...
    LIST_FOREACH (lfe2, &la->la_frontends, lfe_link) {
      ...
      if (lfe2->lfe_type != DVB_TYPE_S)
        continue;
      if (lfe->lfe_master &&
          !strcmp(lfe->lfe_master, idnode_uuid_as_str(&lfe2->ti_id, ubuf))) {
...
static void linuxdvb_frontend_stop_mux(mpegts_input_t* mi,
  ...
  mtimer_arm_rel(&lfe->lfe_monitor_timer,
      linuxdvb_frontend_monitor,
      lfe,
      sec2mono(2));
  ...
  if (lfe->lfe_master) {
    lfe2 = (linuxdvb_frontend_t*)idnode_find(lfe->lfe_master,
    ...
    if (lfe2) { /* master tuner shutdown procedure */
      ...
      if (--lfe2->lfe_refcount == 0) {
        ...
        lfe2->lfe_status2 = 0;
        /* Ensure it won't happen immediately */
        mtimer_arm_rel(&lfe2->lfe_monitor_timer,
            linuxdvb_frontend_monitor,
            lfe,
            sec2mono(2));
        if (lfe2->lfe_satconf)
...
static int linuxdvb_frontend_warm_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                            mmi) {
...
static int linuxdvb_frontend_start_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                             mmi,
    int                                                weight) {
  linuxdvb_frontend_t *lfe = (linuxdvb_frontend_t*)mi, *lfe2;
  int                  res, f;
  ...
  if (lfe->lfe_satconf)
    res = linuxdvb_satconf_start_mux(lfe->lfe_satconf,
        mmi,
        lfe->lfe_refcount > 1);
  else
...
static void linuxdvb_frontend_update_pids(mpegts_input_t* mi,
    mpegts_mux_t*                                         mm) {
  linuxdvb_frontend_t* lfe = (linuxdvb_frontend_t*)mi;
  mpegts_pid_t*        mp;
...
static void linuxdvb_frontend_create_mux_instance(mpegts_input_t* mi,
    mpegts_mux_t*                                                 mm) {
  tvh_hardware_t*      th;
  linuxdvb_adapter_t*  la;
  linuxdvb_frontend_t *lfe = (linuxdvb_frontend_t*)mi, *lfe2;
...
static idnode_set_t* linuxdvb_frontend_network_list(mpegts_input_t* mi) {
  ...
  tvhtrace(LS_LINUXDVB,
      "%s: network list for %s",
      mi->mi_name ?: "",
      dvb_type2str(lfe->lfe_type));
...
static void linuxdvb_frontend_monitor(void* aux) {
  ...
  retune = NOSIGNAL(fe_status) && NOSIGNAL(lfe->lfe_status) &&
      !NOSIGNAL(lfe->lfe_status2);
  ...
  if (!lfe->lfe_locked) {
    ...
    if (status == SIGNAL_GOOD) {
      ...
      /* Table handlers */
      psi_tables_install((mpegts_input_t*)lfe,
          mm,
  ...
  if (gotprop) {
    ...
    if (ioctl_check(lfe, 1) && fe_properties[0].u.st.len > 0) {
      if (fe_properties[0].u.st.stat[0].scale == FE_SCALE_RELATIVE) {
        ...
        mmi->tii_stats.signal =
            sig_multiply(fe_properties[0].u.st.stat[0].uvalue,
                lfe->lfe_sig_multiplier);
      } else if (fe_properties[0].u.st.stat[0].scale == FE_SCALE_DECIBEL) {
        mmi->tii_stats.signal_scale = SIGNAL_STATUS_SCALE_DECIBEL;
        mmi->tii_stats.signal =
            sig_multiply(fe_properties[0].u.st.stat[0].svalue,
                lfe->lfe_sig_multiplier);
      } else if (fe_properties[0].u.st.stat[0].scale ==
    ...
    if (fe_properties[2].u.st.len > 0) {
      if (ioctl_check(lfe, 4) &&
          fe_properties[2].u.st.stat[0].scale == FE_SCALE_COUNTER) {
    ...
    if (ioctl_check(lfe, 6) && fe_properties[3].u.st.len > 0) {
      if (fe_properties[3].u.st.stat[0].scale == FE_SCALE_RELATIVE) {
        ...
        mmi->tii_stats.snr = sig_multiply(fe_properties[3].u.st.stat[0].uvalue,
            lfe->lfe_snr_multiplier);
      } else if (fe_properties[3].u.st.stat[0].scale == FE_SCALE_DECIBEL) {
        mmi->tii_stats.snr_scale = SIGNAL_STATUS_SCALE_DECIBEL;
        mmi->tii_stats.snr = sig_multiply(fe_properties[3].u.st.stat[0].svalue,
            lfe->lfe_snr_multiplier);
      } else if (fe_properties[3].u.st.stat[0].scale ==
          FE_SCALE_NOT_AVAILABLE) {
  ...
  {
    ioctl_bad(lfe, 0);
    if (ioctl_check(lfe, 1) &&
        !ioctl(lfe->lfe_fe_fd, FE_READ_SIGNAL_STRENGTH, &u16)) {
      mmi->tii_stats.signal_scale = SIGNAL_STATUS_SCALE_RELATIVE;
    ...
    if (ioctl_check(lfe, 4) &&
        !ioctl(lfe->lfe_fe_fd, FE_READ_UNCORRECTED_BLOCKS, &u32))
...
static void linuxdvb_frontend_open_pid0(linuxdvb_frontend_t* lfe,
  ...
  if (pids_size == 0) {
    tvherror(LS_LINUXDVB,
        "%s - maximum PID count reached, pid %d ignored",
        name,
        pid);
    return;
  }
  ...
  if (fd == -1) {
    tvherror(LS_LINUXDVB,
        "%s - failed to open dmx for pid %d [e=%s]",
        name,
        pid,
        strerror(errno));
    return;
...
static void linuxdvb_update_pids(linuxdvb_frontend_t* lfe,
  ...
  if (!all) {
    overlimit =
        mpegts_pid_weighted(&wpid, &lfe->lfe_pids, max, MPS_WEIGHT_ALLLIMIT);
    if (tvhtrace_enabled()) {
    ...
    if (overlimit && lfe->lfe_pids_use_all) {
      ...
      if (overlimit)
        tvhwarn(LS_LINUXDVB,
            "%s - unable to set filter for %d PIDs",
            name,
            overlimit);
      mpegts_pid_compare(&wpid, tuned, &padd, &pdel);
      for (i = 0; i < pdel.count; i++)
        linuxdvb_frontend_close_pid0(lfe,
            name,
            pids,
            pids_size,
            pdel.pids[i].pid);
      for (i = 0; i < padd.count; i++)
        linuxdvb_frontend_open_pid0(lfe,
            name,
            pids,
            pids_size,
            padd.pids[i].pid);
...
int linuxdvb_frontend_clear(linuxdvb_frontend_t* lfe,
    mpegts_mux_instance_t*                       mmi) {
  char buf1[256];
  ...
  if (mmi) {
    ...
    if ((ioctl(lfe->lfe_fe_fd, FE_SET_PROPERTY, &delsys_cmdseq)) != 0) {
      tvherror(LS_LINUXDVB,
          "%s - DTV_DELIVERY_SYSTEM failed [e=%s]",
          buf1,
          strerror(errno));
      return -1;
    }
...
int linuxdvb_frontend_tune0(linuxdvb_frontend_t* lfe,
    mpegts_mux_instance_t*                       mmi,
    uint32_t                                     freq) {
...
#define _OFDM(xyz) p.u.ofdm.xyz
      ...
      _OFDM(transmission_mode) =
          TRU(ofdm.transmission_mode, trans_tbl, TRANSMISSION_MODE_AUTO);
      _OFDM(guard_interval) =
          TRU(ofdm.guard_interval, guard_tbl, GUARD_INTERVAL_AUTO);
...
#define S2CMD(c, d)                \
  ...
  if (lfe->lfe_type == DVB_TYPE_T || lfe->lfe_type == DVB_TYPE_DTMB) {
    ...
    r = dmc->dmc_fe_stream_id != DVB_NO_STREAM_ID_FILTER
        ? (dmc->dmc_fe_stream_id & 0xFF) |
            ((dmc->dmc_fe_data_slice & 0xFF) << 8)
...
#if DVB_VER_ATLEAST(5, 1)
    S2CMD(DTV_GUARD_INTERVAL,
        TRU(isdbt.guard_interval, guard_tbl, GUARD_INTERVAL_AUTO));
    ...
    for (i = j = 0; i < 3;
         i++, j += DTV_ISDBT_LAYERB_FEC - DTV_ISDBT_LAYERA_FEC) {
      S2CMD(DTV_ISDBT_LAYERA_FEC + j,
          TRU(isdbt.layers[i].fec, fec_tbl, FEC_AUTO));
      S2CMD(DTV_ISDBT_LAYERA_MODULATION + j,
          TRU(isdbt.layers[i].modulation, mod_tbl, QAM_AUTO));
      S2CMD(DTV_ISDBT_LAYERA_SEGMENT_COUNT + j,
          dmc->u.dmc_fe_isdbt.layers[i].segment_count);
...
int linuxdvb_frontend_tune1(linuxdvb_frontend_t* lfe,
    mpegts_mux_instance_t*                       mmi,
    uint32_t                                     freq) {
  int  r = 0, i, rep;
  char buf1[256];
...
static mpegts_network_t* linuxdvb_frontend_wizard_network(
...
static htsmsg_t* linuxdvb_frontend_wizard_get(tvh_input_t* ti,
    const char*                                            lang) {
...
static void linuxdvb_frontend_wizard_set(tvh_input_t* ti,
    htsmsg_t*                                         conf,
    const char*                                       lang) {
  linuxdvb_frontend_t* lfe   = (linuxdvb_frontend_t*)ti;
...
linuxdvb_frontend_t* linuxdvb_frontend_create(htsmsg_t* conf,
  ...
  snprintf(lname,
      sizeof(lname),
      "/sys/class/dvb/dvb%d.frontend%d",
      la->la_dvb_number,
      number);
