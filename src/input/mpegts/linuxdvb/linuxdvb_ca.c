...
linuxdvb_ca_slot_info(int fd, linuxdvb_transport_t* lcat, linuxdvb_ca_t* lca) {
  ...
  if (csi.flags & CA_CI_MODULE_READY) {
    if (lcat->lcat_fatal_time + sec2mono(TRANSPORT_RECOVERY) < mclk()) {
      ...
      slot =
          en50221_transport_find_slot(lcat->lcat_transport, lca->lca_slotnum);
      if (slot) {
        if (slot->cil_ready)
...
static int linuxdvb_ca_process_cmd(linuxdvb_ca_t* lca,
    linuxdvb_ca_write_t*                          lcw,
    en50221_slot_t*                               slot) {
  int r        = 0;
  ...
  switch (lcw->cmd) {
    ...
    case CA_WRITE_CMD_PCMCIA_RATE:
      if (lcw->len == 1) {
        ...
        if (r < 0)
          tvherror(LS_EN50221,
              "%s: unable to write pcmcia data rate (%d)",
              lca->lca_name,
              r);
      }
      break;
...
static void linuxdvb_ca_thread_create(void) {
  if (linuxdvb_ca_threadid)
    return;
  tvh_thread_create(&linuxdvb_ca_threadid,
      NULL,
      linuxdvb_ca_thread,
      NULL,
      "lnxdvb-ca");
...
static int linuxdvb_ca_write_cmd(linuxdvb_ca_t* lca,
    int                                         cmd,
    const uint8_t*                              data,
    size_t                                      datalen) {
  linuxdvb_ca_write_t* lcw;
...
static void linuxdvb_ca_class_get_title(idnode_t* in,
    const char*                                   lang,
    char*                                         dst,
    size_t                                        dstsize) {
...
linuxdvb_ca_t*
linuxdvb_ca_create(htsmsg_t* conf, linuxdvb_transport_t* lcat, int slotnum) {
  linuxdvb_ca_t* lca;
  char           id[32], buf[32];
...
static linuxdvb_ca_t* linuxdvb_ca_find_slot(linuxdvb_transport_t* lcat,
    en50221_slot_t*                                               slot) {
  linuxdvb_ca_t* lca;
  LIST_FOREACH (lca, &lcat->lcat_slots, lca_link)
...
static int linuxdvb_ca_ops_apdu_write(void* aux,
    en50221_slot_t*                         slot,
    const uint8_t*                          data,
    size_t                                  datalen) {
  linuxdvb_transport_t* lcat = aux;
  ...
  if (datalen > 256) {
    tvherror(LS_EN50221,
        "%s: unable to write APDU with length %zd",
        lcat->lcat_name,
        datalen);
    return -E2BIG;
  }
...
static int linuxdvb_ca_ops_pcmcia_data_rate(void* aux,
    en50221_slot_t*                               slot,
    uint8_t*                                      rate) {
  linuxdvb_transport_t* lcat = aux;
  linuxdvb_ca_t*        lca  = linuxdvb_ca_find_slot(lcat, slot);
...
static int linuxdvb_ca_ops_caids(void* aux,
    en50221_slot_t*                    slot,
    uint16_t*                          list,
    int                                listsize) {
  linuxdvb_transport_t* lcat = aux;
...
static int
linuxdvb_ca_ops_menu(void* aux, en50221_slot_t* slot, htsmsg_t* menu) {
  linuxdvb_transport_t* lcat = aux;
  linuxdvb_ca_t*        lca  = linuxdvb_ca_find_slot(lcat, slot);
...
static int
linuxdvb_ca_ops_enquiry(void* aux, en50221_slot_t* slot, htsmsg_t* enq) {
  linuxdvb_transport_t* lcat = aux;
  linuxdvb_ca_t*        lca  = linuxdvb_ca_find_slot(lcat, slot);
  const char*           text;
  char*                 s;
  ...
  if (text && lca->lca_pin_reply && lca->lca_pin_str &&
      lca->lca_pin_match_str && (strlen(lca->lca_pin_str) == explen) &&
...
linuxdvb_transport_t* linuxdvb_transport_create(linuxdvb_adapter_t* la,
  ...
  r = en50221_create_transport(&linuxdvb_ca_ops,
      lcat,
      slots,
      buf,
      &lcat->lcat_transport);
...
void linuxdvb_ca_enqueue_capmt(linuxdvb_ca_t* lca,
  ...
  if (descramble && lca->lca_capmt_query &&
    ...
    if (!linuxdvb_ca_write_cmd(lca,
            CA_WRITE_CMD_CAPMT_QUERY,
            capmt2,
            capmtlen2)) {
      tvhtrace(LS_EN50221,
          "%s: CAPMT enqueued query (len %zd)",
          lca->lca_name,
          capmtlen2);
