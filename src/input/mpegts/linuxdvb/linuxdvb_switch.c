...
static htsmsg_t* linuxdvb_switch_class_committed_list(void* o, const char* lang) {
...
static htsmsg_t* linuxdvb_switch_class_uncommitted_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("NONE"), -1},
      {N_("0"), 0},
      {N_("1"), 1},
...
static htsmsg_t* linuxdvb_switch_class_toneburst_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("NONE"), -1},
      {N_("A"), 0},
      {N_("B"), 1},
...
static void
linuxdvb_switch_class_get_title(idnode_t* o, const char* lang, char* dst, size_t dstsize) {
...
static int linuxdvb_switch_tune(linuxdvb_diseqc_t* ld,
    ...
    for (i = 0; i <= lsp->ls_diseqc_repeats; i++) {
      ...
      if (ls->ls_uncommitted_first)
        ...
        if (ls->ls_uncommitted >= 0) {
          if (linuxdvb_diseqc_send(fd, 0xE0 | r2, 0x10, 0x39, 1, 0xF0 | ls->ls_uncommitted))
            return -1;
          tvh_safe_usleep(slp);
        }
  ...
  if (ls->ls_toneburst >= 0 &&
    ...
    if (ioctl(fd, FE_DISEQC_SEND_BURST, ls->ls_toneburst ? SEC_MINI_B : SEC_MINI_A)) {
