 ...
 *      * delay time is easily random, but in standard is special (complicated)
 * way described (cap. 8).
 */
...
static void linuxdvb_en50494_class_get_title(idnode_t* o,
    const char*                                        lang,
    char*                                              dst,
    size_t                                             dstsize) {
...
static void linuxdvb_en50607_class_get_title(idnode_t* o,
    const char*                                        lang,
    char*                                              dst,
    size_t                                             dstsize) {
    ...
    .ic_properties = (const property_t[]){
        {
            ...
            .desc = N_(
                "Time (in milliseconds) for the Unicable device to power up."),
...
static int linuxdvb_en50494_freq0(linuxdvb_en50494_t* le,
    int                                               freq,
    int*                                              rfreq,
    uint16_t*                                         t) {
  /* transponder value - t */
...
static int linuxdvb_en50607_freq0(linuxdvb_en50494_t* le,
    int                                               freq,
    int*                                              rfreq,
    uint16_t*                                         t) {
  /* transponder value - t */
...
static int
linuxdvb_en50494_freq(linuxdvb_diseqc_t* ld, dvb_mux_t* lm, int freq) {
...
static int
linuxdvb_en50494_match(linuxdvb_diseqc_t* ld, dvb_mux_t* lm1, dvb_mux_t* lm2) {
  return lm1 == lm2;
}
...
static int linuxdvb_en50494_tune(linuxdvb_diseqc_t* ld,
  ...
  if (!ver2) {
    ...
    data1 |= (le->le_position & 1)
        << 4;                 /* 1bit position (satellite A(0)/B(1)) */
  ...
  for (i = 0; i <= sc->lse_parent->ls_diseqc_repeats; i++) {
    ...
    tvh_safe_usleep(MINMAX(le->le_powerup_time, 10, 500) *
        1000); /* standard: 4ms < x < 22ms */
    ...
    tvh_safe_usleep(
        MINMAX(le->le_cmd_time, 10, 300) * 1000); /* standard: 2ms < x < 60ms */
...
htsmsg_t* linuxdvb_en50494_list(void* o, const char* lang) {
  ...
  htsmsg_add_msg(m,
      NULL,
      htsmsg_create_key_val(UNICABLE_I_NAME,
          tvh_gettext_lang(lang, N_(UNICABLE_I_NAME))));
  htsmsg_add_msg(m,
      NULL,
      htsmsg_create_key_val(UNICABLE_II_NAME,
          tvh_gettext_lang(lang, N_(UNICABLE_II_NAME))));
  return m;
}
...
linuxdvb_diseqc_t* linuxdvb_en50494_create0(const char* name,
  ...
  if (linuxdvb_unicable_is_en50607(name) && port > 63) {
    tvherror(LS_EN50494,
        "only 64 ports/positions are possible. given %i",
        port);
    port = 0;
  }
  ...
  ld = linuxdvb_diseqc_create0((linuxdvb_diseqc_t*)le,
      NULL,
      linuxdvb_unicable_is_en50607(name) ? &linuxdvb_en50607_class
