 ...
 * the lock check.
 */
static void linuxdvb_ddci_mtimer_arm_rel(mtimer_t* mti,
    mti_callback_t*                                callback,
    void*                                          opaque,
    int64_t                                        delta) {
  int locked;
...
static void linuxdvb_ddci_thread_init(linuxdvb_ddci_t* lddci,
...
static inline int linuxdvb_ddci_thread_running(
    linuxdvb_ddci_thread_t* ddci_thread) {
...
static int linuxdvb_ddci_thread_start(linuxdvb_ddci_thread_t* ddci_thread,
  ...
  if (!linuxdvb_ddci_thread_running(ddci_thread)) {
    ...
    tvh_thread_create(&ddci_thread->lddci_thread,
        NULL,
        thread_routine,
        arg,
        name);
...
static void linuxdvb_ddci_send_buffer_init(
...
static inline void linuxdvb_ddci_send_buffer_remove(
    linuxdvb_ddci_send_buffer_t* ddci_snd_buf,
  ...
  if (sp) {
    ...
    // memoryinfo_free(&mpegts_input_queue_memoryinfo, sizeof(mpegts_packet_t) +
    // mp->mp_len);
    ddci_snd_buf->lddci_send_buf_pkgCntR +=
        sp->lddci_send_pkt_len / LDDCI_TS_SIZE;
    TAILQ_REMOVE(&ddci_snd_buf->lddci_send_buf_queue, sp, lddci_send_pkt_link);
  }
}

static linuxdvb_ddci_send_packet_t* linuxdvb_ddci_send_buffer_get(
    linuxdvb_ddci_send_buffer_t* ddci_snd_buf,
    int64_t                      tmo) {
  linuxdvb_ddci_send_packet_t* sp;
...
static void linuxdvb_ddci_send_buffer_put(
    linuxdvb_ddci_send_buffer_t* ddci_snd_buf,
    const uint8_t*               tsb,
    int                          len) {
  tvh_mutex_lock(&ddci_snd_buf->lddci_send_buf_lock);

  if (ddci_snd_buf->lddci_send_buf_size <
      ddci_snd_buf->lddci_send_buf_size_max) {
    linuxdvb_ddci_send_packet_t* sp;
    ...
    // memoryinfo_alloc(&mpegts_input_queue_memoryinfo, sizeof(mpegts_packet_t)
    // + len2);
    TAILQ_INSERT_TAIL(&ddci_snd_buf->lddci_send_buf_queue,
        sp,
        lddci_send_pkt_link);
    ...
    if (tvhlog_limit(&ddci_snd_buf->lddci_send_buf_loglimit, 10))
      tvhwarn(LS_DDCI,
          "too much queued output data in send buffer, discarding new");
  }

  tvh_mutex_unlock(&ddci_snd_buf->lddci_send_buf_lock);
}

static void linuxdvb_ddci_send_buffer_clear(
    linuxdvb_ddci_send_buffer_t* ddci_snd_buf) {
  linuxdvb_ddci_send_packet_t* sp;
...
static void linuxdvb_ddci_send_buffer_statistic(
    linuxdvb_ddci_send_buffer_t* ddci_snd_buf,
    char*                        ci_id) {
...
static void linuxdvb_ddci_wr_thread_statistic(void* aux) {
  ...
  linuxdvb_ddci_send_buffer_statistic(&ddci_wr_thread->lddci_send_buffer,
      ci_id);
...
static void linuxdvb_ddci_wr_thread_statistic_clr(
    linuxdvb_ddci_wr_thread_t* ddci_wr_thread) {
...
static int linuxdvb_ddci_wr_thread_start(
    linuxdvb_ddci_wr_thread_t* ddci_wr_thread) {
  int e;

  // FIXME: Use a configuration parameter
  ddci_wr_thread->lddci_cfg_send_buffer_sz =
      LDDCI_SEND_BUF_NUM_DEF * LDDCI_TS_SIZE;
...
static inline void linuxdvb_ddci_wr_thread_stop(
    linuxdvb_ddci_wr_thread_t* ddci_wr_thread) {
...
static inline void linuxdvb_ddci_wr_thread_buffer_put(
    linuxdvb_ddci_wr_thread_t* ddci_wr_thread,
    const uint8_t*             tsb,
    int                        len) {
...
static void linuxdvb_ddci_rd_thread_statistic(void* aux) {
  ...
  if ((pkgCntS != ddci_rd_thread->lddci_recv_pkgCntSL)) {
    tvhtrace(LS_DDCI,
        "CAM %s got %d scrambled packets from CAM",
        ci_id,
        pkgCntS);
...
static void linuxdvb_ddci_rd_thread_statistic_clr(
    linuxdvb_ddci_rd_thread_t* ddci_rd_thread) {
...
static void* linuxdvb_ddci_read_thread(void* arg) {
  ...
  /* Allocate memory */
  sbuf_init_fixed(&sb,
      MINMAX(ddci_rd_thread->lddci_cfg_recv_buffer_sz,
          LDDCI_TS_SIZE * 100,
          LDDCI_TS_SIZE * 10000));
  ...
  while (tvheadend_is_running() && !ddci_thread->lddci_thread_stop) {
    ...
    if (sb.sb_ptr > 0) {
      ...
      if (skip) {
        tvhwarn(LS_DDCI,
            "CAM %s skipped %d bytes to sync on start of TS packet",
            ci_id,
            skip);
        sbuf_cut(&sb, skip);
        len = sb.sb_ptr;
...
static int linuxdvb_ddci_rd_thread_start(
    linuxdvb_ddci_rd_thread_t* ddci_rd_thread) {
  int e;

  // FIXME: Use a configuration parameter
  ddci_rd_thread->lddci_cfg_recv_buffer_sz =
      LDDCI_RECV_BUF_NUM_DEF * LDDCI_TS_SIZE;
...
static inline void linuxdvb_ddci_rd_thread_stop(
    linuxdvb_ddci_rd_thread_t* ddci_rd_thread) {
...
linuxdvb_ddci_t* linuxdvb_ddci_create(linuxdvb_transport_t* lcat,
    const char*                                             ci_path) {
  linuxdvb_ddci_t* lddci;
  ...
  snprintf(lddci->lddci_id,
      sizeof(lddci->lddci_id),
      "ci%u",
...
void linuxdvb_ddci_put(linuxdvb_ddci_t* lddci,
    service_t*                          t,
    const uint8_t*                      tsb,
    int                                 len) {
  /* ignore duplicates
...
int linuxdvb_ddci_do_not_assign(linuxdvb_ddci_t* lddci,
    service_t*                                   t,
    int                                          multi) {
  mpegts_service_t* s = (mpegts_service_t*)t;
