 ...
 * the lock check.
 */
static void linuxdvb_ddci_mtimer_disarm(mtimer_t* mti) {
  int locked;
 ...
 */
static void
linuxdvb_ddci_mtimer_arm_rel(mtimer_t* mti, mti_callback_t* callback, void* opaque, int64_t delta) {
  int locked;
...
static void linuxdvb_ddci_thread_init(linuxdvb_ddci_t* lddci, linuxdvb_ddci_thread_t* ddci_thread) {
...
static inline void linuxdvb_ddci_thread_signal(linuxdvb_ddci_thread_t* ddci_thread) {
  tvh_cond_signal(&ddci_thread->lddci_thread_cond, 0);
}

static int linuxdvb_ddci_thread_start(linuxdvb_ddci_thread_t* ddci_thread,
  ...
  if (!linuxdvb_ddci_thread_running(ddci_thread)) {
    ...
    do {
      e = tvh_cond_wait(&ddci_thread->lddci_thread_cond, &ddci_thread->lddci_thread_lock);
      if (e == ETIMEDOUT) {
...
static void linuxdvb_ddci_send_buffer_init(linuxdvb_ddci_send_buffer_t* ddci_snd_buf,
    uint64_t                                                            ddci_snd_buf_max) {
...
static inline void linuxdvb_ddci_send_buffer_remove(linuxdvb_ddci_send_buffer_t* ddci_snd_buf,
...
linuxdvb_ddci_send_buffer_get(linuxdvb_ddci_send_buffer_t* ddci_snd_buf, int64_t tmo) {
  linuxdvb_ddci_send_packet_t* sp;
  ...
  if (!sp) {
    ...
    do {
      ...
      r = tvh_cond_timedwait(&ddci_snd_buf->lddci_send_buf_cond,
          &ddci_snd_buf->lddci_send_buf_lock,
          mono);
      if (r == ETIMEDOUT) {
        break;
      }
...
static void linuxdvb_ddci_send_buffer_put(linuxdvb_ddci_send_buffer_t* ddci_snd_buf,
    const uint8_t*                                                     tsb,
    int                                                                len) {
  tvh_mutex_lock(&ddci_snd_buf->lddci_send_buf_lock);
...
static void linuxdvb_ddci_send_buffer_clear(linuxdvb_ddci_send_buffer_t* ddci_snd_buf) {
  linuxdvb_ddci_send_packet_t* sp;
...
static void linuxdvb_ddci_send_buffer_statistic(linuxdvb_ddci_send_buffer_t* ddci_snd_buf,
    char*                                                                    ci_id) {
  ...
  if ((pkgCntR != ddci_snd_buf->lddci_send_buf_pkgCntRL) ||
    ...
    tvhtrace(LS_DDCI, "CAM %s send buff rd(-> CAM):%d, wr:%d", ci_id, pkgCntR, pkgCntW);
 ...
 *
 *****************************************************************************/
static void linuxdvb_ddci_wr_thread_statistic(void* aux) {
...
static void linuxdvb_ddci_wr_thread_statistic_clr(linuxdvb_ddci_wr_thread_t* ddci_wr_thread) {
...
static void* linuxdvb_ddci_write_thread(void* arg) {
...
static inline void linuxdvb_ddci_wr_thread_init(linuxdvb_ddci_t* lddci) {
...
static inline void linuxdvb_ddci_wr_thread_buffer_put(linuxdvb_ddci_wr_thread_t* ddci_wr_thread,
    const uint8_t*                                                               tsb,
    int                                                                          len) {
 ...
 *
 *****************************************************************************/
static void linuxdvb_ddci_rd_thread_statistic(void* aux) {
  ...
  if ((pkgCntR != ddci_rd_thread->lddci_recv_pkgCntRL) ||
    ...
    tvhtrace(LS_DDCI, "CAM %s recv rd(CAM ->):%d, wr:%d", ci_id, pkgCntR, pkgCntW);
  ...
  if ((pkgCntS != ddci_rd_thread->lddci_recv_pkgCntSL)) {
    tvhtrace(LS_DDCI, "CAM %s got %d scrambled packets from CAM", ci_id, pkgCntS);
    ddci_rd_thread->lddci_recv_pkgCntSL = pkgCntS;
  }

  mtimer_arm_rel(&ddci_rd_thread->lddci_recv_stat_tmo,
      linuxdvb_ddci_rd_thread_statistic,
      ddci_rd_thread,
      sec2mono(LDDCI_RD_THREAD_STAT_TMO));
}

static void linuxdvb_ddci_rd_thread_statistic_clr(linuxdvb_ddci_rd_thread_t* ddci_rd_thread) {
...
static inline int ddci_ts_sync_count(const uint8_t* tsb, int len) {
  const uint8_t* start = tsb;

#define LDDCI_TS_SIZE_10 (LDDCI_TS_SIZE * 10)

  while (len >= LDDCI_TS_SIZE) {
    if (len >= LDDCI_TS_SIZE_10 && tsb[0 * LDDCI_TS_SIZE] == LDDCI_TS_SYNC_BYTE &&
...
static int ddci_ts_sync_search(const uint8_t* tsb, int len) {
  int skipped = 0;

  while ((len > LDDCI_MIN_TS_SYN) && (ddci_ts_sync_count(tsb, len) < LDDCI_MIN_TS_SYN)) {
    tsb++;
    len--;
    skipped++;
  }
...
static inline int ddci_ts_sync(const uint8_t* tsb, int len) {
...
static inline int ddci_is_scrambled(const uint8_t* tsb) {
  return tsb[3] & LDDCI_TS_SCRAMBLING_CONTROL;
}

static void* linuxdvb_ddci_read_thread(void* arg) {
  ...
  /* Allocate memory */
  sbuf_init_fixed(&sb,
      MINMAX(ddci_rd_thread->lddci_cfg_recv_buffer_sz, LDDCI_TS_SIZE * 100, LDDCI_TS_SIZE * 10000));
  ...
  while (tvheadend_is_running() && !ddci_thread->lddci_thread_stop) {
    ...
    if (sb.sb_ptr > 0) {
      ...
      mpegts_input_postdemux(ddci_thread->lddci->lddci_mi, ddci_thread->lddci->lddci_mm, tsb, len);
...
static inline void linuxdvb_ddci_rd_thread_init(linuxdvb_ddci_t* lddci) {
  linuxdvb_ddci_thread_init(lddci, LDDCI_TO_THREAD(&lddci->lddci_rd_thread));
}

static int linuxdvb_ddci_rd_thread_start(linuxdvb_ddci_rd_thread_t* ddci_rd_thread) {
  int e;
...
static inline void linuxdvb_ddci_rd_thread_stop(linuxdvb_ddci_rd_thread_t* ddci_rd_thread) {
...
linuxdvb_ddci_t* linuxdvb_ddci_create(linuxdvb_transport_t* lcat, const char* ci_path) {
  linuxdvb_ddci_t* lddci;

  lddci             = calloc(1, sizeof(*lddci));
  lddci->lcat       = lcat;
  lddci->lddci_path = strdup(ci_path);
  snprintf(lddci->lddci_id, sizeof(lddci->lddci_id), "ci%u", lcat->lcat_adapter->la_dvb_number);
  lddci->lddci_fdW = -1;
  lddci->lddci_fdR = -1;
...
void linuxdvb_ddci_destroy(linuxdvb_ddci_t* lddci) {
  if (!lddci)
    return;
...
void linuxdvb_ddci_close(linuxdvb_ddci_t* lddci) {
  int closed = 0;

  if (lddci->lddci_fdW >= 0) {
    tvhtrace(LS_DDCI,
        "closing write %s %s (fd %d)",
        lddci->lddci_id,
        lddci->lddci_path,
        lddci->lddci_fdW);
...
int linuxdvb_ddci_open(linuxdvb_ddci_t* lddci) {
  ...
  if (lddci->lddci_fdW < 0) {
    ...
    if (lddci->lddci_fdW >= 0 && lddci->lddci_fdR >= 0) {
      ...
      tvhtrace(LS_DDCI,
          "open write/read failed %s %s (fd-W %d, fd-R %d)",
          lddci->lddci_id,
          lddci->lddci_path,
          lddci->lddci_fdW,
          lddci->lddci_fdR);
...
void linuxdvb_ddci_put(linuxdvb_ddci_t* lddci, service_t* t, const uint8_t* tsb, int len) {
  /* ignore duplicates
...
void linuxdvb_ddci_assign(linuxdvb_ddci_t* lddci, service_t* t) {
...
void linuxdvb_ddci_unassign(linuxdvb_ddci_t* lddci, service_t* t) {
  if (idnode_set_exists(lddci->lddci_services, &t->s_id)) {
...
int linuxdvb_ddci_do_not_assign(linuxdvb_ddci_t* lddci, service_t* t, int multi) {
  mpegts_service_t* s = (mpegts_service_t*)t;
  ...
  if (multi && (lddci->lddci_mm == s->s_dvb_mux))
    return 0;
