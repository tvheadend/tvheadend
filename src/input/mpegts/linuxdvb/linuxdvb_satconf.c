...
static const void* linuxdvb_satconf_class_network_get(linuxdvb_satconf_t* ls,
    int                                                                   idx) {
...
static int linuxdvb_satconf_class_network_set(linuxdvb_satconf_t* ls,
    int                                                           idx,
    const void*                                                   networks) {
  linuxdvb_satconf_ele_t* lse = linuxdvb_satconf_class_find_ele(ls, idx);
  if (lse)
...
static htsmsg_t* linuxdvb_satconf_class_network_enum(void* o,
    const char*                                            lang) {
...
static char* linuxdvb_satconf_class_network_rend(linuxdvb_satconf_t* ls,
    int                                                              idx,
    const char*                                                      lang) {
  linuxdvb_satconf_ele_t* lse = linuxdvb_satconf_class_find_ele(ls, idx);
  if (lse)
...
#define linuxdvb_satconf_class_network_getset(x)                             \
  ...
  static char* linuxdvb_satconf_class_network_rend##x(void* o,               \
      const char*                                           lang) {                                                    \
...
static void linuxdvb_satconf_class_get_title(idnode_t* p,
    const char*                                        lang,
    char*                                              dst,
    size_t                                             dstsize) {
...
static htsmsg_t* linuxdvb_satconf_class_highvol_list(void* o,
    const char*                                            lang) {
  static const struct strtab tab[] = {{N_("Do not set"), 0},
      {N_("Normal"), 1},
      {N_("Higher"), 2}};
...
static int linuxdvb_satconf_ele_tune(linuxdvb_satconf_ele_t* lse) {
  ...
  linuxdvb_diseqc_t*   lds[] = {ls->ls_switch_rotor
          ? (linuxdvb_diseqc_t*)lse->lse_switch
          : (linuxdvb_diseqc_t*)lse->lse_rotor,
  ...
  for (i = ls->ls_diseqc_idx; i < ARRAY_SIZE(lds); i++) {
    ...
    if (r != 0) {
      ...
      tvhtrace(LS_DISEQC,
          "waiting %d seconds to finish setup for %s",
          r,
          lds[i]->ld_type);
      mtimer_arm_rel(&ls->ls_diseqc_timer,
          linuxdvb_satconf_ele_tune_cb,
          lse,
          sec2mono(r));
  ...
  if (!lse->lse_en50494) {
    if (ls->ls_last_tone_off != band + 1) {
      ...
      if (ioctl(lfe->lfe_fe_fd,
              FE_SET_TONE,
              band ? SEC_TONE_ON : SEC_TONE_OFF)) {
        tvherror(LS_DISEQC,
            "failed to set diseqc tone (e=%s)",
            strerror(errno));
        return -1;
      }
  ...
  f = lse->lse_en50494 ? ((linuxdvb_en50494_t*)lse->lse_en50494)->le_tune_freq
                       : freq;
...
int linuxdvb_satconf_lnb_freq(linuxdvb_satconf_t* ls,
...
int linuxdvb_satconf_start_mux(linuxdvb_satconf_t* ls,
  ...
  ls->ls_diseqc_idx = 0;
  if (lse->lse_rotor)
    if (lse->lse_rotor->ld_start(lse->lse_rotor,
            (dvb_mux_t*)mmi->mmi_mux,
            ls,
            lse))
      return 0;
...
int linuxdvb_satconf_match_mux(linuxdvb_satconf_t* ls, mpegts_mux_t* mm) {
  ...
  if (lse1->lse_en50494 &&
      !lse1->lse_en50494->ld_match(lse1->lse_en50494, lm1, lm2)) {
...
linuxdvb_satconf_t* linuxdvb_satconf_create(linuxdvb_frontend_t* lfe,
    htsmsg_t*                                                    conf) {
  int                           i;
  htsmsg_t *                    l, *e;
  htsmsg_field_t*               f;
  ...
  for (i = 0; i < lst->ports; i++) {
    ...
    if (lst->ports > 1) {
      if (!strcmp(lst->type, "en50494")) {
        if (!lse->lse_en50494)
          lse->lse_en50494 =
              linuxdvb_en50494_create0(UNICABLE_I_NAME, NULL, lse, i);
      } else if (!strcmp(lst->type, "en50607")) {
        if (!lse->lse_en50494)
          lse->lse_en50494 =
              linuxdvb_en50494_create0(UNICABLE_II_NAME, NULL, lse, i);
      } else {
        if (!lse->lse_switch)
          lse->lse_switch =
              linuxdvb_switch_create0("Generic", NULL, lse, i, -1);
      }
    }
...
static htsmsg_t* linuxdvb_satconf_ele_class_network_enum(void* o,
    const char*                                                lang) {
  linuxdvb_satconf_ele_t* ls = o;
  if (ls == NULL)
    return NULL;
...
static char* linuxdvb_satconf_ele_class_network_rend(void* o,
    const char*                                            lang) {
  linuxdvb_satconf_ele_t* ls  = o;
...
static void linuxdvb_satconf_ele_class_get_title(idnode_t* o,
    const char*                                            lang,
    char*                                                  dst,
    size_t                                                 dstsize) {
...
linuxdvb_satconf_ele_t* linuxdvb_satconf_ele_create0(const char* uuid,
    htsmsg_t*                                                    conf,
...
void linuxdvb_satconf_destroy(linuxdvb_satconf_t* ls, int delconf) {
  ...
  char                    ubuf[UUID_HEX_SIZE];
  if (delconf)
    hts_settings_remove("input/linuxdvb/satconfs/%s",
        idnode_uuid_as_str(&ls->ls_id, ubuf));
  mtimer_disarm(&ls->ls_diseqc_timer);
...
static void linuxdvb_diseqc_class_get_title(idnode_t* o,
    const char*                                       lang,
    char*                                             dst,
    size_t                                            dstsize) {
...
int linuxdvb_diseqc_send(int fd,
    uint8_t                  framing,
    uint8_t                  addr,
    uint8_t                  cmd,
    int                      len,
...
int linuxdvb_diseqc_set_volt(linuxdvb_satconf_t* ls, int vol) {
  ...
  if (ls->ls_lnb_highvol > 0) {
    ...
    if (ioctl(linuxdvb_satconf_fe_fd(ls), FE_ENABLE_HIGH_LNB_VOLTAGE, v))
      tvherror(LS_DISEQC,
          "failed to set high voltage %d (e=%s)",
          v,
          strerror(errno));
  }
  /* Set voltage */
  ...
  if (ioctl(linuxdvb_satconf_fe_fd(ls),
          FE_SET_VOLTAGE,
          vol ? (vol < 0 ? SEC_VOLTAGE_OFF : SEC_VOLTAGE_18)
              : SEC_VOLTAGE_13)) {
