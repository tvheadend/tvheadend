...
static struct linuxdvb_satconf_type* linuxdvb_satconf_type_find(const char* type);
...
static const void* linuxdvb_satconf_class_active_get(void* obj) {
  static int              active;
...
static linuxdvb_satconf_ele_t* linuxdvb_satconf_class_find_ele(linuxdvb_satconf_t* ls, int idx) {
  int                     i = 0;
  linuxdvb_satconf_ele_t* lse;
  TAILQ_FOREACH (lse, &ls->ls_elements, lse_link) {
    if (i == idx)
...
static const void* linuxdvb_satconf_class_network_get(linuxdvb_satconf_t* ls, int idx) {
...
static int linuxdvb_satconf_ele_class_network_set(void* o, const void* p);

static int
linuxdvb_satconf_class_network_set(linuxdvb_satconf_t* ls, int idx, const void* networks) {
  linuxdvb_satconf_ele_t* lse = linuxdvb_satconf_class_find_ele(ls, idx);
  if (lse)
...
static htsmsg_t* linuxdvb_satconf_class_network_enum(void* o, const char* lang) {
...
static char* linuxdvb_satconf_ele_class_network_rend(void* o, const char* lang);
...
#define linuxdvb_satconf_class_network_getset(x)                                   \
  static int linuxdvb_satconf_class_network_set##x(void* o, const void* v) {       \
    return linuxdvb_satconf_class_network_set(o, x, (void*)v);                     \
  }                                                                                \
  static const void* linuxdvb_satconf_class_network_get##x(void* o) {              \
    return linuxdvb_satconf_class_network_get(o, x);                               \
  }                                                                                \
  static char* linuxdvb_satconf_class_network_rend##x(void* o, const char* lang) { \
...
static void
linuxdvb_satconf_class_get_title(idnode_t* p, const char* lang, char* dst, size_t dstsize) {
  linuxdvb_satconf_t*           ls  = (linuxdvb_satconf_t*)p;
  struct linuxdvb_satconf_type* lst = linuxdvb_satconf_type_find(ls->ls_type);
  snprintf(dst, dstsize, "%s", lst ? lst->name : ls->ls_type);
}

static void linuxdvb_satconf_class_changed(idnode_t* s) {
...
static const void* linuxdvb_satconf_class_orbitalpos_get(void* p) {
  static int              n;
...
static int linuxdvb_satconf_class_orbitalpos_set(void* p, const void* v) {
  linuxdvb_satconf_ele_t* lse;
  linuxdvb_satconf_t*     ls = p;
...
static idnode_set_t* linuxdvb_satconf_class_get_childs(idnode_t* o) {
...
static htsmsg_t* linuxdvb_satconf_class_highvol_list(void* o, const char* lang) {
  static const struct strtab tab[] = {{N_("Do not set"), 0}, {N_("Normal"), 1}, {N_("Higher"), 2}};
...
const idclass_t linuxdvb_satconf_class = {.ic_class = "linuxdvb_satconf",
 ...
 * Simple LNB only
 */
const idclass_t linuxdvb_satconf_lnbonly_class = {.ic_super = &linuxdvb_satconf_class,
 ...
 * 2 port switch
 */
const idclass_t linuxdvb_satconf_2port_class = {.ic_super = &linuxdvb_satconf_class,
 ...
 * 4 port switch
 */
const idclass_t linuxdvb_satconf_4port_class = {.ic_super = &linuxdvb_satconf_class,
 ...
 * Unicable (EN50494)
 */
static const void* linuxdvb_satconf_class_en50494_id_get(void* p) {
  linuxdvb_satconf_t*     ls  = p;
  linuxdvb_satconf_ele_t* lse = TAILQ_FIRST(&ls->ls_elements);
  return &(((linuxdvb_en50494_t*)lse->lse_en50494)->le_id);
}

static int linuxdvb_satconf_class_en50494_id_set(void* p, const void* v) {
  linuxdvb_satconf_t*     ls = p;
  linuxdvb_satconf_ele_t* lse;
  TAILQ_FOREACH (lse, &ls->ls_elements, lse_link)
...
static const void* linuxdvb_satconf_class_en50494_pin_get(void* p) {
  linuxdvb_satconf_t*     ls  = p;
  linuxdvb_satconf_ele_t* lse = TAILQ_FIRST(&ls->ls_elements);
  return &(((linuxdvb_en50494_t*)lse->lse_en50494)->le_pin);
}

static int linuxdvb_satconf_class_en50494_pin_set(void* p, const void* v) {
  linuxdvb_satconf_t*     ls = p;
  linuxdvb_satconf_ele_t* lse;
  TAILQ_FOREACH (lse, &ls->ls_elements, lse_link)
...
static const void* linuxdvb_satconf_class_en50494_freq_get(void* p) {
  linuxdvb_satconf_t*     ls  = p;
  linuxdvb_satconf_ele_t* lse = TAILQ_FIRST(&ls->ls_elements);
  return &(((linuxdvb_en50494_t*)lse->lse_en50494)->le_frequency);
}

static int linuxdvb_satconf_class_en50494_freq_set(void* p, const void* v) {
  linuxdvb_satconf_t*     ls = p;
  linuxdvb_satconf_ele_t* lse;
  TAILQ_FOREACH (lse, &ls->ls_elements, lse_link)
...
const idclass_t linuxdvb_satconf_en50494_class = {.ic_super = &linuxdvb_satconf_class,
...
const idclass_t linuxdvb_satconf_en50607_class = {.ic_super = &linuxdvb_satconf_class,
 ...
 * Advanced
 */
const idclass_t linuxdvb_satconf_advanced_class = {.ic_super = &linuxdvb_satconf_class,
...
/* List of types */
htsmsg_t* linuxdvb_satconf_type_list(void* p, const char* lang) {
  int       i;
...
static linuxdvb_satconf_ele_t* linuxdvb_satconf_find_ele(linuxdvb_satconf_t* ls,
    mpegts_mux_t*                                                            mux) {
  linuxdvb_satconf_ele_t* lse;
  TAILQ_FOREACH (lse, &ls->ls_elements, lse_link) {
...
int linuxdvb_satconf_get_priority(linuxdvb_satconf_t* ls, mpegts_mux_t* mm) {
...
int linuxdvb_satconf_get_grace(linuxdvb_satconf_t* ls, mpegts_mux_t* mm) {
  linuxdvb_satconf_ele_t* lse = linuxdvb_satconf_find_ele(ls, mm);
  if (lse == NULL)
    return 0;

  int                i, r = 10;
  linuxdvb_diseqc_t* lds[] = {(linuxdvb_diseqc_t*)lse->lse_en50494,
...
static int linuxdvb_satconf_ele_tune(linuxdvb_satconf_ele_t* lse) {
  ...
  linuxdvb_diseqc_t*   lds[] = {ls->ls_switch_rotor ? (linuxdvb_diseqc_t*)lse->lse_switch
                                                    : (linuxdvb_diseqc_t*)lse->lse_rotor,
  ...
  f = lse->lse_en50494 ? ((linuxdvb_en50494_t*)lse->lse_en50494)->le_tune_freq : freq;
...
int linuxdvb_satconf_lnb_freq(linuxdvb_satconf_t* ls, mpegts_mux_instance_t* mmi) {
  int                     f;
...
int linuxdvb_satconf_start_mux(linuxdvb_satconf_t* ls,
 ...
 * diseqc config
 */
int linuxdvb_satconf_match_mux(linuxdvb_satconf_t* ls, mpegts_mux_t* mm) {
  mpegts_mux_instance_t* mmi = ls->ls_mmi;
...
linuxdvb_satconf_t* linuxdvb_satconf_create(linuxdvb_frontend_t* lfe, htsmsg_t* conf) {
  int                           i;
  htsmsg_t *                    l, *e;
  htsmsg_field_t*               f;
  linuxdvb_satconf_ele_t*       lse;
  ...
  if (conf) {
    ...
    if ((l = htsmsg_get_list(conf, "elements"))) {
      HTSMSG_FOREACH(f, l) {
        ...
        /* Fix config */
        if ((str = htsmsg_get_str(e, "network")) && !htsmsg_get_list(e, "networks")) {
...
static const void* linuxdvb_satconf_ele_class_network_get(void* o) {
  ...
  if (save) {
    ...
    TAILQ_FOREACH (lse, &sc->ls_elements, lse_link) {
      for (i = 0; i < lse->lse_networks->is_count; i++) {
        if (!lse->lse_enabled)
          continue;
        htsmsg_add_str(l, NULL, idnode_uuid_as_str(lse->lse_networks->is_array[i], ubuf));
      }
    }
...
static htsmsg_t* linuxdvb_satconf_ele_class_network_enum(void* o, const char* lang) {
  linuxdvb_satconf_ele_t* ls = o;
  if (ls == NULL)
    return NULL;
...
static int linuxdvb_satconf_ele_class_lnbtype_set(void* o, const void* p) {
  linuxdvb_satconf_ele_t* ls  = o;
  const char*             str = p;
  if (ls->lse_lnb && !strcmp(str ?: "", ls->lse_lnb->ld_type))
    return 0;
  if (ls->lse_lnb)
    linuxdvb_lnb_destroy(ls->lse_lnb);
  ls->lse_lnb = linuxdvb_lnb_create0(str, NULL, ls);
  return 1;
}

static const void* linuxdvb_satconf_ele_class_lnbtype_get(void* o) {
  linuxdvb_satconf_ele_t* ls = o;
  prop_ptr                   = ls->lse_lnb ? ls->lse_lnb->ld_type : NULL;
  return &prop_ptr;
}

static int linuxdvb_satconf_ele_class_en50494type_set(void* o, const void* p) {
  linuxdvb_satconf_ele_t* ls  = o;
  const char*             str = p;
...
static const void* linuxdvb_satconf_ele_class_en50494type_get(void* o) {
  linuxdvb_satconf_ele_t* ls = o;
  prop_ptr                   = ls->lse_en50494 ? ls->lse_en50494->ld_type : NULL;
  return &prop_ptr;
}

static int linuxdvb_satconf_ele_class_switchtype_set(void* o, const void* p) {
  linuxdvb_satconf_ele_t* ls  = o;
  const char*             str = p;
  if (ls->lse_switch && !strcmp(str ?: "", ls->lse_switch->ld_type))
    return 0;
  if (ls->lse_switch)
    linuxdvb_switch_destroy(ls->lse_switch);
  ls->lse_switch = linuxdvb_switch_create0(str, NULL, ls, -1, -1);
  return 1;
}

static const void* linuxdvb_satconf_ele_class_switchtype_get(void* o) {
  linuxdvb_satconf_ele_t* ls = o;
  prop_ptr                   = ls->lse_switch ? ls->lse_switch->ld_type : NULL;
  return &prop_ptr;
}

static int linuxdvb_satconf_ele_class_rotortype_set(void* o, const void* p) {
  linuxdvb_satconf_ele_t* ls  = o;
  const char*             str = p;
  if (ls->lse_rotor && !strcmp(str ?: "", ls->lse_rotor->ld_type))
    return 0;
  if (ls->lse_rotor)
    linuxdvb_rotor_destroy(ls->lse_rotor);
  ls->lse_rotor = linuxdvb_rotor_create0(str, NULL, ls);
  return 1;
}

static const void* linuxdvb_satconf_ele_class_rotortype_get(void* o) {
  linuxdvb_satconf_ele_t* ls = o;
...
static void
linuxdvb_satconf_ele_class_get_title(idnode_t* o, const char* lang, char* dst, size_t dstsize) {
  snprintf(dst, dstsize, "%s", ((linuxdvb_satconf_ele_t*)o)->lse_name);
}

static idnode_set_t* linuxdvb_satconf_ele_class_get_childs(idnode_t* o) {
...
static void linuxdvb_satconf_ele_class_changed(idnode_t* in) {
  linuxdvb_satconf_ele_t* lse = (linuxdvb_satconf_ele_t*)in;
  linuxdvb_satconf_class_changed(&lse->lse_parent->ls_id);
}

static const void* linuxdvb_satconf_ele_class_active_get(void* obj) {
  static int              active;
...
const idclass_t linuxdvb_satconf_ele_class = {.ic_class = "linuxdvb_satconf_ele",
...
linuxdvb_satconf_ele_create0(const char* uuid, htsmsg_t* conf, linuxdvb_satconf_t* ls) {
  htsmsg_t*               e;
...
static void
linuxdvb_diseqc_class_get_title(idnode_t* o, const char* lang, char* dst, size_t dstsize) {
  linuxdvb_diseqc_t* ld = (linuxdvb_diseqc_t*)o;
  snprintf(dst, dstsize, "%s", ld->ld_type);
}

static void linuxdvb_diseqc_class_changed(idnode_t* o) {
  linuxdvb_diseqc_t* ld = (linuxdvb_diseqc_t*)o;
  if (ld->ld_satconf)
    linuxdvb_satconf_ele_class_changed(&ld->ld_satconf->lse_id);
}

static const void* linuxdvb_diseqc_class_active_get(void* obj) {
  static int         active;
...
const idclass_t linuxdvb_diseqc_class = {.ic_class = "linuxdvb_diseqc",
...
linuxdvb_diseqc_t* linuxdvb_diseqc_create0(linuxdvb_diseqc_t* ld,
    const char*                                               uuid,
    const idclass_t*                                          idc,
    htsmsg_t*                                                 conf,
    const char*                                               type,
    linuxdvb_satconf_ele_t*                                   parent) {
  /* Insert */
...
int linuxdvb_diseqc_send(int fd, uint8_t framing, uint8_t addr, uint8_t cmd, int len, ...) {
  int                          i;
  va_list                      ap;
  struct dvb_diseqc_master_cmd message;
  char                         buf[256];
  ...
  if (tvhtrace_enabled())
    tvhtrace(LS_DISEQC,
        "sending diseqc (len %d) %02X %02X %02X %s",
        len + 3,
        framing,
        addr,
        cmd,
...
int linuxdvb_diseqc_set_volt(linuxdvb_satconf_t* ls, int vol) {
  vol = vol < 0 ? -1 : !!(vol > 0);
  /* Already set ? */
  ...
  if (ioctl(linuxdvb_satconf_fe_fd(ls),
          FE_SET_VOLTAGE,
