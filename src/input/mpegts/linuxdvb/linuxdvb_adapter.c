...
static void linuxdvb_adapter_class_get_title(idnode_t* in,
    const char*                                        lang,
    char*                                              dst,
    size_t                                             dstsize) {
  linuxdvb_adapter_t* la = (linuxdvb_adapter_t*)in;
...
static linuxdvb_adapter_t* linuxdvb_adapter_create(const char* uuid,
  ...
  if (!tvh_hardware_create0((tvh_hardware_t*)la,
          &linuxdvb_adapter_class,
          uuid,
          conf)) {
...
static void linuxdvb_get_systems(int fd, struct dtv_property* _cmd) {
  ...
  if (!r && cmd.u.buffer.len) {
    struct dtv_property fecmd[2] = {
        {.cmd = DTV_DELIVERY_SYSTEM, .u.data = cmd.u.buffer.data[0]},
        {.cmd = DTV_TUNE}};
    cmdseq.props = fecmd;
    cmdseq.num   = 2;
  ...
  for (i = 0; i < 32; i++) {
    ...
    if (type == DVB_TYPE_NONE) {
      tvherror(LS_LINUXDVB,
          "unable to determine FE type %s - %i",
          fe_path,
          dfi.type);
      continue;
    }
    ...
    for (j = fenum = 0; j < cmd.u.buffer.len; j++) {
      ...
      /* Create */
      linuxdvb_frontend_create(feconf,
          la,
          i,
          fe_path,
          dmx_path,
...
static void linuxdvb_adapter_del(const char* path) {
  ...
  if (sscanf(path, "/dev/dvb/adapter%d", &a) == 1) {
    ...
    for (lcat = LIST_FIRST(&la->la_ca_transports); lcat != NULL;
         lcat = lcat_next) {
...
static fsmonitor_t devdvbmon = {.fsm_create = devdvb_create,
...
static fsmonitor_t devmon = {.fsm_create = dev_create,
