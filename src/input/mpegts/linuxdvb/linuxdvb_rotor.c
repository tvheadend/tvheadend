...
static void linuxdvb_rotor_class_get_title(idnode_t* o,
    const char*                                      lang,
    char*                                            dst,
    size_t                                           dstsize) {
    ...
    .ic_properties =
        (const property_t[]){{
                             ...
                             },
            {
                ...
                .desc = N_(
                    "Satellite longitude (like 9.0 (east) or -33.0 (west))."),
...
const idclass_t linuxdvb_rotor_external_class = {
    ...
    .ic_properties = (const property_t[]){
        ...
        },
        {
            ...
            .name = N_("Satellite longitude"),
            .desc =
                N_("Satellite longitude (like 9.0 (east) or -33.0 (west))."),
            .off = offsetof(linuxdvb_rotor_t, lr_sat_lon),
        },
        {}}};
...
static void sat_azimuth_and_elevation(double site_lat,
  ...
  double Rstation =
      r_eq / sqrt(1.00 - f * (2.00 - f) * sin_site_lat * sin_site_lat);
  ...
  double El_geometric  = to_degrees(atan(
      alfa_r_zenith / sqrt(alfa_r_north * alfa_r_north + alfa_ry * alfa_ry)));
  double x             = fabs(El_geometric + 0.589);
  double refraction =
      fabs(a0 + a1 * x + a2 * x * x + a3 * x * x * x + a4 * x * x * x * x);
  ...
  *elevation = 0.00;
  if (El_geometric > 10.2)
    *elevation = El_geometric +
        0.01617 *
            (cos(to_radians(fabs(El_geometric))) /
...
static double sat_angle(linuxdvb_rotor_t* lr, linuxdvb_satconf_ele_t* ls) {
  ...
  sat_azimuth_and_elevation(site_lat,
      site_lon,
      site_alt,
      sat_lon,
      &azimuth,
...
#ifndef ROTOR_TEST
  tvhtrace(LS_DISEQC,
      "rotor angle azimuth %.4f elevation %.4f",
      azimuth,
      elevation);
#else
  gazimuth   = azimuth;
...
static int linuxdvb_rotor_extcmd(linuxdvb_rotor_t* lr,
  ...
  if (spawn_and_give_stdout(lse->lse_parent->ls_rotor_extcmd,
          argv,
          NULL,
          &rd,
          NULL,
  ...
  if (rd >= 0)
    close(rd);
  tvhinfo(LS_DISEQC,
      "linuxdvb_rotor_extcmd moving to %d returned %d",
      lr->lr_position,
      ret);
  return ret;
}

static int linuxdvb_external_grace(linuxdvb_rotor_t* lr,
    linuxdvb_satconf_ele_t*                          lse) {
...
static int linuxdvb_rotor_check_orbital_pos(linuxdvb_rotor_t* lr,
    dvb_mux_t*                                                lm,
  ...
  tvhdebug(LS_DISEQC,
      "rotor already positioned to %i.%i%c",
      pos / 10,
      pos % 10,
      dir);
  return 1;
...
static int linuxdvb_rotor_post(linuxdvb_diseqc_t* ld,
    linuxdvb_satconf_t*                           lsp,
    linuxdvb_satconf_ele_t*                       ls) {
  linuxdvb_rotor_t* lr = (linuxdvb_rotor_t*)ld;
...
struct {
  ...
  const idclass_t* idc;
    ...
    {.name = N_("GOTOX"), .idc = &linuxdvb_rotor_gotox_class},
...
htsmsg_t* linuxdvb_rotor_list(void* o, const char* lang) {
  ...
  htsmsg_add_msg(m,
      NULL,
      htsmsg_create_key_val("", tvh_gettext_lang(lang, N_("None"))));
...
linuxdvb_diseqc_t* linuxdvb_rotor_create0(const char* name,
    htsmsg_t*                                         conf,
    linuxdvb_satconf_ele_t*                           ls) {
  int                i;
...
int main(int argc, char* argv[]) {
  if (argc < 5) {
    fprintf(stderr,
        "Usage: <site_latitude> <site_lontitude> <site_altitude> "
        "<sat_longtitude>\n");
    return 1;
  }
  linuxdvb_rotor_t       lr;
