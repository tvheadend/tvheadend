...
static satip_satconf_t* satip_satconf_find_ele(satip_frontend_t* lfe,
    mpegts_mux_t*                                                mux) {
  satip_satconf_t*  sfc;
  ...
  if (lfe->sf_master) {
    TAILQ_FOREACH (lfe2, &lfe->sf_device->sd_frontends, sf_link)
      if (lfe2->sf_number != lfe->sf_number &&
          lfe2->sf_number == lfe->sf_master && lfe2->sf_master == 0) {
        lfe = lfe2;
        goto found;
      }
    return 0;
...
static int satip_satconf_master_or_slave(satip_frontend_t* lfe1,
    satip_frontend_t*                                      lfe2) {
  return lfe1->sf_number == lfe2->sf_master ||
      lfe2->sf_number == lfe1->sf_master;
}

static int satip_satconf_in_network_group(satip_frontend_t* lfe,
    int                                                     network_group,
    idnode_t*                                               mn) {
  satip_satconf_t* sfc;
  ...
  TAILQ_FOREACH (sfc, &lfe->sf_satconf, sfc_link) {
    if (sfc->sfc_position != lfe->sf_position)
      continue;
    if (network_group > 0 && sfc->sfc_network_group > 0 &&
...
static int satip_satconf_hash(mpegts_mux_t* mm, int position) {
  ...
  return 1 | (mc->dmc_fe_freq > 11700000 ? 2 : 0) |
      ((int)mc->u.dmc_fe_qpsk.polarisation << 8) | ((position & 0xffff) << 16);
}
...
satip_satconf_t* satip_satconf_get_position(satip_frontend_t* lfe,
  ...
  if (sfc && sfc->sfc_enabled) {
    if (hash)
      *hash = sfc->sfc_network_group > 0
          ? satip_satconf_hash(mm, sfc->sfc_position)
          : 0;
    if (!check)
      return sfc;
    if (check > 1) {
...
static void satip_satconf_class_get_title(idnode_t* o,
    const char*                                     lang,
    char*                                           dst,
    size_t                                          size) {
...
const idclass_t satip_satconf_class = {.ic_class = "satip_satconf",
    ...
    .ic_properties = (const property_t[]){
        ...
        },
        {
            ...
            .desc = N_(
                "Define network group to limit network usage (value 1-1000). "
...
satip_satconf_create0(satip_frontend_t* lfe, htsmsg_t* conf, int position) {
...
void satip_satconf_create(satip_frontend_t* lfe,
    htsmsg_t*                               conf,
    int                                     def_positions) {
  htsmsg_t *      l, *e;
  htsmsg_field_t* f;
