...
static void satip_device_dbus_notify(satip_device_t* sd, const char* sig_name) {
  ...
  snprintf(buf,
      sizeof(buf),
      "/input/mpegts/satip/%s",
      idnode_uuid_as_str(&sd->th_id, ubuf));
  dbus_emit_signal(buf, sig_name, msg);
#endif
}

static void satip_device_block(const char* addr, int block) {
...
static char* satip_device_addr(void* aux, const char* path, char* value) {
...
static const void* satip_device_class_active_get(void* obj) {
  static int        active;
...
static idnode_set_t* satip_device_class_get_childs(idnode_t* in) {
  satip_device_t*   sd = (satip_device_t*)in;
...
static void satip_device_class_get_title(idnode_t* in,
    const char*                                    lang,
    char*                                          dst,
    size_t                                         dstsize) {
  satip_device_t* sd = (satip_device_t*)in;
...
static const char* satip_tunercfg_tab[] = {"DVBS2-1",
    "DVBS2-2",
    "DVBS2-4",
    "DVBS2-8",
    "DVBC-1",
...
static htsmsg_t* satip_device_class_tunercfg_list(void* o, const char* lang) {
...
static void satip_device_class_tunercfg_notify(void* o, const char* lang) {
  satip_device_t* sd = (satip_device_t*)o;
  if (!sd->sd_inload)
    satip_device_destroy_later(sd, 100);
}

static htsmsg_t* satip_device_class_default_rolloff_list(void* o,
    const char*                                                lang) {
  static const struct strtab tab[] = {{N_("Auto"), SATIP_DEFAULT_ROLLOFF_AUTO},
...
const idclass_t satip_device_class = {.ic_class = "satip_client",
    ...
    .ic_properties                              = (const property_t[]){
        ...
        },
        {
            ...
            .desc =
                N_("This is a workaround for some tuners that mess up "
        ...
        },
        {
            ...
            .desc =
                N_("Manufacturer's product information page for the device."),
            .opts = PO_RDONLY | PO_NOSAVE,
...
static void satip_device_hack(satip_device_t* sd) {
...
static satip_device_t* satip_device_create(satip_device_info_t* info) {
  ...
  if (!tvh_hardware_create0((tvh_hardware_t*)sd,
          &satip_device_class,
          uhex,
          conf)) {
...
#undef ASSIGN
  ...
  for (i = m = 0, fenum = 1; i < n; i++) {
    ...
    if (type == DVB_TYPE_NONE) {
      tvherror(LS_SATIP,
          "%s: bad tuner type [%s]",
          satip_device_nicename(sd, buf2, sizeof(buf2)),
          argv[i]);
    } else if (m < 0 || m > 32) {
      tvherror(LS_SATIP,
          "%s: bad tuner count [%s]",
...
static satip_device_t* satip_device_find(const char* satip_uuid) {
  tvh_hardware_t* th;
  uint8_t         binuuid[20];
...
static satip_device_t* satip_device_find_by_descurl(const char* descurl) {
  tvh_hardware_t* th;
...
static void satip_device_destroy_cb(void* aux) {
  satip_device_destroy((satip_device_t*)aux);
  satip_device_discovery_start();
}

void satip_device_destroy_later(satip_device_t* sd, int after) {
  mtimer_arm_rel(&sd->sd_destroy_timer,
      satip_device_destroy_cb,
      sd,
      ms2mono(after));
}
...
static void satip_discovery_destroy(satip_discovery_t* d, int unlink) {
  if (d == NULL)
    return;
  if (unlink) {
...
static satip_discovery_t* satip_discovery_find(satip_discovery_t* d) {
...
static void satip_discovery_http_closed(http_client_t* hc, int errn) {
  ...
  if (errn != 0 || s == NULL || hc->hc_code != 200 || hc->hc_data_size == 0 ||
      hc->hc_data == NULL) {
  ...
  if (!xml) {
    tvherror(LS_SATIP,
        "satip_discovery_desc htsmsg_xml_deserialize error %s",
        errbuf);
    goto finish;
  }
  ...
  if ((manufacturer = htsmsg_xml_get_cdata_str(device, "manufacturer")) == NULL)
    manufacturer = "";
  if ((manufacturerURL = htsmsg_xml_get_cdata_str(device, "manufacturerURL")) ==
      NULL)
    manufacturerURL = "";
  if ((modeldesc = htsmsg_xml_get_cdata_str(device, "modelDescription")) ==
      NULL)
    modeldesc = "";
  ...
  if ((serialnum = htsmsg_xml_get_cdata_str(device, "serialNumber")) == NULL)
    serialnum = "";
  if ((presentation = htsmsg_xml_get_cdata_str(device, "presentationURL")) ==
      NULL)
    presentation = "";
  if ((udn = htsmsg_xml_get_cdata_str(device, "UDN")) == NULL)
    goto finish;
  if ((tunercfg = htsmsg_xml_get_cdata_str(device,
           "urn:ses-com:satipX_SATIPCAP")) == NULL)
    tunercfg = "";
...
static void satip_discovery_timerq_cb(void* aux) {
  ...
  while (next) {
    ...
    d->http_client = http_client_connect(d,
        HTTP_VERSION_1_1,
        d->url.scheme,
        d->url.host,
        d->url.port,
  ...
  if (TAILQ_FIRST(&satip_discoveries))
    mtimer_arm_rel(&satip_discovery_timerq,
        satip_discovery_timerq_cb,
        NULL,
        sec2mono(5));
}

static void satip_discovery_service_received(uint8_t* data,
    size_t                                            len,
  ...
  if (inet_ntop(conn->ip.ss_family,
          IP_IN_ADDR(conn->ip),
          sockbuf,
          sizeof(sockbuf)) == NULL) {
...
add_uuid:
  ...
  if (!satip_device_find(uuid))
    mtimer_arm_rel(&satip_discovery_timer,
...
static void satip_discovery_static(const char* descurl) {
  satip_discovery_t* d;
...
static void satip_discovery_service_destroy(upnp_service_t* us) {
  satip_discovery_service = NULL;
}

static void satip_discovery_send_msearch(void* aux) {
#define MSG \
  "\
M-SEARCH * HTTP/1.1\r\n\
...
ST: urn:ses-com:device:SatIPServer:1\r\n"
  ...
  htsbuf_qprintf(&q,
      "USER-AGENT: unix/1.0 UPnP/1.1 tvheadend/%s\r\n",
      tvheadend_version);
  htsbuf_append(&q, "\r\n", 2);
  upnp_send(&q, NULL, 0, 0);
  htsbuf_queue_flush(&q);

  mtimer_arm_rel(&satip_discovery_msearch_timer,
      satip_discovery_send_msearch,
      (void*)(intptr_t)(attempt + 1),
      ms2mono(attempt * 11));
#undef MSG
}

static void satip_discovery_static_timer_cb(void* aux) {
  ...
  mtimer_arm_rel(&satip_discovery_static_timer,
      satip_discovery_static_timer_cb,
      NULL,
      sec2mono(next_timeout));
