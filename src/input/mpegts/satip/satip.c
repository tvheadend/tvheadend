...
static htsmsg_t* satip_device_class_save(idnode_t* in, char* filename, size_t fsize) {
  ...
  if (filename) {
    ...
    snprintf(filename, fsize, "input/satip/adapters/%s", idnode_uuid_as_str(&sd->th_id, ubuf));
  }
...
static void
satip_device_class_get_title(idnode_t* in, const char* lang, char* dst, size_t dstsize) {
  satip_device_t* sd = (satip_device_t*)in;
...
static htsmsg_t* satip_device_class_default_rolloff_list(void* o, const char* lang) {
...
static void satip_device_hack(satip_device_t* sd) {
  ...
  if (strstr(sd->sd_info.location, ":8888/octonet.xml")) {
  ...
  } else if (strstr(sd->sd_info.manufacturer, "Triax") && strstr(sd->sd_info.modelname, "TSS400")) {
    sd->sd_fullmux_ok      = 0;
    sd->sd_pids_max        = 64;
    sd->sd_pids_len        = 255;
    sd->sd_pilot_on        = 1;
    sd->sd_default_rolloff = SATIP_DEFAULT_ROLLOFF_35;
      ...
      (strstr(sd->sd_info.modelname, "EXIP-4124") || strstr(sd->sd_info.modelname, "EXIP-418") ||
...
static satip_device_t* satip_device_create(satip_device_info_t* info) {
  ...
  if (!tvh_hardware_create0((tvh_hardware_t*)sd, &satip_device_class, uhex, conf)) {
...
static void satip_discovery_timerq_cb(void* aux) {
  ...
  while (next) {
    ...
    d->http_client =
        http_client_connect(d, HTTP_VERSION_1_1, d->url.scheme, d->url.host, d->url.port, NULL);
...
static void satip_discovery_timer_cb(void* aux) {
  ...
  if (!atomic_get(&upnp_running)) {
    mtimer_arm_rel(&satip_discovery_timer, satip_discovery_timer_cb, NULL, sec2mono(1));
    return;
  }
  if (satip_discovery_service == NULL) {
  ...
  mtimer_arm_rel(&satip_discovery_timer, satip_discovery_timer_cb, NULL, sec2mono(next_timeout));
  if (next_timeout < 3600)
    next_timeout = next_timeout >= 30 ? 3600 : 30;
}
...
void satip_device_discovery_start(void) {
  if (!satip_enabled)
    return;
  mtimer_arm_rel(&satip_discovery_timer, satip_discovery_timer_cb, NULL, sec2mono(1));
  mtimer_arm_rel(&satip_discovery_static_timer, satip_discovery_static_timer_cb, NULL, sec2mono(1));
}
