...
static const char* satip_rtsp_setup_find(const char* prefix,
    tvh2satip_t*                                     tbl,
    int                                              src,
    const char*                                      defval) {
  while (tbl->t >= 0) {
...
#define ADD(s, d, def)     \
  strcat(buf, "&" #d "="), \
      strcat(buf, satip_rtsp_setup_find(#d, d, dmc->s, def))
...
static int satip_rtsp_pids_split(http_client_t* hc,
    const char**                                pids,
    int                                         size,
    const char*                                 s,
    int                                         maxlen) {
...
static int satip_rtsp_play0(http_client_t* hc,
    int                                    index,
    const char*                            stream,
    const char*                            query) {
...
int satip_rtsp_play(http_client_t* hc,
  ...
  addcnt =
      satip_rtsp_pids_split(hc, add, ARRAY_SIZE(add), addpids, max_pids_len);
  delcnt =
      satip_rtsp_pids_split(hc, del, ARRAY_SIZE(del), delpids, max_pids_len);
