...
static const char*
satip_rtsp_setup_find(const char* prefix, tvh2satip_t* tbl, int src, const char* defval) {
  while (tbl->t >= 0) {
    if (tbl->t == src)
      return tbl->s;
    tbl++;
...
static void satip_rtsp_add_val(const char* name, char* buf, uint32_t val) {
...
static void satip_rtsp_add_ival(const char* name, char* buf, uint32_t val) {
  sprintf(buf + strlen(buf), "&%s=%i", name, val);
}

int satip_rtsp_setup(http_client_t* hc,
    int                             src,
    int                             fe,
    int                             udp_port,
    ...
    int                             flags,
    int                             weight) {
  static tvh2satip_t msys[]  = {{.t = DVB_SYS_DVBT, "dvbt"},
  ...
  static tvh2satip_t mtype[] = {{.t = DVB_MOD_AUTO, "auto"},
      ...
      {.t = TABLE_EOD}};
  static tvh2satip_t plts[]  = {{.t = DVB_PILOT_AUTO, "auto"},
       {.t = DVB_PILOT_ON, "on"},
       {.t = DVB_PILOT_OFF, "off"},
       {.t = TABLE_EOD}};
  static tvh2satip_t fec[]   = {{.t = DVB_FEC_AUTO, "auto"},
        {.t = DVB_FEC_1_2, "12"},
        {.t = DVB_FEC_2_3, "23"},
        {.t = DVB_FEC_3_4, "34"},
        {.t = DVB_FEC_3_5, "35"},
        ...
        {.t = DVB_FEC_9_10, "910"},
        {.t = TABLE_EOD}};
  static tvh2satip_t plsm[]  = {{.t = DVB_PLS_ROOT, "root"},
       {.t = DVB_PLS_GOLD, "gold"},
       {.t = DVB_PLS_COMBO, "combo"},
       {.t = TABLE_EOD}};
  static tvh2satip_t tmode[] = {{.t = DVB_TRANSMISSION_MODE_AUTO, "auto"},
      ...
      {.t = TABLE_EOD}};
  static tvh2satip_t gi[]    = {{.t = DVB_GUARD_INTERVAL_AUTO, "auto"},
  ...
  if (dmc->dmc_fe_delsys == DVB_SYS_DVBS || dmc->dmc_fe_delsys == DVB_SYS_DVBS2) {
    ...
    if (dmc->dmc_fe_modulation != DVB_MOD_NONE && dmc->dmc_fe_modulation != DVB_MOD_AUTO &&
    ...
    if (dmc->dmc_fe_rolloff != DVB_ROLLOFF_NONE && dmc->dmc_fe_rolloff != DVB_ROLLOFF_AUTO) {
    ...
    if (dmc->dmc_fe_pilot != DVB_PILOT_NONE && dmc->dmc_fe_pilot != DVB_PILOT_AUTO) {
    ...
    if (dmc->dmc_fe_modulation != DVB_MOD_AUTO && dmc->dmc_fe_modulation != DVB_MOD_NONE &&
        dmc->dmc_fe_modulation != DVB_MOD_QAM_AUTO)
    ...
    if (dmc->u.dmc_fe_qam.fec_inner != DVB_FEC_NONE && dmc->u.dmc_fe_qam.fec_inner != DVB_FEC_AUTO)
  ...
  } else if (dmc->dmc_fe_delsys == DVB_SYS_DVBT || dmc->dmc_fe_delsys == DVB_SYS_DVBT2) {
    satip_rtsp_add_val("freq", buf, dmc->dmc_fe_freq / 1000);
    ...
    if (dmc->dmc_fe_modulation != DVB_MOD_AUTO && dmc->dmc_fe_modulation != DVB_MOD_NONE &&
        dmc->dmc_fe_modulation != DVB_MOD_QAM_AUTO)
      ADD(dmc_fe_modulation, mtype, dmc->dmc_fe_delsys == DVB_SYS_ATSC ? "8vsb" : "64qam");
  ...
  if (hc->hc_rtsp_stream_id >= 0)
    snprintf(stream = _stream, sizeof(_stream), "/stream=%li", hc->hc_rtsp_stream_id);
...
static int
satip_rtsp_pids_split(http_client_t* hc, const char** pids, int size, const char* s, int maxlen) {
  char* ptr;
  int   idx = 0;
...
static int satip_rtsp_play0(http_client_t* hc, int index, const char* stream, const char* query) {
  tvhtrace(LS_SATIP, "%04X: PLAY params[%d] - %s", hc->hc_id, index, query);
  return rtsp_play(hc, stream, query);
}

int satip_rtsp_play(http_client_t* hc,
    const char*                    pids,
    const char*                    addpids,
    const char*                    delpids,
    int                            max_pids_len,
  ...
  if (pcnt > 0) {
    ...
    if (i == delcnt - 1 && addcnt > 0 && strlen(del[i]) + strlen(add[0]) <= max_pids_len) {
