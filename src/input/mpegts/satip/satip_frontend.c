...
static void udp_rtp_packet_free(satip_frontend_t* lfe,
    struct satip_udppkt*                          up) {
  free(up->up_data);
  free(up);
}

static void udp_rtp_packet_remove(satip_frontend_t* lfe,
    struct satip_udppkt*                            up) {
  TAILQ_REMOVE(&lfe->sf_udp_packets, up, up_link);
  lfe->sf_udp_packets_count--;
}

static void udp_rtp_packet_destroy(satip_frontend_t* lfe,
    struct satip_udppkt*                             up) {
...
static void udp_rtp_packet_append(satip_frontend_t* lfe,
    uint8_t*                                        p,
    int                                             len,
    uint16_t                                        seq) {
...
static int satip_frontend_rtsp_flags(satip_frontend_t* lfe) {
  ...
  if (lfe->sf_transport_mode != RTP_SERVER_DEFAULT)
    rtsp_flags =
        lfe->sf_transport_mode == RTP_INTERLEAVED ? SATIP_SETUP_TCP : 0;
  return rtsp_flags;
}

static satip_frontend_t* satip_frontend_find_by_number(satip_device_t* sd,
    int                                                                num) {
...
static void satip_frontend_signal_cb(void* aux) {
  ...
  mtimer_arm_rel(&lfe->sf_monitor_timer,
      satip_frontend_signal_cb,
      lfe,
      ms2mono(250));
}
...
static int satip_frontend_set_new_type(satip_frontend_t* lfe,
    const char*                                          type) {
...
static int satip_frontend_dvbs_class_positions_set(void* self,
    const void*                                          val) {
...
static htsmsg_t* satip_frontend_dvbs_class_master_enum(void* self,
    const char*                                              lang) {
...
static int satip_frontend_get_weight(mpegts_input_t* mi,
    mpegts_mux_t*                                    mm,
    int                                              flags,
    int                                              weight) {
...
static int satip_frontend_get_max_weight(satip_frontend_t* lfe,
    mpegts_mux_t*                                          mm,
    int                                                    flags) {
...
int satip_frontend_match_satcfg(satip_frontend_t* lfe2,
    mpegts_mux_t*                                 mm2,
    int                                           flags,
    int                                           weight) {
  ...
  if (lfe2->sf_master)
    lfe_master =
        satip_frontend_find_by_number(lfe2->sf_device, lfe2->sf_master) ?: lfe2;
...
static int satip_frontend_is_enabled(mpegts_input_t* mi,
    mpegts_mux_t*                                    mm,
    int                                              flags,
    int                                              weight) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
  ...
  lfe2 = lfe->sf_master
      ? satip_frontend_find_by_number(lfe->sf_device, lfe->sf_master)
      : lfe;
  if (lfe2 == NULL)
    lfe2 = lfe;
...
static void satip_frontend_stop_mux(mpegts_input_t* mi,
...
static int satip_frontend_warm_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                         mmi) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
  satip_satconf_t*  sfc;
  int               r;
  ...
  if (lfe->sf_positions > 0) {
    sfc = satip_satconf_get_position(lfe,
        mmi->mmi_mux,
        &lfe->sf_netposhash,
        2,
        0,
...
static int satip_frontend_start_mux(mpegts_input_t* mi,
    mpegts_mux_instance_t*                          mmi,
    int                                             weight) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
    ...
    if (rtcp[1] == 204 && l > 20 &&           /* packet type */
      ...
      if (sl > 0 && l - 16 >= sl) {
        ...
        __ver12:
          ...
          if (status == SIGNAL_GOOD && mmi->tii_stats.signal == 0 &&
              mmi->tii_stats.snr == 0) {
...
static int satip_frontend_pid_changed(http_client_t* rtsp,
    satip_frontend_t*                                lfe,
    const char*                                      name) {
  satip_tune_req_t* tr;
  satip_device_t*   sd    = lfe->sf_device;
  ...
  } else if (!sd->sd_pids_deladd || tr->sf_pids_tuned.all ||
      tr->sf_pids.count == 0) {

    overlimit = mpegts_pid_weighted(&wpid,
        &tr->sf_pids,
        max_pids_count,
        MPS_WEIGHT_ALLLIMIT);
...
static int satip_frontend_other_is_waiting(satip_frontend_t* lfe) {
  ...
  TAILQ_FOREACH (lfe2, &lfe->sf_device->sd_frontends, sf_link) {
    ...
    cont = 0;
    if (limit0 > 0) {
      cont = group > 0 && lfe2->sf_req_thread &&
          group != lfe2->sf_req_thread->sf_netgroup;
    } else {
...
static void satip_frontend_wake_other_waiting(satip_frontend_t* lfe,
    satip_tune_req_t*                                           tr) {
  satip_frontend_t* lfe2;
  int               hash1, hash2;
...
static void satip_frontend_request_cleanup(satip_frontend_t* lfe,
    satip_tune_req_t*                                        tr) {
  if (tr && tr != lfe->sf_req) {
    mpegts_pid_done(&tr->sf_pids);
    mpegts_pid_done(&tr->sf_pids_tuned);
...
static void satip_frontend_extra_shutdown(satip_frontend_t* lfe,
    uint8_t*                                                session,
    long                                                    stream_id) {
  http_client_t*  rtsp;
  tvhpoll_t*      efd;
  ...
  tvhtrace(LS_SATIP,
      "TEARDOWN request for session %s stream id %li",
      session,
      stream_id);
  r = rtsp_teardown(rtsp, b, NULL);
  if (r < 0) {
    tvhtrace(LS_SATIP,
        "bad shutdown for session %s stream id %li",
        session,
        stream_id);
  } else {
    while (1) {
...
static void satip_frontend_tuning_error(satip_frontend_t* lfe,
    satip_tune_req_t*                                     tr) {
  mpegts_mux_t*          mm;
  mpegts_mux_instance_t* mmi;
...
static int satip_frontend_rtp_decode(satip_frontend_t* lfe,
  ...
  if (seq == -1)
    ...
    if (lfe->sf_udp_packets_count > 5) {
      ...
      *_unc += (up->up_data_len / 188) *
          (uint32_t)((uint16_t)up->up_data_seq - (uint16_t)(seq + 1));
    }
    tvhtrace(LS_SATIP,
        "RTP discontinuity (%i != %i), queueing packet",
        seq + 1,
        nseq);
...
next:
  ...
  if (up && ((*_seq + 1) & 0xffff) == up->up_data_seq) {
    udp_rtp_packet_remove(lfe, up);
    tvhtrace(LS_SATIP,
        "RTP discontinuity, requeueing packet (%i)",
        up->up_data_seq);
    len = satip_frontend_rtp_decode(lfe,
        _seq,
        _unc,
        up->up_data,
        up->up_data_len);
...
satip_frontend_rtp_data_received(http_client_t* hc, void* buf, size_t len) {
  ...
  if (b[1] == 0) {
    ...
    /* note: satip_frontend_decode_rtcp puts '\0' at the end (string
     * termination) */
    len -= 4;
    memmove(b, b + 4, len);
    ...
    tvh_mutex_lock(&lfe->sf_dvr_lock);
    if (lfe->sf_req == lfe->sf_req_thread)
      satip_frontend_decode_rtcp(lfe,
          lfe->sf_display_name,
          lfe->sf_req->sf_mmi,
          b,
          len);
...
#define RTP_PKTS 64
#define UDP_PKT_SIZE                                      \
  1472 /* this is maximum UDP payload (standard ethernet) \
        */
...
new_tune:
  ...
  if (lfe->sf_master)
    lfe_master =
        satip_frontend_find_by_number(lfe->sf_device, lfe->sf_master) ?: lfe;
  ...
  while ((reply || running) && !fatal) {
    ...
    for (i = 0, unc = 0; i < tc; i++) {
      if (satip_frontend_rtp_decode(lfe,
              &seq,
              &unc,
              iovec[i].iov_base,
              iovec[i].iov_len) == 0)
...
satip_frontend_t* satip_frontend_create(htsmsg_t* conf,
    satip_device_t*                               sd,
    dvb_fe_type_t                                 type,
    int                                           v2,
    int                                           num) {
  ...
  if (type == DVB_TYPE_S)
    idc =
        master ? &satip_frontend_dvbs_slave_class : &satip_frontend_dvbs_class;
  ...
  lfe = (satip_frontend_t*)
      mpegts_input_create0((mpegts_input_t*)lfe, idc, uuid, conf);
  if (!lfe)
    return NULL;
  ...
  tvh_thread_create(&lfe->sf_dvr_thread,
      NULL,
      satip_frontend_input_thread,
      lfe,
      "satip-front");
