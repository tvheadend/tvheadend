 ...
 *
 */
static int udp_rtp_packet_cmp(const void* _a, const void* _b) {
  const struct satip_udppkt *a = _a, *b = _b;
  int                        seq1 = a->up_data_seq, seq2 = b->up_data_seq;
  if (seq1 < 0x4000 && seq2 > 0xc000)
    return 1;
  if (seq2 < 0x4000 && seq1 > 0xc000)
    return -1;
  return seq1 - seq2;
}

static void udp_rtp_packet_free(satip_frontend_t* lfe, struct satip_udppkt* up) {
  free(up->up_data);
  free(up);
}

static void udp_rtp_packet_remove(satip_frontend_t* lfe, struct satip_udppkt* up) {
  TAILQ_REMOVE(&lfe->sf_udp_packets, up, up_link);
  lfe->sf_udp_packets_count--;
}

static void udp_rtp_packet_destroy(satip_frontend_t* lfe, struct satip_udppkt* up) {
  udp_rtp_packet_remove(lfe, up);
  udp_rtp_packet_free(lfe, up);
}

static void udp_rtp_packet_destroy_all(satip_frontend_t* lfe) {
...
static void udp_rtp_packet_append(satip_frontend_t* lfe, uint8_t* p, int len, uint16_t seq) {
...
static satip_frontend_t* satip_frontend_find_by_number(satip_device_t* sd, int num) {
...
static char* satip_frontend_bindaddr(satip_frontend_t* lfe) {
...
static void satip_frontend_signal_cb(void* aux) {
  ...
  if (mmi == NULL)
    return;
  if (!lfe->sf_tables) {
    psi_tables_install(mmi->mmi_input,
        mmi->mmi_mux,
  ...
  mtimer_arm_rel(&lfe->sf_monitor_timer, satip_frontend_signal_cb, lfe, ms2mono(250));
}
...
static void satip_frontend_class_changed(idnode_t* in) {
  satip_device_t* la = ((satip_frontend_t*)in)->sf_device;
  satip_device_changed(la);
}

static int satip_frontend_set_new_type(satip_frontend_t* lfe, const char* type) {
...
static int satip_frontend_class_override_set(void* obj, const void* p) {
...
static htsmsg_t* satip_frontend_class_override_enum(void* p, const char* lang) {
...
static htsmsg_t* satip_frontend_transport_mode_list(void* o, const char* lang) {
...
static htsmsg_t* satip_frontend_specinv_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("Do not use"), 0},
      {N_("Off"), 1},
      {N_("On"), 2},
...
const idclass_t satip_frontend_class = {.ic_super = &mpegts_input_class,
...
static htsmsg_t* satip_frontend_dvbt_delsys_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("All"), DVB_SYS_NONE},
...
const idclass_t satip_frontend_dvbt_class = {.ic_super = &satip_frontend_class,
...
static idnode_set_t* satip_frontend_dvbs_class_get_childs(idnode_t* self) {
...
static int satip_frontend_dvbs_class_positions_set(void* self, const void* val) {
...
static int satip_frontend_dvbs_class_master_set(void* self, const void* val) {
  satip_frontend_t* lfe  = self;
...
static htsmsg_t* satip_frontend_dvbs_class_master_enum(void* self, const char* lang) {
...
static htsmsg_t* satip_frontend_dvbs_delsys_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("All"), DVB_SYS_NONE},
...
const idclass_t satip_frontend_dvbs_class = {.ic_super = &satip_frontend_class,
...
const idclass_t satip_frontend_dvbs_slave_class = {.ic_super = &satip_frontend_class,
...
static htsmsg_t* satip_frontend_dvbc_delsys_list(void* o, const char* lang) {
  static const struct strtab tab[] = {
      {N_("All"), DVB_SYS_NONE},
...
const idclass_t satip_frontend_dvbc_class = {.ic_super = &satip_frontend_class,
...
const idclass_t satip_frontend_atsc_t_class = {.ic_super = &satip_frontend_class,
    .ic_class                                            = "satip_frontend_atsc_t",
    .ic_caption                                          = N_("SAT>IP ATSC-T Frontend"),
    .ic_properties                                       = (const property_t[]){{}}};

const idclass_t satip_frontend_atsc_c_class = {.ic_super = &satip_frontend_class,
    .ic_class                                            = "satip_frontend_atsc_c",
    .ic_caption    = N_("TV Adapters - SAT>IP ATSC-C Frontend"),
    .ic_properties = (const property_t[]){{}}};

const idclass_t satip_frontend_isdb_t_class = {.ic_super = &satip_frontend_class,
...
static void satip_frontend_display_name(mpegts_input_t* mi, char* buf, size_t len) {
...
static int satip_frontend_get_weight(mpegts_input_t* mi, mpegts_mux_t* mm, int flags, int weight) {
  return mpegts_input_get_weight(mi, mm, flags, weight);
}

static int satip_frontend_get_priority(mpegts_input_t* mi, mpegts_mux_t* mm, int flags) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
...
static int satip_frontend_get_grace(mpegts_input_t* mi, mpegts_mux_t* mm) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
...
static int satip_frontend_get_max_weight(satip_frontend_t* lfe, mpegts_mux_t* mm, int flags) {
  ...
  TAILQ_FOREACH (lfe2, &lfe->sf_device->sd_frontends, sf_link) {
    if (!lfe2->sf_running)
      continue;
    if (lfe2->sf_type != DVB_TYPE_S)
      continue;
    if (lfe2 != lfe)
      continue;
    if (lfe->sf_master != lfe2->sf_number && lfe2->sf_master != lfe->sf_number)
      continue;
...
int satip_frontend_match_satcfg(satip_frontend_t* lfe2, mpegts_mux_t* mm2, int flags, int weight) {
...
static int satip_frontend_is_enabled(mpegts_input_t* mi, mpegts_mux_t* mm, int flags, int weight) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
  satip_frontend_t* lfe2;
  satip_satconf_t*  sfc;
  int               r;
  ...
  TAILQ_FOREACH (lfe2, &lfe->sf_device->sd_frontends, sf_link) {
    ...
    if (lfe->sf_master == lfe2->sf_number) {
      ...
      return satip_frontend_match_satcfg(lfe2, mm, flags, weight) ? MI_IS_ENABLED_OK
                                                                  : MI_IS_ENABLED_RETRY;
    }
...
static void satip_frontend_stop_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
  satip_tune_req_t* tr;
  char              buf1[256];
...
static int satip_frontend_warm_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
  satip_satconf_t*  sfc;
  int               r;
  ...
  if (lfe->sf_positions > 0) {
    sfc = satip_satconf_get_position(lfe, mmi->mmi_mux, &lfe->sf_netposhash, 2, 0, -1);
    if (sfc) {
...
static int satip_frontend_start_mux(mpegts_input_t* mi, mpegts_mux_instance_t* mmi, int weight) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
  ...
  if (!lfe->sf_device->sd_no_univ_lnb &&
    ...
    if (lm->lm_tuning.dmc_fe_freq < 10700000 || lm->lm_tuning.dmc_fe_freq > 12750000) {
      tvhwarn(LS_SATIP,
          "DVB-S/S2 frequency %d out of range universal LNB",
          lm->lm_tuning.dmc_fe_freq);
      return SM_CODE_TUNING_FAILED;
    }
  }
  ...
  mtimer_arm_rel(&lfe->sf_monitor_timer, satip_frontend_signal_cb, lfe, ms2mono(50));

  return 0;
}

static void satip_frontend_update_pids(mpegts_input_t* mi, mpegts_mux_t* mm) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
  satip_tune_req_t* tr;
  mpegts_pid_t*     mp;
...
static void satip_frontend_open_service(mpegts_input_t* mi,
    mpegts_service_t*                                   s,
    int                                                 flags,
    int                                                 init,
    int                                                 weight) {
...
static idnode_set_t* satip_frontend_network_list(mpegts_input_t* mi) {
  satip_frontend_t* lfe = (satip_frontend_t*)mi;
...
static void satip_frontend_decode_rtcp(satip_frontend_t* lfe,
    const char*                                          name,
  ...
  while (len >= 12) {
    ...
    if (rtcp[1] == 204 && l > 20 &&           /* packet type */
        rtcp[8] == 'S' && rtcp[9] == 'E' && rtcp[10] == 'S' && rtcp[11] == '1') {
      ...
      if (sl > 0 && l - 16 >= sl) {
        ...
        if (strncmp(s, "ver=1.2;src=1;tuner=", 20) == 0) {
          ...
          goto __ver12;
        } else if (strncmp(s, "ver=0.9;tuner=", 14) == 0 || strncmp(s, "ver=1.2;tuner=", 14) == 0) {
        ...
        __ver12:
          ...
          if (atoi(argv[0]) != lfe->sf_number && device->sd_sig_tunerno)
            goto fail;
          mmi->tii_stats.signal       = atoi(argv[1]) * 0xffff / device->sd_sig_scale;
          ...
          if (status == SIGNAL_GOOD && mmi->tii_stats.signal == 0 && mmi->tii_stats.snr == 0) {
...
static int
satip_frontend_pid_changed(http_client_t* rtsp, satip_frontend_t* lfe, const char* name) {
  satip_tune_req_t* tr;
  satip_device_t*   sd    = lfe->sf_device;
  ...
  } else if (!sd->sd_pids_deladd || tr->sf_pids_tuned.all || tr->sf_pids.count == 0) {

    overlimit = mpegts_pid_weighted(&wpid, &tr->sf_pids, max_pids_count, MPS_WEIGHT_ALLLIMIT);
...
static int satip_frontend_other_is_waiting(satip_frontend_t* lfe) {
  satip_device_t*   sd = lfe->sf_device;
  satip_frontend_t* lfe2;
...
static void satip_frontend_wake_other_waiting(satip_frontend_t* lfe, satip_tune_req_t* tr) {
  satip_frontend_t* lfe2;
  int               hash1, hash2;
...
static void satip_frontend_request_cleanup(satip_frontend_t* lfe, satip_tune_req_t* tr) {
  if (tr && tr != lfe->sf_req) {
    mpegts_pid_done(&tr->sf_pids);
    mpegts_pid_done(&tr->sf_pids_tuned);
    free(tr);
...
static void satip_frontend_extra_shutdown(satip_frontend_t* lfe, uint8_t* session, long stream_id) {
  http_client_t*  rtsp;
  tvhpoll_t*      efd;
  tvhpoll_event_t ev;
  int             r, nfds;
...
static void satip_frontend_shutdown(satip_frontend_t* lfe,
    const char*                                       name,
    http_client_t*                                    rtsp,
    satip_tune_req_t*                                 tr,
    tvhpoll_t*                                        efd) {
...
static void satip_frontend_tuning_error(satip_frontend_t* lfe, satip_tune_req_t* tr) {
  mpegts_mux_t*          mm;
  mpegts_mux_instance_t* mmi;
  char                   uuid[UUID_HEX_SIZE];

  tvh_mutex_lock(&lfe->sf_dvr_lock);
  if (lfe->sf_running && lfe->sf_req == tr && (mmi = tr->sf_mmi) != NULL &&
      (mm = mmi->mmi_mux) != NULL) {
...
static void satip_frontend_close_rtsp(satip_frontend_t* lfe,
    const char*                                         name,
    tvhpoll_t*                                          efd,
    http_client_t*                                      rtsp,
    satip_tune_req_t*                                   tr) {
...
static int satip_frontend_rtp_decode(satip_frontend_t* lfe,
    uint32_t*                                          _seq,
    uint32_t*                                          _unc,
    uint8_t*                                           p,
    int                                                c) {
  ...
  if (seq == -1)
    ...
    if (lfe->sf_udp_packets_count > 5) {
      ...
      tvhtrace(LS_SATIP,
          "RTP discontinuity, reset sequence to %d from %d",
          up->up_data_seq,
          (seq + 1) & 0xffff);
      *_seq = (up->up_data_seq - 1) & 0xffff;
      *_unc +=
          (up->up_data_len / 188) * (uint32_t)((uint16_t)up->up_data_seq - (uint16_t)(seq + 1));
    }
...
static int satip_frontend_rtp_data_received(http_client_t* hc, void* buf, size_t len) {
  uint32_t               unc;
  uint8_t*               b   = buf;
  ...
  if (b[1] == 0) {
    ...
    if (lfe->sf_sbuf.sb_ptr > 64 * 1024 || lfe->sf_last_data_tstamp + sec2mono(1) <= mclk()) {
      tvh_mutex_lock(&lfe->sf_dvr_lock);
    ...
    tvh_mutex_lock(&lfe->sf_dvr_lock);
    if (lfe->sf_req == lfe->sf_req_thread)
      satip_frontend_decode_rtcp(lfe, lfe->sf_display_name, lfe->sf_req->sf_mmi, b, len);
    tvh_mutex_unlock(&lfe->sf_dvr_lock);
  }
...
static void* satip_frontend_input_thread(void* aux) {
#define RTP_PKTS      64
...
new_tune:
  ...
  if (!satip_frontend_other_is_waiting(lfe))
    start |= 2;
  else
    start &= ~2;
  ...
  while (start != 3) {
    ...
    if (rtsp &&
        (getfastmonoclock() - u64_2 > 50000 || /* 50ms */
            (start & 2) == 0)) {
      satip_frontend_close_rtsp(lfe, buf, efd, rtsp, tr);
      rtsp = NULL;
      tr   = NULL;
  ...
  if ((rtsp_flags & SATIP_SETUP_TCP) == 0) {
    ...
    tvhtrace(LS_SATIP,
        "%s - local RTP port %i RTCP port %i",
        lfe->mi_name,
  ...
  while ((reply || running) && !fatal) {
      ...
      if (r < 0) {
        ...
        switch (rtsp->hc_cmd) {
          ...
          case RTSP_CMD_SETUP:
            ...
            if (r < 0 ||
                ((rtsp_flags & SATIP_SETUP_TCP) == 0 &&
                    (rtsp->hc_rtp_port != rtp_port || rtsp->hc_rtcp_port != rtp_port + 1)) ||
                ((rtsp_flags & SATIP_SETUP_TCP) != 0 &&
                    (rtsp->hc_rtp_tcp < 0 || rtsp->hc_rtcp_tcp < 0))) {
              tvherror(LS_SATIP,
                  "%s - RTSP SETUP error %d (%s) [%i-%i]",
                  buf,
                  r,
                  strerror(-r),
                  rtsp->hc_cmd,
              ...
              tvhdebug(LS_SATIP,
                  "%s #%i - new session %s stream id %li",
                  rtsp->hc_host,
                  lfe->sf_number,
                  rtsp->hc_rtsp_session,
                  rtsp->hc_rtsp_stream_id);
              if (lfe->sf_play2) {
                r = -12345678;
                tvh_mutex_lock(&lfe->sf_dvr_lock);
                if (lfe->sf_req == lfe->sf_req_thread) {
          ...
          default:
            if (rtsp->hc_code >= 400) {
              tvherror(LS_SATIP,
                  "%s - RTSP cmd error %d (%s) [%i-%i]",
                  buf,
                  r,
                  r > 0 ? http_client_con2str(r) : strerror(-r),
                  rtsp->hc_cmd,
                  rtsp->hc_code);
              satip_frontend_tuning_error(lfe, tr);
              fatal = 1;
    ...
    if (tc < 0) {
      ...
      tvherror(LS_SATIP, "%s - multirecv error %d (%s)", buf, errno, strerror(errno));
      break;
    } else {
      lfe->sf_last_activity_tstamp = mclk();
    }
...
static int satip_frontend_default_positions(satip_frontend_t* lfe) {
  satip_device_t* sd = lfe->sf_device;
...
static mpegts_network_t* satip_frontend_wizard_network(satip_frontend_t* lfe) {
  satip_satconf_t* sfc;
...
static htsmsg_t* satip_frontend_wizard_get(tvh_input_t* ti, const char* lang) {
...
static void satip_frontend_wizard_set(tvh_input_t* ti, htsmsg_t* conf, const char* lang) {
  satip_frontend_t* lfe   = (satip_frontend_t*)ti;
...
satip_frontend_t*
satip_frontend_create(htsmsg_t* conf, satip_device_t* sd, dvb_fe_type_t type, int v2, int num) {
  const idclass_t*  idc;
  ...
  /* Class */
  if (type == DVB_TYPE_S)
    idc = master ? &satip_frontend_dvbs_slave_class : &satip_frontend_dvbs_class;
  ...
  /* Default name */
  if (!lfe->mi_name ||
      (strncmp(lfe->mi_name, "SAT>IP ", 7) == 0 && strstr(lfe->mi_name, " Tuner"))) {
  ...
  tvh_thread_create(&lfe->sf_dvr_thread, NULL, satip_frontend_input_thread, lfe, "satip-front");

  return lfe;
}

void satip_frontend_save(satip_frontend_t* lfe, htsmsg_t* fe) {
  char      id[16];
