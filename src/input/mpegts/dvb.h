...
static inline uint32_t bcdtoint4(const uint8_t* ptr) {
  return (bcdtoint(ptr[0]) * 1000000) + (bcdtoint(ptr[1]) * 10000) +
      (bcdtoint(ptr[2]) * 100) + bcdtoint(ptr[3]);
}
static inline uint32_t bcdtoint41(const uint8_t* ptr) {
  return (bcdtoint(ptr[0]) * 100000) + (bcdtoint(ptr[1]) * 1000) +
      (bcdtoint(ptr[2]) * 10) + ((ptr[3]) >> 4);
}
...
typedef void (*mpegts_psi_section_callback_t)(const uint8_t* tsb,
    size_t                                                   len,
    void*                                                    opaque);
...
typedef void (*mpegts_psi_parse_callback_t)(mpegts_psi_table_t*,
    const uint8_t* buf,
    int            len);
...
int dvb_table_remux(mpegts_psi_table_t* mt,
    const uint8_t*                      buf,
    int                                 len,
    uint8_t**                           out);
...
static inline int dvb_modulation_is_none_or_auto(int modulation) {
  return modulation == DVB_MOD_NONE || modulation == DVB_MOD_AUTO ||
