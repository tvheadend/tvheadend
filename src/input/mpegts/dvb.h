...
typedef struct dvb_string_conv {
  uint8_t type;
  size_t (*func)(char* dst, size_t* dstlen, const uint8_t* src, size_t srclen);
} dvb_string_conv_t;
...
struct lang_str* atsc_get_string(const uint8_t* src, size_t srclen);
...
static inline uint32_t bcdtoint4(const uint8_t* ptr) {
  return (bcdtoint(ptr[0]) * 1000000) + (bcdtoint(ptr[1]) * 10000) + (bcdtoint(ptr[2]) * 100) +
      bcdtoint(ptr[3]);
}
static inline uint32_t bcdtoint41(const uint8_t* ptr) {
  return (bcdtoint(ptr[0]) * 100000) + (bcdtoint(ptr[1]) * 1000) + (bcdtoint(ptr[2]) * 10) +
      ((ptr[3]) >> 4);
}
...
#define DVB_LOOP_INIT(mt, ptr, len, off, lptr, llen)       \
  do {                                                     \
    ...
    len -= 2 + off + llen;                                 \
    if (len < 0) {                                         \
      tvhtrace(mt->mt_subsys, "%s: len < 0", mt->mt_name); \
      goto dvberr;                                         \
    }                                                      \
  } while (0)

#define DVB_LOOP_EACH(ptr, len, min) for (; len > min;)
...
#define DVB_DESC_EACH(mt, ptr, len, dtag, dlen, dptr)              \
  DVB_LOOP_EACH(ptr, len, 2) {                                     \
    ...
    if ((len -= 2 + dlen) < 0) {                                   \
      tvhtrace(mt->mt_subsys, "%s: dlen < 0", mt->mt_name);        \
      goto dvberr;                                                 \
    }
...
typedef void (*mpegts_psi_section_callback_t)(const uint8_t* tsb, size_t len, void* opaque);
...
typedef void (*mpegts_psi_parse_callback_t)(mpegts_psi_table_t*, const uint8_t* buf, int len);
...
void dvb_mux_conf_init(struct mpegts_network* ln,
    dvb_mux_conf_t*                           dmc,
...
static inline int dvb_modulation_is_none_or_auto(int modulation) {
  return modulation == DVB_MOD_NONE || modulation == DVB_MOD_AUTO || modulation == DVB_MOD_QAM_AUTO;
}
