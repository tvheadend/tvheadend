...
static inline uint16_t extract_len12(const uint8_t* ptr) {
  return ((ptr[0] & 0xf) << 8) | ptr[1];
}

static inline void put_2byte(uint8_t* dst, uint16_t val) {
  dst[0] = val >> 8;
  dst[1] = val;
}

static inline void put_len12(uint8_t* dst, uint16_t val) {
  dst[0] &= 0xf0;
  dst[0] |= (val >> 8) & 0x0f;
  dst[1] = val;
}
...
static int en50221_capmt_check_pid(mpegts_service_t* s, uint16_t pid) {
...
static int en50221_capmt_check_caid(mpegts_service_t* s,
    uint16_t                                          pid,
    uint16_t                                          caid,
...
int en50221_capmt_build(mpegts_service_t* s,
    int                                   bcmd,
    uint16_t                              svcid,
    const uint16_t*                       caids,
    int                                   caids_count,
  ...
  switch (bcmd) {
    case EN50221_CAPMT_BUILD_ONLY:
      d[0] = 3;
      break;
    case EN50221_CAPMT_BUILD_ADD:
      d[0] = 4;
      break;
    case EN50221_CAPMT_BUILD_DELETE:
      cmd_id = EN50221_CAPMT_CMD_NOTSEL;
      /* fallthru */
    case EN50221_CAPMT_BUILD_UPDATE:
      d[0] = 5;
      break;
    default:
      goto reterr;
...
int en50221_capmt_build_query(const uint8_t* capmt,
    size_t                                   capmtlen,
    uint8_t**                                dst,
    size_t*                                  dstlen) {
  uint8_t *      d, *x;
...
void en50221_capmt_dump(int subsys,
    const char*             prefix,
    const uint8_t*          capmt,
    size_t                  capmtlen) {
  uint16_t       l, caid, pid;
  ...
  tvhtrace(subsys,
      "%s: CAPMT list_management %02x sid %04x ver %02x",
      prefix,
      capmt[0],
      ((int)capmt[1]) << 8 | capmt[2],
      capmt[3]);
  l     = extract_len12(capmt + 4);
  p     = capmt + 6;
  capmt = p + l;
  ...
  if (l > 0) {
    ...
    while (l > 0) {
      if (l < 6) {
        tvhtrace(subsys,
            "%s:   CAPMT wrong CA descriptor length %d",
            prefix,
            l);
        break;
      }
      ...
      if (dtag != DVB_DESC_CA) {
        ...
        tvhtrace(subsys,
            "%s:   CAPMT wrong CA descriptor length %d (2)",
            prefix,
            l);
      } else {
        caid = extract_2byte(p + 2);
        pid  = extract_pid(p + 4);
        tvhtrace(subsys,
            "%s:   CAPMT CA descriptor caid %04X pid %04x length %d (%s)",
            prefix,
            caid,
            pid,
            dlen,
            bin2hex(hbuf, sizeof(hbuf), p + 6, dlen - 4));
      }
      p += dlen + 2;
      l -= dlen + 2;
    }
    ...
    if (l > 0) {
      ...
      while (l > 2) {
        ...
        if (dlen + 2 > l) {
          ...
          tvhtrace(subsys,
              "%s:     CAPMT ES wrong CA descriptor length %d (2)",
              prefix,
              l);
        } else {
          caid = extract_2byte(p + 2);
          pid  = extract_pid(p + 4);
          tvhtrace(subsys,
              "%s:     CAPMT ES CA descriptor caid %04X pid %04x length %d "
              "(%s)",
              prefix,
              caid,
              pid,
