...
static int en50221_app_resman_handle(void* self,
  ...
  if (atag == CICAM_AOT_PROFILE_ENQ) {
    ...
    tvherror(LS_EN50221,
        "%s: unknown atag for resman 0x%06x",
        app->cia_name,
        atag);
  }
  return 0;
...
int en50221_send_capmt(en50221_slot_t* slot,
    const uint8_t*                     capmt,
    size_t                             capmtlen) {
  en50221_app_t* app;
  app = en50221_slot_find_application(slot,
      CICAM_RI_CONDITIONAL_ACCESS_SUPPORT,
      ~0);
  if (app)
...
static int en50221_app_datetime_send(en50221_app_datetime_t* app,
    int64_t                                                  clock) {
  time_t    t = time(NULL);
  struct tm tm_gmt;
  struct tm tm_loc;
  ...
  if (gmtime_r(&t, &tm_gmt) && localtime_r(&t, &tm_loc)) {
    ...
    int L      = (M == 1 || M == 2) ? 1 : 0;
    int MJD =
        14956 + D + (int)((Y - L) * 365.25) + (int)((M + 1 + L * 12) * 30.6001);
    buf[0] = MJD >> 8;
    buf[1] = MJD & 0xff;
    ...
    r = en50221_app_pdu_send((en50221_app_t*)app,
        CICAM_AOT_DATE_TIME,
        buf,
        7,
        0);
...
static int en50221_app_datetime_handle(void* self,
  ...
  if (atag == CICAM_AOT_DATE_TIME_ENQ) {
    ...
    tvhtrace(LS_EN50221,
        "%s: CAM requested datetime with interval %d",
        app->cia_name,
        interval);
    en50221_app_datetime_send(app, mclk());
  } else {
    tvherror(LS_EN50221,
        "%s: unknown atag for datetime 0x%06x",
        app->cia_name,
        atag);
  }
  return 0;
  ...
  if (atag >= CICAM_AOT_APPLICATION_INFO &&
    ...
    if (app->cia_info_version >= 3) /* at least CI+ v1.3 */
      if (CICAM_CALL_APP_CB(app, cisw_pcmcia_data_rate, &rate) >= 0)
        return en50221_app_pdu_send((en50221_app_t*)app,
            CICAM_AOT_PCMCIA_DATA_RATE,
            &rate,
            1,
            0);
  } else {
    tvherror(LS_EN50221,
        "%s: unknown atag for appinfo 0x%06x",
        app->cia_name,
        atag);
  }
  return 0;
...
int en50221_pcmcia_data_rate(en50221_slot_t* slot, uint8_t rate) {
  en50221_app_t* app;
  app = en50221_slot_find_application(slot,
      CICAM_RI_APPLICATION_INFORMATION,
      ~0x3f);
...
static int getmenutext(en50221_app_mmi_t* app,
    const char*                           key,
    int                                   list,
    const uint8_t**                       _p,
    size_t*                               _l) {
  ...
  if (tag != CICAM_AOT_TEXT_LAST) {
    tvherror(LS_EN50221,
        "%s: unexpected getmenutext tag %06X",
        app->cia_name,
        tag);
    return -ENXIO;
  }
  ...
  if (atag == CICAM_AOT_DISPLAY_CONTROL) {
    ...
    if (datalen > 0) {
      if (data[0] == MMI_DCC_SET_MMI_MODE) {
        ...
        tvhtrace(LS_EN50221,
            "%s: display control sending reply",
            app->cia_name);
        return en50221_app_pdu_send((en50221_app_t*)app,
...
static int en50221_mmi_basic_io(en50221_slot_t* slot,
    uint32_t                                    ri,
    const uint8_t*                              data,
    size_t                                      datalen) {
  en50221_app_t* app;
...
int en50221_mmi_answer(en50221_slot_t* slot,
    const uint8_t*                     data,
    size_t                             datalen) {
