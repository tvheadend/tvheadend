...
#define BYTE4BE(w) (((w) >> 24) & 0xff), (((w) >> 16) & 0xff), (((w) >> 8) & 0xff), ((w) & 0xff)
...
static int en50221_app_resman_open(void* self) {
  en50221_app_t* app = self;
  return en50221_app_pdu_send(app, CICAM_AOT_PROFILE_ENQ, NULL, 0, 1);
}

static int en50221_app_resman_handle(void* self,
    uint8_t                                status,
    uint32_t                               atag,
    const uint8_t*                         data,
    size_t                                 datalen) {
  ...
  if (atag == CICAM_AOT_PROFILE_ENQ) {
    ...
    return en50221_app_pdu_send((en50221_app_t*)app,
        CICAM_AOT_PROFILE,
        resources,
        sizeof(resources),
        0);
    ...
    if (!app->cia_profile_sent) {
      ...
      return en50221_app_pdu_send((en50221_app_t*)app, CICAM_AOT_PROFILE_CHANGE, NULL, 0, 0);
    }
  } else {
...
#define CICAM_CA_ENABLE(x) (((x) & CICAM_CA_ENABLE_FLAG) ? (x) & ~CICAM_CA_ENABLE_FLAG : 0)
...
static void en50221_app_ca_destroy(void* self) {
  en50221_app_ca_t* app = self;
  CICAM_CALL_APP_CB(app, cisw_ca_close);
}

static int en50221_app_ca_open(void* self) {
  en50221_app_t* app = self;
  return en50221_app_pdu_send(app, CICAM_AOT_CA_INFO_ENQ, NULL, 0, 1);
}

static int en50221_app_ca_handle(void* self,
    uint8_t                            status,
    uint32_t                           atag,
    const uint8_t*                     data,
    size_t                             datalen) {
  ...
  if (atag == CICAM_AOT_CA_INFO) {
    count = 0;
    for (; count < ARRAY_SIZE(app->cia_caids) - 1 && datalen > 1; data += 2, datalen -= 2, count++)
...
int en50221_send_capmt(en50221_slot_t* slot, const uint8_t* capmt, size_t capmtlen) {
  en50221_app_t* app;
...
static int en50221_app_datetime_send(en50221_app_datetime_t* app, int64_t clock) {
  time_t    t = time(NULL);
  struct tm tm_gmt;
  struct tm tm_loc;
  uint8_t   buf[7];
...
static int en50221_app_datetime_handle(void* self,
    uint8_t                                  status,
    uint32_t                                 atag,
    const uint8_t*                           data,
    size_t                                   datalen) {
...
static int en50221_app_datetime_monitor(void* self, int64_t clock) {
  en50221_app_datetime_t* app = self;
  if (app->cia_update_interval > 0 && app->cia_last_clock + app->cia_update_interval < clock)
...
static int en50221_app_appinfo_open(void* self) {
  en50221_app_t* app = self;
  return en50221_app_pdu_send(app, CICAM_AOT_APPLICATION_INFO_ENQ, NULL, 0, 1);
}

static int en50221_app_appinfo_handle(void* self,
    uint8_t                                 status,
    uint32_t                                atag,
    const uint8_t*                          data,
    size_t                                  datalen) {
  ...
  if (atag >= CICAM_AOT_APPLICATION_INFO && atag <= CICAM_AOT_APPLICATION_INFO + 2) {
    if (datalen < 6) {
      tvherror(LS_EN50221,
          "%s: unknown length %zd for appinfo 0x%06x",
          app->cia_name,
          datalen,
          atag);
      return -ENXIO;
...
static void en50221_app_mmi_destroy(void* self) {
...
static int
getmenutext(en50221_app_mmi_t* app, const char* key, int list, const uint8_t** _p, size_t* _l) {
...
static int en50221_app_mmi_handle(void* self,
    uint8_t                             status,
    uint32_t                            atag,
    const uint8_t*                      data,
    size_t                              datalen) {
  ...
  if (atag == CICAM_AOT_DISPLAY_CONTROL) {
    ...
    if (datalen > 0) {
      ...
      p = data + 1;
      l = datalen - 1; /* ignore choice_nb */
      r = getmenutext(app, "title", 0, &p, &l);
      if (!r)
        r = getmenutext(app, "subtitle", 0, &p, &l);
      if (!r)
        r = getmenutext(app, "bottom", 0, &p, &l);
      while (!r && l > 0)
    ...
    if (datalen > 1) {
      ...
      if (dvb_get_string(txt, datalen * 2 + 1, data + 2, datalen - 2, NULL, NULL) > 0 && txt[0])
        htsmsg_add_str(app->cia_enquiry, "text", txt);
      tvhtrace(LS_EN50221, "%s: enquiry data %02x %02x '%s'", app->cia_name, data[0], data[1], txt);
...
static int
en50221_mmi_basic_io(en50221_slot_t* slot, uint32_t ri, const uint8_t* data, size_t datalen) {
  en50221_app_t* app;
...
int en50221_mmi_answer(en50221_slot_t* slot, const uint8_t* data, size_t datalen) {
