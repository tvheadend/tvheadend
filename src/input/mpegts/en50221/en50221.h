...
struct en50221_app_prop {
  ...
  int (
      *ciap_handle)(void* self, uint8_t status, uint32_t atag, const uint8_t* data, size_t datalen);
...
struct en50221_ops {
  ...
  int (*cihw_pdu_write)(void* aux,
      en50221_slot_t*         slot,
      uint8_t                 tcnum,
      const uint8_t*          data,
      size_t                  datalen);
  int (*cihw_apdu_write)(void* aux, en50221_slot_t* slot, const uint8_t* data, size_t datalen);
  /* software level ops */
  int (*cisw_appinfo)(void* aux,
      en50221_slot_t*       slot,
      uint8_t               ver,
      char*                 name,
      uint8_t               type,
...
#define CICAM_CALL_APP_CB(app, name, ...)                                                  \
  ({                                                                                       \
    ...
    int __r = __t->cit_ops ? __t->cit_ops->name(__t->cit_ops_aux, __s, ##__VA_ARGS__) : 0; \
    __r;                                                                                   \
  })
