...
static int  en50221_slot_create(en50221_transport_t* cit,
     uint8_t                                         number,
     en50221_slot_t**                                cil);
...
#endif
static int  en50221_slot_spdu_recv(en50221_slot_t* cil,
     const uint8_t*                                data,
     size_t                                        datalen);
...
static int  en50221_app_handle(en50221_app_t* cia,
     uint8_t                                  status,
     const uint8_t*                           data,
     size_t                                   datalen);
...
static int en50221_transport_pdu_write(en50221_transport_t* cit,
  ...
  return cit->cit_ops->cihw_pdu_write(cit->cit_ops_aux,
      cil,
      tcnum,
      buf,
      datalen + hlen - 1);
...
en50221_slot_t* en50221_transport_find_slot(en50221_transport_t* cit,
    uint8_t                                                      slotnum) {
  en50221_slot_t* slot;
  ...
  while (p + 3 <= end) {
    ...
    switch (tag) {
      case CICAM_T_SB:
        ...
        if (slot->cil_ready == 0) {
          tvhwarn(LS_EN50221,
              "%s: received sb but slot is not ready",
              cit->cit_name);
          break;
        }
        /* data are waiting? tell CAM that we can receive them or flush the
         * write queue */
        if (l == 2) {
          if ((p[1] & 0x80) != 0) {
            r = en50221_transport_pdu_write(cit,
                slot,
                tcnum,
                CICAM_T_RCV,
                NULL,
      ...
      case CICAM_T_CTC_REPLY:
        if (l != 1) {
          tvherror(LS_EN50221,
              "%s: invalid length %zd for tag 0x%02x",
              cit->cit_name,
              l,
              tag);
          return en50221_transport_broken(cit, data, datalen);
      ...
      case CICAM_T_DATA_LAST:
        if (slot->cil_ready == 0) {
          tvhwarn(LS_EN50221,
              "%s: received data but slot is not ready",
              cit->cit_name);
          break;
        }
        if (l > 1) {
          ...
          if (tag == CICAM_T_DATA_LAST) {
            if (cit->cit_rmsg.sb_ptr == 0) {
              ...
              r = en50221_slot_spdu_recv(slot,
                  cit->cit_rmsg.sb_data,
                  cit->cit_rmsg.sb_ptr);
      ...
      default:
        tvherror(LS_EN50221,
            "%s: unknown transport tag 0x%02x",
            cit->cit_name,
            tag);
        return en50221_transport_broken(cit, data, datalen);
    }
...
static en50221_app_t* en50221_slot_find_session(en50221_slot_t* cil,
    uint16_t                                                    session_id) {
  en50221_app_t* cia;
...
en50221_app_t* en50221_slot_find_application(en50221_slot_t* cil,
    uint32_t                                                 resource_id,
    uint32_t                                                 mask) {
  en50221_app_t* cia;
...
static int en50221_slot_spdu_recv(en50221_slot_t* cil,
  ...
  switch (data[0]) {
    case CICAM_ST_SESSION_NUMBER:
      if (datalen <= 4) {
        tvherror(LS_EN50221,
            "%s: unhandled session number length %zd",
            cil->cil_name,
            datalen);
        return -EINVAL;
      }
    ...
    case CICAM_ST_CREATE_SESSION_RESPONSE:
      ...
      if (data[2] != CICAM_SS_OK) {
        tvherror(LS_EN50221,
            "%s: CAM rejected to create session 0x%04x",
            cil->cil_name,
            session);
        return -EINVAL;
      }
...
static int en50221_slot_monitor(en50221_slot_t* cil, int64_t clock) {
  ...
  LIST_FOREACH (cia, &cil->cil_apps, cia_link) {
    if (cil->cil_monitor_read + ms2mono(500) < mclk()) {
      tvherror(LS_EN50221,
          "%s: communication stalled for more than 500ms",
          cil->cil_name);
      return -ENXIO;
    }
...
int en50221_app_pdu_send(en50221_app_t* app,
  ...
  if (slot->cil_apdu_only)
    return cit->cit_ops->cihw_apdu_write(cit->cit_ops_aux,
        slot,
        buf + 4,
        buflen - 4);
  p = buf;
...
static int en50221_app_handle(en50221_app_t* cia,
  ...
  if (prop && prop->ciap_handle) {
    ...
    if (atag < 8) {
      tvherror(LS_EN50221,
          "%s: invalid atag received 0x%06x",
          cia->cia_name,
          atag);
      return -ENXIO;
    }
