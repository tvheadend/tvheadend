    ...
    if (strcmp(class_name, "AVFormatContext") == 0) {
      if (strcmp(fmt, "Negative cts, previous timestamps might be wrong.\n") ==
        ...
        level = AV_LOG_TRACE;
                     ...
                     "Invalid timestamps stream=%d, pts=%s, dts=%s, "
                     "size=%d\n") == 0 &&
          strstr(fmt, ", pts=-") == 0) {
        level = AV_LOG_TRACE;
      }
      ...
      if (strcmp(fmt,
              "forced frame type (%d) at %d was changed to frame type "
              "(%d)\n") == 0) {
        level = AV_LOG_TRACE;
      }
    }
 ...
 */
enum AVCodecID streaming_component_type2codec_id(
 ...
 */
streaming_component_type_t codec_id2streaming_component_type(
    enum AVCodecID id) {
 ...
 *
 */
static void libav_vaapi_init(void) {
#if ENABLE_VAAPI
#ifdef VA_FOURCC_I010
#if !VA_CHECK_VERSION(1, 0, 0)
