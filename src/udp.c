...
udp_connection_t* udp_bind(int subsystem,
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse))) {
    tvherror(subsystem,
        "%s - failed to reuse address for socket [%s]",
        name,
        strerror(errno));
    udp_close(uc);
    return UDP_FATAL_ERROR;
    ...
    if (uc->multicast) {
      ...
      if (multicast_src && *multicast_src) {
        ...
        if (inet_pton(AF_INET, multicast_src, &ms.imr_sourceaddr) < 1) {
          tvherror(subsystem,
              "%s - invalid ipv4 address '%s' specified as multicast source "
              "[e=%s]",
              name,
              multicast_src,
              strerror(errno));
  ...
  if (rxsize > 0 &&
    ...
    tvhwarn(subsystem,
        "%s - cannot change UDP rx buffer size [%s]",
        name,
        strerror(errno));
  ...
  if (txsize > 0 &&
    ...
    tvhwarn(subsystem,
        "%s - cannot change UDP tx buffer size [%s]",
        name,
        strerror(errno));
...
udp_connection_t*
udp_sendinit(int subsystem, const char* name, const char* ifname, int txsize) {
  int               fd, ifindex;
  udp_connection_t* uc;
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &txsize, sizeof(txsize)) == -1)
    tvhwarn(subsystem,
        "%s - cannot increase UDP tx buffer size [%s]",
        name,
        strerror(errno));
...
int udp_connect(udp_connection_t* uc,
    const char*                   name,
    const char*                   host,
    int                           port) {
  char buf[50];
...
int udp_write(udp_connection_t* uc,
    const void*                 buf,
    size_t                      len,
  ...
  while (len) {
    r = sendto(uc->fd,
        ...
        storage->ss_family == AF_INET6 ? sizeof(struct sockaddr_in6)
                                       : sizeof(struct sockaddr_in));
    if (r < 0) {
...
int udp_write_queue(udp_connection_t* uc,
    htsbuf_queue_t*                   q,
    struct sockaddr_storage*          storage) {
  htsbuf_data_t* hd;
  int            l, r = 0;
...
static inline int recvmmsg_i(int sockfd,
    struct mmsghdr*              msgvec,
    unsigned int                 vlen,
    unsigned int                 flags) {
  ssize_t      r;
...
int udp_multirecv_read(udp_multirecv_t* um,
    int                                 fd,
    int                                 packets,
    struct iovec**                      iovec) {
...
static inline int sendmmsg_i(int sockfd,
    struct mmsghdr*              msgvec,
    unsigned int                 vlen,
    unsigned int                 flags) {
  ssize_t      r;
...
void udp_multisend_init(udp_multisend_t* um,
    int                                  packets,
    int                                  psize,
    struct iovec**                       iovec) {
  int i;
