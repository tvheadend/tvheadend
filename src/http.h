...
static inline int http_args_empty(const http_arg_list_t* list) {
...
typedef int(http_callback_t)(http_connection_t* hc, const char* remain, void* opaque);

typedef char*(http_path_modify_t)(http_connection_t* hc, const char* path, int* cut);
...
struct http_client {
  ...
  /* callbacks */
  void (*hc_hdr_create)(http_client_t* hc, http_arg_list_t* h, const url_t* url, int keepalive);
...
void http_client_basic_auth(http_client_t* hc,
    ...
    const char*                            user,
    const char*                            pass);
void http_client_basic_args(http_client_t* hc, http_arg_list_t* h, const url_t* url, int keepalive);
...
static inline int rtsp_send(http_client_t* hc,
    http_cmd_t                             cmd,
    const char*                            path,
    const char*                            query,
    http_arg_list_t*                       hdr) {
...
static inline int rtsp_describe(http_client_t* hc, const char* path, const char* query) {
...
static inline int rtsp_play(http_client_t* hc, const char* path, const char* query) {
  return rtsp_send(hc, RTSP_CMD_PLAY, path, query, NULL);
}

static inline int rtsp_pause(http_client_t* hc, const char* path, const char* query) {
  return rtsp_send(hc, RTSP_CMD_PAUSE, path, query, NULL);
}

static inline int rtsp_teardown(http_client_t* hc, const char* path, const char* query) {
