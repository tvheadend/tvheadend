...
typedef int(
    http_callback_t)(http_connection_t* hc, const char* remain, void* opaque);

typedef char*(
    http_path_modify_t)(http_connection_t* hc, const char* path, int* cut);
...
struct http_client {
  ...
  unsigned int
               hc_handle_location : 1; /* handle the redirection (location) requests */
  ...
  /* callbacks */
  void (*hc_hdr_create)(http_client_t* hc,
      http_arg_list_t*                 h,
      const url_t*                     url,
      int                              keepalive);
