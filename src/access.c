...
static int  passwd_verify(access_t* a,
     const char*                    username,
...
static void access_ticket_rearm(void) {
  ...
  while ((at = TAILQ_FIRST(&access_tickets)) != NULL) {
    if (at->at_timer.mti_expire > mclk()) {
      mtimer_arm_abs(&at->at_timer,
          access_ticket_timeout,
          at,
          at->at_timer.mti_expire);
      break;
    }
 ...
 *
 */
static int netmask_verify(struct access_ipmask_queue* ais,
 ...
 *
 */
static void access_dump_tags(const char* prefix,
    char*                                buf,
    size_t                               buflen,
    size_t*                              _l,
    htsmsg_t*                            tags) {
  ...
  if (tags) {
    ...
    HTSMSG_FOREACH(f, tags) {
      channel_tag_t* ct =
          channel_tag_find_by_uuid(htsmsg_field_get_str(f) ?: "");
      if (ct) {
        if (first)
...
static void access_dump_a(access_t* a) {
  ...
  if (a->aa_dvrcfgs) {
    ...
    HTSMSG_FOREACH(f, a->aa_dvrcfgs) {
      dvr_config_t* cfg =
          dvr_config_find_by_uuid(htsmsg_field_get_str(f) ?: "");
      if (cfg) {
        if (first)
...
static void access_update(access_t* a, access_entry_t* ae) {
  ...
  if (ae->ae_change_chrange) {
    if (ae->ae_chmin || ae->ae_chmax) {
      uint64_t* p =
          realloc(a->aa_chrange, (a->aa_chrange_count + 2) * sizeof(uint64_t));
      if (p) {
    ...
    if (LIST_EMPTY(&ae->ae_profiles)) {
      ...
      LIST_FOREACH (ilm, &ae->ae_profiles, ilm_in1_link) {
        ...
        if (pro && pro->pro_name && pro->pro_name[0] != '\0') {
          ...
          htsmsg_add_str_exclusive(a->aa_profiles,
    ...
    if (LIST_EMPTY(&ae->ae_dvr_configs)) {
      ...
      LIST_FOREACH (ilm, &ae->ae_dvr_configs, ilm_in1_link) {
        ...
        if (dvr && dvr->dvr_config_name[0] != '\0') {
          ...
          htsmsg_add_str_exclusive(a->aa_dvrcfgs,
              idnode_uuid_as_str(&dvr->dvr_id, ubuf));
        }
      }
    }
  }
 ...
 *
 */
static void access_set_prefix(struct access_ipmask_queue* ais,
    const char*                                           prefix,
    int                                                   dflt) {
...
void access_entry_destroy(access_entry_t* ae, int delconf) {
  ...
  if (delconf)
    hts_settings_remove("accesscontrol/%s",
        idnode_uuid_as_str(&ae->ae_id, ubuf));
...
static void access_entry_class_get_title(idnode_t* self,
    const char*                                    lang,
    char*                                          buf,
    size_t                                         dstsize) {
  access_entry_t* ae = (access_entry_t*)self;
...
static htsmsg_t* access_entry_xmltv_output_format_enum(void* p,
    const char*                                              lang) {
...
static htsmsg_t* access_entry_class_streaming_enum(void* obj,
    const char*                                          lang) {
...
const idclass_t access_entry_class = {.ic_class = "access",
    ...
    .ic_properties                              = (const property_t[]){
        ...
        {.type    = PT_STR,
            ...
            .desc = N_("List of allowed IPv4 or IPv6 hosts or networks "
                       "(comma-separated)."),
            ...
            .opts = PO_ADVANCED},
        {
            ...
            .desc   = N_(
                "Specify the parameters to be changed. See Help for details."),
        ...
        },
        {
            ...
            .name   = N_("Streaming"),
            .desc   = N_(
                "Streaming flags, allow/disallow HTTP streaming, "
        ...
        },
        {
            ...
            .desc   = N_(
                "Video recorder flags, allow/disallow access to video recorder "
        ...
        },
        {
            ...
            .name   = N_("Channel tags"),
            .desc =
                N_("Channel tags the user is allowed access to/excluded from."),
...
static int passwd_verify2(const char* username,
    ...
    const char*                       username2,
    const char*                       passwd2) {
  if (username == NULL || username[0] == '\0' || username2 == NULL ||
      username2[0] == '\0' || passwd2 == NULL)
    return -1;
...
passwd_entry_class_save(idnode_t* self, char* filename, size_t fsize) {
  ...
  if (filename)
    snprintf(filename,
        fsize,
        "passwd/%s",
        idnode_uuid_as_str(&pw->pw_id, ubuf));
  return c;
}
...
static void passwd_entry_class_get_title(idnode_t* self,
    const char*                                    lang,
    char*                                          dst,
    size_t                                         dstsize) {
  passwd_entry_t* pw = (passwd_entry_t*)self;
...
ipblock_entry_class_save(idnode_t* self, char* filename, size_t fsize) {
  ...
  if (filename)
    snprintf(filename,
        fsize,
        "ipblock/%s",
        idnode_uuid_as_str(&ib->ib_id, ubuf));
  return c;
}

static void ipblock_entry_class_get_title(idnode_t* self,
    const char*                                     lang,
    char*                                           dst,
    size_t                                          dstsize) {
  ipblock_entry_t* ib = (ipblock_entry_t*)self;
