...
void comet_flush(void) {
  ...
  for (cmb = LIST_FIRST(&mailboxes); cmb != NULL; cmb = next) {
    ...
    if (cmb->cmb_refcount == 1 && cmb->cmb_last_used &&
        cmb->cmb_last_used + sec2mono(60) < mclk())
      cmb_destroy(cmb);
  }
 ...
 *
 */
static comet_mailbox_t* comet_find_mailbox(http_connection_t* hc,
    const char*                                               cometid,
    const char*                                               lang,
    int                                                       create) {
  ...
  if (http_access_verify(hc, ACCESS_ADMIN)) {
    if (!cmb->cmb_restricted) {
      ...
      tvh_mutex_unlock(&comet_mutex);
      comet_mailbox_add_logmsg(
          tvh_gettext_lang(lang, N_("Restricted log mode (no administrator)")),
          0,
          0);
      tvh_mutex_lock(&comet_mutex);
    }
 ...
 *
 */
static void comet_mailbox_ws_msg(http_connection_t* hc,
    comet_mailbox_t*                                cmb,
    int                                             first,
    htsmsg_t*                                       msg) {
  htsmsg_t* m = NULL;
