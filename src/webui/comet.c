 ...
 *
 */
static void cmb_destroy(comet_mailbox_t* cmb) {
  ...
  memoryinfo_free(&comet_memoryinfo,
      sizeof(*cmb) + (strlen(cmb->cmb_boxid) + 1) +
...
void comet_flush(void) {
  ...
  for (cmb = LIST_FIRST(&mailboxes); cmb != NULL; cmb = next) {
    ...
    if (cmb->cmb_refcount == 1 && cmb->cmb_last_used && cmb->cmb_last_used + sec2mono(60) < mclk())
      cmb_destroy(cmb);
  }
 ...
 *
 */
static comet_mailbox_t* comet_mailbox_create(const char* lang) {
  ...
  memoryinfo_alloc(&comet_memoryinfo,
      sizeof(*cmb) + (strlen(id) + 1) + (lang ? strlen(lang) + 1 : 0));
  return cmb;
}
 ...
 *
 */
static void comet_access_update(http_connection_t* hc, comet_mailbox_t* cmb) {
 ...
 *
 */
static void comet_serverIpPort(http_connection_t* hc, comet_mailbox_t* cmb) {
  char     buf[50];
  uint32_t port;
 ...
 *
 */
static htsmsg_t* comet_message(comet_mailbox_t* cmb, int include_boxid, int ignore_null) {
  htsmsg_t* m;
...
comet_find_mailbox(http_connection_t* hc, const char* cometid, const char* lang, int create) {
  ...
  if (http_access_verify(hc, ACCESS_ADMIN)) {
    if (!cmb->cmb_restricted) {
      ...
      tvh_mutex_unlock(&comet_mutex);
      comet_mailbox_add_logmsg(tvh_gettext_lang(lang, N_("Restricted log mode (no administrator)")),
          0,
          0);
      tvh_mutex_lock(&comet_mutex);
    }
 ...
 * Poll callback
 */
static int comet_mailbox_poll(http_connection_t* hc, const char* remain, void* opaque) {
  comet_mailbox_t* cmb       = NULL;
 ...
 * Poll callback
 */
static int comet_mailbox_dbg(http_connection_t* hc, const char* remain, void* opaque) {
  comet_mailbox_t* cmb     = NULL;
  const char*      cometid = http_arg_get(&hc->hc_req_args, "boxid");
  const char*      lang    = hc->hc_access->aa_lang_ui;
  const char*      s;
 ...
 * Websocket handler
 */
static int comet_mailbox_ws(http_connection_t* hc, const char* remain, void* opaque) {
  int              res, first = 1;
  htsmsg_t*        msg;
  const char*      cometid = http_arg_get(&hc->hc_req_args, "boxid");
  const char*      lang    = hc->hc_access->aa_lang_ui;
 ...
 *
 */
static void comet_mailbox_rewrite_str(htsmsg_t* m, const char* key, const char* lang) {
...
static void comet_mailbox_rewrite_title(htsmsg_t* m, const char* lang) {
  idnode_t*   in;
...
static void comet_mailbox_rewrite_msg(int rewrite, htsmsg_t* m, const char* lang) {
  switch (rewrite) {
