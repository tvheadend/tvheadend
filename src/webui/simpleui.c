...
static void dvr_query_add_entry(dvr_query_result_t* dqr, dvr_entry_t* de) {
  if (dqr->dqr_entries == dqr->dqr_alloced) {
    ...
    dqr->dqr_array =
        realloc(dqr->dqr_array, dqr->dqr_alloced * sizeof(dvr_entry_t*));
  }
 ...
 *
 */
static void dvr_query_sort_cmp(dvr_query_result_t* dqr,
    dvr_entry_comparator                           sf) {
 ...
 */
static int
page_simple(http_connection_t* hc, const char* remain, void* opaque) {
  ...
  if (s != NULL) {
    ...
    // Note: force min/max durations for this interface to 0 and INT_MAX seconds
    // respectively
...
static int page_einfo(http_connection_t* hc, const char* remain, void* opaque) {
  ...
  if (remain == NULL ||
      (e = epg_broadcast_find_by_id(strtoll(remain, NULL, 10))) == NULL) {
    tvh_mutex_unlock(&global_lock);
    return 404;
  }
...
page_status(http_connection_t* hc, const char* remain, void* opaque) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (DVR_SCHEDULED == de->de_sched_state) {
      timelefttemp =
          (int)((dvr_entry_get_start_time(de, 0) - now) / 60); // output minutes
 ...
 */
static int
page_epgsave(http_connection_t* hc, const char* remain, void* opaque) {
  htsbuf_queue_t* hq = &hc->hc_reply;
