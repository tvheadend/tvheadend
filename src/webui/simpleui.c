...
#define ACCESS_SIMPLE (ACCESS_WEB_INTERFACE | ACCESS_RECORDER)
...
static void dvr_query_add_entry(dvr_query_result_t* dqr, dvr_entry_t* de) {
  if (dqr->dqr_entries == dqr->dqr_alloced) {
    ...
    dqr->dqr_array   = realloc(dqr->dqr_array, dqr->dqr_alloced * sizeof(dvr_entry_t*));
  }
  dqr->dqr_array[dqr->dqr_entries++] = de;
}

static void dvr_query_filter(dvr_query_result_t* dqr, dvr_entry_filter filter) {
  dvr_entry_t* de;
 ...
 *
 */
static void dvr_query(dvr_query_result_t* dqr) {
 ...
 *
 */
static void dvr_query_free(dvr_query_result_t* dqr) {
  free(dqr->dqr_array);
}
 ...
 *
 */
static dvr_entry_t* dvr_find_by_event(epg_broadcast_t* e) {
  dvr_entry_t* de;
 ...
 *
 */
static void dvr_query_sort_cmp(dvr_query_result_t* dqr, dvr_entry_comparator sf) {
...
static void dvr_query_sort(dvr_query_result_t* dqr) {
 ...
 */
static int page_simple(http_connection_t* hc, const char* remain, void* opaque) {
  ...
  if (s != NULL) {
    ...
    if (eq.entries == 0) {
      ...
      for (k = 0; k < c; k++) {
        ...
        if (a.tm_wday != day.tm_wday || a.tm_mday != day.tm_mday || a.tm_mon != day.tm_mon ||
        ...
        rstatus = de != NULL ? val2str(de->de_sched_state, recstatustxt) : NULL;
 ...
 */
static int page_einfo(http_connection_t* hc, const char* remain, void* opaque) {
  htsbuf_queue_t*         hq = &hc->hc_reply;
  epg_broadcast_t*        e;
  struct tm               a, b;
  dvr_entry_t*            de;
  ...
  htsbuf_qprintf(hq, "<form method=\"post\" action=\"/eventinfo/%u\">", e->id);
 ...
 */
static int page_pvrinfo(http_connection_t* hc, const char* remain, void* opaque) {
  htsbuf_queue_t* hq = &hc->hc_reply;
  struct tm       a, b;
  dvr_entry_t*    de;
  const char*     rstatus;
  ...
  htsbuf_qprintf(hq,
      "<form method=\"post\" action=\"/pvrinfo/%s\">",
 ...
 *
 */
static int page_status(http_connection_t* hc, const char* remain, void* opaque) {
  htsbuf_queue_t*    hq = &hc->hc_reply;
 ...
 */
static int page_epgsave(http_connection_t* hc, const char* remain, void* opaque) {
  htsbuf_queue_t* hq = &hc->hc_reply;
