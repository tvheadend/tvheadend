...
http_stream_run (http_connection_t * hc, streaming_queue_t * sq)
{
  ...
  while (run)
      ...
      if (sm == NULL)
	{
	  ...
	  if (pthread_cond_timedwait (&sq->sq_cond, &sq->sq_mutex, &ts) ==
	      ETIMEDOUT)
	    {
	      int err = 0;
	...
	RB_FOREACH (ch, &channel_name_tree, ch_name_link)
	{
	  LIST_FOREACH (ctm, &ch->ch_ctms, ctm_channel_link)
	  {
	    if (ctm->ctm_tag->ct_identifier == ct->ct_identifier)
	      {
		htsbuf_qprintf (hq, "#EXTINF:-1,%s\n", ch->ch_name);
		snprintf (buf, sizeof (buf), "/stream/channelid/%d",
			  ch->ch_id);
		ticket_id = access_ticket_create (buf);
		htsbuf_qprintf (hq, "http://%s%s?ticket=%s\n", host, buf,
				ticket_id);
...
http_dvrs_playlist (http_connection_t * hc)
{
  ...
  LIST_FOREACH (de, &dvrentries, de_global_link) if (de)
    {
  ...
  for (fbe = fb->entries; fbe->filename != NULL; fbe++)
      ...
      if (!strcmp (fbe->filename, remain))
	{
	  ...
	  http_send_header (hc, 200, content, fbe->size,
			    fbe->original_size == -1 ? NULL : "gzip", NULL,
			    10, 0, NULL);
	  /* ignore return value */
