...
to reauthenticate."));
  ...
  htsbuf_qprintf(&hc->hc_reply,
      ...
      "<P STYLE=\"text-align: center; margin: 2em\"><A HREF=\"%s/\" "
      "STYLE=\"border: 1px solid; border-radius: 4px; padding: "
      ".6em\">%s</A></P>\r\n"
      "</BODY></HTML>\r\n",
      url,
      tvheadend_webroot ? tvheadend_webroot : "",
...
static void http_stream_run(http_connection_t* hc,
  ...
  while (!hc->hc_shutdown && run && tvheadend_is_running()) {
      ...
      do {
        ...
        if (r == ETIMEDOUT) {
          ...
          if (tcp_socket_dead(hc->hc_fd)) {
            tvhdebug(LS_WEBUI,
                "Stop streaming %s, client hung up",
                hc->hc_url_orig);
            run = 0;
          } else if ((!started && mclk() - lastpkt > sec2mono(grace)) ||
              (started && ptimeout > 0 &&
                  mclk() - lastpkt > sec2mono(ptimeout))) {
            tvhwarn(LS_WEBUI,
                "Stop streaming %s, timeout waiting for packets",
                hc->hc_url_orig);
            run = 0;
          }
          break;
    ...
    switch (sm->sm_type) {
      ...
      case SMT_STOP:
        if ((mux->m_caps & MC_CAP_ANOTHER_SERVICE) !=
            0) /* give a chance to use another svc */
          break;
    ...
    if (mux->m_errors) {
      if (!mux->m_eos)
        tvhwarn(LS_WEBUI,
            "Stop streaming %s, muxer reported errors",
            hc->hc_url_orig);
      run = 0;
    }
  }
 ...
 */
static int http_channel_playlist(http_connection_t* hc,
    int                                             pltype,
    int                                             urlauth,
    channel_t*                                      channel) {
  htsbuf_queue_t* hq;
  ...
  if (pltype == PLAYLIST_M3U) {
    ...
    http_satip_m3u_playlist_add(hq,
        hostpath,
        channel,
        blank,
        urlauth,
 ...
 */
static int http_tag_playlist(http_connection_t* hc,
    int                                         pltype,
    int                                         urlauth,
    channel_tag_t*                              tag) {
  htsbuf_queue_t* hq;
...
http_tag_list_playlist(http_connection_t* hc, int pltype, int urlauth) {
  ...
  blank = tvh_gettext_lang(lang, channel_blank_name);
  htsbuf_append_str(hq,
      pltype == PLAYLIST_E2 ? "#NAME Tvheadend Channels\n" : "#EXTM3U\n");
  for (idx = 0; idx < count; idx++) {
    ct = ctlist[idx];
    if (pltype == PLAYLIST_M3U) {
      snprintf(buf,
          sizeof(buf),
          "/playlist/tagid/%d",
          idnode_get_short_uuid(&ct->ct_id));
      http_m3u_playlist_add(hq,
          hostpath,
          buf,
          "playlist",
      ...
      htsbuf_qprintf(hq,
          "#SERVICE 1:64:%d:0:0:0:0:0:0:0::%s\n",
          labelidx++,
          ct->ct_name);
...
http_dvr_list_playlist(http_connection_t* hc, int pltype, int urlauth) {
  ...
  LIST_FOREACH (de, &dvrentries, de_global_link) {
    ...
    htsbuf_qprintf(hq,
        "#EXT-X-STREAM-INF:PROGRAM-ID=%s,BANDWIDTH=%d\n",
        uuid,
        bandwidth);
 ...
 */
static int http_dvr_playlist(http_connection_t* hc,
    int                                         pltype,
    int                                         urlauth,
    dvr_entry_t*                                de) {
 ...
 */
static int page_http_playlist_(http_connection_t* hc,
    const char*                                   remain,
    void*                                         opaque,
    int                                           urlauth) {
  ...
  if (!remain || *remain == '\0') {
    ...
    snprintf(buf,
        sizeof(buf),
        "/playlist%s/%s",
        page_playlist_authpath(urlauth),
        cs);
...
static int page_http_playlist_ticket(http_connection_t* hc,
    const char*                                         remain,
    void*                                               opaque) {
  return page_http_playlist_(hc, remain, opaque, URLAUTH_TICKET);
}

static int page_http_playlist_auth(http_connection_t* hc,
    const char*                                       remain,
    void*                                             opaque) {
...
udp_stream_channel(http_connection_t* hc, channel_t* ch, int weight) {
  ...
  if (!profile_chain_open(&ustream->us_prch, NULL, hints, 0, qsize)) {
    ...
    s = subscription_create_from_channel(&ustream->us_prch,
        ...
        NULL);
    if (s) {
      ustream->us_content_name =
          strdup(channel_get_name(ch, channel_blank_name));
      ustream->us_subscript   = s;
      ustream->us_global_lock = &global_lock;
      udp_stream_run(ustream);
      http_output_html(hc);
...
do_stream(http_connection_t* hc, const char* remain, void* opaque, int isUdp) {
  ...
  if (ch != NULL) {
    r = isUdp ? udp_stream_channel(hc, ch, weight)
              : http_stream_channel(hc, ch, weight);
  } else if (service != NULL) {
    r = isUdp ? udp_stream_service(hc, service, weight)
              : http_stream_service(hc, service, weight);
#if ENABLE_MPEGTS
  } else if (mm != NULL) {
...
stop_udp_stream(http_connection_t* hc, const char* remain, void* opaque) {
  ...
  if (us) {
    ...
    tvhwarn(LS_WEBUI,
        "UDP stream not found (stop request %s)",
        hc->hc_url_orig);
  }
 ...
 */
static int page_xspf(http_connection_t* hc,
    const char*                         remain,
    void*                               opaque,
    int                                 urlauth) {
  htsbuf_queue_t* hq = &hc->hc_reply;
...
static char* page_play_path_modify_(http_connection_t* hc,
    const char*                                        path,
    int*                                               cut,
    int                                                skip) {
  /*
...
static int page_play_(http_connection_t* hc,
    const char*                          remain,
    void*                                opaque,
    int                                  urlauth) {
  char* playlist;
  ...
  if (access_verify2(hc->hc_access,
          ACCESS_OR | ACCESS_STREAMING | ACCESS_ADVANCED_STREAMING |
              ACCESS_RECORDER))
    return http_noaccess_code(hc);
...
static uint32_t hdhomerun_get_deviceid(void) {
  ...
  const uint32_t deviceid =
      tvh_crc32((const uint8_t*)server_name, strlen(server_name), 0);
  return deviceid;
}
...
static const char* hdhomerun_get_model_name(void) {
  if (config.hdhomerun_server_model_name &&
      !strempty(config.hdhomerun_server_model_name))
 ...
 * this will require setting up an access rule without a username. but with IP
 * matching.
...
hdhomerun_verify_user_permission(const http_connection_t* hc,
  ...
  if (access_verify2(perm, ACCESS_STREAMING)) {
    ...
    tvhwarn(LS_WEBUI,
        "hdhomerun server received request but no streaming permission for "
        "user [%s] [%d] [%s]",
        hdhr_user ?: "<none>",
 ...
 */
static int hdhomerun_server_discover(http_connection_t* hc,
    const char*                                         remain,
    void*                                               opaque) {
 ...
 */
static int hdhomerun_server_lineup(http_connection_t* hc,
    const char*                                       remain,
    void*                                             opaque) {
...
static int hdhomerun_server_lineup_status(http_connection_t* hc,
    const char*                                              remain,
    void*                                                    opaque) {
  ...
  htsbuf_append_str(hq,
      "{\"ScanInProgress\":0,\"ScanPossible\":0,\"Source\":\"Antenna\","
      "\"SourceList\":[\"Antenna\"]}");
  http_output_content(hc, "application/json");
  return 0;
}
 ...
 */
static int hdhomerun_server_lineup_post(http_connection_t* hc,
    const char*                                            remain,
    void*                                                  opaque) {
  access_t* perm = hdhomerun_verify_user_permission(hc, "lineup_status");
  if (!perm)
 ...
 * but in xml format.
 */
static int hdhomerun_server_device_xml(http_connection_t* hc,
    const char*                                           remain,
    void*                                                 opaque) {
...
int http_serve_file(http_connection_t* hc,
    ...
    int                                fconv,
    const char*                        content,
    int (*preop)(http_connection_t* hc,
        off_t                       file_start,
        size_t                      content_len,
        void*                       opaque),
...
static int page_dvrfile_preop(http_connection_t* hc,
    off_t                                        file_start,
    size_t                                       content_len,
    void*                                        opaque) {
...
static int page_dvrfile_postop(http_connection_t* hc,
  ...
  tvhdebug(LS_HTTP,
      "page_dvrfile_postop: file start=%ld content len=%ld file size=%ld "
      "done=%d",
      (long)file_start,
      (long)content_len,
      (long)file_size,
...
page_imagecache(http_connection_t* hc, const char* remain, void* opaque) {
  ...
  if (access_verify2(hc->hc_access,
          ACCESS_OR | ACCESS_WEB_INTERFACE | ACCESS_STREAMING |
              ACCESS_ADVANCED_STREAMING | ACCESS_HTSP_STREAMING |
              ACCESS_HTSP_RECORDER | ACCESS_RECORDER))
    return http_noaccess_code(hc);
  ...
  if (nc == 1) {
    ...
    if (!strcmp(components[0], "theme.debug.css")) {
      ...
      if (theme) {
        snprintf(buf,
            sizeof(buf),
            "src/webui/static/extjs/resources/css/xtheme-%s.css",
            theme);
        if (!http_file_test(buf)) {
          snprintf(buf,
              sizeof(buf),
              "/static/extjs/resources/css/xtheme-%s.css",
              theme);
          http_css_import(hc, buf);
          return 0;
        }
...
void webui_init(int xspf) {
  ...
  http_path_add("/satip_server",
      NULL,
      satip_server_http_page,
      ACCESS_ANONYMOUS);
#endif

#if ENABLE_HDHOMERUN_SERVER
  /* These names are specified in https://info.hdhomerun.com/info/http_api */
  http_path_add("/discover.json",
      NULL,
      hdhomerun_server_discover,
      ACCESS_ANONYMOUS);
  http_path_add("/lineup.json",
      NULL,
      hdhomerun_server_lineup,
      ACCESS_ANONYMOUS);
  /* These names are not specified in the documents but are required to make
   * Plex work. */
  http_path_add("/lineup_status.json",
      NULL,
      hdhomerun_server_lineup_status,
      ACCESS_ANONYMOUS);
  http_path_add("/lineup.post",
      NULL,
      hdhomerun_server_lineup_post,
      ACCESS_ANONYMOUS);
  http_path_add("/device.xml",
      NULL,
      hdhomerun_server_device_xml,
      ACCESS_ANONYMOUS);
#endif

  http_path_add_modify("/play",
      NULL,
      page_play,
      ACCESS_ANONYMOUS,
      page_play_path_modify5);
  ...
  http_path_add("/playlist/ticket",
      NULL,
      page_http_playlist_ticket,
      ACCESS_ANONYMOUS);
  http_path_add("/playlist/auth",
      NULL,
