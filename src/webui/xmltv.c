...
static void http_xmltv_channel_add(http_connection_t* hc,
  ...
  htsbuf_append_and_escape_xml(hq,
      http_xmltv_channel_get_name(hc, ch, ubuf, sizeof ubuf));
  htsbuf_qprintf(hq, "\">\n  <display-name>");
...
/// Write each entry in the string list to the xml queue using the given
/// tag_name.
...
static void http_xmltv_programme_one_long(const http_connection_t* hc,
  ...
  if (ebc->subtitle)
    RB_FOREACH (lse, ebc->subtitle, link) {
      ...
      if (!strempty(lse->str)) {
        HTTP_XMLTV_OUTPUT_START_TAG_WITH_LANG(hq,
            ebc->subtitle,
            lse,
            "sub-title");
...
static void http_xmltv_programme_one(const http_connection_t* hc,
  ...
  htsbuf_qprintf(hq,
      "<programme start=\"%s\" stop=\"%s\" channel=\"",
      start,
      stop);
  htsbuf_append_and_escape_xml(hq,
      http_xmltv_channel_get_name(hc, ch, ubuf, sizeof ubuf));
