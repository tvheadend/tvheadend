...
static int md_class(htsbuf_queue_t* hq,
    const char*                     clazz,
    const char*                     lang,
    int                             hdr,
    int                             docs,
...
/* */
static void md_text(htsbuf_queue_t* hq,
    const char*                     first,
    const char*                     next,
    const char*                     text) {
  char *s, *t, *p;
...
static int md_props(htsbuf_queue_t* hq, htsmsg_t* m, const char* lang, int nl) {
  ...
  HTSMSG_FOREACH(f, l) {
    ...
    if (first) {
      ...
      md_nl(hq, 1);
      md_nl(hq, 1);
      // htsbuf_append_str(hq, tvh_gettext_lang(lang, N_("The items have the
      // following functions:"))); md_nl(hq, 1);
      first = 0;
    }
    nl = md_nl(hq, nl);
    md_style(hq, "**", s);
 ...
 *
 */
static int http_markdown_page(http_connection_t* hc,
    const struct tvh_doc_page*                   page) {
