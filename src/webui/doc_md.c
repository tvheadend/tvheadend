...
static int
md_class(htsbuf_queue_t* hq, const char* clazz, const char* lang, int hdr, int docs, int props);

/* */
static int md_nl(htsbuf_queue_t* hq, int nl) {
  if (nl)
    htsbuf_append(hq, "\n", 1);
  return 1;
}
...
/* */
static void md_header(htsbuf_queue_t* hq, const char* prefix, const char* s) {
...
/* */
static void md_style(htsbuf_queue_t* hq, const char* style, const char* s) {
...
/* */
static void md_text(htsbuf_queue_t* hq, const char* first, const char* next, const char* text) {
  char *s, *t, *p;
  int   col, nl;
...
/* */
static int md_props(htsbuf_queue_t* hq, htsmsg_t* m, const char* lang, int nl) {
  htsmsg_t *      l, *n, *e, *x;
  htsmsg_field_t *f, *f2;
  const char*     s;
  int             first = 1, b;
  ...
  HTSMSG_FOREACH(f, l) {
    ...
    if (first) {
      ...
      md_nl(hq, 1);
      md_nl(hq, 1);
      // htsbuf_append_str(hq, tvh_gettext_lang(lang, N_("The items have the following
      // functions:"))); md_nl(hq, 1);
      first = 0;
    }
    nl = md_nl(hq, nl);
    md_style(hq, "**", s);
...
/* */
static void md_render(htsbuf_queue_t* hq, const char* doc, const char* lang) {
 ...
 */
static int http_markdown_classes(http_connection_t* hc) {
 ...
 *
 */
static int http_markdown_class(http_connection_t* hc, const char* clazz) {
 ...
 *
 */
static int http_markdown_page(http_connection_t* hc, const struct tvh_doc_page* page) {
