...
extjs_dvbadapter(http_connection_t *hc, const char *remain, void *opaque)
{
  ...
  if(!strcmp(op, "load")) {
    ...
    htsmsg_add_u32(r, "include_unavailable_services", tda->tda_include_unavailable_services);
    ...
    s = http_arg_get(&hc->hc_req_args, "include_unavailable_services");
    dvb_adapter_set_include_unavailable_services(tda, !!s);
