...
tvheadend.dvrDetails = function(uuid) {
    ...
    function showit(d) {
        ...
        if (start_real)
          content += '<div class="x-epg-time-start"><div class="x-epg-time"><div class="x-epg-prefix">' + _('Scheduled Start Time') + ':</div> ' + tvheadend.niceDate(start_real * 1000) + '</div></div>';
        if (stop_real)
          content += '<div class="x-epg-time-stop"><div class="x-epg-time"><div class="x-epg-prefix">' + _('Scheduled Stop Time') + ':</div> ' + tvheadend.niceDate(stop_real * 1000) + '</div></div>';
        if (duration)
