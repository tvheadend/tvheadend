...
export interface IdnodeProp {
   ...
   * fieldsets. Matches a `number` in the class's `meta.groups`
   * array. Read by inline-form views that group fields by `.group`;
   * the per-row drawer editor groups by level
   * (basic/advanced/expert/readonly) and ignores this. */
  group?: number

  /* The field must always carry a value — no clear-to-null
   * affordance. Drives `IdnodeFieldEnum`'s suppression of the
   * synthetic `(none)` option for str-typed enum dropdowns
   * (language_ui / theme_ui and similar config singletons whose
   * runtime defaults are always set).
   *
   * Not emitted by the server today. Synthesised on the client by
   * `IdnodeConfigForm`'s `mandatoryFields` allowlist; will be
   * replaced by a server-emitted `mandatory: true` once a
   * `PO_MANDATORY` prop-opt flag lands C-side. */
  mandatory?: boolean
}
