...
import { registerShortcut } from '@/composables/useGlobalShortcuts'
...
/* Singleton drill-down editor — driven by `DrillDownCell`
 * chevrons on grid cells with a UUID companion. Same overlay
 * shape as the DVR editor above but class-agnostic: the
 * IdnodeEditor calls `api/idnode/load` which resolves the
 * class for any UUID, so no per-class `list` restriction.
 * Route-change auto-close lives in the composable. */
const {
  editingUuid: drillUuid,
  pickerRows: drillPickerRows,
  pickerColumns: drillPickerColumns,
  pickerTitle: drillPickerTitle,
  close: closeDrillEditor,
} = useEntityEditor()
/* Picker-mode row switch — move the editor to the picked entity
 * without disturbing the picker table (which `open()` would clear).
 * IdnodeEditor's `pick` already ran the dirty-discard confirm. */
function pickDrillEntry(uuid: string) {
  drillUuid.value = uuid
}
...
/*
 * Global command-palette shortcuts. Cmd-K (Mac) / Ctrl-K
 * (Win/Linux) toggle the palette; '/' opens it but only when
 * focus isn't in an input (otherwise it would eat the user's
 * typing). Registered on mount, cleaned up on unmount — AppShell
 * never unmounts in practice so cleanup never runs, but the
 * pairing keeps the lifecycle honest if AppShell is ever re-mounted
 * for testing or HMR.
 */
const commandPalette = useCommandPalette()
let unregisterCmdK: (() => void) | null = null
let unregisterSlash: (() => void) | null = null

onMounted(() => {
  unregisterCmdK = registerShortcut(
    { key: 'k', eitherMetaOrCtrl: true },
    () => commandPalette.toggle(),
  )
  unregisterSlash = registerShortcut(
    { key: '/', ignoreInEditable: true },
    () => commandPalette.open(),
  )
})
...
<template>
  ...
  <div class="app-shell">
    ...
    <!-- Singleton error dialog. Save failures across the app
         (IdnodeEditor drawer, in-grid inline edit, batch ops) pop
         this modal via `useErrorDialog().show(...)` — gives
         server-side validation errors enough screen real estate
         to read and a clear OK-to-dismiss contract that a toast
         can't provide. -->
    <ErrorDialog />
    <!-- Singleton in-app help modal. Mounted once at AppShell
         level so every non-wizard route has it available; the
         IdnodeGrid `helpPage` prop opens it via the
         `useHelp().toggle(page)` composable. The wizard has its
         own surface (`WizardHelpDock`) and bypasses this. -->
    <HelpDialog />
    <!-- Singleton command palette. Cmd-K / Ctrl-K / `/` from
         anywhere outside a text input toggle it open. The NavRail
         and TopBar carry visible triggers for pointer / touch
         users; the dialog itself lives here so the shortcut works
         on every route. -->
    <CommandPalette />
    <!-- Singleton in-browser video player. Opened via
         `useVideoPlayer().open(...)` from the EPG event drawer's
         "Play in browser" action; mounted here so it survives
         the drawer closing. -->
    <VideoPlayerDialog />
    ...
    />
    <!-- Singleton drill-down editor. Independent IdnodeEditor
         instance driven by useEntityEditor: opens whichever
         class the UUID points at (server resolves), with no
         `list` restriction so every property the user has
         access to is rendered. In picker mode it also shows a
         single-select table of the passed entities above the
         form (Home dashboard "This week's recordings"). -->
    <IdnodeEditor
      :uuid="drillUuid"
      :level="access.uilevel"
      :title="drillPickerTitle ?? undefined"
      :picker-rows="drillPickerRows"
      :picker-columns="drillPickerColumns"
      @pick="pickDrillEntry"
      @close="closeDrillEditor"
    />
  </div>
...
.app-shell {
  ...
  /* `100vh` is the fallback for browsers that don't support
   * `100dvh`. On iOS Safari (and modern Chrome / Firefox)
   * `vh` resolves to the LARGE viewport, which overshoots the
   * visible area whenever the URL bar is still on-screen — the
   * shell would overflow at the bottom and the TopBar's pinned
   * position could be dragged out of view by page-level scroll.
   * `dvh` tracks the dynamic viewport so the shell exactly fits
   * what the user can see. Same pattern PhoneSortPopover uses. */
  height: 100vh;
  height: 100dvh;
  overflow: hidden;
...
.app-shell__main {
  flex: 1;
  overflow: auto;
  /* Tighter top padding (was --tvh-space-6 = 24px). Pages with
   * a PageTabs L2 nav at the top sat ~36 px below the TopBar
   * before the first tab-text baseline (24 main-top + 12 tab-
   * top); admins felt this wasted vertical real estate that
   * could go to grid rows. Side + bottom keep the 24 px
   * breathing room from NavRail and the page bottom. */
  padding: var(--tvh-space-2) var(--tvh-space-6) var(--tvh-space-6);
}
