...
describe('IntersectionObserver integration', () => {
  it('events not yet visible are skipped by applyVisible', () => {
    ...
    allocator.applyVisible({ scrollTop: 0, clientHeight: 600 })
...
describe('allocator inputs', () => {
  ...
  it('pins box at scrollTop with shrunk height when scrolled past start', () => {
    ...
    const io = MockIntersectionObserver.lastInstance()!
    io.simulate([{ target: el, isIntersecting: true }])
    allocator.applyVisible({ scrollTop: 100, clientHeight: 600 })
  ...
  it('pinHeight clamps to 0 when block scrolls fully past viewport', () => {
    ...
    const io = MockIntersectionObserver.lastInstance()!
    io.simulate([{ target: el, isIntersecting: true }])
    allocator.applyVisible({ scrollTop: 400, clientHeight: 600 })
...
describe('memoization', () => {
  it('second applyAllocator call with unchanged inputs skips setProperty', () => {
    ...
    io.simulate([{ target: el, isIntersecting: true }])
    allocator._internals.resetSetPropertyCount()
    /* Use a different scrollTop than the harness default so the
     * tick-level early-out doesn't short-circuit before the
     * per-event memo fires. The bind path's initial apply read
     * the harness's default 0; force a different snapshot here
     * so the tick branch runs and the per-event memo gates the
     * write (the actual contract under test). */
    allocator.applyVisible({ scrollTop: 1, clientHeight: 600 })
    /* Inputs unchanged → per-event memo hit → no setProperty calls. */
    expect(allocator._internals.setPropertyCount()).toBe(0)
    unmount()
  })
