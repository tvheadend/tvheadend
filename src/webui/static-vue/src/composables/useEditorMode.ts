...
export interface EditorModeHandle {
  /** Non-null while the Edit drawer is open in single-row mode. */
  editingUuid: Ref<string | null>
  /**
   * Non-null while the Edit drawer is open in MULTI-edit mode
   * (selection length >= 2). The IdnodeEditor renders its
   * apply-checkbox-per-field UI when this is set; only ticked
   * fields land in the save payload (the same Case-2 shape
   * `idnode/save` accepts at `src/api/api_idnode.c:408-429`).
   * Mutually exclusive with `editingUuid` — `openEditor`
   * normalises a single-row selection into `editingUuid`, never
   * `editingUuids: [single]`, so the editor's mode dispatch
   * stays unambiguous.
   */
  editingUuids: Ref<string[] | null>
  ...
  /**
   * Open the Edit drawer for the current selection. Branches on
   * cardinality:
   *   - 0 rows  → no-op.
   *   - 1 row   → sets `editingUuid` (single-edit, unchanged
   *               behaviour from before multi-edit shipped).
   *   - 2+ rows → sets `editingUuids` (multi-edit; the editor
   *               renders apply-checkboxes per field and saves
   *               via Case 2 of `idnode/save`).
   * Rows without a string `uuid` are dropped from the
   * multi-edit list; if all rows lack a uuid the call is a
   * no-op.
   */
...
export function useEditorMode(opts: EditorModeOptions): EditorModeHandle {
  ...
  const editingUuids = ref<string[] | null>(null)
  ...
  function openEditor(selected: BaseRow[]) {
    ...
    /* 2+ rows: multi-edit. Collect string uuids defensively —
     * the grid model always emits string uuids on idnode rows
     * but a future row shape (or test fixture) might not, and a
     * silent drop keeps the action semantics tight. */
    const uuids = selected
      .map((r) => r.uuid)
      .filter((u): u is string => typeof u === 'string' && !!u)
    if (uuids.length >= 2) editingUuids.value = uuids
  }
