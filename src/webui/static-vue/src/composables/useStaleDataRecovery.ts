 ...
 * the other the zombie-WebSocket case where the socket was alive but
 * dead and only `onclose` fires when the kernel notices. A caller
...
export function useStaleDataRecovery(options: StaleDataRecoveryOptions): void {
   ...
   * reconnects the WebSocket and replays buffered notifications via
   * boxid resume, but the server's mailbox can be GC'd during a long
