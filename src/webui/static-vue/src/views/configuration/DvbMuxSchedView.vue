 ...
 * widget; not worth replicating here.
 */
import { ref } from 'vue'
...
/* Phone-card: parent mux name as bold headline; enabled + cron
 * as the 2-up row (cron pattern is short, fits 50% column).
 * timeout / restart stay desktop-only — niche scheduler knobs. */
const cols: ColumnDef[] = [
  ...
  },
  {
     ...
     * AppShell drill-down drawer. Wire value IS the UUID;
     * point `targetUuidField` at the same field. */
    targetUuidField: 'mux',
...
const remove = useBulkAction({
  ...
  confirmText: t('Do you really want to delete the selected mux schedulers?'),
...
function buildActions(selection: BaseRow[], clearSelection: () => void): ActionDef[] {
  return buildAddEditDeleteActions({
    ...
    onAdd: () => openCreate(),
    onEdit: openEditor,
    addTooltip: t('Add a new mux scheduler'),
  })
}
</script>
  ...
  >
    <template #empty>
      <p class="dvb-mux-sched__empty">
        {{ t('No mux schedulers defined. Add one to drive periodic mux tuning via a cron expression.') }}
      </p>
    </template>
