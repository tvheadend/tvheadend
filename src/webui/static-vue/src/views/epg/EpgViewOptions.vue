...
const tooltipOptions: { value: TooltipMode; label: string }[] = [
  ...
  { value: 'always', label: t('All') },
  { value: 'short', label: t('Short events') },
]

const densityOptions: { value: Density; label: string }[] = [
  { value: 'minuscule', label: t('Minuscule') },
...
const overlayOptions: { value: DvrOverlayMode; label: string }[] = [
  { value: 'off', label: t('Hidden') },
  { value: 'event', label: t('Event slot only') },
  { value: 'padded', label: t('Event slot + padding') },
]
...
function setChannelSort(channelSort: ChannelSort) {
  emit('update:options', { ...props.options, channelSort })
}
...
function setDarkChannelBackground(darkChannelBackground: boolean) {
  emit('update:options', { ...props.options, darkChannelBackground })
}

/*
 * Per-section accordion drivers — `isDefault` controls the accent
 * chip + auto-open priority (when auto-open lands; today it's
 * collapse-all), `summary` is the right-aligned chip text shown
 * when collapsed.
 */
const tagsIsDefault = computed(
  () => props.options.tagFilter.tag === props.defaults.tagFilter.tag,
)

const tagsSummary = computed(() => {
  const uuid = props.options.tagFilter.tag
  if (uuid === null) return ''
  const match = props.tags.find((tag) => tag.uuid === uuid)
  return match?.name ?? uuid
})

const channelIsDefault = computed(
  () =>
    props.options.channelDisplay.logo === props.defaults.channelDisplay.logo &&
    props.options.channelDisplay.name === props.defaults.channelDisplay.name &&
    props.options.channelDisplay.number ===
      props.defaults.channelDisplay.number &&
    props.options.channelSort === props.defaults.channelSort,
)

/* Summary: display labels only (Logo / Name / Number), comma-
 * joined in canonical order. Sort key isn't shown in the chip
 * per the design — the accent colour signals "non-default" and
 * the user expands to see the full state including sort. */
const channelSummary = computed(() => {
  const parts: string[] = []
  if (props.options.channelDisplay.logo) parts.push(t('Logo'))
  if (props.options.channelDisplay.name) parts.push(t('Name'))
  if (props.options.channelDisplay.number) parts.push(t('Number'))
  return parts.join(', ')
})

const tooltipsIsDefault = computed(
  () => props.options.tooltipMode === props.defaults.tooltipMode,
)
const tooltipsSummary = computed(
  () =>
    tooltipOptions.find((o) => o.value === props.options.tooltipMode)?.label ??
    '',
)

const densityIsDefault = computed(
  () =>
    props.options.density[props.currentView] ===
    props.defaults.density[props.currentView],
)
const densitySummary = computed(
  () =>
    densityOptions.find((o) => o.value === props.options.density[props.currentView])
      ?.label ?? '',
)

const overlayIsDefault = computed(
  () =>
    props.options.dvrOverlay === props.defaults.dvrOverlay &&
    props.options.dvrOverlayShowDisabled ===
      props.defaults.dvrOverlayShowDisabled,
)
const overlaySummary = computed(
  () =>
    overlayOptions.find((o) => o.value === props.options.dvrOverlay)?.label ?? '',
)

const layoutIsDefault = computed(
  () =>
    props.options.stickyTitles === props.defaults.stickyTitles &&
    props.options.darkChannelBackground === props.defaults.darkChannelBackground,
)
/* Comma-joined list of enabled sub-toggles so the section header
 * conveys what's on at a glance. Falls back to "Off" when no
 * sub-toggle is enabled. */
const layoutSummary = computed(() => {
  const on: string[] = []
  if (props.options.stickyTitles) on.push(t('Sticky titles'))
  if (props.options.darkChannelBackground) on.push(t('Dark channels'))
  return on.length > 0 ? on.join(' · ') : t('Off')
})

const defaultsActive = computed(
  () =>
    tagsIsDefault.value &&
    channelIsDefault.value &&
    tooltipsIsDefault.value &&
    densityIsDefault.value &&
    overlayIsDefault.value &&
    layoutIsDefault.value &&
    /* Table-only fields still belong to the shared EpgViewOptions
     * shape; Reset on this popover preserves them anyway, so they
     * don't influence whether Reset is enabled here. */
...
function resetToDefaults() {
  ...
  emit('update:options', {
    ...
    channelSort: props.defaults.channelSort,
    ...
    darkChannelBackground: props.defaults.darkChannelBackground,
    /* Timeline / Magazine don't expose Table-only column toggles,
     * but the field still belongs on every EpgViewOptions write
     * because the type is shared. Preserve whatever the user set
     * via the Table popover so a Reset here doesn't wipe Table
     * column prefs. */
    columnVisibility: { ...props.options.columnVisibility },
    /* Timeline / Magazine don't surface grouping either —
     * preserve the Table popover's groupField / groupOrder for
     * the same reason. Reset here only affects what THIS view
     * (Timeline / Magazine) controls. */
    groupField: props.options.groupField,
    groupOrder: props.options.groupOrder,
    /* Title-search mode is Table-only; preserve the user's
     * persisted choice through a Reset triggered from the
     * Timeline / Magazine popover. */
    titleSearchMode: props.options.titleSearchMode,
    /* Time window is Table-only; same preservation rule. */
    timeWindow: props.options.timeWindow,
    /* GLOBAL filter axes — all Table-only, all preserved
     * across a Timeline / Magazine reset. */
    genre: props.options.genre,
    newOnly: props.options.newOnly,
    durationMinMinutes: props.options.durationMinMinutes,
    durationMaxMinutes: props.options.durationMaxMinutes,
  })
}
</script>
...
<template>
  <div class="epg-view-options">
    <SettingsPopover :defaults-active="defaultsActive" @reset="resetToDefaults">
      ...
      <!-- Channel section (merged display + order). Phone media
           queries in EpgTimeline / EpgMagazine force channel cells
           to logo-only at narrow widths, so the toggles do nothing
           on phone — hide the whole section there. -->
      <CollapsibleSection
        v-if="!isPhone"
        id="channel"
        :title="t('Channel')"
        :is-default="channelIsDefault"
      ...
      >
        ...
        <!-- Sort key sits inside the merged Channel section. Default
             unchecked = sort by number (matches legacy UI); the user
             can hide the LCN while still sorting by it (or vice
             versa) by leaving this independent of the display flags. -->
        <label class="settings-popover__option">
          <input
            type="checkbox"
            class="settings-popover__checkbox"
            :checked="options.channelSort === 'name'"
            @change="
              setChannelSort(
                ($event.target as HTMLInputElement).checked ? 'name' : 'number',
              )
            "
          />
      ...
      <!-- Tooltips on event blocks: hover-capability-based gate.
           ...
           has no effect, so hide it. -->
      <CollapsibleSection
        v-if="!noHover"
        id="tooltips"
        :title="t('Event tooltips')"
        :is-default="tooltipsIsDefault"
      ...
      >
        ...
        >
          ...
          {{ opt.label }}
          <!-- Inline legend: same colour tokens + half-opacity rule
               as DvrOverlayBar.vue so the legend reads identically to
               the live bar. -->
          <span
            v-if="opt.value !== 'off'"
        ...
        >
          ...
          />
          {{ t('Show disabled entries') }}
        </label>
      </CollapsibleSection>
      <CollapsibleSection
        id="layout"
        :title="t('Layout')"
        :is-default="layoutIsDefault"
      ...
      >
        <label class="settings-popover__option">
          ...
          />
          {{ t('Pin event titles when scrolling past') }}
        </label>
        <label class="settings-popover__option">
          <input
            type="checkbox"
            class="settings-popover__checkbox"
            :checked="options.darkChannelBackground"
            @change="setDarkChannelBackground(($event.target as HTMLInputElement).checked)"
          />
          {{ t('Dark channel background') }}
        </label>
      </CollapsibleSection>
    </SettingsPopover>
  </div>
