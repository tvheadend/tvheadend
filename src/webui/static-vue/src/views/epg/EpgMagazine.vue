 ...
 *
 *   ┌──────────┬──────────┬──────────┬──────────┐
 *   │   Time   │  Ch 1    │  Ch 2    │  Ch 3    │  ← header row
 *   │          │  [logo]  │  [logo]  │  [logo]  │    (sticky-top)
 *   ├──────────┼──────────┼──────────┼──────────┤
 *   │   06:00  │  Show A  │  Show B  │  Show C  │
 *   │          │          │          │          │
 *   │   07:00  │  Show D  │  Show E  │  Show F  │
 *   │          │          │          │          │
...
import type { ViewportScrollState } from '@/composables/useEpgViewportEmitter'
...
interface Props {
  ...
  /*
   * When true, the sticky channel-header row renders on a dark
   * surface with light text so white-on-transparent channel
   * logos stay readable on any theme. Off by default; toggled
   * via the EPG view-options popover's Layout section.
   */
...
const dvrByChannel = computed(() => {
  ...
  for (const e of props.dvrEntries) {
     ...
     * seeing them via the view-options toggle. */
    if (!props.dvrOverlayShowDisabled && !e.enabled) continue
...
/* DOM virtualisation — only mount channel columns whose
 * x-range intersects the visible viewport (plus an overscan
 * buffer), and per visible column only mount events whose
 * time-range overlaps the visible time window (also with
 * overscan). Without this, Safari (desktop + iPhone) takes
 * minutes to render the initial page on a typical 14-day
 * load with 5 000-10 000 absolutely-positioned event
 * elements. The rAF emitter pumps `ViewportScrollState` into
 * `viewportState`; the visible-range computeds derive from
 * that. */
const OVERSCAN_COLS = 5
const OVERSCAN_PX = 200

const viewportState = ref<ViewportScrollState>({
  scrollPos: 0,
  clientExtent: 0,
  rawClientExtent: 0,
  crossScrollPos: 0,
...
/* Cross-axis (horizontal) drives column virtualisation. The
 * sticky time-axis on the left consumes `AXIS_WIDTH` pixels
 * of `crossClientExtent`, so subtract it before deriving the
 * visible-column window — otherwise the rightmost overscan
 * extends past the actual visible columns. */
const visibleColumnRange = computed(() =>
  computeVisibleIndexRange(
    viewportState.value.crossScrollPos,
    Math.max(0, viewportState.value.crossClientExtent - AXIS_WIDTH),
    props.channelColumnWidth,
...
const visibleChannels = computed(() =>
  props.channels.slice(visibleColumnRange.value.startIdx, visibleColumnRange.value.endIdx),
)

const leftSpacerPx = computed(
  () => visibleColumnRange.value.startIdx * props.channelColumnWidth,
)
const rightSpacerPx = computed(
  () => (props.channels.length - visibleColumnRange.value.endIdx) * props.channelColumnWidth,
)

/* Visible time range in seconds, with overscan converted from
 * pixels via `pxPerMinute`. Anchored at `effectiveStart`. */
const visibleTimeRange = computed(() => {
  const ppm = props.pxPerMinute
  if (ppm <= 0) return { start: effectiveStart.value, end: effectiveEnd.value }
  const visiblePx = viewportState.value.scrollPos
  const extentPx = viewportState.value.clientExtent
  const startSec = effectiveStart.value + ((visiblePx - OVERSCAN_PX) / ppm) * 60
  const endSec = effectiveStart.value + ((visiblePx + extentPx + OVERSCAN_PX) / ppm) * 60
  return { start: startSec, end: endSec }
})

const visibleEventsByChannel = computed<Map<string, MagazineEvent[]>>(() => {
  const out = new Map<string, MagazineEvent[]>()
  const range = visibleTimeRange.value
  for (const ch of visibleChannels.value) {
    const bucket = eventsByChannel.value.get(ch.uuid)
    if (!bucket) continue
    out.set(
      ch.uuid,
      bucket.filter((ev) => eventOverlapsTimeRange(ev, range.start, range.end)),
    )
  }
  return out
})
...
useEpgViewportEmitter({
  ...
  onTick: (state) => {
     ...
     * events using the scroll snapshot the emitter just read.
     * Threading scrollTop / clientHeight through means the
     * per-event loop never re-reads layout properties (each
     * read on iPhone forces a reflow when interleaved with the
     * style writes). Also publish the scroll state to the
     * reactive `viewportState` ref that drives column +
     * per-column event virtualisation — single tick reads
     * both axes once and feeds everything that needs scroll
     * snapshots. */
    viewportState.value = state
    allocator.applyVisible({
      scrollTop: state.scrollPos,
      clientHeight: state.rawClientExtent,
    })
  },
})
...
<template>
  <div
    ...
    :class="{
      ...
      'epg-magazine--dark-channels': darkChannelBackground,
    }"
    :style="{
  ...
  >
    <!-- Header row — sticky-top, holds corner + channel headers.
         Only headers for currently-visible columns are mounted;
         left + right spacer divs preserve the row's full width
         so the header stays in lock-step with the body's
         virtualised columns. -->
    <div class="epg-magazine__header-row">
      <div class="epg-magazine__corner">
        <span class="epg-magazine__corner-label">{{ t('Time') }}</span>
      </div>
      <div
        v-if="leftSpacerPx > 0"
        class="epg-magazine__column-spacer"
    ...
    <div class="epg-magazine__body">
      ...
      <div class="epg-magazine__columns">
        <div
          ...
          aria-hidden="true"
        />
        <div v-for="ch in visibleChannels" :key="ch.uuid" class="epg-magazine__column">
...
.epg-magazine__channel-icon {
  ...
  height: 32px;
  object-fit: contain;
  /* No background — see EpgTimeline.vue's matching note. PNGs
   * with transparency would otherwise show a coloured rectangle
   * behind the logo. */
...
/*
 * Dark-channel-background variant — opt-in via the EPG view
 * options popover (Layout → "Dark channel background"). Tints
 * the sticky channel-header row to a fixed dark surface with
 * light text so white-on-transparent channel logos stay readable
 * on every theme. The top-left corner cell stays at the default
 * surface so the visual seam between corner and channel strip
 * is preserved (matches the user's chosen scope — header row
 * only, not the corner).
 *
 * Dark colour hardcoded for the same reason as Timeline's
 * variant: a theme-relative token would defeat the purpose on
 * Light theme.
 */
.epg-magazine--dark-channels .epg-magazine__channel-header {
  background: #1a1a1a;
  color: #fff;
}

.epg-magazine--dark-channels .epg-magazine__channel-name,
.epg-magazine--dark-channels .epg-magazine__channel-number {
  color: #fff;
}

.epg-magazine--dark-channels .epg-magazine__channel-header:hover {
  background: color-mix(in srgb, #fff var(--tvh-hover-strength), #1a1a1a);
}
</style>
