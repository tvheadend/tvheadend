...
interface Props {
  ...
  /*
   * When true, the sticky channel column renders on a dark surface
   * with light text so white-on-transparent channel logos stay
   * readable on any theme. Off by default; toggled via the EPG
   * view-options popover's Layout section.
   */
...
/* DOM virtualisation — only mount channel rows whose y-range
 * intersects the visible viewport (plus an overscan buffer),
 * and per visible row only mount events whose time-range
 * overlaps the visible time window (also with overscan).
 * Without this, Safari (desktop + iPhone) takes minutes to
 * render the initial page on a typical 14-day load with
 * 5 000-10 000 absolutely-positioned event elements; Firefox
 * handles it but the cost is real on every browser.
 *
 * The rAF emitter pumps `ViewportScrollState` into
 * `viewportState`; the visible-range computeds derive from
 * that. Defaults to a zero-shape on first paint — the
 * initial-tick fires within one frame of mount so any flash
 * is invisible. */
...
const visibleChannelRange = computed(() =>
  computeVisibleIndexRange(
    ...
    viewportState.value.crossClientExtent,
    props.rowHeight,
...
const visibleChannels = computed(() =>
  props.channels.slice(visibleChannelRange.value.startIdx, visibleChannelRange.value.endIdx),
)

const topSpacerPx = computed(() => visibleChannelRange.value.startIdx * props.rowHeight)
const bottomSpacerPx = computed(
  () => (props.channels.length - visibleChannelRange.value.endIdx) * props.rowHeight,
)
...
/* Per visible channel, return only the events that overlap
 * the visible time range. Pre-bucketed events come from
 * `eventsByChannel` (one O(N) bucket pass on data change);
 * this computed re-runs only on viewport / channel-list /
 * events changes. */
const visibleEventsByChannel = computed<Map<string, TimelineEvent[]>>(() => {
...
useEpgViewportEmitter({
  ...
  /* Tick the pin with the emitter's already-read scrollLeft so
   * the per-event loop never re-reads it (iPhone reflow path).
   * Also updates the reactive `viewportState` ref that drives
   * the row + per-row event virtualisation computeds — single
   * tick reads both axes once and feeds everything that needs
   * scroll snapshots. */
  onTick: (state) => {
    viewportState.value = state
    boxPin.applyVisible(state.scrollPos)
  },
})
...
<template>
  <div
    ...
    :class="{
      ...
      'epg-timeline--dark-channels': darkChannelBackground,
    }"
    :style="{
  ...
  >
    ...
    <div class="epg-timeline__axis-row">
      <div class="epg-timeline__corner">
        <span class="epg-timeline__corner-label">{{ t('Channel') }}</span>
      </div>
    ...
    <!-- Rows + cursor anchor. Only rows whose y-range
         intersects the visible viewport (plus an overscan
         buffer) are mounted; the top + bottom spacer divs
         preserve the scroll-track height so the channel
         scrollbar still behaves correctly. -->
    <div class="epg-timeline__rows">
      <div
        v-if="topSpacerPx > 0"
        class="epg-timeline__row-spacer"
...
.epg-timeline__channel-icon {
  ...
  object-fit: contain;
  flex: 0 0 auto;
  /* No background. Many channel logos ship as PNGs with
 ...
 * the sticky channel column to a fixed dark surface with light
 * text so white-on-transparent channel logos stay readable on
 * every theme.
 *
 * The dark colour is hardcoded rather than a theme token because
 * the WHOLE POINT is to give Light-theme users a dark surface
 * for their channel logos. A theme-relative token would mean
 * Light → slightly-darker-light, which doesn't solve the
 * readability problem. Access (dark) theme already has a near-
 * black surface so the toggle is a no-op there visually.
 *
 * Hover + focus rules re-tint from the dark base so the existing
 * `--tvh-hover-strength` colour-mix doesn't render near-invisible
 * (the existing rule mixes the primary colour into the surface;
 * mixing into white instead gives the expected ~10 % lift on a
 * dark base).
 */
.epg-timeline--dark-channels .epg-timeline__channel-cell {
  background: #1a1a1a;
  color: #fff;
}

.epg-timeline--dark-channels .epg-timeline__channel-name,
.epg-timeline--dark-channels .epg-timeline__channel-number {
  color: #fff;
}

.epg-timeline--dark-channels .epg-timeline__channel-cell:hover {
