...
/*
 * IdnodePickEntityDialog unit tests. Mocks api/client to control
 * the grid fetch and asserts the entity-picker-specific shape:
 * grid endpoint with start/limit, displayField rendering, pick
 * emit payload (uuid + label), uuid sanity. Common shape tests
 * (visible-gate, cancel, empty, error) live in
 * __helpers__/idnodePickDialogTestUtils.
 */
...
describe('IdnodePickEntityDialog', () => {
  runCommonPickDialogTests({
    ...
    validEntry: { uuid: 'u', networkname: 'n' },
  })
