 ...
 *
 */
int socket_set_dscp(int sockfd,
    uint32_t            dscp,
    char*               errbuf,
    size_t              errbufsize) {
  int r, v;
...
int ip_check_is_local_address(const struct sockaddr_storage* peer,
  ...
  for (ifdev = iflist, ret = 0; ifdev && ret == 0; ifdev = ifdev->ifa_next) {
    ...
    if (!ifaddr || !ifnetmask)
      continue;
    if (ifaddr->ss_family != local->ss_family)
      continue;
    if (!any_address && !ip_check_equal(ifaddr, local))
      continue;
...
again:
  ...
  if (rai == NULL) {
    if (errbuf[0] == '\0')
      snprintf(errbuf,
          errbufsize,
          "Invalid or unresolved hostname '%s'",
          hostname);
    goto error;
  }
  ...
  if (bindip.ss_family != AF_UNSPEC) {
    if (bind(fd, (struct sockaddr*)&bindip, IP_IN_ADDRLEN(bindip)) < 0) {
      snprintf(errbuf,
          errbufsize,
          "Cannot bind to IPv%s addr '%s:%i': %s",
          bindip.ss_family == AF_INET6 ? "6" : "4",
          bindaddr,
          htons(IP_PORT(bindip)),
          strerror(errno));
      goto error;
    }
  }
 ...
 *
 */
struct sockaddr_storage* tcp_get_ip_from_str(const char* src,
 ...
 *
 */
uint32_t tcp_connection_count(access_t* aa) {
...
try_again:
  ...
  if (aa->aa_conn_limit || aa->aa_conn_limit_streaming) {
    ...
    c2 =
        aa->aa_conn_limit_streaming ? sused >= aa->aa_conn_limit_streaming : -1;
  ...
  res->representative =
      aa->aa_representative ? strdup(aa->aa_representative) : NULL;
  res->status    = status;
 ...
 *
 */
static void* tcp_server_start(void* aux) {
...
static void* tcp_server_loop(void* aux) {
  ...
  while (atomic_get(&tcp_server_running)) {
    ...
    if (ev.events & TVHPOLL_IN) {
      ...
      tsl->fd = accept(ts->serverfd, (struct sockaddr*)&tsl->peer, &slen);
      if (tsl->fd == -1) {
        perror("accept");
        free(tsl);
        sleep(1);
...
void tcp_server_done(void) {
  ...
  while (LIST_FIRST(&tcp_server_active) != NULL) {
    if (t + sec2mono(5) < mclk())
      tvhtrace(LS_TCP,
          "tcp server %p active too long",
          LIST_FIRST(&tcp_server_active));
    tvh_mutex_unlock(&global_lock);
