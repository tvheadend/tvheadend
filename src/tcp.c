 ...
 *
 */
struct sockaddr_storage* tcp_get_ip_from_str(const char* src, struct sockaddr_storage* sa) {
  if (sa == NULL || src == NULL)
    return NULL;
  ...
  for (i = 0; i < sd_fds_num && !found; i++) {
    ...
    switch (bound.ss_family) {
      case AF_INET:
        ...
        if (addr4.s_addr == s_addr4->sin_addr.s_addr && htons(port) == s_addr4->sin_port)
          found = 1;
        break;
      case AF_INET6:
