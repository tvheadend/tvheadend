...
static inline int avahi_required(void) {
  ...
  if (avahi_entry_group_is_empty(group)) {
    ...
    if (tvheadend_htsp_port > 0) {
      if ((ret = avahi_entry_group_add_service(group,
        ...
        tvherror(LS_AVAHI, "Failed to add _htsp._tcp service: %s", avahi_strerror(ret));
        goto fail;
      }
    }
    ...
    if (tvheadend_webui_port > 0) {
      ...
      if ((ret = avahi_entry_group_add_service(group,
        ...
        tvherror(LS_AVAHI, "Failed to add _http._tcp service: %s", avahi_strerror(ret));
        goto fail;
      }
    }
    ...
    if ((ret = avahi_entry_group_commit(group)) < 0) {
      tvherror(LS_AVAHI, "Failed to commit entry group: %s", avahi_strerror(ret));
      goto fail;
    }
  }
 ...
 *
 */
static void* avahi_thread(void* aux) {
  AvahiClient* ac;
  ...
  do {
    ...
    if (ac) {
      while ((avahi_do_restart == 0) && (avahi_simple_poll_iterate(avahi_asp, -1) == 0))
        ;
