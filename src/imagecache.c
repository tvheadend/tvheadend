...
typedef struct imagecache_image {
  ...
  enum { IDLE, SAVE, QUEUED, FETCHING } state; ///< save/fetch status
...
const idclass_t imagecache_class = {.ic_snode = (idnode_t*)&imagecache_conf,
    .ic_class                                 = "imagecache",
...
static void imagecache_incref(imagecache_image_t* img) {
  lock_assert(&imagecache_lock);
  assert(img->ref > 0);
  img->ref++;
}

static void imagecache_decref(imagecache_image_t* img) {
  lock_assert(&imagecache_lock);
  if (--img->ref == 0) {
    free((void*)img->url);
    free(img);
...
static inline int sha1_empty(const uint8_t* sha1) {
  int i;
  for (i = 0; i < 20; i++)
    if (sha1[i])
      return 0;
...
static int url_cmp(imagecache_image_t* a, imagecache_image_t* b) {
  return strcmp(a->url, b->url);
}

static int id_cmp(imagecache_image_t* a, imagecache_image_t* b) {
  return (a->id - b->id);
}

static htsmsg_t* imagecache_image_htsmsg(imagecache_image_t* img) {
  char      hex[41];
...
static void imagecache_image_save(imagecache_image_t* img) {
  img->savepend = 1;
...
static void imagecache_new_id(imagecache_image_t* i) {
...
static void imagecache_image_add(imagecache_image_t* img) {
...
static int imagecache_update_sha1(imagecache_image_t* img, const char* path) {
  int     fd;
  uint8_t sha1[20];
  sbuf_t  sb;
...
static int imagecache_new_contents(imagecache_image_t* img,
    const char*                                        tpath,
    char*                                              path,
    const uint8_t*                                     data,
    size_t                                             dsize) {
...
static int imagecache_image_fetch(imagecache_image_t* img) {
  int             res = 1, r;
  url_t           url;
  ...
  /* Open file  */
  if (hts_settings_buildpath(path, sizeof(path), "imagecache/data/%d", img->id))
    goto error;
  ...
  hc = http_client_connect(NULL, HTTP_VERSION_1_1, url.scheme, url.host, url.port, NULL);
  if (hc == NULL)
    goto error_lock;
  ...
  while (tvheadend_is_running()) {
    ...
    if (r == HTTP_CON_DONE) {
      if (hc->hc_code == HTTP_STATUS_OK && hc->hc_data_size > 0)
        res = imagecache_new_contents(img, tpath, path, (uint8_t*)hc->hc_data, hc->hc_data_size);
      break;
    }
  }
...
static void imagecache_timer(void) {
  time_t              now, when;
  ...
  for (img = RB_FIRST(&imagecache_by_url); img; img = img_next) {
    ...
    if (img->state != IDLE)
      continue;
    when = img->failed ? imagecache_conf.fail_period : imagecache_conf.ok_period;
...
static void* imagecache_thread(void* p) {
  ...
  if ((m = hts_settings_load("imagecache/meta"))) {
    HTSMSG_FOREACH(f, m) {
      if (!(e = htsmsg_get_map_by_field(f)))
        continue;
      if (!(id = atoi(htsmsg_field_name(f))))
        continue;
      if (!(url = htsmsg_get_str(e, "url")))
        continue;
