  ...
  while (tvheadend_is_running()) {
    ...
    if (r == HTTP_CON_DONE) {
      if (hc->hc_code == HTTP_STATUS_OK && hc->hc_data_size > 0)
        res = imagecache_new_contents(img,
            tpath,
            path,
            (uint8_t*)hc->hc_data,
            hc->hc_data_size);
...
static void imagecache_timer(void) {
  ...
  for (img = RB_FIRST(&imagecache_by_url); img; img = img_next) {
    ...
    if (img->state != IDLE)
      continue;
    when =
        img->failed ? imagecache_conf.fail_period : imagecache_conf.ok_period;
...
void imagecache_init(void) {
  ...
  /* Start threads */
  tvh_thread_create(&imagecache_tid,
      NULL,
      imagecache_thread,
      NULL,
      "imagecache");
...
void imagecache_clean(void) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (img)
      continue;
    tvhinfo(LS_IMAGECACHE,
        "clean: removing unassociated file '%s/%s'",
        path,
        name);
